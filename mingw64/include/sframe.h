...
#define SFRAME_V2_F_ALL_FLAGS                                                  \
  (SFRAME_F_FDE_SORTED | SFRAME_F_FRAME_POINTER | SFRAME_F_FDE_FUNC_START_PCREL)
...
#define SFRAME_V1_FUNC_INFO(fde_type, fre_enc_type)                            \
  (((SFRAME_AARCH64_PAUTH_KEY_A & 0x1) << 5) | (((fde_type) & 0x1) << 4) |     \
   ((fre_enc_type) & 0xf))
...
typedef struct sframe_fre_info {
  /* Information about
     ...
     ----------------------------------------------------------------------------------
     | Mangled-RA (aarch64) | Size of Data Words |  Number of Data Words  |
     base_reg | | Unused (amd64, s390x)|                    | |            |
     ----------------------------------------------------------------------------------
     8                     7                    5                        1 0
...
#define SFRAME_V2_FRE_RA_UNDEFINED_P(data)                                     \
  (SFRAME_V1_FRE_OFFSET_COUNT(data) == 0)
...
/* In SFrame V3, with the introduction of flexible FDE type
   ...
   bits.  */
#define SFRAME_V3_FRE_DATAWORD_COUNT(data) SFRAME_V1_FRE_OFFSET_COUNT(data)
#define SFRAME_V3_FRE_DATAWORD_SIZE(data) SFRAME_V1_FRE_OFFSET_SIZE(data)
...
#define SFRAME_FRE_TYPE_ADDR1_LIMIT (1ULL << ((SFRAME_FRE_TYPE_ADDR1 + 1) * 8))
...
#define SFRAME_FRE_TYPE_ADDR2_LIMIT (1ULL << ((SFRAME_FRE_TYPE_ADDR2 * 2) * 8))
...
#define SFRAME_FRE_TYPE_ADDR4_LIMIT (1ULL << ((SFRAME_FRE_TYPE_ADDR4 * 2) * 8))
...
#define SFRAME_V2_S390X_OFFSET_ENCODE_REGNUM(regnum) (((regnum) << 1) | 1)
