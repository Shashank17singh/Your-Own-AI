...
using __are_random_access_iterators = std::bool_constant<(
    __is_random_access_iter<std::remove_cvref_t<_IteratorTypes>> && ...)>;
#else
template <typename... _IteratorTypes>
using __are_random_access_iterators = std::__and_<std::is_base_of<
...
using __tag_type = typename std::conditional<
    __internal::__are_random_access_iterators<_IteratorTypes...>::value,
...
template <class... _IteratorTypes>
__tag_type<__internal::__are_random_access_iterators<_IteratorTypes...>,
           _IteratorTypes...>
__select_backend(__pstl::execution::parallel_unsequenced_policy,
                 _IteratorTypes &&...) {
  return {};
}
