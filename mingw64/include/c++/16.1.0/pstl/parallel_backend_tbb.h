...
void __parallel_for(__pstl::__internal::__tbb_backend_tag, _ExecutionPolicy &&,
                    _Index __first, _Index __last, _Fp __f) {
...
_Value __parallel_reduce(__pstl::__internal::__tbb_backend_tag,
                         _ExecutionPolicy &&, _Index __first, _Index __last,
...
template <class _ExecutionPolicy, class _Index, class _Up, class _Tp, class _Cp,
          class _Rp>
_Tp __parallel_transform_reduce(__pstl::__internal::__tbb_backend_tag,
                                _ExecutionPolicy &&, _Index __first,
                                _Index __last, _Up __u, _Tp __init,
                                _Cp __combine, _Rp __brick_reduce) {
  __tbb_backend::__par_trans_red_body<_Index, _Up, _Tp, _Cp, _Rp> __body(
      __u, __init, __combine, __brick_reduce);
  // The grain size of 3 is used in order to provide mininum 2 elements for each
  // body
...
public:
  __trans_scan_body(_Up __u, _Tp __init, _Cp __combine, _Rp __reduce,
                    _Sp __scan)
      : _M_brick_reduce(__reduce), _M_u(__u), _M_combine(__combine),
        _M_scan(__scan), _M_has_sum(true) {
    new (_M_sum_storage) _Tp(__init);
  }

  __trans_scan_body(__trans_scan_body &__b, tbb::split)
      : _M_brick_reduce(__b._M_brick_reduce), _M_u(__b._M_u),
        _M_combine(__b._M_combine), _M_scan(__b._M_scan), _M_has_sum(false) {}
...
void __upsweep(_Index __i, _Index __m, _Index __tilesize, _Tp *__r,
  ...
  if (__m == 1)
    ...
    tbb::parallel_invoke(
        [=] {
          __tbb_backend::__upsweep(__i, __k, __tilesize, __r, __tilesize,
                                   __reduce, __combine);
        },
        [=] {
          __tbb_backend::__upsweep(__i + __k, __m - __k, __tilesize, __r + __k,
...
void __downsweep(_Index __i, _Index __m, _Index __tilesize, _Tp *__r,
  ...
  if (__m == 1)
    ...
    tbb::parallel_invoke(
        [=] {
          __tbb_backend::__downsweep(__i, __k, __tilesize, __r, __tilesize,
                                     __initial, __combine, __scan);
        },
        // Assumes that __combine never throws.
        // TODO: Consider adding a requirement for user functors to be constant.
        [=, &__combine] {
          __tbb_backend::__downsweep(
...
void __parallel_strict_scan(__pstl::__internal::__tbb_backend_tag,
                            _ExecutionPolicy &&, _Index __n, _Tp __initial,
  ...
  tbb::this_task_arena::isolate([=, &__combine]() {
    if (__n > 1) {
      ...
      _Tp *__r = __buf.get();
      __tbb_backend::__upsweep(_Index(0), _Index(__m + 1), __tilesize, __r,
      ...
      __tbb_backend::__downsweep(_Index(0), _Index(__m + 1), __tilesize, __r,
...
template <class _ExecutionPolicy, class _Index, class _Up, class _Tp, class _Cp,
          class _Rp, class _Sp>
_Tp __parallel_transform_scan(__pstl::__internal::__tbb_backend_tag,
                              _ExecutionPolicy &&, _Index __n, _Up __u,
...
template <typename _Func> class __func_task;
template <typename _Func> class __root_task;

#if TBB_INTERFACE_VERSION <= 12000
class __task : public tbb::task {
public:
  template <typename _Fn> __task *make_continuation(_Fn &&__f) {
    return new (allocate_continuation())
        __func_task<typename std::decay<_Fn>::type>(std::forward<_Fn>(__f));
  }

  template <typename _Fn> __task *make_child_of(__task *parent, _Fn &&__f) {
    return new (parent->allocate_child())
        __func_task<typename std::decay<_Fn>::type>(std::forward<_Fn>(__f));
  }

  template <typename _Fn>
  __task *make_additional_child_of(tbb::task *parent, _Fn &&__f) {
    return new (tbb::task::allocate_additional_child_of(*parent))
        __func_task<typename std::decay<_Fn>::type>(std::forward<_Fn>(__f));
  }

  inline void recycle_as_continuation() {
    tbb::task::recycle_as_continuation();
  }

  inline void recycle_as_child_of(__task *parent) {
    tbb::task::recycle_as_child_of(*parent);
  }

  inline void spawn(__task *__t) { tbb::task::spawn(*__t); }

  template <typename _Fn>
  static inline void spawn_root_and_wait(__root_task<_Fn> &__root) {
    tbb::task::spawn_root_and_wait(*__root._M_task);
  }
};

template <typename _Func> class __func_task : public __task {
  _Func _M_func;

  tbb::task *execute() { return _M_func(this); };

public:
  template <typename _Fn>
  __func_task(_Fn &&__f) : _M_func{std::forward<_Fn>(__f)} {}

  _Func &body() { return _M_func; }
};
...
public:
  ...
  __root_task(Args &&...args)
      : _M_task{new (tbb::task::allocate_root())
                    __func_task<_Func>{_Func(std::forward<Args>(args)...)}} {}
...
protected:
  ...
  template <typename _Fn> __task *allocate_func_task(_Fn &&__f) {
    _PSTL_ASSERT(_M_execute_data != nullptr);
    tbb::detail::d1::small_object_allocator __alloc{};
    auto __t = __alloc.new_object<__func_task<typename std::decay<_Fn>::type>>(
        *_M_execute_data, std::forward<_Fn>(__f));
    __t->_M_allocator = __alloc;
    return __t;
  }

public:
  __task *parent() { return _M_parent; }

  void set_ref_count(int __n) {
  ...
  template <typename _Fn> __task *make_child_of(__task *__parent, _Fn &&__f) {
    auto __t = allocate_func_task(std::forward<_Fn &&>(__f));
  ...
  template <typename _Fn>
  __task *make_additional_child_of(__task *__parent, _Fn &&__f) {
  ...
  inline void recycle_as_continuation() { _M_recycle = true; }
  ...
  template <typename _Fn>
  static inline void spawn_root_and_wait(__root_task<_Fn> &__root) {
    tbb::detail::d1::execute_and_wait(*__root._M_func_task, __root._M_context,
                                      __root._M_wait_object, __root._M_context);
  }

  template <typename _Func> friend class __func_task;
};

template <typename _Func> class __func_task : public __task {
  _Func _M_func;

  __task *execute(tbb::detail::d1::execution_data &__ed) override {
  ...
  __task *cancel(tbb::detail::d1::execution_data &__ed) override {
    return finalize(nullptr);
  }

  __task *finalize(__task *__next) {
    ...
    // Placing the deallocation after the refcount decrement allows another
    // thread to proceed with tree folding concurrently with this task cleanup.
...
template <typename _Func> class __root_task : public __task {
  __task *execute(tbb::detail::d1::execution_data &__ed) override {
...
public:
  template <typename... Args> __root_task(Args &&...args) : _M_wait_object{1} {
    tbb::detail::d1::small_object_allocator __alloc{};
    _M_func_task = __alloc.new_object<__func_task<_Func>>(
...
class __merge_func {
  ...
  typedef typename std::common_type<_DifferenceType1, _DifferenceType2>::type
      _SizeType;
  typedef typename std::iterator_traits<_RandomAccessIterator1>::value_type
      _ValueType;
  ...
  _SizeType
      _M_nsort; // number of elements to be sorted for partial_sort alforithm

  static const _SizeType __merge_cut_off = _PSTL_MERGE_CUT_OFF;

  bool _root;   // means a task is merging root task
  bool _x_orig; //"true" means X(or left ) subrange is in the original
                //container; false - in the buffer
  bool _y_orig; //"true" means Y(or right) subrange is in the original
                //container; false - in the buffer
  bool _split; //"true" means a merge task is a split task for parallel merging,
               //the execution logic differs

  bool is_partial() const { return _M_nsort > 0; }

  struct __move_value {
    template <typename Iterator1, typename Iterator2>
    void operator()(Iterator1 __x, Iterator2 __z) {
      *__z = std::move(*__x);
    }
  };
  ...
  struct __move_range {
    template <typename Iterator1, typename Iterator2>
    Iterator2 operator()(Iterator1 __first1, Iterator1 __last1,
                         Iterator2 __first2) {
  ...
  struct __cleanup_range {
    ...
    void operator()(_Iterator __first, _Iterator __last) {
...
public:
  __merge_func(_SizeType __xs, _SizeType __xe, _SizeType __ys, _SizeType __ye,
               _SizeType __zs, _Compare __comp, _Cleanup,
               _LeafMerge __leaf_merge, _SizeType __nsort,
               _RandomAccessIterator1 __x_beg, _RandomAccessIterator2 __z_beg,
               bool __x_orig, bool __y_orig, bool __root)
      : _M_x_beg(__x_beg), _M_z_beg(__z_beg), _M_xs(__xs), _M_xe(__xe),
        _M_ys(__ys), _M_ye(__ye), _M_zs(__zs), _M_comp(__comp),
        _M_leaf_merge(__leaf_merge), _M_nsort(__nsort), _root(__root),
        _x_orig(__x_orig), _y_orig(__y_orig), _split(false) {}

  bool is_left(_SizeType __idx) const { return _M_xs == __idx; }

  template <typename IndexType> void set_odd(IndexType __idx, bool __on_off) {
    if (is_left(__idx))
      _x_orig = __on_off;
    else
      _y_orig = __on_off;
  ...
  __task *operator()(__task *__self);

private:
  __merge_func *parent_merge(__task *__self) const {
    return _root ? nullptr
                 : &static_cast<__func_task<__merge_func> *>(__self->parent())
                        ->body();
  }
  bool x_less_y() {
  ...
  void move_x_range() {
    ...
    if (_x_orig)
      __move_range_construct()(_M_x_beg + _M_xs, _M_x_beg + _M_xe,
                               _M_z_beg + _M_zs);
    else {
      __move_range()(_M_z_beg + _M_zs, _M_z_beg + _M_zs + __nx,
                     _M_x_beg + _M_xs);
  ...
  void move_y_range() {
    ...
    if (_y_orig)
      __move_range_construct()(_M_x_beg + _M_ys, _M_x_beg + _M_ye,
                               _M_z_beg + _M_zs + __nx);
    else {
      __move_range()(_M_z_beg + _M_zs + __nx, _M_z_beg + _M_zs + __nx + __ny,
                     _M_x_beg + _M_ys);
      __cleanup_range()(_M_z_beg + _M_zs + __nx,
                        _M_z_beg + _M_zs + __nx + __ny);
    }

    _y_orig = !_y_orig;
  }
  __task *merge_ranges(__task *__self) {
    _PSTL_ASSERT(
        _x_orig ==
        _y_orig); // two merged subrange must be lie into the same buffer
    ...
    // merge to buffer
    if (_x_orig) {
      _M_leaf_merge(_M_x_beg + _M_xs, _M_x_beg + _M_xe, _M_x_beg + _M_ys,
                    _M_x_beg + _M_ye, _M_z_beg + _M_zs, _M_comp,
                    __move_value_construct(), __move_value_construct(),
    ...
    // merge to "origin"
      ...
      _PSTL_ASSERT(is_partial() ||
                   std::is_sorted(_M_z_beg + _M_xs, _M_z_beg + _M_xe, _M_comp));
      _PSTL_ASSERT(is_partial() ||
                   std::is_sorted(_M_z_beg + _M_ys, _M_z_beg + _M_ye, _M_comp));

      const auto __nx = (_M_xe - _M_xs);
      const auto __ny = (_M_ye - _M_ys);

      _M_leaf_merge(_M_z_beg + _M_xs, _M_z_beg + _M_xe, _M_z_beg + _M_ys,
                    _M_z_beg + _M_ye, _M_x_beg + _M_zs, _M_comp, __move_value(),
                    __move_value(), __move_range(), __move_range());
  ...
  __task *process_ranges(__task *__self) {
    _PSTL_ASSERT(_x_orig == _y_orig);
    _PSTL_ASSERT(!_split);

    auto p = parent_merge(__self);

    if (!p) { // root merging task

      // optimization, just for sort algorithm, //{x} <= {y}
      if (!is_partial() && x_less_y()) // we have a solution
      {
        if (!_x_orig) {   // we have to move the solution to the origin
  ...
  __task *split_merging(__task *__self) {
    _PSTL_ASSERT(_x_orig == _y_orig);
    const auto __nx = (_M_xe - _M_xs);
    const auto __ny = (_M_ye - _M_ys);
    ...
    if (__nx < __ny) {
      ...
      if (_x_orig)
        __xm = std::upper_bound(_M_x_beg + _M_xs, _M_x_beg + _M_xe,
                                *(_M_x_beg + __ym), _M_comp) -
               _M_x_beg;
      else
        __xm = std::upper_bound(_M_z_beg + _M_xs, _M_z_beg + _M_xe,
                                *(_M_z_beg + __ym), _M_comp) -
               _M_z_beg;
    } else {
      __xm = _M_xs + __nx / 2;

      if (_y_orig)
        __ym = std::lower_bound(_M_x_beg + _M_ys, _M_x_beg + _M_ye,
                                *(_M_x_beg + __xm), _M_comp) -
               _M_x_beg;
      else
        __ym = std::lower_bound(_M_z_beg + _M_ys, _M_z_beg + _M_ye,
                                *(_M_z_beg + __xm), _M_comp) -
               _M_z_beg;
    }

    auto __zm = _M_zs + ((__xm - _M_xs) + (__ym - _M_ys));
    __merge_func __right_func(__xm, _M_xe, __ym, _M_ye, __zm, _M_comp,
                              _Cleanup(), _M_leaf_merge, _M_nsort, _M_x_beg,
                              _M_z_beg, _x_orig, _y_orig, _root);
    __right_func._split = true;
    auto __merge_task = __self->make_additional_child_of(
        __self->parent(), std::move(__right_func));
...
__task *
__merge_func<_RandomAccessIterator1, _RandomAccessIterator2, __M_Compare,
             _Cleanup, _LeafMerge>::operator()(__task *__self) {
  ...
  // b. General merging of adjacent sub-ranges x and y (with optimization in
  // case of {x} <= {y} )
  ...
  if (!parent_merge(__self)) { // root merge task
    if (_x_orig)
      move_x_range();
    else
      move_y_range();
...
public:
  __stable_sort_func(_RandomAccessIterator1 __xs, _RandomAccessIterator1 __xe,
                     _RandomAccessIterator2 __zs, bool __root, _Compare __comp,
                     _LeafSort __leaf_sort, _SizeType __nsort,
                     _RandomAccessIterator1 __x_beg,
                     _RandomAccessIterator2 __z_beg)
      : _M_xs(__xs), _M_xe(__xe), _M_x_beg(__x_beg), _M_zs(__zs),
        _M_z_beg(__z_beg), _M_comp(__comp), _M_leaf_sort(__leaf_sort),
        _M_root(__root), _M_nsort(__nsort) {}
...
template <typename _RandomAccessIterator1, typename _RandomAccessIterator2,
          typename _Compare, typename _LeafSort>
__task *__stable_sort_func<_RandomAccessIterator1, _RandomAccessIterator2,
                           _Compare, _LeafSort>::operator()(__task *__self) {
  typedef __merge_func<_RandomAccessIterator1, _RandomAccessIterator2, _Compare,
  ...
  _MergeTaskType __m(_MergeTaskType(
      _M_xs - _M_x_beg, __xm - _M_x_beg, __xm - _M_x_beg, _M_xe - _M_x_beg,
  ...
  auto __right = __self->make_child_of(
      __parent, __stable_sort_func(__xm, _M_xe, __zm, false, _M_comp,
                                   _M_leaf_sort, _M_nsort, _M_x_beg, _M_z_beg));
...
template <class _ExecutionPolicy, typename _RandomAccessIterator,
          typename _Compare, typename _LeafSort>
void __parallel_stable_sort(__pstl::__internal::__tbb_backend_tag,
                            _ExecutionPolicy &&, _RandomAccessIterator __xs,
  ...
  tbb::this_task_arena::isolate([=, &__nsort]() {
    ...
    if (__n > __sort_cut_off) {
      ...
      __root_task<__stable_sort_func<_RandomAccessIterator, _ValueType *,
                                     _Compare, _LeafSort>>
          __root{__xs,        __xe,    __buf.get(), true,       __comp,
                 __leaf_sort, __nsort, __xs,        __buf.get()};
...
//------------------------------------------------------------------------
template <typename _RandomAccessIterator1, typename _RandomAccessIterator2,
          typename _RandomAccessIterator3, typename _Compare,
          typename _LeafMerge>
...
public:
  __merge_func_static(_RandomAccessIterator1 __xs, _RandomAccessIterator1 __xe,
      ...
      : _M_xs(__xs), _M_xe(__xe), _M_ys(__ys), _M_ye(__ye), _M_zs(__zs),
        _M_comp(__comp), _M_leaf_merge(__leaf_merge) {}
...
template <typename _RandomAccessIterator1, typename _RandomAccessIterator2,
          ...
          typename _LeafMerge>
__task *__merge_func_static<_RandomAccessIterator1, _RandomAccessIterator2,
                            _RandomAccessIterator3, __M_Compare,
  ...
  auto __right = __self->make_additional_child_of(
      __self->parent(), __merge_func_static(__xm, _M_xe, __ym, _M_ye, __zm,
                                            _M_comp, _M_leaf_merge));
  __self->spawn(__right);
  __self->recycle_as_continuation();
  _M_xe = __xm;
  _M_ye = __ym;
...
void __parallel_merge(__pstl::__internal::__tbb_backend_tag,
  ...
  if (__n <= __merge_cut_off) {
    ...
    tbb::this_task_arena::isolate([=]() {
      typedef __merge_func_static<_RandomAccessIterator1,
                                  _RandomAccessIterator2,
                                  _RandomAccessIterator3, _Compare, _LeafMerge>
          _TaskType;
      __root_task<_TaskType> __root{__xs, __xe,   __ys,        __ye,
...
void __parallel_invoke(__pstl::__internal::__tbb_backend_tag,
                       _ExecutionPolicy &&, _F1 &&__f1, _F2 &&__f2) {
