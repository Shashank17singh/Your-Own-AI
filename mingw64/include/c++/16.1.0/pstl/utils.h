...
template <typename _Fp> auto __except_handler(_Fp __f) -> decltype(__f()) {
  try {
    return __f();
...
template <typename _Fp> void __invoke_if(std::false_type, _Fp) {}

template <typename _Fp> void __invoke_if_not(std::false_type, _Fp __f) {
  __f();
}

template <typename _Fp> void __invoke_if_not(std::true_type, _Fp) {}
