...
namespace simd {
template <unsigned _Np> struct _SwapNeighbors {
  consteval unsigned operator()(unsigned __i, unsigned __size) const {
    if (__size % (2 * _Np) != 0)
      __builtin_abort(); // swap_neighbors<N> permutation requires a multiple of
                         // 2N elements
...
constexpr auto __bitset_split(const bitset<_Mp> &__b) {
  ...
  if constexpr (_Np % __bits_per_word == 0) {
    ...
    return _Tmp{static_cast<_Lo>(__b.to_ullong()),
                static_cast<_Hi>(__b.to_ullong() >> _Np)};
  }
}
...
/**
 * Computes a member @c type `basic_vec<_Tp, Abi>`, where @c Abi is chosen such
 * that the number of elements is equal to `_Vp::size()` and features of the ABI
 * tag (such as the internal representation of masks, or storage order of
 * complex components) are preserved.
 */
...
struct resize<_Np, _Vp, _Traits> {
  using type =
      __similar_vec<typename _Vp::value_type, _Np, typename _Vp::abi_type>;
};
...
struct resize<_Np, _Mp, _Traits> {
  using _A1 = decltype(__abi_rebind<__mask_element_size<_Mp>, _Np,
                                    typename _Mp::abi_type, true>());

  static_assert(__abi_tag<_A1>);

  static_assert(_Mp::abi_type::_S_variant == _A1::_S_variant ||
                __scalar_abi_tag<_A1> ||
...
permute(const _Vp &__v, _IdxMap &&__idxmap) {
  return resize_t < _Np == 0 ? _Vp::size() : _Np,
         _Vp > ::_S_static_permute(__v, __idxmap);
}

// [simd.permute.dynamic]
template <__simd_vec_or_mask_type _Vp, __simd_integral _Ip>
[[__gnu__::__always_inline__]]
constexpr resize_t<_Ip::size(), _Vp> permute(const _Vp &__v,
                                             const _Ip &__indices) {
...
[[__gnu__::__always_inline__]]
constexpr auto chunk(const basic_vec<_Tp, _Ap> &__x) noexcept
...
[[__gnu__::__always_inline__]]
constexpr auto chunk(const basic_mask<_Bytes, _Ap> &__x) noexcept
...
cat(const basic_vec<_Tp, _A0> &__x0,
    const basic_vec<_Tp, _Abis> &...__xs) noexcept {
  return resize_t<(_A0::_S_size + ... + _Abis::_S_size),
                  basic_vec<_Tp, _A0>>::_S_concat(__x0, __xs...);
}

// LWG???? (reported 2025-11-25)
template <size_t _Bytes, typename _A0, typename... _Abis>
constexpr resize_t<(_A0::_S_size + ... + _Abis::_S_size),
                   basic_mask<_Bytes, _A0>>
cat(const basic_mask<_Bytes, _A0> &__x0,
    const basic_mask<_Bytes, _Abis> &...__xs) noexcept {
  return resize_t<(_A0::_S_size + ... + _Abis::_S_size),
                  basic_mask<_Bytes, _A0>>::_S_concat(__x0, __xs...);
}

// implementation helper for chunk and cat
consteval int __packs_to_skip_at_front(int __offset,
                                       initializer_list<int> __sizes) {
  int __i = 0;
  int __n = 0;
  for (int __s : __sizes) {
    __n += __s;
...
consteval int __packs_to_skip_at_back(int __offset, int __max,
                                      initializer_list<int> __sizes) {
  int __i = 0;
  int __n = -__offset;
  for (int __s : __sizes) {
...
// in principle, this overload allows conversions to _Dst - and it wouldn't be
// wrong - but the general overload below is still a better candidate in
// overload resolution
template <typename _Dst>
[[__gnu__::__always_inline__]]
constexpr _Dst __extract_simd_at(auto _Offset, const _Dst &__r, const auto &...)
...
[[__gnu__::__always_inline__]]
constexpr _Dst __extract_simd_at(auto _Offset, const _V0 &, const _Dst &__r,
                                 const auto &...)
...
[[__gnu__::__always_inline__]]
constexpr _Dst __extract_simd_at(auto _Offset, const _Vs &...__xs) {
  ...
  if constexpr (_Adst::_S_nreg >= 2) {
    ...
    return _Dst::_S_init(
        __extract_simd_at<_Dst0>(_Offset, __xs...),
        __extract_simd_at<_Dst1>(_Offset + _Dst0::size, __xs...));
  } else {
    using _Ret = remove_cvref_t<decltype(declval<_Dst>()._M_get())>;
    constexpr bool __use_bitmask =
        __simd_mask_type<_Dst> && _Adst::_S_is_bitmask;
    ...
    if constexpr (_Offset.value >= _Afirst::_S_size ||
                  __ninputs - _Offset.value - _Alast::_S_size >=
                      _Adst::_S_size) { // can drop inputs at the front and/or
                                        // back of the pack
      constexpr int __skip_front =
          __packs_to_skip_at_front(_Offset.value, {_Vs::size.value...});
      constexpr int __skip_back = __packs_to_skip_at_back(
          _Offset.value, _Adst::_S_size, {_Vs::size.value...});
      static_assert(__skip_front > 0 || __skip_back > 0);
      constexpr auto [... __skip] = _IotaArray<__skip_front>;
      constexpr auto [... __is] =
          _IotaArray<__nargs - __skip_front - __skip_back>;
      constexpr int __new_offset =
          _Offset.value - (0 + ... + _Vs...[__skip] ::size.value);
      return __extract_simd_at<_Dst>(cw<__new_offset>,
                                     __xs...[__is + __skip_front]...);
    } else if constexpr (_Adst::_S_size ==
                         1) { // trivial conversion to one value_type
      return _Dst(__x0[_Offset.value]);
    } else if constexpr (_Afirst::_S_nreg >= 2 ||
                         _Alast::_S_nreg >= 2) { // flatten first and/or last
      ...
      constexpr auto [... __is] =
          _IotaArray<__nargs - __flatten_first - __flatten_last>;
      if constexpr (__flatten_first && __flatten_last)
        return __extract_simd_at<_Dst>(
            _Offset, __x0._M_get_low(), __x0._M_get_high(),
            __xs...[__is + 1]..., __xlast._M_get_low(), __xlast._M_get_high());
      else if constexpr (__flatten_first)
        return __extract_simd_at<_Dst>(_Offset, __x0._M_get_low(),
                                       __x0._M_get_high(),
                                       __xs...[__is + 1]...);
      else
        return __extract_simd_at<_Dst>(_Offset, __xs...[__is]...,
                                       __xlast._M_get_low(),
                                       __xlast._M_get_high());
      ...
      return __extract_simd_at<_Dst>(
          _Offset,
          static_cast<const resize_t<_Vs::size.value, _Dst> &>(__xs)...);
    }

    // at this point __xs should be as small as possible; there may be some
    // corner cases left

    else if constexpr (__nargs == 1) { // simple and optimal
    ...
    } else if constexpr (__use_bitmask) { // fairly simple and optimal bit
                                          // shifting solution
      ...
      return _Dst(__r);
                         ...
                         __has_single_bit(unsigned(
                             _Afirst::_S_size))) { // simple __vec_concat
      if constexpr (_Afirst::_S_size == 1)
        ...
        const auto __v1 =
            __vec_zero_pad_to<sizeof(__v0)>(__xlast._M_concat_data());
        return __vec_concat(__v0, __v1);
      }
    } else if constexpr (__nargs == 2 && _Adst::_S_nreg == 1 && _Offset == 0 &&
                         _Afirst::_S_nreg == 1 &&
                         _Alast::_S_size == 1) { // optimize insertion of one
                                                 // element at the end
      ...
      return __r;
                         ...
                         _Afirst::_S_nreg == 1 &&
                         _Alast::_S_size == 2) { // optimize insertion of two
                                                 // elements at the end
      _Ret __r = __vec_zero_pad_to<sizeof(_Ret)>(__x0._M_concat_data());
      const auto __x1 = __xlast._M_concat_data();
      if constexpr (sizeof(__x1) <= sizeof(double) &&
                    (_Afirst::_S_size & 1) ==
                        0) { // can use a single insert instruction
      ...
      return __r;
    } else if constexpr (__nargs == 2 && _Afirst::_S_nreg == 1 &&
                         _Alast::_S_nreg == 1) { // optimize concat of two input
                                                 // vectors (e.g. using palignr)
      constexpr auto [... __is] = _IotaArray<__dst_full_size>;
      constexpr int __v2_offset = __width_of<decltype(__x0._M_concat_data())>;
      return __builtin_shufflevector(__x0._M_concat_data(),
                                     __xlast._M_concat_data(),
                                     [](int __i) consteval {
    ...
    } else if (__is_const_known(__xs...) ||
               __ninputs ==
                   _Adst::_S_size) { // hard to optimize for the compiler, but
...
// [simd.mask] --------------------------------------------------------------
template <size_t _Bytes, typename _Ap> class basic_mask {
public:
...
template <size_t _Bytes, typename _Ap> class _MaskBase {
...
public:
  ...
  constexpr iterator begin() noexcept { return {static_cast<_Mp &>(*this), 0}; }
  ...
  explicit _MaskBase(const basic_mask<_UBytes, _UAbi> &) =
      delete ("size mismatch");

  template <typename _Up, typename _UAbi>
  explicit _MaskBase(const basic_vec<_Up, _UAbi> &) =
      delete ("use operator! or a comparison to convert a vec into a mask");

  template <typename _Up, typename _UAbi>
    requires(_Ap::_S_size != _UAbi::_S_size)
  operator basic_vec<_Up, _UAbi>() const = delete ("size mismatch");
};
...
public:
  ...
  [[__gnu__::__always_inline__]]
  static constexpr basic_mask _S_init(_DataType __x) {
    basic_mask __r;
    __r._M_data = __x;
    return __r;
  }
  ...
  [[__gnu__::__always_inline__]]
  static constexpr basic_mask _S_init(unsigned_integral auto __bits) {
   ...
   *
   * This is necessary for _S_nreg > 1 where the last element can be bool or
   * when the sizeof doesn't match because of different alignment requirements
   * of the sub-masks.
   */
  template <size_t _UBytes, typename _UAbi>
  [[__gnu__::__always_inline__]]
  ...
  [[__gnu__::__always_inline__]]
  constexpr auto _M_concat_data(bool __do_sanitize = _S_is_partial) const {
    if constexpr (_S_is_scalar)
      return __vec_builtin_type<__integer_from<_Bytes>, 1>{
          __integer_from<_Bytes>(-_M_data)};
    else {
  ...
  /** @internal
   * Returns a mask where the first @p __n elements are true. All remaining
   * elements are false.
   *
   * @pre @p __n > 0 && @p __n < _S_size
   */
  template <_ArchTraits _Traits = {}>
  [[__gnu__::__always_inline__]] static constexpr basic_mask
    ...
    if constexpr (!_S_use_bitmask) {
      ...
      __glibcxx_simd_precondition(
          __n >= 0 && __n <= numeric_limits<_Ip>::max(),
      ...
      if constexpr (_S_size <= 32 && _Traits._M_have_bmi2())
        ...
        return _S_init(
            __builtin_ia32_bzhi_di(~0ull >> (64 - _S_size), unsigned(__n)));
#endif
  ...
  [[__gnu__::__always_inline__]]
  constexpr basic_mask &_M_and_neighbors() {
    if constexpr (_S_use_bitmask)
  ...
  [[__gnu__::__always_inline__]]
  constexpr basic_mask &_M_or_neighbors() {
    if constexpr (_S_use_bitmask)
      _M_data |= ((_M_data >> 1) & 0x5555'5555'5555'5555ull) |
                 ((_M_data << 1) & ~0x5555'5555'5555'5555ull);
    else
  ...
  constexpr auto _M_chunk() const noexcept {
    ...
    if constexpr (__rem == 0)
      return array<_Mp, __n>{
          __extract_simd_at<_Mp>(cw<_Mp::_S_size * __is>, *this)...};
    else {
  ...
  [[__gnu__::__always_inline__]]
  constexpr basic_mask(_DataType __x)
    requires(!_S_is_scalar && !_S_use_bitmask)
      : _M_data(__x) {}

  [[__gnu__::__always_inline__]]
  constexpr operator _DataType()
    requires(!_S_is_scalar && !_S_use_bitmask)
  {
    return _M_data;
  }
  ...
  [[__gnu__::__always_inline__]]
  constexpr explicit basic_mask(same_as<bool> auto __x) noexcept // LWG 4382.
  ...
  [[__gnu__::__always_inline__]] constexpr explicit(
      __is_mask_conversion_explicit<_Ap, _UAbi>(_Bytes, _UBytes))
  ...
  [[__gnu__::__always_inline__]]
  constexpr explicit basic_mask(_Fp &&__gen)
      : _M_data([&] [[__gnu__::__always_inline__]] {
          constexpr auto [... __is] = _IotaArray<_S_size>;
          if constexpr (_S_is_scalar)
            ...
            return _DataType(
                ((_DataType(__gen(__simd_size_c<__is>)) << __is) | ...));
          else
  ...
  constexpr basic_mask(
      const same_as<bitset<_S_size>> auto &__b) noexcept // LWG 4382.
  ...
  [[__gnu__::__always_inline__]] constexpr explicit basic_mask(
      _Tp __val) noexcept
      : _M_data([&] [[__gnu__::__always_inline__]] () {
          if constexpr (_S_use_bitmask)
            return __val;
            ...
            if constexpr (_S_size < __bits_per_element)
              ...
              if constexpr (sizeof(_DataType) == 32) {
                ...
                __v0 = _VecType::_S_static_permute(
                    __v1, [](int __i) { return __i / __CHAR_BIT__; });
                return ((__v0 & __pow2) != cw<0>)._M_data;
              }
            }
          }
  ...
  // Effects: Initializes the first M elements to the corresponding bit values
  // in val, where M is the smaller of size() and the number of bits in the
  // value representation
  //([basic.types.general]) of the type of val. If M is less than size(), the
  ...
  [[__gnu__::__always_inline__]]
  constexpr _VecType operator+() const noexcept
    requires destructible<_VecType>
  {
    return operator _VecType();
  }

  constexpr _VecType operator+() const noexcept = delete;
  ...
  [[__gnu__::__always_inline__]]
  constexpr explicit(sizeof(_Up) !=
                     _Bytes) operator basic_vec<_Up, _UAbi>() const noexcept {
    if constexpr (_S_is_scalar)
      return _Up(_M_data);
    else {
  ...
  // [simd.mask.namedconv] ------------------------------------------------
  [[__gnu__::__always_inline__]]
  constexpr bitset<_S_size> to_bitset() const noexcept {
    // more than 64 elements in one register? not yet.
    static_assert(_S_size <= numeric_limits<unsigned long long>::digits);
    return to_ullong();
  }
   ...
   *
   * @tparam _Offset       Adjust the return type & value to start at bit @p
   * _Offset.
   * @tparam _Use_2_for_1  Store the value of every second element into one bit
   * of the result. (precondition: each even/odd pair stores the same value)
   */
  template <int _Offset = 0, _ArchTraits _Traits = {}>
  [[__gnu__::__always_inline__]] constexpr _Bitmask<_S_size + _Offset>
      _M_to_uint() const {
    constexpr int __nbits = _S_size;
    static_assert(__nbits + _Offset <=
    ...
    if constexpr (_S_is_scalar || _S_use_bitmask) {
      ...
      if (!__is_const_known(*this)) {
        ...
        if constexpr (_Bytes != 2) // movmskb would duplicate each bit
          ...
          return __similar_mask<char, __nbits, _Ap>(*this)
              .template _M_to_uint<_Offset>();
        else
          static_assert(false);
        // TODO: with AVX512 use __builtin_ia32_cvt[bwdq]2mask(128|256|512)
        // TODO: Ask for compiler builtin to do the best of the above. This
        // should also combine with a preceding vector-mask compare to produce a
        // bit-mask compare (on AVX512)
        if constexpr (_S_is_partial)
      ...
      const typename _IV::mask_type &__k =
          [&] [[__gnu__::__always_inline__]] () {
      ...
      if constexpr (_Bytes * __CHAR_BIT__ >=
                    __n) // '1 << __iota' cannot overflow
      {                  // reduce(select(k, powers_of_2, 0))
        constexpr _IV __pow2 = _IV(cw<1>) << __iota<_IV>;
        return _Ur(_U0(__select_impl(__k, __pow2, _IV())._M_reduce(bit_or<>())))
               << _Offset;
      } else if constexpr (__n % __CHAR_BIT__ !=
                           0) { // recurse after splitting in two
        ...
        constexpr _IV __pow2 = _IV(cw<1>)
                               << (__iota<_IV> % _IV(cw<__CHAR_BIT__>));
  ...
  // [simd.mask.binary] ---------------------------------------------------
  [[__gnu__::__always_inline__]]
  friend constexpr basic_mask operator&&(const basic_mask &__x,
                                         const basic_mask &__y) noexcept {
  ...
  // [simd.mask.cassign] --------------------------------------------------
  [[__gnu__::__always_inline__]]
  friend constexpr basic_mask &operator&=(basic_mask &__x,
                                          const basic_mask &__y) noexcept {
  ...
  // [simd.mask.cond] -----------------------------------------------------
  [[__gnu__::__always_inline__]]
  friend constexpr basic_mask __select_impl(const basic_mask &__k,
                                            const basic_mask &__t,
                                            const basic_mask &__f) noexcept {
    if constexpr (!_S_use_bitmask) {
#if _GLIBCXX_X86
      // this works around bad code-gen when the compiler can't see that __k is
      // a vector-mask. This pattern, is recognized to match the x86 blend
  ...
  [[__gnu__::__always_inline__]]
  friend constexpr vec<_T0, _S_size> __select_impl(const basic_mask &__k,
                                                   const _T0 &__t,
                                                   const _T1 &__f) noexcept {
    if constexpr (_S_is_scalar)
  ...
  [[__gnu__::__always_inline__]]
  constexpr bool _M_all_of() const noexcept {
    if constexpr (_S_is_scalar)
      return _M_data;
    else if constexpr (_S_use_bitmask) {
      if constexpr (_S_is_partial)
  ...
  [[__gnu__::__always_inline__]]
  constexpr bool _M_any_of() const noexcept {
    if constexpr (_S_is_scalar)
      return _M_data;
    else if constexpr (_S_use_bitmask) {
      if constexpr (_S_is_partial)
  ...
  [[__gnu__::__always_inline__]]
  constexpr bool _M_none_of() const noexcept {
    if constexpr (_S_is_scalar)
      return !_M_data;
    else if constexpr (_S_use_bitmask) {
      if constexpr (_S_is_partial)
  ...
  [[__gnu__::__always_inline__]]
  constexpr __simd_size_type _M_reduce_count() const noexcept {
    if constexpr (_S_is_scalar)
      return int(_M_data);
  ...
  [[__gnu__::__always_inline__]]
  constexpr __simd_size_type _M_reduce_min_index() const {
    const auto __bits = _M_to_uint();
    __glibcxx_simd_precondition(__bits,
  ...
  [[__gnu__::__always_inline__]]
  constexpr __simd_size_type _M_reduce_max_index() const {
    const auto __bits = _M_to_uint();
    __glibcxx_simd_precondition(__bits,
  ...
  [[__gnu__::__always_inline__]]
  friend constexpr bool __is_const_known(const basic_mask &__x) {
...
class basic_mask<_Bytes, _Ap> : public _MaskBase<_Bytes, _Ap> {
  ...
  // explicitly request _Nreg0 rather than use __abi_rebind. This way _Float16
  // can use half of native registers (since they convert to full float32
  // registers).
  ...
  // _Ap::_S_nreg determines how deep the recursion goes. E.g. basic_mask<4,
  // _Abi<8, 4>> cannot use basic_mask<4, _Abi<4, 1>> as _Mask0/1 types.
  static_assert(_Mask0::abi_type::_S_nreg + _Mask1::abi_type::_S_nreg ==
                _Ap::_S_nreg);
  ...
  static constexpr size_t _S_padding_bytes =
      (__alignof__(_Mask0) == __alignof__(_Mask1)
           ? 0
           : __alignof__(_Mask0) - (sizeof(_Mask1) % __alignof__(_Mask0))) +
...
public:
  ...
  [[__gnu__::__always_inline__]]
  static constexpr basic_mask _S_init(const _Mask0 &__x, const _Mask1 &__y) {
    basic_mask __r;
    __r._M_data0 = __x;
    __r._M_data1 = __y;
    return __r;
  ...
  [[__gnu__::__always_inline__]]
  static constexpr basic_mask _S_init(const __trivial_pair<_U0, _U1> &__bits) {
    if constexpr (is_unsigned_v<_U0>) {
      ...
      return _S_init(_Mask0::_S_init(__bits._M_first),
                     _Mask1(__bits._M_second));
    else
      return _S_init(_Mask0::_S_init(__bits._M_first),
                     _Mask1::_S_init(__bits._M_second));
  }

  [[__gnu__::__always_inline__]]
  constexpr const _Mask0 &_M_get_low() const {
    return _M_data0;
  }

  [[__gnu__::__always_inline__]]
  constexpr const _Mask1 &_M_get_high() const {
    return _M_data1;
  }
  ...
  constexpr auto _M_chunk() const noexcept {
    ...
    if constexpr (__rem == 0)
      ...
      using _Rest = resize_t<__rem, _Mp>;
      return tuple(
          __extract_simd_at<_Mp>(cw<_Mp::_S_size * __is>, _M_data0,
                                 _M_data1)...,
  ...
  [[__gnu__::__always_inline__]]
  static constexpr basic_mask _S_concat(const basic_mask &__x0) noexcept {
    return __x0;
  }
  ...
  constexpr basic_mask(
      const same_as<bitset<_S_size>> auto &__b) noexcept // LWG 4382.
      : _M_data0(__bitset_split<_N0>(__b)._M_lo),
        _M_data1(__bitset_split<_N0>(__b)._M_hi) {}
  ...
  constexpr explicit basic_mask(_Tp __val) noexcept
      : _M_data0(static_cast<_Bitmask<_N0>>(__val)),
        _M_data1(sizeof(_Tp) * __CHAR_BIT__ > _N0
                     ? static_cast<_Bitmask<_N1>>(__val >> _N0)
                     : _Bitmask<_N1>()) {}
  ...
  constexpr value_type operator[](__simd_size_type __i) const {
    ...
    if (__is_const_known(__i))
      ...
      // bit-/vector-mask; e.g. mask<short, 17> is {mask<short, 16>, mask<short,
      // 1>}, where the latter uses _ScalarAbi<1>, which is stored as 'bool'
      return __i < _N0 ? _M_data0[__i] : _M_data1[__i - _N0];
    else if constexpr (abi_type::_S_is_bitmask) {
      using _AliasingByte [[__gnu__::__may_alias__]] = unsigned char;
      return bool(
          (reinterpret_cast<const _AliasingByte *>(this)[__i / __CHAR_BIT__] >>
           (__i % __CHAR_BIT__)) &
          1);
    } else {
  ...
  template <int _Offset = 0, _ArchTraits _Traits = {}>
  [[__gnu__::__always_inline__]] constexpr auto _M_to_uint() const {
    constexpr int _N0x = _N0;
    if constexpr (_N0x >= numeric_limits<unsigned long long>::digits) {
      static_assert(_Offset == 0);
      return __trivial_pair{_M_data0.template _M_to_uint<0>(),
                            _M_data1.template _M_to_uint<0>()};
    } else {
#if _GLIBCXX_X86
      if constexpr (_Bytes == 2 && !_Traits._M_have_bmi2() &&
                    _Ap::_S_nreg == 2 && !_S_use_bitmask)
        return __similar_mask<char, _S_size, _Ap>(*this)
            .template _M_to_uint<_Offset>();
#endif
  ...
  template <_ArchTraits _Traits = {}>
  [[__gnu__::__always_inline__]] constexpr bool _M_all_of() const {
  ...
  template <_ArchTraits _Traits = {}>
  [[__gnu__::__always_inline__]] constexpr bool _M_any_of() const {
    if constexpr (_N0 == _N1)
  ...
  template <_ArchTraits _Traits = {}>
  [[__gnu__::__always_inline__]] constexpr bool _M_none_of() const {
    if constexpr (_N0 == _N1)
