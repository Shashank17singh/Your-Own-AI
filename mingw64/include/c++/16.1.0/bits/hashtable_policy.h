...
template <> struct _NodeBuilder<_Select1st> {
  ...
  static auto _S_build(_Kt &&__k, _Arg &&__arg, _NodeGenerator &__node_gen) ->
      typename _NodeGenerator::__node_ptr {
    return __node_gen(std::forward<_Kt>(__k), std::forward<_Arg>(__arg).second);
  }
};

template <> struct _NodeBuilder<_Identity> {
  template <typename _Kt, typename _Arg, typename _NodeGenerator>
  static auto _S_build(_Kt &&__k, _Arg &&, _NodeGenerator &__node_gen) ->
      typename _NodeGenerator::__node_ptr {
    return __node_gen(std::forward<_Kt>(__k));
  }
};

template <typename _HashtableAlloc, typename _NodePtr> struct _NodePtrGuard {
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wc++17-extensions" // if constexpr
  template <typename _Arg> __node_ptr operator()(_Arg &&__arg) {
    if (!_M_nodes)
...
public:
  ...
  template <typename... _Args> __node_ptr operator()(_Args &&...__args) const {
...
#if __cpp_impl_three_way_comparison >= 201907L
  friend bool operator==(const _Node_iterator &,
                         const _Node_iterator &) = default;
#else
  friend bool operator==(const _Node_iterator &__x,
                         const _Node_iterator &__y) noexcept {
...
private:
  ...
  using __iterator = _Node_iterator<_Value, __constant_iterators, __cache>;
...
public:
...
#if __cpp_impl_three_way_comparison >= 201907L
  friend bool operator==(const _Node_const_iterator &,
                         const _Node_const_iterator &) = default;

  friend bool operator==(const _Node_const_iterator &__x,
  ...
  friend bool operator==(const __iterator &__x,
                         const _Node_const_iterator &__y) noexcept {
    const __base_type &__bx = __x;
    const __base_type &__by = __y;
    return __bx == __by;
...
struct _Mod_range_hashing {
  size_t operator()(size_t __num, size_t __den) const noexcept {
    return __num % __den;
  }
};
...
struct _Prime_rehash_policy {
  ...
  // TODO: 'const' qualifier is kept for abi compatibility reason.
  std::pair<bool, size_t> _M_need_rehash(size_t __n_bkt, size_t __n_elt,
...
template <typename _RehashPolicy> struct _RehashStateGuard {
...
public:
  ...
  template <typename _Kt> mapped_type &_M_at_tr(const _Kt &__k) {
  ...
  template <typename _Kt> const mapped_type &_M_at_tr(const _Kt &__k) const {
...
template <typename _Key, typename _Val, typename _Alloc, typename _Equal,
          typename _Hash, typename _RangeHash, typename _Unused,
          typename _RehashPolicy, typename _Traits>
auto _Map_base<_Key, pair<const _Key, _Val>, _Alloc, _Select1st, _Equal, _Hash,
...
struct _Map_base<const _Key, pair<const _Key, _Val>, _Alloc, _Select1st, _Equal,
 ...
 */
template <typename _Tp, bool __use_ebo = !__is_final(_Tp) && __is_empty(_Tp)>
...
template <typename _Tp> struct _Hashtable_ebo_helper<_Tp, false> {
  _Tp _M_obj;
};
#endif
 ...
 */
template <typename _Key, typename _Value, typename _ExtractKey, typename _Hash,
          typename _RangeHash, typename _Unused, bool __cache_hash_code>
...
public:
  ...
  hasher hash_function() const { return _M_hash._M_obj; }
...
protected:
  ...
  size_t _M_bucket_index(__hash_code __c, size_t __bkt_count) const {
    return _RangeHash{}(__c, __bkt_count);
  }

  size_t _M_bucket_index(const _Hash_node_value<_Value, false> &__n,
                         size_t __bkt_count) const
      noexcept(noexcept(declval<const _Hash &>()(declval<const _Key &>()))) {
    return _RangeHash{}(_M_hash_code(_ExtractKey{}(__n._M_v())), __bkt_count);
  }

  size_t _M_bucket_index(const _Hash_node_value<_Value, true> &__n,
...
protected:
  ...
  _Local_iterator_base(const __hash_code_base &, _Hash_node<_Value, true> *__p,
  ...
  void _M_incr() {
    ...
    if (this->_M_cur) {
      size_t __bkt = _RangeHash{}(this->_M_cur->_M_hash_code, _M_bucket_count);
      if (__bkt != _M_bucket)
        this->_M_cur = nullptr;
    }
  }
...
public:
  size_t _M_get_bucket() const { return _M_bucket; } // for debug mode
};
...
template <typename _Hash> struct _Hash_obj_storage {
...
protected:
  ...
  _Local_iterator_base(const __hash_code_base &__base,
                       _Hash_node<_Value, false> *__p, size_t __bkt,
                       size_t __bkt_count)
  ...
  void _M_init(const _Hash &__h) {
    std::_Construct(std::addressof(__hash_obj_storage::_M_u._M_h), __h);
  }

  void _M_destroy() { __hash_obj_storage::_M_u._M_h.~_Hash(); }

  const _Hash &_M_h() const { return __hash_obj_storage::_M_u._M_h; }
...
/// local iterators
template <typename _Key, typename _Value, typename _ExtractKey, typename _Hash,
          typename _RangeHash, typename _Unused, bool __constant_iterators,
          bool __cache>
struct _Local_iterator
    : public _Local_iterator_base<_Key, _Value, _ExtractKey, _Hash, _RangeHash,
                                  _Unused, __cache> {
private:
...
public:
  ...
  _Local_iterator(const __hash_code_base &__base,
                  _Hash_node<_Value, __cache> *__n, size_t __bkt,
                  size_t __bkt_count)
...
#pragma GCC diagnostic pop
  ...
  const _Equal &_M_eq() const noexcept { return _M_equal._M_obj; }
};
...
public:
  ...
  _Hashtable_alloc(_Alloc &&__a) : _M_alloc{std::forward<_Alloc>(__a)} {}

  __node_alloc_type &_M_node_allocator() { return _M_alloc._M_obj; }

  const __node_alloc_type &_M_node_allocator() const { return _M_alloc._M_obj; }
  ...
  __buckets_ptr _M_allocate_buckets(size_t __bkt_count);

  void _M_deallocate_buckets(__buckets_ptr, size_t __bkt_count);
};
...
template <typename _NodeAlloc>
auto _Hashtable_alloc<_NodeAlloc>::_M_allocate_buckets(size_t __bkt_count)
    -> __buckets_ptr {
...
template <typename _NodeAlloc>
void _Hashtable_alloc<_NodeAlloc>::_M_deallocate_buckets(__buckets_ptr __bkts,
                                                         size_t __bkt_count) {
