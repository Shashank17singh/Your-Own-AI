...
class _File_sink final : _Buf_sink<char> {
  struct _File {
    ...
    std::span<char> _M_write_buf() noexcept {
    ...
    void _M_bump(size_t __n) noexcept { _M_file->_IO_write_ptr += __n; }

    bool _M_line_buffered() const noexcept {
      return __flbf(_M_file);
    } // Or: _M_file->_flags & 0x200

    bool _M_unbuffered() const noexcept {
      return __fbufsize(_M_file) == 1;
    } // Or: _M_file->_flags & 0x2
...
inline void vprint_nonunicode(FILE *__stream, string_view __fmt,
                              format_args __args) {
  std::vformat_to(__format::_File_sink(__stream, false).out(), __fmt, __args);
}

_GLIBCXX_PRINT_INLINE_USED
inline void vprint_nonunicode_buffered(FILE *__stream, string_view __fmt,
                                       format_args __args) {
...
_GLIBCXX_PRINT_INLINE_USED
inline void vprint_unicode(FILE *__stream, string_view __fmt,
                           format_args __args) {
...
_GLIBCXX_PRINT_INLINE_USED
inline void vprint_unicode_buffered(FILE *__stream, string_view __fmt,
                                    format_args __args) {
#if !defined(_WIN32) || defined(__CYGWIN__)
  // For most targets we don't need to do anything special to write
