...
/// @{
template <typename _CharT> inline constexpr bool __is_encoded_char = false;
template <> inline constexpr bool __is_encoded_char<char> = true;
#ifdef _GLIBCXX_USE_CHAR8_T
template <> inline constexpr bool __is_encoded_char<char8_t> = true;
#endif
#if _GLIBCXX_USE_WCHAR_T
template <> inline constexpr bool __is_encoded_char<wchar_t> = true;
#endif
template <> inline constexpr bool __is_encoded_char<char16_t> = true;
template <> inline constexpr bool __is_encoded_char<char32_t> = true;
...
inline constexpr bool __is_path_iter_src<
    _Iter_traits, void_t<typename _Iter_traits::value_type>> =
...
template <> inline constexpr bool __is_path_src<path> = false;

template <> inline constexpr bool __is_path_src<volatile path> = false;

template <> inline constexpr bool __is_path_src<void *> = false;

template <> inline constexpr bool __is_path_src<const void *> = false;

template <> inline constexpr bool __is_path_src<volatile void *> = false;

template <> inline constexpr bool __is_path_src<const volatile void *> = false;

template <typename _CharT, typename _Traits, typename _Alloc>
inline constexpr bool __is_path_src<basic_string<_CharT, _Traits, _Alloc>> =
    __is_encoded_char<_CharT>;

template <typename _CharT, typename _Traits>
inline constexpr bool __is_path_src<basic_string_view<_CharT, _Traits>> =
    __is_encoded_char<_CharT>;

// SFINAE constraint for Source parameters as required by [fs.path.req].
template <typename _Tp> using _Path = enable_if_t<__is_path_src<_Tp>, path>;
...
#else
template <typename _Iter> constexpr bool __is_contiguous = false;
#endif

template <typename _Tp> constexpr bool __is_contiguous<_Tp *> = true;

template <typename _Tp, typename _Seq>
constexpr bool __is_contiguous<__gnu_cxx::__normal_iterator<_Tp *, _Seq>> =
    true;
...
#else
template <typename _EcharT> using __unified_u8_t = _EcharT;
#endif
...
template <typename _CharT, typename _Traits, typename _Alloc>
inline basic_string_view<_CharT> __effective_range(
    const basic_string<_CharT, _Traits, _Alloc> &__source) noexcept {
  return __source;
}

template <typename _CharT, typename _Traits>
inline basic_string_view<_CharT>
__effective_range(const basic_string_view<_CharT, _Traits> &__source) noexcept {
  return __source;
}

// Return the effective range of an NTCTS.
template <typename _Source> auto __effective_range(const _Source &__source) {
...
template <typename _Source> struct __source_value_type_impl {
  using type = typename __safe_iterator_traits<decay_t<_Source>>::value_type;
};
...
using __value_type_is_char = std::enable_if_t<std::is_same_v<_Val, char>, _Val>;
...
inline auto __string_from_range(_InputIterator __first, _InputIterator __last) {
  using _EcharT = typename std::iterator_traits<_InputIterator>::value_type;
...
public:
  ...
  path(_InputIterator __first, _InputIterator __last, format = auto_format)
      : _M_pathname(
            _S_convert(__detail::__string_from_range(__first, __last))) {
    _M_split_cmpts();
  }
  ...
  template <typename _InputIterator>
  __detail::_Path2<_InputIterator> &assign(_InputIterator __first,
  ...
  template <typename _InputIterator>
  __detail::_Path2<_InputIterator> &append(_InputIterator __first,
                                           _InputIterator __last) {
  ...
  template <typename _InputIterator>
  __detail::_Path2<_InputIterator> &concat(_InputIterator __first,
                                           _InputIterator __last) {
...
private:
  ...
  static auto _S_convert(_Tp __str) noexcept(
      is_same_v<typename _Tp::value_type, value_type>) {
    if constexpr (is_same_v<typename _Tp::value_type, value_type>)
      ...
      return string_type(_S_convert(__str.data(), __str.data() + __str.size()));
#endif
    else
  ...
  template <typename _EcharT>
  static auto _S_convert(const _EcharT *__first, const _EcharT *__last);
  ...
  template <typename _Tp>
  static string_type _S_convert_loc(const _Tp &__s, const std::locale &__loc) {
    return _S_convert_loc(__s.data(), __s.data() + __s.size(), __loc);
  }
...
/// @cond undocumented
namespace __detail {
[[noreturn]] inline void __throw_conversion_error() {
...
#ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
template <typename _Tp> inline std::wstring __wstr_from_utf8(const _Tp &__str) {
...
template <
    ...
    typename _Require = __detail::_Path2<_InputIterator>,
    typename _CharT = __detail::__value_type_is_char_or_char8_t<_InputIterator>>
...
inline path u8path(const _Source &__source) {
  ...
  if constexpr (is_same_v<_CharT, char>)
    return path{
        __detail::__wstr_from_utf8(__detail::__effective_range(__source))};
  else
...
template <typename _EcharT>
auto path::_S_convert(const _EcharT *__f, const _EcharT *__l) {
...
public:
  ...
  friend bool operator==(const iterator &__lhs,
                         const iterator &__rhs) noexcept {
...
private:
  ...
  bool _M_is_multi() const noexcept {
  ...
  friend void __path_iter_advance(iterator &__i, difference_type __n) noexcept {
    if (__n == 1)
      ++__i;
    else if (__n == -1)
      --__i;
...
inline __detail::_Path2<_CharT *> &path::operator+=(const _CharT __x) {
...
#endif
  using _Alloc2 =
      typename allocator_traits<_Allocator>::template rebind_alloc<value_type>;
...
inline path::iterator path::begin() const noexcept {
  if (_M_type() == _Type::_Multi)
    return iterator(this, _M_cmpts.begin());
  return iterator(this, empty());
}

inline path::iterator path::end() const noexcept {
  if (_M_type() == _Type::_Multi)
    return iterator(this, _M_cmpts.end());
  return iterator(this, true);
}

inline path::iterator &path::iterator::operator++() noexcept {
...
inline path::iterator::reference path::iterator::operator*() const noexcept {
  __glibcxx_assert(_M_path != nullptr);
  if (_M_is_multi()) {
    __glibcxx_assert(_M_cur != _M_path->_M_cmpts.end());
    return *_M_cur;
...
inline bool path::iterator::_M_equals(iterator __rhs) const noexcept {
...
inline int path::_S_compare(const path &__lhs, const path &__rhs) noexcept {
...
inline ptrdiff_t distance(filesystem::path::iterator __first,
                          filesystem::path::iterator __last) noexcept {
  return __path_iter_distance(__first, __last);
}

template <typename _Distance>
inline void advance(filesystem::path::iterator &__i, _Distance __n) noexcept {
...
template <> struct hash<filesystem::path> {
  size_t operator()(const filesystem::path &__p) const noexcept {
    return filesystem::hash_value(__p);
  }
};

#ifdef __glibcxx_format_path // C++ >= 26 && HOSTED
template <__format::__char _CharT> struct formatter<filesystem::path, _CharT> {
  formatter() = default;
