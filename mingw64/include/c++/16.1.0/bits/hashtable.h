...
public:
  ...
  using iterator = __detail::_Node_iterator<_Value, __constant_iterators::value,
                                            __hash_cached::value>;
...
private:
  ...
  _Hashtable(const _Hash &__h, const _Equal &__eq, const allocator_type &__a)
      : __hashtable_base(__h, __eq), __hashtable_alloc(__node_alloc_type(__a)),
...
public:
  ...
  _Hashtable(initializer_list<value_type> __l, size_type __bkt_count_hint = 0,
             const _Hash &__hf = _Hash(), const key_equal &__eql = key_equal(),
             const allocator_type &__a = allocator_type())
...
#pragma GCC diagnostic pop
  ...
  template <typename _Kt> size_type _M_bucket_tr(const _Kt &__k) const {
...
#pragma GCC diagnostic ignored "-Wc++17-extensions" // if constexpr
  ...
  __hash_code _M_hash_code_ext(const __node_value_type &__from) const {
  ...
  // container instance.
  size_type _M_bucket_index_ext(const __node_value_type &__from) const {
    return _RangeHash{}(_M_hash_code_ext(__from), _M_bucket_count);
  }

  void _M_copy_code(__node_value_type &__to,
                    const __node_value_type &__from) const {
    if constexpr (__hash_cached::value)
      __to._M_hash_code = _M_hash_code_ext(__from);
  }

  void _M_store_code(__node_value_type &__to, __hash_code __code) const {
    if constexpr (__hash_cached::value)
...
#pragma GCC diagnostic pop
  ...
  __node_base_ptr _M_find_before_node(size_type __bkt, const key_type &__k,
  ...
  struct __location_type {
    ...
    explicit operator iterator() const noexcept { return iterator(_M_node()); }
  ...
  __location_type _M_locate(const key_type &__k) const {
    return _M_locate_tr<key_type>(__k);
  }

  template <typename _Kt> __location_type _M_locate_tr(const _Kt &__k) const;
  ...
  void _M_insert_bucket_begin(size_type __bkt, __node_ptr __node) {
    if (_M_buckets[__bkt]) {
  ...
  pair<__node_ptr, __hash_code> _M_compute_hash_code(__node_ptr __hint,
  ...
  template <typename... _Args>
  std::pair<iterator, bool> _M_emplace_uniq(_Args &&...__args);
...
#pragma GCC diagnostic ignored "-Wc++14-extensions" // variable templates
  ...
  static constexpr bool __is_key_type<_Arg, key_type, __detail::_Identity> =
      true;
...
#pragma GCC diagnostic pop
  ...
  template <typename... _Args>
  iterator _M_emplace_multi(const_iterator, _Args &&...__args);

  iterator _M_erase(size_type __bkt, __node_base_ptr __prev_n, __node_ptr __n);

  size_type _M_erase_some(size_type __bkt, __node_base_ptr __prev_n,
                          __node_ptr __n);

  template <typename _InputIterator>
  void _M_insert_range_multi(_InputIterator __first, _InputIterator __last);
...
#pragma GCC diagnostic ignored "-Wc++17-extensions" // if constexpr
  ...
  std::pair<iterator, bool> try_emplace(const_iterator, _KType &&__k,
    ...
    _Scoped_node __node{this, std::piecewise_construct,
...
#ifdef __glibcxx_associative_heterogeneous_insertion // C++26
  template <typename _Kt> std::pair<iterator, bool> _M_insert_tr(_Kt &&__k) {
    auto __loc = _M_locate_tr(__k);
    if (__loc)
      return {iterator(__loc), false};

    _Scoped_node __node(this->_M_allocate_node(std::forward<_Kt>(__k)), this);
    auto __it = _M_insert_unique_node(__loc._M_bucket_index, __loc._M_hash_code,
                                      __node._M_node);
    __node._M_node = nullptr;
    return {__it, true};
  }
...
#pragma GCC diagnostic pop
  ...
  template <typename _Kt> size_type _M_erase_tr(const _Kt &__k);
...
private:
  ...
  __hash_code _M_src_hash_code(const _H2 &,
                               const __node_value_type &__src_n) const {
    if constexpr (__and_<__hash_cached, is_same<_H2, _Hash>,
                         is_empty<_Hash>>::value)
...
public:
  ...
  /// Extract a node.
  node_type extract(const _Key &__k) { return _M_extract_tr<_Key>(__k); }
  ...
  void _M_merge_unique(_Hashtable &__src) {
  ...
  /// Merge from another container of the same type.
  void _M_merge_multi(_Hashtable &__src) {
    __glibcxx_assert(get_allocator() == __src.get_allocator());
  ...
  bool _M_equal(const _Hashtable &__other) const;
...
template <typename _Key, typename _Value, typename _Alloc, typename _ExtractKey,
          ...
          typename _Unused, typename _RehashPolicy, typename _Traits>
template <typename _InputIterator>
inline _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                  _Unused, _RehashPolicy,
...
void _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy,
                _Traits>::_M_assign(_Ht &&__ht, _NodeGenerator &__node_gen) {
  struct _Guard {
    ~_Guard() {
      if (_M_ht) {
        _M_ht->clear();
  ...
  __node_ptr __this_n = __node_gen(static_cast<_FromVal>(__ht_n->_M_v()));
...
void _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy,
                _Traits>::_M_move_assign(_Hashtable &&__ht, true_type) {
...
void _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy,
                _Traits>::_M_move_assign(_Hashtable &&__ht, false_type) {
...
_Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
  ...
  if (__ht._M_node_allocator() == this->_M_node_allocator()) {
    ...
    using _Fwd_Ht = __conditional_t<__move_if_noexcept_cond<value_type>::value,
...
void _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy, _Traits>::
    swap(_Hashtable &__x) noexcept(
        __and_<__is_nothrow_swappable<_Hash>,
               __is_nothrow_swappable<_Equal>>::value) {
  using std::swap;
  swap(__hash_code_base::_M_hash._M_obj, __x.__hash_code_base::_M_hash._M_obj);
...
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
  ...
  for (auto __ref = __it++;
       __it._M_cur && this->_M_node_equals(*__ref._M_cur, *__it._M_cur); ++__it)
    ++__result;
...
template <typename _Key, typename _Value, typename _Alloc, typename _ExtractKey,
          ...
          typename _Unused, typename _RehashPolicy, typename _Traits>
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy, _Traits>::equal_range(const key_type
                                                                  &__k)
...
template <typename _Kt, typename, typename>
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy, _Traits>::_M_equal_range_tr(const _Kt
                                                                        &__k)
    -> pair<iterator, iterator> {
  if (size() <= __small_size_threshold()) {
...
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy,
                _Traits>::_M_equal_range_tr(const _Kt &__k) const
    -> pair<const_iterator, const_iterator> {
  if (size() <= __small_size_threshold()) {
    __node_ptr __n, __beg = nullptr;
    for (__n = _M_begin(); __n; __n = __n->_M_next()) {
...
inline auto
_Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
           _Unused, _RehashPolicy, _Traits>::_M_locate_tr(const _Kt &__k) const
    -> __location_type {
  __location_type __loc;
  const auto __size = size();

  if (__size <= __small_size_threshold()) {
    __loc._M_before = pointer_traits<__node_base_ptr>::pointer_to(
        const_cast<__node_base &>(_M_before_begin));
  ...
  if (__size > __small_size_threshold())
    __loc._M_before =
        _M_find_before_node_tr(__loc._M_bucket_index, __k, __loc._M_hash_code);
...
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy,
                _Traits>::_M_get_previous_node(size_type __bkt, __node_ptr __n)
    -> __node_base_ptr {
...
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy,
                _Traits>::_M_emplace_uniq(_Args &&...__args)
    -> pair<iterator, bool> {
  ...
  if (__kp == nullptr) {
    ...
    __node._M_node = this->_M_allocate_node(std::forward<_Args>(__args)...);
...
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy,
                _Traits>::_M_emplace_multi(const_iterator __hint,
...
template <typename _Key, typename _Value, typename _Alloc, typename _ExtractKey,
          ...
          typename _Unused, typename _RehashPolicy, typename _Traits>
void _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy, _Traits>::_M_rehash_insert(size_type
                                                                       __n) {
...
void _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy,
                _Traits>::_M_insert_range_multi(_InputIterator __first,
...
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy,
                _Traits>::_M_compute_hash_code(__node_ptr __hint,
                                               const key_type &__k) const
...
template <typename _Key, typename _Value, typename _Alloc, typename _ExtractKey,
          ...
          typename _Unused, typename _RehashPolicy, typename _Traits>
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy, _Traits>::erase(const key_type &__k)
    -> size_type {
...
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy,
                _Traits>::_M_erase_some(size_type __bkt,
                                        __node_base_ptr __prev_n,
                                        __node_ptr __n) -> size_type {
  // _GLIBCXX_RESOLVE_LIB_DEFECTS
...
template <typename _Kt>
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy, _Traits>::_M_erase_tr(const _Kt &__k)
    -> size_type {
  auto __loc = _M_locate_tr(__k);
  if (!__loc)
    return 0;
...
template <typename _Key, typename _Value, typename _Alloc, typename _ExtractKey,
          ...
          typename _Unused, typename _RehashPolicy, typename _Traits>
auto _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy, _Traits>::erase(const_iterator __first,
...
void _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
  ...
  __rehash_guard_t __rehash_guard(_M_rehash_policy);
  __bkt_count = std::max(
      _M_rehash_policy._M_bkt_for_elements(_M_element_count + 1), __bkt_count);
...
template <typename _Key, typename _Value, typename _Alloc, typename _ExtractKey,
          typename _Equal, typename _Hash, typename _RangeHash,
          typename _Unused, typename _RehashPolicy, typename _Traits>
bool _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _Hash, _RangeHash,
                _Unused, _RehashPolicy, _Traits>::_M_equal(const _Hashtable
                                                               &__other) const {
  ...
  if constexpr (__unique_keys::value)
    ...
    for (auto __x_n = _M_begin(); __x_n;) {
      ...
      for (; __x_n_end && key_eq()(_ExtractKey{}(__x_n->_M_v()),
