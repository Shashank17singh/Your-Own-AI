...
template <size_t, typename _Tp> struct _Indexed_bound_arg {
...
struct _Bound_arg_storage : _IndexedArgs... {
  ...
  static constexpr decltype(auto) _S_apply(_Fd &&__fd, _Self &&__self,
...
template <bool _Back, typename _Fd, typename... _BoundArgs> class _Binder {
  template <typename _Self, typename... _CallArgs>
  using _Result_t = __conditional_t<
      _Back,
      invoke_result<__like_t<_Self, _Fd>, _CallArgs...,
                    __like_t<_Self, _BoundArgs>...>,
      invoke_result<__like_t<_Self, _Fd>, __like_t<_Self, _BoundArgs>...,
                    _CallArgs...>>::type;

  template <typename _Self, typename... _CallArgs>
  static consteval bool _S_noexcept_invocable() {
    if constexpr (_Back)
      return is_nothrow_invocable_v<__like_t<_Self, _Fd>, _CallArgs...,
                                    __like_t<_Self, _BoundArgs>...>;
    else
      return is_nothrow_invocable_v<
          __like_t<_Self, _Fd>, __like_t<_Self, _BoundArgs>..., _CallArgs...>;
  }
...
public:
  ...
  explicit constexpr _Binder(int, _Fn &&__fn, _Args &&...__args) noexcept(
      __and_<is_nothrow_constructible<_Fd, _Fn>,
             is_nothrow_constructible<_BoundArgs, _Args>...>::value)
      : _M_fd(std::forward<_Fn>(__fn)),
        _M_bound_args(
            __make_bound_args<_BoundArgs...>(std::forward<_Args>(__args)...)) {
...
#pragma GCC diagnostic ignored "-Wc++23-extensions" // deducing this
  ...
  operator()(this _Self &&__self, _CallArgs &&...__call_args) noexcept(
      _S_noexcept_invocable<_Self, _CallArgs...>()) {
...
#pragma GCC diagnostic pop
  ...
  operator()(_CallArgs &&...__call_args) & noexcept(
      _S_noexcept_invocable<_Binder &, _CallArgs...>()) {
  ...
  operator()(_CallArgs &&...__call_args) const & noexcept(
      _S_noexcept_invocable<const _Binder &, _CallArgs...>()) {
    return _S_call(*this, std::forward<_CallArgs>(__call_args)...);
  }
  ...
  operator()(_CallArgs &&...__call_args) && noexcept(
      _S_noexcept_invocable<_Binder &&, _CallArgs...>()) {
    return _S_call(std::move(*this), std::forward<_CallArgs>(__call_args)...);
  }
  ...
  template <typename... _CallArgs> void operator()(_CallArgs &&...) & = delete;
  ...
  static constexpr decltype(auto) _S_call(_Tp &&__g,
                                          _CallArgs &&...__call_args) {
    if constexpr (sizeof...(_BoundArgs) > 1)
      return _BoundArgsStorage::template _S_apply<_Back>(
          std::forward<_Tp>(__g)._M_fd, std::forward<_Tp>(__g)._M_bound_args,
          std::forward<_CallArgs>(__call_args)...);
...
private:
  using _BoundArgsStorage
      ...
      = decltype(__make_bound_args<_BoundArgs...>(
          std::declval<_BoundArgs>()...));
