...
constexpr auto *__cast_to(_Ptrs __ptrs) noexcept {
...
struct _Storage {
  ...
  template <typename _Tp> static consteval bool _S_stored_locally() noexcept {
  ...
  static consteval bool _S_nothrow_init() noexcept {
  ...
  template <typename _Tp, typename... _Args>
  void _M_init(_Args &&...__args) noexcept(_S_nothrow_init<_Tp, _Args...>()) {
  ...
  union {
    _Ptrs _M_ptrs;
    alignas(_Delegate) alignas(
        void (*)()) unsigned char _M_bytes[sizeof(_Delegate)];
  };
};

template <bool _Noex, typename _Ret, typename... _Args> struct _Base_invoker {
  using _Signature = _Ret (*)(_Args...) noexcept(_Noex);

  using __storage_func_t = _Ret (*)(const _Storage &, _Args...) noexcept(_Noex);
  template <typename _Tp> static consteval __storage_func_t _S_storage() {
    return &_S_call_storage<_Adjust_target<_Tp>>;
  }

  using __ptrs_func_t = _Ret (*)(_Ptrs, _Args...) noexcept(_Noex);
  template <typename _Tp> static consteval __ptrs_func_t _S_ptrs() {
    return &_S_call_ptrs<_Adjust_target<_Tp>>;
  }

#ifdef __glibcxx_function_ref // C++ >= 26
  template <typename _Fn>
  static _Ret _S_static(_Ptrs, _Args... __args) noexcept(_Noex) {
    return _Fn::operator()(std::forward<_Args>(__args)...);
  }

  template <auto __fn>
  static _Ret _S_nttp(_Ptrs, _Args... __args) noexcept(_Noex) {
    return std::__invoke_r<_Ret>(__fn, std::forward<_Args>(__args)...);
  }

  template <auto __fn, typename _Tp>
  static _Ret _S_bind_ptr(_Ptrs __ptrs, _Args... __args) noexcept(_Noex) {
    auto *__p = __polyfunc::__cast_to<_Tp>(__ptrs);
    return std::__invoke_r<_Ret>(__fn, __p, std::forward<_Args>(__args)...);
  }

  template <auto __fn, typename _Ref>
  static _Ret _S_bind_ref(_Ptrs __ptrs, _Args... __args) noexcept(_Noex) {
...
private:
  ...
  template <typename _Tp>
  static _Ret _S_call_storage(const _Storage &__ref,
                              _Args... __args) noexcept(_Noex) {
    _Ptrs __ptrs;
    if constexpr (is_function_v<remove_pointer_t<_Tp>>)
  ...
  template <typename _Tp>
  static _Ret _S_call_ptrs(_Ptrs __ptrs, _Args... __args) noexcept(_Noex) {
    if constexpr (is_function_v<remove_pointer_t<_Tp>>)
...
template <typename _Tp> consteval bool __pass_by_value() {
...
template <typename _Func> auto &__invoker_of(_Func &__f) noexcept {
  return __f._M_invoke;
}

template <typename _Func> auto &__base_of(_Func &__f) noexcept {
  return static_cast<__like_t<_Func &, typename _Func::_Base>>(__f);
}

template <typename _Src, typename _Dst>
consteval bool __is_invoker_convertible() noexcept {
...
struct _Manager {
  ...
  template <bool _ProvideCopy, typename _Tp> consteval static auto _S_select() {
    if constexpr (is_function_v<remove_pointer_t<_Tp>>)
      return &_S_func;
...
private:
  static void _S_func(_Op __op, _Storage &__target,
                      const _Storage *__src) noexcept {
    switch (__op) {
    case _Op::_Address:
    case _Op::_Move:
    case _Op::_Copy:
  ...
  static void _S_trivial(_Op __op, _Storage &__target,
                         const _Storage *__src) noexcept {
    switch (__op) {
    case _Op::_Address:
  ...
  static void _S_local(_Op __op, _Storage &__target,
                       const _Storage *__src) noexcept(!_Provide_copy) {
    switch (__op) {
    case _Op::_Address:
      __target._M_ptrs._M_obj = __src->_M_addr();
      return;
  ...
  template <bool _Provide_copy, typename _Tp>
  static void _S_ptr(_Op __op, _Storage &__target,
                     const _Storage *__src) noexcept(!_Provide_copy) {
    switch (__op) {
    case _Op::_Address:
    case _Op::_Move:
...
class _Mo_base {
protected:
  _Mo_base() noexcept : _M_manage(_Manager::_S_empty) {}

  _Mo_base(_Mo_base &&__x) noexcept { _M_move(__x); }
  ...
  _Mo_base &operator=(_Mo_base &&__x) noexcept {
    _M_destroy();
    _M_move(__x);
    return *this;
  }
  ...
  void swap(_Mo_base &__x) noexcept {
    using _Op = _Manager::_Op;
...
template <typename... _Signature> class move_only_function; // not defined
...
protected:
  ...
  _Cpy_base(_Cpy_base const &__x) : _Mo_base() { _M_copy(__x); }

  _Cpy_base &operator=(_Cpy_base &&) = default;
...
template <typename... _Signature> class copyable_function; // not defined
...
template <typename... _Signature>
struct _Never_valueless_alt<std::copyable_function<_Signature...>> : true_type {
};
} // namespace __detail::__variant
...
/// @cond undocumented
namespace __polyfunc {
template <typename _Sig> struct __skip_first_arg;
...
// Returns a function pointer to signature to be used with function_ref, or
// void.
template <typename _Fn, typename _Tr> consteval auto __deduce_funcref() {
...
template <typename... _Signature> class function_ref; // not defined
