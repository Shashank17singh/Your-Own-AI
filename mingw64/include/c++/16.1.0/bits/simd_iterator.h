 ...
 * C++26 [simd.iterator]
 */
template <typename _Vp> class __iterator {
  friend class __iterator<const _Vp>;

  template <typename, typename> friend class _VecBase;

  template <size_t, typename> friend class _MaskBase;

  _Vp *_M_data = nullptr;

  __simd_size_type _M_offset = 0;

  constexpr __iterator(_Vp &__d, __simd_size_type __off)
...
public:
  ...
  constexpr __iterator(const __iterator<remove_const_t<_Vp>> &__i)
  ...
  constexpr value_type operator[](difference_type __i) const {
    return (*_M_data)[_M_offset + __i];
  } // checked in operator[]

  constexpr friend bool operator==(__iterator __a, __iterator __b) = default;

  constexpr friend bool operator==(__iterator __a,
  ...
  constexpr friend __iterator operator+(const __iterator &__it,
                                        difference_type __x) {
    return __iterator(*__it._M_data, __it._M_offset + __x);
  }

  constexpr friend __iterator operator+(difference_type __x,
  ...
  constexpr friend difference_type operator-(__iterator __a, __iterator __b) {
    return __a._M_offset - __b._M_offset;
  }

  constexpr friend difference_type operator-(__iterator __it,
                                             std::default_sentinel_t) noexcept {
    return __it._M_offset - difference_type(_Vp::size.value);
  }

  constexpr friend difference_type operator-(std::default_sentinel_t,
                                             __iterator __it) noexcept {
