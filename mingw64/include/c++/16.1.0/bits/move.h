...
__attribute__((__always_inline__)) inline _GLIBCXX_CONSTEXPR _Tp *
...
template <typename _Tp, typename _Up> struct __like_impl<_Tp &, _Up &> {
  using type = _Up &;
};

template <typename _Tp, typename _Up> struct __like_impl<const _Tp &, _Up &> {
...
constexpr __like_t<_Tp, _Up> forward_like(_Up &&__x) noexcept {
...
template <typename _Tp>
  ...
  requires(!__is_tuple_like<_Tp>::value) && is_move_constructible_v<_Tp> &&
