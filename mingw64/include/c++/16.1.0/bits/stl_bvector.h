...
__attribute__((__nonnull__)) _GLIBCXX20_CONSTEXPR void
...
public:
  ...
  _GLIBCXX_NODISCARD
      _GLIBCXX20_CONSTEXPR operator bool() const _GLIBCXX_NOEXCEPT {
    return !!(*_M_p & _M_mask);
  }
...
struct _Bit_iterator : public _Bit_iterator_base {
  ...
  _GLIBCXX_NODISCARD
  friend _GLIBCXX20_CONSTEXPR iterator operator+(const iterator &__x,
                                                 difference_type __n) {
    iterator __tmp = __x;
    __tmp += __n;
    return __tmp;
  ...
  _GLIBCXX_NODISCARD
  friend _GLIBCXX20_CONSTEXPR iterator operator+(difference_type __n,
                                                 const iterator &__x) {
    return __x + __n;
  }
...
public:
  ...
  vector(const vector &__x)
      : _Base(
            _Bit_alloc_traits::_S_select_on_copy(__x._M_get_Bit_allocator())) {
...
public:
  ...
  max_size() const _GLIBCXX_NOEXCEPT {
    const size_type __isize =
        __gnu_cxx::__numeric_traits<difference_type>::__max - int(_S_word_bit) +
        1;
  ...
  _GLIBCXX_NODISCARD _GLIBCXX20_CONSTEXPR reference operator[](size_type __n) {
    ...
    return _Bit_reference(this->_M_impl._M_start._M_p + __n / int(_S_word_bit),
...
public:
    ...
    if constexpr (ranges::forward_range<_Rg> || ranges::sized_range<_Rg>) {
      if (auto __n = size_type(ranges::distance(__rg))) {
        if (capacity() - size() >= __n) {
          ...
          iterator __i =
              _M_copy_aligned(__begin, __pos._M_const_cast(), __start);
          iterator __j = ranges::copy(__rg, __i).out;
          iterator __finish = std::copy(__pos._M_const_cast(), __end, __j);
          this->_M_deallocate();
      ...
      return insert_range(__pos, vector(from_range, __rg, get_allocator()));
  }
  ...
  template <typename... _Args>
  _GLIBCXX20_CONSTEXPR iterator emplace(const_iterator __pos,
                                        _Args &&...__args) {
...
protected:
  ...
  template <typename _InputIterator>
  _GLIBCXX20_CONSTEXPR void _M_initialize_range(_InputIterator __first,
                                                _InputIterator __last,
  ...
  _GLIBCXX20_CONSTEXPR void _M_initialize_range(_ForwardIterator __first,
  ...
  _M_insert_range(iterator __pos, _InputIterator __first, _InputIterator __last,
                  std::input_iterator_tag) {
    for (; __first != __last; ++__first) {
...
inline void __fill_bvector(_Bit_type *__v, unsigned int __first,
                           unsigned int __last, bool __x) _GLIBCXX_NOEXCEPT {
...
_GLIBCXX20_CONSTEXPR
inline void __fill_a1(_GLIBCXX_STD_C::_Bit_iterator __first,
