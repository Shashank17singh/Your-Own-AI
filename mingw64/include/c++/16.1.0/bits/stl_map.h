...
public:
  ...
  map(from_range_t, _Rg &&__rg, const _Compare &__comp,
...
#ifdef __glibcxx_associative_heterogeneous_insertion // C++26
  template <__heterogeneous_tree_key<map> _Kt> mapped_type &at(const _Kt &__k) {
    iterator __i = lower_bound(__k);
    if (__i == end() || key_comp()(__k, (*__i).first))
      __throw_out_of_range(__N("map::at"));
    return (*__i).second;
  ...
  template <typename, typename> friend struct std::_Rb_tree_merge_helper;
  ...
  pair<iterator, bool> try_emplace(_Kt &&__k, _Args &&...__args) {
    ...
    if (__node) {
      __i = _M_t._M_emplace_here(
          __left == __node, __node, std::piecewise_construct,
  ...
  iterator try_emplace(const_iterator __hint, _Kt &&__k, _Args &&...__args) {
    iterator __i;
    auto [__left, __node] = _M_t._M_get_insert_hint_unique_pos_tr(__hint, __k);
    if (__node) {
      __i = _M_t._M_emplace_here(
          __left == __node, __node, std::piecewise_construct,
          std::forward_as_tuple(std::forward<_Kt>(__k)),
   ...
   */
  size_type erase(const key_type &__x) { return _M_t._M_erase_unique(__x); }
