...
constexpr memory_order operator|(memory_order __m,
                                 __memory_order_modifier __mod) noexcept {
...
struct atomic_flag : public __atomic_flag_base {
  ...
  _GLIBCXX_ALWAYS_INLINE void notify_one() noexcept {
    std::__atomic_notify_address(&_M_i, false);
  }

  // TODO add const volatile overload

  _GLIBCXX_ALWAYS_INLINE void notify_all() noexcept {
...
_Tp __fetch_min(_Tp *__ptr, _Val<_Tp> __i, memory_order __m) noexcept;

template <typename _Tp>
_Tp __fetch_max(_Tp *__ptr, _Val<_Tp> __i, memory_order __m) noexcept;
#endif
} // namespace __atomic_impl
...
public:
  ...
  _GLIBCXX_ALWAYS_INLINE bool
  compare_exchange_strong(__int_type &__i1, __int_type __i2, memory_order __m1,
                          memory_order __m2) noexcept {
    __glibcxx_assert(__is_valid_cmpexch_failure_order(__m2));
...
#if __glibcxx_atomic_min_max
  _GLIBCXX_ALWAYS_INLINE __int_type
  fetch_min(__int_type __i, memory_order __m = memory_order_seq_cst) noexcept {
  ...
  _GLIBCXX_ALWAYS_INLINE __int_type
  fetch_max(__int_type __i, memory_order __m = memory_order_seq_cst) noexcept {
...
private:
  ...
  static constexpr ptrdiff_t _S_type_size(ptrdiff_t __d) {
...
public:
  ...
  _GLIBCXX_ALWAYS_INLINE void
  store(__pointer_type __p, memory_order __m = memory_order_seq_cst) noexcept {
    memory_order __b __attribute__((__unused__)) = __m & __memory_order_mask;
  ...
  _GLIBCXX_ALWAYS_INLINE void notify_one() const noexcept {
    std::__atomic_notify_address(&_M_p, false);
  }

  // TODO add const volatile overload

  _GLIBCXX_ALWAYS_INLINE void notify_all() const noexcept {
...
#if __glibcxx_atomic_min_max
  _GLIBCXX_ALWAYS_INLINE __pointer_type fetch_min(
      __pointer_type __p, memory_order __m = memory_order_seq_cst) noexcept {
  ...
  _GLIBCXX_ALWAYS_INLINE __pointer_type fetch_max(
      __pointer_type __p, memory_order __m = memory_order_seq_cst) noexcept {
...
__compare_exchange(_Tp &__val, _Val<_Tp> &__e, _Val<_Tp> &__i, bool __is_weak,
  ...
  if constexpr (!__atomic_impl::__maybe_has_padding<_Vp>()) {
    return __atomic_compare_exchange(__pval, std::__addressof(__e),
                                     std::__addressof(__i), __is_weak, int(__s),
                                     int(__f));
  } else if constexpr (!_AtomicRef) // std::atomic<T>
  {
    ...
    if (__atomic_compare_exchange(__pval, __pexp, __pi, __is_weak, int(__s),
                                  int(__f)))
      return true;
...
template <typename _Tp>
_GLIBCXX_ALWAYS_INLINE _Val<_Tp> load(const _Tp *__ptr,
                                      memory_order __m) noexcept {
...
template <typename _Tp>
_GLIBCXX_ALWAYS_INLINE _Val<_Tp> exchange(_Tp *__ptr, _Val<_Tp> __desired,
                                          memory_order __m) noexcept {
  alignas(_Tp) unsigned char __buf[sizeof(_Tp)];
  auto *__dest = reinterpret_cast<_Val<_Tp> *>(__buf);
...
template <bool _AtomicRef = false, typename _Tp>
_GLIBCXX_ALWAYS_INLINE bool
compare_exchange_strong(_Tp *__ptr, _Val<_Tp> &__expected, _Val<_Tp> __desired,
...
wait(const _Tp *__ptr, _Val<_Tp> __old,
     memory_order __m = memory_order_seq_cst) noexcept {
  std::__atomic_wait_address_v(
      __ptr, __old, [__ptr, __m]() { return __atomic_impl::load(__ptr, __m); });
}

// TODO add const volatile overload

template <typename _Tp>
_GLIBCXX_ALWAYS_INLINE void notify_one(const _Tp *__ptr) noexcept {
...
template <typename _Tp>
_GLIBCXX_ALWAYS_INLINE void notify_all(const _Tp *__ptr) noexcept {
...
#if __glibcxx_atomic_min_max
template <typename _Tp>
concept __atomic_fetch_minmaxable = requires(_Tp __t) {
...
public:
  ...
  bool compare_exchange_strong(_Fp &__expected, _Fp __desired,
                               memory_order __success,
                               memory_order __failure) noexcept {
    return __atomic_impl::compare_exchange_strong(&_M_fp, __expected, __desired,
                                                  __success, __failure);
  }
...
#if __glibcxx_atomic_min_max
  value_type fetch_min(value_type __i,
                       memory_order __m = memory_order_seq_cst) noexcept {
  ...
  value_type fetch_max(value_type __i,
                       memory_order __m = memory_order_seq_cst) noexcept {
...
template <typename _Tp> struct __atomic_ref_base;

template <typename _Tp> struct __atomic_ref_base<const _Tp> {
private:
  using _Vt = remove_cv_t<_Tp>;
  using _Address_return_t =
      __conditional_t<is_volatile_v<_Tp>, const volatile void *, const void *>;
...
public:
  ...
  explicit __atomic_ref_base(const _Tp *__ptr) noexcept
  ...
  value_type load(memory_order __m = memory_order_seq_cst) const noexcept {
  ...
  _GLIBCXX_ALWAYS_INLINE constexpr _Address_return_t address() const noexcept {
    return _M_ptr;
  }
...
struct __atomic_ref_base : __atomic_ref_base<const _Tp> {
  using _Address_return_t =
      __conditional_t<is_volatile_v<_Tp>, volatile void *, void *>;

public:
  using value_type = typename __atomic_ref_base<const _Tp>::value_type;

  explicit __atomic_ref_base(_Tp *__ptr) noexcept
      : __atomic_ref_base<const _Tp>(__ptr) {}

  value_type operator=(value_type __t) const noexcept {
    this->store(__t);
    return __t;
  }

  void store(value_type __t,
             memory_order __m = memory_order_seq_cst) const noexcept {
    __atomic_impl::store(this->_M_ptr, __t, __m);
  }

  value_type exchange(value_type __desired,
                      memory_order __m = memory_order_seq_cst) const noexcept {
  ...
  bool compare_exchange_weak(
      value_type &__expected, value_type __desired,
...
struct __atomic_ref<_Tp, true, false, false> : __atomic_ref_base<_Tp> {
  ...
  value_type fetch_and(value_type __i,
                       memory_order __m = memory_order_seq_cst) const noexcept {
    return __atomic_impl::fetch_and(this->_M_ptr, __i, __m);
  }

  value_type fetch_or(value_type __i,
                      memory_order __m = memory_order_seq_cst) const noexcept {
    return __atomic_impl::fetch_or(this->_M_ptr, __i, __m);
  }

  value_type fetch_xor(value_type __i,
                       memory_order __m = memory_order_seq_cst) const noexcept {
...
#if __glibcxx_atomic_min_max
  _GLIBCXX_ALWAYS_INLINE value_type fetch_min(
      value_type __i, memory_order __m = memory_order_seq_cst) const noexcept {
    return __atomic_impl::__fetch_min(this->_M_ptr, __i, __m);
  }

  _GLIBCXX_ALWAYS_INLINE value_type fetch_max(
      value_type __i, memory_order __m = memory_order_seq_cst) const noexcept {
    return __atomic_impl::__fetch_max(this->_M_ptr, __i, __m);
  }
#endif
