...
struct __destroy_fn {
  ...
  constexpr borrowed_iterator_t<_Range> operator()(_Range &&__r) const noexcept;
};
...
public:
  constexpr explicit _DestroyGuard(const _Iter &__iter)
      : _M_first(__iter), _M_cur(std::__addressof(__iter)) {}

  constexpr void release() noexcept { _M_cur = nullptr; }

  constexpr ~_DestroyGuard() {
    if (_M_cur != nullptr)
...
struct _DestroyGuard<_Iter> {
  constexpr explicit _DestroyGuard(const _Iter &) {}
...
struct __uninitialized_default_construct_fn {
  ...
  _GLIBCXX26_CONSTEXPR _Iter operator()(_Iter __first, _Sent __last) const {
...
struct __uninitialized_default_construct_n_fn {
  template <__detail::__nothrow_forward_iterator _Iter>
    requires default_initializable<iter_value_t<_Iter>>
  _GLIBCXX26_CONSTEXPR _Iter operator()(_Iter __first,
...
struct __uninitialized_copy_fn {
  ...
  _GLIBCXX26_CONSTEXPR uninitialized_copy_result<_Iter, _Out>
  ...
  _GLIBCXX26_CONSTEXPR uninitialized_copy_result<borrowed_iterator_t<_IRange>,
...
struct __uninitialized_copy_n_fn {
  template <input_iterator _Iter, __detail::__nothrow_forward_iterator _Out,
    ...
    requires constructible_from<iter_value_t<_Out>, iter_reference_t<_Iter>>
  _GLIBCXX26_CONSTEXPR uninitialized_copy_n_result<_Iter, _Out>
...
struct __uninitialized_move_fn {
  ...
  _GLIBCXX26_CONSTEXPR uninitialized_move_result<_Iter, _Out>
  operator()(_Iter __ifirst, _ISent __ilast, _Out __ofirst,
             _OSent __olast) const {
    using _OutType = remove_reference_t<iter_reference_t<_Out>>;
    if constexpr (sized_sentinel_for<_ISent, _Iter> &&
                  ...
                  is_trivially_constructible_v<
                      _OutType, iter_rvalue_reference_t<_Iter>> &&
                  is_trivially_default_constructible_v<_OutType> &&
                  is_trivially_assignable_v<_OutType &,
                                            iter_rvalue_reference_t<_Iter>>) {
      auto __d = __ilast - __ifirst;
      ...
      auto [__in, __out] = ranges::copy_n(
          std::make_move_iterator(std::move(__ifirst)), __d, __ofirst);
  ...
  _GLIBCXX26_CONSTEXPR uninitialized_move_result<borrowed_iterator_t<_IRange>,
                                                 borrowed_iterator_t<_ORange>>
  operator()(_IRange &&__inr, _ORange &&__outr) const {
    return (*this)(ranges::begin(__inr), ranges::end(__inr),
                   ranges::begin(__outr), ranges::end(__outr));
...
struct __uninitialized_move_n_fn {
  template <input_iterator _Iter, __detail::__nothrow_forward_iterator _Out,
    ...
    requires constructible_from<iter_value_t<_Out>,
                                iter_rvalue_reference_t<_Iter>>
  _GLIBCXX26_CONSTEXPR uninitialized_move_n_result<_Iter, _Out>
  operator()(_Iter __ifirst, iter_difference_t<_Iter> __n, _Out __ofirst,
             _Sent __olast) const {
    using _OutType = remove_reference_t<iter_reference_t<_Out>>;
    if constexpr (sized_sentinel_for<_Sent, _Out> &&
      ...
      if (auto __d = __olast - __ofirst; __d < __n)
        __n = static_cast<iter_difference_t<_Iter>>(__d);
      auto [__in, __out] = ranges::copy_n(
          std::make_move_iterator(std::move(__ifirst)), __n, __ofirst);
      return {std::move(__in).base(), __out};
    } else {
      auto __guard = __detail::_DestroyGuard(__ofirst);
      for (; __n > 0 && __ofirst != __olast;
