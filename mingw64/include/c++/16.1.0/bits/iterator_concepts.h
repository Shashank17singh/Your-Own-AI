...
private:
  ...
  template <typename _Tp> using __iter_ref_t = decltype(*std::declval<_Tp>());
  ...
  template <typename _Tp>
    requires(!__adl_imove<_Tp>) && is_lvalue_reference_v<__iter_ref_t<_Tp>>
  struct __result<_Tp> {
  ...
  template <typename _Tp> static consteval bool _S_noexcept() {
...
public:
  ...
  template <typename _Tp> using __type = typename __result<_Tp>::type;
...
namespace __detail {
template <typename _Tp> struct __indirect_value {
...
concept __is_integer_like =
    __integral_nonbool<_Tp> || same_as<_Tp, __max_diff_type> ||
...
concept bidirectional_iterator =
    ...
    derived_from<__detail::__iter_concept<_Iter>, bidirectional_iterator_tag> &&
    requires(_Iter __i) {
      { --__i } -> same_as<_Iter &>;
      { __i-- } -> same_as<_Iter>;
    };
...
concept random_access_iterator =
    ...
    derived_from<__detail::__iter_concept<_Iter>, random_access_iterator_tag> &&
...
struct _IterSwap {
private:
  template <typename _Tp, typename _Up> static consteval bool _S_noexcept() {
    if constexpr (__adl_iswap<_Tp, _Up>)
      ...
      return noexcept(*std::declval<_Tp>() = __iswap::__iter_exchange_move(
...
concept mergeable =
    ...
    indirect_strict_weak_order<_Rel, projected<_I1, _P1>, projected<_I2, _P2>>;
...
constexpr auto __begin(_Tp &__t) {
