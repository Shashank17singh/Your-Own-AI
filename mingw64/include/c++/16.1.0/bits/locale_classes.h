...
public:
  ...
  friend const _Facet *__try_use_facet(const locale &) _GLIBCXX_NOTHROW;
   ...
   */
  template <typename _Facet>
  _GLIBCXX_NODISCARD locale combine(const locale &__other) const;
   ...
   */
  _GLIBCXX_NODISCARD _GLIBCXX_DEFAULT_ABI_TAG string name() const;

#ifdef __glibcxx_text_encoding
#if __CHAR_BIT__ == 8
  text_encoding encoding() const;
#else
  text_encoding encoding() const = delete;
#endif
#endif
...
template <typename _Tp> constexpr bool __is_facet<volatile _Tp> = false;
#endif
