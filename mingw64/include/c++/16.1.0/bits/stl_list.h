...
template <typename _VoidPtr> struct _Node_base {
  ...
  static void swap(_Node_base &__x, _Node_base &__y) noexcept;

  void _M_transfer(_Base_ptr const __first, _Base_ptr const __last) noexcept;
...
struct _Node_header : public _Node_base<_VoidPtr>, _List_size {
  _Node_header() noexcept { _M_init(); }
  ...
  void _M_move_nodes(_Node_header &&__x) noexcept {
...
struct _Node : public __list::_Node_base<__ptr_rebind<_ValPtr, void>> {
  ...
  value_type *_M_valptr() noexcept { return std::__addressof(_M_u._M_data); }

  value_type const *_M_valptr() const noexcept {
...
public:
  ...
  constexpr _Iterator(const _Iterator<_OtherConst, _Ptr> &__i)
#endif
      : _M_node(__i._M_node) {
  }
...
template <typename _Tp, typename _Ptr> struct _Node_traits;
...
template <typename _Tp> struct _Node_traits<_Tp, _Tp *> {
...
struct _Node_traits : _Node_traits<_Tp, _Tp *> {};
#else
...
template <typename _NodeBaseT> struct _Scratch_list : _NodeBaseT {
  ...
  template <typename _Iter, typename _Cmp> struct _Ptr_cmp {
    _Cmp _M_cmp;

    bool operator()(_Base_ptr __lhs, _Base_ptr __rhs) /* not const */
    {
      return _M_cmp(*_Iter(__lhs), *_Iter(__rhs));
    }
  };

  template <typename _Iter> struct _Ptr_cmp<_Iter, void> {
    bool operator()(_Base_ptr __lhs, _Base_ptr __rhs) const {
      return *_Iter(__lhs) < *_Iter(__rhs);
    }
  };

  // Merge nodes from __x into *this. Both lists must be sorted wrt _Cmp.
  template <typename _Cmp> void merge(_Base &__x, _Cmp __comp) {
  ...
  void _M_take_one(_Base_ptr __i) { this->_M_transfer(__i, __i->_M_next); }
...
template <typename _Tp> struct _List_iterator {
  ...
  _List_iterator _M_const_cast() const _GLIBCXX_NOEXCEPT { return *this; }
  ...
  _List_iterator &operator++() _GLIBCXX_NOEXCEPT {
    _M_node = _M_node->_M_next;
    return *this;
  }

  _List_iterator operator++(int) _GLIBCXX_NOEXCEPT {
  ...
  _GLIBCXX_NODISCARD
  friend bool operator==(const _List_iterator &__x,
...
template <typename _Tp> struct _List_const_iterator {
  ...
  _List_const_iterator &operator++() _GLIBCXX_NOEXCEPT {
    _M_node = _M_node->_M_next;
    return *this;
  }

  _List_const_iterator operator++(int) _GLIBCXX_NOEXCEPT {
  ...
  _GLIBCXX_NODISCARD
  friend bool operator==(const _List_const_iterator &__x,
...
protected:
  ...
  typedef __list::_Node_traits<_Tp, typename _Tp_alloc_traits::pointer>
      _Node_traits;
  typedef typename _Tp_alloc_traits::template rebind<
      typename _Node_traits::_Node>::other _Node_alloc_type;
...
public:
  list(list &&__x, const __type_identity_t<allocator_type> &__a) noexcept(
...
#pragma GCC diagnostic ignored "-Wc++17-extensions" // if constexpr
  ...
  list &operator=(list &&__x) noexcept(_Node_alloc_traits::_S_nothrow_move()) {
    ...
    if constexpr (_Node_alloc_traits::_S_propagate_on_move_assign())
      this->_M_get_Node_allocator() = std::move(__x._M_get_Node_allocator());
...
#pragma GCC diagnostic pop
  ...
  void assign_range(_Rg &&__rg) {
    ...
    if (__first2 == __last2)
      ...
      insert_range(__last1, ranges::subrange(std::move(__first2), __last2));
  }
#endif
   ...
   */
  template <__detail::__container_compatible_range<_Tp> _Rg>
  iterator insert_range(const_iterator __position, _Rg &&__rg) {
    list __tmp(from_range, std::forward<_Rg>(__rg), get_allocator());
    if (!__tmp.empty()) {
  ...
  void swap(list &__x) _GLIBCXX_NOEXCEPT {
    _Node_traits::_Node_base::swap(this->_M_impl._M_node, __x._M_impl._M_node);
...
protected:
  ...
  void _M_check_equal_allocators(const list &__x) _GLIBCXX_NOEXCEPT {
  ...
  struct _Finalize_merge {
    explicit _Finalize_merge(list &__dest, list &__src,
...
_GLIBCXX_NODISCARD inline bool operator==(const list<_Tp, _Alloc> &__x,
...
template <typename _Tp, typename _Alloc>
[[nodiscard]]
inline __detail::__synth3way_t<_Tp> operator<=>(const list<_Tp, _Alloc> &__x,
                                                const list<_Tp, _Alloc> &__y) {
...
inline ptrdiff_t __distance(__list::_Iterator<_Const, _Ptr> __first,
...
namespace __list {
template <typename _VoidPtr>
void _Node_base<_VoidPtr>::swap(_Node_base &__x, _Node_base &__y) noexcept {
...
template <typename _VoidPtr>
void _Node_base<_VoidPtr>::_M_transfer(_Base_ptr const __first,
...
template <typename _VoidPtr>
void _Node_header<_VoidPtr>::_M_reverse() noexcept {
