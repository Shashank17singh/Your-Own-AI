...
concept __has_arrow =
    input_iterator<_It> &&
    (is_pointer_v<_It> || requires(const _It __it) { __it.operator->(); });
...
private:
  ...
  static constexpr bool _S_bool(bool) noexcept; // not defined
  ...
  _S_size(_Tp &__t) noexcept(noexcept(ranges::end(__t) - ranges::begin(__t))) {
...
public:
  ...
  template <borrowed_range _Rng>
    requires __detail::__convertible_to_non_slicing<iterator_t<_Rng>, _It> &&
             ...
             constexpr subrange(_Rng &&__r, __size_type __n) noexcept(
                 noexcept(subrange(ranges::begin(__r), ranges::end(__r), __n)))
               requires(_Kind == subrange_kind::sized)
  ...
  [[nodiscard]] constexpr subrange next(iter_difference_t<_It> __n = 1) const &
...
inline constexpr bool enable_borrowed_range<subrange<_It, _Sent, _Kind>> = true;
