...
concept __parsable_with =
    semiregular<_Formatter> && requires(_Formatter __f, _ParseContext __pc) {
...
template <typename _CharT> struct _Iter_for;
template <typename _CharT> using _Iter_for_t = typename _Iter_for<_CharT>::type;
...
concept __has_debug_format =
    requires(_Formatter __f) { __f.set_debug_format(); };
} // namespace __format
/// @endcond
