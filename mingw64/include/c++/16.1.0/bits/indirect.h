...
template <typename _Tp, typename _Alloc = allocator<_Tp>> class indirect;

template <typename _Tp> constexpr bool __is_indirect = false;
...
template <typename _Tp, typename _Alloc> class indirect {
...
public:
  ...
  constexpr explicit indirect(allocator_arg_t, const _Alloc &__a)
      : _M_alloc(__a), _M_objp(_M_make_obj_chk()) {}

  constexpr indirect(const indirect &__o)
      : indirect(allocator_arg,
                 _ATraits::select_on_container_copy_construction(__o._M_alloc),
                 __o) {}

  constexpr indirect(allocator_arg_t, const _Alloc &__a,
  ...
  constexpr indirect(indirect &&__other) noexcept
  ...
  constexpr indirect(
      ...
      : _M_alloc(__a), _M_objp(std::__exchange(__other._M_objp, nullptr)) {
  ...
  template <typename _Up = _Tp>
    requires(!is_same_v<remove_cvref_t<_Up>, in_place_t>) &&
            ...
            is_constructible_v<_Tp, _Up> && is_default_constructible_v<_Alloc>
  ...
  constexpr explicit indirect(allocator_arg_t, const _Alloc &__a, _Up &&__u)
  ...
  constexpr explicit indirect(in_place_t, _Us &&...__us)
      : _M_objp(_M_make_obj(std::forward<_Us>(__us)...)) {}

  template <typename... _Us>
    requires is_constructible_v<_Tp, _Us...>
  constexpr explicit indirect(allocator_arg_t, const _Alloc &__a, in_place_t,
                              _Us &&...__us)
      : _M_alloc(__a), _M_objp(_M_make_obj(std::forward<_Us>(__us)...)) {}

  template <typename _Ip, typename... _Us>
    requires is_constructible_v<_Tp, initializer_list<_Ip> &, _Us...> &&
             is_default_constructible_v<_Alloc>
  constexpr explicit indirect(in_place_t, initializer_list<_Ip> __il,
                              _Us &&...__us)
  ...
  template <typename _Ip, typename... _Us>
    requires is_constructible_v<_Tp, initializer_list<_Ip> &, _Us...>
  constexpr explicit indirect(allocator_arg_t, const _Alloc &__a, in_place_t,
                              initializer_list<_Ip> __il, _Us &&...__us)
      : _M_alloc(__a), _M_objp(_M_make_obj(__il, std::forward<_Us>(__us)...)) {}
  ...
  constexpr indirect &operator=(const indirect &__other) {
    ...
    if (__other._M_objp) {
      if (_ATraits::is_always_equal::value || _M_alloc == __other._M_alloc) {
        if (_M_objp) {
  ...
  constexpr indirect &operator=(indirect &&__other) noexcept(
  ...
  constexpr auto &&operator*(this _Self &&__self) noexcept {
  ...
  constexpr bool valueless_after_move() const noexcept {
    return _M_objp == nullptr;
  }

  constexpr allocator_type get_allocator() const noexcept { return _M_alloc; }

  constexpr void swap(indirect &__other) noexcept(
      _ATraits::propagate_on_container_swap::value ||
      _ATraits::is_always_equal::value) {
    using std::swap;
  ...
  swap(indirect &__lhs,
       indirect &__rhs) noexcept(_ATraits::propagate_on_container_swap::value ||
                                 _ATraits::is_always_equal::value) {
    __lhs.swap(__rhs);
  }
  ...
  operator==(const indirect &__lhs,
             const indirect<_Up, _Alloc2> &__rhs) noexcept(noexcept(*__lhs ==
                                                                    *__rhs)) {
  ...
  friend constexpr bool
  operator==(const indirect<_Vp, _Alloc> &__lhs,
             const _Up &__rhs) noexcept(noexcept(*__lhs == __rhs)) {
    if (!__lhs._M_objp)
      return false;
    else
  ...
  template <typename _Up, typename _Alloc2>
  friend constexpr __detail::__synth3way_t<_Tp, _Up> operator<=>(
      const indirect &__lhs,
      const indirect<_Up, _Alloc2>
          &__rhs) noexcept(noexcept(__detail::__synth3way(*__lhs, *__rhs))) {
    if (!__lhs._M_objp || !__rhs._M_objp)
      return bool(__lhs._M_objp) <=> bool(__rhs._M_objp);
    else
  ...
  operator<=>(const indirect &__lhs, const _Up &__rhs) noexcept(
...
private:
  ...
  constexpr void _M_reset(pointer __ptr) noexcept {
    if (_M_objp) {
  ...
  constexpr pointer _M_make_obj(_Args &&...__args) const {
    _Scoped_allocation __sa(_M_alloc, in_place, std::forward<_Args>(__args)...);
    return __sa.release();
  }
  ...
  constexpr pointer _M_make_obj_chk(_Args &&...__args) const {
  ...
  [[__gnu__::__always_inline__]]
  constexpr const _Tp &__get() const noexcept {
    return *_M_objp;
  }
...
template <typename _Value> indirect(_Value) -> indirect<_Value>;
...
struct hash<indirect<_Tp, _Alloc>> {
  constexpr size_t operator()(const indirect<_Tp, _Alloc> &__t) const
...
template <typename _Tp, typename _Alloc>
struct __is_fast_hash<hash<indirect<_Tp, _Alloc>>> : __is_fast_hash<hash<_Tp>> {
};
#endif // __glibcxx_indirect

#if __glibcxx_polymorphic // C++26 && HOSTED
template <typename _Tp, typename _Alloc = allocator<_Tp>> class polymorphic;
...
template <typename _Tp, typename _Alloc> class polymorphic {
  static_assert(is_object_v<_Tp>);
  static_assert(!is_array_v<_Tp>);
  static_assert(!is_same_v<_Tp, in_place_t>);
  static_assert(!__is_in_place_type_v<_Tp>);
  ...
  struct _Obj {
    ...
    using pointer = typename _ATraits::template rebind_traits<_Obj>::pointer;

    enum class _Op { _Dispose = 1, _Copy = 2, _Move = 3 };

    constexpr virtual pointer _M_manage(const _Alloc &, _Op,
                                        void * = nullptr) = 0;
  };

  template <typename _Up> struct _Obj_impl : _Obj {
    using _MyTraits = typename _ATraits::template rebind_traits<_Obj_impl>;
    ...
    template <typename... _Args>
    constexpr _Obj_impl(typename _MyTraits::allocator_type &__a,
                        _Args &&...__args) {
    ...
    _M_manage(const _Alloc &__a, _Op __op, void *) override {
      ...
      case _Op::_Copy:
        return _S_make_obj<_Up>(__a, const_cast<const _Up &>(_M_u._M_objp));
      case _Op::_Dispose: {
...
public:
  ...
  constexpr explicit polymorphic(allocator_arg_t, const _Alloc &__a)
      : polymorphic(allocator_arg, __a, in_place_type<_Tp>) {}

  constexpr polymorphic(const polymorphic &__other)
      : polymorphic(
            allocator_arg,
            _ATraits::select_on_container_copy_construction(__other._M_alloc),
            __other) {}

  constexpr polymorphic(allocator_arg_t, const _Alloc &__a,
  ...
  constexpr polymorphic(polymorphic &&__other) noexcept
      : _M_alloc(std::move(__other._M_alloc)),
        _M_objp(std::__exchange(__other._M_objp, nullptr)) {}

  constexpr polymorphic(
      allocator_arg_t, const _Alloc &__a,
      polymorphic &&__other) noexcept(_ATraits::is_always_equal::value)
      : _M_alloc(__a), _M_objp(std::__exchange(__other._M_objp, nullptr)) {
    if constexpr (!_ATraits::is_always_equal::value)
      if (_M_objp && _M_alloc != __other._M_alloc) {
        // _M_alloc cannot free _M_objp, give it back to __other.
  ...
  template <typename _Up = _Tp, typename _UUp = remove_cvref_t<_Up>>
    requires(!is_same_v<_UUp, polymorphic>) && (!__is_in_place_type_v<_UUp>) &&
            derived_from<_UUp, _Tp> && is_constructible_v<_UUp, _Up> &&
            is_copy_constructible_v<_UUp> && is_default_constructible_v<_Alloc>
  ...
  constexpr explicit polymorphic(allocator_arg_t, const _Alloc &__a, _Up &&__u)
  ...
  template <typename _Up, typename... _Ts>
    requires is_same_v<remove_cvref_t<_Up>, _Up> && derived_from<_Up, _Tp> &&
             is_constructible_v<_Up, _Ts...> && is_copy_constructible_v<_Up> &&
             is_default_constructible_v<_Alloc>
  constexpr explicit polymorphic(in_place_type_t<_Up> __t, _Ts &&...__ts)
  ...
  constexpr explicit polymorphic(allocator_arg_t, const _Alloc &__a,
      ...
      : _M_alloc(__a), _M_objp(_M_make_obj<_Up>(std::forward<_Ts>(__ts)...)) {}

  template <typename _Up, typename _Ip, typename... _Us>
    requires is_same_v<remove_cvref_t<_Up>, _Up> && derived_from<_Up, _Tp> &&
             is_constructible_v<_Up, initializer_list<_Ip> &, _Us...> &&
             is_copy_constructible_v<_Up> && is_default_constructible_v<_Alloc>
  constexpr explicit polymorphic(in_place_type_t<_Up>,
                                 initializer_list<_Ip> __il, _Us &&...__us)
  ...
  constexpr polymorphic &operator=(const polymorphic &__other) {
    static_assert(sizeof(_Tp) != 0, "must be a complete type");
  ...
  constexpr polymorphic &operator=(polymorphic &&__other) noexcept(
      _ATraits::propagate_on_container_move_assignment::value ||
      _ATraits::is_always_equal::value) {
    if (__builtin_addressof(__other) == this) [[unlikely]]
      return *this;
  ...
  constexpr void swap(polymorphic &__other) noexcept(
      _ATraits::propagate_on_container_swap::value ||
      _ATraits::is_always_equal::value) {
    using std::swap;
    swap(_M_objp, __other._M_objp);
  ...
  friend constexpr void swap(polymorphic &__lhs, polymorphic &__rhs) noexcept(
      _ATraits::propagate_on_container_swap::value ||
      _ATraits::is_always_equal::value) {
    __lhs.swap(__rhs);
  }
...
private:
  ...
  static constexpr typename _Obj::pointer _S_make_obj(const _Alloc &__a,
                                                      _Args &&...__args) {
  ...
  template <typename _Up, typename... _Args>
  constexpr typename _Obj::pointer _M_make_obj(_Args &&...__args) const {
    return _S_make_obj<_Up>(_M_alloc, std::forward<_Args>(__args)...);
  }

  constexpr void _M_reset(typename _Obj::pointer __ptr) noexcept {
    if (_M_objp)
