...
namespace simd {
template <typename _Vp, typename _Tp> struct __vec_load_return {
  using type = _Vp;
};

template <typename _Tp> struct __vec_load_return<void, _Tp> {
...
unchecked_load(_Rg &&__r, flags<_Flags...> __f = {}) {
  ...
  static_assert(
      __loadstore_convertible_to<ranges::range_value_t<_Rg>, _Rp, _Flags...>,
  ...
  if constexpr (!__allow_out_of_bounds && __static_sized_range<_Rg>)
    static_assert(ranges::size(__r) >= _RV::size(),
  ...
  if consteval {
    ...
    if constexpr ((__static_size != dynamic_extent &&
                   __static_size >= size_t(_RV::size())) ||
...
template <typename _Vp = void, __sized_contiguous_range _Rg, typename... _Flags>
[[__gnu__::__always_inline__]]
constexpr __vec_load_return_t<_Vp, ranges::range_value_t<_Rg>> unchecked_load(
    _Rg &&__r,
  ...
  if consteval {
    ...
    constexpr bool __no_size_check =
        !__allow_out_of_bounds || (__static_size != dynamic_extent &&
                                   __static_size >= size_t(_RV::size.value));
    if constexpr (_RV::size() == 1)
      return __mask[0] && (__no_size_check || __rg_size > 0)
                 ? _RV(_LoadCtorTag(), __ptr)
                 : _RV();
      ...
      return _RV::_S_masked_load(
          __ptr,
          __mask && _RV::mask_type::_S_partial_mask_of_n(int(__rg_size)));
    else
      return _RV();
  }
}
...
unchecked_load(_It __first, iter_difference_t<_It> __n,
               flags<_Flags...> __f = {}) {
  return simd::unchecked_load<_Vp>(span<const iter_value_t<_It>>(__first, __n),
                                   __f);
}
...
template <typename _Vp = void, contiguous_iterator _It,
          sized_sentinel_for<_It> _Sp, typename... _Flags>
[[__gnu__::__always_inline__]]
constexpr __vec_load_return_t<_Vp, iter_value_t<_It>>
unchecked_load(_It __first, _Sp __last, flags<_Flags...> __f = {}) {
  return simd::unchecked_load<_Vp>(
      span<const iter_value_t<_It>>(__first, __last), __f);
}
...
unchecked_load(_It __first, _Sp __last,
               ...
               flags<_Flags...> __f = {}) {
  return simd::unchecked_load<_Vp>(
      span<const iter_value_t<_It>>(__first, __last), __mask, __f);
}
...
partial_load(_Rg &&__r,
             const __load_mask_type_t<_Vp, ranges::range_value_t<_Rg>> &__mask,
             flags<_Flags...> __f = {}) {
  return simd::unchecked_load<_Vp>(__r, __mask,
                                   __f | __allow_partial_loadstore);
}
...
partial_load(_It __first, iter_difference_t<_It> __n,
             const __load_mask_type_t<_Vp, iter_value_t<_It>> &__mask,
             flags<_Flags...> __f = {}) {
  return partial_load<_Vp>(span<const iter_value_t<_It>>(__first, __n), __mask,
                           __f);
}
...
partial_load(_It __first, _Sp __last,
             const __load_mask_type_t<_Vp, iter_value_t<_It>> &__mask,
             flags<_Flags...> __f = {}) {
  return partial_load<_Vp>(span<const iter_value_t<_It>>(__first, __last),
                           __mask, __f);
}

template <typename _Tp, typename _Ap, __sized_contiguous_range _Rg,
          typename... _Flags>
  requires indirectly_writable<ranges::iterator_t<_Rg>, _Tp>
[[__gnu__::__always_inline__]]
constexpr void unchecked_store(const basic_vec<_Tp, _Ap> &__v, _Rg &&__r,
                               flags<_Flags...> __f = {}) {
  using _TV = basic_vec<_Tp, _Ap>;
  static_assert(destructible<_TV>);
  static_assert(
      __loadstore_convertible_to<_Tp, ranges::range_value_t<_Rg>, _Flags...>,
      "'flag_convert' must be used for conversions that are not "
      "value-preserving");

  constexpr bool __allow_out_of_bounds = __f._S_test(__allow_partial_loadstore);
  if constexpr (!__allow_out_of_bounds && __static_sized_range<_Rg>)
    static_assert(ranges::size(__r) >= _TV::size(),
                  "given range must have sufficient size");
...
template <typename _Tp, typename _Ap, contiguous_iterator _It,
          typename... _Flags>
  requires indirectly_writable<_It, _Tp>
[[__gnu__::__always_inline__]]
constexpr void unchecked_store(const basic_vec<_Tp, _Ap> &__v, _It __first,
...
unchecked_store(const basic_vec<_Tp, _Ap> &__v, _It __first,
                ...
                const typename basic_vec<_Tp, _Ap>::mask_type &__mask,
                flags<_Flags...> __f = {}) {
  simd::unchecked_store(__v, std::span<iter_value_t<_It>>(__first, __n), __mask,
                        __f);
}
...
[[__gnu__::__always_inline__]]
constexpr void unchecked_store(const basic_vec<_Tp, _Ap> &__v, _It __first,
                               _Sp __last, flags<_Flags...> __f = {}) {
  simd::unchecked_store(__v, std::span<iter_value_t<_It>>(__first, __last),
                        __f);
}
...
template <typename _Tp, typename _Ap, __sized_contiguous_range _Rg,
  ...
  requires indirectly_writable<ranges::iterator_t<_Rg>, _Tp>
[[__gnu__::__always_inline__]]
constexpr void partial_store(const basic_vec<_Tp, _Ap> &__v, _Rg &&__r,
                             flags<_Flags...> __f = {}) {
...
template <typename _Tp, typename _Ap, contiguous_iterator _It,
  ...
  requires indirectly_writable<_It, _Tp>
[[__gnu__::__always_inline__]]
constexpr void partial_store(const basic_vec<_Tp, _Ap> &__v, _It __first,
                             iter_difference_t<_It> __n,
                             flags<_Flags...> __f = {}) {
...
[[__gnu__::__always_inline__]]
constexpr void
partial_store(const basic_vec<_Tp, _Ap> &__v, _It __first,
              iter_difference_t<_It> __n,
              const typename basic_vec<_Tp, _Ap>::mask_type &__mask,
              flags<_Flags...> __f = {}) {
