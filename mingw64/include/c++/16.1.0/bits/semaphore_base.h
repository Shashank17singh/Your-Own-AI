...
struct __semaphore_impl {
  ...
  constexpr explicit __semaphore_impl(__count_type __count) noexcept
      : _M_counter(__count) {}
  ...
  _GLIBCXX_ALWAYS_INLINE __count_type _M_get_current() const noexcept {
  ...
  // value.
  _GLIBCXX_ALWAYS_INLINE bool _M_do_try_acquire(__count_type &__cur) noexcept {
    if (__cur == 0)
      return false; // Cannot decrement when it's already zero.

    return __atomic_impl::compare_exchange_strong(&_M_counter, __cur, __cur - 1,
  ...
  bool _M_try_acquire() noexcept {
    ...
    if (std::__atomic_wait_address_for(&_M_counter, __is_available, __vfn,
                                       __zero, true))
  ...
  _GLIBCXX_ALWAYS_INLINE ptrdiff_t _M_release(ptrdiff_t __update) noexcept {
    auto __old =
        __atomic_impl::fetch_add(&_M_counter, __update, memory_order::release);
...
template <bool _Binary> struct __platform_semaphore_impl {
  using __count_type = __detail::__platform_wait_t;

  static consteval ptrdiff_t _S_calc_max() {
    if (_Binary)
      return 1;
  ...
  constexpr explicit __platform_semaphore_impl(__count_type __count) noexcept
      : _M_counter(__count) {}

  __platform_semaphore_impl(__platform_semaphore_impl &) = delete;
  __platform_semaphore_impl &
  operator=(const __platform_semaphore_impl &) = delete;
  ...
  bool _M_try_acquire_until(
    ...
    while (!_M_do_try_acquire(__val))
      if (__val == 0) {
        if (!std::__atomic_wait_address_until_v(
                &_M_counter, 0, __ATOMIC_ACQUIRE, __atime, true))
          return false; // timed out
        __val = _M_get_current();
      }
    return true;
  ...
  _M_try_acquire_for(const chrono::duration<_Rep, _Period> &__rtime) noexcept {
    ...
    while (!_M_do_try_acquire(__val))
      if (__val == 0) {
        if (!std::__atomic_wait_address_for_v(&_M_counter, 0, __ATOMIC_ACQUIRE,
                                              __rtime, true))
          return false; // timed out
        __val = _M_get_current();
      }
...
using _Semaphore_impl =
    __conditional_t<__platform_wait_uses_type<_Tp> &&
                    ...
                    __platform_semaphore_impl<(_Max <= 1)>, __semaphore_impl>;
