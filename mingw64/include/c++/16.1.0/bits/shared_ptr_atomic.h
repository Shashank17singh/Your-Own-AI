...
inline bool atomic_compare_exchange_weak_explicit(shared_ptr<_Tp> *__p,
  ...
  return std::atomic_compare_exchange_strong_explicit(__p, __v, std::move(__w),
                                                      __success, __failure);
}
...
template <typename _Tp> class _Sp_atomic {
  ...
  struct _Atomic_count {
    ...
    pointer lock(memory_order __o) const noexcept {
      ...
      while (!__aref.compare_exchange_strong(__current, __current | _S_lock_bit,
...
#if __glibcxx_atomic_wait
    // Precondition: caller holds lock!
    void _M_wait_unlock(const element_type *const &__ptr,
                        memory_order __o) const noexcept {
...
public:
  ...
  shared_ptr<_Tp> load(memory_order __o = memory_order_seq_cst) const noexcept {
  ...
  compare_exchange_strong(value_type &__expected, value_type __desired,
    ...
    return compare_exchange_strong(__expected, std::move(__desired), __o, __o2);
  }
