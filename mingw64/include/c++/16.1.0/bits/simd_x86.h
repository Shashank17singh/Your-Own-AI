 ...
 *
 * Caveats:
 * 1. The bit-mask of 2-Byte vector-masks has duplicated entries (because of
 * missing instruction)
 * 2. The return type internally is 'int', but that fails on conversion to
 * uint64 if the MSB of a YMM 1/2-Byte vector-mask is set (sign extension).
 * Therefore these helper functions return unsigned instead.
...
[[__gnu__::__always_inline__]] inline _Bitmask<8>
  ...
  if constexpr (_Traits._M_have_bmi2())
    return _Bitmask<8>(__builtin_ia32_pext_di(
        __builtin_bit_cast(unsigned long long, __x), 0x80000000'80000000ULL));
#endif
...
[[__gnu__::__always_inline__]]
inline unsigned __x86_movmsk(_TV __x) {
  ...
  if constexpr (sizeof(__x) == 32)
    ...
    if constexpr (_Traits._M_have_bmi2())
      return __builtin_ia32_pext_si(__builtin_bit_cast(unsigned int, __x),
                                    0x80808080u);
#endif
    return __x86_movmsk(__vec_zero_pad_to_16(__x));
...
[[__gnu__::__always_inline__]] inline bool __x86_vec_is_zero(_TV __a) {
...
template <__vec_builtin _TV, _ArchTraits _Traits = {}>
[[__gnu__::__always_inline__]] inline int __x86_vec_testz(_TV __a, _TV __b) {
...
template <__vec_builtin _TV, _ArchTraits _Traits = {}>
[[__gnu__::__always_inline__]] inline int __x86_vec_testc(_TV __a, _TV __b) {
  static_assert(sizeof(_TV) == 16 || sizeof(_TV) == 32);
  static_assert(_Traits._M_have_sse4_1());
  if constexpr (sizeof(_TV) == 32)
...
[[__gnu__::__always_inline__]] inline bool __x86_vecmask_all(_TV __k) {
  using _Tp = __vec_value_type<_TV>;
  ...
  if constexpr (_Np == __width) {
    if constexpr (sizeof(__k) <= __x86_max_general_register_size) {
      ...
      constexpr unsigned __valid_bits =
          (1u << (sizeof(_Tp) == 2 ? _Np * 2 : _Np)) - 1;
    ...
    using _Ip = __integer_from<sizeof(__k)>;
    constexpr _Ip __valid_bits =
        (_Ip(1) << (_Np * sizeof(_Tp) * __CHAR_BIT__)) - 1;
...
template <int _Np, __vec_builtin _TV, _ArchTraits _Traits = {}>
[[__gnu__::__always_inline__]] inline bool __x86_vecmask_any(_TV __k) {
  using _Tp = __vec_value_type<_TV>;
  static_assert(is_integral_v<_Tp> && is_signed_v<_Tp>);
  constexpr int __width = __width_of<_TV>;
  static_assert(sizeof(__k) <= 32);
...
template <int _Np, __vec_builtin _TV, _ArchTraits _Traits = {}>
[[__gnu__::__always_inline__]] inline bool __x86_vecmask_none(_TV __k) {
  using _Tp = __vec_value_type<_TV>;
  static_assert(is_integral_v<_Tp> && is_signed_v<_Tp>);
  constexpr int __width = __width_of<_TV>;
  static_assert(sizeof(__k) <= 32);
...
[[__gnu__::__always_inline__]] inline auto __x86_bitmask_cmp(_TV __x, _TV __y) {
...
using __x86_intrin_int = decltype([] {
...
template <typename _Tp>
using __x86_intrin_type = decltype([] {
...
template <typename _Tp>
using __x86_intel_intrin_value_type = decltype([] {
...
[[__gnu__::__always_inline__]]
constexpr _TV __vec_andnot(_TV __a, _TV __b) {
  ...
  if (__builtin_is_constant_evaluated() ||
    ...
    return reinterpret_cast<_TV>(~reinterpret_cast<_UV>(__a) &
                                 reinterpret_cast<_UV>(__b));
  else if constexpr (is_same_v<_Tp, _Float16>)
    return reinterpret_cast<_TV>(
        __vec_andnot(__vec_bit_cast<float>(__a), __vec_bit_cast<float>(__b)));
  ...
  else if constexpr (sizeof(_TV) == 64 && is_same_v<_Tp, float> &&
                     _Traits._M_have_avx512dq())
    return __builtin_ia32_andnps512_mask(__a, __b, _TV{}, -1);
  else if constexpr (sizeof(_TV) == 64 && is_same_v<_Tp, double> &&
                     _Traits._M_have_avx512dq())
    ...
    if constexpr (sizeof(_TV) == 16 && is_integral_v<_Tp>)
      ...
      return reinterpret_cast<_TV>(__builtin_ia32_andnpd256(
          __vec_bit_cast<double>(__a), __vec_bit_cast<double>(__b)));
...
template <__vec_builtin _TV, _ArchTraits _Traits = {}>
[[__gnu__::__always_inline__]] inline auto __x86_bitmask_isinf(_TV __x) {
...
[[__gnu__::__always_inline__]] inline _KV
...
template <unsigned_integral _Kp, __vec_builtin _TV, _ArchTraits _Traits = {}>
  requires is_integral_v<__vec_value_type<_TV>>
[[__gnu__::__always_inline__]] constexpr inline _TV
    __x86_bitmask_blend(_Kp __k, _TV __t, _TV __f) {
  using _Tp = __vec_value_type<_TV>;
  using _Ip = __x86_intrin_int<_Tp>;
  if constexpr (!is_same_v<_Ip, _Tp>)
    return reinterpret_cast<_TV>(__x86_bitmask_blend(
        __k, __vec_bit_cast<_Ip>(__t), __vec_bit_cast<_Ip>(__f)));
    ...
    return _VecOps<_TV>::_S_extract(__x86_bitmask_blend(
        __k, __vec_zero_pad_to_16(__t), __vec_zero_pad_to_16(__f)));
  else
    static_assert(false);
}
...
[[__gnu__::__always_inline__]] constexpr inline _TV
  ...
  if constexpr (sizeof(_TV) == 64 && sizeof(_Tp) == 8)
    ...
    return __vec_bit_cast<_Float16>(__x86_bitmask_blend(
        __k, __vec_bit_cast<_Up>(__t), __vec_bit_cast<_Up>(__f)));
  } else if constexpr (sizeof(_TV) < 16)
    return _VecOps<_TV>::_S_extract(__x86_bitmask_blend(
        __k, __vec_zero_pad_to_16(__t), __vec_zero_pad_to_16(__f)));
  else
...
constexpr _Bitmask<1> __bit_extract_even(_UInt<1> __x) {
...
constexpr _Bitmask<1> __bit_extract_even(_UInt<2> __x) {
...
constexpr _Bitmask<_OutputBits> __bit_extract_even(_UInt<4> __x) {
  if constexpr (_OutputBits <= 4)
    return __bit_extract_even<_OutputBits>(_UInt<1>(__x));
...
constexpr _Bitmask<_OutputBits> __bit_extract_even(_UInt<8> __x) {
  if constexpr (_OutputBits <= 4)
    return __bit_extract_even<_OutputBits>(_UInt<1>(__x));
  else if constexpr (_OutputBits <= 8)
    return __bit_extract_even<_OutputBits>(_UInt<2>(__x));
    ...
    constexpr _UInt<8> __mask =
        0x5555'5555'5555'5555ull >> ((32 - _OutputBits) * 2);
#if __has_builtin(__builtin_ia32_pext_si)
    if constexpr (_Traits._M_have_bmi2()) {
#if __has_builtin(__builtin_ia32_pext_di)
...
constexpr auto __duplicate_each_bit(unsigned_integral auto __x) {
  constexpr int __input_bits =
      _InputBits == -1 ? sizeof(__x) * __CHAR_BIT__ : _InputBits;
  ...
  if constexpr (__input_bits <= 8) {
    ...
    if constexpr (__input_bits == 1)
      ...
      return _Bitmask<__input_bits * 2>(3u *
                                        __builtin_ia32_pdep_si(__x, __mask));
#endif
    ...
    constexpr _UInt<8> __mask =
        0x5555'5555'5555'5555u >> ((32 - __input_bits) * 2);
#if __has_builtin(__builtin_ia32_pdep_si)
    if constexpr (_Traits._M_have_bmi2()) {
      ...
      return (3u * __builtin_ia32_pdep_si(__x, static_cast<unsigned>(__mask))) |
             __hi << 32;
#endif
    }
#endif
...
constexpr auto __duplicate_each_bit(const __trivial_pair<_U0, _U1> &__x) {
  static_assert(_InputBits != -1 || is_unsigned_v<_U1>);
  constexpr int __input_bits = _InputBits == -1
                                   ? (sizeof(_U0) + sizeof(_U1)) * __CHAR_BIT__
                                   : _InputBits;
...
template <__vec_builtin _TV, _ArchTraits _Traits = {}>
[[__gnu__::__always_inline__]] inline _TV __x86_complex_multiplies(_TV __x,
                                                                   _TV __y) {
  using _Tp = __vec_value_type<_TV>;
  ...
  if constexpr (!_Traits._M_have_avx512fp16() && sizeof(_Tp) == 2)
    ...
    if constexpr (sizeof(__x) == 16 && sizeof(_Tp) == 2)
      return __builtin_ia32_vfmaddsubph128_mask(__x_real, __y,
                                                __x_imag * __y_swapped, -1);
    else if constexpr (sizeof(__x) == 32 && sizeof(_Tp) == 2)
      return __builtin_ia32_vfmaddsubph256_mask(__x_real, __y,
                                                __x_imag * __y_swapped, -1);
      ...
      return __builtin_ia32_vfmaddsubps256(__x_real, __y,
                                           __x_imag * __y_swapped);
      ...
      return __builtin_ia32_vfmaddsubpd256(__x_real, __y,
                                           __x_imag * __y_swapped);
...
[[__gnu__::__always_inline__]]
inline _UV __x86_cvt_f16c(_TV __v) {
  ...
  if constexpr (__to_f16 && !is_same_v<__vec_value_type<_TV>, float>)
    ...
    return __vec_cast<_UV>(
        __x86_cvt_f16c<__vec_builtin_type<float, __width_of<_TV>>>(__v));
  else if constexpr (__from_f16) {
    const auto __vi = __vec_bit_cast<__x86_intrin_int<_Float16>>(__v);
    if constexpr (sizeof(_TV) == 4)
      return __vec_split_lo(
          __builtin_ia32_vcvtph2ps(__vec_zero_pad_to_16(__vi)));
      ...
      return __builtin_ia32_vcvtph2ps512_mask(
          __vi, __vec_builtin_type<float, 16>(), -1, 4);
    else if constexpr (sizeof(_TV) >= 64)
      return __vec_concat(
          __x86_cvt_f16c<__half_vec_builtin_t<_UV>>(__vec_split_lo(__v)),
          __x86_cvt_f16c<__half_vec_builtin_t<_UV>>(__vec_split_hi(__v)));
    else
      static_assert(false);
  } else if constexpr (sizeof(_TV) == 8)
    return reinterpret_cast<_UV>(__vec_split_lo(__vec_split_lo(
        __builtin_ia32_vcvtps2ph(__vec_zero_pad_to_16(__v), 4))));
  else if constexpr (sizeof(_TV) == 16)
    return reinterpret_cast<_UV>(
        __vec_split_lo(__builtin_ia32_vcvtps2ph(__v, 4)));
  else if constexpr (sizeof(_TV) == 32)
...
/** @internal
 * AVX instructions typically work per 128-bit chunk. Horizontal operations thus
 * produce vectors where the two 128-bit chunks in the center are swapped. This
 * function works as a fix-up step.
 */
template <__vec_builtin _TV>
[[__gnu__::__always_inline__]]
inline _TV __x86_swizzle4x64_acbd(_TV __x) {
  static_assert(sizeof(_TV) == 32);
  using _UV = __vec_builtin_type_bytes<long long, 32>;
  return reinterpret_cast<_TV>(
      __builtin_shufflevector(reinterpret_cast<_UV>(__x), _UV(), 0, 2, 1, 3));
}

/** @internal
 * Like __builtin_convertvector but with a precondition that input values are
 * either 0 or -1.
 */
template <__vec_builtin _To, __vec_builtin _From>
[[__gnu__::__always_inline__]]
inline _To __x86_cvt_vecmask(_From __k) {
  using _T0 = __vec_value_type<_From>;
  using _T1 = __vec_value_type<_To>;
  if constexpr (sizeof(_From) > sizeof(_To) && sizeof(_From) < 16) {
    using _ToPadded =
        __vec_builtin_type_bytes<_T1, sizeof(_To) * 16 / sizeof(_From)>;
    return _VecOps<_To>::_S_extract(
        __x86_cvt_vecmask<_ToPadded>(__vec_zero_pad_to_16(__k)));
  } else if constexpr (sizeof(_T0) == 2 && sizeof(_T1) == 1) // -> packsswb
  {
    if constexpr (sizeof(__k) == 16)
      return reinterpret_cast<_To>(
          __vec_split_lo(__builtin_ia32_packsswb128(__k, __k)));
    else if constexpr (sizeof(__k) == 32)
      return reinterpret_cast<_To>(__vec_split_lo(
          __x86_swizzle4x64_acbd(__builtin_ia32_packsswb256(__k, __k))));
    else
      static_assert(false);
  } else
...
template <__vec_builtin _To, __vec_builtin _From>
[[__gnu__::__always_inline__]]
inline _To __x86_cvt_vecmask(_From __k0, _From __k1) {
  using _T0 = __vec_value_type<_From>;
  using _T1 = __vec_value_type<_To>;
  ...
  if constexpr (sizeof(_T0) == 2 && sizeof(_T1) == 1) // -> packsswb
  {
    if constexpr (sizeof(__k0) == 16)
      ...
      return reinterpret_cast<_To>(
          __x86_swizzle4x64_acbd(__builtin_ia32_packsswb256(__k0, __k1)));
    else
      static_assert(false);
  } else
    static_assert(false, "TODO");
...
[[__gnu__::__always_inline__]] inline _TV
  ...
  if constexpr (!__converts_trivially<_Up, _Tp>) {
    const auto __uvec =
        __x86_masked_load<__vec_builtin_type<__canonical_vec_type_t<_Up>, __n>>(
            __mem, __k);
    ...
    return __vec_concat(
        ...
        __x86_masked_load<__vec_builtin_type<_Tp, __n / 2>>(__mem + __n / 2,
                                                            __k >> __n / 2));
  } else if constexpr (sizeof(_TV) == 64) {
    ...
    if constexpr (is_floating_point_v<_Tp> && sizeof(_Tp) == 4)
      ...
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddquqi512_mask(__src, __z, __k));
    else if constexpr (sizeof(_Tp) == 2)
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddquhi512_mask(__src, __z, __k));
    else if constexpr (sizeof(_Tp) == 4)
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddqusi512_mask(__src, __z, __k));
    else if constexpr (sizeof(_Tp) == 8)
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddqudi512_mask(__src, __z, __k));
    else
      static_assert(false);
  } else if constexpr (sizeof(_TV) == 32) {
    const auto *__src = reinterpret_cast<const __x86_intrin_type<_Up> *>(__mem);
    ...
    if constexpr (is_floating_point_v<_Tp> && sizeof(_Tp) == 4)
      ...
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddquqi256_mask(__src, __z, __k));
    else if constexpr (sizeof(_Tp) == 2)
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddquhi256_mask(__src, __z, __k));
    else if constexpr (sizeof(_Tp) == 4)
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddqusi256_mask(__src, __z, __k));
    else if constexpr (sizeof(_Tp) == 8)
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddqudi256_mask(__src, __z, __k));
    else
      static_assert(false);
  } else if constexpr (sizeof(_TV) == 16) {
    const auto *__src = reinterpret_cast<const __x86_intrin_type<_Up> *>(__mem);
    ...
    if constexpr (is_floating_point_v<_Tp> && sizeof(_Tp) == 4)
      ...
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddquqi128_mask(__src, __z, __k));
    else if constexpr (sizeof(_Tp) == 2)
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddquhi128_mask(__src, __z, __k));
    else if constexpr (sizeof(_Tp) == 4)
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddqusi128_mask(__src, __z, __k));
    else if constexpr (sizeof(_Tp) == 8)
      return reinterpret_cast<_TV>(
          __builtin_ia32_loaddqudi128_mask(__src, __z, __k));
    else
      static_assert(false);
  } else
    static_assert(false);
 ...
 */
template <__vec_builtin _TV, typename _Up, __vec_builtin _KV,
          _ArchTraits _Traits = {}>
[[__gnu__::__always_inline__]] inline _TV __x86_masked_load(const _Up *__mem,
                                                            const _KV __k) {
  using _Tp = __vec_value_type<_TV>;
  static_assert(_Traits._M_have_avx() && __converts_trivially<_Up, _Tp> &&
                sizeof(_Up) >= 4);
  constexpr int __n = __width_of<_TV>;
  ...
  if constexpr (sizeof(_TV) < 16)
    return _VecOps<_TV>::_S_extract(
        __x86_masked_load<__vec_builtin_type_bytes<_Tp, 16>>(
            __mem, __vec_zero_pad_to_16(__k)));
  else if constexpr (_Traits._M_have_avx2() && is_integral_v<_Up>) {
    const auto *__src = reinterpret_cast<
        const __vec_builtin_type<__x86_intrin_int<_Up>, __n> *>(__mem);
    ...
    const auto *__src =
        reinterpret_cast<const __vec_builtin_type<float, __n> *>(__mem);
    if constexpr (sizeof(_TV) == 32)
    ...
    const auto *__src =
        reinterpret_cast<const __vec_builtin_type<double, __n> *>(__mem);
    if constexpr (sizeof(_TV) == 32)
...
[[__gnu__::__always_inline__]]
inline void __x86_masked_store(const _TV __v, _Up *__mem,
                               unsigned_integral auto __k) {
  using _Tp = __vec_value_type<_TV>;
  constexpr int __n = __width_of<_TV>;
  [[maybe_unused]] const auto __w = __vec_bit_cast<__x86_intrin_type<_Tp>>(__v);
  if constexpr (sizeof(_TV) == 64) {
    if constexpr (sizeof(_Tp) > sizeof(_Up) && is_integral_v<_Tp> &&
                  is_integral_v<_Up>) {
      auto *__dst =
          reinterpret_cast<__vec_builtin_type<__x86_intrin_int<_Up>, __n> *>(
              __mem);
      ...
      if constexpr (is_floating_point_v<_Tp> && is_integral_v<_Up> &&
        ...
        __x86_masked_store(__vec_cast<__integer_from<sizeof(_Tp)>>(__v), __mem,
                           __k);
      else
    ...
    if constexpr (sizeof(_Tp) > sizeof(_Up) && is_integral_v<_Tp> &&
      ...
      if constexpr (sizeof(_Tp) == 2)
        ...
        __builtin_ia32_pmovqd128mem_mask(
            reinterpret_cast<unsigned long long *>(__mem), __w, __k);
      else
        static_assert(false);
    } else if constexpr (__converts_trivially<_Tp, _Up>) {
      auto *__dst = reinterpret_cast<__x86_intrin_type<_Up> *>(__mem);
...
[[__gnu__::__always_inline__]] inline void
  ...
  auto *__dst =
      reinterpret_cast<__vec_builtin_type<__x86_intrin_type<_Up>, __n> *>(
          __mem);
  [[maybe_unused]] const auto __w = __vec_bit_cast<__x86_intrin_type<_Tp>>(__v);
  if constexpr (sizeof(_TV) < 16)
    __x86_masked_store(__vec_zero_pad_to_16(__v), __mem,
                       __vec_zero_pad_to_16(__k));
