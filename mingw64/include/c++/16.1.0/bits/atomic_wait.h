...
inline constexpr bool __platform_wait_uses_type =
    __detail::__waitable<_Tp> && sizeof(_Tp) == sizeof(int) &&
    alignof(_Tp) >= 4;
...
template <typename _Tp>
inline constexpr bool __platform_wait_uses_type =
    __detail::__waitable<_Tp> && ((sizeof(_Tp) == 4 && alignof(_Tp) >= 4) ||
                                  (sizeof(_Tp) == 8 && alignof(_Tp) >= 8));
#else
...
// futex-like syscall.
template <typename> inline constexpr bool __platform_wait_uses_type = false;
#endif
...
template <typename _Tp>
inline bool __atomic_eq(const _Tp &__a, const _Tp &__b) {
...
[[__gnu__::__always_inline__]]
constexpr __wait_flags &operator|=(__wait_flags &__l,
                                   __wait_flags __r) noexcept {
  return __l = __l | __r;
}
...
struct __wait_args_base {
  ...
  bool operator&(__wait_flags __flags) const noexcept {
    using _Ut = underlying_type_t<__wait_flags>;
...
struct __wait_args : __wait_args_base {
  ...
  explicit __wait_args(const _Tp *__addr, bool __bare_wait = false) noexcept
  ...
  _Tp _M_setup_wait(const _Tp *__addr, _ValFn __vfn) {
  ...
  template <typename _Tp, typename _ValFn>
  _Tp _M_on_wake(const _Tp *__addr, _ValFn __vfn, __wait_result_type __res) {
...
private:
  ...
  bool _M_setup_proxy_wait(const void *__addr);

  template <typename _Tp>
  static constexpr __wait_flags _S_flags_for(const _Tp *,
                                             bool __bare_wait) noexcept {
...
void __atomic_wait_address(const _Tp *__addr, _Pred &&__pred, _ValFn &&__vfn,
...
inline void __atomic_wait_address_v(const __detail::__platform_wait_t *__addr,
...
template <typename _Tp>
void __atomic_notify_address(const _Tp *__addr, bool __all,
                             bool __bare_wait = false) noexcept {
  __detail::__wait_args __args{__addr, __bare_wait};
