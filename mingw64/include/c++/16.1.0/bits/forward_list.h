...
private:
  ...
  template <typename, typename> friend struct _Fwd_list_base;
...
private:
  ...
  _Fwd_list_iterator<_Tp> _M_const_cast() const noexcept {
    return _Fwd_list_iterator<_Tp>(const_cast<_Fwd_list_node_base *>(_M_node));
  }
...
template <typename _VoidPtr> struct _Node_base {
  ...
  _Node_base(_Node_base &&__x) noexcept : _M_next(__x._M_next) {
    __x._M_next = nullptr;
  }
...
struct _Node : public _Node_base<__ptr_rebind<_ValPtr, void>> {
...
private:
  ...
  constexpr _Iterator _M_next() const noexcept {
...
public:
  ...
  ~_Fwd_list_base() { _M_erase_after(_M_impl._M_head._M_base_ptr(), nullptr); }
...
public:
  ...
  forward_list(forward_list &&__list,
               const __type_identity_t<_Alloc>
...
#pragma GCC diagnostic ignored "-Wc++17-extensions" // if constexpr
  ...
  forward_list &operator=(forward_list &&__list) noexcept(
    ...
    if constexpr (_Node_alloc_traits::_S_propagate_on_move_assign())
      this->_M_get_Node_allocator() = std::move(__list._M_get_Node_allocator());
    return *this;
  }
...
#pragma GCC diagnostic pop
  ...
  void assign_range(_Rg &&__rg) {
    ...
    if (__curr != __end)
      ...
      insert_range_after(__prev, ranges::subrange(std::move(__first), __last));
  }
#endif // containers_ranges
...
#pragma GCC diagnostic pop
  ...
  void prepend_range(_Rg &&__rg) {
    forward_list __tmp(from_range, std::forward<_Rg>(__rg), get_allocator());
    if (!__tmp.empty())
   ...
   */
  template <__detail::__container_compatible_range<_Tp> _Rg>
  iterator insert_range_after(const_iterator __position, _Rg &&__rg) {
    forward_list __tmp(from_range, std::forward<_Rg>(__rg), get_allocator());
...
inline bool operator<(const forward_list<_Tp, _Alloc> &__lx,
                      const forward_list<_Tp, _Alloc> &__ly) {
  return std::lexicographical_compare(__lx.cbegin(), __lx.cend(), __ly.cbegin(),
                                      __ly.cend());
}
