...
template <typename _Out, typename _Tp> struct out_value_result {
  ...
  constexpr operator out_value_result<_Out2, _Tp2>() const & {
    return {out, value};
  }

  template <typename _Out2, typename _Tp2>
    requires convertible_to<_Out, _Out2> && convertible_to<_Tp, _Tp2>
  constexpr operator out_value_result<_Out2, _Tp2>() && {
    ...
    if constexpr (__detail::__is_normal_iterator<_Iter1> &&
      ...
      constexpr bool __use_memcmp =
          ((is_integral_v<_ValueType1> || is_pointer_v<_ValueType1>) &&
           ...
           is_same_v<_Pred, ranges::equal_to> && is_same_v<_Proj1, identity> &&
...
template <bool _IsMove, bidirectional_iterator _Iter, sentinel_for<_Iter> _Sent,
...
__copy_or_move(_Iter __first, _Sent __last, _Out __result) {
  ...
  if constexpr (__is_move_iterator<_Iter> && same_as<_Iter, _Sent>) {
    auto [__in, __out] = ranges::__copy_or_move<true>(std::move(__first).base(),
...
struct __copy_fn {
  ...
  constexpr copy_result<_Iter, _Out> operator()(_Iter __first, _Sent __last,
                                                _Out __result) const {
    return ranges::__copy_or_move<false>(std::move(__first), std::move(__last),
                                         std::move(__result));
  }
...
struct __fill_fn {
  template <typename _Out, sentinel_for<_Out> _Sent,
            typename _Tp _GLIBCXX26_DEF_VAL_T(iter_value_t<_Out>)>
    requires output_iterator<_Out, const _Tp &>
