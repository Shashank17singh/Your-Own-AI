...
template <typename _BiIter,
          bool _Trivial = is_trivially_copyable<_BiIter>::value>
...
public:
  _Executor(_BiIter __begin, _BiIter __end, _ResultsVec &__results,
            const _RegexT &__re, _FlagT __flags, bool __use_dfs)
      : _M_cur_results(__results.get_allocator()), _M_begin(__begin),
        _M_end(__end), _M_re(__re), _M_nfa(*__re._M_automaton),
        _M_results(__results), _M_rep_count(_M_nfa.size()),
        _M_start(_M_nfa._M_start()), _M_visited_states(nullptr),
        _M_flags(__flags),
  ...
  ~_Executor() { delete[] _M_visited_states; }
...
private:
  ...
  void _M_node(_Match_mode, _StateIdT);
  ...
  bool _M_main_dfs(_Match_mode __match_mode);

  bool _M_main_bfs(_Match_mode __match_mode);
  ...
  bool _M_is_line_terminator(_CharT __c) const {
  ...
  bool _M_match_multiline() const noexcept {
    constexpr auto __m =
