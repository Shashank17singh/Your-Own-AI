...
inline _Tp *__get_temporary_buffer(ptrdiff_t __len) _GLIBCXX_NOTHROW {
  ...
  if (alignof(_Tp) > __STDCPP_DEFAULT_NEW_ALIGNMENT__)
    return (_Tp *)_GLIBCXX_OPERATOR_NEW(__len * sizeof(_Tp),
                                        align_val_t(alignof(_Tp)), nothrow_t());
#endif
 ...
 */
template <typename _Tp>
_GLIBCXX17_DEPRECATED pair<_Tp *, ptrdiff_t>
 ...
 */
template <typename _Tp>
_GLIBCXX17_DEPRECATED inline void return_temporary_buffer(_Tp *__p) {
#if __cpp_aligned_new && __cplusplus >= 201103L
  if (alignof(_Tp) > __STDCPP_DEFAULT_NEW_ALIGNMENT__)
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    explicit _Impl(ptrdiff_t __original_len) {
...
#pragma GCC diagnostic pop
    ...
    ~_Impl() { std::__detail::__return_temporary_buffer(_M_buffer, _M_len); }
...
public:
  /// As per Table mumble.
  size_type size() const { return _M_impl._M_len; }

  /// Returns the size requested by the constructor; may be >size().
  size_type _M_requested_size() const { return _M_original_len; }

  /// As per Table mumble.
  iterator begin() { return _M_impl._M_buffer; }

  /// As per Table mumble.
  iterator end() { return _M_impl._M_buffer + _M_impl._M_len; }
...
inline void __uninitialized_construct_buf(_Tp *__first, _Tp *__last,
                                          _ForwardIterator __seed) {
  std::__uninitialized_construct_buf_dispatch<__has_trivial_constructor(
      _Tp)>::__ucr(__first, __last, __seed);
}
