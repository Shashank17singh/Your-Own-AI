...
#else
template <typename _Tp>
using _BoundedArray = __enable_if_t<__is_array_known_bounds<_Tp>::value, _Tp>;
#endif
...
private:
  ...
  shared_ptr(const _Sp_counted_array_base<_Alloc> &__a, _Init __init = nullptr)
...
struct owner_equal {
  ...
  bool operator()(const shared_ptr<_Tp1> &__lhs,
  ...
  template <typename _Tp1, typename _Tp2>
  bool operator()(const weak_ptr<_Tp1> &__lhs,
                  const shared_ptr<_Tp2> &__rhs) const noexcept {
    return __lhs.owner_equal(__rhs);
  }
...
auto __make_shared_arr_tag(size_t __n, const _Alloc &__a = _Alloc()) noexcept {
...
template <typename _Tp>
inline shared_ptr<_UnboundedArray<_Tp>> make_shared_for_overwrite(size_t __n) {
  return shared_ptr<_Tp>(std::__make_shared_arr_tag<_Tp>(__n),
                         _Sp_overwrite_tag{});
}
#endif // smart_ptr_for_overwrite
...
template <typename _Tp> constexpr bool __is_shared_ptr = false;
template <typename _Tp> constexpr bool __is_shared_ptr<shared_ptr<_Tp>> = true;
#endif
