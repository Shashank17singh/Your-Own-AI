...
constexpr bool __is_scalar_value(char32_t __c) {
...
template <typename _CharT> constexpr bool __is_single_code_unit(char32_t __c) {
...
// Based on
// https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2023/p2728r6.html#add-the-transcoding-iterator-template

struct _Repl {
  constexpr char32_t operator()() const noexcept { return 0xFFFD; }
};
...
struct _Null_sentinel_t {
  ...
  friend constexpr auto operator==(_It __it, _Null_sentinel_t) {
...
template <typename _FromFmt, typename _ToFmt, input_iterator _Iter,
          sentinel_for<_Iter> _Sent = _Iter, typename _ErrorHandler = _Repl>
...
public:
  ...
  constexpr _Utf_iterator(_Iter __first, _Iter __it, _Sent __last)
  ...
  constexpr _Utf_iterator(_Iter __it, _Sent __last)
    requires(!bidirectional_iterator<_Iter>)
  ...
  constexpr _Utf_iterator(const _Utf_iterator<_FromFmt, _ToFmt, _Iter2, _Sent2,
  ...
  [[nodiscard]]
  constexpr iter_difference_t<_Iter> _M_units() const
    requires forward_iterator<_Iter>
  {
  ...
  constexpr _Utf_iterator &operator++() {
  ...
  constexpr _Utf_iterator operator++(int) {
    auto __tmp = *this;
    ++*this;
    return __tmp;
  }
  ...
  [[nodiscard]]
  friend constexpr bool operator==(_Utf_iterator __lhs, _Utf_iterator __rhs)
  ...
  [[nodiscard]]
  friend constexpr bool operator==(_Utf_iterator __lhs, _Sent __rhs) {
    if constexpr (forward_iterator<_Iter>)
...
private:
  ...
  constexpr void _M_read_reverse()
    requires bidirectional_iterator<_Iter>
  {
    if constexpr (sizeof(_FromFmt) == sizeof(uint8_t))
  ...
  template <typename> struct _Guard {
    _Guard(void *, _Iter &) {}
  };
  ...
  constexpr char32_t _M_read_utf8() {
  ...
  constexpr void _M_read_reverse_utf8()
    requires bidirectional_iterator<_Iter>
  {
    ...
    auto __is_continuation = [](uint8_t __b) { return (__b & 0xC0) == 0x80; };
  ...
  constexpr void _M_read_reverse_utf16()
    requires bidirectional_iterator<_Iter>
  {
    _Guard<_Iter> __g{this, _M_curr()};
    char32_t __c{};
  ...
  constexpr void _M_read_reverse_utf32()
    requires bidirectional_iterator<_Iter>
  {
    _Guard<_Iter> __g{this, _M_curr()};
    char32_t __c = *--_M_curr();
  ...
  constexpr void _M_update(char32_t __c, uint8_t __to_incr) {
  ...
  constexpr char32_t _S_error() {
    char32_t __c = _ErrorHandler()();
    __glibcxx_assert(__is_scalar_value(__c));
    return __c;
  }

  constexpr _Iter _M_first() const
    requires bidirectional_iterator<_Iter>
  {
    return _M_first_and_curr._M_first;
  }

  constexpr _Iter &_M_curr() { return _M_first_and_curr._M_curr; }

  constexpr _Iter _M_curr() const { return _M_first_and_curr._M_curr; }

  // _M_first is not needed for non-bidirectional ranges.
  template <typename _It> struct _First_and_curr {
    _First_and_curr() = default;

    constexpr _First_and_curr(_It __curr) : _M_curr(__curr) {}

    template <convertible_to<_It> _It2>
    constexpr _First_and_curr(const _First_and_curr<_It2> &__other)
  ...
  struct _First_and_curr<_It> {
    ...
    constexpr _First_and_curr(_It __first, _It __curr)
  ...
  template <typename _FromFmt2, typename _ToFmt2, input_iterator _Iter2,
            sentinel_for<_Iter2> _Sent2, typename _ErrHandler>
...
class _Utf_view : public ranges::view_interface<_Utf_view<_ToFormat, _View>> {
  using _Iterator =
      _Utf_iterator<ranges::range_value_t<_View>, _ToFormat,
                    ranges::iterator_t<_View>, ranges::sentinel_t<_View>>;

  template <typename _Iter, typename _Sent>
  constexpr auto _M_begin(_Iter __first, _Sent __last) {
  ...
  template <typename _Iter, typename _Sent>
  constexpr auto _M_end(_Iter __first, _Sent __last) {
...
public:
  constexpr explicit _Utf_view(_View __r) : _M_base(std::move(__r)) {}
...
#ifdef __cpp_char8_t
template <typename _View> using _Utf8_view = _Utf_view<char8_t, _View>;
#else
template <typename _View> using _Utf8_view = _Utf_view<char, _View>;
#endif
template <typename _View> using _Utf16_view = _Utf_view<char16_t, _View>;
template <typename _View> using _Utf32_view = _Utf_view<char32_t, _View>;
...
constexpr int __field_width(char32_t __c) noexcept {
...
// @pre c <= 0x10FFFF
constexpr bool __should_escape_category(char32_t __c) noexcept {
  constexpr uint32_t __mask = 0x01;
  auto *__end = std::end(__escape_edges);
  auto *__p = std::lower_bound(__escape_edges, __end, (__c << 1u) + 2);
  return __p[-1] & __mask;
}
...
__grapheme_cluster_break_property(char32_t __c) noexcept {
  ...
  auto *__p =
      std::lower_bound(__gcb_edges, __end, (__c << __gcb_shift_bits) | __mask);
  return _Gcb_property(__p[-1] & __mask);
}

constexpr bool __is_incb_linker(char32_t __c) noexcept {
...
// @pre c <= 0x10FFFF
constexpr _InCB __incb_property(char32_t __c) noexcept {
...
constexpr bool __is_extended_pictographic(char32_t __c) {
...
struct _Grapheme_cluster_iterator_base {
  ...
  constexpr void _M_reset(char32_t __c, _Gcb_property __p) {
    _M_c = __c;
    _M_prop = __p;
  ...
  constexpr void _M_update_xpicto_seq_state(char32_t __c, _Gcb_property __p) {
    ...
    } else                                  // Zwj
    {
      ...
      if (__p == _Gcb_property::_Gcb_Other && __is_extended_pictographic(__c))
        __next_state = _XPicto::_Matched;
    }
    _M_xpicto_seq_state = __next_state;
  }

  constexpr void _M_update_ri_count(_Gcb_property __p) {
  ...
  constexpr void _M_update_incb_state(char32_t __c, _Gcb_property) {
...
public:
  constexpr _Grapheme_cluster_view(_View __v)
...
private:
  ...
  public:
    ...
    constexpr _Iterator(_U32_iterator __i) : _M_base(__i) {
      if (__i != __i.end()) {
        _M_c = *__i;
    ...
    constexpr value_type operator*() const { return _M_c; }

    constexpr auto operator->() const { return &_M_c; }
    ...
    constexpr bool operator==(const _Iterator &__i) const {
      return _M_base == __i._M_base;
    }

    // This supports iter != iter.end()
    constexpr bool operator==(const ranges::sentinel_t<_View> &__i) const {
      return _M_base == __i;
    }
    ...
    constexpr int width() const noexcept { return __field_width(_M_c); }
  ...
  private:
    ...
    constexpr bool _M_is_break(_Gcb_property __p1, _Gcb_property __p2,
      ...
      if (_M_incb_linker_seen && __incb_property(_M_c) == _InCB::_Consonant &&
...
constexpr size_t __field_width(basic_string_view<_CharT> __s) {
...
template <typename _CharT>
constexpr size_t __truncate(basic_string_view<_CharT> &__s, size_t __max) {
  if (__s.empty()) [[unlikely]]
    return 0;
...
template <typename _CharT> consteval bool __literal_encoding_is_unicode() {
...
consteval bool __literal_encoding_is_utf8() {
  return __literal_encoding_is_unicode<char>();
}

consteval bool __literal_encoding_is_extended_ascii() {
  return '0' == 0x30 && 'A' == 0x41 && 'Z' == 0x5a && 'a' == 0x61 &&
         'z' == 0x7a;
}

// https://www.unicode.org/reports/tr22/tr22-8.html#Charset_Alias_Matching
constexpr bool __charset_alias_match(string_view __a, string_view __b) {
  ...
  while (true) {
    ...
    while (__ptr_a != __end_a && (__val_a = __map(*__ptr_a, __num_a)) == 127)
      ++__ptr_a;
    while (__ptr_b != __end_b && (__val_b = __map(*__ptr_b, __num_b)) == 127)
      ++__ptr_b;
