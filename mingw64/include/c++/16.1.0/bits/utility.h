...
#ifdef __glibcxx_constant_wrapper // C++ >= 26
template <typename _Tp> struct _CwFixedValue {
  using __type = _Tp;

  constexpr _CwFixedValue(__type __v) noexcept : _M_data(__v) {}

  __type _M_data;
};

template <typename _Tp, size_t _Extent> struct _CwFixedValue<_Tp[_Extent]> {
  ...
  constexpr _CwFixedValue(_Tp (&__arr)[_Extent],
...
template <typename _Tp>
concept _ConstExprParam = requires { typename constant_wrapper<_Tp::value>; };

struct _CwOperators {
  template <_ConstExprParam _Tp>
  friend constexpr auto operator+(_Tp) noexcept
  ...
  friend constexpr auto operator+(_Left, _Right) noexcept
  ...
  template <_ConstExprParam _Left, _ConstExprParam _Right>
  friend constexpr auto operator,(_Left, _Right) noexcept = delete;
  ...
  constexpr auto operator+=(this _Tp, _Right) noexcept
...
template <_CwFixedValue _Xv, typename> struct constant_wrapper : _CwOperators {
  ...
  constexpr auto operator=(_Right) const noexcept
      -> constant_wrapper<(value = _Right::value)> {
    return {};
  }

  template <typename... _Args,
            bool _ConstExprInvocable =
                requires {
                  requires(_ConstExprParam<remove_cvref_t<_Args>> && ...);
                  typename constant_wrapper<std::__invoke(
                      value, remove_cvref_t<_Args>::value...)>;
                }>
    requires _ConstExprInvocable || is_invocable_v<const value_type &, _Args...>
  static constexpr decltype(auto)
  operator()(_Args &&...__args) noexcept(requires {
    requires _ConstExprInvocable ||
                 is_nothrow_invocable_v<const value_type &, _Args...>;
  }) {
  ...
  template <typename... _Args,
    ...
    requires _ConstExprSubscriptable ||
             requires { value[std::declval<_Args>()...]; }
  static constexpr decltype(auto)
  operator[](_Args &&...__args) noexcept(requires {
    requires _ConstExprSubscriptable ||
...
template <typename> constexpr bool __is_constant_wrapper_v = false;

template <auto __cw, typename _Fn>
constexpr bool __is_constant_wrapper_v<constant_wrapper<__cw, _Fn>> = true;

template <_CwFixedValue _Tp> constexpr auto cw = constant_wrapper<_Tp>{};
#endif
...
template <size_t __i, class _Tp, _Tp... _Idx>
[[nodiscard]]
constexpr _Tp get(integer_sequence<_Tp, _Idx...>) noexcept {
  static_assert(__i < sizeof...(_Idx));
  return _Idx...[__i];
}
/// @}
...
inline constexpr _Tp _IotaArray[_Num] = {__integer_pack(_Tp(_Num))...};
#elif defined __glibcxx_integer_sequence
template <auto _Num, typename _Tp = decltype(_Num),
          typename = make_integer_sequence<_Tp, _Num>>
inline constexpr _Tp _IotaArray[_Num];
...
template <typename> inline constexpr bool __is_in_place_index_v = false;
