...
struct __allocator_traits_base {
...
#endif
  {
    ...
    static_assert(
        is_same<typename _Tp::template rebind<typename _Tp::value_type>::other,
                _Tp>::value,
        "allocator_traits<A>::rebind_alloc<A::value_type> must be A");
  };
...
protected:
  ...
  static constexpr bool __has_allocate_hint =
      requires(_Alloc &__a, _Sz __n, _Vp __hint) { __a.allocate(__n, __hint); };
#else
  template <typename _Alloc, typename _Sz, typename _Vp>
  using __allocate_hint_t = decltype(std::declval<_Alloc &>().allocate(
      std::declval<_Sz>(), std::declval<_Vp>()));
  template <typename _Alloc, typename _Sz, typename _Vp, typename = void>
  static constexpr bool __has_allocate_hint = false;
  template <typename _Alloc, typename _Sz, typename _Vp>
  static constexpr bool __has_allocate_hint<
      _Alloc, _Sz, _Vp, __void_t<__allocate_hint_t<_Alloc, _Sz, _Vp>>> = true;
#endif
...
#else
  template <typename _Alloc, typename _Tp, typename... _Args>
  using __construct_t = decltype(std::declval<_Alloc &>().construct(
      std::declval<_Tp *>(), std::declval<_Args>()...));
  template <typename _Alloc, typename _Tp, typename, typename... _Args>
  static constexpr bool __has_construct_impl = false;
  template <typename _Alloc, typename _Tp, typename... _Args>
  static constexpr bool __has_construct_impl<
      _Alloc, _Tp, __void_t<__construct_t<_Alloc, _Tp, _Args...>>, _Args...> =
      true;
  template <typename _Alloc, typename _Tp, typename... _Args>
  static constexpr bool __has_construct =
  ...
  static constexpr bool __has_destroy =
      requires(_Alloc &__a, _Tp *__p) { __a.destroy(__p); };
#else
  template <typename _Alloc, typename _Tp>
  using __destroy_t =
  ...
  template <typename _Alloc, typename _Tp>
  static constexpr bool
      __has_destroy<_Alloc, _Tp, __void_t<__destroy_t<_Alloc, _Tp>>> = true;
#endif

  // __has_max_size is true if a.max_size() is well-formed.
#if __cpp_concepts
  template <typename _Alloc>
  static constexpr bool __has_max_size =
      requires(const _Alloc &__a) { __a.max_size(); };
#else
  template <typename _Alloc>
  ...
  template <typename _Alloc>
  static constexpr bool __has_max_size<_Alloc, __void_t<__max_size_t<_Alloc>>> =
      true;
#endif
...
#else
  template <typename _Alloc>
  using __soccc_t = decltype(std::declval<const _Alloc &>()
...
public:
   ...
   */
  [[nodiscard]] static constexpr auto allocate_at_least(_Alloc &__a,
                                                        size_type __n)
  ...
  destroy(_Alloc &__a, _Tp *__p) noexcept(_S_nothrow_destroy<_Tp>()) {
...
private:
  ...
  template <typename _Tp, typename... _Args>
  static constexpr bool _S_nothrow_construct(_Alloc *__a = nullptr,
                                             _Tp *__p = nullptr) {
    if constexpr (__has_construct<_Alloc, _Tp, _Args...>)
  ...
  template <typename _Tp>
  static constexpr bool _S_nothrow_destroy(_Alloc *__a = nullptr,
                                           _Tp *__p = nullptr) {
    if constexpr (__has_destroy<_Alloc, _Tp>)
...
#else
  template <typename _Tp, typename... _Args>
  static constexpr __enable_if_t<__has_construct<_Alloc, _Tp, _Args...>, bool>
  ...
  template <typename _Tp>
  static constexpr __enable_if_t<__has_destroy<_Alloc, _Tp>, bool>
...
template <typename _Tp> struct allocator_traits<allocator<_Tp>> {
  ...
  static _GLIBCXX20_CONSTEXPR pointer allocate(allocator_type &__a,
                                               size_type __n) {
    return __a.allocate(__n);
  }
  ...
  static _GLIBCXX20_CONSTEXPR
      pointer allocate(allocator_type &__a, size_type __n,
   ...
   */
  [[nodiscard]] static constexpr auto allocate_at_least(allocator_type &__a,
                                                        size_type __n)
      -> allocation_result<pointer, size_type> {
    return __a.allocate_at_least(__n);
  }
  ...
  static _GLIBCXX20_CONSTEXPR void deallocate(allocator_type &__a, pointer __p,
                                              size_type __n) {
    __a.deallocate(__p, __n);
  }
  ...
  [[__gnu__::__always_inline__]]
  static _GLIBCXX20_CONSTEXPR void construct(allocator_type &__a
...
template <typename _Alloc>
[[__gnu__::__always_inline__]]
_GLIBCXX14_CONSTEXPR inline void __alloc_on_move(_Alloc &__one, _Alloc &__two) {
  using __traits = allocator_traits<_Alloc>;
  using __pocma =
...
template <typename _Alloc>
[[__gnu__::__always_inline__]]
_GLIBCXX14_CONSTEXPR inline void __alloc_on_swap(_Alloc &__one, _Alloc &__two) {
  using __traits = allocator_traits<_Alloc>;
