...
public:
  ...
  operator _Pointer *() const noexcept { return _M_impl._M_get(); }

  operator void **() const noexcept
    requires(!same_as<_Pointer, void *>)
  {
...
private:
  ...
  template <typename, typename, typename...> struct _Impl {
    ...
    void _M_inout_init() { _M_ptr = _M_smart.release(); }

    // The pointer value returned by operator Pointer*().
    _Pointer *_M_get() const {
  ...
  template <typename _Tp> struct _Impl<_Tp *, _Tp *> {
    void _M_out_init() { _M_p = nullptr; }
  ...
  struct _Impl<_Tp *, _Ptr> {
    explicit _Impl(_Tp *&__p) : _M_p(__p) {}

    void _M_out_init() { _M_p = nullptr; }

    void _M_inout_init() { _M_ptr = _M_p; }
  ...
  struct _Impl<unique_ptr<_Tp, _Del>, typename unique_ptr<_Tp, _Del>::pointer> {
    void _M_out_init() { _M_smart.reset(); }

    _Pointer *_M_get() const noexcept {
  ...
  struct _Impl<shared_ptr<_Tp>, typename shared_ptr<_Tp>::element_type *, _Del,
               _Alloc> {
    _Impl(_Smart &__s, _Del __d, _Alloc __a = _Alloc()) : _M_smart(__s) {
      ...
      ::new (__mem)
          _Scd(nullptr, std::forward<_Del>(__d), std::forward<_Alloc>(__a));
    ...
    using _Scd = _Sp_counted_deleter<_Sp, decay_t<_Del>, remove_cvref_t<_Alloc>,
...
public:
  explicit inout_ptr_t(_Smart &__smart, _Args... __args)
      : _M_impl{__smart, std::forward<_Args>(__args)...} {
...
private:
  ...
  using _Out_ptr_t =
      __conditional_t<__is_shared_ptr<_Smart>, out_ptr_t<void *, void *>,
...
template <typename _Tp> consteval auto __pointer_of() {
...
template <typename _Smart, typename _Ptr> consteval auto __pointer_of_or() {
...
template <typename _Ptr, typename _Smart> consteval auto __choose_ptr() {
...
inline auto out_ptr(_Smart &__s, _Args &&...__args) {
 ...
 */
template <typename _Pointer = void, typename _Smart, typename... _Args>
inline auto inout_ptr(_Smart &__s, _Args &&...__args) {
  using _TypeId = decltype(__detail::__choose_ptr<_Pointer, _Smart>());
...
inline out_ptr_t<_Smart, _Pointer, _Args...>::_Impl<_Smart2, _Pointer2,
                                                    _Args2...>::~_Impl() {
