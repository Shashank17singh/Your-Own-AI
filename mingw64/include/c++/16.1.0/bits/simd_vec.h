...
namespace simd {
// disabled basic_vec
template <typename _Tp, typename _Ap> class basic_vec {
public:
...
template <typename _Tp, typename _Ap> class _VecBase {
...
public:
  ...
  constexpr iterator begin() noexcept { return {static_cast<_Vp &>(*this), 0}; }
  ...
  explicit _VecBase(const basic_vec<_Up, _UAbi> &) =
      delete ("the value types are not convertible");

  [[__gnu__::__always_inline__]]
  friend constexpr _Vp operator+(const _Vp &__x, const _Vp &__y) noexcept {
    _Vp __r = __x;
    __r += __y;
    return __r;
  }
  ...
  [[__gnu__::__always_inline__]]
  friend constexpr _Vp operator<<(const _Vp &__x,
  ...
  [[__gnu__::__always_inline__]]
  friend constexpr _Vp operator<<(const _Vp &__x,
                                  __simd_size_type __y) _GLIBCXX_SIMD_NOEXCEPT
...
class basic_vec<_Tp, _Ap> : public _VecBase<_Tp, _Ap> {
  template <typename, typename> friend class basic_vec;

  template <size_t, typename> friend class basic_mask;
   ...
   *
   * This member holds the vector object using a GNU vector type or a
   * platform-specific vector type determined by the ABI tag. For size 1
   * vectors, this is a single value (_Tp).
   */
  _DataType _M_data;
...
public:
  ...
  [[__gnu__::__always_inline__]]
  static constexpr basic_vec _S_init(_DataType __x) {
    basic_vec __r;
    __r._M_data = __x;
    return __r;
  }
  ...
  [[__gnu__::__always_inline__]]
  constexpr const _DataType &_M_get() const {
    return _M_data;
  }

  [[__gnu__::__always_inline__]]
  friend constexpr bool __is_const_known(const basic_vec &__x) {
  ...
  constexpr auto _M_chunk() const noexcept {
    ...
    if constexpr (__rem == 0)
      return array<_Vp, __n>{
          __extract_simd_at<_Vp>(cw<_Vp::_S_size * __is>, *this)...};
    else {
  ...
  [[__gnu__::__always_inline__]]
  static constexpr basic_vec _S_concat(const basic_vec &__x0) noexcept {
    return __x0;
  }
  ...
  /** @internal
   * Shifts elements to the front by @p _Shift positions (or to the back for
   * negative @p _Shift).
   *
   ...
   * Negative shift values shift in the opposite direction.
   *
  ...
  [[__gnu__::__always_inline__]] constexpr basic_vec
    ...
    if constexpr (_Shift == 0)
      ...
      if constexpr (sizeof(_M_data) == 16 && _Shift > 0)
        ...
        if constexpr (_Shift > 0)
          ...
          __x =
              __builtin_ia32_pslldqi128(__x, -_Shift * sizeof(value_type) * 8);
        return _VecOps<_DataType>::_S_extract(
            __vec_bit_cast<__canon_value_type>(__x));
      }
    }
#endif
  ...
  /** @internal
   * @brief Set padding elements to @p __id; add more padding elements if
   * necessary.
   *
   * @note This function can rearrange the element order since the result is
   * only used for reductions.
   */
  template <typename _Vp, __canon_value_type __id>
  [[__gnu__::__always_inline__]]
  constexpr _Vp _M_pad_to_T_with_value() const noexcept {
    static_assert(!_Vp::_S_is_partial);
    static_assert(_Ap::_S_nreg == 1);
    if constexpr (sizeof(_Vp) ==
                  32) { // when we need to reduce from a 512-bit register
      ...
      if constexpr (__id == 0 && _S_is_partial)
        ...
        // This is valid because we shift out padding elements and use all
        // elements in a subsequent reduction.
        __v1 = __v1.template _M_elements_shifted_to_front<-(_Vp::_S_size -
                                                            _S_size)>();
      ...
      else if constexpr (__has_single_bit(unsigned(
                             _Vp::_S_size -
                             _S_size))) { // if 2^n elements need to be changed,
                                          // use a single insert instruction
      ...
      } else if constexpr (__id != 0 &&
                           !_S_is_partial) { // if __vec_zero_pad_to added zeros
                                             // in all the places where we need
                                             // __id, a
        // bitwise or is sufficient (needs a vector constant for the __id
        // vector, which isn't optimal)
        constexpr _Vp __idn([](int __i) {
          return __i >= _S_size ? __id : __canon_value_type();
        });
        __v1._M_data = __vec_or(__v1._M_data, __idn._M_data);
      } else if constexpr (__id != 0 || _S_is_partial) { // fallback
        constexpr auto __k = _Vp::mask_type::_S_partial_mask_of_n(_S_size);
  ...
  [[__gnu__::__always_inline__]]
  constexpr auto _M_reduce_to_half(auto __binary_op) const {
  ...
  [[__gnu__::__always_inline__]]
  constexpr value_type _M_reduce_tail(const _Rest &__rest,
    ...
    else if constexpr (sizeof(_M_data) <= 16 && requires {
                         __default_identity_element<__canon_value_type,
                                                    _BinaryOp>();
                       }) { // extend __rest with identity element for more
                            // parallelism
      constexpr __canon_value_type __id =
          __default_identity_element<__canon_value_type, _BinaryOp>();
      return __binary_op(
                 _M_data,
                 __rest.template _M_pad_to_T_with_value<basic_vec, __id>())
   ...
   *
   * @note The implementation assumes it is most efficient to first reduce to
   * one 128-bit SIMD register and then shuffle elements while sticking to
   * 128-bit registers.
   */
  template <typename _BinaryOp, _ArchTraits _Traits = {}>
  [[__gnu__::__always_inline__]] constexpr value_type
      _M_reduce(_BinaryOp __binary_op) const {
    constexpr bool __have_id_elem = requires {
      __default_identity_element<__canon_value_type, _BinaryOp>();
    };
    if constexpr (_S_size == 1)
      ...
      return value_type(
          rebind_t<float, basic_vec>(*this)._M_reduce(__binary_op));
#ifdef __SSE2__
      ...
      //
      // The left columns under Latency and Throughput show bit-cast to ushort
      // with shift by
      ...
      if constexpr (!_S_is_partial) { // If all elements participate in the
                                      // reduction we can take this shortcut
      ...
      if constexpr (sizeof(_M_data) > 16)
        ...
        if constexpr (sizeof(_M_data) <= 16 && is_integral_v<value_type>) {
          if constexpr (_S_size > 8)
            __x = __binary_op(__x,
                              __x.template _M_elements_shifted_to_front<8>());
          if constexpr (_S_size > 4)
            __x = __binary_op(__x,
                              __x.template _M_elements_shifted_to_front<4>());
          if constexpr (_S_size > 2)
            __x = __binary_op(__x,
                              __x.template _M_elements_shifted_to_front<2>());
          // We could also call __binary_op with vec<T, 1> arguments. However,
          // micro-benchmarking on Intel Ultra 7 165U showed this to be more
          // efficient:
          return __binary_op(__x,
                             __x.template _M_elements_shifted_to_front<1>())[0];
        }
#endif
        if constexpr (_S_size > 8)
        ...
        if constexpr (is_integral_v<value_type> && sizeof(value_type) <= 1)
          return value_type(
              resize_t<4, rebind_t<int, basic_vec>>(chunk<4>(__x)[0])
                  ._M_reduce(__binary_op));
#endif
        if constexpr (_S_size > 2)
        ...
        if constexpr (is_integral_v<value_type> && sizeof(value_type) == 2)
          return __binary_op(__x,
                             _S_static_permute(__x, _SwapNeighbors<1>()))[0];
        else
          return __binary_op(vec<value_type, 1>(__x[0]),
                             vec<value_type, 1>(__x[1]))[0];
      }
      ...
      if constexpr (tuple_size_v<_Cp> == 4) {
        ...
        if constexpr (__have_id_elem && __rest._S_size > 1 &&
                      __amd_cpu) { // do one 256-bit op -> one 128-bit op
          ...
          // 17 cycles on Skylake-AVX512 until _M_reduce (short, 27,
          // multiplies<>)
          const auto &[__a, __rest] =
              chunk<__bit_floor(unsigned(_S_size))>(*this);
  ...
  [[__gnu__::__always_inline__]] constexpr mask_type _M_isnan() const
    requires is_floating_point_v<value_type>
  {
  ...
  [[__gnu__::__always_inline__]] constexpr mask_type _M_isinf() const
    requires is_floating_point_v<value_type>
  {
    if constexpr (_Traits._M_finite_math_only())
      return mask_type(false);
  ...
  [[__gnu__::__always_inline__]]
  constexpr basic_vec _M_abs() const
    requires signed_integral<value_type>
  {
    return _M_data < 0 ? -_M_data : _M_data;
  }

  [[__gnu__::__always_inline__]]
  constexpr basic_vec _M_fabs() const
  ...
  template <_TargetTraits _Traits = {}>
  [[__gnu__::__always_inline__]] constexpr mask_type
      _M_isunordered(basic_vec __y) const
    requires is_floating_point_v<value_type>
  {
    if constexpr (_Traits._M_finite_math_only())
      return mask_type(false);
   ...
   *
   * @param __mem  A pointer to an array of @p __n values. Can be complex or
   * real.
   * @param __n    Read no more than @p __n values from memory. However,
   * depending on @p __mem alignment, out of bounds reads are benign.
   */
  template <typename _Up, _ArchTraits _Traits = {}>
  static inline basic_vec _S_partial_load(const _Up *__mem, size_t __n) {
    if constexpr (_S_is_scalar)
      return __n == 0 ? basic_vec()
                      : basic_vec(static_cast<value_type>(*__mem));
      ...
      if constexpr (_Traits._M_have_avx512f() ||
        ...
        const auto __k = __n < _S_size
                             ? mask_type::_S_partial_mask_of_n(int(__n))
                             : mask_type(true);
      ...
      // allow out-of-bounds read when it cannot lead to a #GP
                   ...
                   is_sufficiently_aligned<sizeof(_Up) * _S_full_size>(__mem),
                   true))
   ...
   * @param __k   Mask controlling which elements to load. For each bit i in the
   * mask:
   *              - If bit i is 1: copy __mem[i] into result[i]
   *              - If bit i is 0: result[i] is default initialized
   *
   * @note This function assumes it's called after determining that no other
   * method (like full load) is more appropriate. Calling with all mask bits set
   * to 1 is suboptimal for performance but still correct.
   */
  template <typename _Up, _ArchTraits _Traits = {}>
  static inline basic_vec _S_masked_load(const _Up *__mem, mask_type __k) {
    if constexpr (_S_size == 1)
  ...
  [[__gnu__::__always_inline__]]
  inline void _M_store(_Up *__mem) const {
   ...
   *
   * @note This is a static function to allow passing @p __v via register in
   * case the function is not inlined.
   *
   * @note The function is not marked @c __always_inline__ since code-gen can
   * become fairly long.
   */
  template <typename _Up, _ArchTraits _Traits = {}>
  static inline void _S_partial_store(const basic_vec __v, _Up *__mem,
                                      size_t __n) {
   ...
   * @param __mem Pointer (in)to array.
   * @param __k   Mask controlling which elements to store. For each bit i in
   * the mask:
   *              - If bit i is 1: store __v[i] to __mem[i]
   *              - If bit i is 0: __mem[i] is left unchanged
   *
   * @note This function assumes it's called after determining that no other
   * method (like full store) is more appropriate. Calling with all mask bits
   * set to 1 is suboptimal for performance but still correct.
   */
  template <typename _Up, _ArchTraits _Traits = {}>
  //[[__gnu__::__always_inline__]]
  static inline void _S_masked_store(const basic_vec __v, _Up *__mem,
                                     const mask_type __k) {
#if _GLIBCXX_X86
    if constexpr (_Traits._M_have_avx512f()) {
   ...
   */
  constexpr basic_vec(_NativeVecType __x)
   ...
   *
   * This operator enables implicit conversion from basic_vec to GCC vector
   * builtins.
   *
   ...
   */
  constexpr operator _NativeVecType() const {
    if constexpr (_S_is_scalar)
  ...
  constexpr basic_vec(_IV __x) : _M_data(reinterpret_cast<_DataType>(__x)) {}
   ...
   *
   * Constructs a vector where all elements are initialized to the same scalar
   * value. The scalar value is converted to the vector's element type.
   *
   * @param __x Scalar value to broadcast to all vector elements.
   * @tparam _Up Type of scalar value (must be explicitly convertible to
   * value_type).
   *
   * @note The constructor is implicit if the conversion (if any) is
   * value-preserving.
   */
  template <__broadcast_constructible<value_type> _Up>
  [[__gnu__::__always_inline__]]
  constexpr basic_vec(_Up &&__x) noexcept
      : _M_data(_DataType() == _DataType() ? static_cast<value_type>(__x)
                                           : value_type()) {}
  ...
  [[__gnu__::__always_inline__]] constexpr explicit(
      !__value_preserving_convertible_to<_Up, value_type> ||
      ...
      : _M_data([&] [[__gnu__::__always_inline__]] {
          if constexpr (_S_is_scalar)
            ...
            // __builtin_convertvector (__vec_cast) is inefficient for
            // over-sized inputs. Also e.g. vec<float, 12> -> vec<char, 12>
            // (with SSE2) would otherwise emit 4 vcvttps2dq instructions, where
            // only 3 are needed
            return _S_concat(resize_t<__x._N0, basic_vec>(__x._M_data0),
                             resize_t<__x._N1, basic_vec>(__x._M_data1))
                ._M_data;
          else
  ...
  [[__gnu__::__always_inline__]]
  constexpr explicit basic_vec(_Fp &&__gen)
      : _M_data([&] [[__gnu__::__always_inline__]] {
          constexpr auto [... __is] = _IotaArray<_S_size>;
          return _DataType{
              static_cast<value_type>(__gen(__simd_size_c<__is>))...};
        }()) {}

  // [simd.ctor] load constructor -----------------------------------------
  template <typename _Up>
  [[__gnu__::__always_inline__]]
  constexpr basic_vec(_LoadCtorTag, const _Up *__ptr)
      : _M_data() {
    if constexpr (_S_is_scalar)
  ...
  [[__gnu__::__always_inline__]]
  constexpr basic_vec(_Rg &&__range, flags<_Flags...> __flags = {})
   ...
   */
  [[__gnu__::__always_inline__]]
  constexpr value_type operator[](__simd_size_type __i) const {
    __glibcxx_simd_precondition(__i >= 0 && __i < _S_size,
  ...
  // increment and decrement are implemented in terms of operator+=/-= which
  // avoids UB on padding elements while not breaking UBsan
  [[__gnu__::__always_inline__]]
   ...
   */
  [[__gnu__::__always_inline__]]
  friend constexpr basic_vec &operator&=(basic_vec &__x,
                                         const basic_vec &__y) noexcept
   ...
   *
   * @pre If @c value_type is a signed integral type, the result is
   * representable by @c value_type. (This does not apply to padding elements
   * the implementation might add for non-power-of-2 widths.) UBsan will only
   * see a call to @c unreachable() on overflow.
   *
  ...
  friend constexpr basic_vec &operator+=(basic_vec &__x,
    ...
    requires requires(value_type __a) { __a + __a; }
  {
    if constexpr (_S_is_partial && is_integral_v<value_type> &&
                  is_signed_v<value_type>) { // avoid spurious UB on signed
                                             // integer overflow of the padding
                                             // element(s). But don't
      // remove UB of the active elements (so that UBsan can still do its job).
      //
      // This check is essentially free (at runtime) because DCE removes
      // everything except the final change to _M_data. The overflow check is
      // only emitted if UBsan is active.
      //
      // The alternative would be to always zero padding elements after
      // operations that can produce non-zero values. However, right now:
      // - auto f(simd::mask<int, 3> k) { return +k; } is a single VPABSD and
      // would have to
      //   sanitize
      ...
      // - shuffles are allowed to put whatever they want into padding elements
      // for
  ...
  {
    if constexpr (_S_is_partial && is_integral_v<value_type> &&
                  is_signed_v<value_type>) { // see comment on operator+=
      using _UV = typename _Ap::template _DataType<make_unsigned_t<value_type>>;
      const _DataType __result =
          reinterpret_cast<_DataType>(reinterpret_cast<_UV>(__x._M_data) -
                                      reinterpret_cast<_UV>(__y._M_data));
    ...
    requires requires(value_type __a) { __a * __a; }
  {
    if constexpr (_S_is_partial && is_integral_v<value_type> &&
      ...
      for (int __i = 0; __i < _S_size; ++__i) {
        if (__builtin_mul_overflow_p(__x._M_data[__i], __y._M_data[__i],
                                     value_type()))
    ...
    // 'uint16 * uint16' promotes to int and can therefore lead to UB. The
    // standard does not require to avoid the undefined behavior. It's
    // unnecessary and easy to avoid. It's also unexpected because there's no UB
    // on the vector types (which don't promote).
  ...
  template <_TargetTraits _Traits = {}>
  [[__gnu__::__always_inline__]] friend constexpr basic_vec &
  operator/=(basic_vec & __x, const basic_vec & __y) noexcept
    requires requires(value_type __a) { __a / __a; }
  {
    const basic_vec __result(
        [&](int __i) -> value_type { return __x[__i] / __y[__i]; });
    ...
    // see PR121274, PR121284, and PR121296 for missed optimizations wrt.
    // conversions
    //
    // With only 1 or 2 divisions, the conversion to and from fp is too
    // expensive.
    if constexpr (is_integral_v<value_type> && _S_size > 2 &&
                  __value_preserving_convertible_to<value_type, double>) {
      // If the denominator (y) is known to the optimizer, don't convert to fp
      // because the integral division can be translated into
      // shifts/multiplications.
    ...
    if constexpr (_S_is_partial) {
      if constexpr (is_integral_v<value_type>) {
        // Assume integral division doesn't have SIMD instructions and must be
        // done per element anyway. Partial vectors should skip their padding
        // elements.
        for (int __i = 0; __i < _S_size; ++__i)
    ...
    requires requires(value_type __a) { __a % __a; }
  {
    ...
    if constexpr (_S_is_partial) {
      const basic_vec __y1 =
          __select_impl(mask_type::_S_init(mask_type::_S_implicit_mask), __y,
                        basic_vec(value_type(1)));
  ...
  operator<<=(basic_vec &__x, const basic_vec &__y) _GLIBCXX_SIMD_NOEXCEPT
    requires requires(value_type __a) { __a << __a; }
  {
    __glibcxx_simd_precondition(is_unsigned_v<value_type> ||
                                    all_of(__y >= value_type()),
  ...
  {
    __glibcxx_simd_precondition(__y >= 0,
                                "negative shift is undefined behavior");
  ...
  [[__gnu__::__always_inline__]]
  constexpr mask_type _M_bitmask_cmp(_DataType __y) const {
  ...
  [[__gnu__::__always_inline__]]
  friend constexpr mask_type operator==(const basic_vec &__x,
                                        const basic_vec &__y) noexcept {
#if _GLIBCXX_X86
  ...
  template <_TargetTraits _Traits = {}>
  [[__gnu__::__always_inline__]] friend constexpr basic_vec
      __select_impl(const mask_type &__k, const basic_vec &__t,
                    const basic_vec &__f) noexcept {
    if constexpr (_S_size == 1)
      ...
      if (__is_const_known(__k, __t, __f))
        return basic_vec(
            [&](int __i) { return __k[__i] ? __t[__i] : __f[__i]; });
      else
      ...
      if (_VO::_S_is_const_known_equal_to(__f._M_data, 0)) {
        if (is_integral_v<value_type> && __uses_simd_register &&
          ...
          // This is equivalent to converting the mask into a vec of 0s and 1s.
          // So +__k. However, basic_mask::operator+ arrives here; returning
          // +__k would be recursive. Instead we use -__k (which is a no-op for
          // vector-masks) and then flip all -1 elements to +1 by taking the
          // absolute value.
          return basic_vec((-__k)._M_abs());
        else
          return __vec_and(reinterpret_cast<_DataType>(__k._M_data),
                           __t._M_data);
      } else if (_VecOps<_DataType>::_S_is_const_known_equal_to(__t._M_data,
                                                                0)) {
        if (is_integral_v<value_type> && __uses_simd_register &&
            _VO::_S_is_const_known_equal_to(__f._M_data, 1))
          return value_type(1) + basic_vec(-__k);
        else
          return __vec_and(reinterpret_cast<_DataType>(__vec_not(__k._M_data)),
                           __f._M_data);
      } else {
#if _GLIBCXX_X86
        // this works around bad code-gen when the compiler can't see that __k
        // is a vector-mask. This pattern, is recognized to match the x86 blend
        // instructions, which only consider the sign bit of the mask register.
        // Also, without SSE4, if the compiler knows that __k is a vector-mask,
        // then the '< 0' is elided.
...
class basic_vec<_Tp, _Ap> : public _VecBase<_Tp, _Ap> {
  ...
  // the implementation (and users) depend on elements being contiguous in
  // memory
  static_assert(_N0 * sizeof(_Tp) == sizeof(_DataType0));

  using _DataType1 = __similar_vec<_Tp, _N1, _Ap>;

  static_assert(_DataType0::abi_type::_S_nreg + _DataType1::abi_type::_S_nreg ==
                _Ap::_S_nreg);
...
public:
  ...
  [[__gnu__::__always_inline__]]
  static constexpr basic_vec _S_init(const _DataType0 &__x,
  ...
  [[__gnu__::__always_inline__]]
  constexpr const _DataType0 &_M_get_low() const {
    return _M_data0;
  }

  [[__gnu__::__always_inline__]]
  constexpr const _DataType1 &_M_get_high() const {
    return _M_data1;
  }
  ...
  _S_static_permute(const basic_vec<value_type, _A0> &__x, _Fp &&__idxmap) {
    return _S_init(
        ...
        _DataType1::template _S_static_permute<_Size, _Offset + _N0>(__x,
                                                                     __idxmap));
  }
  ...
  constexpr auto _M_chunk() const noexcept {
    ...
    if constexpr (__rem == 0)
      ...
      using _Rest = resize_t<__rem, _Vp>;
      return tuple(
          __extract_simd_at<_Vp>(cw<_Vp::_S_size * __is>, _M_data0,
                                 _M_data1)...,
  ...
  [[__gnu__::__always_inline__]]
  static constexpr const basic_vec &_S_concat(const basic_vec &__x0) noexcept {
    return __x0;
  }
  ...
  [[__gnu__::__always_inline__]]
  constexpr value_type _M_reduce_tail(const auto &__rest,
                                      auto __binary_op) const {
  ...
  [[__gnu__::__always_inline__]] constexpr value_type
    ...
    if constexpr (_Traits.template _M_eval_as_f32<value_type>() &&
      ...
      if constexpr (_DataType1::_S_is_scalar)
        return __binary_op(_DataType1(_M_data0._M_reduce(__binary_op)),
                           _M_data1)[0];
      // TODO: optimize trailing scalar (e.g. (8+8)+(8+1))
      else if constexpr (_S_size % 2 ==
                         0) { // If all elements participate in the reduction we
                              // can take this shortcut
        using _V16 = resize_t<_S_size / 2, rebind_t<unsigned short, basic_vec>>;
        auto __a = __builtin_bit_cast(_V16, *this);
  ...
  [[__gnu__::__always_inline__]]
  constexpr mask_type _M_isnan() const
    requires is_floating_point_v<value_type>
  {
  ...
  [[__gnu__::__always_inline__]]
  constexpr mask_type _M_isinf() const
    requires is_floating_point_v<value_type>
  {
  ...
  [[__gnu__::__always_inline__]]
  constexpr mask_type _M_isunordered(basic_vec __y) const
    requires is_floating_point_v<value_type>
  {
  ...
  static inline basic_vec _S_partial_load(const _Up *__mem, size_t __n) {
    if (__n >= _N0)
      ...
      return _S_init(_DataType0::_S_partial_load(__mem, __n), _DataType1());
  }
  ...
  using _NativeVecType =
      __vec_builtin_type<value_type, __bit_ceil(unsigned(_S_size))>;

  [[__gnu__::__always_inline__]]
  constexpr basic_vec(const _NativeVecType &__x)
      : _M_data0(_VecOps<__vec_builtin_type<value_type, _N0>>::_S_extract(__x)),
        _M_data1(
            _VecOps<__vec_builtin_type<value_type, __bit_ceil(unsigned(_N1))>>::
  ...
  [[__gnu__::__always_inline__]]
  constexpr basic_vec(_Up &&__x) noexcept
      : _M_data0(static_cast<value_type>(__x)),
        _M_data1(static_cast<value_type>(__x)) {}
  ...
  template <typename _Up, typename _UAbi>
    requires(_S_size == _UAbi::_S_size) &&
                __explicitly_convertible_to<_Up, value_type>
  [[__gnu__::__always_inline__]]
  constexpr explicit(!__value_preserving_convertible_to<_Up, value_type> ||
                     __higher_rank_than<_Up, value_type>)
      basic_vec(const basic_vec<_Up, _UAbi> &__x) noexcept
      : _M_data0(get<0>(chunk<_N0>(__x))), _M_data1(get<1>(chunk<_N0>(__x))) {}
  ...
  constexpr explicit basic_vec(_Fp &&__gen)
      : _M_data0(__gen),
        _M_data1([&] [[__gnu__::__always_inline__]] (auto __i) {
  ...
  [[__gnu__::__always_inline__]]
  constexpr basic_vec(_LoadCtorTag, const _Up *__ptr)
      : _M_data0(_LoadCtorTag(), __ptr), _M_data1(_LoadCtorTag(), __ptr + _N0) {
  }
...
#define _GLIBCXX_SIMD_DEFINE_OP(sym)                                           \
  [[__gnu__::__always_inline__]]                                               \
  friend constexpr basic_vec &operator sym## =                                 \
      (basic_vec & __x, const basic_vec &__y) _GLIBCXX_SIMD_NOEXCEPT {         \
...
#undef _GLIBCXX_SIMD_DEFINE_OP
  ...
  // [simd.cond] ---------------------------------------------------------
  [[__gnu__::__always_inline__]]
  friend constexpr basic_vec __select_impl(const mask_type &__k,
                                           const basic_vec &__t,
                                           const basic_vec &__f) noexcept {
...
#else
                                static_cast<__simd_size_type>(
                                    decltype(std::span(__r))::extent)>>;
#endif

template <size_t _Bytes, typename _Ap>
basic_vec(basic_mask<_Bytes, _Ap>) -> basic_vec<
    __integer_from<_Bytes>,
    decltype(__abi_rebind<__integer_from<_Bytes>,
                          basic_mask<_Bytes, _Ap>::size.value, _Ap>())>;

// [P3319R5] ----------------------------------------------------------------
template <__vectorizable _Tp>
  requires is_arithmetic_v<_Tp>
inline constexpr _Tp __iota<_Tp> = _Tp();

template <typename _Tp, typename _Ap>
inline constexpr basic_vec<_Tp, _Ap> __iota<basic_vec<_Tp, _Ap>> =
    basic_vec<_Tp, _Ap>([](_Tp __i) -> _Tp {
