...
inline void __throw_bad_weak_ptr() { _GLIBCXX_THROW_OR_ABORT(bad_weak_ptr()); }
...
private:
  ...
  __remove_cv_t<_Tp> *_M_ptr() noexcept { return _M_impl._M_storage._M_ptr(); }
...
#pragma GCC diagnostic ignored "-Wunused-local-typedefs"
      struct _Iter {
        ...
        bool operator==(const _Iter &__i) const { return _M_pos == __i._M_pos; }
      };
#pragma GCC diagnostic pop
...
public:
  ...
  __shared_count(_Tp *&__p, const _Sp_counted_array_base<_Alloc> &__a,
    ...
    if constexpr (alignof(_Up) < alignof(_Sp_ca_type)) {
      ...
      __c = std::align(alignof(_Sp_ca_type), sizeof(_Sp_ca_type), __c, __space);
    }
...
#pragma GCC diagnostic pop
  ...
  size_t _M_owner_hash() const noexcept {
...
inline _Tp *__shared_ptr_deref(_Tp *__p) {
...
#pragma GCC diagnostic pop
  ...
  template <typename _Tp1>
  bool owner_equal(__shared_ptr<_Tp1, _Lp> const &__rhs) const noexcept {
    return _M_refcount == __rhs._M_refcount;
  }

  template <typename _Tp1>
  bool owner_equal(__weak_ptr<_Tp1, _Lp> const &__rhs) const noexcept {
    return _M_refcount == __rhs._M_refcount;
  }
#endif
...
public:
  ...
  template <typename _Tp1>
  bool owner_equal(const __shared_ptr<_Tp1, _Lp> &__rhs) const noexcept {
    return _M_refcount == __rhs._M_refcount;
  }

  template <typename _Tp1>
  bool owner_equal(const __weak_ptr<_Tp1, _Lp> &__rhs) const noexcept {
    return _M_refcount == __rhs._M_refcount;
  }
#endif
