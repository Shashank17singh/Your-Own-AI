...
class copyable_function<_Res(_ArgTypes...)
                            _GLIBCXX_MOF_CV _GLIBCXX_MOF_REF noexcept(_Noex)>
  ...
  using __callable =
      __conditional_t<_Noex, is_nothrow_invocable_r<_Res, _Tp, _ArgTypes...>,
...
public:
  ...
  copyable_function(copyable_function const &__x)
      : _Base(static_cast<const _Base &>(__x)), _M_invoke(__x._M_invoke) {}
  ...
  explicit copyable_function(in_place_type_t<_Tp>, _Args &&...__args) noexcept(
      _S_nothrow_init<_Tp, _Args...>())
  ...
  explicit copyable_function(
      in_place_type_t<_Tp>, initializer_list<_Up> __il,
      _Args &&...__args) noexcept(_S_nothrow_init<_Tp, initializer_list<_Up> &,
                                                  _Args...>())
      : _M_invoke(_Invoker::template _S_storage<_Tp _GLIBCXX_MOF_INV_QUALS>()) {
    static_assert(is_same_v<decay_t<_Tp>, _Tp>);
    static_assert(is_copy_constructible_v<_Tp>);
  ...
  copyable_function &operator=(_Fn &&__f) noexcept(
      is_nothrow_constructible_v<copyable_function, _Fn>) {
   ...
   */
  _Res operator()(_ArgTypes... __args) _GLIBCXX_MOF_CV_REF noexcept(_Noex) {
...
private:
  ...
  friend auto &__polyfunc::__invoker_of(_Func &) noexcept;

  template <typename _Func>
  friend auto &__polyfunc::__base_of(_Func &) noexcept;

  template <typename _Dst, typename _Src>
  friend consteval bool __polyfunc::__is_invoker_convertible() noexcept;
};
