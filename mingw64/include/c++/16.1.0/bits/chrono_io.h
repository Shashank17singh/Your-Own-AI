...
constexpr basic_string_view<_CharT> __units_suffix() noexcept {
...
inline _Out __fmt_units_suffix(_Out __out) noexcept {
  if (auto __s = __detail::__units_suffix<_Period, _CharT>(); __s.size())
    ...
    return std::format_to(std::move(__out), _GLIBCXX_WIDEN("[{}/{}]s"),
                          (uintmax_t)_Period::num, (uintmax_t)_Period::den);
}
} // namespace __detail
/// @endcond
...
template <typename _Duration> struct __local_time_fmt {
...
inline void __not_valid_for_duration() {
  __throw_format_error("format error: chrono-format-spec not valid for "
                       "chrono::duration");
}

[[noreturn, __gnu__::__always_inline__]]
inline void __invalid_chrono_spec() {
  __throw_format_error("format error: chrono-format-spec not valid for "
                       "argument type");
}
...
constexpr _ChronoParts operator&(_ChronoParts __x, _ChronoParts __y) noexcept {
  return static_cast<_ChronoParts>((unsigned)__x & (unsigned)__y);
}

[[__gnu__::__always_inline__]]
constexpr _ChronoParts &operator&=(_ChronoParts &__x,
                                   _ChronoParts __y) noexcept {
  return __x = __x & __y;
}
...
[[__gnu__::__always_inline__]]
constexpr bool operator==(_ChronoParts __x, decltype(nullptr)) noexcept {
  return (unsigned short)__x == 0;
}

template <typename _CharT> struct _ChronoSpec : _Spec<_CharT> {
  ...
  [[__gnu__::__always_inline__]]
  constexpr bool _M_needs(_ChronoParts __parts) const {
...
template <typename _CharT> struct _ChronoFormats {
  ...
  static consteval _String_view _S_ftz() noexcept {
    return _GLIBCXX_WIDEN("%F %T %Z");
  }

  static consteval _String_view _S_ft() noexcept {
    return _S_ftz().substr(0, 5);
  }

  static consteval _String_view _S_f() noexcept {
    return _S_ftz().substr(0, 2);
  }

  static consteval _String_view _S_t() noexcept {
    return _S_ftz().substr(3, 2);
  }

  static consteval _String_view _S_ymd() noexcept {
    return _GLIBCXX_WIDEN("%Y/%b/%d");
  }

  static consteval _String_view _S_ym() noexcept {
    return _S_ymd().substr(0, 5);
  }

  static consteval _String_view _S_md() noexcept { return _S_ymd().substr(3); }

  static consteval _String_view _S_y() noexcept {
    return _S_ymd().substr(0, 2);
  }

  static consteval _String_view _S_m() noexcept {
    return _S_ymd().substr(3, 2);
  }

  static consteval _String_view _S_d() noexcept {
    return _S_ymd().substr(6, 2);
  }

  static consteval _String_view _S_ymwi() noexcept
  // %\0 is extension for handling weekday index
  {
    return _String_view(_GLIBCXX_WIDEN("%Y/%b/%a[%\0]"), 12);
  }

  static consteval _String_view _S_mwi() noexcept {
    return _S_ymwi().substr(3);
  }

  static consteval _String_view _S_wi() noexcept { return _S_ymwi().substr(6); }

  static consteval _String_view _S_w() noexcept {
    return _S_ymwi().substr(6, 2);
  }

  static consteval _String_view _S_ymwl() noexcept {
    return _GLIBCXX_WIDEN("%Y/%b/%a[last]");
  }

  static consteval _String_view _S_mwl() noexcept {
    return _S_ymwl().substr(3);
  }

  static consteval _String_view _S_wl() noexcept { return _S_ymwl().substr(6); }

  static consteval _String_view _S_yml() noexcept {
    return _GLIBCXX_WIDEN("%Y/%b/last");
  }

  static consteval _String_view _S_ml() noexcept { return _S_yml().substr(3); }
};

template <typename _CharT> struct _ChronoData {
  static constexpr unsigned _S_max_prec = 18;
  using _Attoseconds = chrono::duration<__UINT_LEAST64_TYPE__, atto>;

  using _FormatContext = basic_format_context<_Sink_iter<_CharT>, _CharT>;
  ...
  [[__gnu__::__always_inline__]]
  _ChronoParts _M_fill_year_month(const _YearMonth &__ym,
  ...
  [[__gnu__::__always_inline__]]
  _ChronoParts _M_fill_day(chrono::day __d, _ChronoParts __parts) {
    _M_day = __d;
  ...
  [[__gnu__::__always_inline__]]
  _ChronoParts _M_fill_weekday(chrono::weekday_indexed __wi,
                               _ChronoParts __parts) {
  ...
  // pre: _M_year is set
  [[__gnu__::__always_inline__]]
  _ChronoParts _M_fill_aux(chrono::local_days __ld, _ChronoParts __parts) {
  ...
  // pre: _M_year is set
  [[__gnu__::__always_inline__]]
  _ChronoParts _M_fill_ldays(chrono::local_days __ld, _ChronoParts __parts) {
    _M_ldays = __ld;
    __parts -= _ChronoParts::_LocalDays;
    return _M_fill_aux(__ld, __parts);
  }

  void _M_fill_time(chrono::seconds __d) {
  ...
  void _M_fill_date_time(chrono::local_seconds __ls, _ChronoParts __parts) {
  ...
  void _M_fill_zone(const char *__abbrev, const wchar_t *__wabbrev) {
...
template <typename _CharT> struct __formatter_chrono {
  ...
  constexpr explicit __formatter_chrono(_ChronoSpec<_CharT> __spec) noexcept
      : _M_spec(__spec) {}
  ...
  _M_parse(basic_format_parse_context<_CharT> &__pc, _ChronoParts __parts,
    ...
    auto __finalize = [this, &__spec, &__def] {
      ...
      _ChronoParts __checked =
          __spec._M_debug ? _YearMonthDay | _IndexedWeekday : _Month | _Weekday;
      // n.b. for calendar types __def._M_needed contains only parts
      // copied from the input, remaining ones are computed, and thus ok
      __spec._M_needs_ok_check = __spec._M_needs(__def._M_needed & __checked);
      _M_spec = __spec;
    };
  ...
  typename _FormatContext::iterator _M_format(const _ChronoData<_CharT> &__t,
...
protected:
  ...
  [[__gnu__::__always_inline__]]
  static _Dynamic_format_string<_CharT> _S_empty_fs() {
    return _Dynamic_format_string<_CharT>(_S_empty_spec);
  }

  static constexpr const _CharT *_S_weekdays[]{
      _GLIBCXX_WIDEN("Sunday"),   _GLIBCXX_WIDEN("Monday"),
      _GLIBCXX_WIDEN("Tuesday"),  _GLIBCXX_WIDEN("Wednesday"),
      _GLIBCXX_WIDEN("Thursday"), _GLIBCXX_WIDEN("Friday"),
      _GLIBCXX_WIDEN("Saturday"),
  };

  static constexpr const _CharT *_S_months[]{
      _GLIBCXX_WIDEN("January"),   _GLIBCXX_WIDEN("February"),
      _GLIBCXX_WIDEN("March"),     _GLIBCXX_WIDEN("April"),
      _GLIBCXX_WIDEN("May"),       _GLIBCXX_WIDEN("June"),
      _GLIBCXX_WIDEN("July"),      _GLIBCXX_WIDEN("August"),
      _GLIBCXX_WIDEN("September"), _GLIBCXX_WIDEN("October"),
      _GLIBCXX_WIDEN("November"),  _GLIBCXX_WIDEN("December"),
  };

private:
  template <typename _OutIter>
  _OutIter _M_write(_OutIter __out, [[maybe_unused]] const locale &__loc,
    ...
    if constexpr (is_same_v<_CharT, char>)
      if constexpr (__unicode::__literal_encoding_is_utf8())
        if (_M_spec._M_localized && _M_spec._M_locale_specific &&
          ...
          extern string_view __locale_encoding_to_utf8(const locale &,
                                                       string_view, void *);
  ...
  [[__gnu__::__always_inline__]]
  static bool _S_localized_spec(_CharT __conv, _CharT __mod) {
    switch (__conv) {
    case 'a':
    case 'A':
    case 'b':
  ...
  _Iter _M_locale_fmt(_Iter __out, const locale &__loc, const struct tm &__tm,
  ...
  __string_view _M_check_ok(const _ChronoData<_CharT> &__t,
                            _CharT &__conv) const {
    if (!_M_spec._M_debug) {
      switch (__conv) {
      case 'a':
  ...
  _OutIter _M_format_to(const _ChronoData<_CharT> &__t, _OutIter __out,
                        _FormatContext &__fc) const {
    ...
    do {
      ...
      if (__invalid.empty() && __use_locale_fmt &&
        ...
        __out =
            _M_locale_fmt(std::move(__out), __fc.locale(), __tm, __c, __mod);
      else
        switch (__c) {
        // %\0 is extension for handling weekday index
        case '\0':
  ...
  template <typename _OutIter>
  _OutIter _M_wi(unsigned __wi, _OutIter __out) const {
  ...
  template <typename _OutIter>
  _OutIter _M_a_A(chrono::weekday __wd, _OutIter __out, bool __full) const {
  ...
  template <typename _OutIter>
  _OutIter _M_b_B(chrono::month __m, _OutIter __out, bool __full) const {
  ...
  template <typename _OutIter>
  _OutIter _M_c(const _ChronoData<_CharT> &__t, _OutIter __out) const {
    // %c  Locale's date and time representation, for C-locale: %a %b %e %T %Y
    // %Ec Locale's alternate date and time representation, for C-locale same as
    // above
  ...
  template <typename _OutIter>
  _OutIter _M_C_y_Y(chrono::year __y, _OutIter __out, _CharT __conv) const {
  ...
  template <typename _OutIter>
  _OutIter _M_D_x(const _ChronoData<_CharT> &__t, _OutIter __out) const {
    ...
    if (__mi >= 100 || __di >= 100) [[unlikely]] {
      ...
      __out = std::format_to(std::move(__out), _FmtStr(__fs), __mi, __di, __yi);
    } else {
      _CharT __buf[8];
      __buf[2] = _S_slash;
      __buf[5] = _S_slash;
  ...
  template <typename _OutIter>
  _OutIter _M_d_e(chrono::day __d, _OutIter __out, _CharT __conv) const {
  ...
  template <typename _OutIter>
  _OutIter _M_F(const _ChronoData<_CharT> &__t, _OutIter __out) const {
    auto __di = (unsigned)__t._M_day;
    auto __mi = (unsigned)__t._M_month;
    ...
    if (__yi >= 10000 || __mi >= 100 || __di >= 100) [[unlikely]] {
      using _FmtStr = _Dynamic_format_string<_CharT>;
      __string_view __fs = _GLIBCXX_WIDEN("-{:04d}-{:02d}-{:02d}") + !__is_neg;
      __out = std::format_to(std::move(__out), _FmtStr(__fs), __yi, __mi, __di);
    } else {
      _CharT __buf[11];
      __buf[0] = _S_plus_minus[1];
  ...
  template <typename _OutIter>
  _OutIter _M_g_G_V(const _ChronoData<_CharT> &__t, _OutIter __out,
                    _CharT __conv) const {
  ...
  template <typename _OutIter>
  _OutIter _M_H_I(chrono::hours __h, _OutIter __out, _CharT __conv) const {
  ...
  template <typename _OutIter>
  _OutIter _M_j(const _ChronoData<_CharT> &__t, _OutIter __out) const {
  ...
  template <typename _OutIter>
  _OutIter _M_m(chrono::month __m, _OutIter __out) const {
  ...
  template <typename _OutIter>
  _OutIter _M_M(chrono::minutes __m, _OutIter __out) const {
  ...
  template <typename _OutIter>
  _OutIter _M_p(chrono::hours __h, _OutIter __out) const {
  ...
  template <typename _OutIter>
  _OutIter _M_q(__string_view __us, _OutIter __out) const {
    // %q The duration's unit suffix
    return __format::__write(std::move(__out), __us);
  }

  template <typename _OutIter, typename _FormatContext>
  _OutIter _M_Q(const _ChronoData<_CharT> &__t, _OutIter __out,
  ...
  template <typename _OutIter>
  _OutIter _M_r(const _ChronoData<_CharT> &__t, _OutIter __out) const {
  ...
  template <typename _OutIter>
  _OutIter _M_R_X(const _ChronoData<_CharT> &__t, _OutIter __out,
                  bool __secs) const {
    // %R  Equivalent to %H:%M
  ...
  template <typename _OutIter, typename _FormatContext>
  _OutIter _M_S(const _ChronoData<_CharT> &__t, _OutIter __out,
                _FormatContext &__ctx, bool __subs = true) const {
    // %S  Seconds as a decimal number.
    // %OS The locale's alternative representation.
    auto __s = __t._M_seconds;

    __out = __format::__write(std::move(__out), _S_two_digits(__s.count()));
    if (__subs)
  ...
  template <typename _OutIter, typename _FormatContext>
  _OutIter _M_subsecs(const _ChronoData<_CharT> &__t, _OutIter __out,
    ...
    *__out = __dot;
    ...
    if (_M_spec._M_floating_point_rep) {
      ...
      if (_M_spec._M_localized && _M_spec._M_custom_rep) [[unlikely]]
        ...
        std::vformat_to(__sink.out(), _GLIBCXX_WIDEN("{1:0.{2}f}"),
                        __t._M_ereps);
    ...
    using _FmtStr = _Dynamic_format_string<_CharT>;
    return std::format_to(__out, _FmtStr(_GLIBCXX_WIDEN("{0:0{1}}")), __subs,
                          __prec);
  }
  ...
  template <typename _OutIter, typename _FormatContext>
  _OutIter _M_T(const _ChronoData<_CharT> &__t, _OutIter __out,
                _FormatContext &__ctx) const {
  ...
  template <typename _OutIter>
  _OutIter _M_u_w(chrono::weekday __wd, _OutIter __out, _CharT __conv) const {
    ...
    unsigned __wdi = __conv == 'u' ? __wd.iso_encoding() : __wd.c_encoding();
    _CharT __buf[3];
    return __format::__write(std::move(__out), _S_str_d1(__buf, __wdi));
  }

  template <typename _OutIter>
  _OutIter _M_U_W(const _ChronoData<_CharT> &__t, _OutIter __out,
                  _CharT __conv) const {
    ...
    auto __weeks =
        chrono::floor<weeks>(__t._M_day_of_year - __offset - days(1));
    return __format::__write(std::move(__out),
                             _S_two_digits(__weeks.count() + 1));
  }

  template <typename _OutIter>
  _OutIter _M_z(chrono::seconds __ts, _OutIter __out,
                bool __mod = false) const {
    if (__ts == 0s) {
  ...
  template <typename _OutIter>
  _OutIter _M_Z(__string_view __abbrev, _OutIter __out) const {
  ...
  static basic_string_view<_CharT> _S_digit(int __n) noexcept {
  ...
  static basic_string_view<_CharT> _S_two_digits(int __n) noexcept {
    return {_GLIBCXX_WIDEN("0001020304050607080910111213141516171819"
  ...
  [[__gnu__::__always_inline__]]
  static void _S_fill_two_digits(_CharT *__buf, unsigned __n) {
  ...
  [[__gnu__::__always_inline__]]
  static void _S_fill_ampm(_CharT *__buf, chrono::hours __h) {
    auto __hi = __h.count();
    if (__hi >= 24) [[unlikely]]
      __hi %= 24;
  ...
  [[__gnu__::__always_inline__]]
  static basic_string_view<_CharT> _S_str_d1(span<_CharT, 3> __buf,
                                             unsigned __n) {
    if (__n < 10) [[likely]]
      return _S_digit(__n);
  ...
  // Returned string_view may point to __buf.
  [[__gnu__::__always_inline__]]
  static basic_string_view<_CharT> _S_str_d2(span<_CharT, 3> __buf,
                                             unsigned __n) {
    if (__n < 100) [[likely]]
  ...
  [[__gnu__::__always_inline__]]
  static basic_string_view<_CharT> _S_str_d3(span<_CharT, 3> __buf,
                                             unsigned __n) {
...
public:
  template <typename _Duration>
  static consteval _ChronoSpec<_CharT> _S_spec_for(_ChronoParts __parts) {
  ...
  template <typename _Duration>
  static consteval _ChronoSpec<_CharT> _S_spec_for_tp() {
    using enum _ChronoParts;
  ...
  _M_parse(basic_format_parse_context<_CharT> &__pc, _ChronoParts __parts,
    ...
    auto __res = __formatter_chrono<_CharT>::_M_parse(__pc, __parts, __def);
    ...
    if constexpr (!is_floating_point_v<_Rep>)
      if constexpr (chrono::treat_as_floating_point_v<_Rep>)
        if (_M_spec._M_needs(_Subseconds | _EpochUnits) ||
            _M_spec._M_prec_kind != _WP_none || _M_spec._M_prec_value > 0) {
  ...
  template <typename _FormatContext>
  std::locale _M_locale(_FormatContext &__fc) const {
  ...
  _M_format_to_ostream(const chrono::duration<_Rep, _Period> &__d,
    ...
    return __format::__write_padded_as_spec(__str, __str.size(), __fc, _M_spec);
  }

  template <typename _Rep1, typename _Period1, typename _Rep2,
            typename _Period2, typename _FormatContext>
  typename _FormatContext::iterator
  _M_format_units(_ChronoData<_CharT> &__cd,
    ...
    if constexpr (!_S_unit_suffix.empty())
      ...
      chrono::__detail::__fmt_units_suffix<_Period1, _CharT>(
          __suffix_store.out());
...
template <typename _CharT> struct __formatter_chrono_info {
  constexpr typename basic_format_parse_context<_CharT>::iterator
  ...
  format(const _Info &__i, basic_format_context<_Out, _CharT> &__fc) const {
...
private:
  ...
  _Out _M_format_to(_Out __out, const chrono::sys_info &__si) const {
    using _FmtStr = _Dynamic_format_string<_CharT>;
  ...
  template <typename _Out>
  _Out _M_format_to(_Out __out, const chrono::local_info &__li) const {
...
private:
  ...
  static constexpr __format::_ChronoSpec<_CharT> __defSpec = [] {
    using enum __format::_ChronoParts;
    auto __res =
        __format::__formatter_duration<_CharT>::template _S_spec_for<_Duration>(
            _None);
  ...
  typename basic_format_context<_Out, _CharT>::iterator
  _M_format(const chrono::duration<_Rep2, _Period> &__d, bool __is_neg,
            basic_format_context<_Out, _CharT> &__fc) const {
    using namespace chrono;
    using enum __format::_ChronoParts;
    if constexpr (!is_integral_v<_Rep>)
...
template <__format::__char _CharT> struct formatter<chrono::day, _CharT> {
  constexpr typename basic_format_parse_context<_CharT>::iterator
  parse(basic_format_parse_context<_CharT> &__pc) {
    using enum __format::_ChronoParts;
...
#if __glibcxx_print >= 202406L
template <>
inline constexpr bool enable_nonlocking_formatter_optimization<chrono::day> =
    true;
#endif

template <__format::__char _CharT> struct formatter<chrono::month, _CharT> {
  constexpr typename basic_format_parse_context<_CharT>::iterator
  parse(basic_format_parse_context<_CharT> &__pc) {
    using enum __format::_ChronoParts;
...
#if __glibcxx_print >= 202406L
template <>
inline constexpr bool enable_nonlocking_formatter_optimization<chrono::month> =
    true;
#endif

template <__format::__char _CharT> struct formatter<chrono::year, _CharT> {
  constexpr typename basic_format_parse_context<_CharT>::iterator
  parse(basic_format_parse_context<_CharT> &__pc) {
    using enum __format::_ChronoParts;
...
#if __glibcxx_print >= 202406L
template <>
inline constexpr bool enable_nonlocking_formatter_optimization<chrono::year> =
    true;
#endif

template <__format::__char _CharT> struct formatter<chrono::weekday, _CharT> {
  constexpr typename basic_format_parse_context<_CharT>::iterator
  parse(basic_format_parse_context<_CharT> &__pc) {
    using enum __format::_ChronoParts;
...
template <__format::__char _CharT> struct formatter<chrono::month_day, _CharT> {
  constexpr typename basic_format_parse_context<_CharT>::iterator
  parse(basic_format_parse_context<_CharT> &__pc) {
    using enum __format::_ChronoParts;
...
struct formatter<chrono::hh_mm_ss<chrono::duration<_Rep, _Period>>, _CharT> {
  ...
  format(const chrono::hh_mm_ss<chrono::duration<_Rep, _Period>> &__t,
    ...
    if (_M_f._M_spec._M_needs(_TotalSeconds))
      __cd._M_eseconds = __cd._M_hours + __cd._M_minutes + __cd._M_seconds;
...
private:
  ...
  static constexpr __format::_ChronoSpec<_CharT> __defSpec =
      __format::__formatter_duration<_CharT>::template _S_spec_for<_Precision>(
          __format::_ChronoParts::_Time);
...
#if _GLIBCXX_USE_CXX11_ABI || !_GLIBCXX_USE_DUAL_ABI
template <__format::__char _CharT> struct formatter<chrono::sys_info, _CharT> {
  constexpr typename basic_format_parse_context<_CharT>::iterator
  parse(basic_format_parse_context<_CharT> &__pc) {
...
private:
  static constexpr __format::_ChronoSpec<_CharT> __defSpec =
      __format::__formatter_duration<_CharT>::template _S_spec_for_tp<
          _Duration>();
...
private:
  static constexpr __format::_ChronoSpec<_CharT> __defSpec =
      __format::__formatter_duration<_CharT>::template _S_spec_for<_Duration>(
          __format::_ChronoParts::_DateTime);
...
struct formatter<chrono::local_time<_Duration>, _CharT> {
  ...
  parse(basic_format_parse_context<_CharT> &__pc) {
    using enum __format::_ChronoParts;
    auto __res = _M_f.template _M_parse<_Duration>(__pc, _DateTime, __defSpec);
    if constexpr (__defSpec._M_chrono_specs.empty())
      if (_M_f._M_spec._M_chrono_specs.empty())
        __format::__invalid_chrono_spec(); // chrono-specs can't be empty
    return __res;
...
private:
  static constexpr __format::_ChronoSpec<_CharT> __defSpec =
      __format::__formatter_duration<_CharT>::template _S_spec_for<_Duration>(
          __format::_ChronoParts::_ZonedDateTime);
...
/// @cond undocumented
namespace __detail {
template <typename _Duration = seconds> struct _Parser {
  static_assert(is_same_v<common_type_t<_Duration, seconds>, _Duration>);

  explicit _Parser(__format::_ChronoParts __need) : _M_need(__need) {}
...
private:
  ...
  static int_least32_t _S_read_unsigned(basic_istream<_CharT, _Traits> &__is,
  ...
  // Extract no more than __n digits. Set failbit if an integer isn't read.
  template <typename _CharT, typename _Traits>
  static int_least32_t _S_read_signed(basic_istream<_CharT, _Traits> &__is,
                                      ios_base::iostate &__err, int __n) {
  ...
  template <typename _CharT, typename _Traits>
  static int_least32_t _S_try_read_digit(basic_istream<_CharT, _Traits> &__is,
  ...
  template <typename _CharT, typename _Traits>
  static bool _S_read_chr(basic_istream<_CharT, _Traits> &__is,
...
template <typename _Duration> consteval bool __use_floor() {
...
constexpr auto __round(const _Tp &__t) {
...
template <typename _CharT, typename _Traits,
          typename _Alloc = allocator<_CharT>>
inline basic_istream<_CharT, _Traits> &
from_stream(basic_istream<_CharT, _Traits> &__is, const _CharT *__fmt, day &__d,
            basic_string<_CharT, _Traits, _Alloc> *__abbrev = nullptr,
            minutes *__offset = nullptr) {
...
from_stream(basic_istream<_CharT, _Traits> &__is, const _CharT *__fmt,
            ...
            minutes *__offset = nullptr) {
  using __format::_ChronoParts;
  auto __need = _ChronoParts::_Year | _ChronoParts::_Month | _ChronoParts::_Day;
  __detail::_Parser<> __p(__need);
  if (__p(__is, __fmt, __abbrev, __offset))
    __ymd = __p._M_ymd;
  return __is;
...
from_stream(basic_istream<_CharT, _Traits> &__is, const _CharT *__fmt,
  ...
  if (__p(__is, __fmt, __abbrev, __offset)) {
    ...
    auto __ut = utc_clock::from_sys(__p._M_sys_days) + __p._M_time - *__offset;
...
template <typename _Parsable, typename _CharT,
          typename _Traits = std::char_traits<_CharT>, typename... _OptArgs>
concept __parsable = requires(
    basic_istream<_CharT, _Traits> &__is, const _CharT *__fmt, _Parsable &__tp,
    _OptArgs *...__args) { from_stream(__is, __fmt, __tp, __args...); };
...
public:
  _Parse(const _CharT *__fmt, _Parsable &__tp,
         basic_string<_CharT, _Traits, _Alloc> *__abbrev = nullptr,
         minutes *__offset = nullptr)
      : _M_fmt(__fmt), _M_tp(std::__addressof(__tp)), _M_abbrev(__abbrev),
        _M_offset(__offset) {}
...
private:
  ...
  friend __stream_type &operator>>(__stream_type &__is, _Parse &&__p) {
    if (__p._M_offset)
      from_stream(__is, __p._M_fmt, *__p._M_tp, __p._M_abbrev, __p._M_offset);
...
inline auto parse(const _CharT *__fmt, _Parsable &__tp) {
...
[[nodiscard]]
inline auto parse(const basic_string<_CharT, _Traits, _Alloc> &__fmt,
                  _Parsable &__tp) {
...
[[nodiscard, __gnu__::__access__(__read_only__, 1)]]
inline auto parse(const _CharT *__fmt, _Parsable &__tp, minutes &__offset) {
  return __detail::_Parse<_Parsable, _CharT>(__fmt, __tp, nullptr, &__offset);
}
...
[[nodiscard]]
inline auto parse(const basic_string<_CharT, _Traits, _Alloc> &__fmt,
                  _Parsable &__tp, minutes &__offset) {
  return __detail::_Parse<_Parsable, _CharT, _Traits, _Alloc>(
      __fmt.c_str(), __tp, nullptr, &__offset);
}
...
inline auto parse(const _CharT *__fmt, _Parsable &__tp,
                  ...
                  minutes &__offset) {
  auto __pa = std::__addressof(__abbrev);
  return __detail::_Parse<_Parsable, _CharT, _Traits, _Alloc>(__fmt, __tp, __pa,
                                                              &__offset);
}
...
parse(const basic_string<_CharT, _Traits, _Alloc> &__fmt, _Parsable &__tp,
  ...
  auto __pa = std::__addressof(__abbrev);
  return __detail::_Parse<_Parsable, _CharT, _Traits, _Alloc>(
      __fmt.c_str(), __tp, __pa, &__offset);
}

/// @cond undocumented
template <typename _Duration>
template <typename _CharT, typename _Traits, typename _Alloc>
basic_istream<_CharT, _Traits> &__detail::_Parser<_Duration>::operator()(
    basic_istream<_CharT, _Traits> &__is, const _CharT *__fmt,
    basic_string<_CharT, _Traits, _Alloc> *__abbrev, minutes *__offset) {
  ...
  if (sentry __cerb(__is, true); __cerb) {
    ...
    struct _Stream_state {
      explicit _Stream_state(basic_istream<_CharT, _Traits> &__i)
          : _M_is(__i), _M_flags(__i.flags(ios_base::skipws | ios_base::dec)),
            _M_w(__i.width(0)) {}
    ...
    while (*__fmt) {
      ...
      case 'A': // (full or abbreviated, matched case-insensitively).
        if (__mod || __num) [[unlikely]]
          ...
          struct tm __tm{};
          __tmget.get(__is, {}, __is, __err, &__tm, __fmt - 2, __fmt);
      ...
      case 'c': // Locale's date and time representation.
        if (__mod == 'O' || __num) [[unlikely]]
          ...
          struct tm __tm{};
          __tmget.get(__is, {}, __is, __err, &__tm, __fmt - 2 - (__mod == 'E'),
                      __fmt);
          if (!__is_failed(__err)) {
      ...
      case 'C': // Century
        if (!__mod) [[likely]] {
          ...
          struct tm __tm{};
          __tmget.get(__is, {}, __is, __err, &__tm, __fmt - 3, __fmt);
          if (!__is_failed(__err))
      ...
      case 'S': // Seconds
        if (__mod == 'E') [[unlikely]]
          ...
          if (!__is_failed(__err))
            __s = seconds(__tm.tm_sec);
        } else if constexpr (_Duration::period::den == 1 && !__is_floating) {
          auto __val = __read_unsigned(__num ? __num : 2);
        ...
        } else // Read fractional seconds
        {
          ...
          if (_Traits::eq_int_type(__i, _Traits::eof()))
            ...
            if (__c == __dp) {
              (void)__is.get();
              __buf.put('.');
              int __prec = hh_mm_ss<_Duration>::fractional_width;
              do {
                __digit = _S_try_read_digit(__is, __err);
                if (__digit != -1)
                  __buf.put('0' + __digit);
          ...
          if (!__is_failed(__err)) [[likely]] {
            ...
            auto [ptr, ec] = std::from_chars(__first, __last, __val, fixed);
        ...
        if (__mod || __num) [[unlikely]]
          ...
          while (true) {
            ...
            if (!_Traits::eq_int_type(__i, _Traits::eof())) {
              ...
              if (std::isalnum(__a, __loc) || __x.find(__a) != __x.npos) {
      ...
      default:
        if (_CharT('1') <= __c && __c <= _CharT('9')) {
          if (!__mod) [[likely]] {
            ...
            auto [__v, __ptr] = __format::__parse_integer(__fmt - 1, __end);
            if (__ptr) [[likely]] {
              __num = __v;
              __fmt = __ptr;
              continue;
    ...
    if (__is_failed(__err))
      [[unlikely]];                  // Don't bother doing any more work.
        ...
        if (((_M_need & _ChronoParts::_Year) != 0 && __y == __bad_y) ||
          ...
          if (__can_use_doy) {
            if ((0 < __dayofyear && __dayofyear <= 365) ||
                (__dayofyear == 366 && __y.is_leap())) [[likely]] {
              _M_sys_days = sys_days(__y / January / 1) + days(__dayofyear - 1);
              if (__need_ymd)
            ...
            _M_sys_days =
                __wk1 + weeks(__sunday_wk - 1) + days(__wday.c_encoding());
            _M_ymd = year_month_day(_M_sys_days);
            ...
            _M_sys_days =
                __wk1 + weeks(__monday_wk - 1) + days(__wday.c_encoding() - 1);
            _M_ymd = year_month_day(_M_sys_days);
            if (_M_ymd.year() != __y) [[unlikely]]
              __err |= ios_base::failbit;
