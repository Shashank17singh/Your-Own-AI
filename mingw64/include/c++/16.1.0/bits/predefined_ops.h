...
using __by_ref_or_value_fn = std::__conditional_t<
    std::__or_<std::is_empty<_Fn>, std::is_scalar<_Fn>>::value, _Fn, _Fn &>;
...
struct _Comp_with_val {
  ...
  explicit constexpr _Comp_with_val(_Fn __f, const _Value &__v)
      : _M_f(__f), _M_val(__v) {}

  [[__no_unique_address__]] _Fn _M_f;
  const _Value &_M_val;

  template <typename _Tp> _GLIBCXX14_CONSTEXPR bool operator()(_Tp &&__arg) {
...
template <typename _Func> struct _Unary_negate {
  using _Fn = __by_ref_or_value_fn<_Func>;

  explicit constexpr _Unary_negate(_Fn __f) : _M_f(__f) {}
...
template <typename _Func> constexpr _Unary_negate<_Func> not1(_Func &__f) {
...
template <typename _Fn>
struct __by_ref_or_value_fn : __conditional_type<__is_empty(_Fn), _Fn, _Fn &> {
};

template <typename _Fn> struct __by_ref_or_value_fn<_Fn *> {
  typedef _Fn *__type;
};
...
template <typename _Func, typename _Value> struct _Comp_with_val_1st {
  typedef typename __by_ref_or_value_fn<_Func>::__type _Fn;

  explicit _Comp_with_val_1st(_Fn __f, const _Value &__v)
      : _M_f(__f), _M_val(__v) {}

  _Fn _M_f;
  const _Value &_M_val;

  template <typename _Tp> bool operator()(_Tp &__arg) {
    return _M_f(_M_val, __arg);
  }
  template <typename _Tp> bool operator()(const _Tp &__arg) {
    return _M_f(_M_val, __arg);
  }
};

template <typename _Func, typename _Value> struct _Comp_with_val_2nd {
  typedef typename __by_ref_or_value_fn<_Func>::__type _Fn;

  explicit _Comp_with_val_2nd(_Fn __f, const _Value &__v)
      : _M_f(__f), _M_val(__v) {}
...
struct _Unary_negate_1 // N.B. different name for C++98 to satisfy ODR
{
  ...
  template <typename _Tp> bool operator()(_Tp &__arg) { return !_M_f(__arg); }
  template <typename _Tp> bool operator()(const _Tp &__arg) {
    return !_M_f(__arg);
  }
};

template <typename _Func> inline _Unary_negate_1<_Func> not1(_Func &__f) {
