...
protected:
  ...
  constexpr void _M_append_range_to(_Rg &&__rg, pointer &__ptr) {
    __ptr = std::__uninitialized_copy_a(ranges::begin(__rg), ranges::end(__rg),
  ...
  template <ranges::input_range _Rg>
  constexpr void _M_append_range(_Rg &&__rg) {
...
public:
  ...
  vector(const allocator_type &__a) _GLIBCXX_NOEXCEPT : _Base(__a) {}
...
public:
  ...
  constexpr vector(from_range_t, _Rg &&__rg, const _Alloc &__a = _Alloc())
      : vector(__a) {
    if constexpr (ranges::forward_range<_Rg> || ranges::sized_range<_Rg>) {
      const auto __n = static_cast<size_type>(ranges::distance(__rg));
      _M_range_initialize_n(ranges::begin(__rg), ranges::end(__rg), __n);
    } else {
  ...
  constexpr void assign_range(_Rg &&__rg) {
    ...
    if constexpr (ranges::forward_range<_Rg> || ranges::sized_range<_Rg>) {
      ...
      reserve(__n);
      auto __first =
          ranges::copy_n(ranges::begin(__rg), size(), this->_M_impl._M_start)
              .in;
      [[maybe_unused]] const auto __diff = __n - size();
      _GLIBCXX_ASAN_ANNOTATE_GROW(__diff);
      _Base::_M_append_range(
          ranges::subrange(std::move(__first), ranges::end(__rg)));
  ...
  capacity() const _GLIBCXX_NOEXCEPT {
    ptrdiff_t __dif = this->_M_impl._M_end_of_storage - this->_M_impl._M_start;
    if (__dif < 0)
      __builtin_unreachable();
    return size_type(__dif);
  }
...
public:
   ...
   */
  template <__detail::__container_compatible_range<_Tp> _Rg>
  constexpr iterator insert_range(const_iterator __pos, _Rg &&__rg);
   ...
   */
  template <__detail::__container_compatible_range<_Tp> _Rg>
  constexpr void append_range(_Rg &&__rg) {
    ...
    if constexpr (ranges::forward_range<_Rg> || ranges::sized_range<_Rg>) {
      ...
      std::__uninitialized_copy_a(ranges::begin(__rg), ranges::end(__rg), __mid,
                                  __a);
      ...
      if constexpr (_S_use_relocate())
        ...
        struct _Guard_elts {
          ...
          constexpr _Guard_elts(pointer __f, pointer __l, _Tp_alloc_type &__a)
              : _M_first(__f), _M_last(__l), _M_alloc(__a) {}

          constexpr ~_Guard_elts() {
        ...
        std::__uninitialized_move_a(__old_start, __old_finish, __start, __a);
      ...
      for (auto __free = capacity() - size(); __first != __last && __free > 0;
           ++__first, (void)--__free)
        emplace_back(*__first);
...
protected:
  ...
  _GLIBCXX20_CONSTEXPR pointer _M_allocate_and_copy(size_type __n,
    ...
    std::__uninitialized_copy_a(__first, __last, __guard._M_storage,
                                _M_get_Tp_allocator());
  ...
  _GLIBCXX20_CONSTEXPR void _M_range_initialize(_ForwardIterator __first,
    ...
    _M_range_initialize_n(__first, __last, std::distance(__first, __last));
  }

  template <typename _Iterator, typename _Sentinel>
  _GLIBCXX20_CONSTEXPR void
  _M_range_initialize_n(_Iterator __first, _Sentinel __last, size_type __n) {
    pointer __start =
        this->_M_allocate(_S_check_init_len(__n, _M_get_Tp_allocator()));
    this->_M_impl._M_start = this->_M_impl._M_finish = __start;
    this->_M_impl._M_end_of_storage = __start + __n;
    this->_M_impl._M_finish = std::__uninitialized_copy_a(
        _GLIBCXX_MOVE(__first), __last, __start, _M_get_Tp_allocator());
  }
  ...
  template <typename _InputIterator>
  _GLIBCXX20_CONSTEXPR void
  _M_range_insert(iterator __pos, _InputIterator __first, _InputIterator __last,
                  std::input_iterator_tag);
  ...
  _GLIBCXX20_CONSTEXPR
  void _M_fill_append(size_type __n, const value_type &__x);
...
#else
  template <typename _Ptr> value_type *_M_data_ptr(_Ptr __ptr) const {
