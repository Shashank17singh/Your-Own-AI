...
protected:
  ...
  struct _Deque_impl_data {
    ...
    _Deque_impl_data(_Deque_impl_data &&__x) noexcept : _Deque_impl_data(__x) {
...
public:
  ...
  constexpr void assign_range(_Rg &&__rg) {
    ...
    if constexpr (ranges::forward_range<_Rg> || ranges::sized_range<_Rg>) {
      ...
      auto __rest = ranges::copy_n(ranges::begin(__rg), size(), begin()).in;
      _M_range_append(std::move(__rest), ranges::end(__rg), __n - size());
    } else {
      auto __first = ranges::begin(__rg);
      const auto __last = ranges::end(__rg);
      for (iterator __it = begin(), __end = end(); __it != __end;
           (void)++__first, ++__it) {
        if (__first == __last)
...
protected:
  ...
  // insert [__first, __last) at the front, assumes distance(__first, __last) is
  // n
  template <typename _InputIterator, typename _Sentinel>
  void _M_range_prepend(_InputIterator __first, _Sentinel __last,
                        size_type __n);

  // insert [__first, __last) at the back, assumes distance(__first, __last) is
  // n
  template <typename _InputIterator, typename _Sentinel>
  ...
  struct _Temporary_value {
    template <typename... _Args>
    _GLIBCXX20_CONSTEXPR explicit _Temporary_value(deque *__deque,
                                                   _Args &&...__args)
        : _M_this(__deque) {
    ...
    _GLIBCXX20_CONSTEXPR value_type &_M_val() noexcept { return __tmp_val; }

  private:
    _GLIBCXX20_CONSTEXPR _Tp *_M_ptr() noexcept {
      return std::__addressof(__tmp_val);
    }
 ...
 */
template <typename _Tp, typename _Alloc>
_GLIBCXX_NODISCARD inline bool operator==(const deque<_Tp, _Alloc> &__x,
...
template <typename _Tp, typename _Alloc>
[[nodiscard]]
inline __detail::__synth3way_t<_Tp> operator<=>(const deque<_Tp, _Alloc> &__x,
                                                const deque<_Tp, _Alloc> &__y) {
