...
struct common_type<chrono::duration<_Rep1, _Period1>,
    ...
    : __duration_common_type<common_type<_Rep1, _Rep2>, typename _Period1::type,
...
template <> inline constexpr bool treat_as_floating_point_v<long long> = false;
...
template <> inline constexpr bool treat_as_floating_point_v<long double> = true;
#endif // C++17
...
inline constexpr bool is_clock_v<
    _Tp, void_t<typename _Tp::rep, typename _Tp::period, typename _Tp::duration,
                typename _Tp::time_point::duration, decltype(_Tp::is_steady),
                decltype(_Tp::now())>> =
    __and_v<is_same<typename _Tp::duration,
                    duration<typename _Tp::rep, typename _Tp::period>>,
            is_same<typename _Tp::time_point::duration, typename _Tp::duration>,
...
public:
  ...
  _GLIBCXX17_CONSTEXPR duration operator++(int) { return duration(__r++); }
...
_GLIBCXX_NODISCARD constexpr __enable_if_t<__is_duration<_ToDur>::value,
...
template <typename _ToDur, typename _Clock, typename _Dur>
[[nodiscard]] constexpr enable_if_t<
    __is_duration_v<_ToDur> && !treat_as_floating_point_v<typename _ToDur::rep>,
    time_point<_Clock, _ToDur>>
...
struct __sys_t;
template <typename _Duration> using sys_time = time_point<__sys_t, _Duration>;
using sys_seconds = sys_time<seconds>;
using sys_days = sys_time<days>;
...
template <typename _Rep, typename _Period>
[[__nodiscard__]] _GLIBCXX14_CONSTEXPR inline struct ::timespec
