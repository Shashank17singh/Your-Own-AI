...
__wait_result_type __wait_until_impl(const void *__addr,
...
__wait_until(const void *__addr, __wait_args_base &__args,
  ...
  auto __res =
      __detail::__wait_until_impl(__addr, __args, __at.time_since_epoch());
...
template <typename _Tp, typename _Pred, typename _ValFn, typename _Clock,
          typename _Dur>
...
bool __atomic_wait_address_until_v(
    ...
    __detail::__platform_wait_t __old, int __order,
    const chrono::time_point<_Clock, _Dur> &__atime,
    bool __bare_wait = false) noexcept {
...
template <typename _Tp, typename _Pred, typename _ValFn, typename _Rep,
          typename _Period>
bool __atomic_wait_address_for(const _Tp *__addr, _Pred &&__pred,
                               _ValFn &&__vfn,
...
template <typename _Rep, typename _Period>
bool __atomic_wait_address_for_v(const __detail::__platform_wait_t *__addr,
                                 __detail::__platform_wait_t __old, int __order,
                                 const chrono::duration<_Rep, _Period> &__rtime,
                                 bool __bare_wait = false) noexcept {
  // This function must not be used if __wait_impl might use a proxy wait:
...
bool __atomic_wait_address_for_v(const _Tp *__addr, _Tp &&__old, _ValFn &&__vfn,
                                 const chrono::duration<_Rep, _Period> &__rtime,
                                 bool __bare_wait = false) noexcept {
  auto __pfn = [&](const _Tp &__val) {
    return !__detail::__atomic_eq(__old, __val);
