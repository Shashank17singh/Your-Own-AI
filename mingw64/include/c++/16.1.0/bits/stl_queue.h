...
_GLIBCXX_NODISCARD inline bool operator==(const queue<_Tp, _Seq> &__x,
...
public:
  ...
  priority_queue(priority_queue &&__q) noexcept(
      __and_<is_nothrow_move_constructible<_Sequence>,
             is_nothrow_move_constructible<_Compare>>::value)
      : c(std::move(__q.c)), comp(std::move(__q.comp)) {
    __q.c.clear();
  }

  priority_queue &operator=(priority_queue &&__q) noexcept(
      __and_<is_nothrow_move_assignable<_Sequence>,
  ...
  template <typename _InputIterator, typename _Alloc,
            typename _Requires = _Uses<_Alloc>>
  priority_queue(_InputIterator __first, _InputIterator __last,
                 const _Compare &__x, _Sequence &&__s, const _Alloc &__alloc)
...
priority_queue(from_range_t, _Rg &&, _Compare = _Compare(), _Alloc = _Alloc())
    -> priority_queue<ranges::range_value_t<_Rg>,
                      vector<ranges::range_value_t<_Rg>, _Alloc>, _Compare>;
