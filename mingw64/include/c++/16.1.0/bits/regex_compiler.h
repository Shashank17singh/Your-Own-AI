...
public:
  ...
  shared_ptr<const _RegexT> _M_get_nfa() noexcept { return std::move(_M_nfa); }
...
private:
  ...
  struct _BracketState {
    ...
    void set(_CharT __c) noexcept {
      _M_type = _Type::_Char;
      _M_char = __c;
    }

    _GLIBCXX_NODISCARD _CharT get() const noexcept { return _M_char; }

    void reset(_Type __t = _Type::_None) noexcept { _M_type = __t; }

    explicit operator bool() const noexcept { return _M_type != _Type::_None; }

    // Previous token was a single character.
    _GLIBCXX_NODISCARD bool _M_is_char() const noexcept {
      return _M_type == _Type::_Char;
    }

    // Previous token was a character class, equivalent class,
    // collating symbol etc.
    _GLIBCXX_NODISCARD bool _M_is_class() const noexcept {
  ...
  template <bool __icase, bool __collate>
  bool _M_expression_term(_BracketState &__last_char,
  ...
  static _FlagT _S_validate(_FlagT __f) {
...
private:
  ...
  using _CacheT = std::__conditional_t<_UseCache::value,
                                       std::bitset<_S_cache_size>, _Dummy>;
  using _UnsignedCharT = typename std::make_unsigned<_CharT>::type;

  bool _M_apply(_CharT __ch) const;
