...
struct __rand_uint128 {
  ...
  explicit constexpr __rand_uint128(uint64_t __lo) noexcept : _M_lo(__lo) {}

  __rand_uint128(const __rand_uint128 &) = default;
  __rand_uint128 &operator=(const __rand_uint128 &) = default;

  _GLIBCXX14_CONSTEXPR type &operator=(uint64_t __x) noexcept {
    return *this = type(__x);
  }

  _GLIBCXX14_CONSTEXPR type &operator++() noexcept { return *this = *this + 1; }

  _GLIBCXX14_CONSTEXPR type &operator--() noexcept { return *this = *this - 1; }

  _GLIBCXX14_CONSTEXPR type operator++(int) noexcept {
    auto __tmp = *this;
    ++*this;
    return __tmp;
  }
  ...
  _GLIBCXX14_CONSTEXPR type &operator+=(const type &__r) noexcept {
    _M_hi += __r._M_hi + __builtin_add_overflow(_M_lo, __r._M_lo, &_M_lo);
    return *this;
  }

  friend _GLIBCXX14_CONSTEXPR type operator+(type __l,
                                             const type &__r) noexcept {
    return __l += __r;
  }

  // Addition with 64-bit operand
  friend _GLIBCXX14_CONSTEXPR type operator+(type __l, uint64_t __r) noexcept {
    return __l += type(__r);
  }

  // Subtraction with 64-bit operand
  _GLIBCXX14_CONSTEXPR type &operator-=(uint64_t __r) noexcept {
  ...
  _GLIBCXX14_CONSTEXPR type &operator*=(const type &__x) noexcept {
    uint64_t __a[12] = {uint32_t(_M_lo),
                        _M_lo >> 32,
                        uint32_t(_M_hi),
                        _M_hi >> 32,
                        uint32_t(__x._M_lo),
  ...
  _GLIBCXX14_CONSTEXPR type &operator*=(uint64_t __x) noexcept {
    ...
    uint64_t __mid = (__l0x1 & __mask) + (__l1x0 & __mask) + (__l1x1 >> 32);
  ...
  _GLIBCXX14_CONSTEXPR type &operator/=(const type &__r) noexcept {
    if (!_M_hi) {
      ...
      uint64_t __a[13] = {uint32_t(_M_lo),
                          _M_lo >> 32,
                          uint32_t(_M_hi),
                          _M_hi >> 32,
                          0,
  ...
  _GLIBCXX14_CONSTEXPR type &operator%=(uint64_t __m) noexcept {
    if (_M_hi == 0) {
      _M_lo %= __m;
      return *this;
    }
    ...
    while (_M_hi != 0 || _M_lo >= __m) {
      if (__x <= *this) {
        _M_hi -= __x._M_hi;
        _M_hi -= __builtin_sub_overflow(_M_lo, __x._M_lo, &_M_lo);
      }
  ...
  friend _GLIBCXX14_CONSTEXPR type operator%(type __l, uint64_t __m) noexcept {
    return __l %= __m;
  }

  friend _GLIBCXX14_CONSTEXPR type operator~(type __v) noexcept {
    __v._M_hi = ~__v._M_hi;
    __v._M_lo = ~__v._M_lo;
    return __v;
  }

  _GLIBCXX14_CONSTEXPR type &operator>>=(unsigned __c) noexcept {
    if (__c >= 64) {
      _M_lo = _M_hi >>= (__c - 64);
      _M_hi = 0;
    } else if (__c != 0) {
  ...
  friend _GLIBCXX14_CONSTEXPR type operator>>(type __x, unsigned __c) noexcept {
    return __x >>= __c;
  }
...
#else
  friend constexpr bool operator==(const type &__l, const type &__r) noexcept {
    return __l._M_hi == __r._M_hi && __l._M_lo == __r._M_lo;
  }

  friend _GLIBCXX14_CONSTEXPR bool operator<(const type &__l,
                                             const type &__r) noexcept {
  ...
  constexpr explicit operator _RealT() const noexcept {
    ...
    return _M_hi == 0
               ? _RealT(_M_lo)
               : _RealT(_M_hi) * _RealT(18446744073709551616.0) + _RealT(_M_lo);
  }

  // pre: _M_hi == 0
  constexpr explicit operator uint64_t() const noexcept { return _M_lo; }
...
template <int __s> struct _Select_uint_least_t<__s, 1> {
  __extension__ using type = unsigned __int128;
    ...
    __has_builtin(__builtin_sub_overflow) && defined __UINT64_TYPE__
template <int __s> struct _Select_uint_least_t<__s, 1> {
  using type = __rand_uint128;
};
#endif
...
template <typename _Tp, _Tp __m, _Tp __a, _Tp __c,
  ...
  static _Tp __calc(_Tp __x) {
    using _Tp2 = typename _Select_uint_least_t<std::__lg(__a) + std::__lg(__m) +
                                               2>::type;
...
using _If_seed_seq_for =
    _Require<__not_<is_same<__remove_cvref_t<_Sseq>, _Engine>>,
...
class linear_congruential_engine {
  ...
  template <typename _Sseq>
  using _If_seed_seq =
      __detail::_If_seed_seq_for<_Sseq, linear_congruential_engine, _UIntType>;
...
class mersenne_twister_engine {
  ...
  template <typename _Sseq>
  using _If_seed_seq =
      __detail::_If_seed_seq_for<_Sseq, mersenne_twister_engine, _UIntType>;
...
public:
  ...
  template <typename _Sseq>
  using _If_seed_seq =
      __detail::_If_seed_seq_for<_Sseq, discard_block_engine, result_type>;
...
class independent_bits_engine {
  ...
  template <typename _Sseq>
  using _If_seed_seq =
      __detail::_If_seed_seq_for<_Sseq, independent_bits_engine, _UIntType>;
...
public:
  ...
  template <typename _Sseq>
  using _If_seed_seq =
      __detail::_If_seed_seq_for<_Sseq, shuffle_order_engine, result_type>;
...
inline bool
operator!=(const std::shuffle_order_engine<_RandomNumberEngine, __k> &__lhs,
...
class philox_engine {
  ...
  static constexpr array<_UIntType, __n / 2> _S_popArray() {
    if constexpr (__n == 4)
...
public:
  ...
  explicit philox_engine(result_type __value)
   ...
   *          false otherwise
   */
  friend bool operator==(const philox_engine &,
                         const philox_engine &) = default;
  ...
  operator<<(basic_ostream<_CharT, _Traits> &__os, const philox_engine &__x) {
  ...
  operator>>(basic_istream<_CharT, _Traits> &__is, philox_engine &__x) {
...
private:
  ...
  static _UIntType _S_mulhi(_UIntType __a, _UIntType __b); // (A*B)/2^W

  // The low W bits of the product of __a and __b
  static _UIntType _S_mullo(_UIntType __a, _UIntType __b); // (A*B)%2^W
...
typedef philox_engine<uint_fast32_t, 32, 4, 10, 0xCD9E8D57, 0x9E3779B9,
                      0xD2511F53, 0xBB67AE85>
    philox4x32;

/// 64-bit four-word Philox engine.
typedef philox_engine<uint_fast64_t, 64, 4, 10, 0xCA5A826395121157,
...
public:
  ...
  double entropy() const noexcept { return this->_M_getentropy(); }
...
public:
  ...
  template <typename _Func>
  piecewise_linear_distribution(size_t __nw, _RealType __xmin, _RealType __xmax,
                                _Func __fw)
      : _M_param(__nw, __xmin, __xmax, __fw) {}
