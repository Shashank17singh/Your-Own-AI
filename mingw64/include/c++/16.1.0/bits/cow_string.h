...
private:
  ...
  struct _Rep : _Rep_base {
    // Types:
    typedef
        typename __gnu_cxx::__alloc_traits<_Alloc>::template rebind<char>::other
            _Raw_bytes_alloc;
...
private:
  ...
  static _Rep &_S_empty_rep() _GLIBCXX_NOEXCEPT { return _Rep::_S_empty_rep(); }
...
public:
  ...
  ///  Returns the number of characters in the string, not including any
  ///  null-termination.
  size_type length() const _GLIBCXX_NOEXCEPT { return size(); }
...
#pragma GCC diagnostic pop
   ...
   */
  size_type capacity() const _GLIBCXX_NOEXCEPT { return _M_rep()->_M_capacity; }
  ...
  basic_string &append_range(_Rg &&__rg) {
    basic_string __s(from_range, std::forward<_Rg>(__rg), get_allocator());
    append(__s);
    return *this;
  }
#endif
   ...
   */
  template <__detail::__container_compatible_range<_CharT> _Rg>
  basic_string &replace_with_range(const_iterator __i1, const_iterator __i2,
                                   _Rg &&__rg) {
    if (__i1 == cend())
      append_range(std::forward<_Rg>(__rg));
    else {
...
public:
   ...
   */
  allocator_type get_allocator() const _GLIBCXX_NOEXCEPT { return _M_dataplus; }
  ...
  bool contains(basic_string_view<_CharT, _Traits> __x) const noexcept {
    return __sv_type(this->data(), this->size()).contains(__x);
  }

  bool contains(_CharT __x) const noexcept {
    return __sv_type(this->data(), this->size()).contains(__x);
  }

  [[__gnu__::__nonnull__]]
  bool contains(const _CharT *__x) const noexcept {
    return __sv_type(this->data(), this->size()).contains(__x);
  }
#endif // C++23
...
basic_string<_CharT, _Traits, _Alloc>::basic_string(const basic_string &__str,
    ...
    : _M_dataplus(
          _S_construct(__str._M_data() +
                       ...
                       __str._M_data() + __str._M_limit(__pos, npos) + __pos,
                       __a),
          __a) {}
...
inline void basic_string<_CharT, _Traits, _Alloc>::__resize_and_overwrite(
