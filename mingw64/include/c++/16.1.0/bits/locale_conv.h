...
namespace __detail {
template <typename _Tp> struct _Scoped_ptr {
  __attribute__((__nonnull__(2))) explicit _Scoped_ptr(_Tp *__ptr) noexcept
      : _M_ptr(__ptr) {}

  _Scoped_ptr(_Tp *__ptr, const char *__msg) : _M_ptr(__ptr) {
    if (!__ptr)
  ...
  __attribute__((__returns_nonnull__)) _Tp *operator->() const noexcept {
    return _M_ptr;
  }
...
public:
  ...
  wstring_convert(_Codecvt *__pcvt, state_type __state)
      : _M_cvt(__pcvt, "std::wstring_convert"), _M_state(__state),
...
protected:
  int sync() { return _M_buf && _M_conv_put() && !_M_buf->pubsync() ? 0 : -1; }
