...
inline __allocated_ptr<_Alloc> __allocate_guarded(_Alloc &__a) {
  return {__a, std::allocator_traits<_Alloc>::allocate(__a, 1)};
}

/// RAII type for constructing/destroying an object with an allocated pointer
template <typename _Alloc> struct __allocated_obj : __allocated_ptr<_Alloc> {
...
template <typename _Alloc>
inline __allocated_obj<_Alloc> __allocate_guarded_obj(_Alloc &__a) {
...
//     m_ptr = sa.release();
//
template <typename _Alloc> struct _Scoped_allocation {
  using value_type = typename allocator_traits<_Alloc>::value_type;
  using pointer = typename allocator_traits<_Alloc>::pointer;

  // Use `a` to allocate memory for `n` objects.
  constexpr explicit _Scoped_allocation(const _Alloc &__a, size_t __n = 1)
  ...
  constexpr explicit _Scoped_allocation(const _Alloc &__a, in_place_t,
                                        _Args &&...__args)
  ...
  constexpr _Alloc get_allocator() const noexcept { return _M_a; }

  constexpr value_type *get() const noexcept { return std::__to_address(_M_p); }

  [[__nodiscard__]]
  constexpr pointer release() noexcept {
    return std::__exchange(_M_p, nullptr);
  }
...
_Scoped_allocation(_Alloc, in_place_t, _Args...) -> _Scoped_allocation<_Alloc>;
#endif
