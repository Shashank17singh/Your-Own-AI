 ...
 */
struct __convert_flag : _LoadStoreTag {};
 ...
 * C++26 [simd.expos] / [simd.flags]
 */
struct __aligned_flag : _LoadStoreTag {
  template <typename _Tp, typename _Up>
  [[__gnu__::__always_inline__]]
  static constexpr _Up *_S_adjust_pointer(_Up *__ptr) {
 ...
 * C++26 [simd.expos] / [simd.flags]
 */
template <size_t _Np> struct __overaligned_flag : _LoadStoreTag {
...
struct __partial_loadstore_flag : _LoadStoreTag {};

template <typename _Tp>
concept __loadstore_tag = is_base_of_v<_LoadStoreTag, _Tp>;

template <typename...> struct flags;

template <typename... _Flags>
  requires(__loadstore_tag<_Flags> && ...)
struct flags<_Flags...> {
  /** @internal
   * Returns @c true if the given argument is part of this specialization,
   * otherwise returns @c false.
   */
  template <typename _F0> static consteval bool _S_test(flags<_F0>) {
  ...
  friend consteval auto operator|(flags, flags<_T0, _More...>) {
...
/** @internal
 * Pass to unchecked_load or unchecked_store to make it behave like partial_load
 * / partial_store.
 */
