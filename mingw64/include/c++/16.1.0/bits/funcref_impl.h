...
struct __skip_first_arg<_Ret (_Class::*)(_Args...)
                            _GLIBCXX_MOF_CV noexcept(_Noex)> {
...
public:
        ...
        static_assert(
            !requires {
              typename constant_wrapper<std::__invoke(
                  __fn, remove_cvref_t<_ArgTypes>::value...)>;
  ...
  constexpr function_ref(constant_wrapper<__cwfn, _Fn>,
                         _Td _GLIBCXX_MOF_CV *__ptr) noexcept {
    constexpr const _Fn &__fn = constant_wrapper<__cwfn, _Fn>::value;
    if constexpr (is_pointer_v<_Fn> || is_member_pointer_v<_Fn>)
      static_assert(__fn != nullptr);
    if constexpr (is_member_pointer_v<_Fn>)
...
private:
  template <typename _Tp> constexpr void _M_init(_Tp *__ptr) noexcept {
