...
#if __CHAR_BIT__ != 8
// There are simply too many constants and bit operators that currently depend
// on CHAR_BIT == 8. Generalization to CHAR_BIT != 8 does not make sense without
// testability (i.e. a test target).
#error "<simd> is not supported for CHAR_BIT != 8"
#endif

// psabi warnings are bogus because the ABI of the internal types never leaks
// into user code
...
#define __glibcxx_simd_precondition(expr, msg, ...) __glibcxx_assert(expr)
...
inline constexpr _Tp __iota =
    [] { static_assert(false, "invalid __iota specialization"); }();
...
__filter_abi_variant(_AbiVariant __in, same_as<_AbiVariant> auto... __to_keep) {
  ...
  return static_cast<_AbiVariant>(static_cast<_Up>(__in) &
                                  (static_cast<_Up>(__to_keep) | ...));
}
...
using __integer_from = decltype([] consteval {
 ...
 */
template <size_t _Bytes> using _UInt = make_unsigned_t<__integer_from<_Bytes>>;
 ...
 */
template <typename _Tp> consteval _Tp __div_ceil(_Tp __x, _Tp __y) {
...
using _Bitmask =
    _UInt<__div_ceil(__bit_ceil(unsigned(_NBits)), unsigned(__CHAR_BIT__))>;

/** @internal
 * Map a given type @p _Tp to an equivalent type.
 *
 * This helps with reducing the necessary branches && casts in the
 * implementation as well as reducing the number of template instantiations.
 */
template <typename _Tp> struct __canonical_vec_type {
  using type = _Tp;
};
...
template <> struct __canonical_vec_type<long> {
  using type = int;
};

template <> struct __canonical_vec_type<unsigned long> {
...
template <> struct __canonical_vec_type<char8_t> {
  using type = unsigned char;
};

template <> struct __canonical_vec_type<char16_t> {
  using type = uint_least16_t;
};

template <> struct __canonical_vec_type<char32_t> {
  using type = uint_least32_t;
};

template <> struct __canonical_vec_type<wchar_t> {
...
template <> struct __canonical_vec_type<_Float64> {
  using type = double;
};
#endif

#if defined(__FLT32_DIG__) && defined(_GLIBCXX_FLOAT_IS_IEEE_BINARY32)
template <> struct __canonical_vec_type<_Float32> {
  using type = float;
};
#endif
 ...
 * member and basic_mask objects that store one bool data members.
 *
 * @tparam _Np   The number of elements, which also matches the number of data
 * members in basic_vec and basic_mask.
 */
template <int _Np = 1> struct _ScalarAbi {
  ...
  template <typename _Tp> using _DataType = __canonical_vec_type_t<_Tp>;

  static constexpr bool _S_is_vecmask = false;

  // in principle a bool is a 1-bit bitmask, but this is asking for an AVX512
  // bitmask
  static constexpr bool _S_is_bitmask = false;

  template <size_t> using _MaskDataType = bool;

  template <int _N2, int _Nreg2 = _N2>
  static consteval _ScalarAbi<_N2> _S_resize() {
 ...
 * Applied to basic_mask objects, this ABI tag either describes corresponding
 * vector-mask objects or bit-mask objects. Which one is used is determined via
 * @p _Var.
 *
 * @tparam _Np    The number of elements.
 * @tparam _Nreg  The number of registers needed to store @p _Np elements.
 * @tparam _Var   Determines how complex value-types are layed out and whether
 * mask types use bit-masks or vector-masks.
 */
template <int _Np, int _Nreg, underlying_type_t<_AbiVariant> _Var> struct _Abi {
  static constexpr int _S_size = _Np;

  /** @internal
   * The number of registers needed to represent one basic_vec for the element
   * type that was used on ABI deduction.
   *
   * Examples:
  ...
  static constexpr bool _S_is_bitmask =
      __filter_abi_variant(_S_variant, _AbiVariant::_BitMask) ==
  ...
  static consteval auto _S_resize() {
    if constexpr (_N2 == 1)
...
/** @internal
 * Alias for an _Abi specialization where the _AbiVariant bits are combined into
 * a single integer value.
 *
 * Rationale: Consider diagnostic output and mangling of e.g. vec<int, 4> with
 * AVX512. That's an alias for std::simd::basic_vec<int, std::simd::_Abi<4, 1,
 * 1ull>>. If _AbiVariant were the template argument type of _Abi, the
 * diagnostic output would be 'std::simd::basic_vec<int, std::simd::_Abi<4, 1,
 * (std::simd::_AbiVariant)std::simd::_AbiVariant::_BitMask>>'. That's a lot
 * longer, requires longer mangled names, and bakes the names of the enumerators
 * into the ABI. As soon as bits of multiple _AbiVariants are combined, this
 * becomes hard to parse for humans anyway.
 */
template <int _Np, int _Nreg, _AbiVariant... _Vs>
using _Abi_t =
    _Abi<_Np, _Nreg,
         (static_cast<underlying_type_t<_AbiVariant>>(_Vs) | ... | 0)>;
...
/** @internal
 * Satisfied if @p _Tp is a valid simd ABI tag. This is a necessary but not
 * sufficient condition for an enabled basic_vec/basic_mask specialization.
 */
template <typename _Tp>
concept __abi_tag =
    same_as<decltype(_Tp::_S_variant), const _AbiVariant> &&
    (_Tp::_S_size >= _Tp::_S_nreg) && (_Tp::_S_nreg >= 1) && requires(_Tp __x) {
...
// math_errhandling may expand to an extern symbol, in which case we must assume
// fp exceptions need to be considered. A conforming C library must define
// math_errhandling, but in case it isn't defined we simply use the fallback.
#ifdef math_errhandling
template <int = 0>
  requires requires {
    typename bool_constant<0 != (math_errhandling & MATH_ERREXCEPT)>;
  }
consteval bool __handle_fpexcept_impl(int) {
...
// behavior.
consteval bool __handle_fpexcept_impl(float) { return true; }

/** @internal
 * This type can be used as a template parameter for avoiding ODR violations,
 * where code needs to differ depending on optimization flags (mostly fp-math
 * related).
 */
struct _OptTraits {
  consteval bool _M_test(int __bit) const {
    return ((_M_build_flags >> __bit) & 1) == 1;
  }

  // true iff floating-point operations can signal an exception (allow
  // non-default handler)
  consteval bool _M_fp_may_signal() const { return _M_test(0); }

  // true iff floating-point operations can raise an exception flag
  consteval bool _M_fp_may_raise() const { return _M_test(12); }

  consteval bool _M_fast_math() const { return _M_test(1); }

  consteval bool _M_finite_math_only() const { return _M_test(2); }

  consteval bool _M_no_signed_zeros() const { return _M_test(3); }

  consteval bool _M_signed_zeros() const { return !_M_test(3); }

  consteval bool _M_reciprocal_math() const { return _M_test(4); }

  consteval bool _M_no_math_errno() const { return _M_test(5); }

  consteval bool _M_math_errno() const { return !_M_test(5); }

  consteval bool _M_associative_math() const { return _M_test(6); }

  consteval bool _M_conforming_to_STDC_annex_G() const {
    return _M_test(10) && !_M_finite_math_only();
  }

  consteval bool _M_support_snan() const { return _M_test(11); }

  __UINT64_TYPE__ _M_build_flags = 0
  ...
  // C Annex G defines the behavior of complex<T> where T is IEC60559
  // floating-point. If
  // __STDC_IEC_60559_COMPLEX__ is defined then Annex G is implemented - and
  // simd<complex> will do so as well. However, Clang never defines the macro.
#if defined __STDC_IEC_60559_COMPLEX__ || defined __STDC_IEC_559_COMPLEX__ ||  \
 ...
 */
consteval bool __streq_to_1(const char *__s) {
  return __s != nullptr && __s[0] == '1' && __s[1] == '\0';
}

/** @internal
 * If the macro given as @p feat is defined to 1, expands to a bit set at
 * position @p off. Otherwise, expand to zero.
 */
#define _GLIBCXX_SIMD_ARCH_FLAG(off, feat)                                     \
  (static_cast<__UINT64_TYPE__>(                                               \
       std::simd::__streq_to_1(_GLIBCXX_SIMD_TOSTRING_IMPL(feat)))             \
   << off)

#if _GLIBCXX_X86

#define _GLIBCXX_SIMD_ARCH_TRAITS_INIT                                         \
  {_GLIBCXX_SIMD_ARCH_FLAG(0, __MMX__) | _GLIBCXX_SIMD_ARCH_FLAG(1, __SSE__) | \
...
// Should this include __APX_F__? I don't think it's relevant for use in
// constexpr-if branches => no ODR issue? The same could be said about several
// other flags above that are not checked anywhere.
...
struct _ArchTraits {
  ...
  consteval bool _M_have_mmx() const { return _M_test(0); }

  consteval bool _M_have_sse() const { return _M_test(1); }

  consteval bool _M_have_sse2() const { return _M_test(2); }

  consteval bool _M_have_sse3() const { return _M_test(3); }

  consteval bool _M_have_ssse3() const { return _M_test(4); }

  consteval bool _M_have_sse4_1() const { return _M_test(5); }

  consteval bool _M_have_sse4_2() const { return _M_test(6); }

  consteval bool _M_have_popcnt() const { return _M_test(7); }

  consteval bool _M_have_avx() const { return _M_test(8); }

  consteval bool _M_have_f16c() const { return _M_test(9); }

  consteval bool _M_have_bmi() const { return _M_test(10); }

  consteval bool _M_have_bmi2() const { return _M_test(11); }

  consteval bool _M_have_lzcnt() const { return _M_test(12); }

  consteval bool _M_have_avx2() const { return _M_test(13); }

  consteval bool _M_have_fma() const { return _M_test(14); }

  consteval bool _M_have_avx512f() const { return _M_test(15); }

  consteval bool _M_have_avx512cd() const { return _M_test(16); }

  consteval bool _M_have_avx512dq() const { return _M_test(17); }

  consteval bool _M_have_avx512bw() const { return _M_test(18); }

  consteval bool _M_have_avx512vl() const { return _M_test(19); }

  consteval bool _M_have_avx512bitalg() const { return _M_test(20); }

  consteval bool _M_have_avx512vbmi() const { return _M_test(21); }

  consteval bool _M_have_avx512vbmi2() const { return _M_test(22); }

  consteval bool _M_have_avx512ifma() const { return _M_test(23); }

  consteval bool _M_have_avx512vnni() const { return _M_test(24); }

  consteval bool _M_have_avx512vpopcntdq() const { return _M_test(25); }

  consteval bool _M_have_avx512fp16() const { return _M_test(26); }

  consteval bool _M_have_avx512bf16() const { return _M_test(27); }

  consteval bool _M_have_avxifma() const { return _M_test(28); }

  consteval bool _M_have_avxneconvert() const { return _M_test(29); }

  consteval bool _M_have_avxvnni() const { return _M_test(30); }

  consteval bool _M_have_avxvnniint8() const { return _M_test(31); }

  consteval bool _M_have_avxvnniint16() const { return _M_test(32); }

  consteval bool _M_have_avx10_1() const { return _M_test(33); }

  consteval bool _M_have_avx10_2() const { return _M_test(34); }

  consteval bool _M_have_avx512vp2intersect() const { return _M_test(35); }

  consteval bool _M_have_sse4a() const { return _M_test(36); }

  consteval bool _M_have_fma4() const { return _M_test(37); }

  consteval bool _M_have_xop() const { return _M_test(38); }

  template <typename _Tp> consteval bool _M_eval_as_f32() const {
    return is_same_v<_Tp, _Float16> && !_M_have_avx512fp16();
  }
};

template <typename _Tp, _ArchTraits _Traits = {}>
consteval auto __native_abi() {
...
struct _ArchTraits {
  ...
  constexpr bool _M_test(int __bit) const {
    return ((_M_flags >> __bit) & 1) == 1;
  }
};

template <typename _Tp> consteval auto __native_abi() {
  if constexpr (!__vectorizable<_Tp>)
    return _InvalidAbi();
  else
    return _ScalarAbi<1>();
...
/** @internal
 * You must use this type as template argument to function templates that are
 * not declared always_inline (to avoid issues when linking code compiled with
 * different compiler flags).
 */
struct _TargetTraits : _ArchTraits, _OptTraits {};

/** @internal
 * Alias for an ABI tag such that basic_vec<_Tp, __native_abi_t_<_Tp>> stores
 * one SIMD register of optimal width.
 *
...
/** @internal
 * Alias for an ABI tag @c A such that `basic_vec<_Tp, A>` stores @p _Np
 * elements.
 *
 * C++26 [simd.expos.abi]
 */
...
/** @internal
 * \c rebind implementation detail for basic_vec, and basic_mask where we know
 * the destination value-type
 */
template <typename _Tp, int _Np, __abi_tag _A0, _ArchTraits = {}>
consteval auto __abi_rebind() {
 ...
 *
 * The important difference here is that we have no information about the actual
 * value-type other than its @c sizeof. So `_Bytes == 8` could mean
 * `complex<float>`, @c double, or @c int64_t. E.g. `_Np == 4` with AVX w/o AVX2
 * that's `vector(4) int`, `vector(4) long long`, or `2x vector(2) long long`.
 * That's why this overload has the additional @p _IsOnlyResize parameter, which
 * tells us that the value-type doesn't change.
 */
template <size_t _Bytes, int _Np, __abi_tag _A0, bool _IsOnlyResize,
          _ArchTraits _Traits = {}>
consteval auto __abi_rebind() {
...
#if _GLIBCXX_X86
  // AVX w/o AVX2:
  // e.g. resize_t<8, mask<float, Whatever>> needs to be _Abi<8, 1> not _Abi<8,
  // 2> We determine whether _A0 identifies an AVX vector by looking at the size
  // of a native register. If it's 32, it's a YMM register, otherwise it's 16 or
  // less.
  else if constexpr (_IsOnlyResize && _Traits._M_have_avx() &&
                     !_Traits._M_have_avx2() &&
                     __bit_ceil(
                         __div_ceil<unsigned>(_A0::_S_size, _A0::_S_nreg)) *
                             _Bytes ==
                         32) {
 ...
 *
 * On IvyBridge, (vec<float> == 0.f) == (rebind_t<int, vec<float>> == 0) does
 * not compile. It does compile on basically every other target, though. This is
 * due to the difference in ABI tag: _Abi<8, 1, [...]> vs. _Abi<8, 2, [...]> (8
 * elements, 1 vs. 2 registers). I know how to define this funtion for libstdc++
 * to avoid interconvertible masks. The question is whether we can specify this
 * in general for C++29.
 *
 * Idea: Is rebind_t<integer-from<...>, mask>::abi_type the same type as
 *   deduce-t<integer-from<...>, mask::size()>? If yes, it's the "better" ABI
 * tag. However, this makes the conversion behavior dependent on compiler flags.
 * Probably not what we want.
 */
template <typename _To, typename _From>
consteval bool __is_mask_conversion_explicit([[maybe_unused]] size_t __b0,
                                             [[maybe_unused]] size_t __b1) {
  ...
  // everything is better than _ScalarAbi, except when converting to a single
  // bool
 ...
 *
 * libstdc++ unconditionally uses @c int here, since it matches the return type
 * of 'Bit Operation Builtins' in GCC.
 *
 * C++26 [simd.expos.defn]
 */
...
// [simd.syn]
template <typename _Tp, typename _Ap = __native_abi_t<_Tp>> class basic_vec;
...
concept __static_sized_range = ranges::sized_range<_Tp> && requires(_Tp &&__r) {
  typename integral_constant<size_t, ranges::size(__r)>;
  requires(_Np == -1uz || ranges::size(__r) == _Np);
};

template <typename _Rg> consteval size_t __static_range_size(_Rg &__r) {
...
concept __arithmetic_only_value_preserving_convertible_to =
    convertible_to<_From, _To> && is_arithmetic_v<_From> &&
    is_arithmetic_v<_To> && !(is_signed_v<_From> && is_unsigned_v<_To>) &&
...
/** @internal
 * Satisfied if the conversion from @p _From to @p _To is a value-preserving
 * conversion.
 *
 * C++26 [simd.general]
 */
...
// LWG4420
template <typename _From, typename _To>
concept __explicitly_convertible_to =
...
concept __broadcast_constructible =
    ((convertible_to<_From, _To> && !is_arithmetic_v<remove_cvref_t<_From>> &&
     ...
     || (__constexpr_wrapper_like<remove_cvref_t<_From>>              // 4.3
         && __non_narrowing_constexpr_conversion<
                auto(remove_cvref_t<_From>::value), _To>));

// __higher_floating_point_rank_than<_Tp, U> (_Tp has higher or equal floating
// point rank than U)
template <typename _From, typename _To>
consteval bool __higher_floating_point_rank_than() {
  return floating_point<_From> && floating_point<_To> &&
         is_same_v<common_type_t<_From, _To>, _From> && !is_same_v<_From, _To>;
}

// __higher_integer_rank_than<_Tp, U> (_Tp has higher or equal integer rank than
// U)
template <typename _From, typename _To>
consteval bool __higher_integer_rank_than() {
  return integral<_From> && integral<_To> &&
         (sizeof(_From) > sizeof(_To) ||
          is_same_v<common_type_t<_From, _To>, _From>) &&
         !is_same_v<_From, _To>;
}

template <typename _From, typename _To>
concept __higher_rank_than = __higher_floating_point_rank_than<_From, _To>() ||
                             __higher_integer_rank_than<_From, _To>();
...
concept __simd_generator_invokable = requires {
  __simd_generator_invokable_impl<_Fp, _Tp>(
...
/** @internal
 * The value of the @c _Bytes template argument to a @c basic_mask
 * specialization.
 *
 * C++26 [simd.expos.defn]
 */
template <typename _Tp> constexpr size_t __mask_element_size = 0;
...
concept __simd_mask_type =
    same_as<_Vp,
            basic_mask<__mask_element_size<_Vp>, typename _Vp::abi_type>> &&
    is_default_constructible_v<_Vp>;
...
using __deduced_vec_t =
    decltype(declval<const _Tp &>() + declval<const _Tp &>());

template <typename _Vp, typename _Tp>
using __make_compatible_simd_t = decltype([] {
...
constexpr __simd_size_type __highest_bit(std::unsigned_integral auto __bits) {
...
using __similar_mask =
    basic_mask<sizeof(_Tp), decltype(__abi_rebind<_Tp, _Np, _Ap>())>;
...
using __simd_vec_from_mask_t =
    __similar_vec<__integer_from<_Bytes>, _Ap::_S_size, _Ap>;
...
#endif
consteval _To __value_preserving_cast(const _From &__x) {
 ...
 */
template <typename _T0, typename _T1> struct __trivial_pair {
  _T0 _M_first;
  _T1 _M_second;
};

template <typename _From, typename _To>
concept __converts_trivially =
    convertible_to<_From, _To> && sizeof(_From) == sizeof(_To) &&
    is_integral_v<_From> == is_integral_v<_To> &&
    is_floating_point_v<_From> == is_floating_point_v<_To>;

[[__gnu__::__always_inline__]]
constexpr void __bit_foreach(unsigned_integral auto __bits, auto &&__fun) {
 ...
 *
 * The implementation uses at most two fixed-size power-of-2 @c memcpy calls and
 * reduces the number of branches to a minimum. The variable size is achieved by
 * overlapping two @c memcpy calls.
 *
...
inline void __memcpy_chunks(byte *__restrict__ __dst,
                            const byte *__restrict__ __src, size_t __n) {
  static_assert(_Max <= 64);
  static_assert(__has_single_bit(_Chunk) && _Chunk <= 8);
  size_t __bytes = _Chunk * __n;
  if (__builtin_constant_p(
          __bytes)) { // If __n is known via constant propagation use a single
                      // memcpy call. Since this is still
    // a fixed-size memcpy to the compiler, this leaves more room for
    // optimization.
...
template <typename _Tp, typename _BinaryOperation>
  requires __is_one_of<_BinaryOperation, plus<>, multiplies<>, bit_and<>,
                       bit_or<>, bit_xor<>>::value
