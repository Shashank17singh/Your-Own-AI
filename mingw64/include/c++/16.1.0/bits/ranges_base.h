...
[[__gnu__::__always_inline__]]
constexpr auto __to_unsigned_like(_Tp __t) noexcept {
...
public:
  ...
  constexpr auto operator()(_Tp &&__t) const noexcept(_S_noexcept<_Tp &>()) {
    if constexpr (is_array_v<remove_reference_t<_Tp>>) {
...
struct _SSize {
  ...
  [[nodiscard, __gnu__::__always_inline__]]
  constexpr auto operator()(_Tp &&__t) const noexcept(noexcept(_Size{}(__t))) {
...
public:
  ...
  [[nodiscard, __gnu__::__always_inline__]]
  constexpr bool operator()(_Tp &&__t) const noexcept(_S_noexcept<_Tp &>()) {
    if constexpr (__member_empty<_Tp>)
...
template <range _Range>
using range_common_reference_t = iter_common_reference_t<iterator_t<_Range>>;
...
void __is_derived_from_view_interface_fn(
    const _Tp &,
...
[[__gnu__::__always_inline__]]
constexpr auto &__possibly_const_range(_Range &__r) noexcept {
  // _GLIBCXX_RESOLVE_LIB_DEFECTS
...
struct _CData {
  ...
  template <__maybe_borrowed_range _Tp>
  [[nodiscard]]
  constexpr const auto *operator()(_Tp &&__t) const
...
struct __distance_fn final {
  ...
  [[nodiscard, __gnu__::__always_inline__]]
  constexpr iter_difference_t<decay_t<_It>> operator()(_It &&__first,
                                                       _Sent __last) const {
