...
public:
  ...
  set(from_range_t, _Rg &&__rg, const _Compare &__comp,
  ...
  pair<iterator, bool> insert(_Kt &&__k) {
    auto [__left, __node] = _M_t._M_get_insert_unique_pos_tr(__k);
    if (__node) {
      iterator __i = _M_t._M_emplace_here((__left == __node), __node,
                                          std::forward<_Kt>(__k));
      return {__i, true};
    }
...
#ifdef __glibcxx_associative_heterogeneous_insertion // C++26
  template <__heterogeneous_tree_key<set> _Kt>
  iterator insert(const_iterator __position, _Kt &&__k) {
    auto [__left, __node] =
  ...
  // of the key.
  template <__heterogeneous_tree_key<set> _Kt> size_type erase(_Kt &&__key) {
