...
template <typename _Bi_iter, typename _Alloc> class match_results;
...
template <typename, typename, typename> class _Executor;

template <typename _Tp> struct __is_contiguous_iter : false_type {};

template <typename _Tp> struct __is_contiguous_iter<_Tp *> : true_type {};
...
public:
   ...
   */
  flag_type flags() const noexcept { return _M_flags; }
   ...
   *        object.
   */
  locale_type getloc() const noexcept { return _M_loc; }
...
private:
  ...
  void _M_compile(const _Ch_type *__first, const _Ch_type *__last,
                  flag_type __f) {
  ...
  template <typename, typename, typename> friend class __detail::_Executor;
...
public:
  ...
  match_results(match_results &&__m,
                const _Alloc &__a) noexcept(noexcept(_Base_type(std::move(__m),
                                                                __a)))
  ...
  size_type max_size() const noexcept { return _Unchecked::max_size() - 3; }
  ...
  const_reference operator[](size_type __sub) const {
    ...
    return __sub < size() ? _Unchecked::operator[](__sub) : _M_unmatched_sub();
  }
...
_Out_iter regex_replace(
  ...
  return std::__regex_replace(__out, __first, __last, __e, __fmt,
                              char_traits<_Ch_type>::length(__fmt), __flags);
}
...
public:
  ...
  regex_iterator(
      ...
      : _M_begin(__a), _M_end(__b), _M_pregex(&__re), _M_flags(__m),
        _M_match() {
...
public:
  ...
  regex_token_iterator(const regex_token_iterator &__rhs)
      : _M_position(__rhs._M_position), _M_subs(__rhs._M_subs),
        _M_suffix(__rhs._M_suffix), _M_n(__rhs._M_n),
  ...
  // _GLIBCXX_RESOLVE_LIB_DEFECTS
  // 3719. Directory iterators should be usable with default sentinel
  bool operator==(default_sentinel_t) const noexcept { return _M_end_of_seq(); }
#endif
...
private:
  ...
  constexpr bool _M_end_of_seq() const noexcept { return _M_result == nullptr; }
