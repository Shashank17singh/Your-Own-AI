...
public:
  bool _M_has_alt() const noexcept {
  ...
  std::ostream &_M_print(std::ostream &__ostr) const;
...
template <typename _Char_type> struct _State : _State_base {
  ...
  explicit _State(_Opcode __opcode) noexcept : _State_base(__opcode) {
  ...
  // change it over time.
  _Opcode _M_opcode() const noexcept { return _State_base::_M_opcode; }

  bool _M_matches(_Char_type __char) const { return _M_get_matcher()(__char); }

  _MatcherT &_M_get_matcher() noexcept {
    return *static_cast<_MatcherT *>(this->_M_matcher_storage._M_addr());
  }

  const _MatcherT &_M_get_matcher() const noexcept {
...
struct _NFA_base {
  ...
  explicit _NFA_base(_FlagT __f) noexcept
...
public:
  _FlagT _M_options() const noexcept { return _M_flags; }

  _StateIdT _M_start() const noexcept { return _M_start_state; }

  size_t _M_sub_count() const noexcept { return _M_subexpr_count; }
