...
PB_DS_CLASS_C_DEC::find(key_const_reference r_key) {
...
PB_DS_CLASS_C_DEC::find(key_const_reference r_key) const {
  PB_DS_ASSERT_VALID((*this))
...
PB_DS_CLASS_C_DEC::find_imp(key_const_reference r_key) {
  if (empty())
    return 0;
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::a_const_iterator
PB_DS_CLASS_C_DEC::pref_end(node_const_pointer p_nd) {
  if (p_nd->m_type == leaf_node)
