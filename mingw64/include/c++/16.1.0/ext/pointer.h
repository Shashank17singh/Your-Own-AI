...
public:
  ...
  const _Tp *get() const {
    if (_M_diff == 1)
      ...
      return reinterpret_cast<const _Tp *>(reinterpret_cast<uintptr_t>(this) +
                                           _M_diff);
  }
...
#pragma GCC diagnostic pop
  ...
  operator<=>(const _Pointer_adapter &__lhs,
              const _Pointer_adapter &__rhs) noexcept {
...
struct indirectly_readable_traits<__gnu_cxx::_Pointer_adapter<_Policy>> {
  using value_type = typename __gnu_cxx::_Pointer_adapter<_Policy>::value_type;
};
#endif
_GLIBCXX_END_NAMESPACE_VERSION
} // namespace std _GLIBCXX_VISIBILITY(default)
