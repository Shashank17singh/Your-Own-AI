 ...
 */
template <typename _AddrPair> inline std::size_t __num_blocks(_AddrPair __ap) {
 ...
 *  passed to the function.
 */
template <typename _AddrPair> inline std::size_t __num_bitmaps(_AddrPair __ap) {
  return __num_blocks(__ap) / std::size_t(bits_per_block);
}

// _Tp should be a pointer type.
template <typename _Tp> class _Inclusive_between {
  typedef _Tp pointer;
...
template <typename _Functor> class _Functor_Ref {
  _Functor &_M_fref;
...
// the vector.
template <typename _Tp> class _Ffit_finder {
...
private:
  ...
  void _M_validate(std::size_t *__addr) throw() {
    ...
    iterator __temp = __detail::__lower_bound(
        __free_list.begin(), __free_list.end(), *__addr, _LT_pointer_compare());
...
private:
  template <std::size_t _BSize, std::size_t _AlignSize> struct aligned_size {
    enum {
