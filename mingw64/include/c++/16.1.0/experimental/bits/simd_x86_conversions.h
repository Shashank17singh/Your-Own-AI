...
_GLIBCXX_SIMD_INTRINSIC _To __convert_x86(_V __v) {
  ...
  [[maybe_unused]] constexpr bool __ibw_to_f32 =
      is_integral_v<_Tp> && sizeof(_Tp) <= 2 && is_floating_point_v<_Up> &&
      sizeof(_Up) == 4;
  [[maybe_unused]] constexpr bool __ibw_to_f64 =
      is_integral_v<_Tp> && sizeof(_Tp) <= 2 && is_floating_point_v<_Up> &&
      sizeof(_Up) == 8;
  [[maybe_unused]] constexpr bool __f32_to_ibw =
      is_integral_v<_Up> && sizeof(_Up) <= 2 && is_floating_point_v<_Tp> &&
      sizeof(_Tp) == 4;
  ...
  else if constexpr (__i64_to_i32) //{{{2
  {
    if constexpr (__x_to_x && __have_avx512vl)
      ...
      return __intrin_bitcast<_To>(__lo128(_mm256_permute4x64_epi64(
          _mm256_shuffle_epi32(__intrin, 8), 0 + 4 * 2)));
  ...
  } else if constexpr (__i64_to_i16) //{{{2
  {
    if constexpr (__x_to_x && __have_avx512vl)
      ...
      return __intrin_bitcast<_To>(_mm_shuffle_epi8(
          __intrin,
          _mm_setr_epi8(0, 1, 8, 9, -0x80, -0x80, -0x80, -0x80, -0x80, -0x80,
      ...
      const auto __a = _mm256_shuffle_epi8(
          __intrin,
          _mm256_setr_epi8(0, 1, 8, 9, -0x80, -0x80, -0x80, -0x80, -0x80, -0x80,
  ...
  } else if constexpr (__i32_to_i64) //{{{2
  {
    if constexpr (__have_sse4_1 && __x_to_x)
      ...
      return __intrin_bitcast<_To>(_mm_unpacklo_epi32(
          __intrin,
          is_signed_v<_Tp> ? _mm_srai_epi32(__intrin, 31) : __m128i()));
  ...
  } else if constexpr (__i32_to_i16) //{{{2
  {
    if constexpr (__x_to_x && __have_avx512vl)
      ...
      auto __a = _mm256_shuffle_epi8(
          __intrin, _mm256_setr_epi8(0, 1, 4, 5, 8, 9, 12, 13, -0x80, -0x80,
                                     -0x80, -0x80, -0x80, -0x80, -0x80, -0x80,
  ...
  } else if constexpr (__i16_to_i64) //{{{2
  {
    if constexpr (__x_to_x && __have_sse4_1)
      ...
      return __intrin_bitcast<_To>(_mm_unpacklo_epi32(
          _mm_unpacklo_epi16(__intrin, __m128i()), __m128i()));
    else if constexpr (__x_to_y)
      return __intrin_bitcast<_To>(is_signed_v<_Tp>
  ...
  } else if constexpr (__i16_to_i8) //{{{2
  {
    if constexpr (__x_to_x && __have_avx512bw_vl)
      ...
      auto __a = _mm256_shuffle_epi8(
          __intrin, _mm256_setr_epi8(0, 2, 4, 6, 8, 10, 12, 14, -0x80, -0x80,
                                     -0x80, -0x80, -0x80, -0x80, -0x80, -0x80,
                                     -0x80, -0x80, -0x80, -0x80, -0x80, -0x80,
  ...
  } else if constexpr (__i8_to_i64) //{{{2
  {
    if constexpr (__x_to_x && __have_sse4_1)
      ...
      if constexpr (__have_ssse3) {
        ...
        _mm_shuffle_epi8(__epi16, _mm_setr_epi8(0, 1, 1, 1, 1, 1, 1, 1, 2, 3, 3,
                                                3, 3, 3, 3, 3));
      } else {
        auto __x = _mm_unpacklo_epi8(__intrin, __intrin);
        __x = _mm_unpacklo_epi16(__x, __x);
        return __intrin_bitcast<_To>(_mm_unpacklo_epi32(
            _mm_srai_epi32(__x, 24), _mm_srai_epi32(__x, 31)));
      }
    } else if constexpr (__x_to_x) {
      return __intrin_bitcast<_To>(_mm_unpacklo_epi32(
  ...
  } else if constexpr (__i8_to_i32) //{{{2
  {
    if constexpr (__x_to_x && __have_sse4_1)
      ...
      return __intrin_bitcast<_To>(_mm_unpacklo_epi16(
          _mm_unpacklo_epi8(__intrin, __m128i()), __m128i()));
    else if constexpr (__x_to_y)
      return __intrin_bitcast<_To>(is_signed_v<_Tp>
  ...
  } else if constexpr (__ibw_to_f32) //{{{2
  {
    if constexpr (_M <= 4 || __have_avx2)
      ...
      if constexpr (__have_sse4_1) {
        ...
        __b = sizeof(_Tp) == 2 ? (is_signed_v<_Tp> ? _mm_cvtepi16_epi32(__w)
...
_GLIBCXX_SIMD_INTRINSIC _To __convert_x86(_V __v0, _V __v1) {
  ...
  [[maybe_unused]] constexpr bool __i64_to_f32 =
      is_integral_v<_Tp> && sizeof(_Tp) == 8 && is_floating_point_v<_Up> &&
      sizeof(_Up) == 4;
  [[maybe_unused]] constexpr bool __s32_to_f32 =
      is_integral_v<_Tp> && is_signed_v<_Tp> && sizeof(_Tp) == 4 &&
      is_floating_point_v<_Up> && sizeof(_Up) == 4;
  ...
  } else //{{{2
  {
    ...
    static_assert(!(is_floating_point_v<_Tp> == is_floating_point_v<_Up> &&
                    sizeof(_Tp) == sizeof(_Up)));
    // handle all zero extension{{{2
    if constexpr (2 * _Np < _M && sizeof(_To) > 16) {
      constexpr size_t Min = 16 / sizeof(_Up);
      return __zero_extend(
          __convert_x86<__vector_type_t<_Up, (Min > 2 * _Np) ? Min : 2 * _Np>>(
              __v0, __v1));
    } else if constexpr (__i64_to_i32) //{{{2
    {
      if constexpr (__x_to_x)
        ...
        return __auto_bitcast(__xzyw(_mm256_shuffle_ps(
            __auto_bitcast(__v0), __auto_bitcast(__v1), 0x88)));
        // alternative:
    ...
    } else if constexpr (__i64_to_i8) //{{{2
    {
      if constexpr (__x_to_x && __have_sse4_1) {
        ...
        return __intrin_bitcast<_To>(_mm_unpacklo_epi16(
            _mm_shuffle_epi8(__i0,
                             _mm_setr_epi8(0, 8, -0x80, -0x80, -0x80, -0x80,
                                           -0x80, -0x80, -0x80, -0x80, -0x80,
                                           -0x80, -0x80, -0x80, -0x80, -0x80)),
      ...
      if constexpr (__x_to_x && __have_ssse3) {
        ...
        const auto __a = _mm256_shuffle_epi8(
            ...
            _mm256_setr_epi8(0, 4, 8, 12, -0x80, -0x80, -0x80, -0x80, 2, 6, 10,
                             14, -0x80, -0x80, -0x80, -0x80, -0x80, -0x80,
                             -0x80, -0x80, 0, 4, 8, 12, -0x80, -0x80, -0x80,
                             -0x80, 2, 6, 10, 14));
        return __intrin_bitcast<_To>(__lo128(__a) | __hi128(__a));
    ...
    } else if constexpr (__i16_to_i8) //{{{2
    {
      if constexpr (__x_to_x && __have_ssse3) {
        const auto __shuf = reinterpret_cast<__m128i>(
            __vector_type_t<_UChar, 16>{0, 2, 4, 6, 8, 10, 12, 14, 0x80, 0x80,
                                        0x80, 0x80, 0x80, 0x80, 0x80, 0x80});
        return __intrin_bitcast<_To>(_mm_unpacklo_epi64(
            _mm_shuffle_epi8(__i0, __shuf), _mm_shuffle_epi8(__i1, __shuf)));
      } else if constexpr (__x_to_x) {
    ...
    if constexpr (sizeof(_To) >= 32)
      ...
      if constexpr (sizeof(_Up) * _Np == 8) {
        if constexpr (is_floating_point_v<_Up>)
          return __auto_bitcast(_mm_unpacklo_pd(
              __vector_bitcast<double>(__lo), __vector_bitcast<double>(__hi)));
        else
...
_GLIBCXX_SIMD_INTRINSIC _To __convert_x86(_V __v0, _V __v1, _V __v2, _V __v3) {
  ...
  } else //{{{2
  {
    ...
    if constexpr (4 * _Np < _M && sizeof(_To) > 16) {
      constexpr size_t Min = 16 / sizeof(_Up);
      return __zero_extend(
          __convert_x86<__vector_type_t<_Up, (Min > 4 * _Np) ? Min : 4 * _Np>>(
              __v0, __v1, __v2, __v3));
    } else if constexpr (__i64_to_i16) //{{{2
    {
      if constexpr (__x_to_x && __have_sse4_1) {
        ...
        return __intrin_bitcast<_To>(_mm256_shuffle_epi8(
            ...
            _mm256_setr_epi8(0, 1, 4, 5, 8, 9, 12, 13, 2, 3, 6, 7, 10, 11, 14,
                             15, 0, 1, 4, 5, 8, 9, 12, 13, 2, 3, 6, 7, 10, 11,
                             14, 15)));
        /*
        ...
        const auto __a = _mm256_shuffle_epi8(
            __to_intrin((__vector_bitcast<_UShort>(_mm256_blend_epi16(
                        ...
                        (__vector_bitcast<_UShort>(_mm256_blend_epi16(
                         ...
                         << 8)),
            _mm256_setr_epi8(0, 4, 8, 12, 2, 6, 10, 14, 1, 5, 9, 13, 3, 7, 11,
                             15, 0, 4, 8, 12, 2, 6, 10, 14, 1, 5, 9, 13, 3, 7,
                             11, 15));
...
template <typename _To, typename _V, typename _Traits>
_GLIBCXX_SIMD_INTRINSIC _To __convert_x86(_V __v0, _V __v1, _V __v2, _V __v3,
                                          _V __v4, _V __v5, _V __v6, _V __v7) {
  static_assert(__is_vector_type_v<_V>);
  using _Tp = typename _Traits::value_type;
  constexpr size_t _Np = _Traits::_S_full_size;
  [[maybe_unused]] const auto __i0 = __to_intrin(__v0);
  ...
  [[maybe_unused]] constexpr bool __f64_to_i8 =
      is_integral_v<_Up> && sizeof(_Up) == 1 && is_floating_point_v<_Tp> &&
      sizeof(_Tp) == 8;
    ...
    if constexpr (__i64_to_i8) //{{{2
    {
      if constexpr (__x_to_x && __have_ssse3) {
        ...
        auto __b = _mm256_shuffle_epi8( // 0145 89CD GHKL OPST 2367 ABEF
                                        // IJMN QRUV
            __a, _mm256_setr_epi8(0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14,
                                  7, 15, 0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13,
                                  6, 14, 7, 15));
        auto __c = __xzyw(__b); // 0145 89CD 2367 ABEF GHKL OPST IJMN QRUV
        return __intrin_bitcast<_To>(_mm256_shuffle_epi8(
            __c, _mm256_setr_epi8(0, 1, 8, 9, 2, 3, 10, 11, 4, 5, 12, 13, 6, 7,
                                  14, 15, 0, 1, 8, 9, 2, 3, 10, 11, 4, 5, 12,
                                  13, 6, 7, 14, 15)));
      } else if constexpr (__z_to_z) {
        return __concat(
...
_GLIBCXX_SIMD_INTRINSIC _To __convert_x86(_V __v0, _V __v1, _V __v2, _V __v3,
                                          _V __v4, _V __v5, _V __v6, _V __v7,
                                          _V __v8, _V __v9, _V __v10, _V __v11,
                                          _V __v12, _V __v13, _V __v14,
                                          _V __v15) {
