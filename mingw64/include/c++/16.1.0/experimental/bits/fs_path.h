...
using __is_encoded_char = __or_<is_same<_Ch, char>, is_same<_Ch, wchar_t>,
...
template <typename _Source> inline _Source _S_range_begin(_Source __begin) {
  return __begin;
}

struct __nul_terminated {};

template <typename _Source> inline __nul_terminated _S_range_end(_Source) {
  return {};
}
...
private:
  enum class _Type : unsigned char { _Multi, _Root_name, _Root_dir, _Filename };
  ...
  static string_type _S_convert(value_type *__src, __detail::__nul_terminated) {
    return string_type(__src);
  }

  static string_type _S_convert(const value_type *__src,
                                __detail::__nul_terminated) {
    return string_type(__src);
  }
  ...
  static string_type _S_convert_loc(char *__first, char *__last,
                                    const std::locale &__loc) {
...
template <> struct path::_Cvt<path::value_type> {
  ...
  friend bool __str_codecvt_in_all(const char *, const char *, _WStr &,
                                   __codecvt_utf8_to_wide &) noexcept {
    return true;
  }
...
template <typename _CharT> struct path::_Cvt {
  ...
  using __codecvt_utf8_to_wide =
      __conditional_t<is_same<_CharT, wchar_t>::value, __codecvt_utf8_to_wchar,
...
inline path::path() noexcept = default;

inline path::path(const path &) = default;

inline path::path(path &&__p) noexcept
    : _M_pathname(std::move(__p._M_pathname)), _M_cmpts(__p._M_cmpts),
      _M_type(__p._M_type) {
  __p.clear();
}

inline path::path(string_type &&__source) : _M_pathname(std::move(__source)) {
  _M_split_cmpts();
}

inline path::path(string_type __str, _Type __type)
...
inline path &path::operator=(const path &__p) = default;
