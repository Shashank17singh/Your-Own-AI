...
struct _SimdImplScalar {
  ...
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_load(const _Up *__mem,
  ...
  // _S_negate {{{2
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_negate(_Tp __x) noexcept {
    return !__x;
  }
  ...
  // _S_min, _S_max {{{2
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_min(const _Tp __a,
                                                      const _Tp __b) {
    return std::min(__a, __b);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_max(const _Tp __a,
                                                      const _Tp __b) {
  ...
  // _S_complement {{{2
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_complement(_Tp __x) noexcept {
  ...
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_plus(_Tp __x, _Tp __y) {
    return static_cast<_Tp>(__promote_preserving_unsigned(__x) +
                            __promote_preserving_unsigned(__y));
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_minus(_Tp __x, _Tp __y) {
    return static_cast<_Tp>(__promote_preserving_unsigned(__x) -
                            __promote_preserving_unsigned(__y));
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_multiplies(_Tp __x, _Tp __y) {
    return static_cast<_Tp>(__promote_preserving_unsigned(__x) *
                            __promote_preserving_unsigned(__y));
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_divides(_Tp __x, _Tp __y) {
    return static_cast<_Tp>(__promote_preserving_unsigned(__x) /
                            __promote_preserving_unsigned(__y));
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_modulus(_Tp __x, _Tp __y) {
    return static_cast<_Tp>(__promote_preserving_unsigned(__x) %
                            __promote_preserving_unsigned(__y));
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_bit_and(_Tp __x, _Tp __y) {
  ...
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_bit_or(_Tp __x, _Tp __y) {
    if constexpr (is_floating_point_v<_Tp>) {
      using _Ip = __int_for_sizeof_t<_Tp>;
      return __bit_cast<_Tp>(__bit_cast<_Ip>(__x) | __bit_cast<_Ip>(__y));
    } else
  ...
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_bit_xor(_Tp __x, _Tp __y) {
    if constexpr (is_floating_point_v<_Tp>) {
      using _Ip = __int_for_sizeof_t<_Tp>;
      return __bit_cast<_Tp>(__bit_cast<_Ip>(__x) ^ __bit_cast<_Ip>(__y));
    } else
  ...
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_bit_shift_left(_Tp __x,
                                                                 int __y) {
    return static_cast<_Tp>(__promote_preserving_unsigned(__x) << __y);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_bit_shift_right(_Tp __x,
                                                                  int __y) {
    return static_cast<_Tp>(__promote_preserving_unsigned(__x) >> __y);
  }
  ...
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static _Tp _S_remquo(_Tp __x, _Tp __y,
                                               _ST<int> *__z) {
    return std::remquo(__x, __y, &__z->first);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr _ST<int> _S_fpclassify(_Tp __x) {
    return {std::fpclassify(__x)};
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_isfinite(_Tp __x) {
    return std::isfinite(__x);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_isinf(_Tp __x) {
    return std::isinf(__x);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_isnan(_Tp __x) {
    return std::isnan(__x);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_isnormal(_Tp __x) {
    return std::isnormal(__x);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_signbit(_Tp __x) {
    return std::signbit(__x);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_isgreater(_Tp __x, _Tp __y) {
    return std::isgreater(__x, __y);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_isgreaterequal(_Tp __x,
                                                                  _Tp __y) {
    return std::isgreaterequal(__x, __y);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_isless(_Tp __x, _Tp __y) {
    return std::isless(__x, __y);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_islessequal(_Tp __x,
                                                               _Tp __y) {
    return std::islessequal(__x, __y);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_islessgreater(_Tp __x,
                                                                 _Tp __y) {
    return std::islessgreater(__x, __y);
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_isunordered(_Tp __x,
                                                               _Tp __y) {
  ...
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr void _S_increment(_Tp &__x) {
    ++__x;
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr void _S_decrement(_Tp &__x) {
    --__x;
  }

  // compares {{{2
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_equal_to(_Tp __x, _Tp __y) {
    return __x == __y;
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_not_equal_to(_Tp __x,
                                                                _Tp __y) {
    return __x != __y;
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_less(_Tp __x, _Tp __y) {
    return __x < __y;
  }

  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_less_equal(_Tp __x,
                                                              _Tp __y) {
    return __x <= __y;
  }
  ...
  _GLIBCXX_SIMD_INTRINSIC static constexpr _Tp _S_masked_unary(const bool __k,
                                                               const _Tp __v) {
...
struct _MaskImplScalar {
  ...
  // _S_store {{{2
  _GLIBCXX_SIMD_INTRINSIC static constexpr void _S_store(bool __v,
                                                         bool *__mem) noexcept {
    __mem[0] = __v;
  }
  ...
  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_logical_and(bool __x,
                                                               bool __y) {
    return __x && __y;
  }

  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_logical_or(bool __x,
                                                              bool __y) {
    return __x || __y;
  }

  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_bit_not(bool __x) {
    return !__x;
  }

  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_bit_and(bool __x, bool __y) {
    return __x && __y;
  }

  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_bit_or(bool __x, bool __y) {
    return __x || __y;
  }

  _GLIBCXX_SIMD_INTRINSIC static constexpr bool _S_bit_xor(bool __x, bool __y) {
    return __x != __y;
  }
