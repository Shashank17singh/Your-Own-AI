...
// narrowing conversion of '__a' from 'uint64_t' {aka 'long long unsigned int'}
// to
...
// }}}
template <size_t _Xp> using _SizeConstant = integral_constant<size_t, _Xp>;
...
constexpr inline bool __have_avx512vpopcntdq =
    _GLIBCXX_SIMD_HAVE_AVX512VPOPCNTDQ;
constexpr inline bool __have_avx512vp2intersect =
    _GLIBCXX_SIMD_HAVE_AVX512VP2INTERSECT;
...
// Determines _S_handle_fpexcept from math_errhandling if it is defined and
// expands to a constant expression. math_errhandling may expand to an extern
// symbol, in which case a constexpr value must be guessed.
template <int = math_errhandling> constexpr bool __handle_fpexcept_impl(int) {
  return math_errhandling & MATH_ERREXCEPT;
}
#endif

// Fallback if math_errhandling doesn't work: with fast-math assume
// floating-point exceptions are ignored, otherwise implement correct exception
// behavior.
...
/// True if math functions must raise floating-point exceptions as specified by
/// C17.
static constexpr bool _S_handle_fpexcept = __handle_fpexcept_impl(0);

constexpr std::uint_least64_t __floating_point_flags() {
...
constexpr std::uint_least64_t __machine_flags() {
  if constexpr (__have_mmx || __have_sse)
    return __have_mmx | (__have_sse << 1) | (__have_sse2 << 2) |
           (__have_sse3 << 3) | (__have_ssse3 << 4) | (__have_sse4_1 << 5) |
           (__have_sse4_2 << 6) | (__have_xop << 7) | (__have_avx << 8) |
           (__have_avx2 << 9) | (__have_bmi << 10) | (__have_bmi2 << 11) |
           (__have_lzcnt << 12) | (__have_sse4a << 13) | (__have_fma << 14) |
           (__have_fma4 << 15) | (__have_f16c << 16) | (__have_popcnt << 17) |
           (__have_avx512f << 18) | (__have_avx512dq << 19) |
           (__have_avx512vl << 20) | (__have_avx512bw << 21) |
           (__have_avx512bitalg << 22) | (__have_avx512vbmi2 << 23) |
           (__have_avx512vbmi << 24) | (__have_avx512ifma << 25) |
           (__have_avx512cd << 26) | (__have_avx512vnni << 27) |
           (__have_avx512vpopcntdq << 28) | (__have_avx512vp2intersect << 29);
  else if constexpr (__have_neon || __have_sve)
    return __have_neon | (__have_neon_a32 << 1) | (__have_neon_a64 << 2) |
           (__have_neon_a64 << 2) | (__support_neon_float << 3) |
           (__have_sve << 4) | (__have_sve2 << 5);
  else if constexpr (__have_power_vmx)
    return __have_power_vmx | (__have_power_vsx << 1) |
           (__have_power8vec << 2) | (__have_power9vec << 3) |
           (__have_power10vec << 4);
  else
    return 0;
}
...
template <std::uint_least64_t...> struct _MachineFlagsTemplate {};
 ...
 */
using __odr_helper = conditional_t<
    __machine_flags() == 0, _OdrEnforcer,
    _MachineFlagsTemplate<__machine_flags(), __floating_point_flags()>>;

struct _Minimum {
  template <typename _Tp>
  _GLIBCXX_SIMD_INTRINSIC constexpr _Tp operator()(_Tp __a, _Tp __b) const {
    using std::min;
    return min(__a, __b);
  }
};
...
[[__gnu__::__flatten__]] _GLIBCXX_SIMD_INTRINSIC constexpr void
...
[[__gnu__::__flatten__]] _GLIBCXX_SIMD_INTRINSIC constexpr _R
...
[[__gnu__::__flatten__]] _GLIBCXX_SIMD_INTRINSIC constexpr auto
...
// }}}
// __is_vectorizable {{{
template <typename _Tp> struct __is_vectorizable : public is_arithmetic<_Tp> {};
...
template <typename _Tp>
struct __is_bitmask<
    _Tp, void_t<decltype(declval<unsigned &>() = declval<_Tp>() & 1u)>>
    : true_type {};
...
template <size_t _Bytes> constexpr auto __int_for_sizeof() {
  ...
  if constexpr (_Bytes == sizeof(int))
    ...
    struct _Ip {
      ...
      _GLIBCXX_SIMD_INTRINSIC constexpr _Ip operator&(_Ip __rhs) const {
      ...
      _GLIBCXX_SIMD_INTRINSIC constexpr _Ip operator~() const {
        return __generate_from_n_evaluations<_Np, _Ip>(
            [&](auto __i)
                _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA { return ~_M_data[__i]; });
      }
    };
    return _Ip{};
  } else
...
template <typename _Tp> constexpr int __abi_bytes_impl(_Tp *) { return -1; }
...
// }}}
// __is_sve_abi {{{
template <typename _Abi> constexpr bool __is_sve_abi() {
  constexpr auto _Bytes = __abi_bytes_v<_Abi>;
...
// }}}
// __div_roundup {{{
inline constexpr size_t __div_roundup(size_t __a, size_t __b) {
...
struct __is_narrowing_conversion<_From, _To, true, true>
    : public __bool_constant<(__digits_v<_From> > __digits_v<_To> ||
                              __finite_max_v<_From> > __finite_max_v<_To> ||
                              __finite_min_v<_From> < __finite_min_v<_To> ||
                              (is_signed_v<_From> && is_unsigned_v<_To>))> {};

template <typename _Tp>
struct __is_narrowing_conversion<_Tp, bool, true, true> : public true_type {};
...
template <typename _Tp, size_t _Size, typename = void_t<>> struct _SimdWrapper;
...
template <typename _Tp, size_t _Width> struct _SveSimdWrapper;
...
template <
    ...
    typename = enable_if_t<conjunction<
        ...
        disjunction<is_same<_DecayedFrom, _To>, is_same<_DecayedFrom, int>,
                    ...
                    negation<__is_narrowing_conversion<_DecayedFrom, _To>>>>::
                               value>>
...
template <typename _Tp> using __is_intrinsic_type = __is_vector_type<_Tp>;
#else // not SSE (x86)
template <typename _Tp, typename = void_t<>>
...
struct _VectorTraitsImpl<
    _Tp, enable_if_t<__is_vector_type_v<_Tp> || __is_intrinsic_type_v<_Tp>>> {
  using type = _Tp;
  ...
  if constexpr (__is_vector_type_v<_V>)
    ...
    if constexpr (__is_fixed_size_abi_v<typename _V::abi_type>) {
      ...
      static_assert(_V::abi_type::template __traits<
                        typename _V::value_type>::_SimdMember::_S_tuple_size ==
                    1);
...
#else
  static_assert(sizeof(_To) == sizeof(_From));
  constexpr bool __to_is_vectorizable = is_arithmetic_v<_To> || is_enum_v<_To>;
...
static constexpr struct {
  ...
  _GLIBCXX_SIMD_INTRINSIC __v4di operator()(__v4di __a,
    ...
    if constexpr (__have_avx2)
      ...
      return reinterpret_cast<__v4di>(__builtin_ia32_andnpd256(
          reinterpret_cast<__v4df>(__a), reinterpret_cast<__v4df>(__b)));
  }
...
_GLIBCXX_SIMD_INTRINSIC constexpr __vector_type_t<
    remove_reference_t<decltype(declval<_T0>()[0])>, sizeof...(_Is)>
...
_GLIBCXX_SIMD_INTRINSIC constexpr auto __vec_shuffle(_T0 __x, _Seq __seq,
                                                     _Fun __idx_perm) {
...
struct _ZeroExtendProxy {
  ...
  template <
      typename _To, typename _ToVT = _VectorTraits<_To>,
  ...
  _GLIBCXX_SIMD_INTRINSIC operator _To() const {
    ...
    if constexpr (_ToN == _Np)
      ...
      } else if constexpr (__have_avx512f && is_floating_point_v<value_type>) {
        return __vector_bitcast<value_type>(
...
// }}}
// __extract<_Np, By>{{{
template <int _Offset, int _SplitBy, typename _Tp,
...
#ifdef _GLIBCXX_SIMD_WORKAROUND_XXX_1
    using _W =
        conditional_t<is_floating_point_v<value_type>, double,
...
// }}}
// __lo/__hi64[z]{{{
template <typename _Tp, typename _R = __vector_type8_t<
                            typename _VectorTraits<_Tp>::value_type>>
...
#define _GLIBCXX_SIMD_ARM_INTRIN(_Bits, _Np)                                   \
  template <>                                                                  \
  struct __intrinsic_type<__int_with_sizeof_t<_Bits / 8>, _Np * _Bits / 8,     \
                          void> {                                              \
...
struct _SimdWrapper<bool, _Width,
  ...
  _GLIBCXX_SIMD_INTRINSIC
  operator const _BuiltinType &() const { return _M_data; }

  _GLIBCXX_SIMD_INTRINSIC
  operator _BuiltinType &() { return _M_data; }
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr bool _M_is_constprop() const {
...
struct _SimdWrapper<
  ...
  static constexpr bool _S_need_default_init =
      __has_iec559_behavior<__signaling_NaN, _Tp>::value and
  ...
  // Convert from __intrinsic_type_t if __intrinsic_type_t and __vector_type_t
  // differ, otherwise this ctor should not exist. Making the argument type
  // unusable is our next best solution.
  _GLIBCXX_SIMD_INTRINSIC constexpr _SimdWrapper(
      conditional_t<is_same_v<_BuiltinType, __intrinsic_type_t<_Tp, _Width>>,
                    _DisabledSimdWrapper, __intrinsic_type_t<_Tp, _Width>>
          __x)
      : _Base(__vector_bitcast<_Tp, _Width>(__x)) {}

  // Convert from different __vector_type_t, but only if bit reinterpretation is
  // a correct conversion of the value_type
  template <typename _V, typename _TVT = _VectorTraits<_V>,
            typename = enable_if_t<
                sizeof(typename _TVT::value_type) == sizeof(_Tp) and
                sizeof(_V) == sizeof(_BuiltinType) and is_integral_v<_Tp> and
                is_integral_v<typename _TVT::value_type>>>
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr void _M_set(size_t __i, _Tp __x) {
    if (__builtin_is_constant_evaluated())
      _M_data = __generate_from_n_evaluations<_Width, _BuiltinType>(
          [&](auto __j) { return __j == __i ? __x : _M_data[__j()]; });
    else
      _M_data[__i] = __x;
  }
...
template <typename _Tp> constexpr size_t __vectorized_sizeof() {
  ...
  if constexpr (sizeof(_Tp) <= 8) {
    ...
    if constexpr (__have_neon_a64)
      return 16;
    if constexpr (__have_neon_a32 and
                  (not is_floating_point_v<_Tp> or is_same_v<_Tp, float>))
      return 16;
...
inline constexpr int max_fixed_size =
    ((__have_avx512bw && sizeof(_Tp) == 1) ||
     (__have_sve && __sve_vectorized_size_bytes / sizeof(_Tp) >= 64))
        ? 64
        : 32;
...
// }}}
// native {{{
template <typename _Tp> constexpr auto __determine_native_abi() {
...
// traits {{{1
template <typename _Tp> struct is_simd_flag_type : false_type {};

template <> struct is_simd_flag_type<element_aligned_tag> : true_type {};

template <> struct is_simd_flag_type<vector_aligned_tag> : true_type {};
...
struct rebind_simd<
    ...
    void_t<std::conditional_t<
        ...
        simd_abi::__no_sve_deduce_t<_Tp, simd_size_v<_Up, _Abi>, _Abi>,
        simd_abi::deduce_t<_Tp, simd_size_v<_Up, _Abi>, _Abi>>>> {
  using type = simd_mask<
      _Tp, std::conditional_t<
               !__is_sve_abi<_Abi>(),
               simd_abi::__no_sve_deduce_t<_Tp, simd_size_v<_Up, _Abi>, _Abi>,
               simd_abi::deduce_t<_Tp, simd_size_v<_Up, _Abi>, _Abi>>>;
};
...
template <
    typename _Tp, typename _Up, typename _Ap,
...
_GLIBCXX_SIMD_INTRINSIC _GLIBCXX_SIMD_CONSTEXPR _To
simd_bit_cast(const simd<_Up, _Abi> &__x) {
  ...
  static_assert(is_trivially_copyable_v<_ToMember> &&
                is_trivially_copyable_v<_FromMember>);
#if __has_builtin(__builtin_bit_cast)
...
simd_bit_cast(const simd_mask<_Up, _Abi> &__x) {
  ...
  // _To can be simd<T, A>, specifically simd<T, fixed_size<N>> in which case
  // _To is not trivially copyable.
...
to_fixed_size(const simd<_Tp, _Ap> &__x) {
  ...
  return _Rp([&__x](auto __i) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
    return __x[__i];
  });
}
...
public:
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr const_where_expression(const _M &__kk,
                                                           const _Tp &dd)
      : _M_k(__kk), _M_value(const_cast<_Tp &>(dd)) {}

  _GLIBCXX_SIMD_INTRINSIC _GLIBCXX_SIMD_CONSTEXPR _V operator-() const && {
    return {__private_init, _Impl::template _S_masked_unary<negate>(
  ...
  copy_from(const _LoadStorePtr<_Up, value_type> *__mem,
            _IsSimdFlagType<_Flags>) const && {
    return {__private_init,
  ...
  copy_to(_LoadStorePtr<_Up, value_type> *__mem,
          _IsSimdFlagType<_Flags>) const && {
...
public:
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr const_where_expression(const bool __kk,
                                                           const _Tp &dd)
      : _M_k(__kk), _M_value(const_cast<_Tp &>(dd)) {}
...
class where_expression : public const_where_expression<_M, _Tp> {
  ...
  static_assert(is_same<typename _M::abi_type, typename _Tp::abi_type>::value,
                "");
...
public:
  ...
  template <typename _Up>
  _GLIBCXX_SIMD_INTRINSIC _GLIBCXX_SIMD_CONSTEXPR void operator=(_Up &&__x) && {
...
#define _GLIBCXX_SIMD_OP_(__op, __name)                                        \
  template <typename _Up>                                                      \
  _GLIBCXX_SIMD_INTRINSIC _GLIBCXX_SIMD_CONSTEXPR void operator __op## =       \
      (_Up && __x) && {                                                        \
    _Impl::template _S_masked_cassign(                                         \
        __data(_M_k), __data(_M_value),                                        \
        __to_value_type_or_member_type<_Tp>(static_cast<_Up &&>(__x)),         \
        [](auto __impl, auto __lhs,                                            \
           auto __rhs) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {          \
...
#undef _GLIBCXX_SIMD_OP_
  ...
  _GLIBCXX_SIMD_INTRINSIC _GLIBCXX_SIMD_CONSTEXPR void operator++() && {
    __data(_M_value) = _Impl::template _S_masked_unary<__increment>(
        __data(_M_k), __data(_M_value));
  }

  _GLIBCXX_SIMD_INTRINSIC _GLIBCXX_SIMD_CONSTEXPR void operator++(int) && {
    __data(_M_value) = _Impl::template _S_masked_unary<__increment>(
        __data(_M_k), __data(_M_value));
  }

  _GLIBCXX_SIMD_INTRINSIC _GLIBCXX_SIMD_CONSTEXPR void operator--() && {
    __data(_M_value) = _Impl::template _S_masked_unary<__decrement>(
        __data(_M_k), __data(_M_value));
  }
  ...
  _GLIBCXX_SIMD_INTRINSIC _GLIBCXX_SIMD_CONSTEXPR void
  copy_from(const _LoadStorePtr<_Up, value_type> *__mem,
            _IsSimdFlagType<_Flags>) && {
    __data(_M_value) = _Impl::_S_masked_load(
        __data(_M_value), __data(_M_k), _Flags::template _S_apply<_Tp>(__mem));
  }
};
...
template <typename _Tp>
_GLIBCXX_SIMD_INTRINSIC
    _GLIBCXX_SIMD_CONSTEXPR const_where_expression<bool, _Tp>
...
template <typename _Tp, typename _Ap>
_GLIBCXX_SIMD_CONSTEXPR void where(bool __k,
                                   const simd<_Tp, _Ap> &__value) = delete;
...
template <typename _M, typename _V>
_GLIBCXX_SIMD_INTRINSIC _GLIBCXX_SIMD_CONSTEXPR typename _V::value_type
...
clamp(const simd<_Tp, _Ap> &__v, const simd<_Tp, _Ap> &__lo,
  ...
  return {
      __private_init,
      _Impl::_S_min(__data(__hi), _Impl::_S_max(__data(__lo), __data(__v)))};
}
...
_GLIBCXX_SIMD_INTRINSIC _GLIBCXX_CONST constexpr conditional_t<
    _Np == _Total and _Combine == 1, _Tp,
...
template <typename _V, typename _Ap,
          size_t _Parts = simd_size_v<typename _V::value_type, _Ap> /
                          _V::size()>
    ...
    return __gen_fallback();
                     ...
                     (is_same_v<typename _V::abi_type, simd_abi::scalar> ||
                      (__is_fixed_size_abi_v<typename _V::abi_type> &&
                       sizeof(_V) ==
                           sizeof(_Tp) * _V::size() // _V doesn't have padding
                       ))) {
...
template <typename _V, typename _Ap,
          size_t _Parts = simd_size_v<typename _V::simd_type::value_type, _Ap> /
                          _V::size()>
...
_GLIBCXX_SIMD_ALWAYS_INLINE tuple<simd<_Tp, simd_abi::deduce_t<_Tp, _Sizes>>...>
...
inline _GLIBCXX_SIMD_CONSTEXPR
  ...
  if constexpr (sizeof...(__xs) == 1)
    ...
    return {__private_init,
            __vec_shuffle(
                __as_vector(__xs)...,
                std::make_index_sequence<_RW::_S_full_size>(), [](int __i) {
...
_GLIBCXX_SIMD_ALWAYS_INLINE
  ...
  return __call_with_subscripts<_Np>(
      __x, [](const auto &...__xs)
               _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA { return concat(__xs...); });
}
  ...
  static constexpr auto _S_determine_best_abi() {
    ...
    if constexpr (_Np == 1)
      ...
      if constexpr (_A0<_Bytes>::template _S_is_valid_v<_Tp> &&
        ...
        using _Bp = typename __find_next_valid_abi<_A0, _Bytes, _Tp>::type;
        if constexpr (_Bp::template _S_is_valid_v<_Tp> &&
                      _Bp::template _S_size<_Tp> <= _Np)
          return _Bp{};
        else
...
using _NoSveAllNativeAbis =
    _AbiList<simd_abi::_VecBltnBtmsk, simd_abi::_VecBuiltin,
             __scalar_abi_wrapper>;
...
template <typename _Tp, size_t _Np, typename>
struct __no_sve_deduce_impl : public __deduce_fixed_size_fallback<_Tp, _Np> {};
...
public:
  ...
  _GLIBCXX_SIMD_ALWAYS_INLINE _GLIBCXX_SIMD_CONSTEXPR explicit simd_mask(
  ...
  simd_mask(const value_type *__mem, simd_mask __k, _IsSimdFlagType<_Flags>)
      : _M_data{} {
    _M_data = _Impl::_S_masked_load(
        _M_data, __k._M_data, _Flags::template _S_apply<simd_mask>(__mem));
  }
  ...
  // }}}
  // bitset_init ctor {{{
  _GLIBCXX_SIMD_INTRINSIC constexpr simd_mask(_BitsetInit,
                                              bitset<size()> __init)
  ...
  struct _CvtProxy {
    ...
    _GLIBCXX_SIMD_ALWAYS_INLINE operator simd_mask<_Up, _A2>() && {
  ...
  _GLIBCXX_SIMD_ALWAYS_INLINE _GLIBCXX_SIMD_CONSTEXPR friend simd_mask operator
      ?: (const simd_mask &__k, const simd_mask &__where_true,
  ...
  template <typename _U1, typename _U2,
      ...
      ?: (const simd_mask &__k, const _U1 &__where_true,
  ...
  template <typename _Kp, typename _Ak, typename _Up, typename _Au,
      ...
      _GLIBCXX_SIMD_ALWAYS_INLINE _GLIBCXX_SIMD_CONSTEXPR friend simd_mask
      operator
      ?: (const simd_mask<_Kp, _Ak> &__k, const simd_mask &__where_true,
...
class _SimdIntOperators<_V, _Tp, _Abi, true> {
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr const _V &__derived() const {
...
public:
  _GLIBCXX_SIMD_ALWAYS_INLINE _GLIBCXX_SIMD_CONSTEXPR friend _V &
  ...
  _GLIBCXX_SIMD_ALWAYS_INLINE _GLIBCXX_SIMD_CONSTEXPR _V operator~() const {
...
public:
  ...
  simd(_ValuePreservingOrInt<_Up, value_type> &&__x)
      : _M_data(_Impl::_S_broadcast(
            static_cast<value_type>(static_cast<_Up &&>(__x)))) {}
  ...
  simd(const _Up *__mem, _IsSimdFlagType<_Flags>)
      : _M_data(_Impl::_S_load(_Flags::template _S_apply<simd>(__mem),
                               _S_type_tag)) {}
