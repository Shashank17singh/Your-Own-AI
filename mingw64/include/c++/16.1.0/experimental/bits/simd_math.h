...
#define _GLIBCXX_SIMD_MATH_CALL_(__name)                                       \
  ...
  _GLIBCXX_SIMD_ALWAYS_INLINE enable_if_t<is_floating_point_v<_Tp>, _R>        \
      ...
      decltype(std::__name(                                                    \
          ...
          declval<enable_if_t<                                                 \
              conjunction_v<                                                   \
                  ...
                  is_convertible<_Up, simd<_Tp, _Abi>>,                        \
...
#define _GLIBCXX_SIMD_MATH_CALL3_(__name, __arg2, __arg3)                      \
  ...
  template <typename _T0, typename _T1, typename _T2, typename...,             \
            ...
            typename = enable_if_t<conjunction_v<                              \
                ...
                negation<conjunction<                                          \
                    is_simd<_U0>,                                              \
                    is_floating_point<__value_type_or_identity_t<_U0>>>>>>>    \
...
template <typename _Tp, typename _Abi>
_GLIBCXX_SIMD_INTRINSIC rebind_simd_t<int, simd<_Tp, _Abi>>
...
inline auto __impl_or_fallback_dispatch(float, ImplFun &&,
                                        FallbackFun &&__fallback_fun,
                                        _Args &&...__args)
...
_GLIBCXX_SIMD_INTRINSIC _SimdWrapper<_Tp, _Np>
...
frexp(const simd<_Tp, _Abi> &__x, _Samesize<int, simd<_Tp, _Abi>> *__exp) {
  if constexpr (simd_size_v<_Tp, _Abi> == 1) {
    ...
    if (_GLIBCXX_SIMD_IS_LIKELY(all_of(isnormal(__x)))) {
      *__exp = simd_cast<_Samesize<int, _V>>(__exponent_bits - __exp_adjust);
      return __mant;
    }
    ...
    const auto __inf =
        simd_bit_cast<rebind_simd_t<_Ip, _V>>(_V(__infinity_v<_Tp>));
...
#define _GLIBCXX_SIMD_CVTING2(_NAME)                                           \
  ...
  _GLIBCXX_SIMD_INTRINSIC simd<_Tp, _Abi> _NAME(                               \
      ...
      const __type_identity_t<simd<_Tp, _Abi>> &__y) {                         \
...
#define _GLIBCXX_SIMD_CVTING3(_NAME)                                           \
  ...
  _GLIBCXX_SIMD_INTRINSIC simd<_Tp, _Abi> _NAME(                               \
      const __type_identity_t<simd<_Tp, _Abi>> &__x,                           \
      const simd<_Tp, _Abi> &__y, const simd<_Tp, _Abi> &__z) {                \
...
__remove_cvref_t<_VV> __hypot(_VV __x, _VV __y) {
  ...
  if constexpr (_V::size() == 1)
    ...
    if constexpr (__have_neon &&
                  !__have_neon_a32) { // With ARMv7 NEON, we have no subnormals
                                      // and must use slightly
      // different strategy
...
#endif
    {
      ...
      where(__hi >= __norm_min_v<_Tp>, __hi_exp) = __hi & __inf; // no error
  ...
  if constexpr (__is_fixed_size_abi_v<_Abi> && _V::size() > 1) {
    return __fixed_size_apply<simd<_Tp, _Abi>>(
        [](auto __a, auto __b, auto __c)
            _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA { return hypot(__a, __b, __c); },
        __x, __y, __z);
  } else {
    using namespace __float_bitwise_operators;
    using namespace __proposed;
    ...
    if constexpr (__digits_v<_Tp> == 64 && __max_exponent_v<_Tp> == 0x4000 &&
                  __min_exponent_v<_Tp> == -0x3FFD &&
                  _V::size() == 1) { // Seems like x87 fp80, where bit 63 is
                                     // always 1 unless subnormal or NaN. In
                                     // this case the bit-tricks don't work,
                                     // they require IEC559 binary32 or binary64
                                     // format.
#ifdef __STDC_IEC_559__ fixup for Annex F requirements
...
#ifndef __FAST_MATH__
      if constexpr (_V::size() > 1 && __is_neon_abi<_Abi>() && __have_neon &&
                    !__have_neon_a32) { // With ARMv7 NEON, we have no
                                        // subnormals and must use slightly
        // different strategy
        const _V __hi_exp = __hi & __inf;
        _V __scale_back = __hi_exp;
...
copysign(const simd<_Tp, _Abi> &__x, const simd<_Tp, _Abi> &__y) {
  if constexpr (simd_size_v<_Tp, _Abi> == 1)
    ...
    return {__private_init,
            _Abi::_SimdImpl::_S_copysign(__data(__x), __data(__y))};
  else {
    using _V = simd<_Tp, _Abi>;
    using namespace std::experimental::__float_bitwise_operators;
...
_GLIBCXX_SIMD_ALWAYS_INLINE simd_mask<_Tp, _Abi> signbit(simd<_Tp, _Abi> __x) {
