...
_GLIBCXX_SIMD_INTRINSIC constexpr __tuple_element_meta<_Tp, _Abi, _Offset>
...
_GLIBCXX_SIMD_INTRINSIC decltype(auto) __add_offset(_Tp &__base) {
  return static_cast<_WithOffset<_Offset, __remove_cvref_t<_Tp>> &>(__base);
}

template <size_t _Offset, typename _Tp>
_GLIBCXX_SIMD_INTRINSIC decltype(auto) __add_offset(const _Tp &__base) {
...
_GLIBCXX_SIMD_INTRINSIC decltype(auto)
...
struct _SimdTuple<_Tp, _Abi0, _Abis...>
  ...
  operator=(const _SimdTuple &) = default;
  ...
  _M_apply_per_chunk(_Fp &&__fun, _More &&...__more) const {
    if constexpr ((... || conjunction_v<
      ...
      auto &&__first =
          [&](auto... __args) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
            auto __r =
  ...
  _M_test(const _Fp &__fun, const _SimdTuple &__x, const _More &...__more) {
    const _SanitizedBitMask<_S_first_size> __first =
        _Abi0::_MaskImpl::_S_to_bits(__fun(
            __tuple_element_meta<_Tp, _Abi0, 0>(), __x.first, __more.first...));
    if constexpr (_S_tuple_size == 1)
      return __first;
    else
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr _Tp operator[](size_t __i) const noexcept {
    if constexpr (_S_tuple_size == 1)
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr void _M_set(size_t __i,
                                                _Tp __val) noexcept {
    if constexpr (_S_tuple_size == 1)
...
_GLIBCXX_SIMD_INTRINSIC _R constexpr __to_simd_tuple(const _V0 __from0,
  ...
  if constexpr (_R::_S_tuple_size == 1) {
    ...
    if constexpr (1 == _R::_S_first_size) { // extract one scalar and recurse
      if constexpr (_Offset + 1 < _V0VT::_S_full_size)
        return _R{__from0[_Offset], __to_simd_tuple<_Tp, _Np - 1, _Offset + 1>(
                                        __from0, __fromX...)};
      else
        return _R{__from0[_Offset],
  ...
  if constexpr (is_same_v<_R, _Tup>)
    ...
    return {
        __generate_from_n_evaluations<_R::_S_first_size,
            ...
            [&](auto __i)
                _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA { return __x[__i]; }),
...
template <size_t _Offset = 0, typename _Tp, typename _A0, typename _Fp>
_GLIBCXX_SIMD_INTRINSIC constexpr void __for_each(_SimdTuple<_Tp, _A0> &__t,
                                                  _Fp &&__fun) {
  static_cast<_Fp &&>(__fun)(__make_meta<_Offset>(__t), __t.first);
}
...
__extract_part(const _SimdTuple<_Tp, _A0, _As...> &__x) {
  ...
  // recurse to skip unused data members at the beginning of _SimdTuple
  else if constexpr (__values_to_skip >= _Tuple::_S_first_size) { // recurse
    if constexpr (_Tuple::_S_first_size % __values_per_part == 0) {
      ...
      return __extract_part<_Index - __parts_in_first,
                            _Total - __parts_in_first, _Combine>(__x.second);
    } else
...
struct __autocvt_to_simd {
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr operator _TT() { return _M_data; }

  _GLIBCXX_SIMD_INTRINSIC constexpr operator _TT &() {
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr __autocvt_to_simd(_Tp dd) : _M_data(dd) {}
...
template <typename _Tp> struct __autocvt_to_simd<_Tp, true> {
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr operator fixed_size_simd<_TT, 1>() {
    return _M_fd;
  }
...
template <int _Np> struct simd_abi::_Fixed {
  ...
  template <typename _Tp> struct __traits<_Tp, true> {
    ...
    struct _SimdBase {
      ...
      _GLIBCXX_SIMD_ALWAYS_INLINE constexpr _SimdBase(const _SimdBase &) {}
    ...
    struct _SimdCastType {
      _GLIBCXX_SIMD_ALWAYS_INLINE constexpr _SimdCastType(
          const array<_Tp, _Np> &);

      _GLIBCXX_SIMD_ALWAYS_INLINE constexpr _SimdCastType(const _SimdMember &dd)
          : _M_data(dd) {}
...
template <int _Np, typename> struct _SimdImplFixedSize {
  // member types {{{2
  using _MaskMember = _SanitizedBitMask<_Np>;
  ...
  template <typename _Tp> using _Simd = simd<_Tp, simd_abi::fixed_size<_Np>>;
  ...
  _S_masked_load(const _SimdTuple<_Tp, _As...> &__old, const _MaskMember __bits,
    ...
    auto __merge = __old;
    __for_each(__merge, [&](auto __meta,
...
#pragma GCC diagnostic push
      ...
      // It is the responsibility of the caller of the masked load (via the
      // mask's value) to avoid UB. Consequently, the compiler may assume this
...
#pragma GCC diagnostic pop
  ...
  _S_negate(const _SimdTuple<_Tp, _As...> &__x) noexcept {
    ...
    __for_each(
        __x,
        [&__bits](auto __meta,
                  auto __native) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
          __bits |=
  ...
  static constexpr inline _Tp _S_reduce(const _Simd<_Tp> &__x,
    ...
    if constexpr (_Tup::_S_tuple_size == 1)
      ...
      return __binary_op(simd<_Tp, simd_abi::scalar>(reduce(
                             __tup.template _M_simd_at<0>(), __binary_op)),
      ...
      return __binary_op(simd<_Tp, simd_abi::scalar>(reduce(
                             __tup.template _M_simd_at<0>(), __binary_op)),
                         simd<_Tp, simd_abi::scalar>(reduce(
                             __tup.template _M_simd_at<1>(), __binary_op)))[0];
    } else {
      const auto &__x2 = __call_with_n_evaluations<__div_roundup(
          _Tup::_S_tuple_size, 2)>(
  ...
  _S_min(const _SimdTuple<_Tp, _As...> &__a,
    ...
    return __a._M_apply_per_chunk(
        [](auto __impl, auto __aa,
           auto __bb) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
  ...
  _S_complement(const _SimdTuple<_Tp, _As...> &__x) noexcept {
    return __x._M_apply_per_chunk(
        [](auto __impl,
           auto __xx) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
  ...
  _GLIBCXX_SIMD_INTRINSIC static constexpr _SimdTuple<_Tp, _As...> name_(      \
    ...
    return __x._M_apply_per_chunk(                                             \
        [](auto __impl, auto __xx,                                             \
           auto __yy) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {           \
    ...
    if constexpr (sizeof...(_More) == 0) {                                     \
      if constexpr (is_same_v<_Tp, _RetTp>)                                    \
        ...
        return __optimize_simd_tuple(__x.template _M_apply_r<_RetTp>(          \
            [](auto __impl, auto __xx) _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {    \
              return __impl._S_##__name(__xx);                                 \
            }));                                                               \
    } else if constexpr (is_same_v<_Tp, _RetTp> &&                             \
                         (... && is_same_v<_SimdTuple<_Tp, _As...>, _More>))   \
      return __x._M_apply_per_chunk(                                           \
          [](auto __impl, auto __xx,                                           \
             auto... __pack) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {    \
            using _V = typename decltype(__impl)::simd_type;                   \
            return __data(__name(_V(__private_init, __xx),                     \
...
#undef _GLIBCXX_SIMD_APPLY_ON_TUPLE
  ...
  static inline _SimdTuple<_Tp, _Abis...> _S_remquo(
...
#undef _GLIBCXX_SIMD_TEST_ON_TUPLE_
  ...
  _S_increment(_SimdTuple<_Ts...> &__x) {
    __for_each(__x,
               [](auto __meta,
                  auto &native) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
...
#define _GLIBCXX_SIMD_CMP_OPERATIONS(__cmp)                                    \
  template <typename _Tp, typename... _As>                                     \
  _GLIBCXX_SIMD_INTRINSIC static constexpr _MaskMember __cmp(                  \
      const _SimdTuple<_Tp, _As...> &__x,                                      \
      const _SimdTuple<_Tp, _As...> &__y) {                                    \
    return _M_test(                                                            \
        [](auto __impl, auto __xx,                                             \
...
#undef _GLIBCXX_SIMD_CMP_OPERATIONS
  ...
  _S_masked_assign(const _MaskMember __bits, _SimdTuple<_Tp, _As...> &__lhs,
    ...
    __for_each(
        ...
        [&](auto __meta, auto &__native_lhs,
            auto __native_rhs) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
  ...
  _S_masked_assign(const _MaskMember __bits, _SimdTuple<_Tp, _As...> &__lhs,
    ...
    __for_each(
        __lhs,
        [&](auto __meta,
            auto &__native_lhs) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
          __meta._S_masked_assign(__meta._S_make_mask(__bits), __native_lhs,
                                  __rhs);
        });
  }
  ...
  // the scalar to a simd first.
  template <typename _Op, typename _Tp, typename... _As>
  static constexpr inline void _S_masked_cassign(const _MaskMember __bits,
...
template <int _Np, typename> struct _MaskImplFixedSize {
  static_assert(
  ...
  _S_load(const bool *__mem) {
    ...
    // called.
    const simd<_Ip, _Abi> __bools(
        reinterpret_cast<const __may_alias<_Ip> *>(__mem), element_aligned);
  ...
  // _S_load {{{2
  static constexpr inline _MaskMember _S_load(const bool *__mem) noexcept {
    ...
    __for_each(
        _Vs{}, [&](auto __meta, auto) _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
          __r |= __meta._S_mask_to_shifted_ullong(__meta._S_mask_impl._S_load(
              &__mem[__meta._S_offset], _SizeConstant<__meta._S_size()>()));
        });
    return __r;
  }
  ...
  // _S_masked_load {{{2
  static constexpr inline _MaskMember
  _S_masked_load(_MaskMember __merge, _MaskMember __mask,
  ...
  // _S_store {{{2
  static constexpr inline void _S_store(const _MaskMember __bitmask,
                                        bool *__mem) noexcept {
    if constexpr (_Np == 1)
      __mem[0] = __bitmask[0];
    else
  ...
  // _S_masked_store {{{2
  static constexpr inline void _S_masked_store(const _MaskMember __v,
                                               bool *__mem,
                                               const _MaskMember __k) noexcept {
    _BitOps::_S_bit_iteration(__k,
                              [&](auto __i) _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
                                __mem[__i] = __v[__i];
                              });
  }
