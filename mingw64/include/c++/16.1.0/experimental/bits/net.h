...
using __deduced_t = typename async_result<decay_t<_CompletionToken>, _Signature,
                                          void>::return_type;
...
template <typename _Tp, bool = true> struct __sockopt_base {
  __sockopt_base() = default;

  explicit __sockopt_base(_Tp __val) noexcept(
      noexcept(_Tp(std::declval<_Tp &>())))
      : _M_value(__val) {}
...
template <> struct __sockopt_base<bool> : __sockopt_base<int, false> {
  __sockopt_base() = default;

  explicit __sockopt_base(bool __val) noexcept
...
template <> struct __sockopt_base<int> : __sockopt_base<int, false> {
...
struct __sockopt_crtp : __sockopt_base<_Tp> {
  ...
  _Derived &operator=(_Tp __value) noexcept(noexcept(__value = __value)) {
...
template <typename _Tp>
concept __protocol_like = copyable<_Tp> && requires { typename _Tp::endpoint; };

// Endpoint requirements for non-extensible implementations.
template <typename _Tp>
concept __endpoint_base = semiregular<_Tp> && requires {
  typename _Tp::protocol_type;
} && __protocol_like<typename _Tp::protocol_type> && requires(const _Tp __a) {
...
template <typename _Tp>
concept __endpoint =
    __endpoint_base<_Tp> && requires(const _Tp &__a, _Tp &__b, size_t __s) {
...
template <typename _Tp>
concept __protocol_base =
    __protocol_like<_Tp> && __endpoint_base<typename _Tp::endpoint> &&
    same_as<typename _Tp::endpoint::protocol_type, _Tp>;

// Protocol requirements for extensible implementations.
template <typename _Tp>
concept __protocol =
    __protocol_base<_Tp> && __endpoint<typename _Tp::endpoint> &&
...
template <typename _Tp>
concept __acceptable_protocol =
    __protocol<_Tp> && requires { typename _Tp::socket; } &&
    move_constructible<typename _Tp::socket> &&
    derived_from<typename _Tp::socket, basic_socket<_Tp>>;

template <typename _Tp>
concept __inet_protocol =
    __acceptable_protocol<_Tp> && equality_comparable<_Tp> && requires {
      { _Tp::v4() } -> same_as<_Tp>;
      { _Tp::v6() } -> same_as<_Tp>;
...
template <typename _Tp, typename = void> struct __is_endpoint : false_type {};

template <typename _Tp>
auto __endpoint_reqs(const _Tp *__a = nullptr, _Tp *__b = nullptr)
    -> enable_if_t<
        __and_<is_default_constructible<_Tp>, __is_value_constructible<_Tp>,
               ...
               is_same<decltype(__a->capacity()), size_t>>::value,
...
template <typename _Tp, typename = void> struct __is_protocol : false_type {};

template <typename _Tp>
auto __protocol_reqs(const _Tp *__a = nullptr)
    -> enable_if_t<__and_<is_copy_constructible<_Tp>, is_copy_assignable<_Tp>,
                          __is_endpoint<typename _Tp::endpoint>,
                          is_same<decltype(__a->family()), int>,
                          is_same<decltype(__a->type()), int>,
                          is_same<decltype(__a->protocol()), int>>::value>;
...
struct __is_acceptable_protocol : false_type {};

template <typename _Tp>
struct __is_acceptable_protocol<_Tp, __void_t<typename _Tp::socket>>
    : __and_<
          __is_protocol<_Tp>, is_move_constructible<typename _Tp::socket>,
          is_convertible<typename _Tp::socket *, basic_socket<_Tp> *>>::type {};

// Check InternetProtocol requirements
template <typename _Tp, typename = void>
struct __is_inet_protocol : false_type {};

template <typename _Tp>
auto __inet_proto_reqs(const _Tp *__a = nullptr) -> enable_if_t<
    __and_<__is_acceptable_protocol<_Tp>,
           is_same<typename _Tp::resolver, ip::basic_resolver<_Tp>>,
           is_same<decltype(_Tp::v4()), _Tp>, is_same<decltype(_Tp::v6()), _Tp>,
           is_convertible<decltype(*__a == *__a), bool>,
           is_convertible<decltype(*__a != *__a), bool>>::value>;

template <typename _Tp>
struct __is_inet_protocol<_Tp, decltype(__inet_proto_reqs<_Tp>())> : true_type {
};

// Variable templates for requirements (with same names as concepts above).

template <typename _Tp> constexpr bool __endpoint = __is_endpoint<_Tp>::value;
template <typename _Tp> constexpr bool __protocol = __is_protocol<_Tp>::value;
template <typename _Tp>
