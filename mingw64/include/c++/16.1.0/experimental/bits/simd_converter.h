...
struct _SimdConverter<
  ...
  _GLIBCXX_SIMD_INTRINSIC constexpr _Ret operator()(_Arg __x) const noexcept {
...
struct _SimdConverter<_From, simd_abi::fixed_size<_Np>, _To,
  ...
  operator()(const _Arg &__x) const noexcept {
    ...
    else if constexpr (sizeof(_From) == sizeof(_To) && is_integral_v<_From> &&
                       is_integral_v<_To>)
    ...
    // special case if all ABI tags in _Ret are scalar
      ...
      return __call_with_subscripts(
          ...
          [](auto... __values) constexpr _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA
          -> _Ret {
    ...
    // from one vector to multiple vectors
      ...
      else if constexpr (_Arg::_S_tuple_size ==
                         1) { // e.g. <int, 3> -> <double, 2, 1> or <short, 7>
                              // -> <double, 4, 2,
        // 1>
        using _RetRem =
        ...
        if constexpr (__converted2 == _Np)
          ...
          using _RetRem2 =
              __remove_cvref_t<decltype(__simd_tuple_pop_front<
                                        __return_chunks2.size() *
                                        _RetRem::_S_first_size>(_RetRem()))>;
          const auto __return_chunks3 =
              __convert_all<__vector_type_t<_To, _RetRem2::_S_first_size>, 0,
          ...
          if constexpr (__converted3 == _Np)
            return __to_simd_tuple<_To, _Np>(
                __multiple_return_chunks, __return_chunks2, __return_chunks3);
          else {
            using _RetRem3 = __remove_cvref_t<
                decltype(__simd_tuple_pop_front<__return_chunks3.size() *
                                                _RetRem2::_S_first_size>(
                    _RetRem2()))>;
            const auto __return_chunks4 =
                __convert_all<__vector_type_t<_To, _RetRem3::_S_first_size>, 0,
          ...
          [&__x](auto... __uncvted) _GLIBCXX_SIMD_ALWAYS_INLINE_LAMBDA {
            ...
            if constexpr (_Ret::_S_tuple_size == 1)
              ...
              return _Ret{
                  ...
                  _SimdConverter<
                      _From, simd_abi::fixed_size<_Np - _Ret::_S_first_size>,
                      _To, simd_abi::fixed_size<_Np - _Ret::_S_first_size>>()(
