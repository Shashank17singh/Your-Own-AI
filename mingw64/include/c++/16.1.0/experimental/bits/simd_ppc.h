  ...
  _S_bit_shift_right(_SimdWrapper<_Tp, _Np> __x, _SimdWrapper<_Tp, _Np> __y) {
    if constexpr (sizeof(_Tp) < sizeof(int)) {
      ...
      if constexpr (is_unsigned_v<_Tp>)
        ...
        _Base::_S_masked_assign(_SimdWrapper<_Tp, _Np>(__y._M_data >= __nbits),
                                __y, __nbits - 1);
...
struct _MaskImplPpc : _MaskImplBuiltin<_Abi> {
  ...
  _GLIBCXX_SIMD_INTRINSIC static int _S_popcount(simd_mask<_Tp, _Abi> __k) {
    ...
    if constexpr (__have_power10vec) {
      using _Intrin =
          __intrinsic_type16_t<make_unsigned_t<__int_for_sizeof_t<_Tp>>>;
      ...
      const int __sum =
          -vec_sums(__intrin_bitcast<_Intrin>(__kv), _Intrin())[3];
      return __sum / (sizeof(_Tp) / sizeof(int));
    } else {
      const auto __summed_to_int =
          vec_sum4s(__to_intrin(__kv), __intrinsic_type16_t<int>());
