...
template <typename _Container> class _Safe_unordered_container;

#ifdef __glibcxx_node_extract // >= C++17 && HOSTED
template <typename _ExtractKey, typename _Source> struct _UContInvalidatePred {
  template <typename _Iterator> bool operator()(_Iterator __it) const {
...
template <typename _ExtractKey, typename _Source> struct _UMContInvalidatePred {
  template <typename _Iterator> bool operator()(_Iterator __it) const {
    auto __rng =
...
template <typename _Source, typename _InvalidatePred> struct _UContMergeGuard {
  ...
  ~_UContMergeGuard() {
    ...
    __catch(...) { _M_source._M_invalidate_all(); }
  }
...
class _Safe_unordered_container : public _Safe_unordered_container_base {
  const _Container &_M_cont() const noexcept {
    return *static_cast<const _Container *>(this);
  }

  const _Safe_unordered_container *_M_self() const { return this; }
...
protected:
  ...
  template <typename _ExtractKey, typename _Source>
  static _UContMergeGuard<_Source, _UContInvalidatePred<_ExtractKey, _Source>>
  ...
  template <typename _ExtractKey, typename _Source>
  static _UContMergeGuard<_Source, _UMContInvalidatePred<_ExtractKey, _Source>>
  ...
  void _M_invalidate_all() {
    ...
    _M_invalidate_if([__end](decltype(__end) __it) { return __it != __end; },
                     sentry);
  ...
  template <typename _Predicate> void _M_invalidate_all_if(_Predicate __pred) {
    __gnu_cxx::__scoped_lock sentry(_M_self()->_M_get_mutex());
    _M_invalidate_if(__pred, sentry);
    _M_invalidate_local_if(__pred, sentry);
  }

  template <typename _VictimIt> void _M_invalidate(_VictimIt __victim) {
    __gnu_cxx::__scoped_lock sentry(_M_self()->_M_get_mutex());
  ...
  void _M_invalidate(_VictimIt __victim,
                     const __gnu_cxx::__scoped_lock &__lock) {
    auto __end = _M_cont()._M_base().cend();
    _M_invalidate_if(
        [__victim](decltype(__end) __it) { return __it == __victim; }, __lock);

    auto __local_end = _M_cont()._M_base().cend(0);
    _M_invalidate_local_if(
        [__victim](decltype(__local_end) __it) { return __it == __victim; },
        __lock);
  }
...
private:
  ...
  void _M_invalidate_if(_Predicate __pred, const __gnu_cxx::__scoped_lock &);
