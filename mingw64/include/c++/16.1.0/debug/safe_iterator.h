  ...
  if (!std::__is_constant_evaluated()) {                                       \
    _GLIBCXX_DEBUG_VERIFY(                                                     \
        (!_Lhs._M_singular() && !_Rhs._M_singular()) ||                        \
            (_Lhs._M_value_initialized() && _Rhs._M_value_initialized()),      \
...
public:
  ...
  _GLIBCXX20_CONSTEXPR _Safe_iterator(
  ...
  bool _M_can_advance(const std::pair<_Diff, _Distance_precision> &__dist,
                      int __way) const;
  ...
  typename __gnu_cxx::__conditional_type<_IsConstant::__value,
  ...
  _GLIBCXX_NODISCARD _GLIBCXX20_CONSTEXPR friend bool
...
#if __cpp_lib_three_way_comparison
  [[nodiscard]]
  _GLIBCXX20_CONSTEXPR friend auto operator<=>(const _Self &__lhs,
...
__valid_range(const _Safe_iterator<_Iterator, _Sequence, _Category> &__first,
...
_GLIBCXX20_CONSTEXPR inline bool
__can_advance(const _Safe_iterator<_Iterator, _Sequence, _Category> &__it,
              const std::pair<_Diff, _Distance_precision> &__dist, int __way) {
  if (std::__is_constant_evaluated())
    return true;
...
template <typename _Iterator, typename _Sequence>
_GLIBCXX20_CONSTEXPR _Iterator __base(
    const _Safe_iterator<_Iterator, _Sequence, std::random_access_iterator_tag>
        &__it) {
  return __it.base();
}
