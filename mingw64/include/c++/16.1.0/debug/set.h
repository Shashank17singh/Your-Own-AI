...
public:
  ...
  set(set &&__x, const __type_identity_t<allocator_type> &__a) noexcept(
      noexcept(_Base(std::move(__x), __a)))
      : _Safe(std::move(__x), __a), _Base(std::move(__x), __a) {}
  ...
  set(std::from_range_t __t, _Rg &&__rg, const _Compare &__c,
      const allocator_type &__a = allocator_type())
  ...
  set(_Base_ref __x) : _Base(__x._M_ref) {}
...
#ifdef __glibcxx_associative_heterogeneous_insertion
  template <__heterogeneous_tree_key<set> _Kt>
  iterator insert(const_iterator __position, _Kt &&__x) {
    __glibcxx_check_insert(__position);
  ...
  template <__heterogeneous_tree_key<set> _Kt> node_type extract(_Kt &&__key) {
    const auto __position = find(__key);
    if (__position != end())
      return extract(__position);
    return {};
  ...
  // of the key.
  template <__heterogeneous_tree_key<set> _Kt> size_type erase(_Kt &&__x) {
