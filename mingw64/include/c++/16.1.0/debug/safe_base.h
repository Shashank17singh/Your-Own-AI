...
protected:
  ...
  __gnu_cxx::__mutex &_M_get_mutex() _GLIBCXX_USE_NOEXCEPT;
...
public:
  /** Likewise, but not thread-safe. */
  void _M_detach_single() _GLIBCXX_USE_NOEXCEPT;
  ...
  _GLIBCXX_PURE bool _M_singular() const _GLIBCXX_USE_NOEXCEPT;
  ...
  void _M_reset() _GLIBCXX_USE_NOEXCEPT;

  /** Unlink itself */
  void _M_unlink() _GLIBCXX_USE_NOEXCEPT {
    if (_M_prior)
...
protected:
   ...
   */
  void _M_swap(const _Safe_sequence_base &__x) const _GLIBCXX_USE_NOEXCEPT;

  /** For use in _Safe_sequence. */
  __gnu_cxx::__mutex &_M_get_mutex() const _GLIBCXX_USE_NOEXCEPT;

  /** Invalidates all iterators. */
  _GLIBCXX20_CONSTEXPR void _M_invalidate_all() const {
    if (++_M_version == 0)
      _M_version = 1;
  }
...
private:
  ...
  void _M_attach(_Safe_iterator_base *__it, bool __constant) const;

  /** Likewise but not thread safe. */
  void _M_attach_single(_Safe_iterator_base *__it,
                        bool __constant) const _GLIBCXX_USE_NOEXCEPT;

  /** Detach an iterator from this sequence */
  void _M_detach(_Safe_iterator_base *__it) const;

  /** Likewise but not thread safe. */
  void _M_detach_single(_Safe_iterator_base *__it) const _GLIBCXX_USE_NOEXCEPT;
};
} // namespace __gnu_debug
