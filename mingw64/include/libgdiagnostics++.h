...
class text_sink {
public:
  text_sink(diagnostic_text_sink *inner) : m_inner(inner) {}
...
public:
  ...
  execution_path(const diagnostic_execution_path *path)
      : m_inner(const_cast<diagnostic_execution_path *>(path)), m_owned(false) {
  }
  ...
  execution_path(execution_path &&other)
      : m_inner(other.m_inner), m_owned(other.m_owned) {
    other.m_inner = nullptr;
    other.m_owned = false;
  }
  ...
  diagnostic_event_id add_event(physical_location physical_loc,
                                logical_location logical_loc,
                                unsigned stack_depth, const char *fmt, ...)
      LIBGDIAGNOSTICS_PARAM_GCC_FORMAT_STRING(5, 6);

  diagnostic_event_id
  add_event_va(physical_location physical_loc, logical_location logical_loc,
               unsigned stack_depth, const char *fmt, va_list *args)
      LIBGDIAGNOSTICS_PARAM_GCC_FORMAT_STRING(5, 0);

  diagnostic_event_id add_event_via_msg_buf(physical_location physical_loc,
                                            logical_location logical_loc,
...
public:
  ...
  void add_location(physical_location loc);

  void add_location_with_label(physical_location loc, const char *text);

  void add_location_with_label(physical_location loc, message_buffer &&text);

  void set_logical_location(logical_location loc);

  void add_fix_it_hint_insert_before(physical_location loc,
                                     const char *addition);
  void add_fix_it_hint_insert_after(physical_location loc,
                                    const char *addition);
  ...
  void take_graph(graph g);

  void finish(const char *fmt, ...) LIBGDIAGNOSTICS_PARAM_MUST_BE_NON_NULL(2)
      LIBGDIAGNOSTICS_PARAM_GCC_FORMAT_STRING(2, 3);

  void finish_va(const char *fmt, va_list *args)
      LIBGDIAGNOSTICS_PARAM_MUST_BE_NON_NULL(2)
          LIBGDIAGNOSTICS_PARAM_GCC_FORMAT_STRING(2, 0);

  void finish_via_msg_buf(message_buffer &&msg_buf);
...
class manager {
public:
  manager() : m_inner(diagnostic_manager_new()), m_owned(true) {}
  manager(diagnostic_manager *inner, bool owned)
      : m_inner(inner), m_owned(owned) {}
  ~manager() {
    if (m_owned)
  ...
  manager(manager &&other) : m_inner(other.m_inner), m_owned(other.m_owned) {
    other.m_inner = nullptr;
  }
  ...
  void add_sarif_sink(FILE *dst_stream, file main_input_file,
                      enum diagnostic_sarif_version version) {
    diagnostic_manager_add_sarif_sink(m_inner, dst_stream,
                                      main_input_file.m_inner, version);
  }

  bool add_sink_from_spec(const char *option_name, const char *spec,
                          manager control_mgr) {
    return diagnostic_manager_add_sink_from_spec(m_inner, option_name, spec,
                                                 control_mgr.m_inner);
  }
  ...
  file new_file(const char *name, const char *sarif_source_language)
      LIBGDIAGNOSTICS_PARAM_MUST_BE_NON_NULL(2)
  ...
  physical_location new_location_from_file_line_column(file f,
                                                       line_num_t line_num,
                                                       column_num_t column_num);

  physical_location new_location_from_range(physical_location loc_caret,
  ...
  new_logical_location(enum diagnostic_logical_location_kind_t kind,
                       logical_location parent, const char *short_name,
...
public:
  ...
  graph(diagnostic_graph *graph) : m_inner(graph), m_owned(true) {}

  graph(const diagnostic_graph *graph)
      : m_inner(const_cast<diagnostic_graph *>(graph)), m_owned(false) {}

  graph(const graph &other) = delete;
  graph &operator=(const graph &other) = delete;

  graph(graph &&other) : m_inner(other.m_inner), m_owned(other.m_owned) {
    other.m_inner = nullptr;
    other.m_owned = false;
  }
  ...
  edge add_edge(const char *id, node src_node, node dst_node,
                const char *label);
  edge add_edge(const char *id, node src_node, node dst_node,
...
inline void file::set_buffered_content(const char *data, size_t sz) {
  diagnostic_file_set_buffered_content(m_inner, data, sz);
}

// class physical_location

inline file physical_location::get_file() const {
...
inline logical_location logical_location::get_parent() const {
  if (m_inner)
...
inline const char *logical_location::get_short_name() const {
  if (m_inner)
...
inline const char *logical_location::get_fully_qualified_name() const {
  if (m_inner)
...
inline const char *logical_location::get_decorated_name() const {
  if (m_inner)
...
execution_path::add_event(physical_location physical_loc,
                          logical_location logical_loc, unsigned stack_depth,
                          const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  diagnostic_event_id result =
      add_event_va(physical_loc, logical_loc, stack_depth, fmt, &args);
  va_end(args);
...
execution_path::add_event_va(physical_location physical_loc,
  ...
  return diagnostic_execution_path_add_event_va(m_inner, physical_loc.m_inner,
                                                logical_loc.m_inner,
                                                stack_depth, fmt, args);
}

inline diagnostic_event_id execution_path::add_event_via_msg_buf(
    physical_location physical_loc, logical_location logical_loc,
    unsigned stack_depth, message_buffer &&msg_buf) {
  diagnostic_message_buffer *inner_msg_buf = msg_buf.m_inner;
  msg_buf.m_inner = nullptr;
  return diagnostic_execution_path_add_event_via_msg_buf(
      m_inner, physical_loc.m_inner, logical_loc.m_inner, stack_depth,
      inner_msg_buf);
}

// class group

inline group::group(manager &mgr) : m_mgr(mgr) {
  diagnostic_manager_begin_group(m_mgr.m_inner);
}

inline group::~group() { diagnostic_manager_end_group(m_mgr.m_inner); }

// class diagnostic

inline void diagnostic::set_cwe(unsigned cwe_id) {
  diagnostic_set_cwe(m_inner, cwe_id);
}

inline void diagnostic::add_rule(const char *title, const char *url) {
  diagnostic_add_rule(m_inner, title, url);
}

inline void diagnostic::set_location(physical_location loc) {
  diagnostic_set_location(m_inner, loc.m_inner);
}

inline void diagnostic::add_location_with_label(physical_location loc,
                                                const char *text) {
...
inline void diagnostic::add_location_with_label(physical_location loc,
  ...
  diagnostic_message_buffer *inner_msg_buf = msg_buf.m_inner;
  msg_buf.m_inner = nullptr;
  diagnostic_add_location_with_label_via_msg_buf(m_inner, loc.m_inner,
                                                 inner_msg_buf);
}

inline void diagnostic::add_location(physical_location loc) {
  diagnostic_add_location(m_inner, loc.m_inner);
}

inline void diagnostic::set_logical_location(logical_location loc) {
  diagnostic_set_logical_location(m_inner, loc.m_inner);
}

inline void diagnostic::add_fix_it_hint_insert_before(physical_location loc,
                                                      const char *addition) {
  diagnostic_add_fix_it_hint_insert_before(m_inner, loc.m_inner, addition);
}

inline void diagnostic::add_fix_it_hint_insert_after(physical_location loc,
                                                     const char *addition) {
  diagnostic_add_fix_it_hint_insert_after(m_inner, loc.m_inner, addition);
}

inline void diagnostic::add_fix_it_hint_replace(physical_location loc,
                                                const char *replacement) {
  diagnostic_add_fix_it_hint_replace(m_inner, loc.m_inner, replacement);
}

inline void diagnostic::add_fix_it_hint_delete(physical_location loc) {
  diagnostic_add_fix_it_hint_delete(m_inner, loc.m_inner);
}

inline void diagnostic::take_execution_path(execution_path path) {
  diagnostic_take_execution_path(m_inner, path.m_inner);
  path.m_owned = false;
}

inline void diagnostic::take_graph(graph g) {
  diagnostic_take_graph(m_inner, g.m_inner);
  g.m_owned = false;
}

inline void diagnostic::finish(const char *fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
...
inline void diagnostic::finish_va(const char *fmt, va_list *args) {
  diagnostic_finish_va(m_inner, fmt, args);
}

inline void diagnostic::finish_via_msg_buf(message_buffer &&msg_buf) {
  diagnostic_message_buffer *inner_msg_buf = msg_buf.m_inner;
  msg_buf.m_inner = nullptr;
...
inline file manager::new_file(const char *name,
...
manager::new_location_from_file_and_line(file f,
  ...
  return physical_location(diagnostic_manager_new_location_from_file_and_line(
      m_inner, f.m_inner, line_num));
}

inline physical_location
manager::new_location_from_file_line_column(file f, line_num_t line_num,
                                            column_num_t column_num) {
  return physical_location(
      diagnostic_manager_new_location_from_file_line_column(
          m_inner, f.m_inner, line_num, column_num));
}
...
manager::new_location_from_range(physical_location loc_caret,
                                 physical_location loc_start,
                                 physical_location loc_end) {
  return physical_location(diagnostic_manager_new_location_from_range(
      m_inner, loc_caret.m_inner, loc_start.m_inner, loc_end.m_inner));
}

inline void manager::debug_dump(physical_location loc, FILE *out) {
  diagnostic_manager_debug_dump_location(m_inner, loc.m_inner, out);
}
...
manager::new_logical_location(enum diagnostic_logical_location_kind_t kind,
                              ...
                              const char *fully_qualified_name,
                              const char *decorated_name) {
  return logical_location(diagnostic_manager_new_logical_location(
      m_inner, kind, parent.m_inner, short_name, fully_qualified_name,
      decorated_name));
}

inline void manager::debug_dump(logical_location loc, FILE *out) {
  diagnostic_manager_debug_dump_logical_location(m_inner, loc.m_inner, out);
}

inline execution_path manager::new_execution_path() {
  return execution_path(diagnostic_manager_new_execution_path(m_inner));
}

inline diagnostic manager::begin_diagnostic(enum diagnostic_level level) {
  return diagnostic(diagnostic_begin(m_inner, level));
}

inline void manager::set_analysis_target(file f) {
  diagnostic_manager_set_analysis_target(m_inner, f.m_inner);
}

inline void manager::take_global_graph(graph g) {
  diagnostic_manager_take_global_graph(m_inner, g.m_inner);
  g.m_owned = false;
}

inline void manager::set_debug_physical_locations(bool value) {
  diagnostic_manager_set_debug_physical_locations(m_inner, value ? 1 : 0);
}

// class graph

inline void graph::set_description(const char *desc) {
  diagnostic_graph_set_description(m_inner, desc);
}

inline void graph::set_description(message_buffer &&msg_buf) {
  diagnostic_message_buffer *inner_msg_buf = msg_buf.m_inner;
  msg_buf.m_inner = nullptr;
  diagnostic_graph_set_description_via_msg_buf(m_inner, inner_msg_buf);
}

inline node graph::get_node_by_id(const char *id) const {
  return node(diagnostic_graph_get_node_by_id(m_inner, id));
}

inline edge graph::get_edge_by_id(const char *id) const {
  return edge(diagnostic_graph_get_edge_by_id(m_inner, id));
}

inline edge graph::add_edge(const char *id, node src_node, node dst_node,
                            const char *label) {
  return edge(diagnostic_graph_add_edge(m_inner, id, src_node.m_inner,
                                        dst_node.m_inner, label));
}
...
inline edge graph::add_edge(const char *id, node src_node, node dst_node,
  ...
  return edge(diagnostic_graph_add_edge_via_msg_buf(
      m_inner, id, src_node.m_inner, dst_node.m_inner, inner_label));
}

// class node

inline void node::set_label(const char *label) {
  diagnostic_node_set_label(m_inner, label);
}

inline void node::set_label(message_buffer &&label) {
  diagnostic_message_buffer *inner_label = label.m_inner;
  label.m_inner = nullptr;
  diagnostic_node_set_label_via_msg_buf(m_inner, inner_label);
}

inline void node::set_location(physical_location loc) {
  diagnostic_node_set_location(m_inner, loc.m_inner);
}

inline void node::set_logical_location(logical_location loc) {
