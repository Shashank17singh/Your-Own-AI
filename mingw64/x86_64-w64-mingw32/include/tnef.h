...
#define MAPI_ITNEF_METHODS(IPURE)                                              \
  MAPIMETHOD(AddProps)(THIS_ ULONG ulFlags, ULONG ulElemID, LPVOID lpvData,    \
                       LPSPropTagArray lpPropList) IPURE;                      \
  MAPIMETHOD(ExtractProps)(THIS_ ULONG ulFlags, LPSPropTagArray lpPropList,    \
                           LPSTnefProblemArray * lpProblems) IPURE;            \
  MAPIMETHOD(Finish)(THIS_ ULONG ulFlags, WORD * lpKey,                        \
                     LPSTnefProblemArray * lpProblems) IPURE;                  \
  MAPIMETHOD(OpenTaggedBody)(THIS_ LPMESSAGE lpMessage, ULONG ulFlags,         \
                             LPSTREAM * lppStream) IPURE;                      \
  MAPIMETHOD(SetProps)(THIS_ ULONG ulFlags, ULONG ulElemID, ULONG cValues,     \
  ...
  MAPIMETHOD(FinishComponent)(                                                 \
      THIS_ ULONG ulFlags, ULONG ulComponentID,                                \
      LPSPropTagArray lpCustomPropList, LPSPropValue lpCustomProps,            \
      LPSPropTagArray lpPropList, LPSTnefProblemArray * lpProblems) IPURE;
#undef INTERFACE
#define INTERFACE ITnef
DECLARE_MAPI_INTERFACE_(ITnef, IUnknown){
    BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE) MAPI_ITNEF_METHODS(PURE)};
STDMETHODIMP OpenTnefStream(LPVOID lpvSupport, LPSTREAM lpStream,
                            LPTSTR lpszStreamName, ULONG ulFlags,
                            LPMESSAGE lpMessage, WORD wKeyVal,
                            LPITNEF *lppTNEF);
typedef HRESULT(WINAPI *LPOPENTNEFSTREAM)(LPVOID lpvSupport, LPSTREAM lpStream,
                                          LPTSTR lpszStreamName, ULONG ulFlags,
                                          LPMESSAGE lpMessage, WORD wKeyVal,
                                          LPITNEF *lppTNEF);
STDMETHODIMP OpenTnefStreamEx(LPVOID lpvSupport, LPSTREAM lpStream,
                              LPTSTR lpszStreamName, ULONG ulFlags,
                              LPMESSAGE lpMessage, WORD wKeyVal,
                              LPADRBOOK lpAdressBook, LPITNEF *lppTNEF);
typedef HRESULT(WINAPI *LPOPENTNEFSTREAMEX)(
    LPVOID lpvSupport, LPSTREAM lpStream, LPTSTR lpszStreamName, ULONG ulFlags,
    LPMESSAGE lpMessage, WORD wKeyVal, LPADRBOOK lpAdressBook,
    LPITNEF *lppTNEF);
STDMETHODIMP GetTnefStreamCodepage(LPSTREAM lpStream, ULONG *lpulCodepage,
                                   ULONG *lpulSubCodepage);
typedef HRESULT(WINAPI *LPGETTNEFSTREAMCODEPAGE)(LPSTREAM lpStream,
                                                 ULONG *lpulCodepage,
                                                 ULONG *lpulSubCodepage);
...
#define MAKE_TNEF_VERSION(_mj, _mn)                                            \
  (((ULONG)(0x0000FFFF & _mj) << 16) | (ULONG)(0x0000FFFF & _mn))
