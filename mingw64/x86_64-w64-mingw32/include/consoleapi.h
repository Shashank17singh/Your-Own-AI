...
WINBASEAPI HRESULT WINAPI AllocConsoleWithOptions(
    PALLOC_CONSOLE_OPTIONS options, PALLOC_CONSOLE_RESULT result);
...
GetNumberOfConsoleInputEvents(HANDLE console_input, LPDWORD number_of_events);
WINBASEAPI WINBOOL WINAPI ReadConsoleInputA(HANDLE console_input,
                                            PINPUT_RECORD buffer, DWORD length,
                                            LPDWORD number_of_events_read);
...
WINBASEAPI WINBOOL WINAPI ReadConsoleA(
    HANDLE console_input, LPVOID buffer, DWORD number_of_chars_to_read,
    LPDWORD number_of_chars_read, PCONSOLE_READCONSOLE_CONTROL input_control);
WINBASEAPI WINBOOL WINAPI ReadConsoleW(
    HANDLE console_input, LPVOID buffer, DWORD number_of_chars_to_read,
    LPDWORD number_of_chars_read, PCONSOLE_READCONSOLE_CONTROL input_control);
#define ReadConsole __MINGW_NAME_AW(ReadConsole)
WINBASEAPI WINBOOL WINAPI WriteConsoleA(HANDLE console_output,
                                        const void *buffer,
                                        DWORD number_of_chars_to_write,
                                        LPDWORD number_of_chars_written,
                                        LPVOID reserved);
WINBASEAPI WINBOOL WINAPI WriteConsoleW(HANDLE console_output,
                                        const void *buffer,
                                        DWORD number_of_chars_to_write,
                                        LPDWORD number_of_chars_written,
                                        LPVOID reserved);
...
WINBASEAPI WINBOOL WINAPI
SetConsoleCtrlHandler(PHANDLER_ROUTINE handler_routine, WINBOOL add);
#endif /* WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP) */
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
#if (NTDDI_VERSION >= NTDDI_WIN10_RS5)
#define PSEUDOCONSOLE_INHERIT_CURSOR (0x1)
WINBASEAPI HRESULT WINAPI CreatePseudoConsole(COORD size, HANDLE input,
                                              HANDLE output, DWORD flags,
                                              HPCON *pc);
