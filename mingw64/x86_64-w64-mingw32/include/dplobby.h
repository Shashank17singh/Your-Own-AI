 ...
 * Response: Answer is a WINBOOL. TRUE if supports the standard (?) and FALSE
 * otherwise. Of course, it might not respond at all.
 */
 ...
 * Purpose: Request the GUID that identifies the lobby version that the
 * application is communicating with. Response: The GUID which identifies the
 * lobby version
 */
 ...
 * Purpose: Request the GUID that identifies the player for this particular
 * machine. Response: DPLDATA_PLAYERDATA structure.
 */
 ...
 * Purpose: Used to send a score of a player to the lobby. The format is an
 * array of long integers. Response: I don't think there is one.
 */
 ...
 *
 * System messages can be identified by dwMessageFlags having a value of
 * DPLMSG_SYSTEM after a call to ReceiveLobbyMessage.
 *
 * Standard messages can be identified by dwMessageFlags having a value of
 * DPLMSG_STANDARD after a call to ReceiveLobbyMessage.
 */
...
typedef struct tagDPLMSG_SETPROPERTY {
  DWORD dwType;            /* Message type */
  DWORD dwRequestID;       /* Request ID (DPL_NOCONFIRMATION if no confirmation
                              desired) */
 ...
 * A DirectPlay address is composed of multiple data chunks, each associated
 * with a GUID to give significance to the type of data. All chunks have an
 * associated size so that unknown chunks can be ignored for backwards
 * compatibility! EnumAddresses function is used to parse the address data
 * chunks.
 */
/* DPAID_TotalSize {1318F560-912C-11d0-9DAA-00A0C90A43CB}
 * Chunk purpose: Chunk is a DWORD containing the size of the entire DPADDRESS
 * struct
 */
 ...
 * Chunk purpose: Chunk is a GUID indicated what service provider created the
 * chunk.
 */
 ...
 * Chunk purpose: Chunk is a GUID indicating what lobby provider created the
 * chunk.
 */
...
typedef WINBOOL(CALLBACK *LPDPENUMADDRESSCALLBACK)(REFGUID guidDataType,
                                                   DWORD dwDataSize,
                                                   LPCVOID lpData,
                                                   LPVOID lpContext);
typedef WINBOOL(CALLBACK *LPDPLENUMADDRESSTYPESCALLBACK)(REFGUID guidDataType,
                                                         LPVOID lpContext,
                                                         DWORD dwFlags);
...
DECLARE_INTERFACE_(IDirectPlayLobby, IUnknown) {
  ...
  STDMETHOD(CreateAddress)(THIS_ REFGUID, REFGUID, LPCVOID, DWORD, LPVOID,
                           LPDWORD) PURE;
  STDMETHOD(EnumAddress)(THIS_ LPDPENUMADDRESSCALLBACK, LPCVOID, DWORD, LPVOID)
      PURE;
  STDMETHOD(EnumAddressTypes)(THIS_ LPDPLENUMADDRESSTYPESCALLBACK, REFGUID,
                              LPVOID, DWORD) PURE;
  STDMETHOD(EnumLocalApplications)(THIS_ LPDPLENUMLOCALAPPLICATIONSCALLBACK,
                                   LPVOID, DWORD) PURE;
  STDMETHOD(GetConnectionSettings)(THIS_ DWORD, LPVOID, LPDWORD) PURE;
  STDMETHOD(ReceiveLobbyMessage)(THIS_ DWORD, DWORD, LPDWORD, LPVOID, LPDWORD)
      PURE;
...
DECLARE_INTERFACE_(IDirectPlayLobby2, IDirectPlayLobby) {
  ...
  STDMETHOD(CreateCompoundAddress)(THIS_ LPCDPCOMPOUNDADDRESSELEMENT, DWORD,
                                   LPVOID, LPDWORD) PURE;
};
#undef INTERFACE
/*****************************************************************************
