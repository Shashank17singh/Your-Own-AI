...
static inline FLOAT FloatMax() { return 3.402823466e+38f; }
template <typename T>
D2D1FORCEINLINE typename TypeTraits<T>::Point Point2(T x, T y) {
...
template <typename T>
D2D1FORCEINLINE typename TypeTraits<T>::Size Size(T width, T height) {
...
template <typename T>
D2D1FORCEINLINE typename TypeTraits<T>::Rect Rect(T left, T top, T right,
                                                  T bottom) {
  typename TypeTraits<T>::Rect r = {left, top, right, bottom};
  return r;
}
D2D1FORCEINLINE D2D1_RECT_F RectF(FLOAT left = 0.0f, FLOAT top = 0.0f,
                                  FLOAT right = 0.0f, FLOAT bottom = 0.0f) {
  return Rect<FLOAT>(left, top, right, bottom);
}
D2D1FORCEINLINE D2D1_RECT_U RectU(UINT32 left = 0, UINT32 top = 0,
                                  UINT32 right = 0, UINT32 bottom = 0) {
...
D2D1FORCEINLINE D2D1_ARC_SEGMENT ArcSegment(const D2D1_POINT_2F &point,
...
D2D1FORCEINLINE D2D1_BEZIER_SEGMENT BezierSegment(const D2D1_POINT_2F &point1,
                                                  const D2D1_POINT_2F &point2,
                                                  const D2D1_POINT_2F &point3) {
  D2D1_BEZIER_SEGMENT r = {point1, point2, point3};
  return r;
}
D2D1FORCEINLINE D2D1_ELLIPSE Ellipse(const D2D1_POINT_2F &center, FLOAT radiusX,
                                     FLOAT radiusY) {
  D2D1_ELLIPSE r = {center, radiusX, radiusY};
  return r;
}
D2D1FORCEINLINE D2D1_ROUNDED_RECT RoundedRect(const D2D1_RECT_F &rect,
                                              FLOAT radiusX, FLOAT radiusY) {
  D2D1_ROUNDED_RECT r = {rect, radiusX, radiusY};
  return r;
}
D2D1FORCEINLINE D2D1_BRUSH_PROPERTIES
BrushProperties(FLOAT opacity = 1.0f,
                const D2D1_MATRIX_3X2_F &transform = D2D1::IdentityMatrix()) {
  D2D1_BRUSH_PROPERTIES r = {opacity, transform};
  return r;
}
D2D1FORCEINLINE D2D1_GRADIENT_STOP GradientStop(FLOAT position,
                                                const D2D1_COLOR_F &color) {
  D2D1_GRADIENT_STOP r = {position, color};
  return r;
}
D2D1FORCEINLINE D2D1_QUADRATIC_BEZIER_SEGMENT QuadraticBezierSegment(
    const D2D1_POINT_2F &point1, const D2D1_POINT_2F &point2) {
...
D2D1FORCEINLINE D2D1_STROKE_STYLE_PROPERTIES StrokeStyleProperties(
    ...
    D2D1_LINE_JOIN lineJoin = D2D1_LINE_JOIN_MITER, FLOAT miterLimit = 10.0f,
    D2D1_DASH_STYLE dashStyle = D2D1_DASH_STYLE_SOLID,
    FLOAT dashOffset = 0.0f) {
  D2D1_STROKE_STYLE_PROPERTIES r = {startCap,   endCap,    dashCap,   lineJoin,
                                    miterLimit, dashStyle, dashOffset};
  return r;
}
D2D1FORCEINLINE D2D1_BITMAP_BRUSH_PROPERTIES
...
D2D1FORCEINLINE D2D1_LINEAR_GRADIENT_BRUSH_PROPERTIES
LinearGradientBrushProperties(const D2D1_POINT_2F &startPoint,
                              const D2D1_POINT_2F &endPoint) {
  D2D1_LINEAR_GRADIENT_BRUSH_PROPERTIES r = {startPoint, endPoint};
  return r;
}
D2D1FORCEINLINE D2D1_RADIAL_GRADIENT_BRUSH_PROPERTIES
RadialGradientBrushProperties(const D2D1_POINT_2F &center,
                              const D2D1_POINT_2F &gradientOriginOffset,
                              FLOAT radiusX, FLOAT radiusY) {
  D2D1_RADIAL_GRADIENT_BRUSH_PROPERTIES r = {center, gradientOriginOffset,
                                             radiusX, radiusY};
  return r;
}
D2D1FORCEINLINE D2D1_PIXEL_FORMAT
PixelFormat(DXGI_FORMAT dxgiFormat = DXGI_FORMAT_UNKNOWN,
            D2D1_ALPHA_MODE alphaMode = D2D1_ALPHA_MODE_UNKNOWN) {
  D2D1_PIXEL_FORMAT r = {dxgiFormat, alphaMode};
  return r;
}
D2D1FORCEINLINE D2D1_BITMAP_PROPERTIES
...
D2D1FORCEINLINE D2D1_RENDER_TARGET_PROPERTIES RenderTargetProperties(
  ...
  D2D1_RENDER_TARGET_PROPERTIES r = {type, pixelFormat, dpiX,
                                     dpiY, usage,       minLevel};
  return r;
}
D2D1FORCEINLINE D2D1_HWND_RENDER_TARGET_PROPERTIES HwndRenderTargetProperties(
    HWND hwnd,
    D2D1_SIZE_U pixelSize = D2D1::Size(static_cast<UINT>(0),
                                       static_cast<UINT>(0)),
...
D2D1FORCEINLINE D2D1_LAYER_PARAMETERS LayerParameters(
    ...
    FLOAT opacity = 1.0, ID2D1Brush *opacityBrush = NULL,
    D2D1_LAYER_OPTIONS layerOptions = D2D1_LAYER_OPTIONS_NONE) {
  D2D1_LAYER_PARAMETERS r = {contentBounds, geometricMask, maskAntialiasMode,
                             maskTransform, opacity,       opacityBrush,
                             layerOptions};
  return r;
}
D2D1FORCEINLINE D2D1_DRAWING_STATE_DESCRIPTION DrawingStateDescription(
    ...
    const D2D1_MATRIX_3X2_F &transform = D2D1::IdentityMatrix()) {
  D2D1_DRAWING_STATE_DESCRIPTION r = {antialiasMode, textAntialiasMode, tag1,
                                      tag2, transform};
  return r;
}
...
public:
  ...
  FORCEINLINE ColorF(UINT32 rgb, FLOAT _a = 1.0) { init(rgb, _a); }
...
public:
  D2D1FORCEINLINE Matrix3x2F(FLOAT __11, FLOAT __12, FLOAT __21, FLOAT __22,
  ...
  Scale(FLOAT x, FLOAT y, D2D1_POINT_2F center = D2D1::Point2F()) {
    return Matrix3x2F(x, 0.0f, 0.0f, y, center.x - x * center.x,
                      center.y - y * center.y);
  }
  static D2D1FORCEINLINE Matrix3x2F
  ...
  D2D1FORCEINLINE D2D1_POINT_2F TransformPoint(D2D1_POINT_2F point) const {
    return Point2F(point.x * _11 + point.y * _21 + _31,
                   point.x * _12 + point.y * _22 + _32);
  }
};
D2D1FORCEINLINE D2D1_POINT_2F operator*(const D2D1_POINT_2F &point,
...
D2D1FORCEINLINE D2D1_MATRIX_3X2_F operator*(const D2D1_MATRIX_3X2_F &matrix1,
                                            const D2D1_MATRIX_3X2_F &matrix2) {
  D2D1::Matrix3x2F r;
  r.SetProduct(*D2D1::Matrix3x2F::ReinterpretBaseType(&matrix1),
