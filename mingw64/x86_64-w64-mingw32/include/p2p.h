...
typedef HRESULT(CALLBACK *PFNPEER_VALIDATE_RECORD)(
    HGRAPH hGraph, PVOID pvContext, PPEER_RECORD pRecord,
    PEER_RECORD_CHANGE_TYPE changeType);
typedef HRESULT(CALLBACK *PFNPEER_SECURE_RECORD)(
    HGRAPH hGraph, PVOID pvContext, PPEER_RECORD pRecord,
    PEER_RECORD_CHANGE_TYPE changeType, PPEER_DATA *ppSecurityData);
typedef HRESULT(CALLBACK *PFNPEER_FREE_SECURITY_DATA)(HGRAPH hGraph,
                                                      PVOID pvContext,
                                                      PPEER_DATA pSecurityData);
typedef HRESULT(CALLBACK *PFNPEER_ON_PASSWORD_AUTH_FAILED)(HGRAPH hGraph,
                                                           PVOID pvContext);
...
HRESULT WINAPI PeerGraphStartup(WORD wVersionRequested,
...
HRESULT WINAPI PeerGraphGetNextItem(HPEERENUM hPeerEnum, ULONG *pCount,
                                    PVOID **pppvItems);
HRESULT WINAPI PeerGraphEndEnumeration(HPEERENUM hPeerEnum);
HRESULT WINAPI PeerGraphCreate(PPEER_GRAPH_PROPERTIES pGraphProperties,
                               PCWSTR pwzDatabaseName,
                               PPEER_SECURITY_INTERFACE pSecurityInterface,
                               HGRAPH *phGraph);
HRESULT WINAPI PeerGraphOpen(PCWSTR pwzGraphId, PCWSTR pwzPeerId,
                             PCWSTR pwzDatabaseName,
                             PPEER_SECURITY_INTERFACE pSecurityInterface,
                             ULONG cRecordTypeSyncPrecedence,
                             const GUID *pRecordTypeSyncPrecedence,
                             HGRAPH *phGraph);
HRESULT WINAPI PeerGraphListen(HGRAPH hGraph, DWORD dwScope, DWORD dwScopeId,
                               WORD wPort);
HRESULT WINAPI PeerGraphConnect(HGRAPH hGraph, PCWSTR pwzPeerId,
                                PPEER_ADDRESS pAddress,
                                ULONGLONG *pullConnectionId);
HRESULT WINAPI PeerGraphClose(HGRAPH hGraph);
HRESULT WINAPI PeerGraphDelete(PCWSTR pwzGraphId, PCWSTR pwzPeerId,
                               PCWSTR pwzDatabaseName);
HRESULT WINAPI PeerGraphGetStatus(HGRAPH hGraph, DWORD *pdwStatus);
HRESULT WINAPI PeerGraphGetProperties(
    HGRAPH hGraph, PPEER_GRAPH_PROPERTIES *ppGraphProperties);
HRESULT WINAPI PeerGraphSetProperties(HGRAPH hGraph,
                                      PPEER_GRAPH_PROPERTIES pGraphProperties);
HRESULT WINAPI
PeerGraphRegisterEvent(HGRAPH hGraph, HANDLE hEvent, ULONG cEventRegistrations,
...
HRESULT WINAPI PeerGraphGetEventData(HPEEREVENT hPeerEvent,
                                     PPEER_GRAPH_EVENT_DATA *ppEventData);
HRESULT WINAPI PeerGraphGetRecord(HGRAPH hGraph, const GUID *pRecordId,
                                  PPEER_RECORD *ppRecord);
HRESULT WINAPI PeerGraphAddRecord(HGRAPH hGraph, PPEER_RECORD pRecord,
                                  GUID *pRecordId);
HRESULT WINAPI PeerGraphUpdateRecord(HGRAPH hGraph, PPEER_RECORD pRecord);
HRESULT WINAPI PeerGraphDeleteRecord(HGRAPH hGraph, const GUID *pRecordId,
                                     WINBOOL fLocal);
HRESULT WINAPI PeerGraphEnumRecords(HGRAPH hGraph, const GUID *pRecordType,
                                    PCWSTR pwzPeerId, HPEERENUM *phPeerEnum);
HRESULT WINAPI PeerGraphSearchRecords(HGRAPH hGraph, PCWSTR pwzCriteria,
...
HRESULT WINAPI PeerGraphValidateDeferredRecords(HGRAPH hGraph, ULONG cRecordIds,
                                                const GUID *pRecordIds);
HRESULT WINAPI PeerGraphOpenDirectConnection(HGRAPH hGraph, PCWSTR pwzPeerId,
                                             PPEER_ADDRESS pAddress,
                                             ULONGLONG *pullConnectionId);
HRESULT WINAPI PeerGraphSendData(HGRAPH hGraph, ULONGLONG ullConnectionId,
                                 const GUID *pType, ULONG cbData, PVOID pvData);
HRESULT WINAPI PeerGraphCloseDirectConnection(HGRAPH hGraph,
                                              ULONGLONG ullConnectionId);
HRESULT WINAPI PeerGraphEnumConnections(HGRAPH hGraph, DWORD dwFlags,
                                        HPEERENUM *phPeerEnum);
HRESULT WINAPI PeerGraphEnumNodes(HGRAPH hGraph, PCWSTR pwzPeerId,
                                  HPEERENUM *phPeerEnum);
HRESULT WINAPI PeerGraphSetPresence(HGRAPH hGraph, WINBOOL fPresent);
HRESULT WINAPI PeerGraphGetNodeInfo(HGRAPH hGraph, ULONGLONG ullNodeId,
                                    PPEER_NODE_INFO *ppNodeInfo);
HRESULT WINAPI PeerGraphSetNodeAttributes(HGRAPH hGraph, PCWSTR pwzAttributes);
HRESULT WINAPI PeerGraphPeerTimeToUniversalTime(HGRAPH hGraph,
                                                FILETIME *pftPeerTime,
                                                FILETIME *pftUniversalTime);
HRESULT WINAPI PeerGraphUniversalTimeToPeerTime(HGRAPH hGraph,
                                                FILETIME *pftUniversalTime,
                                                FILETIME *pftPeerTime);
#endif
...
HRESULT WINAPI PeerGetNextItem(HPEERENUM hPeerEnum, ULONG *pCount,
                               PVOID **pppvItems);
...
typedef struct peer_group_event_data_tag {
  ...
  [switch_is(eventType)] union {
    ...
    [case (PEER_GROUP_EVENT_RECORD_CHANGED)] PEER_EVENT_RECORD_CHANGE_DATA
        recordChangeData;
    [case (
        PEER_GROUP_EVENT_NEIGHBOR_CONNECTION,
        PEER_GROUP_EVENT_DIRECT_CONNECTION)] PEER_EVENT_CONNECTION_CHANGE_DATA
        connectionChangeData;
    [case (
        PEER_GROUP_EVENT_INCOMING_DATA)] PEER_EVENT_INCOMING_DATA incomingData;
    [case (PEER_GROUP_EVENT_MEMBER_CHANGED)] PEER_EVENT_MEMBER_CHANGE_DATA
        memberChangeData;
    [case (
        PEER_GROUP_EVENT_CONNECTION_FAILED)] HRESULT hrConnectionFailedReason;
    [default];
  };
...
#ifndef __WIDL__
HRESULT WINAPI PeerGroupStartup(WORD wVersionRequested,
                                PPEER_VERSION_DATA pVersionData);
HRESULT WINAPI PeerGroupShutdown();
HRESULT WINAPI PeerGroupCreate(PPEER_GROUP_PROPERTIES pProperties,
                               HGROUP *phGroup);
HRESULT WINAPI PeerGroupOpen(PCWSTR pwzIdentity, PCWSTR pwzGroupPeerName,
                             PCWSTR pwzCloud, HGROUP *phGroup);
HRESULT WINAPI PeerGroupJoin(PCWSTR pwzIdentity, PCWSTR pwzInvitation,
                             PCWSTR pwzCloud, HGROUP *phGroup);
...
HRESULT WINAPI PeerGroupCreateInvitation(HGROUP hGroup, PCWSTR pwzIdentityInfo,
                                         FILETIME *pftExpiration, ULONG cRoles,
                                         const GUID *pRoles,
                                         PWSTR *ppwzInvitation);
HRESULT WINAPI PeerGroupParseInvitation(
    PCWSTR pwzInvitation, PPEER_INVITATION_INFO *ppInvitationInfo);
HRESULT WINAPI PeerGroupGetStatus(HGROUP hGroup, DWORD *pdwStatus);
HRESULT WINAPI PeerGroupGetProperties(HGROUP hGroup,
                                      PPEER_GROUP_PROPERTIES *ppProperties);
HRESULT WINAPI PeerGroupSetProperties(HGROUP hGroup,
                                      PPEER_GROUP_PROPERTIES pProperties);
HRESULT WINAPI PeerGroupEnumMembers(HGROUP hGroup, DWORD dwFlags,
                                    PCWSTR pwzIdentity, HPEERENUM *phPeerEnum);
HRESULT WINAPI PeerGroupOpenDirectConnection(HGROUP hGroup, PCWSTR pwzIdentity,
                                             PPEER_ADDRESS pAddress,
                                             ULONGLONG *pullConnectionId);
HRESULT WINAPI PeerGroupCloseDirectConnection(HGROUP hGroup,
                                              ULONGLONG ullConnectionId);
HRESULT WINAPI PeerGroupEnumConnections(HGROUP hGroup, DWORD dwFlags,
                                        HPEERENUM *phPeerEnum);
HRESULT WINAPI PeerGroupSendData(HGROUP hGroup, ULONGLONG ullConnectionId,
                                 const GUID *pType, ULONG cbData, PVOID pvData);
HRESULT WINAPI
PeerGroupRegisterEvent(HGROUP hGroup, HANDLE hEvent, DWORD cEventRegistration,
                       PEER_GROUP_EVENT_REGISTRATION *pEventRegistrations,
                       HPEEREVENT *phPeerEvent);
HRESULT WINAPI PeerGroupUnregisterEvent(HPEEREVENT hPeerEvent);
HRESULT WINAPI PeerGroupGetEventData(HPEEREVENT hPeerEvent,
                                     PPEER_GROUP_EVENT_DATA *ppEventData);
HRESULT WINAPI PeerGroupGetRecord(HGROUP hGroup, const GUID *pRecordId,
                                  PPEER_RECORD *ppRecord);
HRESULT WINAPI PeerGroupAddRecord(HGROUP hGroup, PPEER_RECORD pRecord,
                                  GUID *pRecordId);
...
HRESULT WINAPI PeerGroupEnumRecords(HGROUP hGroup, const GUID *pRecordType,
                                    HPEERENUM *phPeerEnum);
HRESULT WINAPI PeerGroupSearchRecords(HGROUP hGroup, PCWSTR pwzCriteria,
                                      HPEERENUM *phPeerEnum);
...
HRESULT WINAPI PeerGroupIssueCredentials(HGROUP hGroup,
                                         PCWSTR pwzSubjectIdentity,
                                         PEER_CREDENTIAL_INFO *pCredentialInfo,
                                         DWORD dwFlags, PWSTR *ppwzInvitation);
HRESULT WINAPI PeerGroupExportConfig(HGROUP hGroup, PCWSTR pwzPassword,
                                     PWSTR *ppwzXML);
HRESULT WINAPI PeerGroupImportConfig(PCWSTR pwzXML, PCWSTR pwzPassword,
                                     WINBOOL fOverwrite, PWSTR *ppwzIdentity,
                                     PWSTR *ppwzGroup);
HRESULT WINAPI PeerGroupPeerTimeToUniversalTime(HGROUP hGroup,
                                                FILETIME *pftPeerTime,
                                                FILETIME *pftUniversalTime);
HRESULT WINAPI PeerGroupUniversalTimeToPeerTime(HGROUP hGroup,
                                                FILETIME *pftUniversalTime,
                                                FILETIME *pftPeerTime);
#if NTDDI_VERSION >= 0x06000000
HRESULT WINAPI PeerGroupPasswordJoin(PCWSTR pwzIdentity, PCWSTR pwzInvitation,
                                     PCWSTR pwzPassword, PCWSTR pwzCloud,
                                     HGROUP *phGroup);
HRESULT WINAPI PeerGroupConnectByAddress(HGROUP hGroup, ULONG cAddresses,
                                         PPEER_ADDRESS pAddresses);
HRESULT WINAPI PeerGroupCreatePasswordInvitation(HGROUP hGroup,
                                                 PWSTR *ppwzInvitation);
#endif
#if NTDDI_VERSION >= 0x06010000
HRESULT
    WINAPI PeerGroupResumePasswordAuthentication(HGROUP hGroup,
                                                 HPEEREVENT hPeerEventHandle);
#endif
#endif
#endif
#if !defined(NO_P2P_IDENTITY) && !defined(__WIDL__)
HRESULT WINAPI PeerIdentityCreate(PCWSTR pwzClassifier, PCWSTR pwzFriendlyName,
                                  HCRYPTPROV hCryptProv, PWSTR *ppwzIdentity);
HRESULT WINAPI PeerIdentityGetFriendlyName(PCWSTR pwzIdentity,
                                           PWSTR *ppwzFriendlyName);
HRESULT WINAPI PeerIdentitySetFriendlyName(PCWSTR pwzIdentity,
                                           PCWSTR pwzFriendlyName);
HRESULT WINAPI PeerIdentityGetCryptKey(PCWSTR pwzIdentity,
...
HRESULT WINAPI PeerCreatePeerName(PCWSTR pwzIdentity, PCWSTR pwzClassifier,
                                  PWSTR *ppwzPeerName);
HRESULT WINAPI PeerIdentityGetXML(PCWSTR pwzIdentity, PWSTR *ppwzIdentityXML);
HRESULT WINAPI PeerIdentityExport(PCWSTR pwzIdentity, PCWSTR pwzPassword,
                                  PWSTR *ppwzExportXML);
HRESULT WINAPI PeerIdentityImport(PCWSTR pwzImportXML, PCWSTR pwzPassword,
                                  PWSTR *ppwzIdentity);
...
typedef enum peer_publication_scope_tag {
  ...
  PEER_PUBLICATION_SCOPE_ALL = PEER_PUBLICATION_SCOPE_NEAR_ME |
...
typedef const PEER_APPLICATION_REGISTRATION_INFO
    *PCPEER_APPLICATION_REGISTRATION_INFO;
#ifndef __WIDL__
HRESULT WINAPI PeerCollabAsyncInviteContact(PCPEER_CONTACT pcContact,
                                            PCPEER_ENDPOINT pcEndpoint,
                                            PCPEER_INVITATION pcInvitation,
                                            HANDLE hEvent,
                                            HANDLE *phInvitation);
HRESULT WINAPI PeerCollabGetInvitationResponse(
    HANDLE hInvitation, PPEER_INVITATION_RESPONSE *ppInvitationResponse);
HRESULT WINAPI PeerCollabCancelInvitation(HANDLE hInvitation);
HRESULT WINAPI PeerCollabCloseHandle(HANDLE hInvitation);
HRESULT WINAPI PeerCollabInviteContact(PCPEER_CONTACT pcContact,
                                       PCPEER_ENDPOINT pcEndpoint,
                                       PCPEER_INVITATION pcInvitation,
                                       PPEER_INVITATION_RESPONSE *ppResponse);
HRESULT WINAPI PeerCollabAsyncInviteEndpoint(PCPEER_ENDPOINT pcEndpoint,
                                             PCPEER_INVITATION pcInvitation,
                                             HANDLE hEvent,
                                             HANDLE *phInvitation);
HRESULT WINAPI PeerCollabInviteEndpoint(PCPEER_ENDPOINT pcEndpoint,
                                        PCPEER_INVITATION pcInvitation,
                                        PPEER_INVITATION_RESPONSE *ppResponse);
...
HRESULT WINAPI PeerCollabEnumApplicationRegistrationInfo(
    PEER_APPLICATION_REGISTRATION_TYPE registrationType, HPEERENUM *phPeerEnum);
#endif
...
#ifndef __WIDL__
HRESULT WINAPI PeerCollabGetPresenceInfo(PCPEER_ENDPOINT pcEndpoint,
                                         PPEER_PRESENCE_INFO *ppPresenceInfo);
HRESULT WINAPI PeerCollabEnumApplications(PCPEER_ENDPOINT pcEndpoint,
                                          const GUID *pApplicationId,
                                          HPEERENUM *phPeerEnum);
HRESULT WINAPI PeerCollabEnumObjects(PCPEER_ENDPOINT pcEndpoint,
                                     const GUID *pObjectId,
                                     HPEERENUM *phPeerEnum);
HRESULT WINAPI PeerCollabEnumEndpoints(PCPEER_CONTACT pcContact,
                                       HPEERENUM *phPeerEnum);
...
HRESULT WINAPI PeerCollabQueryContactData(PCPEER_ENDPOINT pcEndpoint,
                                          PWSTR *ppwzContactData);
HRESULT WINAPI
PeerCollabSubscribeEndpointData(const PCPEER_ENDPOINT pcEndpoint);
HRESULT WINAPI
PeerCollabUnsubscribeEndpointData(const PCPEER_ENDPOINT pcEndpoint);
...
typedef struct peer_collab_event_data_tag {
  PEER_COLLAB_EVENT_TYPE eventType;
  [switch_is(eventType)] union {
    [case (PEER_EVENT_WATCHLIST_CHANGED)] PEER_EVENT_WATCHLIST_CHANGED_DATA
        watchListChangedData;
    [case (PEER_EVENT_ENDPOINT_PRESENCE_CHANGED,
           PEER_EVENT_MY_PRESENCE_CHANGED)] PEER_EVENT_PRESENCE_CHANGED_DATA
        presenceChangedData;
    [case (
    ...
    [case (PEER_EVENT_ENDPOINT_OBJECT_CHANGED,
           PEER_EVENT_MY_OBJECT_CHANGED)] PEER_EVENT_OBJECT_CHANGED_DATA
        objectChangedData;
    [case (PEER_EVENT_ENDPOINT_CHANGED,
           PEER_EVENT_MY_ENDPOINT_CHANGED)] PEER_EVENT_ENDPOINT_CHANGED_DATA
        endpointChangedData;
    [case (
        PEER_EVENT_PEOPLE_NEAR_ME_CHANGED)] PEER_EVENT_PEOPLE_NEAR_ME_CHANGED_DATA
        peopleNearMeChangedData;
    [case (
        PEER_EVENT_REQUEST_STATUS_CHANGED)] PEER_EVENT_REQUEST_STATUS_CHANGED_DATA
...
HRESULT WINAPI
PeerCollabRegisterEvent(HANDLE hEvent, DWORD cEventRegistration,
                        PEER_COLLAB_EVENT_REGISTRATION *pEventRegistrations,
                        HPEEREVENT *phPeerEvent);
HRESULT WINAPI PeerCollabGetEventData(HPEEREVENT hPeerEvent,
...
HRESULT WINAPI PeerCollabAddContact(PCWSTR pwzContactData,
                                    PPEER_CONTACT *ppContact);
HRESULT WINAPI PeerCollabDeleteContact(PCWSTR pwzPeerName);
HRESULT WINAPI PeerCollabGetContact(PCWSTR pwzPeerName,
                                    PPEER_CONTACT *ppContact);
HRESULT WINAPI PeerCollabUpdateContact(PCPEER_CONTACT pContact);
HRESULT WINAPI PeerCollabEnumContacts(HPEERENUM *phPeerEnum);
HRESULT WINAPI PeerCollabExportContact(PCWSTR pwzPeerName,
                                       PWSTR *ppwzContactData);
HRESULT WINAPI PeerCollabParseContact(PCWSTR pwzContactData,
                                      PPEER_CONTACT *ppContact);
#endif
#endif
...
HRESULT WINAPI PeerPnrpRegister(PCWSTR pcwzPeerName,
                                PPEER_PNRP_REGISTRATION_INFO pRegistrationInfo,
                                HREGISTRATION *phRegistration);
HRESULT WINAPI
PeerPnrpUpdateRegistration(HREGISTRATION hRegistration,
                           PPEER_PNRP_REGISTRATION_INFO pRegistrationInfo);
HRESULT WINAPI PeerPnrpUnregister(HREGISTRATION hRegistration);
HRESULT WINAPI PeerPnrpResolve(PCWSTR pcwzPeerName, PCWSTR pcwzCloudName,
                               ULONG *pcEndpoints,
                               PPEER_PNRP_ENDPOINT_INFO *ppEndpoints);
HRESULT WINAPI PeerPnrpStartResolve(PCWSTR pcwzPeerName, PCWSTR pcwzCloudName,
                                    ULONG cMaxEndpoints, HANDLE hEvent,
                                    HRESOLUTION *phResolve);
HRESULT WINAPI PeerPnrpGetCloudInfo(ULONG *pcNumClouds,
                                    PPEER_PNRP_CLOUD_INFO *ppCloudInfo);
HRESULT WINAPI PeerPnrpGetEndpoint(HRESOLUTION hResolve,
...
#define PEER_E_CLOUD_NOT_FOUND                                                 \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, WSA_PNRP_CLOUD_NOT_FOUND)
#define PEER_E_CLOUD_DISABLED                                                  \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, WSA_PNRP_CLOUD_DISABLED)
#define PEER_E_INVALID_IDENTITY                                                \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, WSA_PNRP_INVALID_IDENTITY)
#define PEER_E_TOO_MUCH_LOAD                                                   \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, WSA_PNRP_TOO_MUCH_LOAD)
#define PEER_E_CLOUD_IS_SEARCH_ONLY                                            \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, WSA_PNRP_CLOUD_IS_SEARCH_ONLY)
#define PEER_E_CLIENT_INVALID_COMPARTMENT_ID                                   \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32,                                 \
               WSA_PNRP_CLIENT_INVALID_COMPARTMENT_ID)
#define PEER_E_DUPLICATE_PEER_NAME                                             \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, WSA_PNRP_DUPLICATE_PEER_NAME)
#define PEER_E_CLOUD_IS_DEAD                                                   \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, WSA_PNRP_CLOUD_IS_DEAD)
#define PEER_E_NOT_FOUND                                                       \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, ERROR_NOT_FOUND)
#define PEER_E_DISK_FULL                                                       \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, ERROR_DISK_FULL)
#define PEER_E_ALREADY_EXISTS                                                  \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, ERROR_ALREADY_EXISTS)
#endif
#endif
#ifdef DEFINE_GUID
#ifndef NO_P2P_GROUP
DEFINE_GUID(PEER_GROUP_ROLE_ADMIN, 0x04387127, 0xaa56, 0x450a, 0x8c, 0xe5, 0x4f,
            0x56, 0x5c, 0x67, 0x90, 0xf4);
DEFINE_GUID(PEER_GROUP_ROLE_MEMBER, 0xf12dc4c7, 0x0857, 0x4ca0, 0x93, 0xfc,
            0xb1, 0xbb, 0x19, 0xa3, 0xd8, 0xc2);
#if NTDDI_VERSION >= 0x06000000
DEFINE_GUID(PEER_GROUP_ROLE_INVITING_MEMBER, 0x4370fd89, 0xdc18, 0x4cfb, 0x8d,
            0xbf, 0x98, 0x53, 0xa8, 0xa9, 0xf9, 0x05);
#endif
#endif
#ifndef NO_P2P_COLLABORATION
DEFINE_GUID(PEER_COLLAB_OBJECTID_USER_PICTURE, 0xdd15f41f, 0xfc4e, 0x4922, 0xb0,
            0x35, 0x4c, 0x06, 0xa7, 0x54, 0xd0, 0x1d);
#endif
#endif
