 ...
 * #warning Including winnt.h and ntdef.h is deprecated and will be removed in a
 * future release.  Please use winternl.h
 */
#endif
#include <_mingw.h>
#if defined(__x86_64) &&                                                       \
...
#define PROBE_ALIGNMENT(v)                                                     \
  (TYPE_ALIGNMENT(v) > TYPE_ALIGNMENT(ULONG) ? TYPE_ALIGNMENT(v)               \
...
#define DECLARE_CONST_UNICODE_STRING(_var, _string)                            \
  ...
  __pragma(warning(push)) __pragma(warning(disable : 4221))                    \
      __pragma(warning(disable : 4204)) const UNICODE_STRING _var = {          \
          sizeof(_string) - sizeof(WCHAR), sizeof(_string),                    \
          (PWCH)_var##_buffer} __pragma(warning(pop))
#else
#define DECLARE_UNICODE_STRING_SIZE(_var, _size)                               \
  WCHAR _var##_buffer[_size];                                                  \
...
#define DECLARE_CONST_UNICODE_STRING(_var, _string)                            \
  ...
  const UNICODE_STRING _var = {sizeof(_string) - sizeof(WCHAR),                \
                               sizeof(_string), (PWCH)_var##_buffer}
#endif
...
typedef struct _STRING32 {
  ...
  USHORT MaximumLength;
  ULONG Buffer;
} STRING32, *PSTRING32, UNICODE_STRING32, *PUNICODE_STRING32, ANSI_STRING32,
    *PANSI_STRING32;
typedef struct _STRING64 {
  USHORT Length;
  USHORT MaximumLength;
  ULONGLONG Buffer;
} STRING64, *PSTRING64, UNICODE_STRING64, *PUNICODE_STRING64, ANSI_STRING64,
    *PANSI_STRING64;
/* LangID and NLS */
