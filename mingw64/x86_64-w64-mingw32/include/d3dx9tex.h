...
#define D3DX_SKIP_DDS_MIP_LEVELS(l, f)                                         \
  ((((l) & D3DX_SKIP_DDS_MIP_LEVELS_MASK) << D3DX_SKIP_DDS_MIP_LEVELS_SHIFT) | \
   ((f) == D3DX_DEFAULT ? D3DX_FILTER_BOX : (f)))
...
typedef void(WINAPI *LPD3DXFILL2D)(D3DXVECTOR4 *out,
                                   const D3DXVECTOR2 *texcoord,
                                   const D3DXVECTOR2 *texelsize, void *data);
typedef void(WINAPI *LPD3DXFILL3D)(D3DXVECTOR4 *out,
...
#define D3DXGetImageInfoFromResource                                           \
  __MINGW_NAME_AW(D3DXGetImageInfoFromResource)
...
HRESULT WINAPI D3DXLoadSurfaceFromSurface(struct IDirect3DSurface9 *destsurface,
                                          ...
                                          const RECT *srcrect, DWORD filter,
                                          D3DCOLOR colorkey);
HRESULT WINAPI D3DXLoadSurfaceFromMemory(
    IDirect3DSurface9 *dst_surface, const PALETTEENTRY *dst_palette,
...
HRESULT WINAPI D3DXSaveSurfaceToFileInMemory(
    struct ID3DXBuffer **destbuffer, D3DXIMAGE_FILEFORMAT destformat,
    struct IDirect3DSurface9 *srcsurface, const PALETTEENTRY *srcpalette,
    const RECT *srcrect);
...
HRESULT WINAPI D3DXLoadVolumeFromVolume(struct IDirect3DVolume9 *destvolume,
                                        ...
                                        const PALETTEENTRY *srcpalette,
                                        const D3DBOX *srcbox, DWORD filter,
                                        D3DCOLOR colorkey);
HRESULT WINAPI D3DXLoadVolumeFromMemory(
    struct IDirect3DVolume9 *destvolume, const PALETTEENTRY *destpalette,
    const D3DBOX *destbox, const void *srcmemory, D3DFORMAT srcformat,
    UINT srcrowpitch, UINT srcslicepitch, const PALETTEENTRY *srcpalette,
    const D3DBOX *srcbox, DWORD filter, D3DCOLOR colorkey);
HRESULT WINAPI D3DXSaveVolumeToFileInMemory(struct ID3DXBuffer **dst_buffer,
                                            D3DXIMAGE_FILEFORMAT dst_format,
                                            struct IDirect3DVolume9 *src_volume,
...
HRESULT WINAPI D3DXCheckVolumeTextureRequirements(
    struct IDirect3DDevice9 *device, UINT *width, UINT *height, UINT *depth,
    UINT *miplevels, DWORD usage, D3DFORMAT *format, D3DPOOL pool);
HRESULT WINAPI D3DXCreateTexture(struct IDirect3DDevice9 *device, UINT width,
...
HRESULT
    WINAPI D3DXCreateCubeTextureFromFileA(struct IDirect3DDevice9 *device,
                                          const char *srcfile,
                                          struct IDirect3DCubeTexture9 **cube);
...
#define D3DXCreateCubeTextureFromFile                                          \
  __MINGW_NAME_AW(D3DXCreateCubeTextureFromFile)
...
#define D3DXCreateVolumeTextureFromFile                                        \
  __MINGW_NAME_AW(D3DXCreateVolumeTextureFromFile)
HRESULT WINAPI D3DXCreateTextureFromResourceA(
    struct IDirect3DDevice9 *device, HMODULE srcmodule, const char *resource,
    struct IDirect3DTexture9 **texture);
HRESULT WINAPI D3DXCreateTextureFromResourceW(
    struct IDirect3DDevice9 *device, HMODULE srcmodule, const WCHAR *resource,
    struct IDirect3DTexture9 **texture);
#define D3DXCreateTextureFromResource                                          \
  __MINGW_NAME_AW(D3DXCreateTextureFromResource)
...
#define D3DXCreateCubeTextureFromResource                                      \
  __MINGW_NAME_AW(D3DXCreateCubeTextureFromResource)
...
#define D3DXCreateVolumeTextureFromResource                                    \
  __MINGW_NAME_AW(D3DXCreateVolumeTextureFromResource)
HRESULT WINAPI D3DXCreateTextureFromFileExA(
    struct IDirect3DDevice9 *device, const char *srcfile, UINT width,
    UINT height, UINT miplevels, DWORD usage, D3DFORMAT format, D3DPOOL pool,
    DWORD filter, DWORD mipfilter, D3DCOLOR colorkey, D3DXIMAGE_INFO *srcinfo,
    PALETTEENTRY *palette, struct IDirect3DTexture9 **texture);
HRESULT WINAPI D3DXCreateTextureFromFileExW(
    struct IDirect3DDevice9 *device, const WCHAR *srcfile, UINT width,
    UINT height, UINT miplevels, DWORD usage, D3DFORMAT format, D3DPOOL pool,
    DWORD filter, DWORD mipfilter, D3DCOLOR colorkey, D3DXIMAGE_INFO *srcinfo,
    PALETTEENTRY *palette, struct IDirect3DTexture9 **texture);
...
HRESULT WINAPI D3DXCreateCubeTextureFromFileExA(
    struct IDirect3DDevice9 *device, const char *srcfile, UINT size,
    UINT miplevels, DWORD usage, D3DFORMAT format, D3DPOOL pool, DWORD filter,
    DWORD mipfilter, D3DCOLOR colorkey, D3DXIMAGE_INFO *srcinfo,
    PALETTEENTRY *palette, struct IDirect3DCubeTexture9 **cube);
HRESULT WINAPI D3DXCreateCubeTextureFromFileExW(
    struct IDirect3DDevice9 *device, const WCHAR *srcfile, UINT size,
    UINT miplevels, DWORD usage, D3DFORMAT format, D3DPOOL pool, DWORD filter,
    DWORD mipfilter, D3DCOLOR colorkey, D3DXIMAGE_INFO *srcinfo,
    PALETTEENTRY *palette, struct IDirect3DCubeTexture9 **cube);
#define D3DXCreateCubeTextureFromFileEx                                        \
  __MINGW_NAME_AW(D3DXCreateCubeTextureFromFileEx)
HRESULT WINAPI D3DXCreateVolumeTextureFromFileExA(
    ...
    D3DXIMAGE_INFO *srcinfo, PALETTEENTRY *palette,
    struct IDirect3DVolumeTexture9 **volume);
...
#define D3DXCreateVolumeTextureFromFileEx                                      \
  __MINGW_NAME_AW(D3DXCreateVolumeTextureFromFileEx)
...
#define D3DXCreateTextureFromResourceEx                                        \
  __MINGW_NAME_AW(D3DXCreateTextureFromResourceEx)
...
#define D3DXCreateCubeTextureFromResourceEx                                    \
  __MINGW_NAME_AW(D3DXCreateCubeTextureFromResourceEx)
HRESULT WINAPI D3DXCreateVolumeTextureFromResourceExA(
    ...
    D3DCOLOR colorkey, D3DXIMAGE_INFO *srcinfo, PALETTEENTRY *palette,
    struct IDirect3DVolumeTexture9 **volume);
...
#define D3DXCreateVolumeTextureFromResourceEx                                  \
  __MINGW_NAME_AW(D3DXCreateVolumeTextureFromResourceEx)
...
HRESULT WINAPI D3DXComputeNormalMap(IDirect3DTexture9 *texture,
                                    ...
                                    const PALETTEENTRY *srcpalette, DWORD flags,
