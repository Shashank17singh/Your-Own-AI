...
#define STATIC_KoCreateObject                                                  \
  0x72CF721C, 0x525A, 0x11D1, 0x9A, 0xA1, 0x00, 0xA0, 0xC9, 0x22, 0x31, 0x96
...
#define STATIC_IID_IKoInitializeParentDeviceObject                             \
  0x21B36996, 0x8DE3, 0x11D1, 0x8A, 0xE0, 0x00, 0xA0, 0xC9, 0x22, 0x31, 0x96
...
#endif
typedef NTSTATUS (*KoCreateObjectHandler)(REFCLSID ClassId, IUnknown *UnkOuter,
                                          REFIID InterfaceId, PVOID *Interface);
#undef INTERFACE
#define INTERFACE INonDelegatedUnknown
DECLARE_INTERFACE(INonDelegatedUnknown) {
  STDMETHOD(NonDelegatedQueryInterface)(THIS_ REFIID InterfaceId,
...
DECLARE_INTERFACE(IIndirectedUnknown) {
  STDMETHOD(IndirectedQueryInterface)(THIS_ REFIID InterfaceId,
                                      PVOID * Interface) PURE;
...
DECLARE_INTERFACE_(IKoInitializeParentDeviceObject, IUnknown) {
  STDMETHOD(SetParentDeviceObject)(THIS_ PDEVICE_OBJECT ParentDeviceObject)
      PURE;
};
#if defined(__cplusplus)
...
public:
  ...
  COMDDKMETHOD STDMETHODIMP NonDelegatedQueryInterface(REFIID InterfaceId,
                                                       PVOID *Interface);
  COMDDKMETHOD STDMETHODIMP_(ULONG) IndirectedAddRef();
  COMDDKMETHOD STDMETHODIMP_(ULONG) IndirectedRelease();
  COMDDKMETHOD STDMETHODIMP IndirectedQueryInterface(REFIID InterfaceId,
                                                     PVOID *Interface);
};
...
#define DEFINE_STD_UNKNOWN()                                                   \
  STDMETHODIMP NonDelegatedQueryInterface(REFIID InterfaceId,                  \
                                          PVOID *Interface);                   \
...
#define IMPLEMENT_STD_UNKNOWN(Class)                                           \
  ...
  STDMETHODIMP_(ULONG) Class::AddRef() { return m_UnknownOuter->AddRef(); }    \
  STDMETHODIMP_(ULONG) Class::Release() { return m_UnknownOuter->Release(); }
#else
COMDDKAPI
void NTAPI KoRelease(REFCLSID ClassId);
#endif /* !__cplusplus */
COMDDKAPI
NTSTATUS
NTAPI
KoCreateInstance(REFCLSID ClassId, IUnknown *UnkOuter, ULONG ClsContext,
                 REFIID InterfaceId, PVOID *Interface);
COMDDKAPI
NTSTATUS
NTAPI
KoDeviceInitialize(PDEVICE_OBJECT DeviceObject);
COMDDKAPI
NTSTATUS
NTAPI
KoDriverInitialize(PDRIVER_OBJECT DriverObject,
