...
/* This is needed for _COM_SMARTPTR_TYPEDEF using emulated __uuidof. Since we
 * can't pass IID as a template argument, it's passed as a wrapper function. */
...
public:
  ...
  _com_ptr_t(const _com_ptr_t<_OtherIID> &p) : m_pInterface(NULL) {
  ...
  _com_ptr_t(_InterfaceType *p) : m_pInterface(NULL) {
    HRESULT hr = _QueryInterface(p);
    if (FAILED(hr) && (hr != E_NOINTERFACE)) {
      _com_issue_error(hr);
    }
  ...
  _com_ptr_t(Interface *pInterface) throw() : m_pInterface(pInterface) {
    _AddRef();
  }
  ...
  HRESULT CreateInstance(const CLSID &rclsid, IUnknown *pOuter = NULL,
    ...
    if (dwClsContext & (CLSCTX_LOCAL_SERVER | CLSCTX_REMOTE_SERVER)) {
      ...
      hr = CoCreateInstance(rclsid, pOuter, dwClsContext, GetIID(),
                            reinterpret_cast<void **>(&m_pInterface));
    if (FAILED(hr))
      m_pInterface = NULL;
    return hr;
  }
  HRESULT CreateInstance(LPCWSTR clsidString, IUnknown *pOuter = NULL,
                         DWORD dwClsContext = CLSCTX_ALL) throw() {
    if (!clsidString)
      return E_INVALIDARG;
    CLSID clsid;
  ...
  HRESULT CreateInstance(LPCSTR clsidStringA, IUnknown *pOuter = NULL,
    ...
    if (MultiByteToWideChar(CP_ACP, 0, clsidStringA, size, clsidStringW,
                            destSize) == 0) {
      _freea(clsidStringW);
      return HRESULT_FROM_WIN32(GetLastError());
    }
...
private:
  ...
  template <typename _InterfacePtr> int _CompareUnknown(_InterfacePtr p) {
    ...
    if (m_pInterface != NULL) {
      ...
      pu1 = NULL;
    if (p != NULL) {
      HRESULT hr = p->QueryInterface(__uuidof(IUnknown),
  ...
  HRESULT QueryStdInterfaces(const _variant_t &varSrc) throw() {
    ...
    HRESULT hr = VariantChangeType(
        &varDest, const_cast<VARIANT *>(static_cast<const VARIANT *>(&varSrc)),
        0, VT_DISPATCH);
    if (SUCCEEDED(hr))
    ...
    if (hr == E_NOINTERFACE) {
      ...
      hr = VariantChangeType(
          &varDest,
          const_cast<VARIANT *>(static_cast<const VARIANT *>(&varSrc)), 0,
          VT_UNKNOWN);
      if (SUCCEEDED(hr))
...
template <typename _InterfaceType>
bool operator==(int null, _com_ptr_t<_InterfaceType> &p) {
  if (null != 0) {
    _com_issue_error(E_POINTER);
  }
  return !p;
