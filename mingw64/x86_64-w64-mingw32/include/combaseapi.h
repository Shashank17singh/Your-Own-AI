...
#define STDMETHOD(method)                                                      \
  virtual COM_DECLSPEC_NOTHROW HRESULT STDMETHODCALLTYPE method
#define STDMETHOD_(type, method)                                               \
  virtual COM_DECLSPEC_NOTHROW type STDMETHODCALLTYPE method
#define STDMETHODV(method)                                                     \
  virtual COM_DECLSPEC_NOTHROW HRESULT STDMETHODVCALLTYPE method
#define STDMETHODV_(type, method)                                              \
  virtual COM_DECLSPEC_NOTHROW type STDMETHODVCALLTYPE method
#define PURE = 0
#define THIS_
#define THIS void
...
typedef enum tagCOINITBASE { COINITBASE_MULTITHREADED = 0x0 } COINITBASE;
#include <guiddef.h>
...
#endif
WINOLEAPI CreateStreamOnHGlobal(HGLOBAL hGlobal, WINBOOL fDeleteOnRelease,
                                LPSTREAM *ppstm);
...
WINOLEAPI CoGetApartmentType(APTTYPE *pAptType,
...
#if NTDDI_VERSION >= 0x06020000
WINOLEAPI CoDecodeProxy(DWORD dwClientPid, UINT64 ui64ProxyAddress,
                        PServerInformation pServerInformation);
WINOLEAPI CoWaitForMultipleObjects(DWORD dwFlags, DWORD dwTimeout,
                                   ULONG cHandles, const HANDLE *pHandles,
                                   LPDWORD lpdwindex);
WINOLEAPI CoAllowUnmarshalerCLSID(REFCLSID clsid);
#endif
WINOLEAPI CoGetClassObject(REFCLSID rclsid, DWORD dwClsContext,
...
WINOLEAPI CoLockObjectExternal(LPUNKNOWN pUnk, WINBOOL fLock,
                               WINBOOL fLastUnlockReleases);
...
WINOLEAPI CoGetStandardMarshal(REFIID riid, LPUNKNOWN pUnk, DWORD dwDestContext,
...
WINOLEAPI CoInitializeSecurity(PSECURITY_DESCRIPTOR pSecDesc, LONG cAuthSvc,
...
WINOLEAPI CoCreateInstanceFromApp(REFCLSID Clsid, IUnknown *punkOuter,
                                  DWORD dwClsCtx, PVOID reserved, DWORD dwCount,
                                  MULTI_QI *pResults);
#endif
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
...
WINOLEAPI CoQueryProxyBlanket(IUnknown *pProxy, DWORD *pwAuthnSvc,
                              DWORD *pAuthzSvc, LPOLESTR *pServerPrincName,
                              DWORD *pAuthnLevel, DWORD *pImpLevel,
                              RPC_AUTH_IDENTITY_HANDLE *pAuthInfo,
                              DWORD *pCapabilites);
WINOLEAPI CoSetProxyBlanket(IUnknown *pProxy, DWORD dwAuthnSvc,
...
WINOLEAPI CoQueryClientBlanket(DWORD *pAuthnSvc, DWORD *pAuthzSvc,
                               LPOLESTR *pServerPrincName, DWORD *pAuthnLevel,
                               DWORD *pImpLevel, RPC_AUTHZ_HANDLE *pPrivs,
                               DWORD *pCapabilities);
WINOLEAPI CoImpersonateClient(void);
WINOLEAPI CoRevertToSelf(void);
WINOLEAPI
CoQueryAuthenticationServices(DWORD *pcAuthSvc,
                              SOLE_AUTHENTICATION_SERVICE **asAuthSvc);
...
__forceinline HRESULT CoCreateInstance(REFCLSID rclsid, LPUNKNOWN pUnkOuter,
...
#endif
  hr =
      CoCreateInstanceFromApp(rclsid, pUnkOuter, dwClsContext, NULL, 1, &OneQI);
  *ppv = OneQI.pItf;
  return FAILED(hr) ? hr : OneQI.hr;
}
__forceinline HRESULT CoCreateInstanceEx(REFCLSID Clsid, IUnknown *punkOuter,
                                         DWORD dwClsCtx,
...
#endif
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
WINOLEAPI CoWaitForMultipleHandles(DWORD dwFlags, DWORD dwTimeout,
                                   ULONG cHandles, LPHANDLE pHandles,
                                   LPDWORD lpdwindex);
#endif
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP)
...
WINOLEAPI RoGetAgileReference(enum AgileReferenceOptions options, REFIID riid,
                              IUnknown *pUnk,
