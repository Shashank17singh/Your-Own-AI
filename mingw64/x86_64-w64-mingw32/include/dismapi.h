...
typedef void(CALLBACK *DISM_PROGRESS_CALLBACK)(UINT Current, UINT Total,
                                               PVOID UserData);
...
typedef enum _DismMountMode { DismReadWrite = 0, DismReadOnly } DismMountMode;
 ...
 */
HRESULT WINAPI DismInitialize(DismLogLevel LogLevel, PCWSTR LogFilePath,
                              PCWSTR ScratchDirectory);
HRESULT WINAPI DismShutdown(void);
HRESULT WINAPI DismMountImage(PCWSTR ImageFilePath, PCWSTR MountPath,
                              UINT ImageIndex, PCWSTR ImageName,
                              DismImageIdentifier ImageIdentifier, DWORD Flags,
                              HANDLE CancelEvent,
                              DISM_PROGRESS_CALLBACK Progress, PVOID UserData);
HRESULT WINAPI DismUnmountImage(PCWSTR MountPath, DWORD Flags,
                                HANDLE CancelEvent,
                                DISM_PROGRESS_CALLBACK Progress,
                                PVOID UserData);
HRESULT WINAPI DismOpenSession(PCWSTR ImagePath, PCWSTR WindowsDirectory,
                               PCWSTR SystemDrive, DismSession *Session);
...
HRESULT WINAPI DismCommitImage(DismSession Session, DWORD Flags,
                               HANDLE CancelEvent,
                               DISM_PROGRESS_CALLBACK Progress, PVOID UserData);
HRESULT WINAPI DismGetImageInfo(PCWSTR ImageFilePath, DismImageInfo **ImageInfo,
                                UINT *Count);
HRESULT WINAPI DismGetMountedImageInfo(DismMountedImageInfo **MountedImageInfo,
                                       UINT *Count);
HRESULT WINAPI DismCleanupMountpoints(void);
HRESULT WINAPI DismCheckImageHealth(DismSession Session, WINBOOL ScanImage,
                                    HANDLE CancelEvent,
                                    DISM_PROGRESS_CALLBACK Progress,
                                    PVOID UserData,
                                    DismImageHealthState *ImageHealth);
HRESULT WINAPI DismRestoreImageHealth(DismSession Session, PCWSTR *SourcePaths,
                                      UINT SourcePathCount, WINBOOL LimitAccess,
                                      HANDLE CancelEvent,
                                      DISM_PROGRESS_CALLBACK Progress,
                                      PVOID UserData);
HRESULT WINAPI DismDelete(VOID *DismStructure);
HRESULT WINAPI DismAddPackage(DismSession Session, PCWSTR PackagePath,
                              WINBOOL IgnoreCheck, WINBOOL PreventPending,
                              HANDLE CancelEvent,
                              DISM_PROGRESS_CALLBACK Progress, PVOID UserData);
HRESULT WINAPI DismRemovePackage(DismSession Session, PCWSTR Identifier,
                                 DismPackageIdentifier PackageIdentifier,
                                 HANDLE CancelEvent,
                                 DISM_PROGRESS_CALLBACK Progress,
                                 PVOID UserData);
HRESULT WINAPI DismEnableFeature(
    DismSession Session, PCWSTR FeatureName, PCWSTR Identifier,
    DismPackageIdentifier PackageIdentifier, WINBOOL LimitAccess,
    PCWSTR *SourcePaths, UINT SourcePathCount, WINBOOL EnableAll,
    HANDLE CancelEvent, DISM_PROGRESS_CALLBACK Progress, PVOID UserData);
HRESULT WINAPI DismDisableFeature(DismSession Session, PCWSTR FeatureName,
                                  PCWSTR PackageName, WINBOOL RemovePayload,
                                  HANDLE CancelEvent,
                                  DISM_PROGRESS_CALLBACK Progress,
                                  PVOID UserData);
HRESULT WINAPI DismGetPackages(DismSession Session, DismPackage **Package,
                               UINT *Count);
HRESULT WINAPI DismGetPackageInfo(DismSession Session, PCWSTR Identifier,
                                  DismPackageIdentifier PackageIdentifier,
                                  DismPackageInfo **PackageInfo);
HRESULT WINAPI DismGetPackageInfoEx(DismSession Session, PCWSTR Identifier,
                                    DismPackageIdentifier PackageIdentifier,
                                    DismPackageInfoEx **PackageInfoEx);
HRESULT WINAPI DismGetFeatures(DismSession Session, PCWSTR Identifier,
                               DismPackageIdentifier PackageIdentifier,
                               DismFeature **Feature, UINT *Count);
HRESULT WINAPI DismGetFeatureInfo(DismSession Session, PCWSTR FeatureName,
                                  PCWSTR Identifier,
                                  DismPackageIdentifier PackageIdentifier,
                                  DismFeatureInfo **FeatureInfo);
HRESULT WINAPI DismGetFeatureParent(DismSession Session, PCWSTR FeatureName,
                                    PCWSTR Identifier,
                                    DismPackageIdentifier PackageIdentifier,
                                    DismFeature **Feature, UINT *Count);
HRESULT WINAPI DismApplyUnattend(DismSession Session, PCWSTR UnattendFile,
                                 WINBOOL SingleSession);
HRESULT WINAPI DismAddDriver(DismSession Session, PCWSTR DriverPath,
                             WINBOOL ForceUnsigned);
HRESULT WINAPI DismRemoveDriver(DismSession Session, PCWSTR DriverPath);
HRESULT WINAPI DismGetDrivers(DismSession Session, WINBOOL AllDrivers,
                              DismDriverPackage **DriverPackage, UINT *Count);
HRESULT WINAPI DismGetDriverInfo(DismSession Session, PCWSTR DriverPath,
                                 DismDriver **Driver, UINT *Count,
                                 DismDriverPackage **DriverPackage);
HRESULT WINAPI DismGetCapabilities(DismSession Session,
                                   DismCapability **Capability, UINT *Count);
HRESULT WINAPI DismGetCapabilityInfo(DismSession Session, PCWSTR Name,
                                     DismCapabilityInfo **Info);
HRESULT WINAPI DismAddCapability(DismSession Session, PCWSTR Name,
                                 WINBOOL LimitAccess, PCWSTR *SourcePaths,
                                 UINT SourcePathCount, HANDLE CancelEvent,
                                 DISM_PROGRESS_CALLBACK Progress,
                                 PVOID UserData);
HRESULT WINAPI DismRemoveCapability(DismSession Session, PCWSTR Name,
                                    HANDLE CancelEvent,
                                    DISM_PROGRESS_CALLBACK Progress,
                                    PVOID UserData);
