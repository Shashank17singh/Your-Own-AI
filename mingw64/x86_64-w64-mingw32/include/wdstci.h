...
#endif
/* Wdstptc.dll is missing an implib because Vista clients don't have the dll to
 * generate it from */
typedef VOID(CALLBACK *PFN_WdsTransportClientReceiveContents)(
    HANDLE hSessionKey, PVOID pCallerData, PVOID pMetadata, ULONG ulSize,
...
typedef VOID(CALLBACK *PFN_WdsTransportClientSessionComplete)(
    HANDLE hSessionKey, PVOID pCallerData, DWORD dwError);
typedef VOID(CALLBACK *PFN_WdsTransportClientSessionStart)(
    HANDLE hSessionKey, PVOID pCallerData, PULARGE_INTEGER FileSize);
typedef VOID(CALLBACK *PFN_WdsTransportClientSessionStartEx)(
    HANDLE hSessionKey, PVOID pCallerData, PTRANSPORTCLIENT_SESSION_INFO Info);
...
DWORD WDSTCIAPI WdsTransportClientCompleteReceive(HANDLE hSessionKey,
                                                  HANDLE ulSize,
...
DWORD WDSTCIAPI WdsTransportClientInitializeSession(
    PWDS_TRANSPORTCLIENT_REQUEST pSessionRequest, PVOID pCallerData,
    PHANDLE hSessionKey);
DWORD WDSTCIAPI WdsTransportClientQueryStatus(HANDLE hSessionKey,
                                              PULONG puStatus,
                                              PULONG puErrorCode);
DWORD WDSTCIAPI WdsTransportClientRegisterCallback(
    HANDLE hSessionKey, TRANSPORTCLIENT_CALLBACK_ID CallbackId,
    PVOID pfnCallback);
DWORD WDSTCIAPI WdsTransportClientReleaseBuffer(PVOID pvBuffer);
DWORD WDSTCIAPI WdsTransportClientShutdown(void);
DWORD WDSTCIAPI WdsTransportClientWaitForCompletion(HANDLE hSessionKey,
                                                    ULONG uTimeout);
#ifdef __cplusplus
}
#endif
