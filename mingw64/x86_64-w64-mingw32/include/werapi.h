...
HRESULT WINAPI WerRegisterFile(PCWSTR pwzFile,
                               WER_REGISTER_FILE_TYPE regFileType,
                               DWORD dwFlags);
HRESULT WINAPI WerRegisterMemoryBlock(PVOID pvAddress, DWORD dwSize);
HRESULT WINAPI WerRemoveExcludedApplication(PCWSTR pwzExeName,
                                            WINBOOL bAllUsers);
HRESULT WINAPI WerReportAddDump(HREPORT hReportHandle, HANDLE hProcess,
                                HANDLE hThread, WER_DUMP_TYPE dumpType,
                                PWER_EXCEPTION_INFORMATION pExceptionParam,
                                PWER_DUMP_CUSTOM_OPTIONS pDumpCustomOptions,
                                DWORD dwFlags);
HRESULT WINAPI WerReportAddFile(HREPORT hReportHandle, PCWSTR pwzPath,
                                WER_FILE_TYPE repFileType, DWORD dwFileFlags);
HRESULT WINAPI WerReportCloseHandle(HREPORT hReportHandle);
HRESULT WINAPI WerReportCreate(PCWSTR pwzEventType, WER_REPORT_TYPE repType,
                               PWER_REPORT_INFORMATION pReportInformation,
                               HREPORT *phReportHandle);
HRESULT WINAPI WerReportHang(HWND hwndHungWindow,
                             PCWSTR wszHungApplicationName);
HRESULT WINAPI WerReportSetParameter(HREPORT hReportHandle, DWORD dwparamID,
                                     PCWSTR pwzName, PCWSTR pwzValue);
HRESULT WINAPI WerReportSetUIOption(HREPORT hReportHandle,
                                    WER_REPORT_UI repUITypeID, PCWSTR pwzValue);
HRESULT WINAPI WerReportSubmit(HREPORT hReportHandle, WER_CONSENT consent,
...
HRESULT WINAPI WerRegisterAdditionalProcess(DWORD processId,
                                            DWORD captureExtraInfoForThreadId);
...
typedef HRESULT(WINAPI *PFN_WER_RUNTIME_EXCEPTION_EVENT)(
    ...
    WINBOOL *pbOwnershipClaimed, PWSTR pwszEventName, PDWORD pchSize,
    PDWORD pdwSignatureCount);
typedef HRESULT(WINAPI *PFN_WER_RUNTIME_EXCEPTION_DEBUGGER_LAUNCH)(
    PVOID pContext,
    const PWER_RUNTIME_EXCEPTION_INFORMATION pExceptionInformation,
    PBOOL pbIsCustomDebugger, PWSTR pwszDebuggerLaunch,
    PDWORD pchDebuggerLaunch, PBOOL pbIsDebuggerAutolaunch);
typedef HRESULT(WINAPI *PFN_WER_RUNTIME_EXCEPTION_EVENT_SIGNATURE)(
    PVOID pContext,
    const PWER_RUNTIME_EXCEPTION_INFORMATION pExceptionInformation,
    DWORD dwIndex, PWSTR pwszName, PDWORD pchName, PWSTR pwszValue,
    PDWORD pchValue);
HRESULT WINAPI WerRegisterRuntimeExceptionModule(
    PCWSTR pwszOutOfProcessCallbackDll, PVOID pContext);
...
HRESULT WINAPI WerStoreOpen(REPORT_STORE_TYPES repStoreType,
                            PHREPORTSTORE phReportStore);
VOID WINAPI WerStoreClose(HREPORTSTORE hReportStore);
HRESULT WINAPI WerStoreGetFirstReportKey(HREPORTSTORE hReportStore,
                                         PCWSTR *ppszReportKey);
HRESULT WINAPI WerStoreGetNextReportKey(HREPORTSTORE hReportStore,
                                        PCWSTR *ppszReportKey);
HRESULT WINAPI
WerStoreQueryReportMetadataV1(HREPORTSTORE hReportStore, PCWSTR pszReportKey,
                              PWER_REPORT_METADATA_V1 pReportMetadata);
HRESULT WINAPI
WerStoreQueryReportMetadataV2(HREPORTSTORE hReportStore, PCWSTR pszReportKey,
                              PWER_REPORT_METADATA_V2 pReportMetadata);
HRESULT WINAPI
WerStoreQueryReportMetadataV3(HREPORTSTORE hReportStore, PCWSTR pszReportKey,
                              PWER_REPORT_METADATA_V3 pReportMetadata);
VOID WINAPI WerFreeString(PCWSTR pwszStr);
HRESULT WINAPI WerStorePurge(void);
HRESULT WINAPI WerStoreGetReportCount(HREPORTSTORE hReportStore,
                                      DWORD *pdwReportCount);
HRESULT WINAPI WerStoreGetSizeOnDisk(HREPORTSTORE hReportStore,
                                     ULONGLONG *pqwSizeInBytes);
HRESULT WINAPI WerStoreUploadReport(HREPORTSTORE hReportStore,
                                    PCWSTR pszReportKey, DWORD dwFlags,
