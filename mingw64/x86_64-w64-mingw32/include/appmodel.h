...
WINBASEAPI LONG WINAPI GetCurrentPackageFullName(UINT32 *packageFullNameLength,
                                                 PWSTR packageFullName);
WINBASEAPI LONG WINAPI GetCurrentPackageFamilyName(
    UINT32 *packageFamilyNameLength, PWSTR packageFamilyName);
WINBASEAPI LONG WINAPI GetCurrentPackagePath(UINT32 *pathLength, PWSTR path);
WINBASEAPI LONG WINAPI GetPackageId(HANDLE hProcess, UINT32 *bufferLength,
                                    BYTE *buffer);
WINBASEAPI LONG WINAPI GetPackageFullName(HANDLE hProcess,
                                          UINT32 *packageFullNameLength,
                                          PWSTR packageFullName);
WINBASEAPI LONG WINAPI GetPackageFullNameFromToken(
    HANDLE token, UINT32 *packageFullNameLength, PWSTR packageFullName);
WINBASEAPI LONG WINAPI GetPackageFamilyName(HANDLE hProcess,
                                            UINT32 *packageFamilyNameLength,
                                            PWSTR packageFamilyName);
WINBASEAPI LONG WINAPI GetPackageFamilyNameFromToken(
    HANDLE token, UINT32 *packageFamilyNameLength, PWSTR packageFamilyName);
WINBASEAPI LONG WINAPI GetPackagePath(const PACKAGE_ID *packageId,
                                      const UINT32 reserved, UINT32 *pathLength,
                                      PWSTR path);
WINBASEAPI LONG WINAPI GetPackagePathByFullName(PCWSTR packageFullName,
                                                UINT32 *pathLength, PWSTR path);
WINBASEAPI LONG WINAPI GetStagedPackagePathByFullName(PCWSTR packageFullName,
                                                      UINT32 *pathLength,
                                                      PWSTR path);
...
WINBASEAPI LONG WINAPI GetStagedPackagePathByFullName2(
    PCWSTR packageFullName, PackagePathType packagePathType, UINT32 *pathLength,
    PWSTR path);
WINBASEAPI LONG WINAPI GetCurrentPackageInfo2(const UINT32 flags,
                                              PackagePathType packagePathType,
                                              UINT32 *bufferLength,
                                              BYTE *buffer, UINT32 *count);
WINBASEAPI LONG WINAPI GetCurrentPackagePath2(PackagePathType packagePathType,
                                              UINT32 *pathLength, PWSTR path);
...
GetApplicationUserModelId(HANDLE hProcess, UINT32 *applicationUserModelIdLength,
                          PWSTR applicationUserModelId);
WINBASEAPI LONG WINAPI GetApplicationUserModelIdFromToken(
    HANDLE token, UINT32 *applicationUserModelIdLength,
    PWSTR applicationUserModelId);
...
WINBASEAPI LONG WINAPI
VerifyApplicationUserModelId(PCWSTR applicationUserModelId);
WINBASEAPI LONG WINAPI
VerifyPackageRelativeApplicationId(PCWSTR packageRelativeApplicationId);
WINBASEAPI LONG WINAPI PackageIdFromFullName(PCWSTR packageFullName,
                                             const UINT32 flags,
                                             UINT32 *bufferLength,
                                             BYTE *buffer);
WINBASEAPI LONG WINAPI PackageFullNameFromId(const PACKAGE_ID *packageId,
                                             UINT32 *packageFullNameLength,
                                             PWSTR packageFullName);
WINBASEAPI LONG WINAPI PackageFamilyNameFromId(const PACKAGE_ID *packageId,
                                               UINT32 *packageFamilyNameLength,
                                               PWSTR packageFamilyName);
WINBASEAPI LONG WINAPI PackageFamilyNameFromFullName(
    PCWSTR packageFullName, UINT32 *packageFamilyNameLength,
    PWSTR packageFamilyName);
WINBASEAPI LONG WINAPI PackageNameAndPublisherIdFromFamilyName(
    PCWSTR packageFamilyName, UINT32 *packageNameLength, PWSTR packageName,
    UINT32 *packagePublisherIdLength, PWSTR packagePublisherId);
WINBASEAPI LONG WINAPI FormatApplicationUserModelId(
    PCWSTR packageFamilyName, PCWSTR packageRelativeApplicationId,
    UINT32 *applicationUserModelIdLength, PWSTR applicationUserModelId);
WINBASEAPI LONG WINAPI ParseApplicationUserModelId(
    PCWSTR applicationUserModelId, UINT32 *packageFamilyNameLength,
    PWSTR packageFamilyName, UINT32 *packageRelativeApplicationIdLength,
    PWSTR packageRelativeApplicationId);
WINBASEAPI LONG WINAPI GetPackagesByPackageFamily(PCWSTR packageFamilyName,
                                                  UINT32 *count,
                                                  PWSTR *packageFullNames,
                                                  UINT32 *bufferLength,
                                                  WCHAR *buffer);
WINBASEAPI LONG WINAPI FindPackagesByPackageFamily(
    PCWSTR packageFamilyName, UINT32 packageFilters, UINT32 *count,
    PWSTR *packageFullNames, UINT32 *bufferLength, WCHAR *buffer,
...
WINBASEAPI LONG WINAPI GetStagedPackageOrigin(PCWSTR packageFullName,
...
#include <poppack.h>
WINBASEAPI LONG WINAPI GetCurrentPackageInfo(const UINT32 flags,
                                             UINT32 *bufferLength, BYTE *buffer,
                                             UINT32 *count);
WINBASEAPI LONG WINAPI
OpenPackageInfoByFullName(PCWSTR packageFullName, const UINT32 reserved,
                          PACKAGE_INFO_REFERENCE *packageInfoReference);
WINBASEAPI LONG WINAPI OpenPackageInfoByFullNameForUser(
    PSID userSid, PCWSTR packageFullName, const UINT32 reserved,
    PACKAGE_INFO_REFERENCE *packageInfoReference);
WINBASEAPI LONG WINAPI
ClosePackageInfo(PACKAGE_INFO_REFERENCE packageInfoReference);
WINBASEAPI LONG WINAPI
GetPackageInfo(PACKAGE_INFO_REFERENCE packageInfoReference, const UINT32 flags,
               UINT32 *bufferLength, BYTE *buffer, UINT32 *count);
WINBASEAPI LONG WINAPI
GetPackageApplicationIds(PACKAGE_INFO_REFERENCE packageInfoReference,
                         UINT32 *bufferLength, BYTE *buffer, UINT32 *count);
#if NTDDI_VERSION >= NTDDI_WIN10_19H1
WINBASEAPI LONG WINAPI
GetPackageInfo2(PACKAGE_INFO_REFERENCE packageInfoReference, const UINT32 flags,
                PackagePathType packagePathType, UINT32 *bufferLength,
                BYTE *buffer, UINT32 *count);
#endif
WINBASEAPI HRESULT WINAPI CheckIsMSIXPackage(PCWSTR packageFullName,
                                             WINBOOL *isMSIXPackage);
...
WINBASEAPI HRESULT WINAPI TryCreatePackageDependency(
    PSID user, PCWSTR packageFamilyName, PACKAGE_VERSION minVersion,
    PackageDependencyProcessorArchitectures
        packageDependencyProcessorArchitectures,
    PackageDependencyLifetimeKind lifetimeKind, PCWSTR lifetimeArtifact,
...
RemovePackageDependency(PACKAGEDEPENDENCY_CONTEXT packageDependencyContext);
WINBASEAPI HRESULT WINAPI GetResolvedPackageFullNameForPackageDependency(
    PCWSTR packageDependencyId, PWSTR *packageFullName);
...
WINBASEAPI LONG WINAPI AppPolicyGetLifecycleManagement(
    HANDLE processToken, AppPolicyLifecycleManagement *policy);
...
WINBASEAPI LONG WINAPI AppPolicyGetClrCompat(HANDLE processToken,
...
WINBASEAPI LONG WINAPI AppPolicyGetShowDeveloperDiagnostic(
    HANDLE processToken, AppPolicyShowDeveloperDiagnostic *policy);
...
WINBASEAPI LONG WINAPI AppPolicyGetCreateFileAccess(
    HANDLE processToken, AppPolicyCreateFileAccess *policy);
