...
HRESULT __cdecl _com_dispatch_method(IDispatch *, DISPID, WORD, VARTYPE, void *,
                                     const wchar_t *, ...);
HRESULT WINAPI _com_dispatch_raw_propget(IDispatch *, DISPID, VARTYPE,
                                         void *) throw();
HRESULT __cdecl _com_dispatch_raw_propput(IDispatch *, DISPID, VARTYPE,
                                          ...) throw();
HRESULT __cdecl _com_dispatch_raw_method(IDispatch *, DISPID, WORD, VARTYPE,
                                         void *, const wchar_t *, ...) throw();
class _com_error {
public:
  _com_error(HRESULT hr, IErrorInfo *perrinfo = NULL,
...
inline _com_error::_com_error(HRESULT hr, IErrorInfo *perrinfo,
                              bool fAddRef) throw()
    : m_hresult(hr), m_perrinfo(perrinfo), m_pszMsg(NULL) {
  if (m_perrinfo != NULL && fAddRef)
    m_perrinfo->AddRef();
}
inline _com_error::_com_error(const _com_error &that) throw() { Ctor(that); }
inline _com_error::~_com_error() throw() { Dtor(); }
...
inline const TCHAR *_com_error::ErrorMessage() const throw() {
  if (!m_pszMsg) {
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                  NULL, m_hresult, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
                  (LPTSTR)&m_pszMsg, 0, NULL);
    if (m_pszMsg != NULL) {
      ...
      if (m_pszMsg != NULL) {
        ...
        if (wCode != 0) {
          ...
          _COM_PRINTF_S_1(m_pszMsg, 32, TEXT("Unknown error 0x%0lX"),
                          m_hresult);
        }
      }
    }
...
#else
  /* This is designed to use exceptions. If exceptions are disabled, there is
   * not much we can do here. */
  __debugbreak();
#endif
}
__MINGW_SELECTANY void(WINAPI *__mingw_com_error_handler)(
    HRESULT hr, IErrorInfo *perrinfo) = _com_raise_error;
inline void WINAPI _set_com_error_handler(
    void(WINAPI *pHandler)(HRESULT hr, IErrorInfo *perrinfo)) {
...
/* With our __uuidof, its result can't be passed directly as a template
 * argument. We have _com_IIID_getter to work around that. */
#define _COM_SMARTPTR_TYPEDEF(Interface, aIID)                                 \
  inline const IID &__##Interface##_IID_getter(void) { return aIID; }          \
  typedef _COM_SMARTPTR<                                                       \
      _com_IIID_getter<Interface, __##Interface##_IID_getter>>                 \
      Interface##Ptr
#else
#define _COM_SMARTPTR_TYPEDEF(Interface, IID)                                  \
  typedef _COM_SMARTPTR<_COM_SMARTPTR_LEVEL2<Interface, &IID>> Interface##Ptr
#endif
#else
#define _COM_SMARTPTR_TYPEDEF(Interface, IID)                                  \
