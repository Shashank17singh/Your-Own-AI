...
#define WAB_IWABOBJECT_METHODS(IPURE)                                          \
  ...
  MAPIMETHOD(AllocateMore)(THIS_ ULONG cbSize, LPVOID lpObject,                \
                           LPVOID * lppBuffer) IPURE;                          \
  MAPIMETHOD(FreeBuffer)(THIS_ LPVOID lpBuffer) IPURE;                         \
  MAPIMETHOD(Backup)(THIS_ LPSTR lpFileName) IPURE;                            \
  MAPIMETHOD(Import)(THIS_ LPSTR lpImportParam) IPURE;                         \
  MAPIMETHOD(Find)(THIS_ LPADRBOOK lpIAB, HWND hWnd) IPURE;                    \
  MAPIMETHOD(VCardDisplay)(THIS_ LPADRBOOK lpIAB, HWND hWnd,                   \
                           LPSTR lpszFileName) IPURE;                          \
  MAPIMETHOD(LDAPUrl)(THIS_ LPADRBOOK lpIAB, HWND hWnd, ULONG ulFlags,         \
                      LPSTR lpszURL, LPMAILUSER * lppMailUser) IPURE;          \
  MAPIMETHOD(VCardCreate)(THIS_ LPADRBOOK lpIAB, ULONG ulFlags,                \
                          LPSTR lpszVCard, LPMAILUSER lpMailUser) IPURE;       \
  MAPIMETHOD(VCardRetrieve)(THIS_ LPADRBOOK lpIAB, ULONG ulFlags,              \
                            LPSTR lpszVCard, LPMAILUSER * lppMailUser) IPURE;  \
  MAPIMETHOD(GetMe)(THIS_ LPADRBOOK lpIAB, ULONG ulFlags, DWORD * lpdwAction,  \
                    SBinary * lpsbEID, ULONG ulParam) IPURE;                   \
  MAPIMETHOD(SetMe)(THIS_ LPADRBOOK lpIAB, ULONG ulFlags, SBinary sbEID,       \
                    ULONG ulParam) IPURE;
#undef INTERFACE
#define INTERFACE IWABObject
DECLARE_MAPI_INTERFACE_(IWABObject, IUnknown){
    BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE) WAB_IWABOBJECT_METHODS(PURE)};
...
DECLARE_MAPI_INTERFACE(IWABOBJECT_){BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(IMPL)
                                        WAB_IWABOBJECT_METHODS(IMPL)};
...
STDMETHODIMP WABOpen(LPADRBOOK *lppAdrBook, LPWABOBJECT *lppWABObject,
                     LPWAB_PARAM lpWP, DWORD Reserved2);
typedef HRESULT(WINAPI WABOPEN)(LPADRBOOK *lppAdrBook,
                                LPWABOBJECT *lppWABObject, LPWAB_PARAM lpWP,
                                DWORD Reserved2);
typedef WABOPEN *LPWABOPEN;
STDMETHODIMP WABOpenEx(LPADRBOOK *lppAdrBook, LPWABOBJECT *lppWABObject,
                       LPWAB_PARAM lpWP, DWORD Reserved,
                       ALLOCATEBUFFER *fnAllocateBuffer,
                       ALLOCATEMORE *fnAllocateMore, FREEBUFFER *fnFreeBuffer);
typedef HRESULT(WINAPI WABOPENEX)(LPADRBOOK *lppAdrBook,
                                  LPWABOBJECT *lppWABObject, LPWAB_PARAM lpWP,
                                  DWORD Reserved,
                                  ALLOCATEBUFFER *fnAllocateBuffer,
...
DECLARE_MAPI_INTERFACE_(IWABExtInit, IUnknown){
    BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE) WAB_IWABEXTINIT_METHODS(PURE)};
DECLARE_MAPI_INTERFACE_PTR(IWABExtInit, LPWABEXTINIT);
DEFINE_GUID(IID_IWABExtInit, 0xea22ebf0, 0x87a4, 0x11d1, 0x9a, 0xcf, 0x0, 0xa0,
            0xc9, 0x1f, 0x9c, 0x8b);
