...
typedef WINBOOL (*PFN_CPD_ADD_STORE)(struct _CRYPT_PROVIDER_DATA *pProvData,
                                     HCERTSTORE hStore2Add);
typedef WINBOOL (*PFN_CPD_ADD_SGNR)(struct _CRYPT_PROVIDER_DATA *pProvData,
                                    WINBOOL fCounterSigner, DWORD idxSigner,
                                    struct _CRYPT_PROVIDER_SGNR *pSgnr2Add);
typedef WINBOOL (*PFN_CPD_ADD_CERT)(struct _CRYPT_PROVIDER_DATA *pProvData,
                                    DWORD idxSigner, WINBOOL fCounterSigner,
                                    DWORD idxCounterSigner,
                                    PCCERT_CONTEXT pCert2Add);
typedef WINBOOL (*PFN_CPD_ADD_PRIVDATA)(
...
typedef HRESULT (*PFN_PROVIDER_CLEANUP_CALL)(
    struct _CRYPT_PROVIDER_DATA *pProvData);
typedef WINBOOL (*PFN_PROVIDER_CERTCHKPOLICY_CALL)(
    struct _CRYPT_PROVIDER_DATA *pProvData, DWORD idxSigner,
    WINBOOL fCounterSignerChain, DWORD idxCounterSigner);
...
#define WVT_IS_CBSTRUCT_GT_MEMBEROFFSET(structtypedef, structpassedsize,       \
                                        member)                                \
...
typedef WINBOOL (*PFN_PROVUI_CALL)(HWND hWndSecurityDialog,
                                   struct _CRYPT_PROVIDER_DATA *pProvData);
...
typedef WINBOOL (*PFN_ALLOCANDFILLDEFUSAGE)(
    const char *pszUsageOID, struct _CRYPT_PROVIDER_DEFUSAGE *psDefUsage);
typedef WINBOOL (*PFN_FREEDEFUSAGE)(
    const char *pszUsageOID, struct _CRYPT_PROVIDER_DEFUSAGE *psDefUsage);
...
extern WINBOOL WINAPI WintrustAddActionID(GUID *pgActionID, DWORD fdwFlags,
...
extern WINBOOL WINAPI WintrustAddDefaultForUsage(
    const char *pszUsageOID, CRYPT_PROVIDER_REGDEFUSAGE *psDefUsage);
#define DWACTION_ALLOCANDFILL 1
#define DWACTION_FREE 2
extern WINBOOL
    WINAPI WintrustGetDefaultForUsage(DWORD dwAction, const char *pszUsageOID,
                                      CRYPT_PROVIDER_DEFUSAGE *psUsage);
extern CRYPT_PROVIDER_SGNR *WINAPI
WTHelperGetProvSignerFromChain(CRYPT_PROVIDER_DATA *pProvData, DWORD idxSigner,
                               WINBOOL fCounterSigner, DWORD idxCounterSigner);
extern CRYPT_PROVIDER_CERT *WINAPI
WTHelperGetProvCertFromChain(CRYPT_PROVIDER_SGNR *pSgnr, DWORD idxCert);
extern CRYPT_PROVIDER_DATA *WINAPI
WTHelperProvDataFromStateData(HANDLE hStateData);
extern CRYPT_PROVIDER_PRIVDATA *WINAPI WTHelperGetProvPrivateDataFromChain(
    CRYPT_PROVIDER_DATA *pProvData, GUID *pgProviderID);
extern WINBOOL WINAPI WTHelperCertIsSelfSigned(DWORD dwEncoding,
                                               CERT_INFO *pCert);
