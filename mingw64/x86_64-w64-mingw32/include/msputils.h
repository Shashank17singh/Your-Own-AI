...
extern __inline WINBOOL
IsValidAggregatedMediaType(DWORD dwAggregatedMediaType) {
  const DWORD dwAllPossibleMediaTypes =
      TAPIMEDIATYPE_AUDIO | TAPIMEDIATYPE_VIDEO | TAPIMEDIATYPE_DATAMODEM |
      TAPIMEDIATYPE_G3FAX | TAPIMEDIATYPE_MULTITRACK;
...
extern __inline WINBOOL IsSingleMediaType(DWORD dwMediaType) {
  return !((dwMediaType == 0) || ((dwMediaType & (dwMediaType - 1)) != 0));
}
extern __inline WINBOOL IsValidSingleMediaType(DWORD dwMediaType,
                                               DWORD dwMask) {
...
public:
  ...
  enum {
    SUPPORTED_SAFETY_OPTIONS = INTERFACESAFE_FOR_UNTRUSTED_CALLER |
