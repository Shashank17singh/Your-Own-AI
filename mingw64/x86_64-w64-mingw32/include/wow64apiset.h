...
WINBASEAPI UINT WINAPI GetSystemWow64Directory2A(LPSTR lpBuffer, UINT uSize,
                                                 WORD ImageFileMachineType);
WINBASEAPI UINT WINAPI GetSystemWow64Directory2W(LPWSTR lpBuffer, UINT uSize,
                                                 WORD ImageFileMachineType);
#define GetSystemWow64Directory2 __MINGW_NAME_AW(GetSystemWow64Directory2)
WINBASEAPI
    HRESULT WINAPI IsWow64GuestMachineSupported(USHORT WowGuestMachine,
                                                WINBOOL *MachineIsSupported);
#endif
#endif /* WINAPI_PARTITION_DESKTOP */
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP)
WINBASEAPI WINBOOL WINAPI IsWow64Process(HANDLE hProcess, PBOOL Wow64Process);
#if _WIN32_WINNT >= 0x0A00
WINBASEAPI WINBOOL WINAPI IsWow64Process2(HANDLE hProcess,
