...
#if defined(UM_NDIS60) || defined(UM_NDIS61) || defined(UM_NDIS620) ||         \
    defined(UM_NDIS630)
...
#define IOCTL_NDIS_RESERVED5                                                   \
  CTL_CODE(FILE_DEVICE_PHYSICAL_NETCARD, 0xd, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define IOCTL_NDIS_RESERVED6                                                   \
  CTL_CODE(FILE_DEVICE_PHYSICAL_NETCARD, 0xe, METHOD_BUFFERED,                 \
           FILE_WRITE_ACCESS)
...
typedef struct _NDIS_TIMEOUT_DPC_REQUEST_CAPABILITIES {
  ...
  ULONG TimeoutArray[1];
    ...
    *PNDIS_TIMEOUT_DPC_REQUEST_CAPABILITIES;
...
#if NTDDI_VERSION >= 0x06000000 || NDIS_SUPPORT_NDIS6
#define NDIS_MAKE_NET_LUID(PNLUID, IFTYPE, NLUIDIDX)                           \
  {                                                                            \
...
typedef struct _NDIS_IP_OPER_STATUS_INFO {
  ...
  NDIS_IP_OPER_STATUS
      IpOperationalStatus[MAXIMUM_IP_OPER_STATUS_ADDRESS_FAMILIES_SUPPORTED];
...
#define NDIS_SIZEOF_OFFLOAD_PARAMETERS_REVISION_3                              \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_OFFLOAD_PARAMETERS, EncapsulationTypes)
#endif
...
#endif
#if NDIS_SUPPORT_NDIS630
#define NDIS_SIZEOF_NDIS_WMI_OFFLOAD_REVISION_3                                \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_WMI_OFFLOAD, EncapsulatedPacketTaskOffloadGre)
#define NDIS_SIZEOF_NDIS_OFFLOAD_REVISION_3                                    \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_OFFLOAD, EncapsulatedPacketTaskOffloadGre)
...
#if NDIS_SUPPORT_NDIS630
#define NDIS_RECEIVE_FILTER_DYNAMIC_PROCESSOR_AFFINITY_CHANGE_SUPPORTED        \
  0x00000008
...
#define NDIS_RECEIVE_FILTER_PACKET_COALESCING_SUPPORTED_ON_DEFAULT_QUEUE       \
  0x00000100
#endif
...
#define NDIS_NIC_SWITCH_CAPS_ASYMMETRIC_QUEUE_PAIRS_FOR_NONDEFAULT_VPORT_SUPPORTED \
  0x00000004
...
#define NDIS_RECEIVE_QUEUE_PARAMETERS_SUGGESTED_RECV_BUFFER_NUMBERS_CHANGED    \
  0x00040000
#define NDIS_RECEIVE_QUEUE_PARAMETERS_NAME_CHANGED 0x00080000
#if NDIS_SUPPORT_NDIS630
#define NDIS_RECEIVE_QUEUE_PARAMETERS_INTERRUPT_COALESCING_DOMAIN_ID_CHANGED   \
  0x00100000
#endif
...
typedef NDIS_IF_COUNTED_STRING NDIS_SWITCH_PORT_FRIENDLYNAME,
    *PNDIS_SWITCH_PORT_FRIENDLYNAME;
...
#define NDIS_RSS_HASH_FUNC_FROM_HASH_INFO(HINFO)                               \
  ((HINFO) & (NDIS_HASH_FUNCTION_MASK))
#define NDIS_RSS_HASH_TYPE_FROM_HASH_INFO(HINFO)                               \
  ((HINFO) & (NDIS_HASH_TYPE_MASK))
...
#endif
#if NDIS_SUPPORT_NDIS630
#define NDIS_NDK_PERFORMANCE_COUNTER_MASK(CNTFIELD)                            \
  (((ULONG64)1) << (FIELD_OFFSET(NDIS_NDK_PERFORMANCE_COUNTERS, CNTFIELD) /    \
                    sizeof(ULONG64)))
...
DEFINE_GUID(GUID_NDIS_NDK_CAPABILITIES, 0x7969ba4d, 0xdd80, 0x4bc7, 0xb3, 0xe6,
            0x68, 0x04, 0x39, 0x97, 0xe5, 0x19);
DEFINE_GUID(GUID_NDIS_NDK_STATE, 0x530c69c9, 0x2f51, 0x49de, 0xa1, 0xaf, 0x08,
            0x8d, 0x54, 0xff, 0xa4, 0x74);
...
#define NDIS_SWITCH_PORT_PROPERTY_CUSTOM_GET_BUFFER(PPROPC)                    \
  ((PVOID)((PUCHAR)(PPROPC) + (PPROPC)->PropertyBufferOffset))
#define NDIS_SWITCH_PORT_PROPERTY_PARAMETERS_GET_PROPERTY(PPARM)               \
  ((PVOID)((PUCHAR)(PPARM) + (PPARM)->PropertyBufferOffset))
#define NDIS_SWITCH_CREATE_PROPERTY_VERSION(VMAJOR, VMINOR)                    \
  (((VMAJOR) << 8) + (VMINOR))
#define NDIS_SWITCH_PORT_PROPERTY_ENUM_PARAMETERS_GET_FIRST_INFO(PEPARM)       \
  ((PNDIS_SWITCH_PORT_PROPERTY_ENUM_INFO)((PUCHAR)(PEPARM) +                   \
                                          (PEPARM)->FirstPropertyOffset))
#define NDIS_SWITCH_PORT_PROPERTY_ENUM_INFO_GET_NEXT(PEINFO)                       \
  ((PNDIS_SWITCH_PORT_PROPERTY_ENUM_INFO)((ULONG_PTR)(PEINFO) +                    \
                                          (PEINFO)                                 \
                                              ->QwordAlignedPropertyBufferLength + \
                                          sizeof(                                  \
                                              NDIS_SWITCH_PORT_PROPERTY_ENUM_INFO)))
#define NDIS_SWITCH_PORT_PROPERTY_ENUM_INFO_GET_PROPERTY(PEINFO)               \
  ((PVOID)((PUCHAR)(PEINFO) + (PEINFO)->PropertyBufferOffset))
#define NDIS_SWITCH_CREATE_FEATURE_STATUS_VERSION(VMAJOR, VMINOR)              \
  (((VMAJOR) << 8) + (VMINOR))
#define NDIS_SWITCH_PROPERTY_CUSTOM_GET_BUFFER(SWPROPC)                        \
  ((PVOID)((PUCHAR)(SWPROPC) + (SWPROPC)->PropertyBufferOffset))
#define NDIS_SWITCH_PROPERTY_PARAMETERS_GET_PROPERTY(SWPARA)                   \
  ((PVOID)((PUCHAR)(SWPARA) + (SWPARA)->PropertyBufferOffset))
#define NDIS_SWITCH_PROPERTY_ENUM_INFO_GET_NEXT(SWEINFO)                       \
  ((PNDIS_SWITCH_PROPERTY_ENUM_INFO)((ULONG_PTR)(SWEINFO) +                    \
                                     (SWEINFO)                                 \
                                         ->QwordAlignedPropertyBufferLength +  \
                                     sizeof(NDIS_SWITCH_PROPERTY_ENUM_INFO)))
#define NDIS_SWITCH_PROPERTY_ENUM_INFO_GET_PROPERTY(SWEINFO)                   \
  ((PVOID)((PUCHAR)(SWEINFO) + (SWEINFO)->PropertyBufferOffset))
#define NDIS_SWITCH_PROPERTY_ENUM_PARAMETERS_GET_FIRST_INFO(SWEPARM)           \
  ((PNDIS_SWITCH_PROPERTY_ENUM_INFO)((PUCHAR)(SWEPARM) +                       \
                                     (SWEPARM)->FirstPropertyOffset))
#define NDIS_SWITCH_PORT_AT_ARRAY_INDEX(PA, IDX)                               \
  ((PNDIS_SWITCH_PORT_PARAMETERS)((PUCHAR)(PA) + (PA)->FirstElementOffset +    \
                                  ((PA)->ElementSize * (IDX))))
#define NDIS_SWITCH_NIC_AT_ARRAY_INDEX(NA, IDX)                                \
  ((PNDIS_SWITCH_NIC_PARAMETERS)((PUCHAR)(NA) + (NA)->FirstElementOffset +     \
                                 ((NA)->ElementSize * (IDX))))
#include <ndkinfo.h>
#include <ws2def.h>
#ifndef __CYGWIN__
...
typedef NDIS_IF_COUNTED_STRING NDIS_NIC_SWITCH_FRIENDLYNAME,
    *PNDIS_NIC_SWITCH_FRIENDLYNAME;
...
typedef USHORT NDIS_SWITCH_OBJECT_SERIALIZATION_VERSION,
    *PNDIS_SWITCH_OBJECT_SERIALIZATION_VERSION;
