...
typedef HRESULT (*PFNCREATETERM)(CComPtr<IMoniker> pMoniker,
                                 MSP_HANDLE htAddress, ITTerminal **pTerm);
typedef struct {
  DWORD dwMediaType;
...
class ATL_NO_VTABLE CPlugTerminalClassInfo
    : public IDispatchImpl<ITPluggableTerminalClassInfo,
                           &IID_ITPluggableTerminalClassInfo, &LIBID_TAPI3Lib>,
...
public:
  CPlugTerminalClassInfo()
      : m_bstrName(NULL), m_bstrCompany(NULL), m_bstrVersion(NULL),
        m_bstrCLSID(NULL), m_bstrTerminalClass(NULL), m_lMediaType(1),
        m_Direction(TD_CAPTURE), m_pFTM(NULL) {}
  ~CPlugTerminalClassInfo() {
    if (m_bstrName) {
...
class ATL_NO_VTABLE CPlugTerminalSuperclassInfo
    : public IDispatchImpl<ITPluggableTerminalSuperclassInfo,
...
public:
  CPlugTerminalSuperclassInfo()
      : m_bstrCLSID(NULL), m_bstrName(NULL), m_pFTM(NULL) {}
  ~CPlugTerminalSuperclassInfo() {
    if (m_bstrName) {
      SysFreeString(m_bstrName);
    }
...
class ATL_NO_VTABLE CMSPAddress
    : public CComObjectRootEx<CComMultiThreadModelNoCS>,
      public ITMSPAddress,
      public IDispatchImpl<ITTerminalSupport2, &IID_ITTerminalSupport2,
                           &LIBID_TAPI3Lib> {
public:
  ...
  STDMETHOD(Shutdown)();
  STDMETHOD(CreateMSPCall)(MSP_HANDLE htCall, DWORD dwReserved,
                           DWORD dwMediaType, IUnknown *pOuterUnknown,
                           IUnknown **ppMSPCall) = 0;
  STDMETHOD(ShutdownMSPCall)(IUnknown *pMSPCall) = 0;
  STDMETHOD(ReceiveTSPData)(IUnknown *pMSPCall, LPBYTE pBuffer,
                            DWORD dwBufferSize);
  ...
  STDMETHOD(CreateTerminal)(BSTR pTerminalClass, __LONG32 lMediaType,
                            TERMINAL_DIRECTION Direction,
                            ITTerminal **ppTerminal);
  STDMETHOD(GetDefaultStaticTerminal)(__LONG32 lMediaType,
                                      TERMINAL_DIRECTION Direction,
                                      ITTerminal **ppTerminal);
  ...
  STDMETHOD(get_PluggableTerminalClasses)(BSTR bstrTerminalSuperclass,
                                          __LONG32 lMediaType,
                                          VARIANT *pVariant);
...
protected:
  virtual HRESULT GetStaticTerminals(DWORD *pdwNumTerminals,
                                     ITTerminal **ppTerminals);
  virtual HRESULT GetDynamicTerminalClasses(DWORD *pdwNumClasses,
...
template <class T>
HRESULT CreateMSPCallHelper(CMSPAddress *pCMSPAddress, MSP_HANDLE htCall,
                            DWORD dwReserved, DWORD dwMediaType,
                            IUnknown *pOuterUnknown, IUnknown **ppMSPCall,
                            T **ppCMSPCall) {
  ...
  if (IsBadReadPtr(pCMSPAddress, sizeof(CMSPAddress))) {
    LOG((MSP_ERROR,
         "CreateMSPCallHelper - bad address pointer - exit E_POINTER"));
    return E_POINTER;
  }
  if (IsBadReadPtr(pOuterUnknown, sizeof(IUnknown))) {
    LOG((MSP_ERROR, "CreateMSPCallHelper - bad outer unknown - we require "
                    "aggregation - exit E_POINTER"));
    return E_POINTER;
  }
  if (IsBadReadPtr(ppMSPCall, sizeof(IUnknown *))) {
    LOG((MSP_ERROR,
         "CreateMSPCallHelper - bad iunknown return ptr - exit E_POINTER"));
    return E_POINTER;
  }
  if (IsBadReadPtr(ppCMSPCall, sizeof(T *))) {
    LOG((MSP_ERROR,
         "CreateMSPCallHelper - bad class return ptr - exit E_POINTER"));
    return E_POINTER;
  }
  if (!pCMSPAddress->IsValidSetOfMediaTypes(
          dwMediaType, pCMSPAddress->GetCallMediaTypes())) {
    LOG((MSP_ERROR, "CreateMSPCallHelper - unsupported media types - exit "
  ...
  if (!pCall) {
    LOG((MSP_ERROR, "CreateMSPCallHelper - could not create agg call instance "
                    "- exit E_OUTOFMEMORY"));
  ...
  if (!pMSPCall) {
    LOG((MSP_ERROR, "CreateMSPCallHelper - can not cast to agg object to class "
                    "pointer - exit E_UNEXPECTED"));
    pUnknown->Release();
    return E_UNEXPECTED;
  }
...
template <class T>
HRESULT ShutdownMSPCallHelper(IUnknown *pUnknown, T **ppCMSPCall) {
  LOG((MSP_TRACE, "ShutdownMSPCallHelper - enter"));
  if (IsBadReadPtr(pUnknown, sizeof(IUnknown))) {
    LOG((MSP_ERROR,
         "ShutdownMSPCallHelper - bad IUnknown pointer - exit E_POINTER"));
    return E_POINTER;
  }
  if (IsBadWritePtr(ppCMSPCall, sizeof(T *))) {
    LOG((MSP_ERROR,
         "ShutdownMSPCallHelper - bad return pointer - exit E_POINTER"));
    return E_POINTER;
  }
  T *pMSPCall;
  CComAggObject<T> *pCall = dynamic_cast<CComAggObject<T> *>(pUnknown);
  if (!pCall) {
    LOG((MSP_ERROR, "ShutdownMSPCallHelper - can't cast unknown to agg object "
                    "pointer - exit E_UNEXPECTED"));
    return E_UNEXPECTED;
  }
  pMSPCall = dynamic_cast<T *>(&(pCall->m_contained));
  if (!pMSPCall) {
    LOG((MSP_ERROR, "ShutdownMSPCallHelper - can't cast contained unknown to "
                    "class pointer - exit E_UNEXPECTED"));
    return E_UNEXPECTED;
  }
