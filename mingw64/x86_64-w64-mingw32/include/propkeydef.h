 ...
 */
/* This file provides macros and procedures relevant to the PROPERTYKEY
 * structure defined in wtypes.h. */
/* Property identifiers passed to the DEFINE_PROPERTYKEY macro below should be
 * greater than 1; IDs 0 and 1 are reserved. See also:
...
#endif
/* See the definitions of PROPERTYKEY in wtypes.h, and GUID in guiddef.h. "l" is
 * short for "long", "w" for "word", "b" for "byte", and "pid" for "property
 * identifier". */
...
#endif
/* This implementation differs from the Windows SDK in order to correctly match
 * the type of REFGUID used in `IsEqualIID()` (defined in guiddef.h) when
...
extern "C++" {
inline bool operator==(REFPROPERTYKEY k0, REFPROPERTYKEY k1) {
  return IsEqualPropertyKey(k0, k1);
}
inline bool operator!=(REFPROPERTYKEY k0, REFPROPERTYKEY k1) {
  return !IsEqualPropertyKey(k0, k1);
}
