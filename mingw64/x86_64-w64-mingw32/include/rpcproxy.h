...
RPCRTAPI void RPC_ENTRY NdrProxyInitialize(void *This, PRPC_MESSAGE pRpcMsg,
...
HRESULT WINAPI CStdStubBuffer_QueryInterface(IRpcStubBuffer *This, REFIID riid,
                                             void **ppvObject);
...
#define CStdStubBuffer_METHODS                                                 \
  CStdStubBuffer_QueryInterface, CStdStubBuffer_AddRef,                        \
      CStdStubBuffer_Release, CStdStubBuffer_Connect,                          \
      CStdStubBuffer_Disconnect, CStdStubBuffer_Invoke,                        \
      CStdStubBuffer_IsIIDSupported, CStdStubBuffer_CountRefs,                 \
...
#define IID_GENERIC_CHECK_IID(name, pIID, index)                               \
  memcmp(pIID, name##_ProxyVtblList[index]->header.piid, 16)
...
NdrDllGetClassObject(REFCLSID rclsid, REFIID riid, void **ppv,
                     const ProxyFileInfo **pProxyFileList, const CLSID *pclsid,
...
RPCRTAPI HRESULT RPC_ENTRY NdrDllRegisterProxy(
    HMODULE hDll, const ProxyFileInfo **pProxyFileList, const CLSID *pclsid);
RPCRTAPI HRESULT RPC_ENTRY NdrDllUnregisterProxy(
    HMODULE hDll, const ProxyFileInfo **pProxyFileList, const CLSID *pclsid);
#define REGISTER_PROXY_DLL_ROUTINES(pProxyFileList, pClsID)                    \
  HINSTANCE hProxyDll = 0;                                                     \
  WINBOOL WINAPI DLLMAIN_ENTRY(HINSTANCE hinstDLL, DWORD fdwReason,            \
                               LPVOID lpvReserved) {                           \
  ...
  HRESULT WINAPI DLLREGISTERSERVER_ENTRY() {                                   \
    return NdrDllRegisterProxy(hProxyDll, pProxyFileList, pClsID);             \
  }                                                                            \
  HRESULT WINAPI DLLUNREGISTERSERVER_ENTRY() {                                 \
...
ULONG WINAPI NdrCStdStubBuffer2_Release(IRpcStubBuffer *This,
                                        IPSFactoryBuffer *pPSF);
#define CStdStubBuffer_DELEGATING_METHODS                                      \
  0, 0, CStdStubBuffer2_Release, 0, 0, 0, 0, 0, 0, 0
#ifdef PROXY_CLSID
...
#define DLLCANUNLOADNOW(pFactory)                                              \
  HRESULT WINAPI DLLCANUNLOADNOW_ENTRY() {                                     \
...
#define DLLDATA_ROUTINES(pProxyFileList, pClsID)                               \
  ...
  DLLGETCLASSOBJECTROUTINE(pProxyFileList, pClsID, &gPFactory)                 \
      DLLCANUNLOADNOW(&gPFactory) CSTDSTUBBUFFERRELEASE(&gPFactory)            \
          CSTDSTUBBUFFER2RELEASE(&gPFactory) DLLDUMMYPURECALL                  \
          DLLREGISTRY_ROUTINES(pProxyFileList, pClsID)
#define DLLDATA_STANDARD_ROUTINES                                              \
  DLLDATA_ROUTINES((const ProxyFileInfo **)pProxyFileList,                     \
