...
#define INLINE_DWORD_FLIP(out, in)                                             \
  {                                                                            \
    DWORD _in = (in);                                                          \
    (out) = ((_in << 8) & 0x00ff0000) | (_in << 24) |                          \
            ((_in >> 8) & 0x0000ff00) | (_in >> 24);                           \
  }
...
#define DNS_IP4_REVERSE_DOMAIN_STRING                                          \
  __MINGW_NAME_UAW(DNS_IP4_REVERSE_DOMAIN_STRING)
#define DNS_IP6_REVERSE_DOMAIN_STRING                                          \
  __MINGW_NAME_UAW(DNS_IP6_REVERSE_DOMAIN_STRING)
...
DNS_STATUS WINAPI DnsQueryConfig(DNS_CONFIG_TYPE Config, DWORD Flag,
                                 PWSTR pwsAdapterName, PVOID pReserved,
...
typedef struct _DnsRecordW {
  ...
  union {
    ...
    DNS_PTR_DATAW PTR, Ptr, NS, Ns, CNAME, Cname, DNAME, Dname, MB, Mb, MD, Md,
        MF, Mf, MG, Mg, MR, Mr;
...
typedef struct _DnsRecordA {
  ...
  union {
    ...
    DNS_PTR_DATAA PTR, Ptr, NS, Ns, CNAME, Cname, DNAME, Dname, MB, Mb, MD, Md,
        MF, Mf, MG, Mg, MR, Mr;
...
typedef VOID(WINAPI *DNS_PROXY_COMPLETION_ROUTINE)(void *completionContext,
                                                   DNS_STATUS status);
...
PDNS_RECORD WINAPI DnsRecordCopyEx(PDNS_RECORD pRecord, DNS_CHARSET CharSetIn,
                                   DNS_CHARSET CharSetOut);
PDNS_RECORD WINAPI DnsRecordSetCopyEx(PDNS_RECORD pRecordSet,
...
WINBOOL WINAPI DnsRecordSetCompare(PDNS_RECORD pRR1, PDNS_RECORD pRR2,
...
DNS_STATUS WINAPI DnsQuery_A(PCSTR pszName, WORD wType, DWORD Options,
                             PIP4_ARRAY aipServers, PDNS_RECORD *ppQueryResults,
                             PVOID *pReserved);
DNS_STATUS WINAPI DnsQuery_UTF8(PCSTR pszName, WORD wType, DWORD Options,
...
typedef VOID WINAPI DNS_QUERY_COMPLETION_ROUTINE(
    PVOID pQueryContext, PDNS_QUERY_RESULT pQueryResults);
...
DNS_STATUS WINAPI DnsAcquireContextHandle_W(DWORD CredentialFlags,
                                            PVOID pCredentials,
...
DNS_STATUS WINAPI DnsModifyRecordsInSet_A(PDNS_RECORD pAddRecords,
                                          ...
                                          PIP4_ARRAY pServerList,
                                          PVOID pReserved);
DNS_STATUS WINAPI DnsModifyRecordsInSet_UTF8(PDNS_RECORDA pAddRecords,
                                             PDNS_RECORDA pDeleteRecords,
                                             DWORD Options, HANDLE hContext,
                                             PIP4_ARRAY pServerList,
                                             PVOID pReserved);
DNS_STATUS WINAPI DnsReplaceRecordSetW(PDNS_RECORD pNewSet, DWORD Options,
                                       HANDLE hContext, PIP4_ARRAY pServerList,
                                       PVOID pReserved);
DNS_STATUS WINAPI DnsReplaceRecordSetA(PDNS_RECORD pNewSet, DWORD Options,
                                       HANDLE hContext, PIP4_ARRAY pServerList,
                                       PVOID pReserved);
DNS_STATUS WINAPI DnsReplaceRecordSetUTF8(PDNS_RECORDA pNewSet, DWORD Options,
                                          HANDLE hContext,
                                          PIP4_ARRAY pServerList,
                                          PVOID pReserved);
...
WINBOOL WINAPI DnsWriteQuestionToBuffer_W(PDNS_MESSAGE_BUFFER pDnsBuffer,
                                          PDWORD pdwBufferSize, LPWSTR pszName,
                                          WORD wType, WORD Xid,
                                          WINBOOL fRecursionDesired);
WINBOOL WINAPI DnsWriteQuestionToBuffer_UTF8(PDNS_MESSAGE_BUFFER pDnsBuffer,
                                             LPDWORD pdwBufferSize,
                                             LPSTR pszName, WORD wType,
                                             WORD Xid,
                                             WINBOOL fRecursionDesired);
...
DnsExtractRecordsFromMessage_UTF8(PDNS_MESSAGE_BUFFER pDnsBuffer,
                                  WORD wMessageLength, PDNS_RECORDA *ppRecord);
...
DWORD DnsConnectionGetProxyInfoForHostUrl(
    PCWSTR pwszHostUrl, BYTE *pSelectionContext, DWORD dwSelectionContextLength,
    DWORD dwExplicitInterfaceIndex, DNS_CONNECTION_PROXY_INFO_EX *pProxyInfoEx);
VOID DnsConnectionFreeProxyInfoEx(DNS_CONNECTION_PROXY_INFO_EX *pProxyInfoEx);
DWORD DnsConnectionGetProxyInfo(PCWSTR pwszConnectionName,
                                DNS_CONNECTION_PROXY_TYPE Type,
                                DNS_CONNECTION_PROXY_INFO *pProxyInfo);
VOID DnsConnectionFreeProxyInfo(DNS_CONNECTION_PROXY_INFO *pProxyInfo);
DWORD DnsConnectionSetProxyInfo(PCWSTR pwszConnectionName,
                                DNS_CONNECTION_PROXY_TYPE Type,
                                const DNS_CONNECTION_PROXY_INFO *pProxyInfo);
DWORD DnsConnectionDeleteProxyInfo(PCWSTR pwszConnectionName,
                                   DNS_CONNECTION_PROXY_TYPE Type);
DWORD DnsConnectionGetProxyList(PCWSTR pwszConnectionName,
...
typedef VOID WINAPI DNS_SERVICE_BROWSE_CALLBACK(DWORD Status,
                                                PVOID pQueryContext,
...
DNS_STATUS WINAPI DnsServiceBrowse(PDNS_SERVICE_BROWSE_REQUEST pRequest,
...
DNS_STATUS WINAPI DnsServiceResolve(PDNS_SERVICE_RESOLVE_REQUEST pRequest,
                                    PDNS_SERVICE_CANCEL pCancel);
...
DWORD WINAPI DnsServiceRegister(PDNS_SERVICE_REGISTER_REQUEST pRequest,
                                PDNS_SERVICE_CANCEL pCancel);
DWORD WINAPI DnsServiceDeRegister(PDNS_SERVICE_REGISTER_REQUEST pRequest,
                                  PDNS_SERVICE_CANCEL pCancel);
...
typedef VOID WINAPI MDNS_QUERY_CALLBACK(PVOID pQueryContext,
...
DNS_STATUS WINAPI DnsStartMulticastQuery(PMDNS_QUERY_REQUEST pQueryRequest,
