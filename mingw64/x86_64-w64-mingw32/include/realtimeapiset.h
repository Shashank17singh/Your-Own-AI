...
WINBASEAPI WINBOOL WINAPI QueryProcessCycleTime(HANDLE ProcessHandle,
                                                PULONG64 CycleTime);
WINBASEAPI WINBOOL WINAPI QueryIdleProcessorCycleTime(
    PULONG BufferLength, PULONG64 ProcessorIdleCycleTime);
#endif
#if _WIN32_WINNT >= 0x0601
WINBASEAPI WINBOOL WINAPI QueryIdleProcessorCycleTimeEx(
    USHORT Group, PULONG BufferLength, PULONG64 ProcessorIdleCycleTime);
#endif
#endif /* WINAPI_PARTITION_DESKTOP */
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP)
WINBASEAPI
    VOID WINAPI QueryInterruptTimePrecise(PULONGLONG lpInterruptTimePrecise);
