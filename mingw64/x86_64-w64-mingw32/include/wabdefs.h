...
#define IsEqualMAPIUID(lpuid1, lpuid2)                                         \
  (!memcmp(lpuid1, lpuid2, sizeof(MAPIUID)))
...
#define PROP_TAG(ulPropType, ulPropID)                                         \
  ((((ULONG)(ulPropID)) << 16) | ((ULONG)(ulPropType)))
...
#define CbFLATENTRYLIST(_lplist)                                               \
  (offsetof(FLATENTRYLIST, abEntries) + (_lplist)->cbEntries)
...
#define CbFLATMTSIDLIST(_lplist)                                               \
  (offsetof(FLATMTSIDLIST, abMTSIDs) + (_lplist)->cbMTSIDs)
...
#define CbADRLIST(_lpadrlist)                                                  \
  (offsetof(ADRLIST, aEntries) +                                               \
   (UINT)(_lpadrlist)->cEntries * sizeof(ADRENTRY))
...
typedef SCODE(WINAPI ALLOCATEMORE)(ULONG cbSize, LPVOID lpObject,
                                   LPVOID *lppBuffer);
...
#define MAPIMETHOD_TYPEDEF(type, method, prefix)                               \
  typedef type(WINAPI prefix##method##_METHOD)
...
DECLARE_MAPI_INTERFACE_(IMAPIAdviseSink,
                        IUnknown){BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE)
                                      MAPI_IMAPIADVISESINK_METHODS(PURE)};
typedef __LONG32(WINAPI NOTIFCALLBACK)(LPVOID lpvContext, ULONG cNotification,
...
#define MAPI_IMAPIPROGRESS_METHODS(IPURE)                                      \
  MAPIMETHOD(Progress)(THIS_ ULONG ulValue, ULONG ulCount, ULONG ulTotal)      \
      IPURE;                                                                   \
  ...
  MAPIMETHOD(SetLimits)(THIS_ LPULONG lpulMin, LPULONG lpulMax,                \
...
#define MAPI_IMAPIPROP_METHODS(IPURE)                                          \
  ...
  MAPIMETHOD(GetProps)(THIS_ LPSPropTagArray lpPropTagArray, ULONG ulFlags,    \
                       ULONG * lpcValues, LPSPropValue * lppPropArray) IPURE;  \
  MAPIMETHOD(GetPropList)(THIS_ ULONG ulFlags,                                 \
                          LPSPropTagArray * lppPropTagArray) IPURE;            \
  MAPIMETHOD(OpenProperty)(THIS_ ULONG ulPropTag, LPCIID lpiid,                \
                           ULONG ulInterfaceOptions, ULONG ulFlags,            \
                           LPUNKNOWN * lppUnk) IPURE;                          \
  MAPIMETHOD(SetProps)(THIS_ ULONG cValues, LPSPropValue lpPropArray,          \
                       LPSPropProblemArray * lppProblems) IPURE;               \
  MAPIMETHOD(DeleteProps)(THIS_ LPSPropTagArray lpPropTagArray,                \
                          LPSPropProblemArray * lppProblems) IPURE;            \
  MAPIMETHOD(CopyTo)(THIS_ ULONG ciidExclude, LPCIID rgiidExclude,             \
                     LPSPropTagArray lpExcludeProps, ULONG ulUIParam,          \
                     LPMAPIPROGRESS lpProgress, LPCIID lpInterface,            \
                     LPVOID lpDestObj, ULONG ulFlags,                          \
                     LPSPropProblemArray * lppProblems) IPURE;                 \
  MAPIMETHOD(CopyProps)(THIS_ LPSPropTagArray lpIncludeProps, ULONG ulUIParam, \
                        LPMAPIPROGRESS lpProgress, LPCIID lpInterface,         \
                        LPVOID lpDestObj, ULONG ulFlags,                       \
                        LPSPropProblemArray * lppProblems) IPURE;              \
  MAPIMETHOD(GetNamesFromIDs)(THIS_ LPSPropTagArray * lppPropTags,             \
                              LPGUID lpPropSetGuid, ULONG ulFlags,             \
                              ULONG * lpcPropNames,                            \
                              LPMAPINAMEID * *lpppPropNames) IPURE;            \
  MAPIMETHOD(GetIDsFromNames)(THIS_ ULONG cPropNames,                          \
                              LPMAPINAMEID * lppPropNames, ULONG ulFlags,      \
...
#define MAPI_IMAPITABLE_METHODS(IPURE)                                         \
  MAPIMETHOD(GetLastError)(THIS_ HRESULT hResult, ULONG ulFlags,               \
                           LPMAPIERROR * lppMAPIError) IPURE;                  \
  MAPIMETHOD(Advise)(THIS_ ULONG ulEventMask, LPMAPIADVISESINK lpAdviseSink,   \
                     ULONG * lpulConnection) IPURE;                            \
  MAPIMETHOD(Unadvise)(THIS_ ULONG ulConnection) IPURE;                        \
  MAPIMETHOD(GetStatus)(THIS_ ULONG * lpulTableStatus, ULONG * lpulTableType)  \
      IPURE;                                                                   \
  MAPIMETHOD(SetColumns)(THIS_ LPSPropTagArray lpPropTagArray, ULONG ulFlags)  \
      IPURE;                                                                   \
  MAPIMETHOD(QueryColumns)(THIS_ ULONG ulFlags,                                \
                           LPSPropTagArray * lpPropTagArray) IPURE;            \
  MAPIMETHOD(GetRowCount)(THIS_ ULONG ulFlags, ULONG * lpulCount) IPURE;       \
  MAPIMETHOD(SeekRow)(THIS_ BOOKMARK bkOrigin, LONG lRowCount,                 \
                      LONG * lplRowsSought) IPURE;                             \
  MAPIMETHOD(SeekRowApprox)(THIS_ ULONG ulNumerator, ULONG ulDenominator)      \
      IPURE;                                                                   \
  MAPIMETHOD(QueryPosition)(THIS_ ULONG * lpulRow, ULONG * lpulNumerator,      \
                            ULONG * lpulDenominator) IPURE;                    \
  MAPIMETHOD(FindRow)(THIS_ LPSRestriction lpRestriction, BOOKMARK bkOrigin,   \
                      ULONG ulFlags) IPURE;                                    \
  MAPIMETHOD(Restrict)(THIS_ LPSRestriction lpRestriction, ULONG ulFlags)      \
      IPURE;                                                                   \
  MAPIMETHOD(CreateBookmark)(THIS_ BOOKMARK * lpbkPosition) IPURE;             \
  MAPIMETHOD(FreeBookmark)(THIS_ BOOKMARK bkPosition) IPURE;                   \
  MAPIMETHOD(SortTable)(THIS_ LPSSortOrderSet lpSortCriteria, ULONG ulFlags)   \
      IPURE;                                                                   \
  MAPIMETHOD(QuerySortOrder)(THIS_ LPSSortOrderSet * lppSortCriteria) IPURE;   \
  MAPIMETHOD(QueryRows)(THIS_ LONG lRowCount, ULONG ulFlags,                   \
                        LPSRowSet * lppRows) IPURE;                            \
  MAPIMETHOD(Abort)(THIS) IPURE;                                               \
  MAPIMETHOD(ExpandRow)(THIS_ ULONG cbInstanceKey, LPBYTE pbInstanceKey,       \
                        ULONG ulRowCount, ULONG ulFlags, LPSRowSet * lppRows,  \
                        ULONG * lpulMoreRows) IPURE;                           \
  MAPIMETHOD(CollapseRow)(THIS_ ULONG cbInstanceKey, LPBYTE pbInstanceKey,     \
                          ULONG ulFlags, ULONG * lpulRowCount) IPURE;          \
  MAPIMETHOD(WaitForCompletion)(THIS_ ULONG ulFlags, ULONG ulTimeout,          \
                                ULONG * lpulTableStatus) IPURE;                \
  MAPIMETHOD(GetCollapseState)(                                                \
      THIS_ ULONG ulFlags, ULONG cbInstanceKey, LPBYTE lpbInstanceKey,         \
      ULONG * lpcbCollapseState, LPBYTE * lppbCollapseState) IPURE;            \
  MAPIMETHOD(SetCollapseState)(THIS_ ULONG ulFlags, ULONG cbCollapseState,     \
                               LPBYTE pbCollapseState,                         \
                               BOOKMARK * lpbkLocation) IPURE;
#undef INTERFACE
#define INTERFACE IMAPITable
DECLARE_MAPI_INTERFACE_(IMAPITable, IUnknown){
    BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE) MAPI_IMAPITABLE_METHODS(PURE)};
...
#define MAPI_IMAPISTATUS_METHODS(IPURE)                                        \
  ...
  MAPIMETHOD(SettingsDialog)(THIS_ ULONG ulUIParam, ULONG ulFlags) IPURE;      \
  MAPIMETHOD(ChangePassword)(THIS_ LPTSTR lpOldPass, LPTSTR lpNewPass,         \
                             ULONG ulFlags) IPURE;                             \
...
#define MAPI_IMAPICONTAINER_METHODS(IPURE)                                     \
  MAPIMETHOD(GetContentsTable)(THIS_ ULONG ulFlags, LPMAPITABLE * lppTable)    \
      IPURE;                                                                   \
  MAPIMETHOD(GetHierarchyTable)(THIS_ ULONG ulFlags, LPMAPITABLE * lppTable)   \
      IPURE;                                                                   \
  ...
  MAPIMETHOD(SetSearchCriteria)(THIS_ LPSRestriction lpRestriction,            \
                                LPENTRYLIST lpContainerList,                   \
                                ULONG ulSearchFlags) IPURE;                    \
  MAPIMETHOD(GetSearchCriteria)(                                               \
      THIS_ ULONG ulFlags, LPSRestriction * lppRestriction,                    \
      LPENTRYLIST * lppContainerList, ULONG * lpulSearchState) IPURE;
#undef INTERFACE
...
#define MAPI_IABCONTAINER_METHODS(IPURE)                                       \
  MAPIMETHOD(CreateEntry)(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID,          \
                          ULONG ulCreateFlags, LPMAPIPROP * lppMAPIPropEntry)  \
      IPURE;                                                                   \
  MAPIMETHOD(CopyEntries)(THIS_ LPENTRYLIST lpEntries, ULONG ulUIParam,        \
                          LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;     \
  MAPIMETHOD(DeleteEntries)(THIS_ LPENTRYLIST lpEntries, ULONG ulFlags) IPURE; \
  MAPIMETHOD(ResolveNames)(THIS_ LPSPropTagArray lpPropTagArray,               \
                           ULONG ulFlags, LPADRLIST lpAdrList,                 \
...
#define MAPI_IMAPIFOLDER_METHODS(IPURE)                                        \
  MAPIMETHOD(CreateMessage)(THIS_ LPCIID lpInterface, ULONG ulFlags,           \
                            LPMESSAGE * lppMessage) IPURE;                     \
  MAPIMETHOD(CopyMessages)(THIS_ LPENTRYLIST lpMsgList, LPCIID lpInterface,    \
                           LPVOID lpDestFolder, ULONG ulUIParam,               \
                           LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;    \
  MAPIMETHOD(DeleteMessages)(THIS_ LPENTRYLIST lpMsgList, ULONG ulUIParam,     \
                             LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;  \
  MAPIMETHOD(CreateFolder)(THIS_ ULONG ulFolderType, LPTSTR lpszFolderName,    \
                           LPTSTR lpszFolderComment, LPCIID lpInterface,       \
                           ULONG ulFlags, LPMAPIFOLDER * lppFolder) IPURE;     \
  MAPIMETHOD(CopyFolder)(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID,           \
                         LPCIID lpInterface, LPVOID lpDestFolder,              \
                         LPTSTR lpszNewFolderName, ULONG ulUIParam,            \
                         LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;      \
  MAPIMETHOD(DeleteFolder)(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID,         \
                           ULONG ulUIParam, LPMAPIPROGRESS lpProgress,         \
                           ULONG ulFlags) IPURE;                               \
  MAPIMETHOD(SetReadFlags)(THIS_ LPENTRYLIST lpMsgList, ULONG ulUIParam,       \
                           LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;    \
  MAPIMETHOD(GetMessageStatus)(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID,     \
                               ULONG ulFlags, ULONG * lpulMessageStatus)       \
      IPURE;                                                                   \
  MAPIMETHOD(SetMessageStatus)(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID,     \
                               ULONG ulNewStatus, ULONG ulNewStatusMask,       \
                               ULONG * lpulOldStatus) IPURE;                   \
  MAPIMETHOD(SaveContentsSort)(THIS_ LPSSortOrderSet lpSortCriteria,           \
                               ULONG ulFlags) IPURE;                           \
  MAPIMETHOD(EmptyFolder)(THIS_ ULONG ulUIParam, LPMAPIPROGRESS lpProgress,    \
                          ULONG ulFlags) IPURE;
#undef INTERFACE
...
#define MAPI_IMSGSTORE_METHODS(IPURE)                                          \
  ...
  MAPIMETHOD(OpenEntry)(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID,            \
                        LPCIID lpInterface, ULONG ulFlags,                     \
                        ULONG * lpulObjType, LPUNKNOWN * lppUnk) IPURE;        \
  MAPIMETHOD(SetReceiveFolder)(THIS_ LPTSTR lpszMessageClass, ULONG ulFlags,   \
                               ULONG cbEntryID, LPENTRYID lpEntryID) IPURE;    \
  MAPIMETHOD(GetReceiveFolder)(THIS_ LPTSTR lpszMessageClass, ULONG ulFlags,   \
                               ULONG * lpcbEntryID, LPENTRYID * lppEntryID,    \
                               LPTSTR * lppszExplicitClass) IPURE;             \
  MAPIMETHOD(GetReceiveFolderTable)(THIS_ ULONG ulFlags,                       \
                                    LPMAPITABLE * lppTable) IPURE;             \
  MAPIMETHOD(StoreLogoff)(THIS_ ULONG * lpulFlags) IPURE;                      \
  MAPIMETHOD(AbortSubmit)(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID,          \
                          ULONG ulFlags) IPURE;                                \
  MAPIMETHOD(GetOutgoingQueue)(THIS_ ULONG ulFlags, LPMAPITABLE * lppTable)    \
      IPURE;                                                                   \
  MAPIMETHOD(SetLockState)(THIS_ LPMESSAGE lpMessage, ULONG ulLockState)       \
      IPURE;                                                                   \
  MAPIMETHOD(FinishedMsg)(THIS_ ULONG ulFlags, ULONG cbEntryID,                \
...
#define MAPI_IMESSAGE_METHODS(IPURE)                                           \
  MAPIMETHOD(GetAttachmentTable)(THIS_ ULONG ulFlags, LPMAPITABLE * lppTable)  \
      IPURE;                                                                   \
  MAPIMETHOD(OpenAttach)(THIS_ ULONG ulAttachmentNum, LPCIID lpInterface,      \
                         ULONG ulFlags, LPATTACH * lppAttach) IPURE;           \
  MAPIMETHOD(CreateAttach)(THIS_ LPCIID lpInterface, ULONG ulFlags,            \
                           ULONG * lpulAttachmentNum, LPATTACH * lppAttach)    \
      IPURE;                                                                   \
  MAPIMETHOD(DeleteAttach)(THIS_ ULONG ulAttachmentNum, ULONG ulUIParam,       \
                           LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;    \
  MAPIMETHOD(GetRecipientTable)(THIS_ ULONG ulFlags, LPMAPITABLE * lppTable)   \
      IPURE;                                                                   \
...
typedef SCODE(WINAPI *LPFNBUTTON)(ULONG ulUIParam, LPVOID lpvContext,
...
#define MAPI_IMAPICONTROL_METHODS(IPURE)                                       \
  MAPIMETHOD(GetLastError)(THIS_ HRESULT hResult, ULONG ulFlags,               \
                           LPMAPIERROR * lppMAPIError) IPURE;                  \
  MAPIMETHOD(Activate)(THIS_ ULONG ulFlags, ULONG ulUIParam) IPURE;            \
...
#define MAPI_IPROVIDERADMIN_METHODS(IPURE)                                     \
  MAPIMETHOD(GetLastError)(THIS_ HRESULT hResult, ULONG ulFlags,               \
                           LPMAPIERROR * lppMAPIError) IPURE;                  \
  MAPIMETHOD(GetProviderTable)(THIS_ ULONG ulFlags, LPMAPITABLE * lppTable)    \
      IPURE;                                                                   \
  MAPIMETHOD(CreateProvider)(THIS_ LPTSTR lpszProvider, ULONG cValues,         \
                             LPSPropValue lpProps, ULONG ulUIParam,            \
                             ULONG ulFlags, MAPIUID * lpUID) IPURE;            \
  MAPIMETHOD(DeleteProvider)(THIS_ LPMAPIUID lpUID) IPURE;                     \
  MAPIMETHOD(OpenProfileSection)(THIS_ LPMAPIUID lpUID, LPCIID lpInterface,    \
