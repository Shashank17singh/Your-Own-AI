 ...
 */
#define D3DPRESENTFLAG_LOCKABLE_BACKBUFFER                                     \
  0x00000001 /* Create a lockable backbuffer */
#define D3DPRESENTFLAG_DISCARD_DEPTHSTENCIL 0x00000002 /* Discard Z buffer */
#define D3DPRESENTFLAG_DEVICECLIP                                              \
  0x00000004 /* Clip the window blited into the client area 2k + xp only */
#define D3DPRESENTFLAG_VIDEO                                                   \
  0x00000010 /* backbuffer 'may' contain video data */
#ifndef D3D_DISABLE_9EX
#define D3DPRESENTFLAG_NOAUTOROTATE                                            \
  0x00000020 /* d3d9ex, ignore display rotation */
#define D3DPRESENTFLAG_UNPRUNEDMODE                                            \
...
#define D3DVSD_TESSNORMAL(RegisterIn, RegisterOut)                             \
  (D3DVSD_MAKETOKENTYPE(D3DVSD_TOKEN_TESSELLATOR) |                            \
   ((RegisterIn) << D3DVSD_VERTEXREGINSHIFT) |                                 \
...
#define D3DVSD_TESSUV(Register)                                                \
  (D3DVSD_MAKETOKENTYPE(D3DVSD_TOKEN_TESSELLATOR) | 0x10000000 |               \
   ((0x01) << D3DVSD_DATATYPESHIFT) | (Register))
/********************************
...
#define D3DSHADER_COMMENT(commentSize)                                         \
  ((((commentSize) << D3DSI_COMMENTSIZE_SHIFT) & D3DSI_COMMENTSIZE_MASK) |     \
   D3DSIO_COMMENT)
...
typedef struct D3DDEVINFO_VCACHE {
  ...
  DWORD CacheSize;
  /* internal for deciding when to restart strips, non user modifiable (only
   * valid if OptMethod==1) */
  DWORD MagicNumber;
} D3DDEVINFO_VCACHE;
