...
typedef enum { SHGFP_TYPE_CURRENT = 0, SHGFP_TYPE_DEFAULT = 1 } SHGFP_TYPE;
...
DECLARE_INTERFACE_IID_(IExtractIconA, IUnknown,
  ...
  STDMETHOD(GetIconLocation)(THIS_ UINT uFlags, PSTR pszIconFile, UINT cchMax,
                             int *piIndex, UINT *pwFlags) PURE;
  STDMETHOD(Extract)(THIS_ PCSTR pszFile, UINT nIconIndex, HICON * phiconLarge,
                     HICON * phiconSmall, UINT nIconSize) PURE;
};
...
DECLARE_INTERFACE_IID_(IExtractIconW, IUnknown,
  ...
  STDMETHOD(GetIconLocation)(THIS_ UINT uFlags, PWSTR pszIconFile, UINT cchMax,
                             int *piIndex, UINT *pwFlags) PURE;
  STDMETHOD(Extract)(THIS_ PCWSTR pszFile, UINT nIconIndex, HICON * phiconLarge,
                     HICON * phiconSmall, UINT nIconSize) PURE;
};
...
DECLARE_INTERFACE_IID_(IShellIconOverlayIdentifier, IUnknown,
                       "0c6c4200-c589-11d0-999a-00c04fd655e1") {
  STDMETHOD(IsMemberOf)(THIS_ PCWSTR pwszPath, DWORD dwAttrib) PURE;
  STDMETHOD(GetOverlayInfo)(THIS_ PWSTR pwszIconFile, int cchMax, int *pIndex,
                            DWORD *pdwFlags) PURE;
...
DECLARE_INTERFACE_IID_(IShellIconOverlayManager, IUnknown,
                       "f10b5e34-dd3b-42a7-aa7d-2f4ec54bb09b") {
  STDMETHOD(GetFileOverlayInfo)(THIS_ PCWSTR pwszPath, DWORD dwAttrib,
                                int *pIndex, DWORD dwflags) PURE;
  STDMETHOD(GetReservedOverlayInfo)(THIS_ PCWSTR pwszPath, DWORD dwAttrib,
                                    int *pIndex, DWORD dwflags, int iReservedID)
      PURE;
  STDMETHOD(RefreshOverlayImages)(THIS_ DWORD dwFlags) PURE;
  STDMETHOD(LoadNonloadedOverlayIdentifiers)(THIS) PURE;
  STDMETHOD(OverlayIndexFromImageIndex)(THIS_ int iImage, int *piIndex,
                                        WINBOOL fAdd) PURE;
};
...
DECLARE_INTERFACE_IID_(IShellIconOverlay, IUnknown,
...
DECLARE_INTERFACE_IID_(IShellExecuteHookA, IUnknown,
                       "000214f5-0000-0000-c000-000000000046") {
  STDMETHOD(Execute)(THIS_ LPSHELLEXECUTEINFOA pei) PURE;
};
#undef INTERFACE
#define INTERFACE IShellExecuteHookW
DECLARE_INTERFACE_IID_(IShellExecuteHookW, IUnknown,
...
DECLARE_INTERFACE_IID_(IURLSearchHook, IUnknown,
...
DECLARE_INTERFACE_IID_(ISearchContext, IUnknown,
...
DECLARE_INTERFACE_IID_(IURLSearchHook2, IURLSearchHook,
  ...
  STDMETHOD(TranslateWithSearchContext)(THIS_ PWSTR pwszSearchURL,
                                        DWORD cchBufferSize,
                                        ISearchContext * pSearchContext) PURE;
};
#undef INTERFACE
#define INTERFACE INewShortcutHookA
DECLARE_INTERFACE_IID_(INewShortcutHookA, IUnknown,
...
DECLARE_INTERFACE_IID_(INewShortcutHookW, IUnknown,
...
DECLARE_INTERFACE_IID_(IFileViewerSite, IUnknown,
...
ILCombine(PCIDLIST_ABSOLUTE pidl1, PCUIDLIST_RELATIVE pidl2);
...
SHSTDAPI_(PUIDLIST_RELATIVE)
ILFindChild(PIDLIST_ABSOLUTE pidlParent, PCIDLIST_ABSOLUTE pidlChild);
...
SHSTDAPI_(WINBOOL)
ILIsParent(PCIDLIST_ABSOLUTE pidl1, PCIDLIST_ABSOLUTE pidl2,
           WINBOOL fImmediate);
...
#endif
SHSTDAPI SHILCreateFromPath(PCWSTR pszPath, PIDLIST_ABSOLUTE *ppidl,
                            DWORD *rgfInOut);
#define VOID_OFFSET(pv, cb) ((void *)(((BYTE *)(pv)) + (cb)))
#if defined(STRICT_TYPED_ITEMIDS) && defined(__cplusplus)
}
inline PIDLIST_ABSOLUTE ILCloneFull(PCUIDLIST_ABSOLUTE p) {
  return reinterpret_cast<PIDLIST_ABSOLUTE>(ILClone(p));
}
inline PITEMID_CHILD ILCloneChild(PCUITEMID_CHILD p) { return ILCloneFirst(p); }
#if NTDDI_VERSION >= 0x06000000
inline HRESULT ILLoadFromStreamEx(IStream *s, PIDLIST_ABSOLUTE *p) {
  return ILLoadFromStreamEx(s, reinterpret_cast<PIDLIST_RELATIVE *>(p));
}
inline HRESULT ILLoadFromStreamEx(IStream *s, PITEMID_CHILD *p) {
  return ILLoadFromStreamEx(s, reinterpret_cast<PIDLIST_RELATIVE *>(p));
}
#endif
inline PCUIDLIST_RELATIVE ILSkip(PCUIDLIST_RELATIVE p, UINT c) {
  return reinterpret_cast<PCUIDLIST_RELATIVE>(VOID_OFFSET(p, c));
}
inline PUIDLIST_RELATIVE ILSkip(PUIDLIST_RELATIVE p, UINT c) {
  return const_cast<PUIDLIST_RELATIVE>(
      ILSkip(const_cast<PCUIDLIST_RELATIVE>(p), c));
}
inline PCUIDLIST_RELATIVE ILNext(PCUIDLIST_RELATIVE p) {
  return ILSkip(p, p->mkid.cb);
}
inline PUIDLIST_RELATIVE ILNext(PUIDLIST_RELATIVE p) {
  return const_cast<PUIDLIST_RELATIVE>(
      ILNext(const_cast<PCUIDLIST_RELATIVE>(p)));
}
inline WINBOOL ILIsAligned(PCUIDLIST_RELATIVE p) {
  return ((((DWORD_PTR)p) & (sizeof(void *) - 1)) == 0);
}
inline WINBOOL ILIsEmpty(PCUIDLIST_RELATIVE p) {
  return (!p || p->mkid.cb == 0);
}
inline WINBOOL ILIsChild(PCUIDLIST_RELATIVE p) {
  return (ILIsEmpty(p) || ILIsEmpty(ILNext(p)));
}
#ifdef STRICT_TYPED_ITEMIDS
inline PCUIDLIST_RELATIVE ILFindChild(PCIDLIST_ABSOLUTE p,
                                      PCIDLIST_ABSOLUTE c) {
  return const_cast<PCUIDLIST_RELATIVE>(
      ILFindChild(const_cast<PIDLIST_ABSOLUTE>(p), c));
}
#endif
extern "C" {
#else
...
ILAppendID(PIDLIST_RELATIVE pidl, LPCSHITEMID pmkid, WINBOOL fAppend);
#if NTDDI_VERSION >= 0x06000000
...
SHSTDAPI_(WINBOOL)
SHGetPathFromIDListEx(PCIDLIST_ABSOLUTE pidl, PWSTR pszPath, DWORD cchPath,
                      GPFIDL_FLAGS uOpts);
#endif
...
SHSTDAPI_(int)
SHCreateDirectoryExA(HWND hwnd, LPCSTR pszPath, const SECURITY_ATTRIBUTES *psa);
SHSTDAPI_(int)
SHCreateDirectoryExW(HWND hwnd, LPCWSTR pszPath,
...
#endif
SHSTDAPI SHOpenFolderAndSelectItems(PCIDLIST_ABSOLUTE pidlFolder, UINT cidl,
                                    PCUITEMID_CHILD_ARRAY apidl, DWORD dwFlags);
SHSTDAPI SHCreateShellItem(PCIDLIST_ABSOLUTE pidlParent,
                           IShellFolder *psfParent, PCUITEMID_CHILD pidl,
                           IShellItem **ppsi);
...
SHSTDAPI SHGetSpecialFolderLocation(HWND hwnd, int csidl,
                                    PIDLIST_ABSOLUTE *ppidl);
SHSTDAPI_(PIDLIST_ABSOLUTE)
SHCloneSpecialIDList(HWND hwnd, int csidl, WINBOOL fCreate);
SHSTDAPI_(WINBOOL)
SHGetSpecialFolderPathA(HWND hwnd, LPSTR pszPath, int csidl, WINBOOL fCreate);
SHSTDAPI_(WINBOOL)
SHGetSpecialFolderPathW(HWND hwnd, LPWSTR pszPath, int csidl, WINBOOL fCreate);
SHSTDAPI_(void) SHFlushSFCache(void);
SHFOLDERAPI SHGetFolderPathA(HWND hwnd, int csidl, HANDLE hToken, DWORD dwFlags,
                             LPSTR pszPath);
SHSTDAPI SHGetFolderLocation(HWND hwnd, int csidl, HANDLE hToken, DWORD dwFlags,
                             PIDLIST_ABSOLUTE *ppidl);
SHSTDAPI SHSetFolderPathA(int csidl, HANDLE hToken, DWORD dwFlags,
                          LPCSTR pszPath);
SHSTDAPI SHSetFolderPathW(int csidl, HANDLE hToken, DWORD dwFlags,
                          LPCWSTR pszPath);
SHSTDAPI SHGetFolderPathAndSubDirA(HWND hwnd, int csidl, HANDLE hToken,
                                   DWORD dwFlags, LPCSTR pszSubDir,
                                   LPSTR pszPath);
SHSTDAPI SHGetFolderPathAndSubDirW(HWND hwnd, int csidl, HANDLE hToken,
                                   DWORD dwFlags, LPCWSTR pszSubDir,
                                   LPWSTR pszPath);
...
STDAPI SHGetKnownFolderIDList(REFKNOWNFOLDERID rfid, DWORD dwFlags,
                              HANDLE hToken, PIDLIST_ABSOLUTE *ppidl);
STDAPI SHSetKnownFolderPath(REFKNOWNFOLDERID rfid, DWORD dwFlags, HANDLE hToken,
                            PCWSTR pszPath);
STDAPI SHGetKnownFolderPath(REFKNOWNFOLDERID rfid, DWORD dwFlags, HANDLE hToken,
                            PWSTR *ppszPath);
#endif
#if NTDDI_VERSION >= 0x06010000
STDAPI SHGetKnownFolderItem(REFKNOWNFOLDERID rfid, KNOWN_FOLDER_FLAG flags,
                            HANDLE hToken, REFIID riid, void **ppv);
#endif
...
#include <poppack.h>
SHSTDAPI SHGetSetFolderCustomSettings(LPSHFOLDERCUSTOMSETTINGS pfcs,
                                      PCWSTR pszPath, DWORD dwReadWrite);
#endif
typedef int(CALLBACK *BFFCALLBACK)(HWND hwnd, UINT uMsg, LPARAM lParam,
                                   LPARAM lpData);
#include <pshpack8.h>
...
DECLARE_INTERFACE_IID_(IShellDetails, IUnknown,
                       "000214EC-0000-0000-c000-000000000046") {
  STDMETHOD(GetDetailsOf)(THIS_ PCUITEMID_CHILD pidl, UINT iColumn,
                          SHELLDETAILS * pDetails) PURE;
  STDMETHOD(ColumnClick)(THIS_ UINT iColumn) PURE;
};
#undef INTERFACE
#define INTERFACE IObjMgr
DECLARE_INTERFACE_IID_(IObjMgr, IUnknown,
...
DECLARE_INTERFACE_IID_(ICurrentWorkingDirectory, IUnknown,
...
DECLARE_INTERFACE_IID_(IACList, IUnknown,
...
DECLARE_INTERFACE_IID_(IACList2, IACList,
...
DECLARE_INTERFACE_IID_(IProgressDialog, IUnknown,
                       "EBBC7C04-315E-11d2-B62F-006097DF5BD4") {
  STDMETHOD(StartProgressDialog)(THIS_ HWND hwndParent,
                                 IUnknown * punkEnableModless, DWORD dwFlags,
  ...
  STDMETHOD(SetProgress64)(THIS_ ULONGLONG ullCompleted, ULONGLONG ullTotal)
      PURE;
  STDMETHOD(SetLine)(THIS_ DWORD dwLineNum, PCWSTR pwzString,
                     WINBOOL fCompactPath, LPCVOID pvResevered) PURE;
...
DECLARE_INTERFACE_IID_(IDockingWindowSite, IOleWindow,
...
DECLARE_INTERFACE_IID_(IDockingWindowFrame, IOleWindow,
                       "47d2657a-7b27-11d0-8ca9-00a0c92dbfe8") {
  STDMETHOD(AddToolbar)(THIS_ IUnknown * punkSrc, PCWSTR pwszItem,
                        DWORD dwAddFlags) PURE;
  STDMETHOD(RemoveToolbar)(THIS_ IUnknown * punkSrc, DWORD dwRemoveFlags) PURE;
  STDMETHOD(FindToolbar)(THIS_ PCWSTR pwszItem, REFIID riid, void **ppv) PURE;
};
#undef INTERFACE
#define INTERFACE IThumbnailCapture
DECLARE_INTERFACE_IID_(IThumbnailCapture, IUnknown,
                       "4ea39266-7211-409f-b622-f63dbd16c533") {
  STDMETHOD(CaptureThumbnail)(THIS_ const SIZE *pMaxSize, IUnknown *pHTMLDoc2,
...
DECLARE_INTERFACE_IID_(IEnumShellImageStore, IUnknown,
                       "6DFD582B-92E3-11D1-98A3-00C04FB687DA") {
  STDMETHOD(QueryInterface)(THIS_ REFIID riid, void **ppv) PURE;
  STDMETHOD_(ULONG, AddRef)(THIS) PURE;
  STDMETHOD_(ULONG, Release)(THIS) PURE;
  STDMETHOD(Reset)(THIS) PURE;
  STDMETHOD(Next)(THIS_ ULONG celt, PENUMSHELLIMAGESTOREDATA * prgElt,
...
DECLARE_INTERFACE_IID_(IShellImageStore, IUnknown,
  ...
  STDMETHOD(AddEntry)(THIS_ PCWSTR pszName, const FILETIME *pftTimeStamp,
                      DWORD dwMode, HBITMAP hImage) PURE;
  STDMETHOD(GetEntry)(THIS_ PCWSTR pszName, DWORD dwMode, HBITMAP * phImage)
      PURE;
...
DECLARE_INTERFACE_IID_(IShellFolderBand, IUnknown,
...
enum { SFBID_PIDLCHANGED };
#undef INTERFACE
#define INTERFACE IDeskBarClient
DECLARE_INTERFACE_IID_(IDeskBarClient, IOleWindow,
...
#define COMP_ELEM_ALL                                                          \
  (COMP_ELEM_TYPE | COMP_ELEM_CHECKED | COMP_ELEM_DIRTY | COMP_ELEM_NOSCROLL | \
   COMP_ELEM_POS_LEFT | COMP_ELEM_SIZE_WIDTH | COMP_ELEM_SIZE_HEIGHT |         \
   COMP_ELEM_POS_ZINDEX | COMP_ELEM_SOURCE | COMP_ELEM_FRIENDLYNAME |          \
   COMP_ELEM_POS_TOP | COMP_ELEM_SUBSCRIBEDURL | COMP_ELEM_ORIGINAL_CSI |      \
...
DECLARE_INTERFACE_IID_(IActiveDesktop, IUnknown,
                       "f490eb00-1240-11d1-9888-006097deacf9") {
  STDMETHOD(QueryInterface)(THIS_ REFIID riid, void **ppv) PURE;
  STDMETHOD_(ULONG, AddRef)(THIS) PURE;
  STDMETHOD_(ULONG, Release)(THIS) PURE;
  STDMETHOD(ApplyChanges)(THIS_ DWORD dwFlags) PURE;
  STDMETHOD(GetWallpaper)(THIS_ PWSTR pwszWallpaper, UINT cchWallpaper,
                          DWORD dwFlags) PURE;
  ...
  STDMETHOD(SetWallpaperOptions)(THIS_ LPCWALLPAPEROPT pwpo, DWORD dwReserved)
      PURE;
  STDMETHOD(GetPattern)(THIS_ PWSTR pwszPattern, UINT cchPattern,
  ...
  STDMETHOD(AddDesktopItemWithUI)(THIS_ HWND hwnd, LPCOMPONENT pcomp,
                                  DWORD dwReserved) PURE;
  ...
  STDMETHOD(GetDesktopItem)(THIS_ int nComponent, LPCOMPONENT pcomp,
                            DWORD dwReserved) PURE;
  STDMETHOD(GetDesktopItemByID)(THIS_ ULONG_PTR dwID, LPCOMPONENT pcomp,
                                DWORD dwReserved) PURE;
  STDMETHOD(GenerateDesktopItemHtml)(THIS_ PCWSTR pwszFileName,
                                     LPCOMPONENT pcomp, DWORD dwReserved) PURE;
  STDMETHOD(AddUrl)(THIS_ HWND hwnd, PCWSTR pszSource, LPCOMPONENT pcomp,
                    DWORD dwFlags) PURE;
  STDMETHOD(GetDesktopItemBySource)(THIS_ PCWSTR pwszSource, LPCOMPONENT pcomp,
                                    DWORD dwReserved) PURE;
};
...
DECLARE_INTERFACE_IID_(IActiveDesktopP, IUnknown,
  ...
  STDMETHOD(GetScheme)(THIS_ PWSTR pwszSchemeName, DWORD * pdwcchBuffer,
                       DWORD dwFlags) PURE;
};
...
DECLARE_INTERFACE_IID_(IADesktopP2, IUnknown,
...
DECLARE_INTERFACE_IID_(IColumnProvider, IUnknown,
...
DECLARE_INTERFACE_IID_(IShellChangeNotify, IUnknown,
                       "D82BE2B1-5764-11D0-A96E-00C04FD705A2") {
  STDMETHOD(OnChange)(THIS_ LONG lEvent, PCIDLIST_ABSOLUTE pidl1,
                      PCIDLIST_ABSOLUTE pidl2) PURE;
};
#undef INTERFACE
#define INTERFACE IQueryInfo
DECLARE_INTERFACE_IID_(IQueryInfo, IUnknown,
...
SHSTDAPI_(ULONG)
SHChangeNotifyRegister(HWND hwnd, int fSources, LONG fEvents, UINT wMsg,
                       int cEntries, const SHChangeNotifyEntry *pshcne);
SHSTDAPI_(WINBOOL) SHChangeNotifyDeregister(ULONG ulID);
typedef enum { SCNRT_ENABLE = 0, SCNRT_DISABLE = 1 } SCNRT_STATUS;
#if NTDDI_VERSION >= 0x06000000
STDAPI_(void) SHChangeNotifyRegisterThread(SCNRT_STATUS status);
#endif
SHSTDAPI_(HANDLE)
SHChangeNotification_Lock(HANDLE hChange, DWORD dwProcId,
                          PIDLIST_ABSOLUTE **pppidl, LONG *plEvent);
SHSTDAPI_(WINBOOL) SHChangeNotification_Unlock(HANDLE hLock);
SHSTDAPI SHGetRealIDL(IShellFolder *psf, PCUITEMID_CHILD pidlSimple,
...
#include <poppack.h>
SHSTDAPI SHGetDataFromIDListA(IShellFolder *psf, PCUITEMID_CHILD pidl,
                              int nFormat, void *pv, int cb);
SHSTDAPI SHGetDataFromIDListW(IShellFolder *psf, PCUITEMID_CHILD pidl,
                              int nFormat, void *pv, int cb);
...
SHSTDAPI_(int)
RestartDialogEx(HWND hwnd, PCWSTR pszPrompt, DWORD dwReturn,
                DWORD dwReasonCode);
SHSTDAPI SHCoCreateInstance(PCWSTR pszCLSID, const CLSID *pclsid,
                            IUnknown *pUnkOuter, REFIID riid, void **ppv);
#if NTDDI_VERSION >= 0x06000000
SHSTDAPI SHCreateDataObject(PCIDLIST_ABSOLUTE pidlFolder, UINT cidl,
                            PCUITEMID_CHILD_ARRAY apidl, IDataObject *pdtInner,
                            REFIID riid, void **ppv);
#endif
SHSTDAPI CIDLData_CreateFromIDArray(PCIDLIST_ABSOLUTE pidlFolder, UINT cidl,
                                    PCUIDLIST_RELATIVE_ARRAY apidl,
                                    IDataObject **ppdtobj);
SHSTDAPI SHCreateStdEnumFmtEtc(UINT cfmt, const FORMATETC afmt[],
                               IEnumFORMATETC **ppenumFormatEtc);
SHSTDAPI SHDoDragDrop(HWND hwnd, IDataObject *pdata, IDropSource *pdsrc,
...
SHSTDAPI_(WINBOOL)
PathMakeUniqueName(PWSTR pszUniqueName, UINT cchMax, PCWSTR pszTemplate,
                   PCWSTR pszLongPlate, PCWSTR pszDir);
...
SHSTDAPI_(WINBOOL)
GetFileNameFromBrowse(HWND hwnd, PWSTR pszFilePath, UINT cchFilePath,
                      PCWSTR pszWorkingDir, PCWSTR pszDefExt, PCWSTR pszFilters,
                      PCWSTR pszTitle);
...
SHSTDAPI_(UINT)
Shell_MergeMenus(HMENU hmDst, HMENU hmSrc, UINT uInsert, UINT uIDAdjust,
                 UINT uIDAdjustMax, ULONG uFlags);
SHSTDAPI_(WINBOOL)
SHObjectProperties(HWND hwnd, DWORD shopObjectType, PCWSTR pszObjectName,
...
#endif
WINSHELLAPI HPSXA WINAPI SHCreatePropSheetExtArray(HKEY hKey, PCWSTR pszSubKey,
                                                   UINT max_iface);
WINSHELLAPI void WINAPI SHDestroyPropSheetExtArray(HPSXA hpsxa);
WINSHELLAPI UINT WINAPI SHAddFromPropSheetExtArray(
    HPSXA hpsxa, LPFNADDPROPSHEETPAGE lpfnAddPage, LPARAM lParam);
WINSHELLAPI UINT WINAPI SHReplaceFromPropSheetExtArray(
    HPSXA hpsxa, UINT uPageID, LPFNADDPROPSHEETPAGE lpfnReplaceWith,
    LPARAM lParam);
#if (NTDDI_VERSION >= 0x05000000 && NTDDI_VERSION < 0x06000000)
#undef INTERFACE
#define INTERFACE IDefViewFrame
DECLARE_INTERFACE_IID_(IDefViewFrame, IUnknown,
...
SHSTDAPI_(IStream *)
OpenRegStream(HKEY hkey, PCWSTR pszSubkey, PCWSTR pszValue, DWORD grfMode);
SHSTDAPI_(WINBOOL)
SHFindFiles(PCIDLIST_ABSOLUTE pidlFolder, PCIDLIST_ABSOLUTE pidlSaveFile);
SHSTDAPI_(void) PathGetShortPath(PWSTR pszLongPath);
SHSTDAPI_(WINBOOL)
PathYetAnotherMakeUniqueName(PWSTR pszUniqueName, PCWSTR pszPath,
                             PCWSTR pszShort, PCWSTR pszFileSpec);
...
SHSTDAPI_(LONG)
PathProcessCommand(PCWSTR pszSrc, PWSTR pszDest, int cchDest, DWORD dwFlags);
#endif
SHSTDAPI_(DWORD) SHRestricted(RESTRICTIONS rest);
SHSTDAPI_(WINBOOL) SignalFileOpen(PCIDLIST_ABSOLUTE pidl);
#if NTDDI_VERSION >= 0x06000000
SHSTDAPI AssocGetDetailsOfPropKey(IShellFolder *psf, PCUITEMID_CHILD pidl,
                                  const PROPERTYKEY *pkey, VARIANT *pv,
                                  WINBOOL *pfFoundPropKey);
#endif
#if NTDDI_VERSION < 0x06000000
SHSTDAPI SHLoadOLE(LPARAM lParam);
#endif
SHSTDAPI SHStartNetConnectionDialogA(HWND hwnd, LPCSTR pszRemoteName,
                                     DWORD dwType);
SHSTDAPI SHStartNetConnectionDialogW(HWND hwnd, LPCWSTR pszRemoteName,
                                     DWORD dwType);
...
SHSTDAPI_(int)
Shell_GetCachedImageIndex(PCWSTR pwszIconPath, int iIconIndex, UINT uIconFlags);
#if NTDDI_VERSION >= 0x06000000
SHSTDAPI_(int)
Shell_GetCachedImageIndexA(LPCSTR pszIconPath, int iIconIndex, UINT uIconFlags);
SHSTDAPI_(int)
Shell_GetCachedImageIndexW(LPCWSTR pszIconPath, int iIconIndex,
                           UINT uIconFlags);
#define Shell_GetCachedImageIndex __MINGW_NAME_AW(Shell_GetCachedImageIndex)
#endif
#undef INTERFACE
#define INTERFACE IDocViewSite
DECLARE_INTERFACE_IID_(IDocViewSite, IUnknown,
...
SHSTDAPI_(int)
PifMgr_GetProperties(HANDLE hProps, PCSTR pszGroup, void *lpProps, int cbProps,
                     UINT flOpt);
SHSTDAPI_(int)
PifMgr_SetProperties(HANDLE hProps, PCSTR pszGroup, const void *lpProps,
...
DECLARE_INTERFACE_IID_(IInitializeObject, IUnknown,
...
DECLARE_INTERFACE_IID_(IBanneredBar, IUnknown,
...
DECLARE_INTERFACE_IID_(IShellFolderViewCB, IUnknown,
...
DECLARE_INTERFACE_IID_(IShellFolderView, IUnknown,
  ...
  STDMETHOD(UpdateObject)(THIS_ PUITEMID_CHILD pidlOld, PUITEMID_CHILD pidlNew,
                          UINT * puItem) PURE;
  ...
  STDMETHOD(SetCallback)(THIS_ IShellFolderViewCB * pNewCB,
                         IShellFolderViewCB * *ppOldCB) PURE;
...
#if NTDDI_VERSION >= 0x06000000
SHSTDAPI SHCreateDefaultContextMenu(const DEFCONTEXTMENU *pdcm, REFIID riid,
                                    void **ppv);
#endif
SHSTDAPI_(WINBOOL)
SHOpenPropSheetA(LPCSTR pszCaption, HKEY ahkeys[], UINT ckeys,
                 const CLSID *pclsidDefault, IDataObject *pdtobj,
                 IShellBrowser *psb, LPCSTR pStartPage);
SHSTDAPI_(WINBOOL)
SHOpenPropSheetW(LPCWSTR pszCaption, HKEY ahkeys[], UINT ckeys,
                 const CLSID *pclsidDefault, IDataObject *pdtobj,
...
typedef HRESULT(CALLBACK *LPFNVIEWCALLBACK)(IShellView *psvOuter,
                                            IShellFolder *psf, HWND hwndMain,
...
#define ShellFolderView_ReArrange(_hwnd, _lparam)                              \
  (WINBOOL) SHShellFolderView_Message(_hwnd, SFVM_REARRANGE, _lparam)
...
#define ShellFolderView_GetSelectedObjects(_hwnd, ppidl)                       \
  (LPARAM) SHShellFolderView_Message(_hwnd, SFVM_GETSELECTEDOBJECTS,           \
                                     (LPARAM)(ppidl))
#define ShellFolderView_SetItemPos(_hwnd, _pidl, _x, _y)                       \
  {                                                                            \
    SFV_SETITEMPOS _sip = {_pidl, {_x, _y}};                                   \
    SHShellFolderView_Message(_hwnd, SFVM_SETITEMPOS,                          \
                              (LPARAM)(LPSFV_SETITEMPOS) & _sip);              \
  }
#define ShellFolderView_SetClipboard(_hwnd, _dwEffect)                         \
  (void)SHShellFolderView_Message(_hwnd, SFVM_SETCLIPBOARD,                    \
                                  (LPARAM)(DWORD)(_dwEffect))
...
SHFind_InitMenuPopup(HMENU hmenu, HWND hwndOwner, UINT idCmdFirst,
                     UINT idCmdLast);
...
SHSTDAPI SHBindToParent(PCIDLIST_ABSOLUTE pidl, REFIID riid, void **ppv,
                        PCUITEMID_CHILD *ppidlLast);
#if NTDDI_VERSION >= 0x06000000
SHSTDAPI SHBindToFolderIDListParent(IShellFolder *psfRoot,
                                    PCUIDLIST_RELATIVE pidl, REFIID riid,
                                    void **ppv, PCUITEMID_CHILD *ppidlLast);
SHSTDAPI SHBindToFolderIDListParentEx(IShellFolder *psfRoot,
                                      PCUIDLIST_RELATIVE pidl, IBindCtx *ppbc,
                                      REFIID riid, void **ppv,
                                      PCUITEMID_CHILD *ppidlLast);
SHSTDAPI SHBindToObject(IShellFolder *psf, PCUIDLIST_RELATIVE pidl,
                        IBindCtx *pbc, REFIID riid, void **ppv);
#endif
__forceinline WINBOOL IDListContainerIsConsistent(PCUIDLIST_RELATIVE p,
                                                  UINT sz) {
  UINT c = sizeof(p->mkid.cb);
  while (c <= sz && p->mkid.cb >= sizeof(p->mkid.cb) &&
         p->mkid.cb <= (sz - c)) {
    c += p->mkid.cb;
    p = ILNext(p);
  }
...
SHSTDAPI SHParseDisplayName(PCWSTR pszName, IBindCtx *pbc,
                            PIDLIST_ABSOLUTE *ppidl, SFGAOF sfgaoIn,
                            SFGAOF *psfgaoOut);
...
SHSTDAPI SHPathPrepareForWriteA(HWND hwnd, IUnknown *punkEnableModless,
                                LPCSTR pszPath, DWORD dwFlags);
SHSTDAPI SHPathPrepareForWriteW(HWND hwnd, IUnknown *punkEnableModless,
                                LPCWSTR pszPath, DWORD dwFlags);
#define SHPathPrepareForWrite __MINGW_NAME_AW(SHPathPrepareForWrite)
#undef INTERFACE
#define INTERFACE INamedPropertyBag
DECLARE_INTERFACE_IID_(INamedPropertyBag, IUnknown,
                       "FB700430-952C-11d1-946F-000000000000") {
  STDMETHOD(ReadPropertyNPB)(THIS_ PCWSTR pszBagname, PCWSTR pszPropName,
                             PROPVARIANT * pVar) PURE;
  STDMETHOD(WritePropertyNPB)(THIS_ PCWSTR pszBagname, PCWSTR pszPropName,
                              PROPVARIANT * pVar) PURE;
  STDMETHOD(RemovePropertyNPB)(THIS_ PCWSTR pszBagname, PCWSTR pszPropName)
      PURE;
};
#ifdef __urlmon_h__
SHDOCAPI_(DWORD)
SoftwareUpdateMessageBox(HWND hWnd, PCWSTR pszDistUnit, DWORD dwFlags,
                         LPSOFTDISTINFO psdi);
#endif
SHSTDAPI SHPropStgCreate(IPropertySetStorage *psstg, REFFMTID fmtid,
                         const CLSID *pclsid, DWORD grfFlags, DWORD grfMode,
                         DWORD dwDisposition, IPropertyStorage **ppstg,
                         UINT *puCodePage);
SHSTDAPI SHPropStgReadMultiple(IPropertyStorage *pps, UINT uCodePage,
                               ULONG cpspec, PROPSPEC const rgpspec[],
                               PROPVARIANT rgvar[]);
SHSTDAPI SHPropStgWriteMultiple(IPropertyStorage *pps, UINT *puCodePage,
                                ULONG cpspec, PROPSPEC const rgpspec[],
                                PROPVARIANT rgvar[], PROPID propidNameFirst);
SHSTDAPI SHCreateFileExtractIconA(LPCSTR pszFile, DWORD dwFileAttributes,
                                  REFIID riid, void **ppv);
SHSTDAPI SHCreateFileExtractIconW(LPCWSTR pszFile, DWORD dwFileAttributes,
                                  REFIID riid, void **ppv);
...
STDAPI SHGetAttributesFromDataObject(IDataObject *pdo, DWORD dwAttributeMask,
                                     DWORD *pdwAttributes, UINT *pcItems);
SHSTDAPI SHMultiFileProperties(IDataObject *pdtobj, DWORD dwFlags);
SHSTDAPI_(int)
SHMapPIDLToSystemImageListIndex(IShellFolder *pshf, PCUITEMID_CHILD pidl,
                                int *piIndexSel);
...
SHSTDAPI_(int)
PickIconDlg(HWND hwnd, PWSTR pszIconPath, UINT cchIconPath, int *piIconIndex);
#include <pshpack8.h>
...
#include <poppack.h>
#if NTDDI_VERSION >= 0x06010000
STDAPI StgMakeUniqueName(IStorage *pstgParent, PCWSTR pszFileSpec,
...
#if _WIN32_IE >= 0x0600
SHDOCAPI_(WINBOOL)
ImportPrivacySettings(PCWSTR pszFilename, WINBOOL *pfParsePrivacyPreferences,
                      WINBOOL *pfParsePerSiteRules);
#ifndef IEnumPrivacyRecords
typedef interface IEnumPrivacyRecords IEnumPrivacyRecords;
#endif
SHDOCAPI DoPrivacyDlg(HWND hwndOwner, PCWSTR pszUrl,
                      IEnumPrivacyRecords *pPrivacyEnum,
                      WINBOOL fReportAllSites);
#endif
#include <poppack.h>
#ifdef __cplusplus
}
