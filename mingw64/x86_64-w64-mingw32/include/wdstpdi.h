...
extern "C" {
#endif
/* 	Wdsmc.dll is missing an implib because Vista clients don't have the dll
 * to generate it from */
#ifndef WDSMCSAPI
...
PVOID WDSMCSAPI WdsTransportServerAllocateBuffer(HANDLE hProvider,
                                                 ULONG ulBufferSize);
HRESULT WDSMCSAPI WdsTransportServerCompleteRead(HANDLE hProvider,
                                                 ULONG ulBytesRead,
                                                 PVOID pvUserData,
                                                 HRESULT hReadResult);
HRESULT WDSMCSAPI WdsTransportServerFreeBuffer(HANDLE hProvider,
                                               PVOID pvBuffer);
HRESULT WDSMCSAPI WdsTransportServerRegisterCallback(
    HANDLE hProvider, TRANSPORTPROVIDER_CALLBACK_ID CallbackId,
    PVOID pfnCallback);
HRESULT WDSMCSAPI WdsTransportServerTraceV(HANDLE hProvider,
                                           WDS_MC_SEVERITY Severity,
                                           LPCWSTR pwszFormat, va_list Params);
HRESULT WDSMCSAPI WdsTransportServerTrace(HANDLE hProvider,
                                          WDS_MC_SEVERITY Severity,
                                          LPCWSTR pwszFormat);
#ifdef __cplusplus
}
