...
extern "C" {
#endif
HRESULT WINAPI HcsEnumerateComputeSystems(PCWSTR query,
...
HCS_OPERATION WINAPI HcsCreateOperationWithNotifications(
    HCS_OPERATION_OPTIONS eventTypes, const void *context,
...
HRESULT WINAPI HcsSetOperationContext(HCS_OPERATION operation,
                                      const void *context);
...
HRESULT WINAPI HcsGetOperationResult(HCS_OPERATION operation,
                                     PWSTR *resultDocument);
HRESULT WINAPI HcsGetOperationResultAndProcessInfo(
    HCS_OPERATION operation, HCS_PROCESS_INFORMATION *processInformation,
    PWSTR *resultDocument);
HRESULT WINAPI HcsAddResourceToOperation(HCS_OPERATION operation,
                                         HCS_RESOURCE_TYPE type, PCWSTR uri,
                                         HANDLE handle);
HRESULT WINAPI HcsGetProcessorCompatibilityFromSavedState(
    PCWSTR RuntimeFileName, PCWSTR *ProcessorFeaturesString);
HRESULT WINAPI HcsWaitForOperationResult(HCS_OPERATION operation,
                                         DWORD timeoutMs,
                                         PWSTR *resultDocument);
HRESULT WINAPI HcsWaitForOperationResultAndProcessInfo(
    HCS_OPERATION operation, DWORD timeoutMs,
    HCS_PROCESS_INFORMATION *processInformation, PWSTR *resultDocument);
HRESULT WINAPI HcsSetOperationCallback(HCS_OPERATION operation,
                                       const void *context,
                                       HCS_OPERATION_COMPLETION callback);
HRESULT WINAPI HcsCancelOperation(HCS_OPERATION operation);
HRESULT WINAPI HcsGetOperationProperties(HCS_OPERATION operation,
                                         PCWSTR options, PWSTR *resultDocument);
HRESULT WINAPI HcsCreateComputeSystem(
    PCWSTR id, PCWSTR configuration, HCS_OPERATION operation,
    const SECURITY_DESCRIPTOR *securityDescriptor, HCS_SYSTEM *computeSystem);
HRESULT WINAPI HcsCreateComputeSystemInNamespace(
    PCWSTR idNamespace, PCWSTR id, PCWSTR configuration,
...
HRESULT WINAPI HcsTerminateComputeSystem(HCS_SYSTEM computeSystem,
                                         HCS_OPERATION operation,
                                         PCWSTR options);
HRESULT WINAPI HcsCrashComputeSystem(HCS_SYSTEM computeSystem,
                                     HCS_OPERATION operation, PCWSTR options);
HRESULT WINAPI HcsPauseComputeSystem(HCS_SYSTEM computeSystem,
                                     HCS_OPERATION operation, PCWSTR options);
HRESULT WINAPI HcsResumeComputeSystem(HCS_SYSTEM computeSystem,
                                      HCS_OPERATION operation, PCWSTR options);
...
HRESULT WINAPI HcsModifyComputeSystem(HCS_SYSTEM computeSystem,
                                      HCS_OPERATION operation,
                                      PCWSTR configuration, HANDLE identity);
HRESULT WINAPI HcsWaitForComputeSystemExit(HCS_SYSTEM computeSystem,
                                           DWORD timeoutMs, PWSTR *result);
HRESULT WINAPI HcsSetComputeSystemCallback(HCS_SYSTEM computeSystem,
                                           HCS_EVENT_OPTIONS callbackOptions,
                                           const void *context,
                                           HCS_EVENT_CALLBACK callback);
HRESULT WINAPI HcsInitializeLiveMigrationOnSource(HCS_SYSTEM computeSystem,
                                                  HCS_OPERATION operation,
                                                  PCWSTR options);
HRESULT WINAPI HcsStartLiveMigrationOnSource(HCS_SYSTEM computeSystem,
                                             HCS_OPERATION operation,
                                             PCWSTR options);
HRESULT WINAPI HcsStartLiveMigrationTransfer(HCS_SYSTEM computeSystem,
                                             HCS_OPERATION operation,
                                             PCWSTR options);
HRESULT WINAPI HcsFinalizeLiveMigration(HCS_SYSTEM computeSystem,
                                        HCS_OPERATION operation,
                                        PCWSTR options);
HRESULT WINAPI HcsCreateProcess(HCS_SYSTEM computeSystem,
                                ...
                                HCS_PROCESS *process);
HRESULT WINAPI HcsOpenProcess(HCS_SYSTEM computeSystem, DWORD processId,
                              DWORD requestedAccess, HCS_PROCESS *process);
void WINAPI HcsCloseProcess(HCS_PROCESS process);
HRESULT WINAPI HcsTerminateProcess(HCS_PROCESS process, HCS_OPERATION operation,
                                   PCWSTR options);
HRESULT WINAPI HcsSignalProcess(HCS_PROCESS process, HCS_OPERATION operation,
                                PCWSTR options);
HRESULT WINAPI HcsGetProcessInfo(HCS_PROCESS process, HCS_OPERATION operation);
HRESULT WINAPI HcsGetProcessProperties(HCS_PROCESS process,
                                       HCS_OPERATION operation,
                                       PCWSTR propertyQuery);
HRESULT WINAPI HcsModifyProcess(HCS_PROCESS process, HCS_OPERATION operation,
                                PCWSTR settings);
HRESULT WINAPI HcsSetProcessCallback(HCS_PROCESS process,
                                     HCS_EVENT_OPTIONS callbackOptions,
                                     void *context,
                                     HCS_EVENT_CALLBACK callback);
HRESULT WINAPI HcsWaitForProcessExit(HCS_PROCESS computeSystem, DWORD timeoutMs,
                                     PWSTR *result);
