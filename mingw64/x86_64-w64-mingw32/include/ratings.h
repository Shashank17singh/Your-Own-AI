...
STDAPI RatingCheckUserAccess(LPCSTR pszUsername, LPCSTR pszURL,
                             LPCSTR pszRatingInfo, LPBYTE pData, DWORD cbData,
                             void **ppRatingDetails);
STDAPI RatingCheckUserAccessW(LPCWSTR pszUsername, LPCWSTR pszURL,
                              LPCWSTR pszRatingInfo, LPBYTE pData, DWORD cbData,
                              void **ppRatingDetails);
STDAPI RatingAccessDeniedDialog(HWND hDlg, LPCSTR pszUsername,
                                LPCSTR pszContentDescription,
                                void *pRatingDetails);
STDAPI RatingAccessDeniedDialog2(HWND hDlg, LPCSTR pszUsername,
                                 void *pRatingDetails);
STDAPI RatingAccessDeniedDialogW(HWND hDlg, LPCWSTR pszUsername,
...
STDAPI RatingObtainQuery(LPCSTR pszTargetUrl, DWORD dwUserData,
                         void (*fCallback)(DWORD dwUserData, HRESULT hr,
                                           LPCSTR pszRating,
...
#endif
STDAPI RatingAddToApprovedSites(HWND hDlg, DWORD cbPasswordBlob,
                                BYTE *pbPasswordBlob, LPCWSTR lpszUrl,
                                WINBOOL fAlwaysNever, WINBOOL fSitePage,
                                WINBOOL fApprovedSitesEnforced);
STDAPI RatingClickedOnPRFInternal(HWND hWndOwner, HINSTANCE, LPSTR lpszFileName,
                                  int nShow);
STDAPI RatingClickedOnRATInternal(HWND hWndOwner, HINSTANCE, LPSTR lpszFileName,
                                  int nShow);
...
static inline WINBOOL IS_RATINGS_ENABLED() {
  ...
  return (SHGetValue(HKEY_LOCAL_MACHINE,
                     TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Polici"
                          "es\\Ratings"),
                     TEXT("Key"), &typ, &sz, &sz) == ERROR_SUCCESS);
}
