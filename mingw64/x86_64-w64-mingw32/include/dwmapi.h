...
#endif
DWMAPI_(WINBOOL)
DwmDefWindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam,
                 LRESULT *plResult);
...
DWMAPI DwmGetWindowAttribute(HWND hwnd, DWORD dwAttribute, PVOID pvAttribute,
                             DWORD cbAttribute);
DWMAPI DwmIsCompositionEnabled(WINBOOL *pfEnabled);
DWMAPI DwmModifyPreviousDxFrameDuration(HWND hwnd, INT cRefreshes,
                                        WINBOOL fRelative);
DWMAPI DwmQueryThumbnailSourceSize(HTHUMBNAIL hThumbnail, PSIZE pSize);
DWMAPI DwmRegisterThumbnail(HWND hwndDestination, HWND hwndSource,
...
DWMAPI DwmSetWindowAttribute(HWND hwnd, DWORD dwAttribute, LPCVOID pvAttribute,
                             DWORD cbAttribute);
DWMAPI DwmUnregisterThumbnail(HTHUMBNAIL hThumbnailId);
DWMAPI
DwmUpdateThumbnailProperties(HTHUMBNAIL hThumbnailId,
...
DWMAPI DwmGetTransportAttributes(WINBOOL *pfIsRemoting, WINBOOL *pfIsConnected,
                                 DWORD *pDwGeneration);
DWMAPI DwmTransitionOwnedWindow(HWND hwnd,
                                enum DWMTRANSITION_OWNEDWINDOW_TARGET target);
#if _WIN32_WINNT >= 0x0601
DWMAPI DwmSetIconicThumbnail(HWND hwnd, HBITMAP hbmp, DWORD dwSITFlags);
DWMAPI DwmSetIconicLivePreviewBitmap(HWND hwnd, HBITMAP hbmp, POINT *pptClient,
                                     DWORD dwSITFlags);
DWMAPI DwmInvalidateIconicBitmaps(HWND hwnd);
#endif
#if NTDDI_VERSION >= NTDDI_WIN8
DWMAPI DwmRenderGesture(enum GESTURE_TYPE gt, UINT cContacts,
                        const DWORD *pdwPointerID, const POINT *pPoints);
...
#endif
#if NTDDI_VERSION >= NTDDI_WIN10_RS4
DWMAPI DwmGetUnmetTabRequirements(HWND appWindow,
                                  enum DWM_TAB_WINDOW_REQUIREMENTS *value);
#endif
#ifdef __cplusplus
}
#endif
