...
#define RSC_KERNEL_DISABLED                                                    \
  ((RSC_MODE_LY << RSC_MODE) | (RSC_BE_LITTLE << RSC_BE))
#define IFS_IFM 0
...
#define TR_VALUE(ed, ppn, ar, pl, d, a, ma, p)                                 \
  ((ed << TR_ED) | (ppn & IITR_ATTRIBUTE_PPN_MASK) | (ar << TR_AR) |           \
   (pl << TR_PL) | (d << TR_D) | (a << TR_A) | (ma << TR_MA) | (p << TR_P))
...
#define VALID_KERNEL_PTE                                                       \
  (PTE_VALID_MASK | PTE_ACCESS_MASK | PTE_WRITE_MASK | PTE_CACHE_MASK |        \
   PTE_DIRTY_MASK)
#define VALID_KERNEL_EXECUTE_PTE                                               \
  (PTE_VALID_MASK | PTE_ACCESS_MASK | PTE_EXECUTE_MASK | PTE_WRITE_MASK |      \
   PTE_CACHE_MASK | PTE_DIRTY_MASK | PTE_EXC_DEFER)
#define PTE_VALID 0
#define PTE_ACCESS 5
#define PTE_OWNER 7
#define PTE_WRITE 10
...
#define USER_RSC_INITIAL                                                       \
  ((RSC_MODE_LY << RSC_MODE) | (RSC_BE_LITTLE << RSC_BE) | (0x3 << RSC_PL))
