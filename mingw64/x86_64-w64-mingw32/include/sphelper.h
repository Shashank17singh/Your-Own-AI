...
#include <tchar.h>
#include <wchar.h>
inline HRESULT SpGetCategoryFromId(const WCHAR *category_id,
                                   ISpObjectTokenCategory **ret,
                                   BOOL fCreateIfNotExist = FALSE) {
  ISpObjectTokenCategory *obj_token_cat;
  HRESULT hres;
  hres = ::CoCreateInstance(CLSID_SpObjectTokenCategory, NULL, CLSCTX_ALL,
...
inline HRESULT SpEnumTokens(const WCHAR *category_id, const WCHAR *req_attrs,
                            const WCHAR *opt_attrs, IEnumSpObjectTokens **ret) {
...
inline void SpHexFromUlong(WCHAR *str, ULONG ul) { ::_ultow(ul, str, 16); }
inline HRESULT SpGetDescription(ISpObjectToken *obj_token, WCHAR **description,
