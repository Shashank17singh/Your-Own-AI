...
inline CONSTRUCTOR CdbVariant::CdbVariant(LPCTSTR pstr)
    : COleVariant(pstr, VT_BSTRT) {
  if (!pstr) {
    VariantClear(this);
    vt = VT_ERROR;
    scode = DISP_E_PARAMNOTFOUND;
...
inline CONSTRUCTOR CdbVariant::CdbVariant(SHORT s, WINBOOL bIsBool)
    : COleVariant(s) {
  if (bIsBool) {
    vt = VT_BOOL;
    boolVal = s;
...
public:
  ...
  DESTRUCTOR ~CdbWide() { SysFreeString(m_bstr); }
...
public:
  ...
  LPUNKNOWN GetInterface(WINBOOL bAddRef = FALSE,
                         WINBOOL bThrowException = TRUE) const;
  virtual VOID OnInterfaceChange(VOID);
  VOID SetRichErrorInfo(LPOLESTR pstrSource, LPOLESTR pstrDescription,
                        LPOLESTR pstrHelpFile, ULONG ulHelpID) const;
...
#define DAOMFC_STATIC_COLLECTION_IMPL(objColl, objSingle, intColl, intSingle)  \
  objSingle objColl::Item(LONG i) {                                            \
    return (intSingle *)(ObItem(i).GetInterface(TRUE));                        \
  }                                                                            \
  objSingle objColl::Item(LPCTSTR pstr) {                                      \
    return (intSingle *)(ObItem(pstr).GetInterface(TRUE));                     \
  }                                                                            \
  objSingle objColl::operator[](LONG i) {                                      \
    return (intSingle *)(Item(i).GetInterface(TRUE));                          \
  }                                                                            \
  objSingle objColl::operator[](LPCTSTR pstr) {                                \
    return (intSingle *)(Item(pstr).GetInterface(TRUE));                       \
  }
