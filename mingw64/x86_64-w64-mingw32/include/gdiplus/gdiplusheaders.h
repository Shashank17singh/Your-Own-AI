...
public:
  ...
  virtual Image *Clone() const {
    ...
    Status status =
        updateStatus(DllExports::GdipCloneImage(nativeImage, &cloneImage));
    if (status == Ok) {
  ...
  Status GetAllPropertyItems(UINT totalBufferSize, UINT numProperties,
...
public:
  ...
  static Bitmap *FromStream(IStream *stream,
                            BOOL useEmbeddedColorManagement = FALSE);
  static Status ApplyEffect(Bitmap **inputs, INT numInputs, Effect *effect,
                            RECT *ROI, RECT *outputRect, Bitmap **output);
  ...
  Bitmap *Clone(INT x, INT y, INT width, INT height, PixelFormat format) const;
  ...
  Status GetHistogram(HistogramFormat format, UINT numberOfEntries,
                      UINT *channel0, UINT *channel1, UINT *channel2,
                      UINT *channel3) const;
  Status GetHistogramSize(HistogramFormat format, UINT *numberOfEntries) const;
...
public:
  CustomLineCap(const GraphicsPath *fillPath, const GraphicsPath *strokePath,
                LineCap baseCap = LineCapFlat, REAL baseInset = 0.0f);
  ...
  virtual CustomLineCap *Clone() const {
    ...
    if (status == Ok) {
      CustomLineCap *result = new CustomLineCap(cloneCustomLineCap, lastStatus);
      if (!result) {
...
public:
  Font(const FontFamily *family, REAL emSize, INT style = FontStyleRegular,
       Unit unit = UnitPoint);
  ...
  Unit GetUnit() const;
  Status GetLastStatus() const { return lastStatus; }
  BOOL IsAvailable() const { return nativeFont != NULL; }

private:
  Font(GpFont *font, Status status) : nativeFont(font), lastStatus(status) {}
  Font(const Font &);
...
public:
  ...
  Status GetFamilies(INT numSought, FontFamily *families, INT *numFound) const;
...
private:
  ...
  Status updateStatus(Status newStatus) const { return lastStatus = newStatus; }
...
public:
  ...
  BOOL IsAvailable() const { return nativeFontFamily != NULL; }
...
public:
  ...
  BOOL IsVisible(REAL x, REAL y, const Graphics *graphics = NULL) const;
  BOOL IsVisible(INT x, INT y, const Graphics *graphics = NULL) const;
  BOOL IsVisible(const PointF &point, const Graphics *graphics = NULL) const;
  BOOL IsVisible(const Point &point, const Graphics *graphics = NULL) const;
  BOOL IsVisible(REAL x, REAL y, REAL width, REAL height,
                 const Graphics *graphics = NULL) const;
  BOOL IsVisible(INT x, INT y, INT width, INT height,
                 const Graphics *graphics = NULL) const;
  BOOL IsVisible(const RectF &rect, const Graphics *graphics = NULL) const;
  BOOL IsVisible(const Rect &rect, const Graphics *graphics = NULL) const;
  Status MakeEmpty();
  Status MakeInfinite();
