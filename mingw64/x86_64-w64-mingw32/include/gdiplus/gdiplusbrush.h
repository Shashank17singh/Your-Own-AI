...
public:
  HatchBrush(HatchStyle hatchStyle, const Color &foreColor,
    ...
    lastStatus = DllExports::GdipCreateHatchBrush(
        hatchStyle, foreColor.GetValue(), backColor.GetValue(), &nativeHatch);
  ...
  virtual HatchBrush *Clone() const {
    ...
    if (status == Ok) {
      HatchBrush *result = new HatchBrush(cloneBrush, lastStatus);
      if (!result) {
  ...
  Status GetBackgroundColor(Color *color) const {
    return updateStatus(DllExports::GdipGetHatchBackgroundColor(
        (GpHatch *)nativeBrush, color ? &color->Value : NULL));
  }
  ...
  HatchStyle GetHatchStyle() const {
    HatchStyle result;
    updateStatus(
        DllExports::GdipGetHatchStyle((GpHatch *)nativeBrush, &result));
    return result;
  }
...
public:
  ...
  Status GetInterpolationColors(Color *presetColors, REAL *blendPositions,
    ...
    Status status = updateStatus(DllExports::GdipGetLinePresetBlend(
        (GpLineGradient *)nativeBrush, presetArgb, blendPositions, count));
  ...
  Status GetTransform(Matrix *matrix) const {
    return updateStatus(DllExports::GdipGetLineTransform(
        (GpLineGradient *)nativeBrush, matrix ? matrix->nativeMatrix : NULL));
  }
  WrapMode GetWrapMode() const {
    WrapMode wrapMode = WrapModeTile;
    updateStatus(DllExports::GdipGetLineWrapMode((GpLineGradient *)nativeBrush,
                                                 &wrapMode));
    return wrapMode;
  }
...
public:
  SolidBrush(const Color &color) {
    ...
    lastStatus =
        DllExports::GdipCreateSolidFill(color.GetValue(), &nativeSolidFill);
  ...
  virtual SolidBrush *Clone() const {
    ...
    Status status =
        updateStatus(DllExports::GdipCloneBrush(nativeBrush, &cloneBrush));
    if (status == Ok) {
      SolidBrush *result = new SolidBrush(cloneBrush, lastStatus);
      if (!result) {
        DllExports::GdipDeleteBrush(cloneBrush);
        updateStatus(OutOfMemory);
      }
  ...
  Status GetColor(Color *color) const {
    return updateStatus(DllExports::GdipGetSolidFillColor(
        (GpSolidFill *)nativeBrush, color ? &color->Value : NULL));
  }
...
public:
  TextureBrush(Image *image, WrapMode wrapMode = WrapModeTile) {
    ...
    lastStatus = DllExports::GdipCreateTexture(
        image ? image->nativeImage : NULL, wrapMode, &nativeTexture);
    nativeBrush = nativeTexture;
  }
  TextureBrush(Image *image, WrapMode wrapMode, REAL dstX, REAL dstY,
               REAL dstWidth, REAL dstHeight) {
    GpTexture *nativeTexture = NULL;
    lastStatus = DllExports::GdipCreateTexture2(
        image ? image->nativeImage : NULL, wrapMode, dstX, dstY, dstWidth,
        dstHeight, &nativeTexture);
    nativeBrush = nativeTexture;
  }
  TextureBrush(Image *image, WrapMode wrapMode, INT dstX, INT dstY,
  ...
  TextureBrush(Image *image, WrapMode wrapMode, const RectF &dstRect) {
    GpTexture *nativeTexture = NULL;
    lastStatus = DllExports::GdipCreateTexture2(
        image ? image->nativeImage : NULL, wrapMode, dstRect.X, dstRect.Y,
  ...
  TextureBrush(Image *image, const RectF &dstRect,
    ...
    lastStatus = DllExports::GdipCreateTextureIA(
        ...
        dstRect.X, dstRect.Y, dstRect.Width, dstRect.Height, &nativeTexture);
    nativeBrush = nativeTexture;
  }
  ...
  virtual TextureBrush *Clone() const {
    ...
    Status status =
        updateStatus(DllExports::GdipCloneBrush(nativeBrush, &cloneBrush));
    if (status == Ok) {
      TextureBrush *result = new TextureBrush(cloneBrush, lastStatus);
      if (!result) {
        DllExports::GdipDeleteBrush(cloneBrush);
        updateStatus(OutOfMemory);
      }
  ...
  Status GetTransfrom(Matrix *matrix) const {
    return updateStatus(DllExports::GdipGetTextureTransform(
        (GpTexture *)nativeBrush, matrix ? matrix->nativeMatrix : NULL));
  }
  WrapMode GetWrapMode() const {
    WrapMode result = WrapModeTile;
    updateStatus(
        DllExports::GdipGetTextureWrapMode((GpTexture *)nativeBrush, &result));
    return result;
  }
  Status MultiplyTransform(const Matrix *matrix,
                           MatrixOrder order = MatrixOrderPrepend) {
    return updateStatus(DllExports::GdipMultiplyTextureTransform(
        (GpTexture *)nativeBrush, matrix ? matrix->nativeMatrix : NULL, order));
  }
  Status ResetTransform() {
    return updateStatus(
