...
public:
  ...
  GraphicsPath(const PointF *points, const BYTE *types, INT count,
      ...
      : nativePath(NULL), lastStatus(Ok) {
    lastStatus = DllExports::GdipCreatePath2(points, types, count, fillMode,
                                             &nativePath);
  }
  GraphicsPath(const Point *points, const BYTE *types, INT count,
               FillMode fillMode = FillModeAlternate)
      : nativePath(NULL), lastStatus(Ok) {
    lastStatus = DllExports::GdipCreatePath2I(points, types, count, fillMode,
                                              &nativePath);
  }
  ...
  GraphicsPath *Clone() const {
    ...
    Status status =
        updateStatus(DllExports::GdipClonePath(nativePath, &clonePath));
    if (status == Ok) {
  ...
  Status AddArc(REAL x, REAL y, REAL width, REAL height, REAL startAngle,
                REAL sweepAngle) {
    return updateStatus(DllExports::GdipAddPathArc(
        nativePath, x, y, width, height, startAngle, sweepAngle));
  }
  Status AddArc(INT x, INT y, INT width, INT height, REAL startAngle,
                REAL sweepAngle) {
    return updateStatus(DllExports::GdipAddPathArcI(
        nativePath, x, y, width, height, startAngle, sweepAngle));
  }
  Status AddArc(const RectF &rect, REAL startAngle, REAL sweepAngle) {
    return updateStatus(DllExports::GdipAddPathArc(nativePath, rect.X, rect.Y,
                                                   rect.Width, rect.Height,
                                                   startAngle, sweepAngle));
  }
  Status AddArc(const Rect &rect, REAL startAngle, REAL sweepAngle) {
    return updateStatus(DllExports::GdipAddPathArcI(nativePath, rect.X, rect.Y,
                                                    rect.Width, rect.Height,
                                                    startAngle, sweepAngle));
  }
  Status AddBezier(REAL x1, REAL y1, REAL x2, REAL y2, REAL x3, REAL y3,
                   REAL x4, REAL y4) {
    return updateStatus(DllExports::GdipAddPathBezier(nativePath, x1, y1, x2,
                                                      y2, x3, y3, x4, y4));
  }
  Status AddBezier(INT x1, INT y1, INT x2, INT y2, INT x3, INT y3, INT x4,
                   INT y4) {
    return updateStatus(DllExports::GdipAddPathBezierI(nativePath, x1, y1, x2,
                                                       y2, x3, y3, x4, y4));
  }
  Status AddBezier(const PointF &pt1, const PointF &pt2, const PointF &pt3,
                   const PointF &pt4) {
    return updateStatus(DllExports::GdipAddPathBezier(
        nativePath, pt1.X, pt1.Y, pt2.X, pt2.Y, pt3.X, pt3.Y, pt4.X, pt4.Y));
  }
  Status AddBezier(const Point &pt1, const Point &pt2, const Point &pt3,
                   const Point &pt4) {
    return updateStatus(DllExports::GdipAddPathBezierI(
        nativePath, pt1.X, pt1.Y, pt2.X, pt2.Y, pt3.X, pt3.Y, pt4.X, pt4.Y));
  }
  Status AddBeziers(const PointF *points, INT count) {
    return updateStatus(
        DllExports::GdipAddPathBeziers(nativePath, points, count));
  }
  ...
  Status AddClosedCurve(const PointF *points, INT count, REAL tension) {
    return updateStatus(DllExports::GdipAddPathClosedCurve2(nativePath, points,
                                                            count, tension));
  }
  Status AddClosedCurve(const Point *points, INT count, REAL tension) {
    return updateStatus(DllExports::GdipAddPathClosedCurve2I(nativePath, points,
                                                             count, tension));
  }
  Status AddCurve(const PointF *points, INT count) {
    return updateStatus(
        DllExports::GdipAddPathCurve(nativePath, points, count));
  }
  Status AddCurve(const Point *points, INT count) {
    return updateStatus(
        DllExports::GdipAddPathCurveI(nativePath, points, count));
  }
  ...
  Status AddEllipse(const RectF &rect) {
    return updateStatus(DllExports::GdipAddPathEllipse(
        nativePath, rect.X, rect.Y, rect.Width, rect.Height));
  }
  Status AddEllipse(const Rect &rect) {
    return updateStatus(DllExports::GdipAddPathEllipseI(
        nativePath, rect.X, rect.Y, rect.Width, rect.Height));
  }
  Status AddLine(REAL x1, REAL y1, REAL x2, REAL y2) {
    return updateStatus(
        DllExports::GdipAddPathLine(nativePath, x1, y1, x2, y2));
  }
  Status AddLine(INT x1, INT y1, INT x2, INT y2) {
    return updateStatus(
        DllExports::GdipAddPathLineI(nativePath, x1, y1, x2, y2));
  }
  Status AddLine(const PointF &pt1, const PointF &pt2) {
    return updateStatus(
        DllExports::GdipAddPathLine(nativePath, pt1.X, pt1.Y, pt2.X, pt2.Y));
  }
  Status AddLine(const Point &pt1, const Point &pt2) {
    return updateStatus(
        DllExports::GdipAddPathLineI(nativePath, pt1.X, pt1.Y, pt2.X, pt2.Y));
  }
  Status AddLines(const PointF *points, INT count) {
    return updateStatus(
        DllExports::GdipAddPathLine2(nativePath, points, count));
  }
  Status AddLines(const Point *points, INT count) {
    return updateStatus(
        DllExports::GdipAddPathLine2I(nativePath, points, count));
  }
  Status AddPath(const GraphicsPath *addingPath, BOOL connect) {
    return updateStatus(DllExports::GdipAddPathPath(
        nativePath, addingPath ? addingPath->nativePath : NULL, connect));
  }
  Status AddPie(REAL x, REAL y, REAL width, REAL height, REAL startAngle,
                REAL sweepAngle) {
    return updateStatus(DllExports::GdipAddPathPie(
        nativePath, x, y, width, height, startAngle, sweepAngle));
  }
  Status AddPie(INT x, INT y, INT width, INT height, REAL startAngle,
                REAL sweepAngle) {
    return updateStatus(DllExports::GdipAddPathPieI(
        nativePath, x, y, width, height, startAngle, sweepAngle));
  }
  Status AddPie(const RectF &rect, REAL startAngle, REAL sweepAngle) {
    return updateStatus(DllExports::GdipAddPathPie(nativePath, rect.X, rect.Y,
                                                   rect.Width, rect.Height,
                                                   startAngle, sweepAngle));
  }
  Status AddPie(const Rect &rect, REAL startAngle, REAL sweepAngle) {
    return updateStatus(DllExports::GdipAddPathPieI(nativePath, rect.X, rect.Y,
                                                    rect.Width, rect.Height,
                                                    startAngle, sweepAngle));
  }
  Status AddPolygon(const PointF *points, INT count) {
    return updateStatus(
        DllExports::GdipAddPathPolygon(nativePath, points, count));
  }
  ...
  Status AddRectangle(const RectF &rect) {
    return updateStatus(DllExports::GdipAddPathRectangle(
        nativePath, rect.X, rect.Y, rect.Width, rect.Height));
  }
  ...
  Status AddString(const WCHAR *string, INT length, const FontFamily *family,
                   INT style, REAL emSize, const PointF &origin,
                   const StringFormat *format) {
    RectF layoutRect(origin, SizeF(0.0f, 0.0f));
    return updateStatus(DllExports::GdipAddPathString(
        nativePath, string, length, family ? family->nativeFontFamily : NULL,
        style, emSize, &layoutRect,
        format ? format->nativeStringFormat : NULL));
  }
  Status AddString(const WCHAR *string, INT length, const FontFamily *family,
                   INT style, REAL emSize, const Point &origin,
                   const StringFormat *format) {
    Rect layoutRect(origin, Size(0, 0));
    return updateStatus(DllExports::GdipAddPathStringI(
        nativePath, string, length, family ? family->nativeFontFamily : NULL,
        style, emSize, &layoutRect,
        format ? format->nativeStringFormat : NULL));
  }
  Status AddString(const WCHAR *string, INT length, const FontFamily *family,
                   INT style, REAL emSize, const RectF &layoutRect,
                   const StringFormat *format) {
    return updateStatus(DllExports::GdipAddPathString(
        nativePath, string, length, family ? family->nativeFontFamily : NULL,
        style, emSize, &layoutRect,
  ...
  Status AddString(const WCHAR *string, INT length, const FontFamily *family,
                   INT style, REAL emSize, const Rect &layoutRect,
                   const StringFormat *format) {
    return updateStatus(DllExports::GdipAddPathStringI(
        nativePath, string, length, family ? family->nativeFontFamily : NULL,
        style, emSize, &layoutRect,
  ...
  Status ClearMarkers() {
    return updateStatus(DllExports::GdipClearPathMarkers(nativePath));
  }
  Status CloseAllFigures() {
    return updateStatus(DllExports::GdipClosePathFigures(nativePath));
  }
  Status CloseFigure() {
    return updateStatus(DllExports::GdipClosePathFigure(nativePath));
  }
  Status Flatten(const Matrix *matrix = NULL, REAL flatness = FlatnessDefault) {
    return updateStatus(DllExports::GdipFlattenPath(
        nativePath, matrix ? matrix->nativeMatrix : NULL, flatness));
  }
  Status GetBounds(RectF *bounds, const Matrix *matrix = NULL,
    ...
    return updateStatus(DllExports::GdipGetPathWorldBounds(
        nativePath, bounds, matrix ? matrix->nativeMatrix : NULL,
  ...
  FillMode GetFillMode() const {
    ...
    updateStatus(DllExports::GdipGetPathFillMode(nativePath, &result));
    return result;
  }
  Status GetLastPoint(PointF *lastPoint) const {
    return updateStatus(
        DllExports::GdipGetPathLastPoint(nativePath, lastPoint));
  }
  ...
  Status GetPathData(PathData *pathData) const {
    ...
    Status status;
    INT count;
    status = updateStatus(DllExports::GdipGetPointCount(nativePath, &count));
    if (status != Ok)
      return status;
    ...
    if (status != Ok)
      return status;
    return updateStatus(
        DllExports::GdipGetPathData(nativePath, (GpPathData *)pathData));
  }
  Status GetPathPoints(PointF *points, INT count) const {
    return updateStatus(
        DllExports::GdipGetPathPoints(nativePath, points, count));
  }
  Status GetPathPoints(Point *points, INT count) const {
    return updateStatus(
        DllExports::GdipGetPathPointsI(nativePath, points, count));
  }
  Status GetPathTypes(BYTE *types, INT count) const {
    return updateStatus(DllExports::GdipGetPathTypes(nativePath, types, count));
  }
  INT GetPointCount() const {
    INT result = 0;
    updateStatus(DllExports::GdipGetPointCount(nativePath, &result));
    return result;
  }
  ...
  Status Reset() { return updateStatus(DllExports::GdipResetPath(nativePath)); }
  Status Reverse() {
    return updateStatus(DllExports::GdipReversePath(nativePath));
  }
  Status SetFillMode(FillMode fillMode) {
    return updateStatus(DllExports::GdipSetPathFillMode(nativePath, fillMode));
  }
  Status SetMarker() {
    return updateStatus(DllExports::GdipSetPathMarker(nativePath));
  }
  Status StartFigure() {
    return updateStatus(DllExports::GdipStartPathFigure(nativePath));
  }
  Status Transform(const Matrix *matrix) {
    return updateStatus(DllExports::GdipTransformPath(
        nativePath, matrix ? matrix->nativeMatrix : NULL));
  }
  Status Warp(const PointF *destPoints, INT count, const RectF &srcRect,
    ...
    return updateStatus(DllExports::GdipWarpPath(
        nativePath, matrix ? matrix->nativeMatrix : NULL, destPoints, count,
        srcRect.X, srcRect.Y, srcRect.Width, srcRect.Height, warpMode,
        flatness));
  }
  Status Widen(const Pen *pen, const Matrix *matrix = NULL,
               REAL flatness = FlatnessDefault) {
    return updateStatus(DllExports::GdipWidenPath(
        nativePath, pen ? pen->nativePen : NULL,
        matrix ? matrix->nativeMatrix : NULL, flatness));
  }
...
public:
  GraphicsPathIterator(GraphicsPath *path)
    ...
    lastStatus = DllExports::GdipCreatePathIter(&nativePathIterator,
  ...
  INT CopyData(PointF *points, BYTE *types, INT startIndex, INT endIndex) {
    ...
    updateStatus(DllExports::GdipPathIterCopyData(
        nativePathIterator, &result, points, types, startIndex, endIndex));
    return result;
  }
  INT Enumerate(PointF *points, BYTE *types, INT count) {
    INT result = 0;
    updateStatus(DllExports::GdipPathIterEnumerate(nativePathIterator, &result,
                                                   points, types, count));
    return result;
  }
  INT GetCount() const {
    INT result = 0;
    updateStatus(DllExports::GdipPathIterGetCount(nativePathIterator, &result));
    return result;
  }
  Status GetLastStatus() const {
    Status result = lastStatus;
  ...
  INT GetSubpathCount() const {
    INT result = 0;
    updateStatus(
        DllExports::GdipPathIterGetSubpathCount(nativePathIterator, &result));
    return result;
  }
  BOOL HasCurve() const {
    BOOL result = FALSE;
    updateStatus(DllExports::GdipPathIterHasCurve(nativePathIterator, &result));
    return result;
  }
  INT NextMarker(INT *startIndex, INT *endIndex) {
    INT result = 0;
    updateStatus(DllExports::GdipPathIterNextMarker(nativePathIterator, &result,
                                                    startIndex, endIndex));
    return result;
  }
...
public:
  ...
  PathGradientBrush(const GraphicsPath *path) {
    ...
    lastStatus = DllExports::GdipCreatePathGradientFromPath(
        path ? path->nativePath : NULL, &nativePathGradient);
    nativeBrush = nativePathGradient;
  }
  virtual PathGradientBrush *Clone() const {
    GpBrush *cloneBrush = NULL;
    Status status =
        updateStatus(DllExports::GdipCloneBrush(nativeBrush, &cloneBrush));
    if (status == Ok) {
      PathGradientBrush *result = new PathGradientBrush(cloneBrush, lastStatus);
      if (!result) {
  ...
  Status GetCenterColor(Color *color) const {
    return updateStatus(DllExports::GdipGetPathGradientCenterColor(
        (GpPathGradient *)nativeBrush, color ? &color->Value : NULL));
  }
  ...
  Status GetInterpolationColors(Color *presetColors, REAL *blendPositions,
                                INT count) const {
    if (!presetColors || count <= 0)
      return lastStatus = InvalidParameter;
    ARGB *presetArgb = (ARGB *)DllExports::GdipAlloc(count * sizeof(ARGB));
    if (!presetArgb)
  ...
  Status GetSurroundColors(Color *colors, INT *count) {
    if (!colors || !count || *count <= 0)
      return lastStatus = InvalidParameter;
    ARGB *colorsArgb = (ARGB *)DllExports::GdipAlloc(*count * sizeof(ARGB));
    if (!colorsArgb)
      return lastStatus = OutOfMemory;
    Status status =
        updateStatus(DllExports::GdipGetPathGradientSurroundColorsWithCount(
            (GpPathGradient *)nativeBrush, colorsArgb, count));
    for (INT i = 0; i < *count; ++i) {
  ...
  Status GetTransform(Matrix *matrix) const {
    return updateStatus(DllExports::GdipGetPathGradientTransform(
        (GpPathGradient *)nativeBrush, matrix ? matrix->nativeMatrix : NULL));
  }
  ...
  Status SetInterpolationColors(const Color *presetColors, REAL *blendPositions,
                                INT count) {
    if (!presetColors || count <= 0)
      return lastStatus = InvalidParameter;
    ARGB *presetArgb = (ARGB *)DllExports::GdipAlloc(count * sizeof(ARGB));
  ...
  Status SetSurroundColors(const Color *colors, INT *count) {
    ...
    Status status =
        updateStatus(DllExports::GdipSetPathGradientSurroundColorsWithCount(
            (GpPathGradient *)nativeBrush, colorsArgb, count));
    DllExports::GdipFree((void *)colorsArgb);
    return status;
  }
