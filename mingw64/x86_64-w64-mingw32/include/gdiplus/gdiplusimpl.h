...
__inline__ Status Image::FindFirstItem(ImageItemData *item) {
  return updateStatus(DllExports::GdipFindFirstImageItem(nativeImage, item));
}
__inline__ Status Image::FindNextItem(ImageItemData *item) {
  return updateStatus(DllExports::GdipFindNextImageItem(nativeImage, item));
}
...
__inline__ UINT Image::GetFrameCount(const GUID *dimensionID) {
  UINT result = 0;
  updateStatus(
      DllExports::GdipImageGetFrameCount(nativeImage, dimensionID, &result));
  return result;
}
__inline__ UINT Image::GetFrameDimensionsCount() {
  UINT result = 0;
  updateStatus(
      DllExports::GdipImageGetFrameDimensionsCount(nativeImage, &result));
  return result;
}
__inline__ Status Image::GetFrameDimensionsList(GUID *dimensionIDs,
                                                UINT count) {
...
__inline__ REAL Image::GetHorizontalResolution() {
  REAL result = 0.0f;
  updateStatus(
      DllExports::GdipGetImageHorizontalResolution(nativeImage, &result));
  return result;
}
__inline__ Status Image::GetItemData(ImageItemData *item) {
  return updateStatus(DllExports::GdipGetImageItemData(nativeImage, item));
}
__inline__ Status Image::GetPalette(ColorPalette *palette, INT size) {
  return updateStatus(
      DllExports::GdipGetImagePalette(nativeImage, palette, size));
}
...
__inline__ UINT Image::GetPropertyItemSize(PROPID propId) {
  UINT result = 0;
  updateStatus(
      DllExports::GdipGetPropertyItemSize(nativeImage, propId, &result));
  return result;
}
__inline__ Status Image::GetPropertySize(UINT *totalBufferSize,
                                         UINT *numProperties) {
...
__inline__ Status Image::GetRawFormat(GUID *format) {
  return updateStatus(DllExports::GdipGetImageRawFormat(nativeImage, format));
}
__inline__ Image *Image::GetThumbnailImage(UINT thumbWidth, UINT thumbHeight,
  ...
  Status status = updateStatus(
      DllExports::GdipGetImageThumbnail(nativeImage, thumbWidth, thumbHeight,
...
__inline__ REAL Image::GetVerticalResolution() {
  REAL result = 0.0f;
  updateStatus(
      DllExports::GdipGetImageVerticalResolution(nativeImage, &result));
  return result;
}
...
__inline__ Status Image::RemovePropertyItem(PROPID propId) {
  return updateStatus(DllExports::GdipRemovePropertyItem(nativeImage, propId));
}
...
__inline__ Status Image::SaveAdd(const EncoderParameters *encoderParams) {
  return updateStatus(DllExports::GdipSaveAdd(nativeImage, encoderParams));
}
__inline__ Status Image::SaveAdd(Image *newImage,
                                 const EncoderParameters *encoderParams) {
  return updateStatus(DllExports::GdipSaveAddImage(
      nativeImage, newImage ? newImage->nativeImage : NULL, encoderParams));
}
__inline__ Status Image::SelectActiveFrame(const GUID *dimensionID,
                                           UINT frameIndex) {
...
__inline__ Status Image::SetAbort(GdiplusAbort *pIAbort) {
  return updateStatus(DllExports::GdipImageSetAbort(nativeImage, pIAbort));
}
__inline__ Status Image::SetPalette(const ColorPalette *palette) {
  return updateStatus(DllExports::GdipSetImagePalette(nativeImage, palette));
}
...
__inline__ Bitmap *
Bitmap::FromDirectDrawSurface7(IDirectDrawSurface7 *surface) {
...
__inline__ Bitmap *Bitmap::FromHICON(HICON icon) { return new Bitmap(icon); }
__inline__ Bitmap *Bitmap::FromResource(HINSTANCE hInstance,
...
__inline__ Status Bitmap::InitializePalette(ColorPalette *palette,
  ...
  return DllExports::GdipInitializePalette(
      palette, paletteType, optimalColors, useTransparentColor,
...
__inline__ Bitmap::Bitmap(IDirectDrawSurface7 *surface) : Image(NULL, Ok) {
  GpBitmap *nativeBitmap = NULL;
  lastStatus =
      DllExports::GdipCreateBitmapFromDirectDrawSurface(surface, &nativeBitmap);
  nativeImage = nativeBitmap;
}
__inline__ Bitmap::Bitmap(const WCHAR *filename,
  ...
  if (useEmbeddedColorManagement) {
    lastStatus =
        DllExports::GdipCreateBitmapFromFileICM(filename, &nativeBitmap);
  } else {
    lastStatus = DllExports::GdipCreateBitmapFromFile(filename, &nativeBitmap);
  }
  nativeImage = nativeBitmap;
}
__inline__ Bitmap::Bitmap(HBITMAP hbm, HPALETTE hpal) : Image(NULL, Ok) {
  GpBitmap *nativeBitmap = NULL;
  lastStatus =
      DllExports::GdipCreateBitmapFromHBITMAP(hbm, hpal, &nativeBitmap);
  nativeImage = nativeBitmap;
}
__inline__ Bitmap::Bitmap(HICON hicon) : Image(NULL, Ok) {
  GpBitmap *nativeBitmap = NULL;
...
__inline__ Bitmap::Bitmap(HINSTANCE hInstance, const WCHAR *bitmapName)
    : Image(NULL, Ok) {
  GpBitmap *nativeBitmap = NULL;
  lastStatus = DllExports::GdipCreateBitmapFromResource(hInstance, bitmapName,
                                                        &nativeBitmap);
  nativeImage = nativeBitmap;
}
__inline__ Bitmap::Bitmap(IStream *stream, BOOL useEmbeddedColorManagement)
  ...
  if (useEmbeddedColorManagement) {
    lastStatus =
        DllExports::GdipCreateBitmapFromStreamICM(stream, &nativeBitmap);
  } else {
    lastStatus = DllExports::GdipCreateBitmapFromStream(stream, &nativeBitmap);
  }
  nativeImage = nativeBitmap;
}
__inline__ Bitmap::Bitmap(INT width, INT height, Graphics *target)
  ...
  lastStatus = DllExports::GdipCreateBitmapFromGraphics(
      width, height, target ? target->nativeGraphics : NULL, &nativeBitmap);
  nativeImage = nativeBitmap;
}
__inline__ Bitmap::Bitmap(INT width, INT height, PixelFormat format)
    : Image(NULL, Ok) {
  GpBitmap *nativeBitmap = NULL;
  lastStatus = DllExports::GdipCreateBitmapFromScan0(width, height, 0, format,
                                                     NULL, &nativeBitmap);
  nativeImage = nativeBitmap;
}
__inline__ Bitmap::Bitmap(INT width, INT height, INT stride, PixelFormat format,
                          BYTE *scan0)
    : Image(NULL, Ok) {
  GpBitmap *nativeBitmap = NULL;
...
__inline__ Bitmap *Bitmap::Clone(REAL x, REAL y, REAL width, REAL height,
  ...
  Status status = updateStatus(DllExports::GdipCloneBitmapArea(
      x, y, width, height, format, (GpBitmap *)nativeImage, &cloneBitmap));
  if (status == Ok) {
...
__inline__ Status Bitmap::ConvertFormat(PixelFormat format,
  ...
  return updateStatus(DllExports::GdipBitmapConvertFormat(
      (GpBitmap *)nativeImage, format, ditherType, paletteType, palette,
      alphaThresholdPercent));
}
__inline__ Status Bitmap::GetHBITMAP(const Color &colorBackground,
                                     HBITMAP *hbmReturn) const {
  return updateStatus(DllExports::GdipCreateHBITMAPFromBitmap(
      (GpBitmap *)nativeImage, hbmReturn, colorBackground.GetValue()));
}
__inline__ Status Bitmap::GetHICON(HICON *icon) const {
  return updateStatus(
      DllExports::GdipCreateHICONFromBitmap((GpBitmap *)nativeImage, icon));
}
__inline__ Status Bitmap::GetHistogram(HistogramFormat format,
                                       UINT numberOfEntries, UINT *channel0,
                                       UINT *channel1, UINT *channel2,
                                       UINT *channel3) const {
  return updateStatus(DllExports::GdipBitmapGetHistogram(
      (GpBitmap *)nativeImage, format, numberOfEntries, channel0, channel1,
      channel2, channel3));
}
__inline__ Status Bitmap::GetHistogramSize(HistogramFormat format,
                                           UINT *numberOfEntries) const {
  return updateStatus(
      DllExports::GdipBitmapGetHistogramSize(format, numberOfEntries));
}
__inline__ Status Bitmap::GetPixel(INT x, INT y, Color *color) const {
  return updateStatus(DllExports::GdipBitmapGetPixel(
      (GpBitmap *)nativeImage, x, y, color ? &color->Value : NULL));
}
...
__inline__ Status Bitmap::SetPixel(INT x, INT y, const Color &color) {
  return updateStatus(DllExports::GdipBitmapSetPixel((GpBitmap *)nativeImage, x,
                                                     y, color.GetValue()));
}
__inline__ Status Bitmap::SetResolution(REAL xdpi, REAL ydpi) {
  return updateStatus(
      DllExports::GdipBitmapSetResolution((GpBitmap *)nativeImage, xdpi, ydpi));
}
__inline__ Status Bitmap::UnlockBits(BitmapData *lockedBitmapData) {
  return updateStatus(DllExports::GdipBitmapUnlockBits((GpBitmap *)nativeImage,
                                                       lockedBitmapData));
}
__inline__ CachedBitmap::CachedBitmap(Bitmap *bitmap, Graphics *graphics)
  ...
  lastStatus = DllExports::GdipCreateCachedBitmap(
      bitmap ? ((GpBitmap *)bitmap->nativeImage) : NULL,
      graphics ? graphics->nativeGraphics : NULL, &nativeCachedBitmap);
}
__inline__ CachedBitmap::~CachedBitmap() {
  DllExports::GdipDeleteCachedBitmap(nativeCachedBitmap);
}
__inline__ CustomLineCap::CustomLineCap(const GraphicsPath *fillPath,
  ...
  lastStatus = DllExports::GdipCreateCustomLineCap(
      ...
      strokePath ? strokePath->nativePath : NULL, baseCap, baseInset,
      &nativeCustomLineCap);
}
__inline__ LineCap CustomLineCap::GetBaseCap() const {
  LineCap result = LineCapFlat;
  updateStatus(
      DllExports::GdipGetCustomLineCapBaseCap(nativeCustomLineCap, &result));
  return result;
}
__inline__ REAL CustomLineCap::GetBaseInset() const {
  REAL result = 0.0f;
  updateStatus(
      DllExports::GdipGetCustomLineCapBaseInset(nativeCustomLineCap, &result));
  return result;
}
...
__inline__ LineJoin CustomLineCap::GetStrokeJoin() const {
  ...
  updateStatus(
      DllExports::GdipGetCustomLineCapStrokeJoin(nativeCustomLineCap, &result));
  return result;
}
__inline__ REAL CustomLineCap::GetWidthScale() const {
  REAL result = 0.0f;
  updateStatus(
      DllExports::GdipGetCustomLineCapWidthScale(nativeCustomLineCap, &result));
  return result;
}
__inline__ Status CustomLineCap::SetBaseCap(LineCap baseCap) {
  return updateStatus(
      DllExports::GdipSetCustomLineCapBaseCap(nativeCustomLineCap, baseCap));
}
__inline__ Status CustomLineCap::SetBaseInset(REAL inset) {
  return updateStatus(
      DllExports::GdipSetCustomLineCapBaseInset(nativeCustomLineCap, inset));
}
...
__inline__ Status CustomLineCap::SetStrokeCaps(LineCap startCap,
                                               LineCap endCap) {
  return updateStatus(DllExports::GdipSetCustomLineCapStrokeCaps(
      nativeCustomLineCap, startCap, endCap));
}
...
__inline__ Font::Font(const FontFamily *family, REAL emSize, INT style,
                      Unit unit)
    : nativeFont(NULL), lastStatus(Ok) {
  lastStatus =
      DllExports::GdipCreateFont(family ? family->nativeFontFamily : NULL,
                                 emSize, style, unit, &nativeFont);
}
__inline__ Font::Font(HDC hdc, HFONT hfont) : nativeFont(NULL), lastStatus(Ok) {
  LOGFONTA logfont;
  if (hfont && GetObjectA(hfont, sizeof logfont, &logfont)) {
    lastStatus =
        DllExports::GdipCreateFontFromLogfontA(hdc, &logfont, &nativeFont);
  } else {
...
__inline__ Font::Font(HDC hdc, const LOGFONTW *logfont)
    : nativeFont(NULL), lastStatus(Ok) {
  lastStatus =
      DllExports::GdipCreateFontFromLogfontW(hdc, logfont, &nativeFont);
}
__inline__ Font::Font(HDC hdc) : nativeFont(NULL), lastStatus(Ok) {
  lastStatus = DllExports::GdipCreateFontFromDC(hdc, &nativeFont);
}
__inline__ Font::Font(const WCHAR *familyName, REAL emSize, INT style,
  ...
  lastStatus = DllExports::GdipCreateFontFamilyFromName(
      familyName, fontCollection ? fontCollection->nativeFontCollection : NULL,
      &nativeFamily);
  if (nativeFamily) {
    lastStatus = DllExports::GdipCreateFont(nativeFamily, emSize, style, unit,
                                            &nativeFont);
    DllExports::GdipDeleteFontFamily(nativeFamily);
  }
}
__inline__ Font::~Font() { DllExports::GdipDeleteFont(nativeFont); }
__inline__ Font *Font::Clone() const {
  GpFont *cloneFont = NULL;
  Status status =
      updateStatus(DllExports::GdipCloneFont(nativeFont, &cloneFont));
  if (status == Ok) {
...
__inline__ Status Font::GetFamily(FontFamily *family) const {
  if (!family)
    return lastStatus = InvalidParameter;
  return family->lastStatus = updateStatus(
             DllExports::GdipGetFamily(nativeFont, &family->nativeFontFamily));
}
__inline__ REAL Font::GetHeight(const Graphics *graphics) const {
  REAL result = 0.0f;
  updateStatus(DllExports::GdipGetFontHeight(
      nativeFont, graphics ? graphics->nativeGraphics : NULL, &result));
  return result;
}
__inline__ REAL Font::GetHeight(REAL dpi) const {
  REAL result = 0.0f;
  updateStatus(DllExports::GdipGetFontHeightGivenDPI(nativeFont, dpi, &result));
  return result;
}
__inline__ Status Font::GetLogFontA(const Graphics *graphics,
                                    LOGFONTA *logfontA) const {
  return updateStatus(DllExports::GdipGetLogFontA(
      nativeFont, graphics ? graphics->nativeGraphics : NULL, logfontA));
}
__inline__ Status Font::GetLogFontW(const Graphics *graphics,
                                    LOGFONTW *logfontW) const {
  return updateStatus(DllExports::GdipGetLogFontW(
      nativeFont, graphics ? graphics->nativeGraphics : NULL, logfontW));
}
...
__inline__ Status FontCollection::GetFamilies(INT numSought,
  ...
  *numFound = 0;
  GpFontFamily **nativeFamilyArray = (GpFontFamily **)DllExports::GdipAlloc(
      numSought * sizeof(GpFontFamily *));
  if (!nativeFamilyArray)
    return lastStatus = OutOfMemory;
  Status status = updateStatus(DllExports::GdipGetFontCollectionFamilyList(
      nativeFontCollection, numSought, nativeFamilyArray, numFound));
  if (status == Ok) {
    for (int i = 0; i < *numFound; ++i) {
      families[i].lastStatus = updateStatus(DllExports::GdipCloneFontFamily(
          nativeFamilyArray[i], &families[i].nativeFontFamily));
    }
  }
...
__inline__ const FontFamily *FontFamily::GenericMonospace() {
  if (!_GdipFontFamilyCachedGenericMonospace) {
    ...
    Status status =
        DllExports::GdipGetGenericFontFamilyMonospace(&nativeFontFamily);
    if (status == Ok && nativeFontFamily) {
      _GdipFontFamilyCachedGenericMonospace =
          (void *)new FontFamily(nativeFontFamily, Ok);
    }
  }
...
__inline__ const FontFamily *FontFamily::GenericSansSerif() {
  if (!_GdipFontFamilyCachedGenericSansSerif) {
    GpFontFamily *nativeFontFamily = 0;
    Status status =
        DllExports::GdipGetGenericFontFamilySansSerif(&nativeFontFamily);
    if (status == Ok && nativeFontFamily) {
      _GdipFontFamilyCachedGenericSansSerif =
          (void *)new FontFamily(nativeFontFamily, Ok);
    }
  }
...
__inline__ const FontFamily *FontFamily::GenericSerif() {
  if (!_GdipFontFamilyCachedGenericSerif) {
    GpFontFamily *nativeFontFamily = 0;
    Status status =
        DllExports::GdipGetGenericFontFamilySerif(&nativeFontFamily);
    if (status == Ok && nativeFontFamily) {
      _GdipFontFamilyCachedGenericSerif =
          (void *)new FontFamily(nativeFontFamily, Ok);
    }
  }
  return (FontFamily *)_GdipFontFamilyCachedGenericSerif;
}
__inline__ FontFamily::FontFamily() : nativeFontFamily(NULL), lastStatus(Ok) {}
...
__inline__ FontFamily *FontFamily::Clone() const {
  ...
  Status status = updateStatus(
      DllExports::GdipCloneFontFamily(nativeFontFamily, &cloneFontFamily));
  if (status == Ok) {
...
__inline__ UINT16 FontFamily::GetCellAscent(INT style) const {
  UINT16 result = 0;
  updateStatus(DllExports::GdipGetCellAscent(nativeFontFamily, style, &result));
  return result;
}
__inline__ UINT16 FontFamily::GetCellDescent(INT style) const {
  UINT16 result = 0;
  updateStatus(
      DllExports::GdipGetCellDescent(nativeFontFamily, style, &result));
  return result;
}
__inline__ UINT16 FontFamily::GetEmHeight(INT style) const {
  UINT16 result = 0;
  updateStatus(DllExports::GdipGetEmHeight(nativeFontFamily, style, &result));
  return result;
}
__inline__ Status FontFamily::GetFamilyName(WCHAR name[LF_FACESIZE],
                                            LANGID language) const {
  return updateStatus(
      DllExports::GdipGetFamilyName(nativeFontFamily, name, language));
}
__inline__ UINT16 FontFamily::GetLineSpacing(INT style) const {
  UINT16 result = 0;
  updateStatus(
      DllExports::GdipGetLineSpacing(nativeFontFamily, style, &result));
  return result;
}
__inline__ BOOL FontFamily::IsStyleAvailable(INT style) const {
  BOOL result = FALSE;
  updateStatus(
      DllExports::GdipIsStyleAvailable(nativeFontFamily, style, &result));
  return result;
}
...
__inline__ Status PrivateFontCollection::AddMemoryFont(const VOID *memory,
                                                       INT length) {
  return updateStatus(DllExports::GdipPrivateAddMemoryFont(nativeFontCollection,
                                                           memory, length));
}
__inline__ Region *Region::FromHRGN(HRGN hrgn) { return new Region(hrgn); }
__inline__ Region::Region() : nativeRegion(NULL), lastStatus(Ok) {
  lastStatus = DllExports::GdipCreateRegion(&nativeRegion);
}
__inline__ Region::Region(const RectF &rect)
    : nativeRegion(NULL), lastStatus(Ok) {
  lastStatus = DllExports::GdipCreateRegionRect(&rect, &nativeRegion);
}
__inline__ Region::Region(const Rect &rect)
    : nativeRegion(NULL), lastStatus(Ok) {
  lastStatus = DllExports::GdipCreateRegionRectI(&rect, &nativeRegion);
}
__inline__ Region::Region(const GraphicsPath *path)
    : nativeRegion(NULL), lastStatus(Ok) {
  lastStatus = DllExports::GdipCreateRegionPath(path ? path->nativePath : NULL,
                                                &nativeRegion);
}
__inline__ Region::Region(const BYTE *regionData, INT size)
    : nativeRegion(NULL), lastStatus(Ok) {
  lastStatus =
      DllExports::GdipCreateRegionRgnData(regionData, size, &nativeRegion);
}
...
__inline__ Region::~Region() { DllExports::GdipDeleteRegion(nativeRegion); }
__inline__ Region *Region::Clone() const {
  GpRegion *cloneRegion = NULL;
  Status status =
      updateStatus(DllExports::GdipCloneRegion(nativeRegion, &cloneRegion));
  if (status == Ok) {
...
__inline__ Status Region::Complement(const RectF &rect) {
  return updateStatus(DllExports::GdipCombineRegionRect(nativeRegion, &rect,
...
__inline__ Status Region::Complement(const GraphicsPath *path) {
  return updateStatus(DllExports::GdipCombineRegionPath(
      nativeRegion, path ? path->nativePath : NULL, CombineModeComplement));
}
__inline__ BOOL Region::Equals(const Region *region,
...
__inline__ Status Region::Exclude(const Rect &rect) {
  return updateStatus(DllExports::GdipCombineRegionRectI(nativeRegion, &rect,
                                                         CombineModeExclude));
}
__inline__ Status Region::Exclude(const Region *region) {
  return updateStatus(DllExports::GdipCombineRegionRegion(
      nativeRegion, region ? region->nativeRegion : NULL, CombineModeExclude));
}
__inline__ Status Region::Exclude(const GraphicsPath *path) {
  return updateStatus(DllExports::GdipCombineRegionPath(
      nativeRegion, path ? path->nativePath : NULL, CombineModeExclude));
}
__inline__ Status Region::GetBounds(RectF *rect,
                                    const Graphics *graphics) const {
  return updateStatus(DllExports::GdipGetRegionBounds(
      nativeRegion, graphics ? graphics->nativeGraphics : NULL, rect));
}
__inline__ Status Region::GetBounds(Rect *rect,
                                    const Graphics *graphics) const {
  return updateStatus(DllExports::GdipGetRegionBoundsI(
      nativeRegion, graphics ? graphics->nativeGraphics : NULL, rect));
}
__inline__ Status Region::GetData(BYTE *buffer, UINT bufferSize,
                                  UINT *sizeFilled) const {
  return updateStatus(DllExports::GdipGetRegionData(nativeRegion, buffer,
...
__inline__ HRGN Region::GetHRGN(const Graphics *graphics) const {
  HRGN result = NULL;
  updateStatus(DllExports::GdipGetRegionHRgn(
      nativeRegion, graphics ? graphics->nativeGraphics : NULL, &result));
  return result;
}
__inline__ Status Region::GetRegionScans(const Matrix *matrix, RectF *rects,
                                         INT *count) const {
  return updateStatus(DllExports::GdipGetRegionScans(
      nativeRegion, rects, count, matrix ? matrix->nativeMatrix : NULL));
}
__inline__ Status Region::GetRegionScans(const Matrix *matrix, Rect *rects,
                                         INT *count) const {
...
__inline__ UINT Region::GetRegionScansCount(const Matrix *matrix) const {
  ...
  updateStatus(DllExports::GdipGetRegionScansCount(
      nativeRegion, &result, matrix ? matrix->nativeMatrix : NULL));
  return result;
}
...
__inline__ Status Region::Intersect(const GraphicsPath *path) {
  return updateStatus(DllExports::GdipCombineRegionPath(
      nativeRegion, path ? path->nativePath : NULL, CombineModeIntersect));
}
__inline__ BOOL Region::IsEmpty(const Graphics *graphics) const {
  BOOL result = FALSE;
  updateStatus(DllExports::GdipIsEmptyRegion(
      nativeRegion, graphics ? graphics->nativeGraphics : NULL, &result));
  return result;
}
__inline__ BOOL Region::IsInfinite(const Graphics *graphics) const {
  BOOL result = FALSE;
  updateStatus(DllExports::GdipIsInfiniteRegion(
      nativeRegion, graphics ? graphics->nativeGraphics : NULL, &result));
  return result;
}
__inline__ BOOL Region::IsVisible(REAL x, REAL y,
  ...
  updateStatus(DllExports::GdipIsVisibleRegionPoint(
      nativeRegion, x, y, graphics ? graphics->nativeGraphics : NULL, &result));
  return result;
}
...
__inline__ Status Region::Translate(REAL dx, REAL dy) {
  return updateStatus(DllExports::GdipTranslateRegion(nativeRegion, dx, dy));
}
__inline__ Status Region::Translate(INT dx, INT dy) {
  return updateStatus(DllExports::GdipTranslateRegionI(nativeRegion, dx, dy));
}
...
__inline__ Status Region::Union(const Region *region) {
  return updateStatus(DllExports::GdipCombineRegionRegion(
      nativeRegion, region ? region->nativeRegion : NULL, CombineModeUnion));
}
__inline__ Status Region::Union(const GraphicsPath *path) {
  return updateStatus(DllExports::GdipCombineRegionPath(
      nativeRegion, path ? path->nativePath : NULL, CombineModeUnion));
}
...
__inline__ Status Region::Xor(const Region *region) {
  return updateStatus(DllExports::GdipCombineRegionRegion(
      nativeRegion, region ? region->nativeRegion : NULL, CombineModeXor));
}
__inline__ Status Region::Xor(const GraphicsPath *path) {
  return updateStatus(DllExports::GdipCombineRegionPath(
      nativeRegion, path ? path->nativePath : NULL, CombineModeXor));
}
...
__inline__ BOOL GraphicsPath::IsOutlineVisible(const PointF &point,
                                               const Pen *pen,
                                               const Graphics *g) const {
  BOOL result = FALSE;
  updateStatus(DllExports::GdipIsOutlineVisiblePathPoint(
      nativePath, point.X, point.Y, pen ? pen->nativePen : NULL,
      g ? g->nativeGraphics : NULL, &result));
  return result;
}
__inline__ BOOL GraphicsPath::IsOutlineVisible(const Point &point,
                                               const Pen *pen,
                                               const Graphics *g) const {
  BOOL result = FALSE;
  updateStatus(DllExports::GdipIsOutlineVisiblePathPointI(
...
__inline__ BOOL GraphicsPath::IsVisible(REAL x, REAL y,
                                        const Graphics *g) const {
  BOOL result = FALSE;
  updateStatus(DllExports::GdipIsVisiblePathPoint(
      nativePath, x, y, g ? g->nativeGraphics : NULL, &result));
  return result;
}
...
__inline__ BOOL GraphicsPath::IsVisible(const PointF &point,
  ...
  BOOL result = FALSE;
  updateStatus(DllExports::GdipIsVisiblePathPoint(
      nativePath, point.X, point.Y, g ? g->nativeGraphics : NULL, &result));
  return result;
}
__inline__ BOOL GraphicsPath::IsVisible(const Point &point,
                                        const Graphics *g) const {
  BOOL result = FALSE;
  updateStatus(DllExports::GdipIsVisiblePathPointI(
      nativePath, point.X, point.Y, g ? g->nativeGraphics : NULL, &result));
...
__inline__ Status PathData::AllocateArrays(INT capacity) {
  if (capacity < 0) {
    ...
    FreeArrays();
    PointF *pointArray =
        (PointF *)DllExports::GdipAlloc(capacity * sizeof(PointF));
    if (!pointArray)
      return OutOfMemory;
    BYTE *typeArray = (BYTE *)DllExports::GdipAlloc(capacity * sizeof(BYTE));
    if (!typeArray) {
...
__inline__ VOID PathData::FreeArrays() {
  if (Points)
    DllExports::GdipFree(Points);
  if (Types)
    DllExports::GdipFree(Types);
  Count = 0;
  Points = NULL;
  Types = NULL;
}
