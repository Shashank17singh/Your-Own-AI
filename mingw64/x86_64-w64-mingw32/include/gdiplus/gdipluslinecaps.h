...
public:
  AdjustableArrowCap(REAL height, REAL width, BOOL isFilled)
    ...
    lastStatus = DllExports::GdipCreateAdjustableArrowCap(
        height, width, isFilled, &nativeAdjustableArrowCap);
  ...
  virtual AdjustableArrowCap *Clone() const {
    ...
    if (status == Ok) {
      AdjustableArrowCap *result =
          new AdjustableArrowCap(cloneCustomLineCap, lastStatus);
      if (!result) {
  ...
  REAL GetHeight() const {
    ...
    updateStatus(DllExports::GdipGetAdjustableArrowCapHeight(
        (GpAdjustableArrowCap *)nativeCustomLineCap, &result));
    return result;
  }
