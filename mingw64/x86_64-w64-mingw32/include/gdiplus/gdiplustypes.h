...
typedef struct Point {
  ...
  BOOL Equals(const Point &point) const { return X == point.X && Y == point.Y; }
...
typedef struct Rect {
  ...
  BOOL Contains(const Rect &rect) const {
    return X <= rect.X && Y <= rect.Y && rect.X + rect.Width <= X + Width &&
  ...
  BOOL IntersectsWith(const Rect &rc) const {
    ...
    INT intersectRight =
        (GetRight() < rc.GetRight()) ? GetRight() : rc.GetRight();
    INT intersectBottom =
        (GetBottom() < rc.GetBottom()) ? GetBottom() : rc.GetBottom();
  ...
  static BOOL Union(Rect &c, const Rect &a, const Rect &b) {
    ...
    INT unionRight =
        (a.GetRight() < b.GetRight()) ? b.GetRight() : a.GetRight();
    INT unionBottom =
        (a.GetBottom() < b.GetBottom()) ? b.GetBottom() : a.GetBottom();
    c.X = unionLeft;
    c.Y = unionTop;
