...
/* The purpose of this file is to provide support for MSVC's intrinsics (what
   ...
   in various platform sdk headers. In gcc, this is implemented as follows:
   ...
   'call', performance may be degraded. If you wish to implement intrinsic
   functions that are defined in intrin.h but are not yet implemented in
   mingw-w64, see the comments at the top of intrin-impl.h.
*/
#ifndef __INTRIN_H_
#define __INTRIN_H_
#ifndef RC_INVOKED
 ...
 *
 * On GCC 4.9 and Clang we may always include those headers. On older GCCs, we
 * may do it only if CPU features used by them are enabled, so we need to check
 * macros like __SSE__ or __MMX__ first.
 */
...
__MACHINEIA32(int __cdecl
              __attribute__((__nothrow__, __returns_twice__)) _setjmp(jmp_buf))
__MACHINEX64(int __cdecl
             __attribute__((__nothrow__, __returns_twice__)) _setjmp(jmp_buf,
                                                                     void *))
__MACHINEIA64(int __cdecl
              __attribute__((__nothrow__, __returns_twice__)) _setjmpex(jmp_buf,
                                                                        void *))
__MACHINEX64(int __cdecl
             __attribute__((__nothrow__, __returns_twice__)) _setjmpex(jmp_buf,
                                                                       void *))
#else
__MACHINE(int __cdecl
          __attribute__((__nothrow__, __returns_twice__)) _setjmp(jmp_buf))
__MACHINEIA64(int __cdecl __attribute__((__nothrow__,
                                         __returns_twice__)) _setjmpex(jmp_buf))
__MACHINEX64(int __cdecl
             __attribute__((__nothrow__, __returns_twice__)) _setjmpex(jmp_buf))
#endif
#endif
