...
HINDEX WINAPI NewIndex(const PBYTE pbSourceName, UINT uiTime1, UINT uiTime2,
                       UINT iCharsetDefault, UINT lcidDefault, UINT fdwOptions);
ERRORCODE WINAPI ScanTopicTitle(HINDEX hinx, PBYTE pbTitle, UINT cbTitle,
                                UINT iTopic, HANDLE hTopic, UINT iCharset,
                                UINT lcid);
ERRORCODE WINAPI ScanTopicText(HINDEX hinx, PBYTE pbText, UINT cbText,
                               UINT iCharset, UINT lcid);
...
INT WINAPI OpenIndex(HSEARCHER hsrch, PSZ pszIndexFileName, PBYTE pbSourceName,
                     PUINT pcbSourceNameLimit, PUINT pTime1, PUINT pTime2);
...
ERRORCODE WINAPI ScanText(HCOMPRESSOR hcmp, PBYTE pbText, UINT cbText,
                          UINT iCharset);
ERRORCODE WINAPI GetPhraseTable(HCOMPRESSOR hcmp, PUINT pcPhrases,
                                PBYTE *ppbImages, PUINT pcbImages,
                                PBYTE *ppacbImageCompressed,
                                PUINT pcbCompressed);
ERRORCODE WINAPI SetPhraseTable(HCOMPRESSOR hcmp, PBYTE pbImages, UINT cbImages,
                                PBYTE pacbImageCompressed, UINT cbCompressed);
INT WINAPI CompressText(HCOMPRESSOR hcmp, PBYTE pbText, UINT cbText,
                        PBYTE *ppbCompressed, UINT iCharset);
INT WINAPI DecompressText(HCOMPRESSOR hcmp, PBYTE pbCompressed,
                          UINT cbCompressed, PBYTE pbText);
...
ERRORCODE WINAPI ScanDisplayText(HHILITER hhil, PBYTE pbText, int cbText,
...
int WINAPI QueryHilites(HHILITER hhil, int base, int limit, int cHilites,
                        HILITE *paHilites);
#ifdef __cplusplus
}
#endif
