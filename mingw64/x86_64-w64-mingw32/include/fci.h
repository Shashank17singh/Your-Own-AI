...
typedef INT_PTR(DIAMONDAPI *PFNFCIOPEN)(char *pszFile, int oflag, int pmode,
                                        int *err, void *pv);
typedef UINT(DIAMONDAPI *PFNFCIREAD)(INT_PTR hf, void *memory, UINT cb,
                                     int *err, void *pv);
typedef UINT(DIAMONDAPI *PFNFCIWRITE)(INT_PTR hf, void *memory, UINT cb,
                                      int *err, void *pv);
typedef int(DIAMONDAPI *PFNFCICLOSE)(INT_PTR hf, int *err, void *pv);
typedef __LONG32(DIAMONDAPI *PFNFCISEEK)(INT_PTR hf, __LONG32 dist,
                                         int seektype, int *err, void *pv);
typedef int(DIAMONDAPI *PFNFCIDELETE)(char *pszFile, int *err, void *pv);
#define FNFCIOPEN(fn)                                                          \
  INT_PTR DIAMONDAPI fn(char *pszFile, int oflag, int pmode, int *err, void *pv)
#define FNFCIREAD(fn)                                                          \
  UINT DIAMONDAPI fn(INT_PTR hf, void *memory, UINT cb, int *err, void *pv)
#define FNFCIWRITE(fn)                                                         \
  UINT DIAMONDAPI fn(INT_PTR hf, void *memory, UINT cb, int *err, void *pv)
#define FNFCICLOSE(fn) int DIAMONDAPI fn(INT_PTR hf, int *err, void *pv)
#define FNFCISEEK(fn)                                                          \
  __LONG32 DIAMONDAPI fn(INT_PTR hf, __LONG32 dist, int seektype, int *err,    \
                         void *pv)
#define FNFCIDELETE(fn) int DIAMONDAPI fn(char *pszFile, int *err, void *pv)
typedef WINBOOL(DIAMONDAPI *PFNFCIGETNEXTCABINET)(PCCAB pccab, ULONG cbPrevCab,
                                                  void *pv);
#define FNFCIGETNEXTCABINET(fn)                                                \
  WINBOOL DIAMONDAPI fn(PCCAB pccab, ULONG cbPrevCab, void *pv)
typedef int(DIAMONDAPI *PFNFCIFILEPLACED)(PCCAB pccab, char *pszFile,
                                          __LONG32 cbFile,
                                          WINBOOL fContinuation, void *pv);
#define FNFCIFILEPLACED(fn)                                                    \
  int DIAMONDAPI fn(PCCAB pccab, char *pszFile, __LONG32 cbFile,               \
                    WINBOOL fContinuation, void *pv)
typedef INT_PTR(DIAMONDAPI *PFNFCIGETOPENINFO)(char *pszName, USHORT *pdate,
                                               USHORT *ptime, USHORT *pattribs,
                                               int *err, void *pv);
#define FNFCIGETOPENINFO(fn)                                                   \
  INT_PTR DIAMONDAPI fn(char *pszName, USHORT *pdate, USHORT *ptime,           \
                        USHORT *pattribs, int *err, void *pv)
#define statusFile 0
#define statusFolder 1
#define statusCabinet 2
typedef __LONG32(DIAMONDAPI *PFNFCISTATUS)(UINT typeStatus, ULONG cb1,
                                           ULONG cb2, void *pv);
#define FNFCISTATUS(fn)                                                        \
  __LONG32 DIAMONDAPI fn(UINT typeStatus, ULONG cb1, ULONG cb2, void *pv)
typedef WINBOOL(DIAMONDAPI *PFNFCIGETTEMPFILE)(char *pszTempName,
                                               int cbTempName, void *pv);
#define FNFCIGETTEMPFILE(fn)                                                   \
  WINBOOL DIAMONDAPI fn(char *pszTempName, int cbTempName, void *pv)
HFCI DIAMONDAPI FCICreate(PERF perf, PFNFCIFILEPLACED pfnfcifp,
                          PFNFCIALLOC pfna, PFNFCIFREE pfnf, PFNFCIOPEN pfnopen,
                          PFNFCIREAD pfnread, PFNFCIWRITE pfnwrite,
                          PFNFCICLOSE pfnclose, PFNFCISEEK pfnseek,
                          PFNFCIDELETE pfndelete, PFNFCIGETTEMPFILE pfnfcigtf,
                          PCCAB pccab, void *pv);
WINBOOL DIAMONDAPI FCIAddFile(HFCI hfci, char *pszSourceFile, char *pszFileName,
                              WINBOOL fExecute, PFNFCIGETNEXTCABINET pfnfcignc,
                              PFNFCISTATUS pfnfcis, PFNFCIGETOPENINFO pfnfcigoi,
                              TCOMP typeCompress);
WINBOOL DIAMONDAPI FCIFlushCabinet(HFCI hfci, WINBOOL fGetNextCab,
                                   PFNFCIGETNEXTCABINET pfnfcignc,
                                   PFNFCISTATUS pfnfcis);
WINBOOL DIAMONDAPI FCIFlushFolder(HFCI hfci, PFNFCIGETNEXTCABINET pfnfcignc,
                                  PFNFCISTATUS pfnfcis);
