...
#define MAPI_IPERSISTMESSAGE_METHODS(IPURE)                                    \
  ...
  MAPIMETHOD(InitNew)(THIS_ LPMAPIMESSAGESITE pMessageSite,                    \
                      LPMESSAGE pMessage) IPURE;                               \
  MAPIMETHOD(Load)(THIS_ LPMAPIMESSAGESITE pMessageSite, LPMESSAGE pMessage,   \
                   ULONG ulMessageStatus, ULONG ulMessageFlags) IPURE;         \
...
DECLARE_MAPI_INTERFACE_(IPersistMessage, IUnknown){
    BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE) MAPI_GETLASTERROR_METHOD(PURE)
        MAPI_IPERSISTMESSAGE_METHODS(PURE)};
#define MAPI_IMAPIMESSAGESITE_METHODS(IPURE)                                   \
  MAPIMETHOD(GetSession)(THIS_ LPMAPISESSION * ppSession) IPURE;               \
  MAPIMETHOD(GetStore)(THIS_ LPMDB * ppStore) IPURE;                           \
  MAPIMETHOD(GetFolder)(THIS_ LPMAPIFOLDER * ppFolder) IPURE;                  \
  MAPIMETHOD(GetMessage)(THIS_ LPMESSAGE * ppmsg) IPURE;                       \
  MAPIMETHOD(GetFormManager)(THIS_ LPMAPIFORMMGR * ppFormMgr) IPURE;           \
  MAPIMETHOD(NewMessage)(                                                      \
      THIS_ ULONG fComposeInFolder, LPMAPIFOLDER pFolderFocus,                 \
      LPPERSISTMESSAGE pPersistMessage, LPMESSAGE * ppMessage,                 \
      LPMAPIMESSAGESITE * ppMessageSite, LPMAPIVIEWCONTEXT * ppViewContext)    \
      IPURE;                                                                   \
  MAPIMETHOD(CopyMessage)(THIS_ LPMAPIFOLDER pFolderDestination) IPURE;        \
  MAPIMETHOD(MoveMessage)(THIS_ LPMAPIFOLDER pFolderDestination,               \
                          LPMAPIVIEWCONTEXT pViewContext, LPCRECT prcPosRect)  \
      IPURE;                                                                   \
  MAPIMETHOD(DeleteMessage)(THIS_ LPMAPIVIEWCONTEXT pViewContext,              \
                            LPCRECT prcPosRect) IPURE;                         \
...
#define MAPI_IMAPIFORM_METHODS(IPURE)                                          \
  ...
  MAPIMETHOD(GetViewContext)(THIS_ LPMAPIVIEWCONTEXT * ppViewContext) IPURE;   \
  MAPIMETHOD(ShutdownForm)(THIS_ ULONG ulSaveOptions) IPURE;                   \
  MAPIMETHOD(DoVerb)(THIS_ LONG iVerb, LPMAPIVIEWCONTEXT lpViewContext,        \
                     ULONG hwndParent, LPCRECT lprcPosRect) IPURE;             \
  MAPIMETHOD(Advise)(THIS_ LPMAPIVIEWADVISESINK pAdvise, ULONG * pdwStatus)    \
      IPURE;                                                                   \
...
#define MAPI_IMAPIVIEWCONTEXT_METHODS(IPURE)                                   \
  ...
  MAPIMETHOD(GetPrintSetup)(THIS_ ULONG ulFlags,                               \
                            LPFORMPRINTSETUP * lppFormPrintSetup) IPURE;       \
  MAPIMETHOD(GetSaveStream)(THIS_ ULONG * pulFlags, ULONG * pulFormat,         \
                            LPSTREAM * ppstm) IPURE;                           \
...
#define MAPI_IMAPIFORMADVISESINK_METHODS(IPURE)                                \
  ...
  STDMETHOD(OnActivateNext)(THIS_ LPCSTR lpszMessageClass,                     \
                            ULONG ulMessageStatus, ULONG ulMessageFlags,       \
                            LPPERSISTMESSAGE * ppPersistMessage) IPURE;
#undef INTERFACE
...
#define CbMAPIFormPropArray(_c)                                                \
  (offsetof(SMAPIFormPropArray, aFormProp) + (_c) * sizeof(SMAPIFormProp))
typedef struct {
  LONG lVerb;
  LPTSTR szVerbname;
  DWORD fuFlags;
...
#define CbMAPIVerbArray(_c)                                                    \
  (offsetof(SMAPIVerbArray, aMAPIVerb) + (_c) * sizeof(SMAPIVerb))
#define MAPI_IMAPIFORMINFO_METHODS(IPURE)                                      \
  MAPIMETHOD(CalcFormPropSet)(THIS_ ULONG ulFlags,                             \
                              LPMAPIFORMPROPARRAY * ppFormPropArray) IPURE;    \
  MAPIMETHOD(CalcVerbSet)(THIS_ ULONG ulFlags,                                 \
                          LPMAPIVERBARRAY * ppMAPIVerbArray) IPURE;            \
  MAPIMETHOD(MakeIconFromBinary)(THIS_ ULONG nPropID, HICON * phicon) IPURE;   \
  MAPIMETHOD(SaveForm)(THIS_ LPCTSTR szFileName) IPURE;                        \
  MAPIMETHOD(OpenFormContainer)(THIS_ LPMAPIFORMCONTAINER * ppformcontainer)   \
      IPURE;
#undef INTERFACE
...
#define CbMAPIFormInfoArray(_c)                                                \
  (offsetof(SMAPIFormInfoArray, aFormInfo) + (_c) * sizeof(LPMAPIFORMINFO))
...
#define MAPI_IMAPIFORMMGR_METHODS(IPURE)                                       \
  MAPIMETHOD(LoadForm)(                                                        \
      THIS_ ULONG ulUIParam, ULONG ulFlags, LPCSTR lpszMessageClass,           \
      ULONG ulMessageStatus, ULONG ulMessageFlags, LPMAPIFOLDER pFolderFocus,  \
      LPMAPIMESSAGESITE pMessageSite, LPMESSAGE pmsg,                          \
      LPMAPIVIEWCONTEXT pViewContext, REFIID riid, LPVOID * ppvObj) IPURE;     \
  MAPIMETHOD(ResolveMessageClass)(THIS_ LPCSTR szMsgClass, ULONG ulFlags,      \
                                  LPMAPIFOLDER pFolderFocus,                   \
                                  LPMAPIFORMINFO * ppResult) IPURE;            \
  MAPIMETHOD(ResolveMultipleMessageClasses)(                                   \
      THIS_ LPSMESSAGECLASSARRAY pMsgClasses, ULONG ulFlags,                   \
      LPMAPIFOLDER pFolderFocus, LPSMAPIFORMINFOARRAY * pfrminfoarray) IPURE;  \
  MAPIMETHOD(CalcFormPropSet)(THIS_ LPSMAPIFORMINFOARRAY pfrminfoarray,        \
                              ULONG ulFlags, LPMAPIFORMPROPARRAY * ppResults)  \
      IPURE;                                                                   \
  MAPIMETHOD(CreateForm)(THIS_ ULONG ulUIParam, ULONG ulFlags,                 \
                         LPMAPIFORMINFO pfrminfoToActivate,                    \
                         REFIID refiidToAsk, LPVOID * ppvObj) IPURE;           \
  MAPIMETHOD(SelectForm)(THIS_ ULONG ulUIParam, ULONG ulFlags,                 \
                         LPCTSTR pszTitle, LPMAPIFOLDER pfld,                  \
                         LPMAPIFORMINFO * ppfrminfoReturned) IPURE;            \
  MAPIMETHOD(SelectMultipleForms)(                                             \
      THIS_ ULONG ulUIParam, ULONG ulFlags, LPCTSTR pszTitle,                  \
      LPMAPIFOLDER pfld, LPSMAPIFORMINFOARRAY pfrminfoarray,                   \
      LPSMAPIFORMINFOARRAY * ppfrminfoarray) IPURE;                            \
  MAPIMETHOD(SelectFormContainer)(THIS_ ULONG ulUIParam, ULONG ulFlags,        \
                                  LPMAPIFORMCONTAINER * lppfcnt) IPURE;        \
  MAPIMETHOD(OpenFormContainer)(THIS_ HFRMREG hfrmreg, LPUNKNOWN lpunk,        \
                                LPMAPIFORMCONTAINER * lppfcnt) IPURE;          \
  MAPIMETHOD(PrepareForm)(THIS_ ULONG ulUIParam, ULONG ulFlags,                \
                          LPMAPIFORMINFO pfrminfo) IPURE;                      \
  MAPIMETHOD(IsInConflict)(THIS_ ULONG ulMessageFlags, ULONG ulMessageStatus,  \
...
#define MAPI_IMAPIFORMCONTAINER_METHODS(IPURE)                                 \
  MAPIMETHOD(InstallForm)(THIS_ ULONG ulUIParam, ULONG ulFlags,                \
                          LPCTSTR szCfgPathName) IPURE;                        \
  MAPIMETHOD(RemoveForm)(THIS_ LPCSTR szMessageClass) IPURE;                   \
  MAPIMETHOD(ResolveMessageClass)(THIS_ LPCSTR szMessageClass, ULONG ulFlags,  \
                                  LPMAPIFORMINFO * pforminfo) IPURE;           \
  MAPIMETHOD(ResolveMultipleMessageClasses)(                                   \
      THIS_ LPSMESSAGECLASSARRAY pMsgClassArray, ULONG ulFlags,                \
      LPSMAPIFORMINFOARRAY * ppfrminfoarray) IPURE;                            \
  MAPIMETHOD(CalcFormPropSet)(THIS_ ULONG ulFlags,                             \
                              LPMAPIFORMPROPARRAY * ppResults) IPURE;          \
  MAPIMETHOD(GetDisplay)(THIS_ ULONG ulFlags, LPTSTR * pszDisplayName) IPURE;
#undef INTERFACE
...
#define MAPI_IMAPIFORMFACTORY_METHODS(IPURE)                                   \
  MAPIMETHOD(CreateClassFactory)(THIS_ REFCLSID clsidForm, ULONG ulFlags,      \
                                 LPCLASSFACTORY * lppClassFactory) IPURE;      \
