...
__CRT_INLINE WINBOOL DXIsValidAddress(const void *lp, UINT nBytes,
                                      WINBOOL bReadWrite) {
...
inline void DXCopyElements(TYPE *pDest, const TYPE *pSrc, int nCount) {
  _ASSERT(
      (nCount == 0 || DXIsValidAddress(pDest, nCount * sizeof(TYPE), TRUE)));
  _ASSERT(
      (nCount == 0 || DXIsValidAddress(pSrc, nCount * sizeof(TYPE), FALSE)));
...
template <class ARG_KEY> inline UINT DXHashKey(ARG_KEY key) {
...
struct CDXPlex {
  ...
  static CDXPlex *PASCAL_INLINE Create(CDXPlex *&pHead, UINT nMax,
                                       UINT cbElement) {
...
template <class TYPE, class ARG_TYPE> class CDXArray {
public:
  CDXArray();
  int GetSize() const;
...
template <class TYPE, class ARG_TYPE> CDXArray<TYPE, ARG_TYPE>::CDXArray() {
  m_pData = NULL;
  m_nSize = m_nMaxSize = m_nGrowBy = 0;
}
emplate<class TYPE, class ARG_TYPE> CDXArray<TYPE, ARG_TYPE>::~CDXArray() {
  DXASSERT_VALID(this);
  if (m_pData != NULL) {
...
template <class TYPE, class ARG_TYPE>
void CDXArray<TYPE, ARG_TYPE>::InsertAt(int nIndex, ARG_TYPE newElement,
                                        int nCount) {
  DXASSERT_VALID(this);
  _ASSERT(nIndex >= 0);
  _ASSERT(nCount > 0);
...
template <class TYPE, class ARG_TYPE>
void CDXArray<TYPE, ARG_TYPE>::Sort(int(__cdecl *compare)(const void *elem1,
                                                          const void *elem2)) {
  DXASSERT_VALID(this);
...
#endif
template <class TYPE, class ARG_TYPE> class CDXList {
protected:
  struct CNode {
    CNode *pNext;
    CNode *pPrev;
...
template <class TYPE, class ARG_TYPE>
inline WINBOOL CDXList<TYPE, ARG_TYPE>::IsEmpty() const {
  return m_nCount == 0;
}
template <class TYPE, class ARG_TYPE>
...
template <class TYPE, class ARG_TYPE> CDXList<TYPE, ARG_TYPE>::~CDXList() {
  RemoveAll();
  _ASSERT(m_nCount == 0);
}
template <class TYPE, class ARG_TYPE>
...
CDXList<TYPE, ARG_TYPE>::NewNode(CNode *pPrev, CNode *pNext) {
  if (!m_pNodeFree) {
    CDXPlex *pNewBlock =
        CDXPlex::Create(m_pBlocks, m_nBlockSize, sizeof(CNode));
...
template <class TYPE, class ARG_TYPE>
DXLISTPOS CDXList<TYPE, ARG_TYPE>::InsertBefore(DXLISTPOS position,
                                                ARG_TYPE newElement) {
  DXASSERT_VALID(this);
  if (!position)
...
template <class TYPE, class ARG_TYPE>
DXLISTPOS CDXList<TYPE, ARG_TYPE>::InsertAfter(DXLISTPOS position,
                                               ARG_TYPE newElement) {
  DXASSERT_VALID(this);
  if (!position)
...
template <class TYPE, class ARG_TYPE>
DXLISTPOS CDXList<TYPE, ARG_TYPE>::Find(ARG_TYPE searchValue,
                                        DXLISTPOS startAfter) const {
  DXASSERT_VALID(this);
...
#endif
template <class KEY, class ARG_KEY, class VALUE, class ARG_VALUE> class CDXMap {
protected:
  struct CAssoc {
    CAssoc *pNext;
    UINT nHashValue;
...
template <class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
inline WINBOOL CDXMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::IsEmpty() const {
  return m_nCount == 0;
}
template <class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
inline void CDXMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::SetAt(ARG_KEY key,
                                                          ARG_VALUE newValue) {
  (*this)[key] = newValue;
}
template <class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
inline DXLISTPOS
CDXMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::GetStartPosition() const {
...
template <class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
void CDXMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::InitHashTable(UINT nHashSize,
                                                           WINBOOL bAllocNow) {
  DXASSERT_VALID(this);
  _ASSERT(m_nCount == 0);
  _ASSERT(nHashSize > 0);
...
CDXMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::NewAssoc() {
  if (!m_pFreeList) {
    CDXPlex *newBlock =
        CDXPlex::Create(m_pBlocks, m_nBlockSize, sizeof(CDXMap::CAssoc));
...
template <class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
typename CDXMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::CAssoc *
CDXMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::GetAssocAt(ARG_KEY key,
                                                   UINT &nHash) const {
...
template <class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
WINBOOL CDXMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::Lookup(ARG_KEY key,
                                                       VALUE &rValue) const {
  DXASSERT_VALID(this);
  UINT nHash;
...
template <class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
void CDXMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::GetNextAssoc(
    DXLISTPOS &rNextPosition, KEY &rKey, VALUE &rValue) const {
  DXASSERT_VALID(this);
  ...
  if (!(pAssocNext = pAssocRet->pNext)) {
    for (UINT nBucket = pAssocRet->nHashValue + 1; nBucket < m_nHashTableSize;
         nBucket++)
