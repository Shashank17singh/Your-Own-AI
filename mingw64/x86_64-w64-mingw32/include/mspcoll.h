...
public:
  ...
  STDMETHOD(get_Item)(__LONG32 Index, VARIANT *retval) {
    ...
    if (FAILED(hr)) {
      LOG((MSP_ERROR,
           "CTapiCollection::get_Item - VariantCopy failed. hr = %lx", hr));
      return hr;
    }
  ...
  HRESULT WINAPI get__NewEnum(IUnknown **retval) {
    ...
    *retval = NULL;
    typedef CComObject<
        CSafeComEnum<IEnumVARIANT, &IID_IEnumVARIANT, VARIANT, _Copy<VARIANT>>>
        enumvar;
    enumvar *p;
...
public:
  ...
  STDMETHOD(get_Item)(__LONG32 Index, VARIANT *retval) {
    ...
    if (FAILED(hr)) {
      LOG((MSP_ERROR,
           "CTapiBstrCollection::get_Item - VariantCopy failed. hr = %lx", hr));
      return hr;
    }
