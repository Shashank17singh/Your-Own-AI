...
#else
template <typename __CountofType, size_t __wspiapi_countof_helper_N>
char (&__wspiapi_countof_helper(__CountofType (
    &_Array)[__wspiapi_countof_helper_N]))[__wspiapi_countof_helper_N];
...
typedef int(WINAPI *WSPIAPI_PGETADDRINFO)(const char *nodename,
                                          const char *servname,
                                          const struct addrinfo *hints,
                                          struct addrinfo **res);
typedef int(WINAPI *WSPIAPI_PGETNAMEINFO)(const struct sockaddr *sa,
...
#define WSPIAPI_FUNCTION_ARRAY                                                 \
  {                                                                            \
    {"getaddrinfo", (FARPROC)(void (*)(void))WspiapiLegacyGetAddrInfo},        \
...
int WINAPI WspiapiLookupNode(const char *pszNodeName, int iSocketType,
                             int iProtocol, WORD wPort, WINBOOL bAI_CANONNAME,
                             struct addrinfo **pptResult);
...
int WINAPI WspiapiLegacyGetNameInfo(const struct sockaddr *ptSocketAddress,
                                    socklen_t tSocketLength, char *pszNodeName,
                                    size_t tNodeLength, char *pszServiceName,
...
__CRT_INLINE WINBOOL WINAPI WspiapiParseV4Address(const char *pszAddress,
                                                  PDWORD pdwAddress) {
  DWORD dwAddress = 0;
  const char *h = NULL;
  int cnt;
...
__CRT_INLINE struct addrinfo *WINAPI WspiapiNewAddrInfo(int iSocketType,
                                                        int iProtocol,
                                                        WORD wPort,
                                                        DWORD dwAddress) {
  struct addrinfo *n;
  ...
  if ((n = (struct addrinfo *)WspiapiMalloc(sizeof(struct addrinfo))) == NULL)
    return NULL;
  if ((pa = (struct sockaddr_in *)WspiapiMalloc(sizeof(struct sockaddr_in))) ==
      NULL) {
    WspiapiFree(n);
    return NULL;
  }
...
__CRT_INLINE int WINAPI WspiapiLookupNode(const char *pszNodeName,
