...
#define REC_S_NOTCOMPLETEBUTPROPAGATE                                          \
  MAKE_SCODE(SEVERITY_SUCCESS, FACILITY_ITF, 0x1002)
...
#define REC_E_INEEDTODOTHEUPDATES                                              \
  MAKE_SCODE(SEVERITY_ERROR, FACILITY_ITF, 0x1004)
#undef INTERFACE
...
DECLARE_INTERFACE_(INotifyReplica, IUnknown) {
  ...
  STDMETHOD(YouAreAReplica)(THIS_ ULONG ulcOtherReplicas,
...
typedef enum _reconcilef {
  ...
  ALL_RECONCILE_FLAGS =
      (RECONCILEF_MAYBOTHERUSER | RECONCILEF_FEEDBACKWINDOWVALID |
       RECONCILEF_NORESIDUESOK | RECONCILEF_OMITSELFRESIDUE |
       RECONCILEF_RESUMERECONCILIATION | RECONCILEF_YOUMAYDOTHEUPDATES |
...
DECLARE_INTERFACE_(IReconcilableObject, IUnknown) {
  ...
  STDMETHOD_(ULONG, Release)(THIS) PURE;
  STDMETHOD(Reconcile)(
      THIS_ IReconcileInitiator * pInitiator, DWORD dwFlags, HWND hwndOwner,
      HWND hwndProgressFeedback, ULONG ulcInput, IMoniker * *rgpmkOtherInput,
      PLONG plOutIndex, IStorage * pstgNewResidues, PVOID pvReserved) PURE;
