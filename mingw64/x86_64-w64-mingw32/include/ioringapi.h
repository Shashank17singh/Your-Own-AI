...
#else
#define IoRingHandleRefFromHandle(h)                                           \
  {                                                                            \
    IORING_REF_RAW, { .Handle = h }                                            \
  }
#define IoRingHandleRefFromIndex(i)                                            \
  {                                                                            \
    IORING_REF_REGISTERED, { .Index = i }                                      \
  }
#endif
...
#else
#define IoRingBufferRefFromPointer(p)                                          \
  {                                                                            \
    IORING_REF_RAW, { .Address = p }                                           \
  }
#define IoRingBufferRefFromIndexAndOffset(i, o)                                \
  {                                                                            \
    IORING_REF_REGISTERED, { .IndexAndOffset = {(i), (o)} }                    \
  }
#endif
...
STDAPI SubmitIoRing(HIORING ioRing, UINT32 waitOperations, UINT32 milliseconds,
...
STDAPI BuildIoRingCancelRequest(HIORING ioRing, IORING_HANDLE_REF file,
                                UINT_PTR opToCancel, UINT_PTR userData);
STDAPI BuildIoRingReadFile(HIORING ioRing, IORING_HANDLE_REF fileRef,
                           IORING_BUFFER_REF dataRef,
                           UINT32 numberOfBytesToRead, UINT64 fileOffset,
                           UINT_PTR userData, IORING_SQE_FLAGS flags);
STDAPI BuildIoRingRegisterFileHandles(HIORING ioRing, UINT32 count,
                                      HANDLE const handles[],
                                      UINT_PTR userData);
STDAPI BuildIoRingRegisterBuffers(HIORING ioRing, UINT32 count,
