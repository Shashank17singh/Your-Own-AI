...
#define ALC_ALL                                                                \
  (ALC_SYSMINIMUM | ALC_MATH | ALC_MONETARY | ALC_OTHER | ALC_NONPRINT)
...
#define PWF_FULL                                                               \
  PWF_RC1 | PWF_PEN | PWF_INKDISPLAY | PWF_RECOGNIZER | PWF_BEDIT |            \
      PWF_HEDIT | PWF_IEDIT | PWF_ENHANCED
...
#define SetAlcBitAnsi(lprc, i)                                                 \
  do {                                                                         \
    MpAlcB(lprc, i) |= MpIbf(i);                                               \
  } while (0)
#define ResetAlcBitAnsi(lprc, i)                                               \
  do {                                                                         \
    MpAlcB(lprc, i) &= ~MpIbf(i);                                              \
  } while (0)
...
#define dwDiffAT(at1, at2)                                                     \
  (1000 * ((at2).sec - (at1).sec) - (DWORD)(at1).ms + (DWORD)(at2).ms)
...
#define MakeAbsTime(lpat, sec, ms)                                             \
  do {                                                                         \
...
#define FIsStdGesture(syv)                                                     \
  (FIsSelectGesture(syv) || (syv) == SYV_CLEAR || (syv) == SYV_HELP ||         \
   (syv) == SYV_EXTENDSELECT || (syv) == SYV_UNDO || (syv) == SYV_COPY ||      \
   (syv) == SYV_CUT || (syv) == SYV_PASTE || (syv) == SYV_CLEARWORD ||         \
   (syv) == SYV_KKCONVERT || (syv) == SYV_USER || (syv) == SYV_CORRECT)
#define FIsAnsiGesture(syv)                                                    \
  ((syv) == SYV_BACKSPACE || (syv) == SYV_TAB || (syv) == SYV_RETURN ||        \
   (syv) == SYV_SPACE)
#endif
#ifndef NOPENINKPUT
...
typedef HWL *LPHWL;
typedef WINBOOL(CALLBACK *HRCRESULTHOOKPROC)(HREC, HRC, UINT, UINT, UINT,
                                             LPVOID);
...
WINBOOL WINAPI GetPenDataStroke(LPPENDATA, UINT, LPPOINT *, LPVOID *,
                                LPSTROKEINFO);
#endif
...
int WINAPI ExtractPenDataPoints(HPENDATA, UINT, UINT, UINT, LPPOINT, LPVOID,
                                UINT);
...
#endif
#ifdef JAPAN
WINBOOL WINAPI KKConvert(HWND hwndConvert, HWND hwndCaller, LPSTR lpBuf,
                         UINT cbBuf, LPPOINT lpPnt);
#endif
#endif
#ifdef __cplusplus
}
