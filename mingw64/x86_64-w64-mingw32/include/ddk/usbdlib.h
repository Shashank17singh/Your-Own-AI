...
#define UsbBuildInterruptOrBulkTransferRequest(                                \
    urb, length, pipeHandle, transferBuffer, transferBufferMDL,                \
...
#define UsbBuildGetDescriptorRequest(                                          \
    urb, length, descriptorType, descriptorIndex, languageId, transferBuffer,  \
    transferBufferMDL, transferBufferLength, link)                             \
  {                                                                            \
...
#define UsbBuildGetStatusRequest(urb, op, index, transferBuffer,               \
                                 transferBufferMDL, link)                      \
  {                                                                            \
...
#define UsbBuildSelectConfigurationRequest(urb, length,                        \
...
#define UsbBuildSelectInterfaceRequest(urb, length, configurationHandle,       \
  ...
  {                                                                            \
    ...
    (urb)->UrbSelectInterface.Interface.Length =                               \
        (length - sizeof(struct _URB_HEADER) -                                 \
         sizeof(USBD_CONFIGURATION_HANDLE));                                   \
    (urb)->UrbSelectInterface.ConfigurationHandle = (configurationHandle);     \
  }
#define UsbBuildVendorRequest(urb, cmd, length, transferFlags, reservedbits,   \
                              request, value, index, transferBuffer,           \
                              transferBufferMDL, transferBufferLength, link)   \
  {                                                                            \
...
#define UsbBuildOsFeatureDescriptorRequest(urb, length, interface, index,      \
                                           transferBuffer, transferBufferMDL,  \
                                           transferBufferLength, link)         \
  {                                                                            \
...
#define GET_ISO_URB_SIZE(n)                                                    \
  (sizeof(struct _URB_ISOCH_TRANSFER) + sizeof(USBD_ISO_PACKET_DESCRIPTOR) * n)
#ifndef _USBD_
DECLSPEC_IMPORT
VOID NTAPI
USBD_GetUSBDIVersion(OUT PUSBD_VERSION_INFORMATION VersionInformation);
DECLSPEC_IMPORT
PUSB_INTERFACE_DESCRIPTOR
NTAPI
USBD_ParseConfigurationDescriptor(
    IN PUSB_CONFIGURATION_DESCRIPTOR ConfigurationDescriptor,
    IN UCHAR InterfaceNumber, IN UCHAR AlternateSetting);
DECLSPEC_IMPORT
PURB NTAPI USBD_CreateConfigurationRequest(
    IN PUSB_CONFIGURATION_DESCRIPTOR ConfigurationDescriptor, OUT PUSHORT Siz);
DECLSPEC_IMPORT
PUSB_COMMON_DESCRIPTOR
NTAPI
USBD_ParseDescriptors(IN PVOID DescriptorBuffer, IN ULONG TotalLength,
                      IN PVOID StartPosition, IN LONG DescriptorType);
DECLSPEC_IMPORT
PUSB_INTERFACE_DESCRIPTOR
NTAPI
USBD_ParseConfigurationDescriptorEx(
    IN PUSB_CONFIGURATION_DESCRIPTOR ConfigurationDescriptor,
    IN PVOID StartPosition, IN LONG InterfaceNumber, IN LONG AlternateSetting,
    IN LONG InterfaceClass, IN LONG InterfaceSubClass,
    IN LONG InterfaceProtocol);
DECLSPEC_IMPORT
PURB NTAPI USBD_CreateConfigurationRequestEx(
    IN PUSB_CONFIGURATION_DESCRIPTOR ConfigurationDescriptor,
    IN PUSBD_INTERFACE_LIST_ENTRY InterfaceList);
DECLSPEC_EXPORT
ULONG
NTAPI
USBD_GetInterfaceLength(IN PUSB_INTERFACE_DESCRIPTOR InterfaceDescriptor,
                        IN PUCHAR BufferEnd);
DECLSPEC_EXPORT
VOID NTAPI USBD_RegisterHcFilter(IN PDEVICE_OBJECT DeviceObject,
                                 IN PDEVICE_OBJECT FilterDeviceObject);
DECLSPEC_EXPORT
NTSTATUS
NTAPI
USBD_GetPdoRegistryParameter(IN PDEVICE_OBJECT PhysicalDeviceObject,
                             IN OUT PVOID Parameter, IN ULONG ParameterLength,
                             IN PWSTR KeyName, IN ULONG KeyNameLength);
DECLSPEC_EXPORT
NTSTATUS
NTAPI
USBD_QueryBusTime(IN PDEVICE_OBJECT RootHubPdo, OUT PULONG CurrentFrame);
#if (NTDDI_VERSION >= NTDDI_WINXP)
DECLSPEC_IMPORT
ULONG
NTAPI
USBD_CalculateUsbBandwidth(IN ULONG MaxPacketSize, IN UCHAR EndpointType,
                           IN BOOLEAN LowSpeed);
#endif
...
USBD_ValidateConfigurationDescriptor(
    IN PUSB_CONFIGURATION_DESCRIPTOR ConfigDesc, IN ULONG BufferLength,
    IN USHORT Level, OUT PUCHAR *Offset, IN ULONG Tag OPTIONAL);
#endif
#endif /* ! _USBD_ */
