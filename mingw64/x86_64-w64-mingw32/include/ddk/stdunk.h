...
DECLARE_INTERFACE(INonDelegatingUnknown) {
  STDMETHOD_(NTSTATUS, NonDelegatingQueryInterface)(THIS_ IN REFIID,
                                                    OUT PVOID *) PURE;
...
public:
  ...
  virtual ~CUnknown();
  PUNKNOWN GetOuterUnknown() { return m_outer_unknown; }
  /* INonDelegatingUnknown */
  STDMETHODIMP_(ULONG) NonDelegatingAddRef();
  STDMETHODIMP_(ULONG) NonDelegatingRelease();
  STDMETHODIMP_(NTSTATUS)
  NonDelegatingQueryInterface(REFIID rIID, PVOID *ppVoid);
};
#define DECLARE_STD_UNKNOWN()                                                  \
  STDMETHODIMP_(NTSTATUS)                                                      \
  NonDelegatingQueryInterface(REFIID iid, PVOID *ppvObject);                   \
                                                                               \
...
#define DEFINE_STD_CONSTRUCTOR(classname)                                      \
  classname(PUNKNOWN outer_unknown) : CUnknown(outer_unknown) {}
#else  /* Not C++ - this is probably very buggy... */
NTSTATUS
STDMETHODCALLTYPE
Unknown_QueryInterface(IUnknown *this, IN REFIID refiid, OUT PVOID *output);
ULONG
STDMETHODCALLTYPE
Unknown_AddRef(IUnknown *unknown_this);
ULONG
STDMETHODCALLTYPE
Unknown_Release(IUnknown *unknown_this);
...
*/
#define QICAST(typename) PVOID((typename)(this))
#define QICASTUNKNOWN(typename) PVOID(PUNKNOWN((typename)(this)))
#define STD_CREATE_BODY_WITH_TAG_(classname, unknown, outer_unknown,           \
                                  pool_type, tag, base)                        \
...
#define STD_CREATE_BODY_WITH_TAG(classname, unknown, outer_unknown, pool_type, \
                                 tag, base)                                    \
...
inline PVOID KCOM_New(size_t size, POOL_TYPE pool_type, ULONG tag) {
  PVOID result;
...
inline PVOID operator new(size_t size, POOL_TYPE pool_type) {
  return KCOM_New(size, pool_type, 'wNcP');
}
inline PVOID operator new(size_t size, POOL_TYPE pool_type, ULONG tag) {
  return KCOM_New(size, pool_type, tag);
}
inline void __cdecl operator delete(PVOID ptr) { ExFreePool(ptr); }
