...
public:
  ComPtrRef(T *ptr) throw() { ComPtrRefBase<T>::ptr_ = ptr; }
  operator void **() const throw() {
    return reinterpret_cast<void **>(
        ComPtrRefBase<T>::ptr_->ReleaseAndGetAddressOf());
  }
  operator T *() throw() {
...
public:
  ...
  InterfaceType *Get() const throw() { return ptr_; }
  ...
  InterfaceType *const *GetAddressOf() const throw() { return &ptr_; }
  InterfaceType **GetAddressOf() throw() { return &ptr_; }
...
template <class T> bool operator==(const ComPtr<T> &a, std::nullptr_t) throw() {
  return a.Get() == nullptr;
}
template <class T> bool operator==(std::nullptr_t, const ComPtr<T> &a) throw() {
  return a.Get() == nullptr;
}
template <class T, class U>
bool operator!=(const ComPtr<T> &a, const ComPtr<U> &b) throw() {
...
void **IID_PPV_ARGS_Helper(::Microsoft::WRL::Details::ComPtrRef<T> pp) throw() {
  static_assert(__is_base_of(IUnknown, typename T::InterfaceType),
...
template <typename T>
inline HRESULT
ActivateInstance(HSTRING classid,
                 ::Microsoft::WRL::Details::ComPtrRef<T> instance) throw() {
  return ActivateInstance(classid, instance.ReleaseAndGetAddressOf());
}
template <typename T>
inline HRESULT
GetActivationFactory(HSTRING classid,
                     ::Microsoft::WRL::Details::ComPtrRef<T> factory) throw() {
  return RoGetActivationFactory(classid,
                                IID_INS_ARGS(factory.ReleaseAndGetAddressOf()));
}
} // namespace Foundation
} // namespace Windows
namespace ABI {
