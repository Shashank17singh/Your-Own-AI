...
public:
  ...
  operator HSTRING() const throw() { return hstr_; }
  ~HString() throw() { Release(); }
  ...
  template <size_t s> HRESULT Set(const wchar_t (&str)[s]) throw() {
    static_assert(static_cast<size_t>(static_cast<UINT32>(s - 1)) == s - 1,
                  "mismatch string length");
    return Set(str, s - 1);
  }
  template <size_t s> HRESULT Set(wchar_t (&strRef)[s]) throw() {
  ...
  template <typename T>
  HRESULT Set(const T &s,
              typename ::std::enable_if<
                  ::std::is_convertible<const T &, const wchar_t *>::value,
                  ::Microsoft::WRL::Details::Dummy>::type =
                  ::Microsoft::WRL::Details::Dummy()) throw() {
    HRESULT hr = S_OK;
  ...
  HSTRING Get() const throw() { return hstr_; }
  void Release() throw() {
    ::WindowsDeleteString(hstr_);
    hstr_ = nullptr;
  }
  bool IsValid() const throw() { return hstr_ != nullptr; }
  UINT32 Length() const throw() { return ::WindowsGetStringLen(hstr_); }
  ...
  bool IsEmpty() const throw() { return hstr_ == nullptr; }
  ...
  HRESULT Replace(const HString &s1, const HString &s2, HString &n) const
      throw() {
    HSTRING l;
  ...
  template <unsigned int s>
  static HStringReference MakeReference(wchar_t const (&str)[s],
                                        unsigned int l) throw();
...
public:
  ...
  ~HStringReference() throw() { hstr_ = nullptr; }
