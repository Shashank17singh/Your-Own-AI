...
#define CREDUI_IS_AUTHENTICATION_ERROR(_Status)                                \
  (CREDUIP_IS_USER_PASSWORD_ERROR(_Status) ||                                  \
   CREDUIP_IS_DOWNGRADE_ERROR(_Status) || CREDUIP_IS_EXPIRED_ERROR(_Status))
...
#define CRED_SESSION_WILDCARD_NAME_LENGTH                                      \
  (sizeof(CRED_SESSION_WILDCARD_NAME_A) - 1)
...
#define CREDUI_FLAGS_PROMPT_VALID                                              \
  (CREDUI_FLAGS_INCORRECT_PASSWORD | CREDUI_FLAGS_DO_NOT_PERSIST |             \
   CREDUI_FLAGS_REQUEST_ADMINISTRATOR | CREDUI_FLAGS_EXCLUDE_CERTIFICATES |    \
   CREDUI_FLAGS_REQUIRE_CERTIFICATE | CREDUI_FLAGS_SHOW_SAVE_CHECK_BOX |       \
   CREDUI_FLAGS_ALWAYS_SHOW_UI | CREDUI_FLAGS_REQUIRE_SMARTCARD |              \
   CREDUI_FLAGS_PASSWORD_ONLY_OK | CREDUI_FLAGS_VALIDATE_USERNAME |            \
...
#define CredUICmdLinePromptForCredentials                                      \
  __MINGW_NAME_AW(CredUICmdLinePromptForCredentials)
...
WINADVAPI WINBOOL WINAPI CredReadW(LPCWSTR TargetName, DWORD Type, DWORD Flags,
                                   PCREDENTIALW *Credential);
WINADVAPI WINBOOL WINAPI CredReadA(LPCSTR TargetName, DWORD Type, DWORD Flags,
                                   PCREDENTIALA *Credential);
WINADVAPI WINBOOL WINAPI CredEnumerateW(LPCWSTR Filter, DWORD Flags,
                                        DWORD *Count,
                                        PCREDENTIALW **Credential);
WINADVAPI WINBOOL WINAPI CredEnumerateA(LPCSTR Filter, DWORD Flags,
                                        DWORD *Count,
                                        PCREDENTIALA **Credential);
...
WINADVAPI WINBOOL WINAPI CredReadDomainCredentialsW(
    PCREDENTIAL_TARGET_INFORMATIONW TargetInfo, DWORD Flags, DWORD *Count,
    PCREDENTIALW **Credential);
WINADVAPI WINBOOL WINAPI CredReadDomainCredentialsA(
    PCREDENTIAL_TARGET_INFORMATIONA TargetInfo, DWORD Flags, DWORD *Count,
    PCREDENTIALA **Credential);
WINADVAPI WINBOOL WINAPI CredDeleteW(LPCWSTR TargetName, DWORD Type,
                                     DWORD Flags);
WINADVAPI WINBOOL WINAPI CredDeleteA(LPCSTR TargetName, DWORD Type,
                                     DWORD Flags);
WINADVAPI WINBOOL WINAPI CredRenameW(LPCWSTR OldTargetName,
                                     LPCWSTR NewTargetName, DWORD Type,
                                     DWORD Flags);
WINADVAPI WINBOOL WINAPI CredRenameA(LPCSTR OldTargetName, LPCSTR NewTargetName,
                                     DWORD Type, DWORD Flags);
#define CRED_ALLOW_NAME_RESOLUTION 0x1
WINADVAPI WINBOOL WINAPI
CredGetTargetInfoW(LPCWSTR TargetName, DWORD Flags,
                   PCREDENTIAL_TARGET_INFORMATIONW *TargetInfo);
WINADVAPI WINBOOL WINAPI
CredGetTargetInfoA(LPCSTR TargetName, DWORD Flags,
                   PCREDENTIAL_TARGET_INFORMATIONA *TargetInfo);
WINADVAPI WINBOOL WINAPI CredMarshalCredentialW(CRED_MARSHAL_TYPE CredType,
                                                PVOID Credential,
                                                LPWSTR *MarshaledCredential);
WINADVAPI WINBOOL WINAPI CredMarshalCredentialA(CRED_MARSHAL_TYPE CredType,
                                                PVOID Credential,
                                                LPSTR *MarshaledCredential);
WINADVAPI WINBOOL WINAPI CredUnmarshalCredentialW(LPCWSTR MarshaledCredential,
                                                  PCRED_MARSHAL_TYPE CredType,
                                                  PVOID *Credential);
WINADVAPI WINBOOL WINAPI CredUnmarshalCredentialA(LPCSTR MarshaledCredential,
                                                  PCRED_MARSHAL_TYPE CredType,
                                                  PVOID *Credential);
WINADVAPI WINBOOL WINAPI
CredIsMarshaledCredentialW(LPCWSTR MarshaledCredential);
WINADVAPI WINBOOL WINAPI CredIsMarshaledCredentialA(LPCSTR MarshaledCredential);
WINADVAPI WINBOOL WINAPI CredGetSessionTypes(DWORD MaximumPersistCount,
                                             LPDWORD MaximumPersist);
WINADVAPI VOID WINAPI CredFree(PVOID Buffer);
CREDUIAPI DWORD WINAPI CredUIPromptForCredentialsW(
    PCREDUI_INFOW pUiInfo, PCWSTR pszTargetName, PCtxtHandle pContext,
    DWORD dwAuthError, PWSTR pszUserName, ULONG ulUserNameBufferSize,
    PWSTR pszPassword, ULONG ulPasswordBufferSize, WINBOOL *save,
    DWORD dwFlags);
...
CREDUIAPI DWORD WINAPI CredUIParseUserNameW(CONST WCHAR *UserName, WCHAR *user,
                                            ULONG userBufferSize, WCHAR *domain,
                                            ULONG domainBufferSize);
CREDUIAPI DWORD WINAPI CredUIParseUserNameA(CONST CHAR *userName, CHAR *user,
                                            ULONG userBufferSize, CHAR *domain,
                                            ULONG domainBufferSize);
CREDUIAPI DWORD WINAPI CredUICmdLinePromptForCredentialsW(
    PCWSTR pszTargetName, PCtxtHandle pContext, DWORD dwAuthError,
...
CREDUIAPI DWORD WINAPI CredUICmdLinePromptForCredentialsA(
    PCSTR pszTargetName, PCtxtHandle pContext, DWORD dwAuthError, PSTR UserName,
    ULONG ulUserBufferSize, PSTR pszPassword, ULONG ulPasswordBufferSize,
    PBOOL pfSave, DWORD dwFlags);
CREDUIAPI DWORD WINAPI CredUIConfirmCredentialsW(PCWSTR pszTargetName,
                                                 WINBOOL bConfirm);
CREDUIAPI DWORD WINAPI CredUIConfirmCredentialsA(PCSTR pszTargetName,
                                                 WINBOOL bConfirm);
CREDUIAPI DWORD WINAPI CredUIStoreSSOCredW(PCWSTR pszRealm, PCWSTR pszUsername,
                                           PCWSTR pszPassword,
                                           WINBOOL bPersist);
...
#define CredPackAuthenticationBuffer                                           \
  __MINGW_NAME_AW(CredPackAuthenticationBuffer)
#define CredProtect __MINGW_NAME_AW(CredProtect)
#define CredUIPromptForWindowsCredentials                                      \
  __MINGW_NAME_AW(CredUIPromptForWindowsCredentials)
#define CredUnPackAuthenticationBuffer                                         \
  __MINGW_NAME_AW(CredUnPackAuthenticationBuffer)
...
WINADVAPI WINBOOL WINAPI CredFindBestCredentialA(LPCSTR TargetName, DWORD Type,
                                                 DWORD Flags,
                                                 PCREDENTIALA *Credential);
WINADVAPI WINBOOL WINAPI CredFindBestCredentialW(LPCWSTR TargetName, DWORD Type,
                                                 DWORD Flags,
                                                 PCREDENTIALW *Credential);
WINADVAPI WINBOOL WINAPI CredIsProtectedA(
    LPSTR pszProtectedCredentials, CRED_PROTECTION_TYPE *pProtectionType);
WINADVAPI WINBOOL WINAPI CredIsProtectedW(
    LPWSTR pszProtectedCredentials, CRED_PROTECTION_TYPE *pProtectionType);
CREDUIAPI WINBOOL WINAPI CredPackAuthenticationBufferA(
    DWORD dwFlags, LPSTR pszUserName, LPSTR pszPassword,
    PBYTE pPackedCredentials, DWORD *pcbPackedCredentials);
CREDUIAPI WINBOOL WINAPI CredPackAuthenticationBufferW(
    DWORD dwFlags, LPWSTR pszUserName, LPWSTR pszPassword,
    PBYTE pPackedCredentials, DWORD *pcbPackedCredentials);
WINADVAPI WINBOOL WINAPI CredProtectW(WINBOOL fAsSelf, LPWSTR pszCredentials,
                                      DWORD cchCredentials,
                                      LPWSTR pszProtectedCredentials,
                                      DWORD *pcchMaxChars,
                                      CRED_PROTECTION_TYPE *ProtectionType);
WINADVAPI WINBOOL WINAPI CredProtectA(WINBOOL fAsSelf, LPSTR pszCredentials,
                                      DWORD cchCredentials,
...
CREDUIAPI DWORD WINAPI CredUIPromptForWindowsCredentialsA(
    ...
    LPCVOID pvInAuthBuffer, ULONG ulInAuthBufferSize, LPVOID *ppvOutAuthBuffer,
    ULONG *pulOutAuthBufferSize, WINBOOL *pfSave, DWORD dwFlags);
...
CREDUIAPI WINBOOL WINAPI CredUnPackAuthenticationBufferA(
    DWORD dwFlags, PVOID pAuthBuffer, DWORD cbAuthBuffer, LPSTR pszUserName,
    DWORD *pcchMaxUserName, LPSTR pszDomainName, DWORD *pcchMaxDomainame,
    LPSTR pszPassword, DWORD *pcchMaxPassword);
CREDUIAPI WINBOOL WINAPI CredUnPackAuthenticationBufferW(
    DWORD dwFlags, PVOID pAuthBuffer, DWORD cbAuthBuffer, LPWSTR pszUserName,
    DWORD *pcchMaxUserName, LPWSTR pszDomainName, DWORD *pcchMaxDomainame,
    LPWSTR pszPassword, DWORD *pcchMaxPassword);
WINADVAPI WINBOOL WINAPI CredUnprotectA(WINBOOL fAsSelf,
                                        LPSTR pszProtectedCredentials,
                                        DWORD cchCredentials,
                                        LPSTR pszCredentials,
                                        DWORD *pcchMaxChars);
WINADVAPI WINBOOL WINAPI CredUnprotectW(WINBOOL fAsSelf,
                                        LPWSTR pszProtectedCredentials,
                                        DWORD cchCredentials,
