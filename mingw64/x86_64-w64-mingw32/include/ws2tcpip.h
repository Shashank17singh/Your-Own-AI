...
WS2TCPIP_INLINE int idealsendbacklogquery(SOCKET s, ULONG *isb) {
  DWORD bytes;
  return WSAIoctl(s, SIO_IDEAL_SEND_BACKLOG_QUERY, NULL, 0, isb, sizeof(*isb),
                  &bytes, NULL, NULL);
}
WS2TCPIP_INLINE int
idealsendbacklognotify(SOCKET s, LPWSAOVERLAPPED overlapped,
...
typedef int(WSAAPI *LPFN_GETADDRINFO)(const char *nodename,
                                      ...
                                      const struct addrinfo *hints,
                                      struct addrinfo **res);
typedef int(WSAAPI *LPFN_GETADDRINFOW)(PCWSTR pNodeName, PCWSTR pServiceName,
                                       const ADDRINFOW *pHints,
                                       PADDRINFOW *ppResult);
...
WINSOCK_API_LINKAGE int WSAAPI GetAddrInfoExA(
    ...
    const ADDRINFOEXA *pHints, PADDRINFOEXA *ppResult, PTIMEVAL timeout,
...
WINSOCK_API_LINKAGE int WSAAPI GetAddrInfoExW(
    ...
    const ADDRINFOEXW *pHints, PADDRINFOEXW *ppResult, PTIMEVAL timeout,
    LPOVERLAPPED lpOverlapped,
    LPLOOKUPSERVICE_COMPLETION_ROUTINE lpCompletionRoutine,
    LPHANDLE lpNameHandle);
WINSOCK_API_LINKAGE int WSAAPI SetAddrInfoExA(
    PCSTR pName, PCSTR pServiceName, SOCKET_ADDRESS *pAddresses,
...
WINSOCK_API_LINKAGE int WSAAPI SetAddrInfoExW(
    PCWSTR pName, PCWSTR pServiceName, SOCKET_ADDRESS *pAddresses,
    DWORD dwAddressCount, LPBLOB lpBlob, DWORD dwFlags, DWORD dwNameSpace,
    LPGUID lpNspId, PTIMEVAL timeout, LPOVERLAPPED lpOverlapped,
    LPLOOKUPSERVICE_COMPLETION_ROUTINE lpCompletionRoutine,
    LPHANDLE lpNameHandle);
...
WINSOCK_API_LINKAGE int WSAAPI WSAImpersonateSocketPeer(
    SOCKET Socket, const struct sockaddr *PeerAddress, ULONG peerAddressLen);
WINSOCK_API_LINKAGE int WSAAPI WSAQuerySocketSecurity(
    SOCKET Socket, const SOCKET_SECURITY_QUERY_TEMPLATE *SecurityQueryTemplate,
...
WINSOCK_API_LINKAGE int WSAAPI WSASetSocketPeerTargetName(
    SOCKET Socket, const SOCKET_PEER_TARGET_NAME *PeerTargetName,
    ULONG PeerTargetNameLen, LPWSAOVERLAPPED Overlapped,
    LPWSAOVERLAPPED_COMPLETION_ROUTINE CompletionRoutine);
WINSOCK_API_LINKAGE int WSAAPI WSASetSocketSecurity(
    SOCKET Socket, const SOCKET_SECURITY_SETTINGS *SecuritySettings,
    ULONG SecuritySettingsLen, LPWSAOVERLAPPED Overlapped,
    LPWSAOVERLAPPED_COMPLETION_ROUTINE CompletionRoutine);
#define InetNtopA inet_ntop
WINSOCK_API_LINKAGE LPCWSTR WSAAPI InetNtopW(INT Family, LPCVOID pAddr,
                                             LPWSTR pStringBuf,
