...
static __inline UINT DisplayDibWindowMessage(HWND hwnd, UINT msg, WPARAM wParam,
                                             LPARAM lParam, DWORD cbSize) {
  COPYDATASTRUCT cds;
  cds.dwData = MAKELONG(msg, wParam);
  cds.cbData = lParam ? cbSize : 0;
  cds.lpData = (LPVOID)lParam;
  return (UINT)SendMessage(hwnd, WM_COPYDATA, (WPARAM)(HWND)NULL,
                           (LPARAM)(LPVOID)&cds);
}
static __inline HWND
DisplayDibWindowCreateEx(HWND hwndParent, HINSTANCE hInstance, DWORD dwStyle) {
  DWORD show = 2;
  DWORD zero = 0;
  ...
  if ((UINT)LoadModule(DISPLAYDIB_DLL, &params) < (UINT)HINSTANCE_ERROR)
    return NULL;
  return CreateWindow(
      DISPLAYDIB_WINDOW_CLASS, "", dwStyle, 0, 0, GetSystemMetrics(SM_CXSCREEN),
      GetSystemMetrics(SM_CYSCREEN), hwndParent, NULL,
      (hInstance ? hInstance : GetWindowInstance(hwndParent)), NULL);
}
#define DisplayDibWindowCreate(hwndP, hInstance)                               \
  DisplayDibWindowCreateEx(hwndP, hInstance, WS_POPUP)
#define DisplayDibWindowSetFmt(hwnd, lpbi)                                     \
  DisplayDibWindowMessage(hwnd, DDM_SETFMT, 0, (LPARAM)(LPVOID)(lpbi),         \
                          sizeof(BITMAPINFOHEADER) + 256 * sizeof(RGBQUAD))
#define DisplayDibWindowDraw(hwnd, flags, bits, size)                          \
  DisplayDibWindowMessage(hwnd, DDM_DRAW, (WPARAM)(UINT)(flags),               \
                          (LPARAM)(LPVOID)(bits), (DWORD)(size))
#ifdef __cplusplus
#define DisplayDibWindowBegin(hwnd) ::ShowWindow(hwnd, SW_SHOWNORMAL)
#define DisplayDibWindowEnd(hwnd) ::ShowWindow(hwnd, SW_HIDE)
#define DisplayDibWindowBeginEx(hwnd, f)                                       \
  ::SendMessage(hwnd, DDM_BEGIN, (WPARAM)(UINT)(f), 0)
