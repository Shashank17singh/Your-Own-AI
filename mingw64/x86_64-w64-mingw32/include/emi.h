...
DEFINE_GUID(GUID_DEVICE_ENERGY_METER, 0x45bd8344, 0x7ed6, 0x49cf, 0xa4, 0x40,
            0xc2, 0x76, 0xc9, 0x33, 0xb0, 0x53);
#define IOCTL_EMI_GET_VERSION                                                  \
  CTL_CODE(FILE_DEVICE_UNKNOWN, 0, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_EMI_GET_METADATA_SIZE                                            \
  CTL_CODE(FILE_DEVICE_UNKNOWN, 1, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_EMI_GET_METADATA                                                 \
  CTL_CODE(FILE_DEVICE_UNKNOWN, 2, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_EMI_GET_MEASUREMENT                                              \
  CTL_CODE(FILE_DEVICE_UNKNOWN, 3, METHOD_BUFFERED, FILE_READ_ACCESS)
#define EMI_NAME_MAX 16
#define EMI_VERSION_V1 1
#define EMI_VERSION_V2 2
typedef enum { EmiMeasurementUnitPicowattHours } EMI_MEASUREMENT_UNIT;
typedef struct {
  USHORT EmiVersion;
} EMI_VERSION;
typedef struct {
