...
/* There are 3 separate ways this file is intended to be used:
   1) Included from intrin.h.  In this case, all intrinsics in this file get
   declarations and implementations.  No special #defines are needed for this
   case. 2) Included from the library versions of these functions (ie
   mingw-w64-crt\intrincs\*.c).  All intrinsics in this file must also be
   included in the library.  In this case, only the specific functions requested
   will get defined, and they will not be defined as inline.  If you have
   followed the instructions (below) for adding functions to this file, then all
   you need to have in the .c file is the following: #define
   __INTRINSIC_ONLYSPECIAL #define __INTRINSIC_SPECIAL___stosb // Causes code
   generation in intrin-impl.h #include <intrin.h> 3) Included from various
   platform sdk headers.  Some platform sdk headers (such as winnt.h) define a
   subset of intrinsics.  To avoid potential conflicts, this file is designed to
      allow for specific subsets of functions to be defined.  This is done by
   defining the appropriate variable before including this file: #define
   __INTRINSIC_GROUP_WINNT #include <psdk_inc/intrin-impl.h> In all cases, it is
   acceptable to include this file multiple times in any order (ie include
   winnt.h to get its subset, then include intrin.h to get everything, or vice
   versa). See also the comments at the top of intrin.h.
*/
/* To add an implementation for a new intrinsic to this file, you should comment
out the current prototype in intrin.h. If the function you are adding is not in
intrin.h, you should not be adding it to this file.  This file is only for MSVC
intrinsics. Make sure you put your definition in the right section (x86 vs x64),
and use this outline when adding definitions to this file: #if
__INTRINSIC_PROLOG(__int2c) <prototype goes here>
...
/* Note that there is no file-wide #if to prevent intrin-impl.h from being
   included multiple times.  This is because this file might be included
   multiple times to define various subsets of the functions it contains. */
...
#define __buildstos(x, y, z)                                                   \
  void x(y *Dest, y Data, size_t Count) {                                      \
...
#define __buildlogicali(x, y, o)                                               \
  y x(volatile y *Destination, y Value) {                                      \
    return __sync_fetch_and_##o(Destination, Value);                           \
  }
/* This macro is used by InterlockedBitTestAndSet, InterlockedBitTestAndReset,
   InterlockedBitTestAndComplement, InterlockedBitTestAndSet64,
   InterlockedBitTestAndReset64, InterlockedBitTestAndComplement64
   _interlockedbittestandset, _interlockedbittestandreset,
   _interlockedbittestandcomplement _interlockedbittestandset64,
   _interlockedbittestandreset64, _interlockedbittestandcomplement64 */
/* Parameters: (FunctionName, DataType, AsmCode, OffsetConstraint)
   ...
   OffsetConstraint: either "I" for 32bit data types or "J" for 64. */
    ...
    (defined(_AMD64_) && !defined(_ARM64EC_)) || defined(__i386__) ||          \
    defined(_X86_)
#define __buildbittesti(x, y, z, a)                                            \
  unsigned char x(y volatile *Base, y Offset) {                                \
    unsigned char old;                                                         \
...
#define __buildbittesti(x, y, z, a)                                            \
  unsigned char x(y volatile *Base, y Offset) {                                \
    ...
    __asm__ __volatile__("dmb	sy\n\t"                                          \
                         ...
                         "bne	1b\n\t"                                          \
                         "dmb	sy"                                              \
                         : [old] "=&r"(old), [tmp1] "=&r"(tmp1),               \
                           [tmp2] "=&r"(tmp2), [Base] "+m"(*Base)              \
                         : [bit] a "r"(bit)                                    \
                         : "memory", "cc");                                    \
    return (old >> Offset) & 1;                                                \
  }
...
#define __buildbittesti64(x, y, z, a)                                          \
  unsigned char x(y volatile *Base, y Offset) {                                \
...
#endif /* (defined(__x86_64__) && !defined(__arm64ec__)) || (defined(_AMD64_)  \
          && !defined(_ARM64EC_)) || defined(__i386__) || defined(_X86_) */
...
#define __buildmemorybarrier()                                                 \
  {                                                                            \
    ...
    __asm__ __volatile__("xchg{b %%| }al, %0"                                  \
                         : "=m"(Barrier)                                       \
                         : /* no inputs */                                     \
                         : "eax", "memory");                                   \
  }
...
__readgsqword Parameters: (FunctionName, DataType, Segment) FunctionName: Any
valid function name DataType: char, short, __LONG32 or __int64 Segment: fs or gs
   Type: b, w, l, q
   */
#define __buildreadseg(x, y, z, a)                                             \
  y x(unsigned __LONG32 Offset) {                                              \
    y ret;                                                                     \
    __asm__("mov{" a " %%" z ":%a[offset], %[ret] | %[ret], " z                \
            ":[%[offset]] }"                                                   \
            : [ret] "=r"(ret)                                                  \
            : [offset] "ir"(Offset)                                            \
            : "memory");                                                       \
...
__writegsqword Parameters: (FunctionName, DataType, Segment) FunctionName: Any
valid function name DataType: char, short, __LONG32 or __int64 Segment: fs or gs
   Type: b, w, l, q
   */
#define __buildwriteseg(x, y, z, a)                                            \
  void x(unsigned __LONG32 Offset, y Data) {                                   \
...
_BitScanReverse64 Parameters: (FunctionName, DataType, Segment) FunctionName:
Any valid function name DataType: unsigned __LONG32 or unsigned __int64
   Statement: BSF or BSR */
/* GCC v6 added support for outputting flags.  This allows better code to be
   produced for a number of intrinsics. */
#ifndef __GCC_ASM_FLAG_OUTPUTS__
#define __buildbitscan(x, y, z)                                                \
  unsigned char x(unsigned __LONG32 *Index, y Mask) {                          \
    y n;                                                                       \
    __asm__(z : [Index] "=r"(n) : [Mask] "r"(Mask) : "cc");                    \
    *Index = n;                                                                \
    return Mask != 0;                                                          \
  }
#else
#define __buildbitscan(x, y, z)                                                \
  unsigned char x(unsigned __LONG32 *Index, y Mask) {                          \
    y n;                                                                       \
    unsigned char old;                                                         \
    __asm__(z : "=@ccnz"(old), [Index] "=r"(n) : [Mask] "r"(Mask));            \
    *Index = n;                                                                \
    return old;                                                                \
  }
#endif
...
Parameters: (FunctionName, DataType, OffsetConstraint)
   ...
   OffsetConstraint: either "I" for 32bit data types or "J" for 64.
   */
#define __buildbittest(x, y, z, a)                                             \
  unsigned char x(const y *Base, y Offset) {                                   \
    unsigned char old;                                                         \
...
_bittestandcomplement, _bittestandset64, _bittestandreset64,
_bittestandcomplement64 Parameters: (FunctionName, DataType, Statement,
OffsetConstraint) FunctionName: Any valid function name DataType: __LONG32 or
__int64 Statement: asm statement (bts, btr, btc) OffsetConstraint: either "I"
for 32bit data types or "J" for 64. Type: l, q
   */
#define __buildbittestand(x, y, z, a, b)                                       \
  unsigned char x(y *Base, y Offset) {                                         \
    unsigned char old;                                                         \
...
Parameters: (FunctionName, DataType)
   ...
   Type: b, w, l
   */
#define __build_inport(x, y, z)                                                \
  y x(unsigned short Port) {                                                   \
    y value;                                                                   \
...
Parameters: (FunctionName, DataType)
   ...
   Type: b, w, l
   */
#define __build_outport(x, y, z)                                               \
  void x(unsigned short Port, y Data) {                                        \
...
Parameters: (FunctionName, DataType, InstructionSizeAtt, InstructionSizeIntel)
   ...
   */
#define __build_inportstring(x, y, z, a)                                       \
  void x(unsigned short Port, y *Buffer, unsigned __LONG32 Count) {            \
...
Parameters: (FunctionName, DataType, InstructionSizeAtt, InstructionSizeIntel)
   ...
   InstructionSizeIntel: b, w, d (not b,w,l)
   */
#define __build_outportstring(x, y, z, a)                                      \
  void x(unsigned short Port, y *Buffer, unsigned __LONG32 Count) {            \
...
/* This macro is used by __writecr0, __writecr2, __writecr3, __writecr4,
__writecr8 Parameters: (FunctionName, DataType, RegisterNumber) FunctionName:
Any valid function name DataType: unsigned __LONG32, unsigned __int64
   RegisterNumber: 0, 2, 3, 4, 8
   */
#define __build_writecr(x, y, z)                                               \
  void x(y Data) {                                                             \
...
Parameters: (FunctionName, DataType, RegisterNumber)
   ...
   */
#define __buildmov(x, y, z, a)                                                 \
  void x(y *Destination, y const *Source, size_t Count) {                      \
    __asm__ __volatile__("rep movs{" z "|" a "}"                               \
...
/* If __INTRINSIC_ONLYSPECIAL is defined at this point, we are processing
   case 2.  In that case, don't go looking for groups */
#ifndef __INTRINSIC_ONLYSPECIAL
#ifdef __INTRINSIC_GROUP_WINNT
#undef __INTRINSIC_GROUP_WINNT /* Remove this for efficiency if intrin-impl.h  \
                                  is included again */
...
#undef __INTRINSIC_GROUP_WINBASE /* Remove this for efficiency if              \
                                    intrin-impl.h is included again */
/* Note that this gets undefined at the end of this file */
#define __INTRINSIC_ONLYSPECIAL
#define __INTRINSIC_SPECIAL__InterlockedIncrement
#define __INTRINSIC_SPECIAL__InterlockedDecrement
...
#endif /* __INTRINSIC_PROLOG */
#if (defined(__x86_64__) && !defined(__arm64ec__)) ||                          \
    (defined(_AMD64_) && !defined(_ARM64EC_))
...
__INTRINSICS_USEINLINE
__buildbittesti(_interlockedbittestandset64, __int64,
                "lock bts{q %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "J")
#endif
...
__INTRINSICS_USEINLINE
__buildbittesti(_interlockedbittestandreset64, __int64,
                "lock btr{q %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "J")
#endif
#define __INTRINSIC_DEFINED__interlockedbittestandreset64
    ...
    __MINGW_EXTENSION
    unsigned char _interlockedbittestandcomplement64(__int64 volatile *a,
                                                     __int64 b);
#if !__has_builtin(_interlockedbittestandcomplement64)
__INTRINSICS_USEINLINE
__buildbittesti(_interlockedbittestandcomplement64, __int64,
                "lock btc{q %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "J")
#endif
...
__INTRINSICS_USEINLINE
__buildbittesti(InterlockedBitTestAndSet64, __int64,
                "lock bts{q %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "J")
#endif
...
__INTRINSICS_USEINLINE
__buildbittesti(InterlockedBitTestAndReset64, __int64,
                "lock btr{q %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "J")
#endif
#define __INTRINSIC_DEFINED_InterlockedBitTestAndReset64
    ...
    __MINGW_EXTENSION
    unsigned char InterlockedBitTestAndComplement64(volatile __int64 *a,
                                                    __int64 b);
#if !__has_builtin(InterlockedBitTestAndComplement64)
__INTRINSICS_USEINLINE
__buildbittesti(InterlockedBitTestAndComplement64, __int64,
                "lock btc{q %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "J")
#endif
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE __int64
_InterlockedIncrement64(__int64 volatile *Addend) {
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE __int64
_InterlockedDecrement64(__int64 volatile *Addend) {
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE __int64
_InterlockedExchange64(__int64 volatile *Target, __int64 Value) {
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE __int64
_InterlockedExchangeAdd64(__int64 volatile *Addend, __int64 Value) {
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE __buildreadseg(__readgsqword,
                                                        unsigned __int64, "gs",
                                                        "q")
#endif
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE __buildwriteseg(__writegsqword,
                                                         unsigned __int64, "gs",
                                                         "q")
#endif
...
__buildbitscan(_BitScanForward64, unsigned __int64,
               "bsf{q %[Mask],%[Index] | %[Index],%[Mask]}")
#endif
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE
__buildbitscan(_BitScanReverse64, unsigned __int64,
               "bsr{q %[Mask],%[Index] | %[Index],%[Mask]}")
#endif
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE __buildbittest(_bittest64, __int64,
                                                        "q", "J")
#endif
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE __buildbittestand(_bittestandset64,
                                                           __int64, "bts", "J",
                                                           "q")
#endif
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE __buildbittestand(_bittestandreset64,
                                                           __int64, "btr", "J",
                                                           "q")
#endif
...
#define __INTRINSIC_DEFINED___writecr8
    ...
    __MINGW_EXTENSION
    void __movsq(unsigned __int64 *Dest, unsigned __int64 const *Source,
                 size_t Count);
#if !__has_builtin(__movsq)
__MINGW_EXTENSION __INTRINSICS_USEINLINE __buildmov(__movsq, unsigned __int64,
                                                    "q", "q")
#endif
...
unsigned __int64 __shiftleft128(unsigned __int64 LowPart,
  ...
  __asm__(
      "shld {%[Shift],%[LowPart],%[HighPart]|%[HighPart], %[LowPart], %[Shift]}"
      : [ret] "=r"(ret)
...
unsigned __int64 __shiftright128(unsigned __int64 LowPart,
  ...
  unsigned __int64 ret;
  __asm__(
      "shrd {%[Shift],%[HighPart],%[LowPart]|%[LowPart], %[HighPart], %[Shift]}"
      : [ret] "=r"(ret)
...
__buildbittesti(_interlockedbittestandreset, __LONG32, "bic",
                /* unused param */)
#endif
#define __INTRINSIC_DEFINED__interlockedbittestandreset
#endif /* __INTRINSIC_PROLOG */
#if __INTRINSIC_PROLOG(_interlockedbittestandcomplement)
    unsigned char _interlockedbittestandcomplement(__LONG32 volatile *a,
                                                   __LONG32 b);
#if !__has_builtin(_interlockedbittestandcomplement)
__INTRINSICS_USEINLINE
__buildbittesti(_interlockedbittestandcomplement, __LONG32, "eor",
                /* unused param */)
#endif
...
#define __INTRINSIC_DEFINED_InterlockedBitTestAndReset
    ...
    unsigned char InterlockedBitTestAndComplement(volatile __LONG32 *a,
                                                  __LONG32 b);
#if !__has_builtin(InterlockedBitTestAndComplement)
__INTRINSICS_USEINLINE
__buildbittesti(InterlockedBitTestAndComplement, __LONG32, "eor",
                /* unused param */)
#endif
#define __INTRINSIC_DEFINED_InterlockedBitTestAndComplement
#endif /* __INTRINSIC_PROLOG */
#if __INTRINSIC_PROLOG(_BitScanForward)
    __MINGW_EXTENSION unsigned char _BitScanForward(unsigned __LONG32 *Index,
                                                    unsigned __LONG32 Mask);
#if !__has_builtin(_BitScanForward)
__MINGW_EXTENSION __INTRINSICS_USEINLINE unsigned char
_BitScanForward(unsigned __LONG32 *Index, unsigned __LONG32 Mask) {
  if (Mask == 0)
    return 0;
...
__MINGW_EXTENSION __INTRINSICS_USEINLINE unsigned char
_BitScanReverse(unsigned __LONG32 *Index, unsigned __LONG32 Mask) {
  if (Mask == 0)
    return 0;
...
#if !__has_builtin(_interlockedbittestandset64)
__INTRINSICS_USEINLINE
__buildbittesti64(_interlockedbittestandset64, __int64, "orr",
                  /* unused param */)
#endif
#define __INTRINSIC_DEFINED__interlockedbittestandset64
#endif /* __INTRINSIC_PROLOG */
...
__INTRINSICS_USEINLINE
#if !__has_builtin(_interlockedbittestandreset64)
__buildbittesti64(_interlockedbittestandreset64, __int64, "bic",
                  /* unused param */)
#endif
#define __INTRINSIC_DEFINED__interlockedbittestandreset64
#endif /* __INTRINSIC_PROLOG */
...
#if !__has_builtin(_interlockedbittestandcomplement64)
__INTRINSICS_USEINLINE
__buildbittesti64(_interlockedbittestandcomplement64, __int64, "eor",
                  /* unused param */)
#endif
#define __INTRINSIC_DEFINED__interlockedbittestandcomplement64
#endif /* __INTRINSIC_PROLOG */
...
#if !__has_builtin(InterlockedBitTestAndSet64)
__INTRINSICS_USEINLINE
__buildbittesti64(InterlockedBitTestAndSet64, __int64, "orr",
                  /* unused param */)
#endif
#define __INTRINSIC_DEFINED_InterlockedBitTestAndSet64
#endif /* __INTRINSIC_PROLOG */
...
#if !__has_builtin(InterlockedBitTestAndReset64)
__INTRINSICS_USEINLINE
__buildbittesti64(InterlockedBitTestAndReset64, __int64, "bic",
                  /* unused param */)
#endif
#define __INTRINSIC_DEFINED_InterlockedBitTestAndReset64
#endif /* __INTRINSIC_PROLOG */
...
#if !__has_builtin(InterlockedBitTestAndComplement64)
__INTRINSICS_USEINLINE
__buildbittesti64(InterlockedBitTestAndComplement64, __int64, "eor",
                  /* unused param */)
#endif
#define __INTRINSIC_DEFINED_InterlockedBitTestAndComplement64
#endif /* __INTRINSIC_PROLOG */
...
#if !__has_builtin(_BitScanForward64)
__MINGW_EXTENSION __INTRINSICS_USEINLINE unsigned char
_BitScanForward64(unsigned __LONG32 *Index, unsigned __int64 Mask) {
  if (Mask == 0)
    return 0;
...
#if !__has_builtin(_BitScanReverse64)
__MINGW_EXTENSION __INTRINSICS_USEINLINE unsigned char
_BitScanReverse64(unsigned __LONG32 *Index, unsigned __int64 Mask) {
  if (Mask == 0)
    return 0;
...
#define __INTRINSIC_DEFINED__BitScanReverse64
#endif /* __INTRINSIC_PROLOG */
#endif /* defined(__aarch64__) || define(_ARM64_) || defined(__arm64ec__) ||   \
          defined(_ARM64EC_) */
...
/* ***************************************************** */
    ...
    defined(__aarch64__) || defined(_ARM64_)
...
__INTRINSICS_USEINLINE
unsigned int __popcnt(unsigned int value) { return __builtin_popcount(value); }
#endif
...
__MINGW_EXTENSION __int64 _InterlockedAdd64(__int64 volatile *Addend,
                                            __int64 Value);
#if !__has_builtin(_InterlockedAdd64)
__MINGW_EXTENSION __INTRINSICS_USEINLINE __int64
_InterlockedAdd64(__int64 volatile *Addend, __int64 Value) {
  return __sync_add_and_fetch(Addend, Value);
}
#endif
...
_InterlockedCompareExchange64(__int64 volatile *Destination, __int64 ExChange,
                              __int64 Comperand);
...
__INTRINSICS_USEINLINE
void __int2c(void) { __buildint(0x2c); }
#endif
#define __INTRINSIC_DEFINED___int2c
#endif /* __INTRINSIC_PROLOG */
...
#if !__has_builtin(_interlockedbittestandset)
__INTRINSICS_USEINLINE
__buildbittesti(_interlockedbittestandset, __LONG32,
                "lock bts{l %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "I")
#endif
#define __INTRINSIC_DEFINED__interlockedbittestandset
#endif /* __INTRINSIC_PROLOG */
...
#if !__has_builtin(_interlockedbittestandreset)
__INTRINSICS_USEINLINE
__buildbittesti(_interlockedbittestandreset, __LONG32,
                "lock btr{l %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "I")
#endif
#define __INTRINSIC_DEFINED__interlockedbittestandreset
#endif /* __INTRINSIC_PROLOG */
...
#if !__has_builtin(_interlockedbittestandcomplement)
__INTRINSICS_USEINLINE
__buildbittesti(_interlockedbittestandcomplement, __LONG32,
                "lock btc{l %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "I")
#endif
#define __INTRINSIC_DEFINED__interlockedbittestandcomplement
#endif /* __INTRINSIC_PROLOG */
...
#if !__has_builtin(InterlockedBitTestAndSet)
__INTRINSICS_USEINLINE
__buildbittesti(InterlockedBitTestAndSet, __LONG32,
                "lock bts{l %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "I")
#endif
#define __INTRINSIC_DEFINED_InterlockedBitTestAndSet
...
#if !__has_builtin(InterlockedBitTestAndReset)
__INTRINSICS_USEINLINE
__buildbittesti(InterlockedBitTestAndReset, __LONG32,
                "lock btr{l %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "I")
#endif
#define __INTRINSIC_DEFINED_InterlockedBitTestAndReset
...
#if !__has_builtin(InterlockedBitTestAndComplement)
__INTRINSICS_USEINLINE
__buildbittesti(InterlockedBitTestAndComplement, __LONG32,
                "lock btc{l %[Offset],%[Base] | %[Base],%[Offset]}" __FLAGSET,
                "I")
#endif
#define __INTRINSIC_DEFINED_InterlockedBitTestAndComplement
#endif /* __INTRINSIC_PROLOG */
#if __INTRINSIC_PROLOG(_BitScanForward)
    unsigned char _BitScanForward(unsigned __LONG32 *Index,
                                  unsigned __LONG32 Mask);
#if !__has_builtin(_BitScanForward)
__INTRINSICS_USEINLINE
__buildbitscan(_BitScanForward, unsigned __LONG32,
               "bsf{l %[Mask],%[Index] | %[Index],%[Mask]}")
#endif
#define __INTRINSIC_DEFINED__BitScanForward
#endif /* __INTRINSIC_PROLOG */
#if __INTRINSIC_PROLOG(_BitScanReverse)
    unsigned char _BitScanReverse(unsigned __LONG32 *Index,
                                  unsigned __LONG32 Mask);
#if !__has_builtin(_BitScanReverse)
__INTRINSICS_USEINLINE
__buildbitscan(_BitScanReverse, unsigned __LONG32,
               "bsr{l %[Mask],%[Index] | %[Index],%[Mask]}")
#endif
#define __INTRINSIC_DEFINED__BitScanReverse
#endif /* __INTRINSIC_PROLOG */
#if __INTRINSIC_PROLOG(_bittest)
...
#define __INTRINSIC_DEFINED___inwordstring
    ...
    void __indwordstring(unsigned short Port, unsigned __LONG32 *Buffer,
                         unsigned __LONG32 Count);
...
#define __INTRINSIC_DEFINED___outwordstring
    ...
    void __outdwordstring(unsigned short Port, unsigned __LONG32 *Buffer,
                          unsigned __LONG32 Count);
...
void __movsb(unsigned char *Destination, unsigned char const *Source,
             size_t Count);
...
#define __INTRINSIC_DEFINED___movsb
    ...
    void __movsw(unsigned short *Dest, unsigned short const *Source,
                 size_t Count);
...
#define __INTRINSIC_DEFINED___movsw
    ...
    void __movsd(unsigned __LONG32 *Dest, unsigned __LONG32 const *Source,
                 size_t Count);
...
unsigned __int64 _xgetbv(unsigned int index) {
  ...
  unsigned __LONG32 val1, val2;
#endif /* defined(__x86_64__) || defined(_AMD64_) */
  __asm__ __volatile__("xgetbv" : "=a"(val1), "=d"(val2) : "c"(index));
