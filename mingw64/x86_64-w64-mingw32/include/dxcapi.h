...
#if defined(_MSC_VER)
#define CROSS_PLATFORM_UUIDOF(iface, spec) struct __declspec(uuid(spec)) iface;
...
#define __CRT_UUID_DECL(type, l, w1, w2, b1, b2, b3, b4, b5, b6, b7, b8)       \
  extern "C++" {                                                               \
  template <> struct __mesa_emulated_uuidof_s<type> {                          \
    static constexpr IID __uuid_inst = {                                       \
        l, w1, w2, {b1, b2, b3, b4, b5, b6, b7, b8}};                          \
  };                                                                           \
  template <> constexpr const GUID &__mesa_emulated_uuidof<type>() {           \
...
DxcCreateInstance(_In_ REFCLSID rclsid, _In_ REFIID riid, _Out_ LPVOID *ppv);
extern "C" DXC_API_IMPORT HRESULT __stdcall
...
typedef enum DXC_OUT_KIND {
  ...
  DXC_OUT_SHADER_HASH = 4, // IDxcBlob - DxcShaderHash of shader or shader with
                           // source info (-Zsb/-Zss)
...
struct IDxcContainerBuilder : public IUnknown {
  ...
  virtual HRESULT STDMETHODCALLTYPE SerializeContainer(
      _Out_ IDxcOperationResult *
          *ppResult) = 0; // Builds a container of the given container builder
                          // state
};
