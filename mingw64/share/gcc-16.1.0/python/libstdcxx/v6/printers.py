...
def lookup_templ_spec(templ, *args):
    ...
    t = "{}<{}>".format(
        templ,
        ", ".join(
            [a.tag if isinstance(a, gdb.Type) and a.tag else str(a) for a in args]
        ),
    )
    try:
...
def build_libstdcxx_dictionary():
    ...
    libstdcxx_printer.add_version(
        "std::", "stacktrace_entry", StdStacktraceEntryPrinter
    )
    libstdcxx_printer.add_version("std::", "basic_stacktrace", StdStacktracePrinter)
