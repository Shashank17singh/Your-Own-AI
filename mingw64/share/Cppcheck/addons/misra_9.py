class ElementDef:
    ...
    def getChildByValueElement(self, ed):
        ...
        return (
            self.children[self.children.index(potentialChild)]
            if potentialChild
            else None
        )
...
class InitializerParser:
    ...
    def parseInitializer(self, root, token):
        ...
        while self.token:
            if self.token.str == ",":
                ...
                if (
                    self.token == self.token.astParent.astOperand1
                    and self.token.astParent.astOperand2
                ):
                ...
                if self.ed and self.ed.isValue:
                    if (
                        not isDesignated
                        and len(self.rootStack) > 0
                        and self.rootStack[-1][1] == self.root
                    ):
                        self.rootStack[-1][0].markStuctureViolation(self.token)
                    if (
                        isFirstElement
                        and self.token.str == "0"
                        and self.token.next.str == "}"
                    ):
                        self.root.initializeChildren()
                    elif (
                        self.token.isString
                        and self.ed.valueType
                        and self.ed.valueType.pointer > 0
                    ):
                        if self.ed.valueType.pointer - self.ed.getEffectiveLevel() == 1:
                            ...
                            if (
                                self.root.name != "->"
                                and self.ed.parent.parent != self.root
                            ) or (
                                self.root.name == "->"
                                and self.root.children[0] != self.ed.parent
                            ):
                                self.root.markStuctureViolation(self.token)
                            else:
                    ...
                    while parent and parent != self.root:
                        parent.isDesignated = (
                            isDesignated
                            if isDesignated and not parent.isPositional
                            else parent.isDesignated
                        )
                        parent.isPositional = (
                            not isDesignated
                            if not isDesignated and not parent.isDesignated
                            else parent.isPositional
                        )
    ...
    def unwindAndContinue(self):
        while self.token:
            if (
                self.token.astParent.astOperand1 == self.token
                and self.token.astParent.astOperand2
            ):
                if self.ed:
                    if (
                        self.token.astParent.astOperand2.str == "{"
                        and self.ed.isDesignated
                    ):
                        self.popFromStackIfExitElement()
                    else:
                        self.ed.markAsCurrent()
...
def createArrayChildrenDefs(ed, token, var, rawTokens=None):
    if token and token.str == "[":
        ...
        if (token.astOperand2 is not None) and (
            token.astOperand2.getKnownIntValue() is not None
        ):
    ...
    for scopes in valueType.typeScope.nestedList:
        ...
        if scopes.nestedIn == valueType.typeScope:
            for variable in valueType.typeScope.varlist:
                if (
                    variable.nameToken
                    and variable.nameToken.valueType
                    and variable.nameToken.valueType.typeScope == scopes
                ):
                    varscope = True
                    break
            if not varscope:
