...
def reportNamingError(
    location,
    message,
    errorId="namingConvention",
    severity="style",
    extra="",
    column=None,
):
    cppcheckdata.reportError(
        location, severity, message, "namingng", errorId, extra, columnOverride=column
    )
...
def loadConfig(configfile):
    ...
    if config.include_guard:
        config.include_guard_header_re = config.include_guard.get(
            "RE_HEADERFILE", "[^/].*\\.h\\Z"
        )
    return config
        ...
        if phase == 0:
            if not directive.str.startswith("#ifndef"):
                if conf.include_guard.get("required", 1):
                    report(
                        directive,
                        "first preprocessor directive should be include guard #ifndef",
                        "includeGuardMissing",
                    )
                phase = -1
                continue
            ...
            if not directive.str.startswith("#define"):
                report(
                    directive,
                    "second preprocessor directive should be include guard #define",
                    "includeGuardIncomplete",
                )
                phase = -1
                continue
...
def check_file_naming(conf, data):
    for source_file in data.files:
        ...
        if not good:
            ...
            reportNamingError(
                mockToken, "File name " + basename + " violates naming convention"
            )
...
def check_namespace_naming(conf, data):
    for tk in data.rawTokens:
        ...
        mockToken = DataStruct(
            tk.next.file, tk.next.linenr, tk.next.str, tk.next.column
        )
...
def check_variable_naming(conf, cfg, debugprint):
    for var in cfg.variables:
        ...
        if debugprint:
            ...
            print(
                "original Type Name: " + str(var.nameToken.valueType.originalTypeName)
            )
        ...
        if varType in conf.variable_prefixes:
            ...
            if not var.nameToken.str.startswith(prefix):
                reportNamingError(
                    var.typeStartToken,
                    "Variable " + var.nameToken.str + " violates naming convention",
    ...
    for token in cfg.tokenlist:
        ...
        if retval and retval in conf.function_prefixes:
            if not token.function.name.startswith(conf.function_prefixes[retval]):
                reportNamingError(
                    token,
                    "Function " + token.function.name + " violates naming convention",
                    column=token.column,
                )
        mockToken = DataStruct(
            token.file, token.linenr, token.function.name, token.column
        )
        msgType = "Function"
...
def check_class_naming(conf, cfg):
    for fnc in cfg.functions:
        if fnc.type not in ("Constructor", "Destructor"):
            continue
        mockToken = DataStruct(
            fnc.tokenDef.file, fnc.tokenDef.linenr, fnc.name, fnc.tokenDef.column
        )
...
def process_data(conf, data, cli, debugprint):
    ...
    if conf.include_guard and conf.include_guard.get("required", 1):
        unguarded_include_files = [
            fn for fn in data.files if re.match(conf.include_guard_header_re, fn)
        ]
    for cfg in data.configurations:
        ...
        if conf.private_member:
            check_gpp_naming(
                conf.private_member, cfg, "Private", "Private member variable"
            )
        if conf.public_member:
            check_gpp_naming(
                conf.public_member, cfg, "Public", "Public member variable"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--debugprint", action="store_true", default=False, help="Add debug prints"
    )
    parser.add_argument(
        "--configfile",
        type=str,
        default="namingng.config.json",
