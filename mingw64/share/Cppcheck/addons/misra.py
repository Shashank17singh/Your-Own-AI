...
C11_STDLIB_IDENTIFIERS = {
    ...
    "complex.h": C99_STDLIB_IDENTIFIERS["complex.h"]
    + ["__STDC_NO_COMPLEX__", "CMPLX", "CMPLXF", "CMPLXL"],
    "ctype.h": C99_STDLIB_IDENTIFIERS["ctype.h"],
    "errno.h": C99_STDLIB_IDENTIFIERS["errno.h"]
    + ["__STDC_WANT_LIB_EXT1__", "errno_t"],
...
def is_errno_setting_function(function_name):
    return function_name and function_name in (
        "ftell",
        "fgetpos",
        "fsetpos",
        "fgetwc",
...
def is_composite_expr(expr, composite_operator=False):
    ...
    if not composite_operator:
        if expr.str == "?" and simpleMatch(expr.astOperand2, ":"):
            ...
            return is_composite_expr(colon.astOperand1, True) or is_composite_expr(
                colon.astOperand2, True
            )
        if expr.str in (
            "+",
            "-",
            ...
            return is_composite_expr(expr.astOperand1, True) or is_composite_expr(
                expr.astOperand2, True
            )
        return False
    if expr.isNumber:
        return False
...
def getEssentialTypeCategory(expr):
    ...
    if (
        expr.valueType
        and expr.valueType.typeScope
        and expr.valueType.typeScope.className
    ):
        return "enum<" + expr.valueType.typeScope.className + ">"
    vartok = expr
    while True:
        if simpleMatch(vartok, "[") or (
            vartok and vartok.str == "*" and vartok.astOperand2 is None
        ):
    ...
    if vartok and vartok.variable:
        ...
        while typeToken and typeToken.isName:
            if (
                typeToken.str == "char"
                and not typeToken.isSigned
                and not typeToken.isUnsigned
            ):
                return "char"
    ...
    if expr.isCast and expr.str == "(":
        ...
        while castTok.isName or castTok.str == "*":
            if (
                castTok.str == "char"
                and not castTok.isSigned
                and not castTok.isUnsigned
            ):
                return "char"
...
def getEssentialCategorylist(operand1, operand2):
    if not operand1 or not operand2:
        return None, None
    if operand1.str in ("++", "--") or operand2.str in ("++", "--"):
        return None, None
    ...
    if expr.variable or isCast(expr):
        ...
        if is_constant_integer_expression(expr):
            ...
            if sign1 == sign2 and sign1 in ("signed", "unsigned"):
                e = get_essential_type_from_value(
                    expr.getKnownIntValue(), sign1 == "signed"
                )
                if e:
                    return e
            ...
            if cur_clause == 1 and tn.variable.nameToken == tn:
                ...
                if tn.next and countSideEffectsRecursive(tn.next) > 0:
                    ...
                    if tn_ast and tn_ast == tn.previous:
                        ...
                        tn_ast
                        and tn_ast.str == "."
                        and tn_ast.astOperand2
                        and tn_ast.astOperand2.variable
                    ):
        ...
        if cur_clause == 1 and tn.isName and tn.next.str == "(":
            ...
            for arg_nr in range(len(function_args_in_init)):
                ...
                if function_scope is None:
                    ...
                    if function_scope is None or is_variable_changed(
                        function_body_start, function_body_end, args
                    ):
                        changed = True
                if changed:
                    vars_initialized.add(var_token.variable)
...
def isFloatCounterInWhileLoop(whileToken):
    ...
    elif simpleMatch(whileToken.previous, "} while") and simpleMatch(
        whileToken.previous.link.previous, "do {"
    ):
    ...
    while token != whileBodyStart.link:
        ...
        while parent:
            if (
                ...
                and parent.str != "="
                and parent.astOperand1 == token
            ):
                return True
            if (
                parent.str == "="
                and parent.astOperand1.str == token.str
                and parent.astOperand1 != token
            ):
                return True
...
def countSideEffectsRecursive(expr):
    ...
    if expr.isAssignmentOp or expr.str in {"++", "--"}:
        return 1
    return countSideEffectsRecursive(expr.astOperand1) + countSideEffectsRecursive(
        expr.astOperand2
    )
...
def isUnsignedInt(expr):
    return (
        expr
        and expr.valueType
        and expr.valueType.type in ("short", "int")
        and expr.valueType.sign == "unsigned"
    )
...
def getTernaryOperandsRecursive(token):
    ...
    if token.astOperand2.astOperand1 and not isTernaryOperator(
        token.astOperand2.astOperand1
    ):
        result += [token.astOperand2.astOperand1]
    result += getTernaryOperandsRecursive(token.astOperand2.astOperand2)
    if token.astOperand2.astOperand2 and not isTernaryOperator(
        token.astOperand2.astOperand2
    ):
...
class Define:
    def __init__(self, directive):
        ...
        self.expansionList = ""
        res = re.match(
            r"#define ([A-Za-z0-9_]+)\(([A-Za-z0-9_, ]+)\)[ ]+(.*)", directive.str
        )
        if res:
...
class MisraChecker:
            ...
            summary.append(
                {
                    ...
                    "used": used,
                    "file": scope.bodyStart.file,
                    "line": scope.bodyStart.linenr,
    ...
    def _save_ctu_summary_identifiers(self, dumpfile, cfg):
        ...
        cppcheckdata.reportSummary(
            dumpfile, "MisraExternalIdentifiers", external_identifiers
        )
        cppcheckdata.reportSummary(
            dumpfile, "MisraInternalIdentifiers", internal_identifiers
        )
        ...
        for token in cfg.tokenlist:
            ...
            if token.function and token != token.function.tokenDef:
                ...
                if token == token.variable.nameToken:
                    continue
                if (
                    token.variable.access == "Global"
                    and (not token.variable.isStatic)
                    and (token.str not in names)
                ):
                    names.append({"name": token.str, "file": token.file})
        if len(names) > 0:
    ...
    def misra_2_5(self, dumpfile, cfg):
        ...
        for directive in cfg.directives:
            res_define = re.match(
                r"#define[ \t]+([a-zA-Z_][a-zA-Z_0-9]*).*", directive.str
            )
            res_undef = re.match(
                r"#undef[ \t]+([a-zA-Z_][a-zA-Z_0-9]*).*", directive.str
            )
            if res_define:
                macro_name = res_define.group(1)
                unused_macro[macro_name] = {
                    "name": macro_name,
                    "used": (macro_name in used_macros),
                    "file": directive.file,
                    "line": directive.linenr,
        ...
        if unused_macro:
            cppcheckdata.reportSummary(
                dumpfile, "MisraMacro", list(unused_macro.values())
            )
            ...
            for scope in data.scopes:
                if (scope.type == "Function") and (scope.function == func):
                    ...
                    while (
                        ...
                        and token != scope.bodyEnd
                        and len(func_param_list) > 0
                    ):
                        if (
                            token.variable is not None
                            and token.variable in func_param_list
                        ):
    ...
    def misra_4_1(self, rawTokens):
        for token in rawTokens:
            ...
            for sequence in ["\\" + t for t in symbols.split("\\")][1:]:
                if (
                    isHexEscapeSequence(sequence)
                    or isOctalEscapeSequence(sequence)
                    or isSimpleEscapeSequence(sequence)
                ):
                    continue
        ...
        for scope in scopeVars:
            ...
            for i, variable1 in enumerate(scopeVars[scope]["varlist"]):
                for variable2 in scopeVars[scope]["varlist"][i + 1 :]:
                    ...
                    if hasExternalLinkage(variable1) or hasExternalLinkage(variable2):
                        continue
                    if (
                        variable1.nameToken.str[:num_sign_chars]
                        == variable2.nameToken.str[:num_sign_chars]
                        and variable1 is not variable2
                    ):
                ...
                for innerscope in scopeVars[scope]["scopelist"]:
                    if (
                        variable1.nameToken.str[:num_sign_chars]
                        == innerscope.className[:num_sign_chars]
                    ):
                        if int(variable1.nameToken.linenr) > int(
            ...
            for i, scopename1 in enumerate(scopeVars[scope]["scopelist"]):
                for scopename2 in scopeVars[scope]["scopelist"][i + 1 :]:
                    if (
                        scopename1.className[:num_sign_chars]
                        == scopename2.className[:num_sign_chars]
                    ):
        ...
        for mvar in macro_w_arg:
            for i, macroparam1 in enumerate(macro[mvar]["params"]):
                for j, macroparam2 in enumerate(macro[mvar]["params"]):
                    if (
                        j > i
                        and macroparam1[:num_sign_chars] == macroparam2[:num_sign_chars]
                    ):
    ...
    def misra_6_1(self, data):
        for token in data.tokenlist:
            ...
            if data.standards.c == "c89":
                if token.valueType.type != "int" and not isUnsignedType(
            ...
            while typeToken:
                if (
                    typeToken.isUnsigned
                    or typeToken.isSigned
                    or isUnsignedType(typeToken.str)
                ):
    ...
    def misra_7_2(self, data):
        for token in data.tokenlist:
            if (
                token.isInt
                and ("U" not in token.str.upper())
                and token.valueType
                and token.valueType.sign == "unsigned"
            ):
    ...
    def misra_7_4(self, data):
        ...
        for token in data.tokenlist:
            ...
            if (
                ...
                and token.astOperand1.function
            ):
                ...
                if functionDeclaration.tokenDef:
                    ...
                    for i in range(len(parametersUsed)):
                        ...
                        if (
                            usedParameter.isString
                            and parameterDefinition
                            and parameterDefinition.nameToken
                        ):
                            reportErrorIfVariableIsNotConst(
                                parameterDefinition.nameToken, usedParameter
                            )
    ...
    def misra_8_2(self, data, rawTokens):
            ...
            for rawToken in rawTokens:
                if (
                    ...
                    for _ in range(count):
                        ...
                        while rawToken and (
                            rawToken.str.startswith("/*")
                            or rawToken.str.startswith("//")
                        ):
                            rawToken = rawToken.next
                        if rawToken is None:
                            break
        ...
        for var in data.variables:
            ...
            if len(rawTokensFollowingPtr) != 3:
                continue
            if (
                rawTokensFollowingPtr[0].str == ")"
                and rawTokensFollowingPtr[1].str == "("
                and rawTokensFollowingPtr[2].str == ")"
            ):
    ...
    def misra_8_11(self, data):
        for var in data.variables:
            if (
                var.isExtern
                and simpleMatch(var.nameToken.next, "[ ]")
                and var.nameToken.scope.type == "Global"
            ):
        ...
        for scope in data.scopes:
            ...
            while e_token != scope.bodyEnd:
                ...
                if (
                    e_token.isName
                    and e_token.values
                    and e_token.valueType
                    and e_token.valueType.typeScope == scope
                ):
    ...
    def misra_10_1(self, data):
        for token in data.tokenlist:
            ...
            for t1, t2 in itertools.product(
                list(
                    getTernaryOperandsRecursive(token.astOperand1)
                    or [token.astOperand1]
                ),
                list(
                    getTernaryOperandsRecursive(token.astOperand2)
                    or [token.astOperand2]
                ),
            ):
    ...
    def misra_10_2(self, data):
        ...
        def isEssentiallyChar(op):
            if op is None:
                return False
            if op.str == "+":
                return isEssentiallyChar(op.astOperand1) or isEssentiallyChar(
                    op.astOperand2
                )
            return op.isChar
        ...
        for token in data.tokenlist:
            if token.str not in ("+", "-"):
                continue
            if (not isEssentiallyChar(token.astOperand1)) and (
                not isEssentiallyChar(token.astOperand2)
            ):
                continue
            if token.str == "+":
                if isEssentiallyChar(
                    token.astOperand1
                ) and not isEssentiallySignedOrUnsigned(token.astOperand2):
                    self.reportError(token, 10, 2)
                if isEssentiallyChar(
                    token.astOperand2
                ) and not isEssentiallySignedOrUnsigned(token.astOperand1):
                    self.reportError(token, 10, 2)
            if token.str == "-":
                e1 = getEssentialType(token.astOperand1)
                if e1 and e1.split(" ")[-1] != "char":
                    self.reportError(token, 10, 2)
                if not isEssentiallyChar(
                    token.astOperand2
                ) and not isEssentiallySignedOrUnsigned(token.astOperand2):
                    self.reportError(token, 10, 2)
    ...
    def misra_10_3(self, cfg):
        ...
        for tok in cfg.tokenlist:
            ...
            find_std = (
                cfg.standards.c
                if cfg.standards and cfg.standards.c
                else self.stdversion
            )
            rhs_tok = tok.astOperand2
            rhs_macro_name = rhs_tok.macroName if rhs_tok else None
            rhs_spelling = (
                rhs_macro_name if rhs_macro_name in ("true", "false") else rhs_tok.str
            )
            ...
            if (
                lhs_category
                and rhs_category
                and lhs_category != rhs_category
                and rhs_category not in ("signed", "unsigned")
            ):
    ...
    def misra_10_4(self, data):
        ...
        for token in data.tokenlist:
            ...
            if (
                ...
                and e2
                and (
                    e1.find("Anonymous") != -1 and (e2 == "signed" or e2 == "unsigned")
                )
            ):
                continue
            if (
                ...
                and e2
                and (
                    e2.find("Anonymous") != -1 and (e1 == "signed" or e1 == "unsigned")
                )
            ):
                continue
            if e1 and e2 and e1 != e2:
    ...
    def misra_10_5(self, cfg):
        ...
        for token in cfg.tokenlist:
            ...
            if to_type == "bool" or from_type == "bool":
                if (
                    token.astOperand1.isInt
                    and token.astOperand1.getKnownIntValue() == 1
                ):
                    continue
    ...
    def misra_10_7(self, cfg):
        for token in cfg.tokenlist:
            ...
            sibling = (
                parent.astOperand1
                if (token == parent.astOperand2)
                else parent.astOperand2
            )
    ...
    def misra_10_8(self, data):
        for token in data.tokenlist:
            ...
            if not token.valueType or token.valueType.pointer > 0:
                continue
            if (
                not token.astOperand1.valueType
                or token.astOperand1.valueType.pointer > 0
            ):
                continue
            ...
            if e1 != e2:
                ...
                try:
                    ...
                    if not e:
                        continue
                    if bitsOfEssentialType(token.valueType.type) > bitsOfEssentialType(
                        e
                    ):
                        self.reportError(token, 10, 8)
                except ValueError:
    ...
    def misra_11_2(self, data):
        def get_pointer_type(type_token):
            ...
            if not type_token.isName:
                return None
            return (
                type_token if (type_token.next and type_token.next.str == "*") else None
            )
        ...
        for token in data.tokenlist:
            if (
                ...
                and token.next
                and token.next.next
                and token.next.isName
                and token.next.next.str == ";"
            ):
    ...
    def misra_11_4(self, data):
        ...
        for token in data.tokenlist:
            ...
            if (
                vt2.pointer > 0
                and vt1.pointer == 0
                and (vt1.isIntegral() or vt1.isEnum())
                and vt2.type != "void"
            ):
                ...
                vt1.pointer > 0
                and vt2.pointer == 0
                and (vt2.isIntegral() or vt2.isEnum())
                and vt1.type != "void"
            ):
                if (
                    token.macroName is not None
                    and token.macroName == token.astOperand1.macroName
                    and token.astOperand1.isInt
                    and token.link.previous.str == "*"
                    and token.macroName == token.link.previous.macroName
                    and token.macroName in macros
                    and len(macros[token.macroName]) == 1
                ):
        ...
        for token in data.tokenlist:
            if (
                ...
                if (
                    vt1.pointer > 0
                    and vt2.pointer == 0
                    and token.astOperand2.str == "NULL"
                ):
                    continue
                if (
                    token.astOperand2.values
                    and vt1.pointer > 0
                    and vt2.pointer == 0
        ...
        for token in data.tokenlist:
            if token.str == "for":
                ...
                if countSideEffectsRecursive(expressions[2]) > 1:
                    self.reportError(token, 14, 2)
                counter_vars_first_clause, counter_vars_exit_modified = (
                    getForLoopCounterVariables(token, data)
                )
                if len(counter_vars_exit_modified) == 0:
                    ...
                    if (
                        expressions[0] is not None
                        and loop_counter not in counter_vars_first_clause
                    ):
                        self.reportError(token, 14, 2)
    ...
    def misra_15_4(self, data):
        ...
        for token in data.tokenlist:
            if token.str not in ["break", "goto"]:
                continue
            affectedLoopScopes = []
            getLoopsAffectedByBreak(
                affectedLoopScopes, token.scope, token.str == "goto"
            )
    ...
    def misra_16_1(self, cfg):
        for scope in cfg.scopes:
            ...
            while tok != scope.bodyEnd:
                ...
                if tok.str == "{":
                    tok = tok.link
                    if tok.scope.type == "Unconditional" and simpleMatch(
                        tok.previous.previous, "break ;"
                    ):
                        in_case_or_default = False
                tok = tok.next
    ...
    def misra_16_6(self, data):
        for token in data.tokenlist:
            if not (
                simpleMatch(token, "switch (") and simpleMatch(token.next.link, ") {")
            ):
                continue
    ...
    def misra_17_2(self, data):
        def find_recursive_call(
            search_for_function, direct_call, calls_map, visited=None
        ):
            if visited is None:
                visited = set()
        ...
        for token in cfg.tokenlist:
            if token.isName and token.function is None and token.valueType is None:
                if (
                    token.next
                    and token.next.str == "("
                    and token.next.valueType is None
                ):
                    if (
                        token.next.next.str == "*"
                        and token.next.next.next.isName
                        and token.next.next.next.valueType is not None
                        and token.next.next.next.valueType.pointer > 0
                    ):
                        continue
                    if not isKeyword(token.str, cfg.standards.c) and not isStdLibId(
                        token.str, cfg.standards.c
                    ):
    ...
    def misra_config(self, data):
        for var in data.variables:
            if (
                not var.isArray
                or var.nameToken is None
                or not cppcheckdata.simpleMatch(var.nameToken.next, "[")
            ):
                continue
            ...
            while tok.str == "[":
                ...
                if sz and sz.getKnownIntValue() is None:
                    ...
                    if not unknown_constant and not has_var:
                        self.report_config_error(
                            tok, "Unknown array size, please review configuration"
                        )
                tok = tok.link.next
        for token in data.tokenlist:
            ...
            while tok != end_token:
                ...
                if tok.next.str == "(" or tok.str in ["EOF"]:
                    continue
                if isKeyword(tok.str, data.standards.c) or isStdLibId(
                    tok.str, data.standards.c
                ):
                    continue
    ...
    def misra_17_7(self, data):
        for token in data.tokenlist:
            ...
            if token.astOperand1 is None or not token.astOperand1.isName:
                continue
            if token.astOperand1.varId and (
                token.astOperand1.variable is None
                or get_function_pointer_type(token.astOperand1.variable.typeStartToken)
                is None
            ):
                continue
    ...
    def misra_18_8(self, data):
        for var in data.variables:
            ...
            if not typetok.astOperand2:
                continue
            if not isConstantExpression(
                typetok.astOperand2
            ) and not isUnknownConstantExpression(typetok.astOperand2):
    ...
    def misra_20_12(self, cfg):
        ...
        def _is_arg_macro_usage(directive, arg):
            for macro_usage in cfg.macro_usage:
                if (
                    macro_usage.file == directive.file
                    and macro_usage.linenr == directive.linenr
                ):
                    for macro_usage_arg in cfg.macro_usage:
                        if macro_usage_arg == macro_usage:
                            continue
                        if (
                            macro_usage.usefile == macro_usage_arg.usefile
                            and macro_usage.uselinenr == macro_usage_arg.uselinenr
                            and macro_usage.usecolumn == macro_usage_arg.usecolumn
                        ):
                            return True
            return False
    ...
    def misra_20_14(self, data):
        ...
        for directive in data.directives:
            if (
                directive.str.startswith("#if ")
                or directive.str.startswith("#ifdef ")
    ...
    def misra_21_3(self, data):
        for token in data.tokenlist:
            if isFunctionCall(token, data.standards.c) and (
                token.astOperand1.str in ("malloc", "calloc", "realloc", "free")
            ):
    ...
    def misra_21_6(self, data):
        for token in data.tokenlist:
            ...
            if funcname in standard_id.get(
                "stdio.h", []
            ) or funcname in standard_id.get("wchar.h", []):
                self.reportError(token, 21, 6)

    def misra_21_7(self, data):
        for token in data.tokenlist:
            if isFunctionCall(token, data.standards.c) and (
                token.astOperand1.str in ("atof", "atoi", "atol", "atoll")
            ):
    ...
    def misra_21_8(self, data):
        for token in data.tokenlist:
            if isFunctionCall(token, data.standards.c) and (
                token.astOperand1.str in ("abort", "exit", "getenv")
            ):
        ...
        for token in cfg.tokenlist:
            if token.str in (
                ...
                while simpleMatch(parent.astParent, "+"):
                    parent = parent.astParent
                if (
                    simpleMatch(parent.astParent, "=")
                    and parent == parent.astParent.astOperand2
                ):
                    lhs = parent.astParent.astOperand1
                    if (
                        lhs
                        and lhs.valueType
                        and lhs.valueType.pointer > 0
                        and lhs.valueType.constness == 0
                    ):
            ...
            if token.str == "=":
                ...
                if (
                    lhs
                    and lhs.variable
                    and simpleMatch(lhs.variable.typeStartToken, "lconv")
                ):
                    self.reportError(token, 21, 19)
        ...
        for token in cfg.tokenlist:
            ...
            if token.str in (
                ...
                if name and name == token.str:
                    ...
                    parent = token.next
                    while parent.astParent and (
                        parent.astParent.str == "+" or isCast(parent.astParent)
                    ):
                        parent = parent.astParent
    ...
    def misra_22_5(self, cfg):
        for token in cfg.tokenlist:
            if token.isUnaryOp("*") or (token.isBinaryOp() and token.str == "."):
                ...
                if fileptr.variable and cppcheckdata.simpleMatch(
                    fileptr.variable.typeStartToken, "FILE *"
                ):
    ...
    def misra_22_7(self, cfg):
        for eofToken in cfg.tokenlist:
            ...
            while (
                isCast(eofTokenSibling)
                and eofTokenSibling.valueType
                and eofTokenSibling.valueType.type
                and eofTokenSibling.valueType.type == "int"
            ):
                eofTokenSibling = (
                    eofTokenSibling.astOperand2
                    if eofTokenSibling.astOperand2
                    else eofTokenSibling.astOperand1
                )
            if (
                eofTokenSibling is not None
                and eofTokenSibling.valueType
                and eofTokenSibling.valueType
                and eofTokenSibling.valueType.type in ("bool", "char", "short")
            ):
    ...
    def misra_22_10(self, cfg):
        ...
        for token in cfg.tokenlist:
            if (
                ...
                and token.next
                and token.next.str == "("
                and not simpleMatch(token.next.link, ") {")
            ):
                name, _ = cppcheckdata.get_function_call_name_args(token)
    ...
    def addSuppressedRule(
        self, ruleNum, fileName=None, lineNumber=None, symbolName=None
    ):
        """
                ...
                for item in ruleItemList:
                    ...
                    outlist.append(
                        ...
                        % (
                            ...
                            fname,
                            item_str,
                            self.suppressionStats.get(ruleNum, 0),
                        )
                    )
    ...
    def report_config_error(self, location, errmsg):
        ...
        if self.settings.verify:
            ...
            cppcheckdata.reportError(
                location, cppcheck_severity, errmsg, "misra", error_id
            )
        ...
        if self.settings.verify:
            ...
            if ruleNum in self.ruleTexts:
                ...
                if self.ruleText_filename is None:
                    errmsg = (
                        "misra violation (use --rule-texts=<file> to get proper output)"
                    )
                else:
                    errmsg = (
                        "misra violation (rule-texts-file not found: "
                        + self.ruleText_filename
                        + ")"
                    )
            else:
                errmsg = (
    ...
    def loadRuleTexts(self, filename):
        ...
        for line in file_stream:
            ...
            if not appendixA:
                if (
                    line.find("Appendix A") >= 0
                    and line.find("Summary of guidelines") >= 10
                ):
                    appendixA = True
                continue
    ...
    def parseDump(self, dumpfile, path_premium_addon=None):
        ...
        if self.settings.verify:
            ...
            include_re = re.compile(
                r'^#include [<"]([a-zA-Z0-9]+[a-zA-Z\-_./\\0-9]*)[">]$'
            )
                ...
                for line in open(filename, "rt"):
                    ...
                    if summary_type == "MisraTypedefInfo":
                        for new_typedef_info in summary_data:
                            ...
                            if existing_typedef_info:
                                if is_different_location(
                                    existing_typedef_info, new_typedef_info
                                ):
                                    self.reportError(
                                        Location(existing_typedef_info), 5, 6
                                    )
                                    self.reportError(Location(new_typedef_info), 5, 6)
                                else:
                                    existing_typedef_info["used"] = (
                                        existing_typedef_info["used"]
                                        or new_typedef_info["used"]
                                    )
                            else:
                    ...
                    if summary_type == "MisraTagName":
                        for new_tagname_info in summary_data:
                            ...
                            if existing_tagname_info:
                                if is_different_location(
                                    existing_tagname_info, new_tagname_info
                                ):
                                    self.reportError(
                                        Location(existing_tagname_info), 5, 7
                                    )
                                    self.reportError(Location(new_tagname_info), 5, 7)
                                else:
                                    existing_tagname_info["used"] = (
                                        existing_tagname_info["used"]
                                        or new_tagname_info["used"]
                                    )
                            else:
                    ...
                    if summary_type == "MisraMacro":
                        for new_macro in summary_data:
                            ...
                            if existing_macro:
                                existing_macro["used"] = (
                                    existing_macro["used"] or new_macro["used"]
                                )
                            else:
                    ...
                    if summary_type == "MisraExternalIdentifiers":
                        for s in sorted(
                            ...
                            if name in all_external_identifiers:
                                if is_declaration and is_different_location(
                                    s, all_external_identifiers[name]
                                ):
                                    self.reportError(Location(s), 8, 5)
                                    self.reportError(
                                        Location(all_external_identifiers[name]), 8, 5
                                    )
                                elif is_different_file(
                                    s, all_external_identifiers[name]
                                ):
                                    self.reportError(Location(s), 8, 6)
                                    self.reportError(
                                        Location(all_external_identifiers[name]), 8, 6
                                    )
                        ...
                        for s in summary_data:
                            if s["name"] in all_internal_identifiers:
                                if (
                                    not s["inlinefunc"]
                                    or s["file"]
                                    != all_internal_identifiers[s["name"]]["file"]
                                ):
                                    self.reportError(Location(s), 5, 9)
                                    self.reportError(
                                        Location(all_internal_identifiers[s["name"]]),
                                        5,
                                        9,
                                    )
...
def get_args_parser():
    ...
    parser.add_argument(
        "--no-summary", help="Hide summary of violations", action="store_true"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--severity",
        type=str,
        help="Set a custom severity string, for example 'error' or 'warning'. ",
    )
    return parser
