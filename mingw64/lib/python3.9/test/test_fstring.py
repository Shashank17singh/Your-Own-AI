...
f'{a * f"-{x()}-"}'"""
    ...
    @unittest.skipIf(
        use_old_parser(), "The old parser gets the offsets incorrectly for fstrings"
    )
...
4}""",
    ...
    def test_invalid_string_prefixes(self):
        ...
        error_msg = (
            "invalid syntax" if use_old_parser() else "unexpected EOF while parsing"
        )
        self.assertAllRaise(
            SyntaxError, error_msg, single_quote_cases + double_quote_cases
        )
    ...
    @unittest.skipIf(
        use_old_parser(), "The old parser only supports <fstring> as the filename"
    )
