...
class BaseTaskTests:
    ...
    def test_task_del_collect(self):
        ...
        self.loop.run_until_complete(
            asyncio.gather(
                *[self.new_task(self.loop, run()) for _ in range(100)], loop=self.loop
            )
        )
    ...
    def test_task_repr_coro_decorator(self):
        ...
        self.assertEqual(notmuch.__name__, "notmuch")
        self.assertRegex(
            notmuch.__qualname__,
            r"\w+.test_task_repr_coro_decorator" r"\.<locals>\.notmuch",
        )
        self.assertEqual(notmuch.__module__, __name__)
    ...
    def test_coroutine_never_yielded(self, m_log):
        ...
        regex = (
            ...
            % (
                ...
                re.escape(func_filename),
                func_lineno,
                re.escape(tb_filename),
                tb_lineno,
            )
        )
        ...
        for cancel_args, expected_args in cases:
            with self.subTest(cancel_args=cancel_args):
                ...
                async def test():
                    ...
                    while True:
                        ...
                        await asyncio.gather(
                            asyncio.sleep(0.05), return_exceptions=True, loop=loop
                        )
                        if time > 1:
                            return
                ...
                try:
                    ...
                    self.assertEqual(
                        (exc_type, exc_args), (asyncio.CancelledError, expected_args)
                    )
...
class GatherTestsBase:
    ...
    def test_env_var_debug(self):
        code = "\n".join(
            ("import asyncio.coroutines", "print(asyncio.coroutines._DEBUG)")
        )
...
class CompatibilityTests(test_utils.TestCase):
    ...
    def test_debug_mode_interop(self):
        ...
        assert_python_ok(
            "-Wignore::DeprecationWarning", "-c", code, PYTHONASYNCIODEBUG="1"
        )
