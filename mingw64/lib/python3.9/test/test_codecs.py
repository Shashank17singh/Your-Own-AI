...
class ReadTest(MixInCheckStateHandling):
    ...
    def test_lone_surrogates(self):
        ...
        for before, after in [("\U00010fff", "A"), ("[", "]"), ("A", "\U00010fff")]:
            ...
            test_sequence = (
                bom + before_sequence + self.ill_formed_sequence + after_sequence
            )
...
class UTF32Test(ReadTest, unittest.TestCase):
    ...
    def test_issue8941(self):
        ...
        self.assertEqual("\U00010000" * 1024, codecs.utf_32_decode(encoded_le)[0])
        encoded_be = b"\x00\x00\xfe\xff" + b"\x00\x01\x00\x00" * 1024
        self.assertEqual("\U00010000" * 1024, codecs.utf_32_decode(encoded_be)[0])
...
class UTF32LETest(ReadTest, unittest.TestCase):
    ...
    def test_issue8941(self):
        ...
        self.assertEqual("\U00010000" * 1024, codecs.utf_32_le_decode(encoded)[0])
...
class UTF32BETest(ReadTest, unittest.TestCase):
    ...
    def test_issue8941(self):
        ...
        self.assertEqual("\U00010000" * 1024, codecs.utf_32_be_decode(encoded)[0])
...
class UTF7Test(ReadTest, unittest.TestCase):
    ...
    def test_ascii(self):
        ...
        other_ascii = "".join(
            sorted(set(bytes(range(0x80)).decode()) - set(set_d + set_o + "+" + ws))
        )
        self.assertEqual(
    ...
    def test_nonbmp(self):
        ...
        self.assertEqual(b"+IKzYAdyg".decode(self.encoding), "\u20ac\U000104a0")
        self.assertEqual(
            "\u20ac\u20ac\U000104a0".encode(self.encoding), b"+IKwgrNgB3KA-"
        )
        self.assertEqual(
...
class EscapeDecodeTest(unittest.TestCase):
    ...
    def test_errors(self):
        decode = codecs.escape_decode
        self.assertRaises(ValueError, decode, rb"\x")
        self.assertRaises(ValueError, decode, rb"[\x]")
        self.assertEqual(decode(rb"[\x]\x", "ignore"), (b"[]", 6))
        self.assertEqual(decode(rb"[\x]\x", "replace"), (b"[?]?", 6))
        self.assertRaises(ValueError, decode, rb"\x0")
        self.assertRaises(ValueError, decode, rb"[\x0]")
        self.assertEqual(decode(rb"[\x0]\x0", "ignore"), (b"[]", 8))
        self.assertEqual(decode(rb"[\x0]\x0", "replace"), (b"[?]?", 8))
...
class IDNACodecTest(unittest.TestCase):
    ...
    def test_incremental_encode(self):
        ...
        self.assertEqual(
            b"".join(codecs.iterencode("pyth\xf6n.org.", "idna")), b"xn--pythn-mua.org."
        )
        self.assertEqual(
            b"".join(codecs.iterencode("pyth\xf6n.org.", "idna")), b"xn--pythn-mua.org."
        )
...
class CodecsModuleTest(unittest.TestCase):
    ...
    def test_decode(self):
        ...
        # test keywords
        self.assertEqual(
            codecs.decode(obj=b"\xe4\xf6\xfc", encoding="latin-1"), "\xe4\xf6\xfc"
        )
        self.assertEqual(codecs.decode(b"[\xff]", "ascii", errors="ignore"), "[]")

    def test_encode(self):
        self.assertEqual(codecs.encode("\xe4\xf6\xfc", "latin-1"), b"\xe4\xf6\xfc")
        ...
        # test keywords
        self.assertEqual(
            codecs.encode(obj="\xe4\xf6\xfc", encoding="latin-1"), b"\xe4\xf6\xfc"
        )
        self.assertEqual(codecs.encode("[\xff]", "ascii", errors="ignore"), b"[]")
    ...
    def test_open(self):
        ...
        for mode in ("w", "r", "r+", "w+", "a", "a+"):
            with self.subTest(mode), codecs.open(support.TESTFN, mode, "ascii") as file:
...
class CharmapTest(unittest.TestCase):
    ...
    def test_decode_with_int2int_map(self):
        ...
        # Issue #15379
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "strict", {0: 0x10FFFF, 1: b, 2: c}),
            ("\U0010ffffbc", 3),
        )
        ...
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "replace", {0: a, 1: b, 2: 0xFFFE}),
            ("ab\ufffd", 3),
        )
        ...
        self.assertEqual(
            codecs.charmap_decode(
                b"\x00\x01\x02", "backslashreplace", {0: a, 1: b, 2: 0xFFFE}
            ),
            ("ab\\x02", 3),
        )
        ...
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "ignore", {0: a, 1: b, 2: 0xFFFE}),
            ("ab", 3),
        )
...
class TypesTest(unittest.TestCase):
    ...
    def test_unicode_escape(self):
        ...
        self.assertEqual(codecs.unicode_escape_decode(rb"\u1234"), ("\u1234", 6))
        self.assertEqual(codecs.raw_unicode_escape_decode(r"\u1234"), ("\u1234", 6))
        self.assertEqual(codecs.raw_unicode_escape_decode(rb"\u1234"), ("\u1234", 6))

        self.assertRaises(
            UnicodeDecodeError, codecs.unicode_escape_decode, rb"\U00110000"
        )
        self.assertEqual(
            codecs.unicode_escape_decode(r"\U00110000", "replace"), ("\ufffd", 10)
        )
        self.assertEqual(
            codecs.unicode_escape_decode(r"\U00110000", "backslashreplace"),
            (r"\x5c\x55\x30\x30\x31\x31\x30\x30\x30\x30", 10),
        )

        self.assertRaises(
            UnicodeDecodeError, codecs.raw_unicode_escape_decode, rb"\U00110000"
        )
        self.assertEqual(
            codecs.raw_unicode_escape_decode(r"\U00110000", "replace"), ("\ufffd", 10)
        )
        self.assertEqual(
            codecs.raw_unicode_escape_decode(r"\U00110000", "backslashreplace"),
            (r"\x5c\x55\x30\x30\x31\x31\x30\x30\x30\x30", 10),
        )
...
class UnicodeEscapeTest(unittest.TestCase):
    ...
    def test_decode_errors(self):
        ...
        self.assertRaises(UnicodeDecodeError, decode, rb"\U00110000")
        self.assertEqual(decode(rb"\U00110000", "ignore"), ("", 10))
        self.assertEqual(decode(rb"\U00110000", "replace"), ("\ufffd", 10))
...
class ExceptionChainingTest(unittest.TestCase):
    ...
    def test_unflagged_non_text_codec_handling(self):
        ...
        fmt = (
            r"^{!r} encoder returned 'str' instead of 'bytes'; "
        ...
        with self.assertRaisesRegex(TypeError, msg):
            "str_input".encode(self.codec_name)
        fmt = (
            r"^{!r} decoder returned 'bytes' instead of 'str'; "
...
class CodePageTest(unittest.TestCase):
    ...
    def test_code_page_name(self):
        self.assertRaisesRegex(
            UnicodeEncodeError, "cp932", codecs.code_page_encode, 932, "\xff"
        )
        self.assertRaisesRegex(
            UnicodeDecodeError,
            "cp932",
...
class LocaleCodecTest(unittest.TestCase):
    ...
    def test_encode_surrogatepass(self):
        try:
            ...
            if str(exc) == "unsupported error handler":
                self.skipTest(
                    f"{self.ENCODING!r} encoder doesn't support "
    ...
    def test_decode_surrogatepass(self):
        try:
            ...
            if str(exc) == "unsupported error handler":
                self.skipTest(
                    f"{self.ENCODING!r} decoder doesn't support "
                    f"surrogatepass error handler"
                )
            else:
                raise
