...
class ParseArgsTestCase(unittest.TestCase):
    ...
    def test_coverdir(self):
        for opt in "-D", "--coverdir":
            with self.subTest(opt=opt):
                ns = libregrtest._parse_args([opt, "foo"])
                self.assertEqual(ns.coverdir, os.path.join(support.SAVEDCWD, "foo"))
                self.checkError([opt], "expected one argument")
    ...
    def check_executed_tests(
        ...
        if rerun:
            ...
            for name, match in rerun.items():
                regex = (
                    LOG_PREFIX
                    + f"Re-running {name} in verbose mode \\(matching: {match}\\)"
                )
                self.check_line(output, regex)
...
class ArgsTestCase(BaseTestCase):
    ...
    def test_rerun_fail(self):
        ...
        self.check_executed_tests(
            output, [testname], failed=testname, rerun={testname: "test_fail_always"}
        )
    ...
    def test_rerun_success(self):
        ...
        self.check_executed_tests(
            output, [testname], rerun={testname: "test_fail_once"}
        )
