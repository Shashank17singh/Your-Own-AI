...
class PosixTester(unittest.TestCase):
    ...
    def test_utime_nofollow_symlinks(self):
        ...
        self.assertRaises(
            TypeError, posix.utime, support.TESTFN, (None, None), follow_symlinks=False
        )
        self.assertRaises(
            TypeError, posix.utime, support.TESTFN, (now, None), follow_symlinks=False
        )
        self.assertRaises(
            TypeError, posix.utime, support.TESTFN, (None, now), follow_symlinks=False
        )
    ...
    def test_osexlock(self):
        fd = os.open(support.TESTFN, os.O_WRONLY | os.O_EXLOCK | os.O_CREAT)
        self.assertRaises(
            OSError, os.open, support.TESTFN, os.O_WRONLY | os.O_EXLOCK | os.O_NONBLOCK
        )
        os.close(fd)

        if hasattr(posix, "O_SHLOCK"):
            fd = os.open(support.TESTFN, os.O_WRONLY | os.O_SHLOCK | os.O_CREAT)
            self.assertRaises(
                OSError,
                os.open,
                support.TESTFN,
    ...
    def test_osshlock(self):
        ...
        fd2 = os.open(support.TESTFN, os.O_WRONLY | os.O_SHLOCK | os.O_CREAT)
        os.close(fd2)
        os.close(fd1)
    ...
    def test_stat(self):
        ...
        self.assertWarnsRegex(
            ...
            "should be string, bytes, os.PathLike or integer, not",
            posix.stat,
            bytearray(os.fsencode(support.TESTFN)),
        )
        self.assertRaisesRegex(
            TypeError,
            "should be string, bytes, os.PathLike or integer, not",
        ...
        self.assertRaisesRegex(
            ...
            "should be string, bytes, os.PathLike or integer, not",
            posix.stat,
            list(os.fsencode(support.TESTFN)),
        )
    ...
    def test_mknod(self):
        ...
        support.unlink(support.TESTFN)
        try:
            posix.mknod(path=support.TESTFN, mode=mode, device=0, dir_fd=None)
        except OSError as e:
            self.assertIn(e.errno, (errno.EPERM, errno.EINVAL, errno.EACCES))
    ...
    def test_lchown(self):
        ...
        self._test_all_chown_common(
            posix.lchown, support.TESTFN, getattr(posix, "lstat", None)
        )
    ...
    def test_lchflags_regular_file(self):
        ...
        self._test_chflags_regular_file(
            posix.chflags, support.TESTFN, follow_symlinks=False
        )
    ...
    @unittest.skipUnless(
        os.chown in os.supports_dir_fd, "test needs dir_fd support in os.chown()"
    )
    ...
    def test_stat_dir_fd(self):
        ...
        try:
            ...
            self.assertRaises(OverflowError, posix.stat, support.TESTFN, dir_fd=10**20)
        finally:
            posix.close(f)
    ...
    def test_utime_dir_fd(self):
        ...
        try:
            ...
            self.assertRaises(
                TypeError, posix.utime, support.TESTFN, (None, None), dir_fd=f
            )
            self.assertRaises(
                TypeError, posix.utime, support.TESTFN, (now, None), dir_fd=f
            )
            self.assertRaises(
                TypeError, posix.utime, support.TESTFN, (None, now), dir_fd=f
            )
            self.assertRaises(
                TypeError, posix.utime, support.TESTFN, (now, "x"), dir_fd=f
            )
            posix.utime(support.TESTFN, (int(now), int(now)), dir_fd=f)
            posix.utime(support.TESTFN, (now, now), dir_fd=f)
            posix.utime(
                support.TESTFN, (int(now), int((now - int(now)) * 1e9)), dir_fd=f
            )
            posix.utime(
                support.TESTFN, dir_fd=f, times=(int(now), int((now - int(now)) * 1e9))
            )
    ...
    @unittest.skipUnless(
        (os.mknod in os.supports_dir_fd) and hasattr(stat, "S_IFIFO"),
    ...
    @unittest.skipUnless(
        os.readlink in os.supports_dir_fd, "test needs dir_fd support in os.readlink()"
    )
    def test_readlink_dir_fd(self):
        ...
        try:
            self.assertEqual(
                posix.readlink(support.TESTFN + "link"),
    ...
    def test_rename_dir_fd(self):
        ...
        try:
            posix.rename(
                support.TESTFN + "ren", support.TESTFN, src_dir_fd=f, dst_dir_fd=f
            )
        except:
