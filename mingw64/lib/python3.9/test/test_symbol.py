...
GEN_SYMBOL_FILE = os.path.join(
    os.path.dirname(__file__), "..", "..", "Tools", "scripts", "generate_symbol_py.py"
)
GRAMMAR_FILE = os.path.join(
    os.path.dirname(__file__), "..", "..", "Include", "graminit.h"
)
...
class TestSymbolGeneration(unittest.TestCase):
    ...
    def _generate_symbols(self, grammar_file, target_symbol_py_file):
        proc = subprocess.Popen(
            [sys.executable, GEN_SYMBOL_FILE, grammar_file, target_symbol_py_file],
    ...
    @unittest.skipUnless(
        sysconfig.is_python_build(), "test only works from source build directory"
    )
