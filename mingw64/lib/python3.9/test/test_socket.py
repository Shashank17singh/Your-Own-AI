...
class GeneralModuleTests(unittest.TestCase):
    ...
    def check_sendall_interrupted(self, with_timeout):
        ...
        try:
            ...
            if with_timeout:
                ...
                signal.alarm(1)
                self.assertRaises(
                    socket.timeout, c.sendall, b"x" * support.SOCK_MAX_SIZE
                )
        finally:
            signal.alarm(0)
    ...
    def test_socket_fileno_requires_valid_fd(self):
        ...
        with self.assertRaises(OSError) as cm:
            socket.socket(
                socket.AF_INET, socket.SOCK_STREAM, fileno=support.make_bad_fd()
            )
        self.assertIn(cm.exception.errno, (errno.EBADF, WSAENOTSOCK))
...
class TCPTimeoutTest(SocketTCPTest):
    ...
    def testTCPTimeout(self):
        ...
        self.assertRaises(
            socket.timeout, raise_timeout, "Error generating a timeout exception (TCP)"
        )
...
class UDPTimeoutTest(SocketUDPTest):
    ...
    def testUDPTimeout(self):
        ...
        self.assertRaises(
            socket.timeout, raise_timeout, "Error generating a timeout exception (UDP)"
        )
