...
template <typename Traits> struct property {
  explicit property(const char *key) : m_key(label_text::borrow(key)) {}
...
template <typename EnumType> struct enum_traits {
...
public:
  ...
  /* Append V to this array, requiring V
     ...
     leading to a compile-time error if VALUE is not of the
     appropriate subclass.  */
  template <typename JsonType> void append(std::unique_ptr<JsonType> v) {
    append(v.release());
  }
...
template <typename EnumType>
inline bool object::maybe_get_enum(const enum_property<EnumType> &property,
                                   EnumType &out) const {
  if (value *jv = get(property.m_key.get()))
    if (string *jstr = jv->dyn_cast_string()) {
      if (enum_traits<EnumType>::maybe_get_value_from_string(jstr->get_string(),
                                                             out))
        return true;
    }
  return false;
...
template <typename EnumType>
inline void object::set_enum(const enum_property<EnumType> &property,
                             EnumType value) {
  const char *str = json::enum_traits<EnumType>::get_string_for_value(value);
...
template <>
template <>
inline bool is_a_helper<json::value *>::test(json::value *) {
  return true;
}

template <>
template <>
inline bool is_a_helper<const json::value *>::test(const json::value *) {
  return true;
}

template <>
template <>
inline bool is_a_helper<json::object *>::test(json::value *jv) {
  return jv->get_kind() == json::JSON_OBJECT;
}

template <>
template <>
inline bool is_a_helper<const json::object *>::test(const json::value *jv) {
  return jv->get_kind() == json::JSON_OBJECT;
}

template <>
template <>
inline bool is_a_helper<json::array *>::test(json::value *jv) {
  return jv->get_kind() == json::JSON_ARRAY;
}

template <>
template <>
inline bool is_a_helper<const json::array *>::test(const json::value *jv) {
  return jv->get_kind() == json::JSON_ARRAY;
}

template <>
template <>
inline bool is_a_helper<json::float_number *>::test(json::value *jv) {
...
template <>
template <>
inline bool is_a_helper<json::integer_number *>::test(json::value *jv) {
...
template <>
template <>
inline bool is_a_helper<json::string *>::test(json::value *jv) {
  return jv->get_kind() == json::JSON_STRING;
}

template <>
template <>
inline bool is_a_helper<const json::string *>::test(const json::value *jv) {
  return jv->get_kind() == json::JSON_STRING;
}

template <>
template <>
inline bool is_a_helper<json::literal *>::test(json::value *jv) {
...
template <>
template <>
inline bool is_a_helper<const json::literal *>::test(const json::value *jv) {
  return (jv->get_kind() == json::JSON_TRUE ||
          jv->get_kind() == json::JSON_FALSE ||
          jv->get_kind() == json::JSON_NULL);
}
...
extern void assert_print_eq(const location &loc, const json::value &jv,
                            bool formatted, const char *expected_json);
