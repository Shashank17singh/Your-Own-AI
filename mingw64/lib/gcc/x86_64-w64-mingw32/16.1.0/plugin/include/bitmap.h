...
public:
  ...
  inline void dump(mem_location *loc, const mem_usage &total) const {
...
inline bool bitmap_empty_p(const_bitmap map) { return !map->first; }
...
inline void bitmap_initialize(bitmap head,
...
inline void bitmap_release(bitmap head) {
  bitmap_clear(head);
...
inline void bmp_iter_set_init(bitmap_iterator *bi, const_bitmap map,
...
inline void bmp_iter_and_init(bitmap_iterator *bi, const_bitmap map1,
...
inline void bmp_iter_and_compl_init(bitmap_iterator *bi, const_bitmap map1,
                                    const_bitmap map2, unsigned start_bit,
                                    unsigned *bit_no) {
  bi->elt1 = map1->first;
  bi->elt2 = map2->first;
...
inline void bmp_iter_next(bitmap_iterator *bi, unsigned *bit_no) {
  bi->bits >>= 1;
  *bit_no += 1;
}
...
inline void bmp_iter_next_bit(bitmap_iterator *bi, unsigned *bit_no) {
#if (GCC_VERSION >= 3004)
  {
...
inline bool bmp_iter_set(bitmap_iterator *bi, unsigned *bit_no) {
  ...
  *bit_no =
      ((*bit_no + BITMAP_WORD_BITS - 1) / BITMAP_WORD_BITS * BITMAP_WORD_BITS);
  bi->word_no++;
...
inline bool bmp_iter_and(bitmap_iterator *bi, unsigned *bit_no) {
  /* If our current word is nonzero, it contains the bit we want.  */
  if (bi->bits) {
  next_bit:
    bmp_iter_next_bit(bi, bit_no);
...
inline bool bmp_iter_and_compl(bitmap_iterator *bi, unsigned *bit_no) {
  /* If our current word is nonzero, it contains the bit we want.  */
  if (bi->bits) {
  next_bit:
    bmp_iter_next_bit(bi, bit_no);
...
private:
  ...
  static const size_t num_bitmap_elements = CEIL(
      CHAR_BIT * sizeof(typename Traits::element_type) * Traits::constant_size,
...
base_bitmap_view<T, Traits>::base_bitmap_view(const T &array,
  ...
  unsigned int indx = 0;
  for (size_t array_base = 0; array_base < array_size;
       array_base += array_step, indx += 1) {
    /* How many array elements are in this particular bitmap_element.  */
    unsigned int array_count = (STATIC_CONSTANT_P(array_size % array_step == 0)
                                    ? array_step
                                    : MIN(array_step, array_size - array_base));
    ...
    if (array_element_bits < BITMAP_WORD_BITS) {
      ...
      for (unsigned int word_i = 0; word_i < BITMAP_ELEMENT_WORDS; ++word_i) {
        BITMAP_WORD word = 0;
