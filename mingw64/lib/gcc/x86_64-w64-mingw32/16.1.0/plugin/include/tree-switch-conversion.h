...
public:
  ...
  virtual bool is_single_value_p() { return false; }
...
public:
  ...
  cluster_type get_type() final override { return SIMPLE_CASE; }

  tree get_low() final override { return m_low; }

  tree get_high() final override { return m_high; }

  void set_high(tree high) { m_high = high; }

  void debug() final override { dump(stderr); }

  void dump(FILE *f, bool details ATTRIBUTE_UNUSED = false) final override {
  ...
  unsigned get_comparison_count() { return m_range_p ? 2 : 1; }
...
simple_cluster::simple_cluster(tree low, tree high, tree case_label_expr,
    ...
    : cluster(case_label_expr, case_bb, prob, prob), m_low(low), m_high(high),
      m_has_forward_bb(has_forward_bb) {
...
public:
  ...
  tree get_low() final override { return m_cases[0]->get_low(); }
...
public:
  ...
  cluster_type get_type() final override { return JUMP_TABLE; }

  void emit(tree index_expr, tree index_type, tree default_label_expr,
            basic_block default_bb, location_t loc) final override;
...
public:
  ...
  cluster_type get_type() final override { return BIT_TEST; }
  ...
  static inline bool is_enabled(void) { return flag_bit_tests; }
...
public:
  ...
  bool has_child() { return m_left != NULL || m_right != NULL; }
...
inline case_tree_node::case_tree_node()
...
inline void release_clusters(vec<cluster *> &clusters) {
