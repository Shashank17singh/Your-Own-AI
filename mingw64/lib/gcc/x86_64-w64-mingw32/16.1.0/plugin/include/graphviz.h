...
public:
  ...
  void write_character(char ch) { pp_character(&m_pp, ch); }
  void write_string(const char *str) { pp_string(&m_pp, str); }
  void write_newline() { pp_newline(&m_pp); }
...
struct stmt_list : public ast_node {
  void print(writer &w) const final override;
  void add_stmt(std::unique_ptr<stmt> s) { m_stmts.push_back(std::move(s)); }
...
struct graph : public ast_node {
  ...
  graph(id id_) : m_id(std::make_unique<id>(std::move(id_))) {}

  void print(writer &w) const final override;

  void add_stmt(std::unique_ptr<stmt> s) { m_stmt_list.add_stmt(std::move(s)); }
...
struct attr_stmt : public stmt_with_attr_list {
  ...
  attr_stmt(enum kind kind_) : m_kind(kind_) {}
...
struct kv_stmt : public stmt {
  kv_stmt(kv_pair kv) : m_kv(std::move(kv)) {}
...
struct port : public ast_node {
  port(id id_)
      : m_id(std::make_unique<id>(std::move(id_))), m_compass_pt(nullptr) {}

  port(enum compass_pt compass_pt_)
      : m_id(nullptr), m_compass_pt(std::make_unique<compass_pt>(compass_pt_)) {
  }
  ...
  port(const port &other) : m_id(nullptr), m_compass_pt(nullptr) {
    if (other.m_id)
...
struct node_id : public ast_node {
  node_id(id id_) : m_id(id_), m_port(nullptr) {}
  node_id(id id_, port port_)
      : m_id(id_), m_port(std::make_unique<port>(std::move(port_))) {}
  node_id(const node_id &other) : m_id(other.m_id), m_port(nullptr) {
    if (other.m_port)
...
struct subgraph : public stmt {
  ...
  void add_attr(id key, id value) {
    m_stmt_list.add_stmt(
        std::make_unique<kv_stmt>(kv_pair(std::move(key), std::move(value))));
  }
...
extern std::unique_ptr<xml::node> make_svg_from_graph(const graph &g);
...
public:
  ...
  void print(const char *fmt, ...) ATTRIBUTE_GCC_PPDIAG(2, 3);
  void println(const char *fmt, ...) ATTRIBUTE_GCC_PPDIAG(2, 3);
