...
public:
  /* Default constructor.  */
  fibonacci_node()
      : m_parent(NULL), m_child(NULL), m_left(this), m_right(this),
        m_data(NULL), m_degree(0), m_mark(0) {}
  ...
  int compare_data(K key) { return fibonacci_node_t(key).compare(this); }
  ...
  K get_key() { return m_key; }

  /* Return data associated with the node.  */
  V *get_data() { return m_data; }
...
private:
  ...
  void insert_before(fibonacci_node_t *b) { m_left->insert_after(b); }
...
public:
  ...
  fibonacci_heap(K global_min_key, pool_allocator *allocator = NULL)
    ...
    if (!m_allocator) {
      m_allocator =
          new pool_allocator("Fibonacci heap", sizeof(fibonacci_node_t));
...
template <class K, class V>
void fibonacci_node<K, V>::link(fibonacci_node<K, V> *parent) {
...
template <class K, class V>
void fibonacci_node<K, V>::insert_after(fibonacci_node<K, V> *b) {
...
template <class K, class V>
fibonacci_node<K, V> *fibonacci_heap<K, V>::insert(K key, V *data) {
  /* Create the new node.  */
  fibonacci_node<K, V> *node =
      new (m_allocator->allocate()) fibonacci_node_t(key, data);
...
template <class K, class V>
V *fibonacci_heap<K, V>::replace_key_data(fibonacci_node<K, V> *node, K key,
                                          V *data) {
  K okey;
...
template <class K, class V>
void fibonacci_heap<K, V>::cut(fibonacci_node<K, V> *node,
...
template <class K, class V>
void fibonacci_heap<K, V>::cascading_cut(fibonacci_node<K, V> *y) {
  fibonacci_node<K, V> *z;
...
template <class K, class V>
void fibonacci_heap<K, V>::remove_root(fibonacci_node<K, V> *node) {
  if (node->m_left == node)
    m_root = NULL;
  else
    m_root = node->remove();
