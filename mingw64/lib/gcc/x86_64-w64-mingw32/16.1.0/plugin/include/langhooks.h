...
struct lang_hooks_for_decls {
  ...
  /* True if OpenMP should regard this DECL as being a scalar which has
     Fortran's allocatable or pointer attribute.  */
...
struct lang_hooks {
  ...
  /* This compares two types for equivalence ("compatible" in C-based
     languages). This routine should only return 1 if it is sure.  It should not
     be used in contexts where erroneously returning 0 causes problems.  */
  int (*types_compatible_p)(tree x, tree y);

  /* Called by diagnostic_report_current_function to print out function name
     for textual diagnostic output.  */
  void (*print_error_function)(diagnostics::text_sink &, const char *,
...
extern tree add_builtin_function_ext_scope(const char *name, tree type,
                                           ...
                                           tree attrs);
extern tree simulate_builtin_function_decl(location_t, const char *, tree, int,
                                           const char *, tree);
