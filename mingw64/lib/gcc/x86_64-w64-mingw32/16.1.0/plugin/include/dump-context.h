...
public:
  ...
  void dump_printf_loc_va(const dump_metadata_t &metadata,
                          const dump_user_location_t &loc, const char *format,
                          va_list *ap) ATTRIBUTE_GCC_DUMP_PRINTF(4, 0);
  ...
  bool optimization_records_enabled_p() const { return m_json_writer != NULL; }
...
private:
  struct custom_token_printer : public token_printer {
    custom_token_printer(dump_pretty_printer &dump_pp)
        : m_dump_pp(dump_pp), m_optinfo(nullptr) {}
...
void verify_item(const location &loc, const optinfo_item *item,
                 ...
                 location_t expected_location, const char *expected_text);
