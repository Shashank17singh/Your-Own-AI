...
template <typename GraphTraits, typename Path_t> class shortest_paths {
public:
...
inline shortest_paths<GraphTraits, Path_t>::shortest_paths(
    const graph_t &graph, const node_t *given_node,
    enum shortest_path_sense sense)
    : m_graph(graph), m_sense(sense), m_dist(graph.m_nodes.length()),
  ...
  while (queue.length() > 0) {
    ...
    node_t *n = static_cast<node_t *>(m_graph.m_nodes[idx_with_min_dist]);
...
template <typename GraphTraits, typename Path_t>
inline Path_t shortest_paths<GraphTraits, Path_t>::get_shortest_path(
    const node_t *other_node) const {
  Path_t result;
...
template <typename GraphTraits, typename Path_t>
inline int shortest_paths<GraphTraits, Path_t>::get_shortest_distance(
    const node_t *other_node) const {
