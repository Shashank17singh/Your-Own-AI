...
protected:
  ...
  virtual class relation_chain *next_relation(basic_block, relation_chain *,
                                              tree) const {
    return NULL;
  }
...
public:
  ...
  bool record(basic_block bb, relation_kind k, tree op1,
              tree op2) final override;

  relation_kind query(basic_block bb, tree ssa1, tree ssa2) final override;
  relation_kind query(basic_block bb, const_bitmap b1,
...
protected:
  ...
  relation_kind find_relation_dom(basic_block bb, unsigned v1,
                                  unsigned v2) const;
...
#define FOR_EACH_EQUIVALENCE(oracle, bb, name, equiv_name)                     \
  for (equiv_relation_iterator iter(oracle, bb, name, true, false);            \
       ((equiv_name) = iter.get_name()); iter.next())
...
#define FOR_EACH_PARTIAL_EQUIV(oracle, bb, name, equiv_name, equiv_rel)        \
  for (equiv_relation_iterator iter(oracle, bb, name, false, true);            \
       ((equiv_name) = iter.get_name(&equiv_rel)); iter.next())
...
inline relation_trio relation_trio::lhs_op1(relation_kind k) {
  return relation_trio(k, VREL_VARYING, VREL_VARYING);
}
inline relation_trio relation_trio::lhs_op2(relation_kind k) {
  return relation_trio(VREL_VARYING, k, VREL_VARYING);
}
inline relation_trio relation_trio::op1_op2(relation_kind k) {
  return relation_trio(VREL_VARYING, VREL_VARYING, k);
}

inline relation_kind relation_trio::lhs_op1() {
  return (relation_kind)(m_val & TRIO_MASK);
}

inline relation_kind relation_trio::lhs_op2() {
  return (relation_kind)((m_val >> TRIO_SHIFT) & TRIO_MASK);
}

inline relation_kind relation_trio::op1_op2() {
  return (relation_kind)((m_val >> (TRIO_SHIFT * 2)) & TRIO_MASK);
}

inline relation_trio relation_trio::swap_op1_op2() {
...
inline void value_relation::set_relation(relation_kind r, tree n1, tree n2) {
  gcc_checking_assert(TREE_CODE(n1) == SSA_NAME && TREE_CODE(n2) == SSA_NAME);
  related = r;
  name1 = n1;
  name2 = n2;
}
...
inline value_relation::value_relation() {
...
inline value_relation::value_relation(relation_kind kind, tree n1, tree n2) {
...
#define FOR_EACH_RELATION_BB(oracle, bb, vr)                                   \
  for (block_relation_iterator iter(oracle, bb, vr); !iter.m_done;             \
       iter.get_next_relation(vr))

#define FOR_EACH_RELATION_NAME(oracle, bb, name, vr)                           \
  for (block_relation_iterator iter(oracle, bb, vr, name); !iter.m_done;       \
       iter.get_next_relation(vr))
...
inline int pe_to_bits(relation_kind t) {
  switch (t) {
  case VREL_PE8:
    return 8;
  case VREL_PE16:
...
inline relation_kind bits_to_pe(int bits) {
  switch (bits) {
  case 8:
    return VREL_PE8;
  case 16:
...
inline relation_kind pe_min(relation_kind t1, relation_kind t2) {
