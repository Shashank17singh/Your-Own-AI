...
public:
  ...
  explicit hash_set(size_t n = 13, bool ggc = false CXX_MEM_STAT_INFO)
      : m_table(n, ggc, true, GATHER_STATISTICS,
...
#define DEFINE_DEBUG_HASH_SET(T)                                               \
  ...
  DEBUG_FUNCTION void debug(hash_set<T> &ref) { debug_helper<T>(ref); }        \
...
inline void gt_ggc_mx(hash_set<K, false, H> *h) {
  gt_ggc_mx(&h->m_table);
}

template <typename K, typename H>
inline void gt_pch_nx(hash_set<K, false, H> *h) {
  gt_pch_nx(&h->m_table);
}

template <typename K, typename H>
inline void gt_pch_nx(hash_set<K, false, H> *h, gt_pointer_operator op,
                      void *cookie) {
