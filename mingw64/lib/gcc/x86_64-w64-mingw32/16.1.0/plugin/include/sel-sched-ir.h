...
inline _list_t _list_alloc(void) { return sched_lists_pool.allocate(); }

inline void _list_add(_list_t *lp) {
...
inline void _list_remove_nofree(_list_t *lp) {
  _list_t n = *lp;

  *lp = _LIST_NEXT(n);
}

inline void _list_remove(_list_t *lp) {
  _list_t n = *lp;
...
inline void _list_clear(_list_t *l) {
  while (*l)
    _list_remove(l);
}
...
inline void _list_iter_start(_list_iterator *ip, _list_t *lp,
                             bool can_remove_p) {
  ip->lp = lp;
...
inline void _list_iter_next(_list_iterator *ip) {
  if (!ip->removed_p)
...
inline void _list_iter_remove(_list_iterator *ip) {
  gcc_assert(!ip->removed_p && ip->can_remove_p);
  _list_remove(ip->lp);
  ip->removed_p = true;
}

inline void _list_iter_remove_nofree(_list_iterator *ip) {
  gcc_assert(!ip->removed_p && ip->can_remove_p);
...
#define _FOR_EACH(TYPE, ELEM, I, L)                                            \
  for (_list_iter_start(&(I), &(L), false);                                    \
       _list_iter_cond_##TYPE(*(I).lp, &(ELEM)); _list_iter_next(&(I)))
...
inline void _xlist_add(_xlist_t *lp, rtx x) {
  _list_add(lp);
  _XLIST_X(*lp) = x;
}
...
inline bool _xlist_is_in_p(_xlist_t l, rtx x) {
  while (l) {
    if (_XLIST_X(l) == x)
      return true;
    l = _XLIST_NEXT(l);
...
inline bool _list_iter_cond_x(_xlist_t l, rtx *xp) {
  if (l) {
    *xp = _XLIST_X(l);
    return true;
  }
...
inline void ilist_add(ilist_t *lp, insn_t insn) {
  _list_add(lp);
  ILIST_INSN(*lp) = insn;
}
...
inline bool ilist_is_in_p(ilist_t l, insn_t insn) {
  while (l) {
...
/* Used through _FOR_EACH.  */
inline bool _list_iter_cond_insn(ilist_t l, insn_t *ip) {
  if (l) {
    *ip = ILIST_INSN(l);
    return true;
  }
...
inline bool _list_iter_cond_def(def_list_t def_list, def_t *def) {
  if (def_list) {
...
inline bool inner_loop_header_p(basic_block bb) {
...
inline vec<edge> get_loop_exit_edges_unique_dests(const class loop *loop) {
...
inline bool sel_bb_empty_or_nop_p(basic_block bb) {
...
/* Collect all loop exits recursively, skipping empty BBs between them.
   ...
   as well.  */
inline vec<edge> get_all_loop_exits(basic_block bb) {
  vec<edge> exits = vNULL;

  /* If bb is empty, and we're skipping to loop exits, then
     consider bb as a possible gate to the inner loop now.  */
  while (sel_bb_empty_or_nop_p(bb) && in_current_region_p(bb) &&
...
/* We need to return a succ_iterator to avoid 'unitialized' warning
   during bootstrap.  */
inline succ_iterator _succ_iter_start(insn_t *succp, insn_t insn, int flags) {
  succ_iterator i;
...
inline bool _succ_iter_cond(succ_iterator *ip, insn_t *succp, insn_t insn,
...
inline void _succ_iter_next(succ_iterator *ip) {
...
inline bool _eligible_successor_edge_p(edge e1, succ_iterator *ip) {
  edge e2 = e1;
  basic_block bb;
  int flags = ip->flags;
...
inline basic_block bb_next_bb(basic_block bb) {
...
extern void insert_in_history_vect(vec<expr_history_def> *, unsigned,
                                   enum local_trans_type, vinsn_t, vinsn_t,
                                   ds_t);
