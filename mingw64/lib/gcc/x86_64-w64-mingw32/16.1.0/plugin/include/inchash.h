...
public:
  ...
  hashval_t end() const { return val; }

  /* Add unsigned value V.  */
  void add_int(unsigned v) { val = iterative_hash_hashval_t(v, val); }
  ...
  void add_hwi(HOST_WIDE_INT v) { val = iterative_hash_host_wide_int(v, val); }
  ...
  /* Hash in pointer PTR.  */
  void add_ptr(const void *ptr) { add(&ptr, sizeof(ptr)); }
  ...
  void merge(const hash &other) { merge_hash(other.val); }

  template <class T> void add_object(T &obj) { add(&obj, sizeof(T)); }

  /* Support for accumulating boolean flags */

  void add_flag(bool flag) { bits = (bits << 1) | flag; }
...
inline hashval_t iterative_hash_host_wide_int(HOST_WIDE_INT val,
                                              hashval_t val2) {
