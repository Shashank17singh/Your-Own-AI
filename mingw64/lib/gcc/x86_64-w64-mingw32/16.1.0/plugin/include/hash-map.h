...
class GTY((user)) hash_map
{
  ...
  struct hash_entry {
    ...
    static hashval_t hash(const hash_entry &e) { return Traits::hash(e.m_key); }
    ...
    static int keep_cache_entry(hash_entry &e) { return ggc_marked_p(e.m_key); }
...
public:
  ...
  static hash_map *
  create_ggc(size_t size = default_hash_map_size,
             bool gather_mem_stats = GATHER_STATISTICS CXX_MEM_STAT_INFO) {
  ...
  bool put(const Key &k, const Value &v) {
    ...
    if (ins) {
      ...
      gcc_checking_assert(!Traits::is_empty(*e) && !Traits::is_deleted(*e));
    } else
      e->m_value = v;
...
inline void gt_ggc_mx(hash_map<K, V, H> *h) {
  gt_ggc_mx(&h->m_table);
}

template <typename K, typename V, typename H>
inline void gt_pch_nx(hash_map<K, V, H> *h) {
  gt_pch_nx(&h->m_table);
}

template <typename K, typename V, typename H>
inline void gt_cleare_cache(hash_map<K, V, H> *h) {
  if (h)
    gt_cleare_cache(&h->m_table);
}

template <typename K, typename V, typename H>
inline void gt_pch_nx(hash_map<K, V, H> *h, gt_pointer_operator op,
                      void *cookie) {
...
template <typename K, typename V, typename H>
inline V *hash_map_safe_get(hash_map<K, V, H> *h, const K &k) {
  return h ? h->get(k) : NULL;
}

/* Like h->get, but handles null h.  */
template <bool ggc, typename K, typename V, typename H>
inline V &hash_map_safe_get_or_insert(hash_map<K, V, H> *&h, const K &k,
                                      bool *e = NULL,
                                      size_t size = default_hash_map_size) {
...
template <bool ggc, typename K, typename V, typename H>
inline bool hash_map_safe_put(hash_map<K, V, H> *&h, const K &k, const V &v,
                              size_t size = default_hash_map_size) {
