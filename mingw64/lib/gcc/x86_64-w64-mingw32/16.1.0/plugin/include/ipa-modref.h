...
static const int implicit_const_eaf_flags =
    EAF_NO_DIRECT_CLOBBER | EAF_NO_INDIRECT_CLOBBER | EAF_NO_DIRECT_ESCAPE |
    EAF_NO_INDIRECT_ESCAPE | EAF_NO_DIRECT_READ | EAF_NO_INDIRECT_READ |
...
static const int implicit_retslot_eaf_flags =
    ...
    EAF_NO_INDIRECT_CLOBBER | EAF_NOT_RETURNED_INDIRECTLY;
...
/* If function does not bind to current def (i.e. it is inline in comdat
   ...
   etc.)  */
inline int interposable_eaf_flags(int modref_flags, int flags) {
