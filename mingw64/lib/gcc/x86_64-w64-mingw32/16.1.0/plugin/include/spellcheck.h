...
/* spellcheck.cc  */
extern edit_distance_t get_edit_distance(const char *s, int len_s,
...
template <typename StringlikeType> struct edit_distance_traits {};
...
template <typename GoalType, typename CandidateType> class best_match {
public:
  ...
  best_match(goal_t goal,
      ...
      : m_goal(goal_traits::get_string(goal)),
        m_goal_len(goal_traits::get_length(goal)), m_best_candidate(NULL),
        m_best_distance(best_distance_so_far), m_best_candidate_len(0) {}
  ...
  candidate_t blithely_get_best_candidate() const { return m_best_candidate; }
