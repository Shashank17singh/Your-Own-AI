...
extern bool parse_profile_quality(const char *value, profile_quality *quality);
...
inline bool safe_scale_64bit(uint64_t a, uint64_t b, uint64_t c,
                             uint64_t *res) {
#if (GCC_VERSION >= 5000)
  uint64_t tmp;
...
/* Data type to hold probabilities.  It implements fixed point arithmetics
   ...
   through all operations.  Special value UNINITIALIZED_PROFILE is used for
   probabilities that has not been determined yet (for example because of
...
public:
  ...
  profile_probability(uint32_t val, profile_quality quality) : m_val(val) {
    set_quality(quality);
  }
  ...
  static profile_probability even() { return guessed_always() / 2; }

  static profile_probability very_likely() {
    return always() - very_unlikely();
  }

  static profile_probability likely() { return always() - unlikely(); }
  /* Return true when value is not zero and can be used for scaling.   */
  bool nonzero_p() const { return initialized_p() && m_val != 0; }
  ...
  bool initialized_p() const { return m_val != uninitialized_probability; }

  /* Return true if value can be trusted.  */
  bool reliable_p() const { return quality() >= ADJUSTED; }
  ...
  profile_probability operator/(const profile_probability &other) const {
    ...
    if (m_val >= other.m_val) {
      ret.m_val = max_probability;
      ret.set_quality(MIN(MIN(quality(), other.quality()), GUESSED));
      return ret;
    } else if (!m_val)
      ret.m_val = 0;
    else {
  ...
  profile_probability &operator/=(const profile_probability &other) {
    ...
    if (!initialized_p() || !other.initialized_p())
      ...
      if (m_val > other.m_val) {
        ...
        set_quality(MIN(MIN(quality(), other.quality()), GUESSED));
        return *this;
      } else if (!m_val)
        ;
      else {
  ...
  /* Return 1-*THIS.  */
  profile_probability invert() const { return always() - *this; }
  ...
  profile_probability apply_scale(profile_probability num,
    ...
    profile_probability ret;
    ret.m_val =
        MIN(RDIV((uint64_t)m_val * num.m_val, den.m_val), max_probability);
    ret.set_quality(
        MIN(MIN(MIN(quality(), ADJUSTED), num.quality()), den.quality()));
    return ret;
  }
  ...
  /* Return true when the probability of edge is reliable.
     ...
     or below 1%.  In future we might want to propagate reliability information
     across the CFG if we find this information useful on multiple places.   */
...
public:
  ...
  bool compatible_p(const profile_count other) const {
    ...
    /* Do not allow nonzero global profile together with local guesses
       that are globally0.  */
    if (ipa().nonzero_p() && !(other.ipa() == other))
      return false;
    if (other.ipa().nonzero_p() && !(ipa() == *this))
      return false;
  ...
  static profile_count zero() { return from_gcov_type(0); }
  ...
  static profile_count one() { return from_gcov_type(1); }
  ...
  /* Return true if value has been initialized.  */
  bool initialized_p() const { return m_val != uninitialized_count; }

  /* Return true if value can be trusted.  */
  bool reliable_p() const { return m_quality >= ADJUSTED; }
  ...
  bool precise_p() const { return m_quality == PRECISE; }
  ...
  bool ok_for_merging(profile_count other) const {
    if (m_quality < ADJUSTED || other.m_quality < ADJUSTED)
      return true;
    return !(other < *this);
  }
  ...
  profile_count operator*(int64_t num) const { return apply_scale(num, 1); }
  ...
  profile_count operator/(int64_t den) const { return apply_scale(1, den); }
  ...
  static profile_count max_prefer_initialized(const profile_count a,
                                              const profile_count b) {
    if (!a.initialized_p())
      return b;
    if (!b.initialized_p())
      return a;
  ...
  profile_count apply_scale(profile_count num, profile_count den) const {
    ...
    ret.m_quality =
        MIN(MIN(MIN(m_quality, ADJUSTED), num.m_quality), den.m_quality);
    ...
    if (num.ipa_p()) {
      ...
      if (num.m_quality == AFDO &&
          (ret.m_quality != GUESSED && ret.m_quality != GUESSED_LOCAL))
        ret.m_quality = AFDO;
      else
        ret.m_quality =
            MAX(ret.m_quality, num == num.ipa() ? GUESSED : num.m_quality);
    }
    return ret;
  }
  ...
  profile_probability probability_in(const profile_count overall) const {
    ...
    ret.set_quality(
        MIN(MAX(MIN(m_quality, overall.m_quality), GUESSED), ADJUSTED));
    return ret;
  }
