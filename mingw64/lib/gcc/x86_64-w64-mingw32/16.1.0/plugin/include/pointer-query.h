...
public:
  ssa_name_limit_t() : visited(), ssa_def_max(param_ssa_name_def_chain_limit) {}
...
struct access_ref {
  ...
  bool offset_zero() const { return offrng[0] == 0 && offrng[1] == 0; }
  ...
  bool ref_declared() const { return DECL_P(ref) && base0 && deref < 1; }
  ...
  void add_offset(const offset_int &off) { add_offset(off, off); }
...
struct access_data {
  ...
  access_data(range_query *, gimple *, access_mode, tree = NULL_TREE,
              bool = false, tree = NULL_TREE, bool = false);

  /* Set the access to at most MAXWRITE and MAXREAD bytes, and
     at least 1 when MINWRITE or MINREAD, respectively, is set.  */
  access_data(range_query *, tree, access_mode, tree = NULL_TREE, bool = false,
...
extern tree compute_objsize(tree, gimple *, int, access_ref *,
                            range_query * = nullptr);
extern tree compute_objsize(tree, gimple *, int, access_ref *, pointer_query *);
...
extern tree array_elt_at_offset(tree, HOST_WIDE_INT, HOST_WIDE_INT * = nullptr,
                                HOST_WIDE_INT * = nullptr);
