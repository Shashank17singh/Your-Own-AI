...
public:
  ...
  rtx_obj_reference(unsigned int regno, uint16_t flags, machine_mode mode,
...
inline rtx_obj_reference::rtx_obj_reference(unsigned int regno, uint16_t flags,
    ...
    : regno(regno), flags(flags), mode(mode), multireg_offset(multireg_offset) {
}
...
inline rtx_properties::rtx_properties()
    : ref_begin(nullptr), ref_iter(nullptr), ref_end(nullptr), has_asm(false),
      has_call(false), has_pre_post_modify(false), has_volatile_refs(false),
      spare(0) {}

/* Like add_src, but treat X has being part of a REG_EQUAL or
   REG_EQUIV note.  */

inline void rtx_properties::try_to_add_note(const_rtx x) {
...
inline bool rtx_properties::has_side_effects() const {
...
inline iterator_range<rtx_obj_reference *> rtx_properties::refs() const {
...
template <typename Base> class growing_rtx_properties : public Base {
public:
  template <typename... Args> growing_rtx_properties(Args...);

  template <typename AddFn> void repeat(AddFn add);
...
inline void growing_rtx_properties<Base>::repeat(AddFn add) {
...
template <typename Base>
inline void growing_rtx_properties<Base>::add_dest(const_rtx x,
                                                   unsigned int flags) {
  repeat([&]() { this->try_to_add_dest(x, flags); });
}

template <typename Base>
inline void growing_rtx_properties<Base>::add_src(const_rtx x,
                                                  unsigned int flags) {
  repeat([&]() { this->try_to_add_src(x, flags); });
}

template <typename Base>
inline void growing_rtx_properties<Base>::add_pattern(const_rtx pat) {
  repeat([&]() { this->try_to_add_pattern(pat); });
}

template <typename Base>
inline void growing_rtx_properties<Base>::add_note(const_rtx x) {
  repeat([&]() { this->try_to_add_note(x); });
}

template <typename Base>
inline void growing_rtx_properties<Base>::add_insn(const rtx_insn *insn,
                                                   bool include_notes) {
...
bool vec_series_highpart_p(machine_mode result_mode, machine_mode op_mode,
                           rtx sel);

bool vec_series_lowpart_p(machine_mode result_mode, machine_mode op_mode,
                          rtx sel);
