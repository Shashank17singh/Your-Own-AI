...
extern void asan_maybe_insert_dynamic_shadow_at_function_entry(function *);
...
extern gimple_stmt_iterator create_cond_insert_point(gimple_stmt_iterator *,
                                                     bool, bool, bool,
                                                     basic_block *,
                                                     basic_block *);
...
inline unsigned int asan_red_zone_size(unsigned int size) {
...
inline bool asan_intercepted_p(enum built_in_function fcode) {
  /* This list should be kept up-to-date with upstream's version at
     compiler-rt/lib/hwasan/hwasan_platform_interceptors.h.  */
  if (hwasan_sanitize_p())
    return fcode == BUILT_IN_MEMCMP || fcode == BUILT_IN_MEMCPY ||
           fcode == BUILT_IN_MEMMOVE || fcode == BUILT_IN_MEMSET;

  return fcode == BUILT_IN_INDEX || fcode == BUILT_IN_MEMCHR ||
         fcode == BUILT_IN_MEMCMP || fcode == BUILT_IN_MEMCPY ||
         fcode == BUILT_IN_MEMMOVE || fcode == BUILT_IN_MEMSET ||
         fcode == BUILT_IN_STRCASECMP || fcode == BUILT_IN_STRCAT ||
         fcode == BUILT_IN_STRCHR || fcode == BUILT_IN_STRCMP ||
         fcode == BUILT_IN_STRCPY || fcode == BUILT_IN_STRDUP ||
         fcode == BUILT_IN_STRLEN || fcode == BUILT_IN_STRNCASECMP ||
         fcode == BUILT_IN_STRNCAT || fcode == BUILT_IN_STRNCMP ||
         fcode == BUILT_IN_STRCSPN || fcode == BUILT_IN_STRPBRK ||
         fcode == BUILT_IN_STRSPN || fcode == BUILT_IN_STRSTR ||
         fcode == BUILT_IN_STRNCPY;
}

/* Return TRUE if we should instrument for use-after-scope sanity checking.  */

inline bool asan_sanitize_use_after_scope(void) {
...
inline bool asan_protect_stack_decl(tree decl) {
  return DECL_P(decl) &&
...
inline bool sanitize_flags_p(sanitize_code_type flag,
...
inline bool sanitize_coverage_p(const_tree fn = current_function_decl) {
  return (flag_sanitize_coverage &&
          (fn == NULL_TREE ||
           lookup_attribute("no_sanitize_coverage", DECL_ATTRIBUTES(fn)) ==
               NULL_TREE));
}
