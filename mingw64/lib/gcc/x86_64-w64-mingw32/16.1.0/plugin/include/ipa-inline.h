...
public:
  ...
  edge_growth_cache_entry(int64_t time, int64_t nonspec_time, int size,
                          ipa_hints hints)
...
inline int estimate_edge_size(struct cgraph_edge *edge) {
  edge_growth_cache_entry *entry;
  if (edge_growth_cache == NULL ||
      (entry = edge_growth_cache->get(edge)) == NULL || entry->size == 0)
...
inline int estimate_min_edge_growth(struct cgraph_edge *edge) {
...
inline int estimate_edge_growth(struct cgraph_edge *edge) {
  ipa_call_summary *s = ipa_call_summaries->get(edge);
...
inline sreal estimate_edge_time(struct cgraph_edge *edge,
                                sreal *nonspec_time = NULL) {
  edge_growth_cache_entry *entry;
  if (edge_growth_cache == NULL ||
      (entry = edge_growth_cache->get(edge)) == NULL || entry->time == 0)
...
inline ipa_hints estimate_edge_hints(struct cgraph_edge *edge) {
  edge_growth_cache_entry *entry;
  if (edge_growth_cache == NULL ||
      (entry = edge_growth_cache->get(edge)) == NULL || entry->hints == 0)
