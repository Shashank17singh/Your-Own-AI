...
public:
  ...
  bool same_scc(const ipcp_value<valtype> *o) { return o->scc_no == scc_no; }

  /* Return true, if a this value has been generated for a self-recursive call
     as a result of an arithmetic pass-through jump-function acting on a value
     in the same lattice function.  */
...
template <typename valtype> struct ipcp_lattice {
public:
  ...
  bool add_value(valtype newval, cgraph_edge *cs,
                 ipcp_value<valtype> *src_val = NULL, int src_idx = 0,
                 HOST_WIDE_INT offset = -1, ipcp_value<valtype> **val_p = NULL,
...
public:
  ...
  bool meet_with(ipcp_bits_lattice &other, unsigned, signop, enum tree_code,
                 tree, bool);
...
inline void ipcp_vr_lattice::init(tree type) {
  if (type)
    m_vr.set_type(type);
...
static inline bool ipa_vr_supported_type_p(tree type) {
  return irange::supports_p(type) || prange::supports_p(type);
}

class ipa_vr;

bool ipa_vr_operation_and_type_effects(vrange &dst_vr, const vrange &src_vr,
                                       enum tree_code operation, tree dst_type,
                                       tree src_type);
bool ipa_vr_operation_and_type_effects(vrange &dst_vr, const ipa_vr &src_vr,
                                       enum tree_code operation, tree dst_type,
                                       tree src_type);
