...
#define SUBTARGET_EXTRA_SPECS {"mingw_include_path", DEFAULT_TARGET_MACHINE}
...
#define PROFILE_HOOK(LABEL)                                                    \
  if (MAIN_NAME_P(DECL_NAME(current_function_decl))) {                         \
    emit_call_insn(gen_rtx_CALL(                                               \
        VOIDmode,                                                              \
        gen_rtx_MEM(FUNCTION_MODE, gen_rtx_SYMBOL_REF(Pmode, "_monstartup")),  \
        const0_rtx));                                                          \
  }
...
#define ASM_OUTPUT_DEF_FROM_DECLS(STREAM, DECL, TARGET)                        \
  do {                                                                         \
    const char *alias = IDENTIFIER_POINTER(DECL_ASSEMBLER_NAME(DECL));         \
    mingw_pe_maybe_record_exported_symbol(DECL, alias, 0);                     \
    if (TREE_CODE(DECL) == FUNCTION_DECL)                                      \
      mingw_pe_declare_type(STREAM, alias, TREE_PUBLIC(DECL), 1);              \
...
#define SUBTARGET_ATTRIBUTE_TABLE                                              \
  {"selectany", 0,     0,     true,                                            \
   false,       false, false, mingw_handle_selectany_attribute,                \
   NULL}
...
#define WINDRES_FORMAT_SPEC "%{m32:-F pe-i386;m64|!m32:-F pe-x86-64} "
#else
#define WINDRES_FORMAT_SPEC "%{m64:-F pe-x86-64;m32|!m64:-F pe-i386} "
#endif

#define EXTRA_DEFAULT_COMPILERS                                                \
  {".rc", "@windres-rc", 0, 0, 0},                                             \
      {"@windres-rc",                                                          \
       "%{!E:%{!M:%{!MM:windres -J rc -O coff " WINDRES_FORMAT_SPEC            \
       "%{I*:-I%*} %{D*:-D%*} %{U*:-U%*} \
