...
struct binary_traits<T1, T2, FLEXIBLE_PRECISION, CONST_PRECISION> {
  typedef generic_wide_int<widest_int_storage<int_traits<T2>::precision>>
      result_type;
  typedef result_type operator_result;
  typedef bool predicate_result;
  typedef result_type signed_shift_result_type;
...
struct binary_traits<T1, T2, CONST_PRECISION, FLEXIBLE_PRECISION> {
  typedef generic_wide_int<widest_int_storage<int_traits<T1>::precision>>
      result_type;
  typedef result_type operator_result;
  typedef bool predicate_result;
  typedef result_type signed_shift_result_type;
...
inline unsigned int wi::storage_ref::get_precision() const { return precision; }

inline const HOST_WIDE_INT *wi::storage_ref::get_val() const { return val; }
...
/* Like elt, but sign-extend beyond the upper bit, instead of returning
   the raw encoding.  */
template <typename storage>
inline HOST_WIDE_INT generic_wide_int<storage>::sext_elt(unsigned int i) const {
...
inline wi::storage_ref wi::int_traits<generic_wide_int<storage>>::decompose(
    HOST_WIDE_INT *, unsigned int precision,
    const generic_wide_int<storage> &x) {
...
inline unsigned int wide_int_storage::get_len() const { return len; }

inline HOST_WIDE_INT *wide_int_storage::write_val(unsigned int) {
  return UNLIKELY(precision > WIDE_INT_MAX_INL_PRECISION) ? u.valp : u.val;
}
...
inline wide_int wide_int_storage::from_array(const HOST_WIDE_INT *val,
  ...
  wide_int result = wide_int::create(precision);
  result.set_len(
      wi::from_array(result.write_val(len), val, len, precision, need_canon_p));
  return result;
}
...
inline wide_int wide_int_storage::create(unsigned int precision) {
  ...
  if (UNLIKELY(precision > WIDE_INT_MAX_INL_PRECISION))
    x.u.valp = XNEWVEC(HOST_WIDE_INT, CEIL(precision, HOST_BITS_PER_WIDE_INT));
  return x;
}
...
template <int N>
inline HOST_WIDE_INT *fixed_wide_int_storage<N>::write_val(unsigned int) {
  return val;
}
...
inline FIXED_WIDE_INT(N) fixed_wide_int_storage<N>::from_array(
  ...
  FIXED_WIDE_INT(N) result;
  result.set_len(
      wi::from_array(result.write_val(len), val, len, N, need_canon_p));
  return result;
}
...
inline FIXED_WIDE_INT(N)
    wi::int_traits<fixed_wide_int_storage<N>>::get_binary_result(const T1 &,
                                                                 const T2 &) {
...
template <typename T1, typename T2>
inline unsigned int
wi::int_traits<fixed_wide_int_storage<N>>::get_binary_precision(const T1 &,
                                                                const T2 &) {
  return N;
}
...
template <int N> class GTY(()) widest_int_storage {
private:
  union {
    HOST_WIDE_INT val[WIDE_INT_MAX_INL_ELTS];
    HOST_WIDE_INT *valp;
...
public:
  ...
  template <typename T> widest_int_storage(const T &);
  ~widest_int_storage();
  widest_int_storage &operator=(const widest_int_storage &);
  template <typename T> inline widest_int_storage &operator=(const T &);
...
namespace wi {
template <int N> struct int_traits<widest_int_storage<N>> {
...
template <int N> inline widest_int_storage<N>::widest_int_storage() : len(0) {}
...
template <int N>
inline widest_int_storage<N>::widest_int_storage(const widest_int_storage &x) {
...
template <int N> inline widest_int_storage<N>::~widest_int_storage() {
  if (UNLIKELY(len > WIDE_INT_MAX_INL_ELTS))
    XDELETEVEC(u.valp);
}
...
template <int N>
template <typename T>
inline widest_int_storage<N> &widest_int_storage<N>::operator=(const T &x) {
...
template <int N>
inline unsigned int widest_int_storage<N>::get_precision() const {
  return N;
}

template <int N>
inline const HOST_WIDE_INT *widest_int_storage<N>::get_val() const {
  return UNLIKELY(len > WIDE_INT_MAX_INL_ELTS) ? u.valp : u.val;
}

template <int N> inline unsigned int widest_int_storage<N>::get_len() const {
  return len;
}

template <int N>
inline HOST_WIDE_INT *widest_int_storage<N>::write_val(unsigned int l) {
  if (UNLIKELY(len > WIDE_INT_MAX_INL_ELTS))
    XDELETEVEC(u.valp);
  len = l;
...
template <int N>
inline void widest_int_storage<N>::set_len(unsigned int l, bool) {
  gcc_checking_assert(l <= len);
  if (UNLIKELY(len > WIDE_INT_MAX_INL_ELTS) && l <= WIDE_INT_MAX_INL_ELTS) {
...
/* Treat X as having signedness SGN and convert it to an N-bit number.  */
template <int N>
inline WIDEST_INT(N) widest_int_storage<N>::from(const wide_int_ref &x,
                                                 signop sgn) {
  WIDEST_INT(N) result;
...
/* Create a WIDEST_INT (N) from the explicit block encoding given by
   ...
   trailing blocks.  */
template <int N>
inline WIDEST_INT(N) widest_int_storage<N>::from_array(const HOST_WIDE_INT *val,
                                                       unsigned int len,
                                                       bool need_canon_p) {
  WIDEST_INT(N) result;
...
template <int N>
template <typename T1, typename T2>
inline WIDEST_INT(N) wi::int_traits<widest_int_storage<N>>::get_binary_result(
    const T1 &, const T2 &) {
  return WIDEST_INT(N)();
}
...
template <typename T1, typename T2>
inline unsigned int
wi::int_traits<widest_int_storage<N>>::get_binary_precision(const T1 &,
                                                            const T2 &) {
  return N;
}
...
/* A variable-length array of wide_int-like objects that can be put
   ...
   structure.  */
template <int N> struct GTY((user)) trailing_wide_ints {
private:
...
public:
  ...
  size_t extra_size() const { return extra_size(m_precision, m_num_elements); }
};

inline trailing_wide_int_storage::trailing_wide_int_storage(
    unsigned int precision, unsigned short *len, HOST_WIDE_INT *val)
...
inline HOST_WIDE_INT *trailing_wide_int_storage::write_val(unsigned int) {
  return m_val;
}
...
inline wi::storage_ref
wi::int_traits<wi::hwi_with_prec>::decompose(HOST_WIDE_INT *scratch,
                                             unsigned int precision,
...
unsigned int set_bit_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int,
                           unsigned int, unsigned int);
unsigned int bswap_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int,
                         unsigned int);
...
inline WI_UNARY_RESULT(T) wi::sext(const T &x, unsigned int offset) {
  ...
  if (result.needs_write_val_arg)
    val = result.write_val(MAX(xi.len, CEIL(offset, HOST_BITS_PER_WIDE_INT)));
...
inline WI_UNARY_RESULT(T) wi::zext(const T &x, unsigned int offset) {
  ...
  if (result.needs_write_val_arg)
    val = result.write_val(MAX(xi.len, offset / HOST_BITS_PER_WIDE_INT + 1));
...
inline WI_UNARY_RESULT(T) wi::set_bit(const T &x, unsigned int bit) {
  ...
  WIDE_INT_REF_FOR(T) xi(x, precision);
  if (result.needs_write_val_arg)
    val = result.write_val(MAX(xi.len, bit / HOST_BITS_PER_WIDE_INT + 1));
  if (precision <= HOST_BITS_PER_WIDE_INT) {
...
template <typename T> inline WI_UNARY_RESULT(T) wi::bswap(const T &x) {
  WI_UNARY_RESULT_VAR(result, val, T, x);
  unsigned int precision = get_precision(result);
  WIDE_INT_REF_FOR(T) xi(x, precision);
...
template <typename T> inline WI_UNARY_RESULT(T) wi::bitreverse(const T &x) {
  WI_UNARY_RESULT_VAR(result, val, T, x);
  unsigned int precision = get_precision(result);
  WIDE_INT_REF_FOR(T) xi(x, precision);
  static_assert(!result.needs_write_val_arg,
...
inline WI_BINARY_RESULT(T1, T2) wi::div_trunc(const T1 &x, const T2 &y,
  ...
  if (quotient.needs_write_val_arg)
    quotient_val =
        quotient.write_val((sgn == UNSIGNED && xi.val[xi.len - 1] < 0)
                               ? CEIL(precision, HOST_BITS_PER_WIDE_INT) + 1
                               : xi.len + 1);
  quotient.set_len(divmod_internal(quotient_val, 0, 0, xi.val, xi.len,
                                   precision, yi.val, yi.len, yi.precision, sgn,
                                   overflow));
  return quotient;
}
...
inline WI_BINARY_RESULT(T1, T2) wi::div_floor(const T1 &x, const T2 &y,
  ...
  quotient.set_len(divmod_internal(quotient_val, &remainder_len, remainder_val,
                                   xi.val, xi.len, precision, yi.val, yi.len,
...
inline WI_BINARY_RESULT(T1, T2) wi::mod_trunc(const T1 &x, const T2 &y,
  ...
  if (remainder.needs_write_val_arg)
    remainder_val =
        remainder.write_val((sgn == UNSIGNED && xi.val[xi.len - 1] < 0)
                                ? CEIL(precision, HOST_BITS_PER_WIDE_INT) + 1
                                : xi.len + 1);
...
wi::lrotate(const T1 &x, const T2 &y, unsigned int width) {
  ...
  WI_UNARY_RESULT(T1)
  right = wi::lrshift(width != precision ? wi::zext(x, width) : x,
                      wi::sub(width, ymod));
  if (width != precision)
...
wi::rrotate(const T1 &x, const T2 &y, unsigned int width) {
  ...
  WI_UNARY_RESULT(T2) ymod = umod_trunc(y, width);
  WI_UNARY_RESULT(T1)
  right = wi::lrshift(width != precision ? wi::zext(x, width) : x, ymod);
...
/* Return 0 if the number of 1s in X is even and 1 if the number of 1s
   is odd.  */
inline int wi::parity(const wide_int_ref &x) { return popcount(x) & 1; }
...
void gt_pch_nx(generic_wide_int<wide_int_storage> *, gt_pointer_operator,
               void *) = delete;
...
template <int N>
void gt_pch_nx(generic_wide_int<widest_int_storage<N>> *, gt_pointer_operator,
               void *) = delete;
...
inline wide_int wi::shifted_mask(unsigned int start, unsigned int width,
  ...
  wide_int result = wide_int::create(precision);
  result.set_len(
      shifted_mask(result.write_val(0), start, width, negate_p, precision));
  return result;
}
...
/* Return an integer of type T in which the low START bits are clear,
   ...
   inverse if NEGATE_P.  */
template <typename T>
inline T wi::shifted_mask(unsigned int start, unsigned int width,
                          bool negate_p) {
  STATIC_ASSERT(wi::int_traits<T>::precision);
  T result;
  ...
  unsigned int est_len =
      result.needs_write_val_arg
          ...
          : 0;
  result.set_len(
      shifted_mask(result.write_val(est_len), start, width, negate_p, prec));
  return result;
}
...
inline void wi::accumulate_overflow(wi::overflow_type &overflow,
