...
public:
  ...
  template <typename U> ALWAYS_INLINE CONSTEXPR opt_mode(const opt_mode<U> &);
...
template <typename T>
inline machine_mode opt_mode<T>::else_mode(machine_mode fallback) const {
...
template <typename T> struct pod_mode {
  ...
  ALWAYS_INLINE CONSTEXPR operator T() const { return from_int(m_mode); }
...
public:
  ...
  ALWAYS_INLINE CONSTEXPR scalar_mode(from_int m) : m_mode(machine_mode(m)) {}

  ALWAYS_INLINE CONSTEXPR scalar_mode(const scalar_int_mode &m) : m_mode(m) {}

  ALWAYS_INLINE CONSTEXPR scalar_mode(const scalar_float_mode &m) : m_mode(m) {}
...
public:
  ...
  ALWAYS_INLINE CONSTEXPR complex_mode(from_int m) : m_mode(machine_mode(m)) {}
...
#if GCC_VERSION >= 4001
  return scalar_mode::from_int(
      __builtin_constant_p(mode) ? mode_inner_inline(mode) : mode_inner[mode]);
#else
...
template <typename T> ALWAYS_INLINE opt_mode<T> GET_MODE_NEXT_MODE(const T &m) {
...
public:
  ...
  ALWAYS_INLINE CONSTEXPR fixed_size_mode(const scalar_mode &m) : m_mode(m) {}
  ...
  ALWAYS_INLINE CONSTEXPR fixed_size_mode(const complex_mode &m) : m_mode(m) {}
...
inline opt_scalar_int_mode smallest_int_mode_for_size(poly_uint64 size) {
...
public:
  bit_field_mode_iterator(HOST_WIDE_INT, HOST_WIDE_INT, poly_int64, poly_int64,
                          unsigned int, bool);
...
extern bool get_best_mode(HOST_WIDE_INT, HOST_WIDE_INT, poly_uint64,
                          poly_uint64, unsigned int, unsigned HOST_WIDE_INT,
...
inline bool iterate_p(machine_mode *iter) { return *iter != E_VOIDmode; }

/* Set mode iterator *ITER to the next mode in the same class,
   if any.  */

template <typename T> inline void get_next(opt_mode<T> *iter) {
  *iter = GET_MODE_NEXT_MODE(iter->require());
}

inline void get_next(machine_mode *iter) {
...
template <typename T> inline void get_known_next(T *iter) {
  *iter = GET_MODE_NEXT_MODE(*iter).require();
}
