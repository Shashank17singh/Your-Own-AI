...
extern cselib_val *cselib_lookup_from_insn(rtx, machine_mode, int, machine_mode,
                                           rtx_insn *);
...
extern rtx cselib_expand_value_rtx_cb(rtx, bitmap, int, cselib_expand_callback,
                                      void *);
...
inline cselib_val *canonical_cselib_val(cselib_val *val) {
  cselib_val *canon;

  if (!val->locs || val->locs->next || !val->locs->loc ||
      GET_CODE(val->locs->loc) != VALUE ||
...
inline bool rtx_equal_for_cselib_p(rtx x, rtx y) {
  if (x == y)
    return true;
