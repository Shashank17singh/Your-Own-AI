...
inline unsigned HOST_WIDE_INT least_bit_hwi(unsigned HOST_WIDE_INT x) {
  return (x & -x);
}

/* True if X is zero or a power of two.  */

inline bool pow2_or_zerop(unsigned HOST_WIDE_INT x) {
  return least_bit_hwi(x) == x;
}

/* True if X is a power of two.  */

inline bool pow2p_hwi(unsigned HOST_WIDE_INT x) {
...
inline int clz_hwi(unsigned HOST_WIDE_INT x) {
  if (x == 0)
...
inline int ctz_hwi(unsigned HOST_WIDE_INT x) {
  if (x == 0)
    return HOST_BITS_PER_WIDE_INT;
#if HOST_BITS_PER_WIDE_INT == HOST_BITS_PER_LONG
...
inline int ffs_hwi(unsigned HOST_WIDE_INT x) {
#if HOST_BITS_PER_WIDE_INT == HOST_BITS_PER_LONG
...
inline int popcount_hwi(unsigned HOST_WIDE_INT x) {
#if HOST_BITS_PER_WIDE_INT == HOST_BITS_PER_LONG
...
inline int floor_log2(unsigned HOST_WIDE_INT x) {
  return HOST_BITS_PER_WIDE_INT - 1 - clz_hwi(x);
}

inline int ceil_log2(unsigned HOST_WIDE_INT x) {
  return x == 0 ? 0 : floor_log2(x - 1) + 1;
}

inline int exact_log2(unsigned HOST_WIDE_INT x) {
...
inline int ctz_or_zero(unsigned HOST_WIDE_INT x) { return ffs_hwi(x) - 1; }

/* Sign extend SRC starting from PREC.  */

inline HOST_WIDE_INT sext_hwi(HOST_WIDE_INT src, unsigned int prec) {
...
inline unsigned HOST_WIDE_INT zext_hwi(unsigned HOST_WIDE_INT src,
                                       unsigned int prec) {
  if (prec == HOST_BITS_PER_WIDE_INT)
    return src;
  else {
...
inline HOST_WIDE_INT add_hwi(HOST_WIDE_INT a, HOST_WIDE_INT b, bool *overflow) {
#if GCC_VERSION < 11000
  unsigned HOST_WIDE_INT result = a + (unsigned HOST_WIDE_INT)b;
  if ((((result ^ a) & (result ^ b)) >> (HOST_BITS_PER_WIDE_INT - 1)) & 1)
    *overflow = true;
  else
    *overflow = false;
  return result;
...
inline HOST_WIDE_INT add_sat_hwi(HOST_WIDE_INT a, HOST_WIDE_INT b) {
  bool overflow;
...
inline HOST_WIDE_INT mul_sat_hwi(HOST_WIDE_INT a, HOST_WIDE_INT b) {
  bool overflow;
