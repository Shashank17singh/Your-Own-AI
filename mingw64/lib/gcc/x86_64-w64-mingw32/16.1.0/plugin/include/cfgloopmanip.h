...
#define DLTHE_RECORD_HIERARCHICAL_DISCRIMINATOR                                \
  16 /* Assign hierarchical                                                    \
        discriminators to                                                      \
        distinguish loop                                                       \
        iterations.  */
...
extern bool duplicate_loop_body_to_header_edge(class loop *, edge, unsigned,
                                               sbitmap, edge, vec<edge> *, int);
extern bool mfb_keep_just(edge);
basic_block create_preheader(class loop *, int);
extern void create_preheaders(int);
extern void force_single_succ_latches(void);
class loop *loop_version(class loop *, void *, basic_block *,
