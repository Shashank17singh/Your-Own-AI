...
template <int M> struct bbitmap_operators {
  ...
  template <typename Arg> static constexpr bool non_zero(const Arg &x) {
  ...
  static constexpr Result from_index(int index, Rest... rest) {
    return bbitmap_operators<M - 1>::template from_index<Result>(
        index, uint64_t((index - (M - 1) * 64) == (index & 63)) << (index & 63),
        rest...);
  }
};
...
/* These functions form the base for the recursive functions above.  They
   ...
   bool result.  */
template <> struct bbitmap_operators<0> {
  template <typename Result, typename Operator, typename Arg, typename... Rest>
  ...
  template <typename Arg> static constexpr bool non_zero(const Arg) {
    return false;
  }

  template <typename Arg> static constexpr bool equal(const Arg, const Arg) {
    return true;
  }
...
template <typename T> constexpr T bbitmap_element_or(T x, T y) { return x | y; }

template <typename T> constexpr T bbitmap_element_and(T x, T y) {
  return x & y;
}

template <typename T> constexpr T bbitmap_element_xor(T x, T y) {
  return x ^ y;
}

template <int N> class GTY((user)) bbitmap {
public:
  uint64_t val[N];
...
template <int N> void gt_ggc_mx(bbitmap<N> *) {}

template <int N> void gt_pch_nx(bbitmap<N> *) {}

template <int N> void gt_pch_nx(bbitmap<N> *, gt_pointer_operator, void *) {}
