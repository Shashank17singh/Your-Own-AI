...
inline void irange_bitmask::set_unknown(unsigned prec) {
...
inline bool irange_bitmask::unknown_p() const { return m_mask == -1; }

inline irange_bitmask::irange_bitmask(const wide_int &value,
                                      const wide_int &mask) {
  m_value = value;
  m_mask = mask;
  if (flag_checking)
...
inline unsigned irange_bitmask::get_precision() const {
...
inline wide_int irange_bitmask::get_nonzero_bits() const {
  return m_value | m_mask;
}

// Set the bitmask to the nonzero bits in BITS.
inline void irange_bitmask::set_nonzero_bits(const wide_int &bits) {
...
inline bool irange_bitmask::member_p(const wide_int &val) const {
  if (unknown_p())
    return true;
...
inline bool irange_bitmask::operator==(const irange_bitmask &src) const {
...
inline void irange_bitmask::union_(const irange_bitmask &src) {
...
public:
  unsupported_range() : vrange(VR_UNKNOWN) { set_undefined(); }
  unsupported_range(const unsupported_range &src) : vrange(VR_UNKNOWN) {
...
inline nan_state::nan_state(bool nan_p) {
  m_pos_nan = nan_p;
  m_neg_nan = nan_p;
}
...
inline nan_state::nan_state(bool pos_nan, bool neg_nan) {
  m_pos_nan = pos_nan;
  m_neg_nan = neg_nan;
}
...
inline bool nan_state::pos_p() const { return m_pos_nan; }

// Return if -NAN is possible.

inline bool nan_state::neg_p() const { return m_neg_nan; }
...
inline const REAL_VALUE_TYPE &frange::lower_bound() const {
  gcc_checking_assert(!undefined_p() && !known_isnan());
  return m_min;
}

inline const REAL_VALUE_TYPE &frange::upper_bound() const {
  gcc_checking_assert(!undefined_p() && !known_isnan());
  return m_max;
}
...
inline nan_state frange::get_nan_state() const {
...
template <typename T> inline bool is_a(vrange &) {
  gcc_unreachable();
  return false;
}

template <typename T> inline bool is_a(const vrange &v) {
  // Reuse is_a <vrange> to implement the const version.
  const T &derived = static_cast<const T &>(v);
  return is_a<T>(const_cast<T &>(derived));
}

template <typename T> inline T &as_a(vrange &v) {
  gcc_checking_assert(is_a<T>(v));
  return static_cast<T &>(v);
}

template <typename T> inline const T &as_a(const vrange &v) {
  gcc_checking_assert(is_a<T>(v));
...
template <> inline bool is_a<irange>(vrange &v) {
  return v.m_discriminator == VR_IRANGE;
}

template <> inline bool is_a<prange>(vrange &v) {
  return v.m_discriminator == VR_PRANGE;
}

template <> inline bool is_a<frange>(vrange &v) {
  return v.m_discriminator == VR_FRANGE;
}

template <> inline bool is_a<unsupported_range>(vrange &v) {
...
inline void irange::maybe_resize(int needed) {
...
inline int_range<N, RESIZABLE>::~int_range() {
...
public:
  ...
  void set_zero(tree type) {
    init(type);
    return m_vrange->set_zero(type);
  }
  void set_nonzero(tree type) {
    init(type);
    return m_vrange->set_nonzero(type);
  }
...
inline value_range::value_range() : m_buffer() { m_vrange = NULL; }

// Copy constructor.

inline value_range::value_range(const value_range &r) { init(*r.m_vrange); }

// Copy constructor from a vrange.

inline value_range::value_range(const vrange &r) { init(r); }

// Construct an UNDEFINED range that can hold ranges of TYPE.  If TYPE
// is not supported, default to unsupported_range.

inline value_range::value_range(tree type) { init(type); }

// Construct a range that can hold a range of [MIN, MAX], where MIN
// and MAX are trees.

inline value_range::value_range(tree min, tree max, value_range_kind kind) {
  init(TREE_TYPE(min));
  m_vrange->set(min, max, kind);
}

inline value_range::~value_range() {
  if (m_vrange)
    m_vrange->~vrange();
}
...
inline void value_range::set_type(tree type) {
  if (m_vrange)
    m_vrange->~vrange();
  init(type);
}
...
inline void value_range::init(tree type) {
...
inline void value_range::init(const vrange &r) {
  if (is_a<irange>(r))
...
inline vrange &value_range::operator=(const vrange &r) {
  if (m_vrange)
    m_vrange->~vrange();
  init(r);
  return *m_vrange;
...
inline value_range &value_range::operator=(const value_range &r) {
...
inline bool value_range::operator==(const value_range &r) const {
...
inline value_range::operator vrange &() { return *m_vrange; }

inline value_range::operator const vrange &() const { return *m_vrange; }

// Return TRUE if TYPE is supported by the vrange infrastructure.

inline bool value_range::supports_type_p(const_tree type) {
...
inline unsigned irange::num_pairs() const { return m_num_ranges; }

inline tree irange::type() const {
  gcc_checking_assert(m_num_ranges > 0);
  return m_type;
}

inline bool irange::varying_compatible_p() const {
  if (m_num_ranges != 1)
    return false;
  ...
  if (INTEGRAL_TYPE_P(t) || POINTER_TYPE_P(t))
    return (l == wi::min_value(prec, sign) && u == wi::max_value(prec, sign) &&
            m_bitmask.unknown_p());
  return true;
}

inline bool vrange::varying_p() const { return m_kind == VR_VARYING; }

inline bool vrange::undefined_p() const { return m_kind == VR_UNDEFINED; }

inline bool irange::zero_p() const {
  return (m_kind == VR_RANGE && m_num_ranges == 1 && lower_bound(0) == 0 &&
          upper_bound(0) == 0);
}

inline bool irange::nonzero_p() const {
  if (undefined_p())
    return false;
...
inline bool irange::supports_p(const_tree type) {
  return INTEGRAL_TYPE_P(type);
}

inline bool irange::contains_p(tree cst) const {
  return contains_p(wi::to_wide(cst));
}

inline bool range_includes_zero_p(const vrange &vr) {
  if (vr.undefined_p())
    return false;
...
inline irange::irange(wide_int *base, unsigned nranges, bool resizable)
    : vrange(VR_IRANGE), m_resizable(resizable), m_max_ranges(nranges) {
  m_base = base;
  set_undefined();
}
...
template <unsigned N, bool RESIZABLE>
inline int_range<N, RESIZABLE>::int_range() : irange(m_ranges, N, RESIZABLE) {}
...
template <unsigned N, bool RESIZABLE>
int_range<N, RESIZABLE>::int_range(tree type) : irange(m_ranges, N, RESIZABLE) {
  set_varying(type);
}

template <unsigned N, bool RESIZABLE>
int_range<N, RESIZABLE>::int_range(tree type, const wide_int &wmin,
                                   const wide_int &wmax, value_range_kind kind)
    : irange(m_ranges, N, RESIZABLE) {
...
inline void irange::set_undefined() {
  m_kind = VR_UNDEFINED;
  m_num_ranges = 0;
}

inline void irange::set_varying(tree type) {
  m_kind = VR_VARYING;
  m_num_ranges = 1;
...
inline wide_int irange::lower_bound(unsigned pair) const {
  gcc_checking_assert(m_num_ranges > 0);
...
inline wide_int irange::upper_bound(unsigned pair) const {
  gcc_checking_assert(m_num_ranges > 0);
  gcc_checking_assert(pair + 1 <= num_pairs());
...
inline wide_int irange::upper_bound() const {
...
inline void irange::set_nonzero(tree type) {
...
inline void irange::set_zero(tree type) {
  wide_int zero = wi::zero(TYPE_PRECISION(type));
  set(type, zero, zero);
}
...
inline void irange::normalize_kind() {
  if (m_num_ranges == 0)
    set_undefined();
...
inline bool contains_zero_p(const irange &r) {
  if (r.undefined_p())
    return false;
...
inline wide_int irange_val_min(const_tree type) {
  gcc_checking_assert(irange::supports_p(type));
  return wi::min_value(TYPE_PRECISION(type), TYPE_SIGN(type));
}

inline wide_int irange_val_max(const_tree type) {
  gcc_checking_assert(irange::supports_p(type));
  return wi::max_value(TYPE_PRECISION(type), TYPE_SIGN(type));
}

inline prange::prange() : vrange(VR_PRANGE) { set_undefined(); }

inline prange::prange(const prange &r) : vrange(VR_PRANGE) { *this = r; }

inline prange::prange(tree type) : vrange(VR_PRANGE) { set_varying(type); }

inline prange::prange(tree type, const wide_int &lb, const wide_int &ub,
                      value_range_kind kind)
    : vrange(VR_PRANGE) {
  set(type, lb, ub, kind);
}

inline bool prange::supports_p(const_tree type) { return POINTER_TYPE_P(type); }

inline bool prange::supports_type_p(const_tree type) const {
  return POINTER_TYPE_P(type);
}

inline void prange::set_undefined() { m_kind = VR_UNDEFINED; }

inline void prange::set_varying(tree type) {
  m_kind = VR_VARYING;
  m_type = type;
...
inline void prange::set_nonzero(tree type) {
  m_kind = VR_RANGE;
  m_type = type;
...
inline void prange::set_zero(tree type) {
  m_kind = VR_RANGE;
  m_type = type;
  wide_int zero = wi::zero(TYPE_PRECISION(type));
  m_min = m_max = zero;
...
inline bool prange::contains_p(tree cst) const {
  return contains_p(wi::to_wide(cst));
}

inline bool prange::zero_p() const {
  return m_kind == VR_RANGE && m_min == 0 && m_max == 0;
}

inline bool prange::nonzero_p() const {
  return m_kind == VR_RANGE && m_min == 1 && m_max == -1;
}

inline tree prange::type() const {
  gcc_checking_assert(!undefined_p());
  return m_type;
}

inline wide_int prange::lower_bound() const {
  gcc_checking_assert(!undefined_p());
  return m_min;
}

inline wide_int prange::upper_bound() const {
  gcc_checking_assert(!undefined_p());
  return m_max;
}

inline bool prange::varying_compatible_p() const {
  return (!undefined_p() && m_min == 0 && m_max == -1 &&
          get_bitmask().unknown_p());
}

inline irange_bitmask prange::get_bitmask() const { return m_bitmask; }

inline bool prange::fits_p(const vrange &) const { return true; }

inline frange::frange() : vrange(VR_FRANGE) { set_undefined(); }

inline frange::frange(const frange &src) : vrange(VR_FRANGE) { *this = src; }

inline frange::frange(tree type) : vrange(VR_FRANGE) { set_varying(type); }

// frange constructor from REAL_VALUE_TYPE endpoints.

inline frange::frange(tree type, const REAL_VALUE_TYPE &min,
                      const REAL_VALUE_TYPE &max, value_range_kind kind)
    : vrange(VR_FRANGE) {
...
inline frange::frange(tree min, tree max, value_range_kind kind)
    : vrange(VR_FRANGE) {
  set(min, max, kind);
}

inline tree frange::type() const {
  gcc_checking_assert(!undefined_p());
  return m_type;
}

inline void frange::set_varying(tree type) {
  m_kind = VR_VARYING;
  m_type = type;
...
inline void frange::set_undefined() {
  m_kind = VR_UNDEFINED;
  m_type = NULL;
  m_pos_nan = false;
  m_neg_nan = false;
...
inline void frange::update_nan(const nan_state &nan) {
  gcc_checking_assert(!undefined_p());
  if (HONOR_NANS(m_type)) {
    m_pos_nan = nan.pos_p();
    m_neg_nan = nan.neg_p();
...
inline void frange::update_nan(bool sign) {
  gcc_checking_assert(!undefined_p());
  nan_state nan(/*pos=*/!sign, /*neg=*/sign);
  update_nan(nan);
}

inline bool frange::contains_p(tree cst) const {
  return contains_p(*TREE_REAL_CST_PTR(cst));
}

// Clear the NAN bit and adjust the range.

inline void frange::clear_nan() {
  gcc_checking_assert(!undefined_p());
  m_pos_nan = false;
  m_neg_nan = false;
  normalize_kind();
...
inline REAL_VALUE_TYPE real_max_representable(const_tree type) {
  REAL_VALUE_TYPE r;
  char buf[128];
  get_max_float(REAL_MODE_FORMAT(TYPE_MODE(type)), buf, sizeof(buf), false);
...
inline REAL_VALUE_TYPE real_min_representable(const_tree type) {
...
inline bool frange_val_is_min(const REAL_VALUE_TYPE &r, const_tree type) {
...
inline bool frange_val_is_max(const REAL_VALUE_TYPE &r, const_tree type) {
...
inline void frange::set_nan(tree type, const nan_state &nan) {
...
inline void frange::set_nan(tree type) {
  nan_state nan(true);
  set_nan(type, nan);
}
...
inline void frange::set_nan(tree type, bool sign) {
  nan_state nan(/*pos=*/!sign, /*neg=*/sign);
  set_nan(type, nan);
}
...
inline bool frange::known_isfinite() const {
...
inline bool frange::known_isnormal() const {
  if (!known_isfinite())
    return false;
...
inline bool frange::known_isdenormal_or_zero() const {
  if (!known_isfinite())
    return false;
...
inline bool frange::maybe_isinf() const {
...
inline bool frange::known_isinf() const {
  return (m_kind == VR_RANGE && !maybe_isnan() &&
          real_identical(&m_min, &m_max) && real_isinf(&m_min));
}

// Return TRUE if range is possibly a NAN.

inline bool frange::maybe_isnan() const {
  if (undefined_p())
    return false;
...
inline bool frange::maybe_isnan(bool sign) const {
  if (undefined_p())
    return false;
  if (sign)
    return m_neg_nan;
...
inline bool frange::known_isnan() const { return m_kind == VR_NAN; }

// If the signbit for the range is known, set it in SIGNBIT and return
// TRUE.

inline bool frange::signbit_p(bool &signbit) const {
  if (undefined_p())
    return false;
...
inline bool frange::nan_signbit_p(bool &signbit) const {
  if (undefined_p())
    return false;
...
inline bool range_compatible_p(tree type1, tree type2) {
