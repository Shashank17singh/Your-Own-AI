...
struct GTY((desc("%h.type"), tag("TOPLEVEL_BASE"), chain_next("%h.next"),
...
struct GTY((tag("SYMTAB_SYMBOL"))) symtab_node : public toplevel_node {
public:
  friend class symbol_table;

  /* Constructor.  */
  explicit symtab_node(toplevel_type t)
      : toplevel_node(t), resolution(LDPR_UNKNOWN), definition(false),
        alias(false), transparent_alias(false), weakref(false),
        cpp_implicit_alias(false), symver(false), analyzed(false),
        writeonly(false), refuse_visibility_changes(false),
        externally_visible(false), no_reorder(false), force_output(false),
        forced_by_abi(false), ref_by_asm(false), must_remain_in_tu_name(false),
        must_remain_in_tu_body(false), unique_name(false),
        implicit_section(false), body_removed(false),
        ...
        next_sharing_asm_name(NULL), previous_sharing_asm_name(NULL),
        same_comdat_group(NULL), ref_list(), alias_target(NULL), aux(NULL),
  ...
  ipa_ref *find_reference(symtab_node *referred_node, gimple *stmt,
                          unsigned int lto_stmt_uid, enum ipa_ref_use use_type);
  ...
  inline unsigned num_references(void) { return ref_list.references.length(); }
  ...
  /* Return comdat group.  */
  tree get_comdat_group() { return x_comdat_group; }
  ...
  inline bool can_be_discarded_p(void) {
    return ((DECL_EXTERNAL(decl) && !in_other_partition) ||
  ...
  inline int get_uid() const { return m_uid; }
...
#define FOR_EACH_ALIAS(NODE, ALIAS)                                            \
  for (unsigned ALIAS##_iter_ = 0;                                             \
       (NODE)->iterate_direct_aliases(ALIAS##_iter_, ALIAS); ALIAS##_iter_++)
...
struct GTY((tag("SYMTAB_FUNCTION"))) cgraph_node : public symtab_node {
  ...
  explicit cgraph_node()
      : symtab_node(SYMTAB_FUNCTION), callees(NULL), callers(NULL),
        ...
        only_called_at_exit(false), tm_clone(false), dispatcher_function(false),
        dispatcher_resolver_function(false), is_target_clone(false),
        calls_comdat_local(false), icf_merged(false), nonfreeing_fn(false),
        merged_comdat(false), merged_extern_inline(false),
        parallelized_function(false), split_part(false),
        indirect_call_target(false), local(false), versionable(false),
        can_change_signature(false), redefined_extern_inline(false),
        tm_may_enter_irr(false), ipcp_clone(false), gc_candidate(false),
  ...
  /* Create node representing clone of N executed COUNT times.  Decrease
     ...
     dump file.  */
  cgraph_node *
  create_clone(tree decl, profile_count count, bool update_original,
               vec<cgraph_edge *> redirect_callers, bool call_duplication_hook,
               cgraph_node *new_inlined_to,
               ipa_param_adjustments *param_adjustments, const char *suffix);
  ...
  cgraph_node *create_version_clone_with_body(
      vec<cgraph_edge *> redirect_callers,
      vec<ipa_replace_map *, va_gc> *tree_map,
      ipa_param_adjustments *param_adjustments, bitmap bbs_to_copy,
      basic_block new_entry_block, const char *suffix,
  ...
  cgraph_node *create_thunk(tree alias, tree, bool this_adjusting,
                            ...
                            HOST_WIDE_INT indirect_offset, tree virtual_offset,
                            tree real_alias);
  ...
  cgraph_edge *create_edge(cgraph_node *callee, gcall *call_stmt,
                           profile_count count, bool cloning_p = false);
  ...
  /* Return true when function cgraph_node can be expected to be removed
     ...
     direct calls.  */
  bool
  will_be_removed_from_program_if_no_direct_calls_p(bool will_inline = false);
  ...
  static inline void debug_cgraph(void) { dump_cgraph(stderr); }

  /* Get summary id of the node.  */
  inline int get_summary_id() { return m_summary_id; }
...
class GTY((tag("CIIK_SIMPLE"))) cgraph_simple_indirect_info
    : public cgraph_indirect_call_info {
public:
...
class GTY((tag("CIIK_POLYMORPHIC"))) cgraph_polymorphic_indirect_info
    : public cgraph_indirect_call_info {
public:
  ...
  /* Return true if the information is usable for devirtualization.  This can
     ...
     information too.  */
  bool usable_p() const { return !!otr_type; }
...
inline bool usable_polymorphic_info_p(cgraph_indirect_call_info *ii) {
...
public:
  ...
  cgraph_edge *speculative_call_indirect_edge() {
    ...
    if (!callee)
      return this;
    for (cgraph_edge *e2 = caller->indirect_calls; true; e2 = e2->next_callee)
  ...
  ipa_ref *speculative_call_target_ref() {
    ...
    for (unsigned int i = 0; caller->iterate_reference(i, ref); i++)
      if (ref->speculative && ref->speculative_id == speculative_id &&
          ref->stmt == (gimple *)call_stmt && ref->lto_stmt_uid == lto_stmt_uid)
        return ref;
    gcc_unreachable();
  }
  ...
  static gimple *
  redirect_call_stmt_to_callee(cgraph_edge *e,
  ...
  inline int get_uid() { return m_uid; }
...
template <>
template <>
inline bool is_a_helper<cgraph_node *>::test(toplevel_node *p) {
  return p && p->type == SYMTAB_FUNCTION;
}
...
template <>
template <>
inline bool is_a_helper<varpool_node *>::test(toplevel_node *p) {
  return p && p->type == SYMTAB_VARIABLE;
}
...
template <>
template <>
inline bool is_a_helper<symtab_node *>::test(toplevel_node *p) {
...
template <>
template <>
inline bool is_a_helper<asm_node *>::test(toplevel_node *p) {
...
template <>
template <>
inline bool is_a_helper<const cgraph_simple_indirect_info *>::test(
    const cgraph_indirect_call_info *p) {
  return p && p->kind == CIIK_SIMPLE;
}
...
template <>
template <>
inline bool is_a_helper<cgraph_polymorphic_indirect_info *>::test(
    cgraph_indirect_call_info *p) {
...
template <>
template <>
inline bool is_a_helper<const cgraph_polymorphic_indirect_info *>::test(
    const cgraph_indirect_call_info *p) {
  return p && p->kind == CIIK_POLYMORPHIC;
}
...
public:
  ...
  symbol_table()
      : cgraph_count(0), cgraph_max_uid(1), cgraph_max_summary_id(0),
        ...
        cgraph_released_summary_ids(), edge_released_summary_ids(), nodes(NULL),
        asmnodes(NULL), asm_last_node(NULL), order(0), max_unit(0),
        global_info_ready(false), state(PARSING), function_flags_ready(false),
        cpp_implicit_aliases_done(false), section_hash(NULL),
        assembler_name_hash(NULL), init_priority_hash(NULL), dump_file(NULL),
        ipa_clones_dump_file(NULL), cloned_nodes(), m_thunks(NULL),
        m_clones(NULL), m_first_edge_removal_hook(NULL),
        m_first_cgraph_removal_hook(NULL), m_first_edge_duplicated_hook(NULL),
        m_first_cgraph_duplicated_hook(NULL),
        m_first_cgraph_insertion_hook(NULL),
  ...
  inline asm_node *first_asm_symbol(void) { return asmnodes; }
...
void tree_function_versioning(tree, tree, vec<ipa_replace_map *, va_gc> *,
                              ipa_param_adjustments *, bool, bitmap,
                              basic_block);
...
inline bool decl_in_symtab_p(const_tree decl) {
...
inline tree symtab_node::get_alias_target_tree() {
  if (alias_target)
    return alias_target;
...
/* Return first static symbol with definition.  */
inline symtab_node *symbol_table::first_symbol(void) { return nodes; }
...
#define FOR_EACH_FUNCTION_REMOVABLE(node)                                      \
  ...
  for ((node) = symtab->first_function(),                                      \
      next = (node) ? symtab->next_function((node)) : NULL;                    \
       (node);                                                                 \
...
inline bool cgraph_node::only_called_directly_or_aliased_p(void) {
  ...
  return (!force_output && !address_taken && !ref_by_asm && !ifunc_resolver &&
...
inline bool cgraph_node::can_remove_if_no_direct_calls_and_refs_p(void) {
  ...
  if (externally_visible && ((!DECL_COMDAT(decl) || ifunc_resolver) ||
                             forced_by_abi || used_from_object_file_p()))
    return false;
  return true;
}
...
inline bool varpool_node::can_remove_if_no_refs_p(void) {
  if (DECL_EXTERNAL(decl))
    return true;
  return (
      !force_output && !used_from_other_partition && !ref_by_asm &&
...
inline int cgraph_edge::frequency() {
  return count.to_cgraph_frequency(
      caller->inlined_to ? caller->inlined_to->count : caller->count);
}

/* Return true if the TM_CLONE bit is set for a given FNDECL.  */
inline bool decl_is_tm_clone(const_tree fndecl) {
...
inline enum optimize_size_level cgraph_node::optimize_for_size_p(void) {
...
inline const char *xstrdup_for_dump(const char *transient_str) {
