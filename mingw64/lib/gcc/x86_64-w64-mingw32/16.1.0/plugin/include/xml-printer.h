...
public:
  ...
  void push_tag_with_class(std::string name, std::string class_,
...
public:
  auto_check_tag_nesting(const printer &xp)
      : m_xp(xp), m_initial_insertion_element(xp.get_insertion_point()) {}
...
public:
  auto_print_element(printer &printer, std::string name,
                     bool preserve_whitespace = false)
      : m_printer(printer), m_name(std::move(name)) {
    m_printer.push_tag(m_name, preserve_whitespace);
  }
  ~auto_print_element() { m_printer.pop_tag(m_name.c_str()); }
