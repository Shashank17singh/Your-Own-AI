...
extern void assert_streq(const location &loc, const char *desc_val1,
                         const char *desc_val2, const char *val1,
                         const char *val2);
...
public:
  temp_source_file(const location &loc, const char *suffix, const char *content,
                   diagnostics::file_cache *fc = nullptr);
  temp_source_file(const location &loc, const char *suffix, const char *content,
                   size_t sz);
};
...
#define ASSERT_STR_CONTAINS_AT(LOC, HAYSTACK, NEEDLE)                          \
  SELFTEST_BEGIN_STMT                                                          \
  ::selftest::assert_str_contains(LOC, #HAYSTACK, #NEEDLE, (HAYSTACK),         \
                                  (NEEDLE));                                   \
  SELFTEST_END_STMT
...
#define ASSERT_STR_STARTSWITH(STR, PREFIX)                                     \
  SELFTEST_BEGIN_STMT                                                          \
  ::selftest::assert_str_startswith(SELFTEST_LOCATION, #STR, #PREFIX, (STR),   \
                                    (PREFIX));                                 \
  SELFTEST_END_STMT
