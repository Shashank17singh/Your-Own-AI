...
inline void timevar_push(timevar_id_t tv) {
  if (g_timer)
    g_timer->push(tv);
}

inline void timevar_pop(timevar_id_t tv) {
  if (g_timer)
    g_timer->pop(tv);
}
...
public:
  auto_cond_timevar(timer *t, timevar_id_t tv) : m_timer(t), m_tv(tv) {
    start();
  }

  explicit auto_cond_timevar(timevar_id_t tv) : m_timer(g_timer), m_tv(tv) {
    start();
  }
