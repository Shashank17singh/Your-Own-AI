/* Generated automatically by the program
   'R:\winlibs_staging_ucrt64\gcc-16.1.0\build_mingw\gcc\build\genpreds.exe'
...
#define TEST_REGISTER_FILTER_BIT(ID, REGNO) ((void)(ID), (void)(REGNO), false)

inline bool test_register_filters(unsigned int, unsigned int) { return true; }
#endif
...
static inline bool insn_extra_special_memory_constraint(enum constraint_num c) {
  return c >= CONSTRAINT_Bm && c <= CONSTRAINT_ja;
}

static inline bool insn_extra_relaxed_memory_constraint(enum constraint_num) {
  return false;
}
...
static inline size_t insn_constraint_len(char fc, const char *str) {
  switch (fc) {
  case 'B':
    return 2;
  case 'T':
...
#ifdef GCC_HARD_REG_SET_H
static inline const HARD_REG_SET *get_register_filter(constraint_num) {
  return nullptr;
}
#endif

static inline int get_register_filter_id(constraint_num) { return -1; }
#endif /* tm-preds.h */
