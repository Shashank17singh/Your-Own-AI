...
template <typename T> inline void vec_destruct(T *dst, unsigned n) {
  for (; n; ++dst, --n)
    dst->~T();
}
...
inline void va_heap::reserve(vec<T, va_heap, vl_embed> *&v, unsigned reserve,
  ...
  if (GATHER_STATISTICS && v)
    v->m_vecpfx.release_overhead(v, elt_size * v->allocated(), v->allocated(),
                                 false);
...
#define DEFINE_DEBUG_VEC(T)                                                    \
  ...
  DEBUG_FUNCTION void debug(vec<T> &ref) { debug_helper<T>(ref); }             \
  ...
  DEBUG_FUNCTION void debug(vec<T, va_gc> &ref) { debug_helper<T>(ref); }      \
...
public:
  ...
  using pop_ret_type =
      typename std::conditional<std::is_trivially_destructible<T>::value, T &,
                                void>::type;
  ...
  T *bsearch(const void *key, int (*compar)(const void *, const void *, void *),
             void *);
...
template <typename T, typename A>
inline void vec_safe_grow(vec<T, A, vl_embed> *&v, unsigned len,
                          bool exact = false CXX_MEM_STAT_INFO) {
...
template <typename T, typename A>
inline void vec_safe_grow_cleared(vec<T, A, vl_embed> *&v, unsigned len,
                                  bool exact = false CXX_MEM_STAT_INFO) {
  unsigned oldlen = vec_safe_length(v);
  gcc_checking_assert(len >= oldlen);
  vec_safe_reserve(v, len - oldlen, exact PASS_MEM_STAT);
...
template <typename T>
inline void vec_safe_grow_cleared(vec<T, va_heap, vl_ptr> *&v, unsigned len,
                                  bool exact = false CXX_MEM_STAT_INFO) {
...
template <typename T>
inline bool vec_safe_reserve(vec<T, va_heap, vl_ptr> *&v, unsigned nelems,
                             bool exact = false CXX_MEM_STAT_INFO) {
...
template <typename T, typename A>
inline bool vec_safe_iterate(const vec<T, A, vl_embed> *v, unsigned ix,
                             T **ptr) {
  if (v)
...
/* If V has no room for one more element, reallocate it.  Then call
   V->quick_push(OBJ).  */
template <typename T, typename A>
inline T *vec_safe_push(vec<T, A, vl_embed> *&v,
...
template <typename T, typename A>
inline const T &vec<T, A, vl_embed>::last(void) const {
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::sort(int (*cmp)(const void *, const void *,
                                                 void *),
                                      void *data) {
#if GCC_VERSION >= 5000
  static_assert(vec_detail::is_trivially_copyable_or_pair<T>::value, "");
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::stablesort(int (*cmp)(const void *,
                                                       const void *, void *),
                                            void *data) {
#if GCC_VERSION >= 5000
  static_assert(vec_detail::is_trivially_copyable_or_pair<T>::value, "");
...
inline size_t vec<T, A, vl_embed>::embedded_size(unsigned alloc) {
  ...
  typedef typename std::conditional<std::is_standard_layout<T>::value, vec,
                                    vec_embedded>::type vec_stdlayout;
...
template <typename T> void gt_pch_nx(vec<T, va_gc_atomic, vl_embed> *) {
  /* No pointers to note.  */
}
...
public:
  ...
  /* Prevent implicit conversion from auto_vec.  Use auto_vec::to_vec()
     instead.  */
  template <size_t N> vec(auto_vec<T, N> &) = delete;

  template <size_t N> void operator=(auto_vec<T, N> &) = delete;
  ...
  /* Vector operations.  */
  bool exists(void) const { return m_vec != NULL; }

  bool is_empty(void) const { return m_vec ? m_vec->is_empty() : true; }

  unsigned allocated(void) const { return m_vec ? m_vec->allocated() : 0; }

  unsigned length(void) const { return m_vec ? m_vec->length() : 0; }

  T *address(void) { return m_vec ? m_vec->address() : NULL; }

  const T *address(void) const { return m_vec ? m_vec->address() : NULL; }

  T *begin() { return address(); }
  const T *begin() const { return address(); }
  T *end() { return begin() + length(); }
  const T *end() const { return begin() + length(); }
  const T &operator[](unsigned ix) const { return (*m_vec)[ix]; }
  const T &last(void) const { return m_vec->last(); }
...
template <typename T> class auto_delete_vec : public auto_vec<T *> {
public:
  auto_delete_vec() {}
...
template <typename T> inline auto_delete_vec<T>::~auto_delete_vec() {
  int i;
  T *item;
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::safe_grow(unsigned len,
                                               bool exact MEM_STAT_DECL) {
  unsigned oldlen = length();
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::qsort(int (*cmp)(const void *,
                                                      const void *)) {
  if (m_vec)
    m_vec->qsort(cmp);
}
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::sort(int (*cmp)(const void *, const void *,
                                                     void *),
                                          void *data) {
  if (m_vec)
    m_vec->sort(cmp, data);
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::stablesort(
...
public:
  ...
  array_slice(iterator base, unsigned int size) : m_base(base), m_size(size) {}

  template <size_t N> array_slice(T (&array)[N]) : m_base(array), m_size(N) {}

  template <typename OtherT>
  array_slice(const vec<OtherT> &v) : m_base(v.address()), m_size(v.length()) {}

  template <typename OtherT>
  array_slice(vec<OtherT> &v) : m_base(v.address()), m_size(v.length()) {}
...
template <typename T>
inline typename array_slice<T>::value_type &array_slice<T>::front() {
...
template <typename T>
inline typename array_slice<T>::value_type &array_slice<T>::back() {
  gcc_checking_assert(m_size);
  return m_base[m_size - 1];
}

template <typename T>
inline const typename array_slice<T>::value_type &array_slice<T>::back() const {
  gcc_checking_assert(m_size);
  return m_base[m_size - 1];
}
...
public:
  ...
  explicit string_slice(const char *str, size_t len) : array_slice(str, len) {}
  ...
  static string_slice invalid() { return string_slice(nullptr, ~0U); }
