...
inline bool single_succ_p(const_basic_block bb) {
  return EDGE_COUNT(bb->succs) == 1;
}

/* Returns true if BB has precisely one predecessor.  */

inline bool single_pred_p(const_basic_block bb) {
...
inline edge single_succ_edge(const_basic_block bb) {
...
inline edge single_pred_edge(const_basic_block bb) {
...
inline basic_block single_succ(const_basic_block bb) {
...
inline basic_block single_pred(const_basic_block bb) {
...
inline vec<edge, va_gc> *ei_container(edge_iterator i) {
...
inline edge_iterator ei_start_1(vec<edge, va_gc> **ev) {
  edge_iterator i;
...
/* Return an iterator pointing to the last element of an edge
   vector.  */
inline edge_iterator ei_last_1(vec<edge, va_gc> **ev) {
  edge_iterator i;
...
inline bool ei_end_p(edge_iterator i) {
  return (i.index == EDGE_COUNT(ei_container(i)));
}

/* Is the iterator `i' at one position before the end of the
   sequence?  */
inline bool ei_one_before_end_p(edge_iterator i) {
  return (i.index + 1 == EDGE_COUNT(ei_container(i)));
}

/* Advance the iterator to the next element.  */
inline void ei_next(edge_iterator *i) {
  gcc_checking_assert(i->index < EDGE_COUNT(ei_container(*i)));
  i->index++;
}

/* Move the iterator to the previous element.  */
inline void ei_prev(edge_iterator *i) {
  gcc_checking_assert(i->index > 0);
  i->index--;
}

/* Return the edge pointed to by the iterator `i'.  */
inline edge ei_edge(edge_iterator i) {
...
/* Return an edge pointed to by the iterator.  Do it safely so that
   ...
   sequence.  */
inline edge ei_safe_edge(edge_iterator i) {
...
inline bool ei_cond(edge_iterator ei, edge *p) {
  if (!ei_end_p(ei)) {
    *p = ei_edge(ei);
    return 1;
  } else {
...
inline bool bb_in_transaction(basic_block bb) {
  return bb->flags & BB_IN_TRANSACTION;
}

/* Return true when one of the predecessor edges of BB is marked with EDGE_EH.
 */
inline bool bb_has_eh_pred(basic_block bb) {
  edge e;
  edge_iterator ei;
 ...
 * EDGE_ABNORMAL.  */
inline bool bb_has_abnormal_pred(basic_block bb) {
  edge e;
  edge_iterator ei;
...
inline edge find_fallthru_edge(vec<edge, va_gc> *edges) {
  edge e;
  edge_iterator ei;
...
inline void check_probability(int prob) {
...
inline int combine_probabilities(int prob1, int prob2) {
...
inline gcov_type apply_scale(gcov_type freq, gcov_type scale) {
  return RDIV(freq * scale, REG_BR_PROB_BASE);
}

/* Apply probability PROB on frequency or count FREQ.  */

inline gcov_type apply_probability(gcov_type freq, int prob) {
...
inline int inverse_probability(int prob1) {
  check_probability(prob1);
...
inline bool has_abnormal_or_eh_outgoing_edge_p(basic_block bb) {
  edge e;
  edge_iterator ei;
...
inline bool has_abnormal_call_or_eh_pred_edge_p(basic_block bb) {
  edge e;
  edge_iterator ei;
