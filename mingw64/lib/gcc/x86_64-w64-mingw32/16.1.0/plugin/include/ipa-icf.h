...
public:
  ...
  congruence_class(unsigned int _id)
      : in_worklist(false), id(_id), referenced_by_count(0) {}
...
inline hashval_t sem_usage_pair_hash::hash(sem_usage_pair *pair) {
  inchash::hash hstate;
...
inline bool sem_usage_pair_hash::equal(sem_usage_pair *p1, sem_usage_pair *p2) {
  return p1->item == p2->item && p1->index == p2->index;
}

struct sem_usage_hash : sem_usage_pair_hash,
                        typed_delete_remove<sem_usage_pair> {};
...
public:
  ...
  /* Update hash by computed local hash values taken from different
     semantic items.  */
  void update_hash_by_local_refs(
      hash_map<symtab_node *, sem_item *> &m_symtab_node_map);
...
protected:
  ...
  void hash_referenced_symbol_properties(symtab_node *ref,
                                         inchash::hash &hstate, bool address);

  /* For a given symbol table nodes N1 and N2, we check that FUNCTION_DECLs
     point to a same function. Comparison can be skipped if IGNORED_NODES
     contains these nodes.  ADDRESS indicate if address is taken.  */
  bool
  compare_symbol_references(hash_map<symtab_node *, sem_item *> &ignored_nodes,
...
public:
  ...
  bool
  equals_wpa(sem_item *item,
             hash_map<symtab_node *, sem_item *> &ignored_nodes) final override;
  hashval_t get_hash(void) final override;
  bool
  equals(sem_item *item,
         hash_map<symtab_node *, sem_item *> &ignored_nodes) final override;
  ...
  /* Returns cgraph_node.  */
  inline cgraph_node *get_node(void) { return dyn_cast<cgraph_node *>(node); }
...
public:
  ...
  inline varpool_node *get_node(void) { return dyn_cast<varpool_node *>(node); }
