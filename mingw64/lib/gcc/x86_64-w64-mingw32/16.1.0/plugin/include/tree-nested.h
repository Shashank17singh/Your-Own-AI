...
public:
  /* Constructor.  */
  nested_function_info() : origin(NULL), nested(NULL), next_nested(NULL) {}
  /* Copy constructor.  We can not simply copy the structure,
     because the linked lists would go wrong.  However we should never
     need that.  */
  nested_function_info(const nested_function_info &) { gcc_unreachable(); }
...
inline cgraph_node *first_nested_function(cgraph_node *node) {
...
inline cgraph_node *next_nested_function(cgraph_node *node) {
  return nested_function_info::get(node)->next_nested;
}

/* Return origin of nested function (and NULL otherwise).  */
inline cgraph_node *nested_function_origin(cgraph_node *node) {
  nested_function_info *info = nested_function_info::get(node);
