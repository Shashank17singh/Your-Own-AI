...
extern tree private_lookup_attribute(const char *attr_name, size_t attr_len,
                                     tree list);
extern tree private_lookup_attribute(const char *attr_ns, const char *attr_name,
...
template <typename T> inline bool canonicalize_attr_name(const char *&s, T &l) {
...
inline tree canonicalize_attr_name(tree attr_name) {
...
inline bool cmp_attribs(const char *attr1, size_t attr1_len, const char *attr2,
                        size_t attr2_len) {
...
inline bool cmp_attribs(const char *attr1, const char *attr2) {
...
inline bool is_attribute_p(const char *attr_name, const_tree ident) {
...
inline bool is_attribute_namespace_p(const char *attr_ns, const_tree attr) {
...
inline tree lookup_attribute(const char *attr_ns, const char *attr_name,
                             tree list) {
  if (CHECKING_P && attr_name[0] != '_') {
    size_t attr_len = strlen(attr_name);
    gcc_checking_assert(!canonicalize_attr_name(attr_name, attr_len));
  ...
  if (list == NULL_TREE)
    ...
    /* Do the strlen() before calling the out-of-line implementation.
       In most cases attr_name is a string constant, and the compiler
       will optimize the strlen() away.  */
    return private_lookup_attribute(attr_ns, attr_name, attr_ns_len, attr_len,
                                    list);
  }
}
...
inline tree lookup_attribute_by_prefix(const char *attr_name, tree list) {
...
struct attr_access {
  ...
  static constexpr char mode_names[4][11] = {"none", "read_only", "write_only",
                                             "read_write"};
};

inline access_mode attr_access::from_mode_char(char c) {
  switch (c) {
  case mode_chars[access_none]:
    return access_none;
  case mode_chars[access_read_only]:
    return access_read_only;
  case mode_chars[access_write_only]:
    return access_write_only;
  case mode_chars[access_read_write]:
    return access_read_write;
  case mode_chars[access_deferred]:
