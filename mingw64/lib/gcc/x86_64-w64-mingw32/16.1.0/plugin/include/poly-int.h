...
template <typename T> struct poly_coeff_traits<T, wi::FLEXIBLE_PRECISION> {
  ...
  static const T max_value =
      (signedness
           ? ((T(1) << (precision - 2)) + ((T(1) << (precision - 2)) - 1))
           : T(-1));
  static const int rank = sizeof(T) * 2 + !signedness;

  template <typename Arg> struct init_cast {
    using type = T;
  };
};
...
template <typename T> struct poly_coeff_traits<T, wi::INL_CONST_PRECISION> {
...
template <unsigned int N, typename C> struct poly_int_traits<poly_int<N, C>> {
...
template <> struct poly_int_fullness<false> {
  using type = poly_int_hungry;
};
template <> struct poly_int_fullness<true> {
  using type = poly_int_full;
};

/* A class containing polynomial integers.  The polynomial has N coefficients
   of type C, and N - 1 indeterminates.  */
template <unsigned int N, typename C> class poly_int {
public:
  poly_int() = default;
  ...
  template <typename... Cs> constexpr poly_int(const Cs &...);

  poly_int &operator=(const poly_int &) = default;

  template <typename Ca> poly_int &operator=(const poly_int<N, Ca> &);
...
private:
  template <typename... Cs> constexpr poly_int(poly_int_full, const Cs &...);
...
inline constexpr poly_int<N, C>::poly_int(const Cs &...cs)
    : poly_int(typename poly_int_fullness<sizeof...(Cs) >= N>::type(), cs...) {}

/* Initialize with c0, cs..., and some trailing zeros.  */
template <unsigned int N, typename C>
template <typename C0, typename... Cs>
inline constexpr poly_int<N, C>::poly_int(poly_int_hungry, const C0 &c0,
                                          const Cs &...cs)
...
template <unsigned int N, typename C>
template <typename... Cs>
inline constexpr poly_int<N, C>::poly_int(poly_int_full, const Cs &...cs)
    : coeffs{(
          typename poly_coeff_traits<C>::template init_cast<Cs>::type(cs))...} {
}

template <unsigned int N, typename C>
template <typename Ca>
inline poly_int<N, C> &poly_int<N, C>::operator=(const poly_int<N, Ca> &a) {
  for (unsigned int i = 0; i < N; i++)
    POLY_SET_COEFF(C, *this, i, a.coeffs[i]);
  return *this;
}
...
template <unsigned int N, typename C>
inline bool poly_int<N, C>::is_constant() const {
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
      if (this->coeffs[i] != 0)
        return false;
...
template <unsigned int N, typename C>
inline C poly_int<N, C>::to_constant() const {
...
template <unsigned int N, typename C>
template <typename Ca>
inline poly_int<N, C> poly_int<N, C>::from(const poly_int<N, Ca> &a,
...
template <unsigned int N, typename C>
inline bool poly_int<N, C>::to_shwi(poly_int<N, HOST_WIDE_INT> *r) const {
  for (unsigned int i = 0; i < N; i++)
...
template <unsigned int N, typename C>
inline poly_int<N, HOST_WIDE_INT> poly_int<N, C>::force_shwi() const {
...
template <unsigned int N, typename C>
inline poly_int<N, unsigned HOST_WIDE_INT> poly_int<N, C>::force_uhwi() const {
...
template <unsigned int N, typename C>
inline poly_int<N, C>::operator C() const {
...
inline poly_int<N, hwi_with_prec> shwi(const poly_int<N, HOST_WIDE_INT> &a,
...
inline POLY_POLY_RESULT(N, Ca, Cb) operator+(const poly_int<N, Ca> &a,
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_CONST_RESULT(N, Ca, Cb) operator+(const poly_int<N, Ca> &a,
                                              const Cb &b) {
  typedef POLY_CAST(Ca, Cb) NCa;
...
template <unsigned int N, typename Ca, typename Cb>
inline CONST_POLY_RESULT(N, Ca, Cb) operator+(const Ca &a,
                                              const poly_int<N, Cb> &b) {
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> add(const poly_int<N, Ca> &a,
                                                 const poly_int<N, Cb> &b) {
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> add(const Ca &a,
                                                 const poly_int<N, Cb> &b) {
  typedef WI_BINARY_RESULT(Ca, Cb) C;
  poly_int<N, C> r;
...
add(const poly_int<N, Ca> &a, const poly_int<N, Cb> &b, signop sgn,
  ...
  for (unsigned int i = 1; i < N; i++) {
    ...
    POLY_SET_COEFF(C, r, i,
                   wi::add(a.coeffs[i], b.coeffs[i], sgn, &suboverflow));
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> sub(const poly_int<N, Ca> &a,
                                                 const poly_int<N, Cb> &b) {
  typedef WI_BINARY_RESULT(Ca, Cb) C;
  poly_int<N, C> r;
  for (unsigned int i = 0; i < N; i++)
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> sub(const Ca &a,
                                                 const poly_int<N, Cb> &b) {
  typedef WI_BINARY_RESULT(Ca, Cb) C;
  poly_int<N, C> r;
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)>
sub(const poly_int<N, Ca> &a, const poly_int<N, Cb> &b, signop sgn,
    wi::overflow_type *overflow) {
  typedef WI_BINARY_RESULT(Ca, Cb) C;
  poly_int<N, C> r;
  ...
  for (unsigned int i = 1; i < N; i++) {
    wi::overflow_type suboverflow;
    POLY_SET_COEFF(C, r, i,
                   wi::sub(a.coeffs[i], b.coeffs[i], sgn, &suboverflow));
    wi::accumulate_overflow(*overflow, suboverflow);
  }
  return r;
}
...
template <unsigned int N, typename Ca>
inline POLY_POLY_RESULT(N, Ca, Ca) operator-(const poly_int<N, Ca> &a) {
...
template <unsigned int N, typename Ca>
inline poly_int<N, WI_UNARY_RESULT(Ca)> neg(const poly_int<N, Ca> &a) {
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> mul(const poly_int<N, Ca> &a,
                                                 const Cb &b) {
  typedef WI_BINARY_RESULT(Ca, Cb) C;
  poly_int<N, C> r;
  for (unsigned int i = 0; i < N; i++)
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> mul(const Ca &a,
                                                 const poly_int<N, Cb> &b) {
  typedef WI_BINARY_RESULT(Ca, Cb) C;
  poly_int<N, C> r;
  for (unsigned int i = 0; i < N; i++)
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Ca)> lshift(const poly_int<N, Ca> &a,
                                                    const Cb &b) {
...
template <unsigned int N, typename C>
inline poly_int<N, HOST_WIDE_INT> sext_hwi(const poly_int<N, C> &a,
                                           unsigned int precision) {
  poly_int<N, HOST_WIDE_INT> r;
  for (unsigned int i = 0; i < N; i++)
...
template <typename Ca, typename Cb>
inline bool maybe_eq_2(const Ca &a0, const Ca &a1, const Cb &b) {
  if (a1 != 0)
    /*      a0 + a1 * x == b
      ==>             x == (b - a0) / a1
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_eq(const poly_int<N, Ca> &a, const poly_int<N, Cb> &b) {
  STATIC_ASSERT(N <= 2);
  if (N == 2)
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_eq(const poly_int<N, Ca> &a,
                                                    const Cb &b) {
  STATIC_ASSERT(N <= 2);
  if (N == 2)
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_eq(const Ca &a,
                                                    const poly_int<N, Cb> &b) {
  STATIC_ASSERT(N <= 2);
  if (N == 2)
...
template <typename Ca, typename Cb>
inline typename if_nonpoly2<Ca, Cb, bool>::type maybe_eq(const Ca &a,
                                                         const Cb &b) {
  return a == b;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_ne(const poly_int<N, Ca> &a, const poly_int<N, Cb> &b) {
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_ne(const poly_int<N, Ca> &a,
                                                    const Cb &b) {
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
      if (a.coeffs[i] != 0)
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_ne(const Ca &a,
                                                    const poly_int<N, Cb> &b) {
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
      if (b.coeffs[i] != 0)
...
template <typename Ca, typename Cb>
inline typename if_nonpoly2<Ca, Cb, bool>::type maybe_ne(const Ca &a,
                                                         const Cb &b) {
  return a != b;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_le(const poly_int<N, Ca> &a, const poly_int<N, Cb> &b) {
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
      if (a.coeffs[i] < b.coeffs[i])
        return true;
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_le(const poly_int<N, Ca> &a,
                                                    const Cb &b) {
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
      if (a.coeffs[i] < 0)
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_le(const Ca &a,
                                                    const poly_int<N, Cb> &b) {
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
      if (b.coeffs[i] > 0)
...
template <typename Ca, typename Cb>
inline typename if_nonpoly2<Ca, Cb, bool>::type maybe_le(const Ca &a,
                                                         const Cb &b) {
  return a <= b;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_lt(const poly_int<N, Ca> &a, const poly_int<N, Cb> &b) {
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
      if (a.coeffs[i] < b.coeffs[i])
        return true;
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_lt(const poly_int<N, Ca> &a,
                                                    const Cb &b) {
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
      if (a.coeffs[i] < 0)
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_lt(const Ca &a,
                                                    const poly_int<N, Cb> &b) {
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
      if (b.coeffs[i] > 0)
...
template <typename Ca, typename Cb>
inline typename if_nonpoly2<Ca, Cb, bool>::type maybe_lt(const Ca &a,
                                                         const Cb &b) {
  return a < b;
}
...
inline bool ordered_p(const T1 &a, const T2 &b) {
...
template <unsigned int N, typename Ca>
inline Ca constant_lower_bound(const poly_int<N, Ca> &a) {
...
template <unsigned int N, typename Ca>
inline POLY_BINARY_COEFF(Ca, Ca) coeff_gcd(const poly_int<N, Ca> &a) {
...
template <unsigned int N, typename Ca, typename Cb>
inline int compare_sizes_for_sort(const poly_int<N, Ca> &a,
                                  const poly_int<N, Cb> &b) {
...
template <unsigned int N, typename Ca, typename Cb>
inline bool can_align_p(const poly_int<N, Ca> &value, Cb align) {
  for (unsigned int i = 1; i < N; i++)
...
template <unsigned int N, typename Ca, typename Cb>
inline bool can_align_up(const poly_int<N, Ca> &value, Cb align,
...
template <unsigned int N, typename Ca, typename Cb>
inline bool can_align_down(const poly_int<N, Ca> &value, Cb align,
                           poly_int<N, Ca> *aligned) {
  if (!can_align_p(value, align))
    return false;
  *aligned = value - (value.coeffs[0] & (align - 1));
...
template <unsigned int N, typename Ca, typename Cb, typename Cc>
inline bool known_equal_after_align_up(const poly_int<N, Ca> &a,
                                       const poly_int<N, Cb> &b, Cc align) {
  poly_int<N, Ca> aligned_a;
  poly_int<N, Cb> aligned_b;
  return (can_align_up(a, align, &aligned_a) &&
          can_align_up(b, align, &aligned_b) && known_eq(aligned_a, aligned_b));
}
...
template <unsigned int N, typename Ca, typename Cb, typename Cc>
inline bool known_equal_after_align_down(const poly_int<N, Ca> &a,
                                         const poly_int<N, Cb> &b, Cc align) {
  poly_int<N, Ca> aligned_a;
  poly_int<N, Cb> aligned_b;
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_up(const poly_int<N, Ca> &value, Cb align) {
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_down(const poly_int<N, Ca> &value,
                                        Cb align) {
  gcc_checking_assert(can_align_p(value, align));
  return value - (value.coeffs[0] & (align - 1));
}
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> aligned_lower_bound(const poly_int<N, Ca> &value,
                                           Cb align) {
  poly_int<N, Ca> r;
  for (unsigned int i = 0; i < N; i++)
    /* This form copes correctly with more type combinations than
       value.coeffs[i] & -align would.  */
    POLY_SET_COEFF(Ca, r, i,
                   (value.coeffs[i] - (value.coeffs[i] & (align - 1))));
  return r;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> aligned_upper_bound(const poly_int<N, Ca> &value,
                                           Cb align) {
  poly_int<N, Ca> r;
  for (unsigned int i = 0; i < N; i++)
    POLY_SET_COEFF(Ca, r, i,
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_down_and_div(const poly_int<N, Ca> &value,
                                                Cb align) {
  gcc_checking_assert(can_align_p(value, align));

  poly_int<N, Ca> r;
  POLY_SET_COEFF(Ca, r, 0,
                 ((value.coeffs[0] - (value.coeffs[0] & (align - 1))) / align));
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_up_and_div(const poly_int<N, Ca> &value,
                                              Cb align) {
  gcc_checking_assert(can_align_p(value, align));

  poly_int<N, Ca> r;
  POLY_SET_COEFF(
      Ca, r, 0, ((value.coeffs[0] + (-value.coeffs[0] & (align - 1))) / align));
  if (N >= 2)
    for (unsigned int i = 1; i < N; i++)
      POLY_SET_COEFF(Ca, r, i, value.coeffs[i] / align);
  return r;
...
inline bool known_misalignment(const poly_int<N, Ca> &value, Cb align,
                               Cm *misalign) {
...
template <unsigned int N, typename Ca>
inline POLY_BINARY_COEFF(Ca, Ca) known_alignment(const poly_int<N, Ca> &a) {
  typedef POLY_BINARY_COEFF(Ca, Ca) C;
  C r = a.coeffs[0];
  for (unsigned int i = 1; i < N; ++i)
    r |= a.coeffs[i];
...
inline typename if_nonpoly<Cb, bool>::type can_and_p(const poly_int<N, Ca> &a,
                                                     Cb b, Cr *result) {
...
template <unsigned int N, typename Ca, typename Cb, typename Cr>
inline typename if_nonpoly<Cb, bool>::type can_ior_p(const poly_int<N, Ca> &a,
                                                     Cb b, Cr *result) {
  /* Coefficients 1 and above must be a multiple of something greater
     than B.  */
  typedef POLY_INT_TYPE(Ca) int_type;
...
template <unsigned int N, typename Ca, typename Cb, typename Cm>
inline bool constant_multiple_p(const poly_int<N, Ca> &a,
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type multiple_p(const poly_int<N, Ca> &a,
                                                      Cb b) {
  for (unsigned int i = 0; i < N; ++i)
    if (a.coeffs[i] % b != 0)
      return false;
...
template <unsigned int N, typename Ca, typename Cb>
inline bool multiple_p(const poly_int<N, Ca> &a, const poly_int<N, Cb> &b) {
  if (b.is_constant())
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_CONST_RESULT(N, Ca, Cb) exact_div(const poly_int<N, Ca> &a, Cb b) {
  typedef POLY_CONST_COEFF(Ca, Cb) C;
  poly_int<N, C> r;
  for (unsigned int i = 0; i < N; i++) {
...
template <unsigned int N, typename Ca, typename Cb, typename Cq>
inline typename if_nonpoly<Cb, bool>::type
can_div_trunc_p(const poly_int<N, Ca> &a, Cb b, poly_int<N, Cq> *quotient) {
...
template <unsigned int N, typename Ca, typename Cb, typename Cq>
inline bool can_div_trunc_p(const poly_int<N, Ca> &a, const poly_int<N, Cb> &b,
...
template <unsigned int N, typename C>
void print_dec(const poly_int<N, C> &value, FILE *file, signop sgn) {
...
template <unsigned int N, typename C>
void print_dec(const poly_int<N, C> &value, FILE *file) {
...
template <unsigned int N, typename C>
void print_hex(const poly_int<N, C> &value, FILE *file) {
  if (value.is_constant())
...
struct poly_span_traits {
  template <typename T> static const T &cast(const T &x) { return x; }
};
...
struct poly_span_traits<T1, T2, unsigned HOST_WIDE_INT> {
  ...
  static poly_int<N, unsigned HOST_WIDE_INT> cast(const poly_int<N, T> &x) {
    return x;
  }
};
...
inline bool ranges_maybe_overlap_p(const T1 &pos1, const T2 &size1,
...
template <typename T1, typename T2, typename T3, typename T4>
inline bool ranges_known_overlap_p(const T1 &pos1, const T2 &size1,
                                   const T3 &pos2, const T4 &size2) {
...
template <typename T1, typename T2, typename T3, typename T4>
inline bool known_subrange_p(const T1 &pos1, const T2 &size1, const T3 &pos2,
                             const T4 &size2) {
  ...
  return (known_gt(size1, POLY_INT_TYPE(T2)(0)) &&
          ...
          known_size_p(size2) && known_ge(pos1, pos2) &&
...
template <unsigned int N, typename C>
inline bool endpoint_representable_p(const poly_int<N, C> &pos,
...
template <unsigned int N, typename C> void gt_ggc_mx(poly_int<N, C> *) {}

template <unsigned int N, typename C> void gt_pch_nx(poly_int<N, C> *) {}

template <unsigned int N, typename C>
void gt_pch_nx(poly_int<N, C> *, gt_pointer_operator, void *) {}
