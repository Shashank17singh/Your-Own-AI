...
class rtx_vector_builder
    : public vector_builder<rtx, machine_mode, rtx_vector_builder> {
...
private:
  ...
  static poly_uint64 nelts_of(const_rtx x) { return CONST_VECTOR_NUNITS(x); }
...
inline rtx_vector_builder::rtx_vector_builder(machine_mode mode,
...
inline void rtx_vector_builder::new_vector(machine_mode mode,
                                           unsigned int npatterns,
                                           unsigned int nelts_per_pattern) {
  m_mode = mode;
...
inline poly_wide_int rtx_vector_builder::step(rtx elt1, rtx elt2) const {
