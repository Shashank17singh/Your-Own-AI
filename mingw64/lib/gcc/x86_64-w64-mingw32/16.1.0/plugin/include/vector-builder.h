...
template <typename T, typename Shape, typename Derived>
inline const Derived *vector_builder<T, Shape, Derived>::derived() const {
  return static_cast<const Derived *>(this);
}

template <typename T, typename Shape, typename Derived>
inline vector_builder<T, Shape, Derived>::vector_builder()
...
template <typename T, typename Shape, typename Derived>
inline unsigned int vector_builder<T, Shape, Derived>::encoded_nelts() const {
...
template <typename T, typename Shape, typename Derived>
inline bool vector_builder<T, Shape, Derived>::encoded_full_vector_p() const {
...
template <typename T, typename Shape, typename Derived>
void vector_builder<T, Shape, Derived>::new_vector(
    poly_uint64 full_nelts, unsigned int npatterns,
...
template <typename T, typename Shape, typename Derived>
bool vector_builder<T, Shape, Derived>::new_unary_operation(
    Shape shape, T vec, bool allow_stepped_p) {
...
template <typename T, typename Shape, typename Derived>
bool vector_builder<T, Shape, Derived>::new_binary_operation(
    Shape shape, T vec1, T vec2, bool allow_stepped_p) {
  poly_uint64 full_nelts = Derived::shape_nelts(shape);
  ...
  /* Conceptually we split the patterns in VEC1 and VEC2 until we have
     ...
     into two gives:
       ...
       { 1, 0, ... }
       { 0, 0, ... }.  */
  unsigned int npatterns = least_common_multiple(Derived::npatterns_of(vec1),
                                                 Derived::npatterns_of(vec2));
  unsigned int nelts_per_pattern = MAX(Derived::nelts_per_pattern_of(vec1),
...
template <typename T, typename Shape, typename Derived>
unsigned int vector_builder<T, Shape, Derived>::count_dups(int start, int end,
                                                           int step) const {
  gcc_assert((end - start) % step == 0);

  unsigned int ndups = 1;
  for (int i = start + step; i != end && derived()->equal_p(elt(i), elt(start));
       i += step)
    ndups++;
  return ndups;
}
...
template <typename T, typename Shape, typename Derived>
void vector_builder<T, Shape, Derived>::reshape(
    unsigned int npatterns, unsigned int nelts_per_pattern) {
...
template <typename T, typename Shape, typename Derived>
bool vector_builder<T, Shape, Derived>::repeating_sequence_p(
