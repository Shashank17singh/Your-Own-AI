...
template <typename Key, typename Value,
          typename Traits =
              simple_hashmap_traits<default_hash_traits<Key>, Value>>
class hash_map;
...
public:
  ...
  inline mem_location(mem_alloc_origin origin, bool ggc,
      ...
      : m_filename(filename), m_function(function), m_line(line),
        m_origin(origin), m_ggc(ggc) {}

  /* Copy constructor.  */
  inline mem_location(mem_location &other)
      : m_filename(other.m_filename), m_function(other.m_function),
        m_line(other.m_line), m_origin(other.m_origin), m_ggc(other.m_ggc) {}
...
public:
  ...
  inline void dump(mem_location *loc, const mem_usage &total) const {
    ...
    fprintf(stderr,
            "%-48s " PRsa(9) ":%5.1f%%" PRsa(9) PRsa(9) ":%5.1f%%%10s\n",
...
/* Memory usage pair that connectes memory usage and number
   of allocated bytes.  */
template <class T> class mem_usage_pair {
public:
  mem_usage_pair(T *usage_, size_t allocated_)
      : usage(usage_), allocated(allocated_) {}
...
public:
  ...
  /* Register memory allocation descriptor for container PTR.  ORIGIN identifies
     ...
     FUNCTION name.  */
  T *register_descriptor(const void *ptr, mem_alloc_origin origin, bool ggc,
...
template <class T>
inline T *mem_alloc_description<T>::register_descriptor(
    const void *ptr, mem_alloc_origin origin, bool ggc, const char *filename,
...
template <class T>
void mem_alloc_description<T>::register_object_overhead(T *usage, size_t size,
                                                        const void *ptr) {
...
template <class T>
inline T *mem_alloc_description<T>::register_overhead(
    size_t size, mem_alloc_origin origin, const char *filename, int line,
...
template <class T>
inline void mem_alloc_description<T>::release_object_overhead(void *ptr) {
...
/* Unregister a memory allocation descriptor registered with
   ...
   REMOVE_FROM_MAP = true.  */
template <class T>
inline void mem_alloc_description<T>::unregister_descriptor(void *ptr) {
  m_reverse_map->remove(ptr);
}
...
template <class T>
inline T mem_alloc_description<T>::get_sum(mem_alloc_origin origin) {
  unsigned length;
