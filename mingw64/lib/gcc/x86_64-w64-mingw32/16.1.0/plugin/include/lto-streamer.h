...
struct lto_section {
  ...
  inline void set_compression(lto_compression c) { flags = c; }

  /* Get compression from FLAGS.  */
  inline lto_compression get_compression() { return (lto_compression)flags; }
};
...
inline bool string_slot_hasher::equal(const string_slot *ds1,
...
extern class lto_input_block *
lto_create_simple_input_block(struct lto_file_decl_data *,
                              enum lto_section_type, const char **, size_t *);
extern void lto_destroy_simple_input_block(struct lto_file_decl_data *,
                                           enum lto_section_type,
                                           class lto_input_block *,
                                           const char *, size_t);
...
extern const char *lto_get_section_data(struct lto_file_decl_data *,
                                        enum lto_section_type, const char *,
                                        int, size_t *, bool decompress = false);
...
extern void lto_record_renamed_decl(struct lto_file_decl_data *, const char *,
                                    const char *);
...
extern struct lto_simple_output_block *
...
hashval_t lto_input_scc(class lto_input_block *, class data_in *, unsigned *,
                        unsigned *, bool);
tree lto_input_tree_1(class lto_input_block *, class data_in *, enum LTO_tags,
                      hashval_t hash);
...
void cl_target_option_stream_in(class data_in *, struct bitpack_d *,
                                struct cl_target_option *);

void cl_optimization_stream_out(struct output_block *, struct bitpack_d *,
                                struct cl_optimization *);

void cl_optimization_stream_in(class data_in *, struct bitpack_d *,
                               struct cl_optimization *);
...
inline enum LTO_tags lto_gimple_code_to_tag(enum gimple_code code) {
...
inline enum gimple_code lto_tag_to_gimple_code(enum LTO_tags tag) {
...
inline void lto_tag_check_range(enum LTO_tags actual, enum LTO_tags tag1,
                                enum LTO_tags tag2) {
  if (actual < tag1 || actual > tag2)
    internal_error("bytecode stream: tag %s is not in the expected range "
                   "[%s, %s]",
                   lto_tag_name(actual), lto_tag_name(tag1),
                   lto_tag_name(tag2));
}

/* Initialize an lto_out_decl_buffer ENCODER.  */
inline void lto_init_tree_ref_encoder(struct lto_tree_ref_encoder *encoder) {
...
inline void lto_destroy_tree_ref_encoder(struct lto_tree_ref_encoder *encoder) {
...
inline tree lto_tree_ref_encoder_get_tree(struct lto_tree_ref_encoder *encoder,
                                          unsigned int idx) {
  return encoder->trees[idx];
}

/* Return number of encoded nodes in ENCODER.  */
inline int lto_symtab_encoder_size(lto_symtab_encoder_t encoder) {
...
inline int lto_symtab_encoder_lookup(lto_symtab_encoder_t encoder,
                                     toplevel_node *node) {
...
inline void lsei_next(lto_symtab_encoder_iterator *lsei) { lsei->index++; }

/* Return the node pointed to by LSI.  */
inline toplevel_node *lsei_node(lto_symtab_encoder_iterator lsei) {
  return lsei.encoder->nodes[lsei.index].node;
}

/* Return the node pointed to by LSI.  */
inline struct cgraph_node *lsei_cgraph_node(lto_symtab_encoder_iterator lsei) {
  return dyn_cast<cgraph_node *>(lsei.encoder->nodes[lsei.index].node);
}

/* Return the node pointed to by LSI.  */
inline varpool_node *lsei_varpool_node(lto_symtab_encoder_iterator lsei) {
  return dyn_cast<varpool_node *>(lsei.encoder->nodes[lsei.index].node);
}

/* Return the cgraph node corresponding to REF using ENCODER.  */

inline toplevel_node *lto_symtab_encoder_deref(lto_symtab_encoder_t encoder,
                                               int ref) {
...
inline lto_symtab_encoder_iterator lsei_start(lto_symtab_encoder_t encoder) {
...
/* Advance iterator LSE.  */
inline void lsei_next_in_partition(lto_symtab_encoder_iterator *lsei) {
  lsei_next(lsei);
...
/* Advance iterator LSE.  */
inline void lsei_next_variable_in_partition(lto_symtab_encoder_iterator *lsei) {
  lsei_next(lsei);
  while (!lsei_end_p(*lsei) &&
