...
template <typename Type> struct typed_const_free_remove {
...
inline void typed_const_free_remove<Type>::remove(const Type *p) {
...
template <typename Type>
inline void typed_delete_remove<Type>::remove(Type *p) {
  delete p;
}
...
template <typename Type>
inline hashval_t int_hash_base<Type>::hash(value_type x) {
  return x;
}

template <typename Type>
inline bool int_hash_base<Type>::equal(value_type x, value_type y) {
  return x == y;
}
...
template <typename Type>
inline hashval_t pointer_hash<Type>::hash(const value_type &candidate) {
...
template <typename Type>
inline bool pointer_hash<Type>::equal(const value_type &existing,
                                      const compare_type &candidate) {
  return existing == candidate;
}

template <typename Type>
inline void pointer_hash<Type>::mark_deleted(Type *&e) {
...
template <typename T> struct ggc_remove {
  ...
  static void ggc_maybe_mx(T &p) { ggc_mx(p); }
...
template <typename T> struct ggc_cache_remove : ggc_remove<T> {
  ...
  static int keep_cache_entry(T &e) { return ggc_marked_p(e) ? -1 : 0; }
};
...
template <typename T1, typename T2> struct pair_hash {
  typedef std::pair<typename T1::value_type, typename T2::value_type>
      value_type;
  typedef std::pair<typename T1::compare_type, typename T2::compare_type>
      compare_type;
...
inline hashval_t pair_hash<T1, T2>::hash(const value_type &x) {
  return iterative_hash_hashval_t(T1::hash(x.first), T2::hash(x.second));
}

template <typename T1, typename T2>
inline bool pair_hash<T1, T2>::equal(const value_type &x,
                                     const compare_type &y) {
  return T1::equal(x.first, y.first) && T2::equal(x.second, y.second);
}

template <typename T1, typename T2>
inline void pair_hash<T1, T2>::remove(value_type &x) {
  T1::remove(x.first);
  T2::remove(x.second);
}

template <typename T1, typename T2>
inline void pair_hash<T1, T2>::mark_deleted(value_type &x) {
  T1::mark_deleted(x.first);
}

template <typename T1, typename T2>
inline void pair_hash<T1, T2>::mark_empty(value_type &x) {
  T1::mark_empty(x.first);
}

template <typename T1, typename T2>
inline bool pair_hash<T1, T2>::is_deleted(const value_type &x) {
  return T1::is_deleted(x.first);
}

template <typename T1, typename T2>
inline bool pair_hash<T1, T2>::is_empty(const value_type &x) {
...
template <typename Type> struct vec_hash_base {
...
template <typename Type>
inline hashval_t vec_hash_base<Type>::hash(value_type x) {
  inchash::hash hstate;
...
template <typename Type>
inline bool vec_hash_base<Type>::equal(value_type x, compare_type y) {
...
template <typename Type> struct vec_free_hash_base : vec_hash_base<Type> {
  static void remove(typename vec_hash_base<Type>::value_type &);
};

template <typename Type>
void vec_free_hash_base<Type>::remove(
    typename vec_hash_base<Type>::value_type &x) {
  for (auto &value : x)
    Type::remove(x);
  x.release();
}
