...
inline bool ssa_name_maybe_undef_p(tree var) {
...
inline void ssa_name_set_maybe_undef(tree var, bool value = true) {
  gcc_checking_assert(TREE_CODE(var) == SSA_NAME);
...
inline tree redirect_edge_var_map_def(edge_var_map *v) { return v->def; }

/* Given an edge_var_map V, return the PHI result.  */

inline tree redirect_edge_var_map_result(edge_var_map *v) { return v->result; }

/* Given an edge_var_map V, return the PHI arg location.  */

inline location_t redirect_edge_var_map_location(edge_var_map *v) {
  return v->locus;
}

/* Verify SSA invariants, if internal consistency checks are enabled.  */

inline void checking_verify_ssa(bool check_modified_stmt,
