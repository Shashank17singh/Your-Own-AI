...
public:
  ...
  public:
    cell_placement(rect_t rect, table_cell_content &&content, x_align x_align,
                   y_align y_align)
        : m_rect(rect), m_content(std::move(content)), m_x_align(x_align),
          m_y_align(y_align) {}
    ...
    void paint_cell_contents_to_canvas(canvas &canvas, canvas::coord_t offset,
  ...
  void set_cell(coord_t coord, table_cell_content &&content,
                enum x_align x_align = x_align::CENTER,
                enum y_align y_align = y_align::CENTER);

  void set_cell_span(rect_t span, table_cell_content &&content,
                     enum x_align x_align = x_align::CENTER,
                     enum y_align y_align = y_align::CENTER);

  void maybe_set_cell_span(rect_t span, table_cell_content &&content,
                           enum x_align x_align = x_align::CENTER,
                           enum y_align y_align = y_align::CENTER);
  ...
  void paint_to_canvas(canvas &canvas, canvas::coord_t offset,
                       const table_geometry &tg, const theme &theme) const;
...
private:
  ...
  void paint_cell_borders_to_canvas(canvas &canvas, canvas::coord_t offset,
  ...
  array2<int, size_t, coord_t>
      m_occupancy; /* indices into the m_placements vec.  */
};
...
public:
  table_cell_sizes(table_dimension_sizes &col_widths,
      ...
      : m_col_widths(col_widths), m_row_heights(row_heights) {}
