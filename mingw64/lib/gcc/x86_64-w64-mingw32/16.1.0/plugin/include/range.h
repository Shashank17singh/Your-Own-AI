...
inline int_range<1> range_true(tree type = boolean_type_node) {
...
inline int_range<1> range_false(tree type = boolean_type_node) {
  unsigned prec = TYPE_PRECISION(type);
...
inline int_range<1> range_true_and_false(tree type = boolean_type_node) {
  unsigned prec = TYPE_PRECISION(type);
  if (prec == 1)
