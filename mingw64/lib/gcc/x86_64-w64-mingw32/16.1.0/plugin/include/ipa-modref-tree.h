...
struct GTY(()) modref_access_node {
  ...
  bool useful_p() const { return parm_index != MODREF_UNKNOWN_PARM; }
  ...
  bool useful_for_kill_p() const {
    return parm_offset_known && parm_index != MODREF_UNKNOWN_PARM &&
           ...
           known_eq(max_size, size) && known_gt(size, 0);
  }
  ...
  static bool insert_kill(vec<modref_access_node> &kills, modref_access_node &a,
                          bool record_adjustments);
...
private:
  ...
  bool update_for_kills(poly_int64, poly_int64, poly_int64, poly_int64,
                        poly_int64, bool);
  ...
  void update2(poly_int64, poly_int64, poly_int64, poly_int64, poly_int64,
               poly_int64, poly_int64, bool);
  bool combined_offsets(const modref_access_node &, poly_int64 *, poly_int64 *,
                        poly_int64 *) const;
...
template <typename T> struct GTY((user)) modref_ref_node {
  T ref;
  bool every_access;
  vec<modref_access_node, va_gc> *accesses;

  modref_ref_node(T ref) : ref(ref), every_access(false), accesses(NULL) {}
...
/* Base of an access.  */
template <typename T> struct GTY((user)) modref_base_node {
  T base;
  vec<modref_ref_node<T> *, va_gc> *refs;
  bool every_ref;

  modref_base_node(T base) : base(base), refs(NULL), every_ref(false) {}
...
template <typename T> struct GTY((user)) modref_tree {
  vec<modref_base_node<T> *, va_gc> *bases;
  bool every_base;

  modref_tree() : bases(NULL), every_base(false) {}
  ...
  modref_base_node<T> *insert_base(T base, T ref, unsigned int max_bases,
                                   bool *changed = NULL) {
    ...
    base_node =
        new (ggc_alloc<modref_base_node<T>>()) modref_base_node<T>(base);
  ...
  bool insert(unsigned int max_bases, unsigned int max_refs,
              unsigned int max_accesses, T base, T ref, modref_access_node a,
              bool record_adjustments) {
    if (every_base)
      return false;
    ...
    if (a.range_info_useful_p() && known_size_p(a.size) &&
        known_size_p(a.max_size) && known_lt(a.max_size, a.size)) {
      if (dump_file)
        fprintf(dump_file, "   - Paradoxical range. Ignoring\n");
      return false;
    }
    if (known_size_p(a.size) && known_eq(a.size, 0)) {
      if (dump_file)
        fprintf(dump_file, "   - Zero size. Ignoring\n");
      return false;
    }
    if (known_size_p(a.max_size) && known_eq(a.max_size, 0)) {
      if (dump_file)
        fprintf(dump_file, "   - Zero max_size. Ignoring\n");
      return false;
    }
    gcc_checking_assert(!known_size_p(a.max_size) || !known_le(a.max_size, 0));
    ...
    if (ref_node->every_access)
      return changed;
    changed |= ref_node->insert_access(a, max_accesses, record_adjustments);
  ...
  /* Insert memory access to the tree.
     Return true if something changed.  */
  bool insert(tree fndecl, T base, T ref, const modref_access_node &a,
              bool record_adjustments) {
    return insert(opt_for_fn(fndecl, param_modref_max_bases),
                  opt_for_fn(fndecl, param_modref_max_refs),
                  opt_for_fn(fndecl, param_modref_max_accesses), base, ref, a,
                  record_adjustments);
  }
    ...
    for (i = 0; vec_safe_iterate(bases, i, &base_node);) {
      if (base_node->refs)
        for (j = 0; vec_safe_iterate(base_node->refs, j, &ref_node);) {
          if (!ref_node->every_access &&
              (!ref_node->accesses || !ref_node->accesses->length())) {
  ...
  bool merge(unsigned int max_bases, unsigned int max_refs,
             unsigned int max_accesses, modref_tree<T> *other,
             vec<modref_parm_map> *parm_map, modref_parm_map *static_chain_map,
             bool record_accesses, bool promote_unknown_to_global = false) {
    if (!other || every_base)
      return false;
    if (other->every_base) {
      collapse();
    ...
    FOR_EACH_VEC_SAFE_ELT(other->bases, i, base_node) {
      if (base_node->every_ref) {
        my_base_node = insert_base(base_node->base, 0, max_bases, &changed);
          ...
          if (ref_node->every_access) {
            ...
            FOR_EACH_VEC_SAFE_ELT(ref_node->accesses, k, access_node) {
              ...
              if (a.parm_index != MODREF_UNKNOWN_PARM &&
                  a.parm_index != MODREF_GLOBAL_MEMORY_PARM && parm_map) {
                if (a.parm_index >= (int)parm_map->length())
                  a.parm_index = MODREF_UNKNOWN_PARM;
                else {
                  modref_parm_map &m = a.parm_index == MODREF_STATIC_CHAIN_PARM
                                           ? *static_chain_map
              ...
              if (a.parm_index == MODREF_UNKNOWN_PARM &&
                ...
                a.parm_index = MODREF_GLOBAL_MEMORY_PARM;
              changed |=
                  insert(max_bases, max_refs, max_accesses, base_node->base,
                         ref_node->ref, a, record_accesses);
            }
        }
    }
    if (release)
  ...
  /* Merge OTHER into the tree.
     ...
     Similar CHAIN_MAP, if non-NULL, maps static chain of callee to caller.
     Return true if something has changed.  */
  bool merge(tree fndecl, modref_tree<T> *other, vec<modref_parm_map> *parm_map,
             modref_parm_map *static_chain_map, bool record_accesses,
             bool promote_unknown_to_global = false) {
    return merge(opt_for_fn(fndecl, param_modref_max_bases),
                 opt_for_fn(fndecl, param_modref_max_refs),
                 opt_for_fn(fndecl, param_modref_max_accesses), other, parm_map,
                 static_chain_map, record_accesses, promote_unknown_to_global);
  }
  ...
  static modref_tree<T> *create_ggc() {
    return new (ggc_alloc_no_dtor<modref_tree<T>>()) modref_tree<T>();
  }
