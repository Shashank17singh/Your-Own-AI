...
inline tree vect_scalar_ops_slice::op(unsigned int i) const {
...
inline bool mat_gather_scatter_p(vect_memory_access_type mat) {
  return (mat == VMAT_GATHER_SCATTER_LEGACY || mat == VMAT_GATHER_SCATTER_IFN ||
...
struct _slp_tree {
  ...
  template <class T> T &get_data(T &else_) {
    return data ? *static_cast<T *>(data) : else_;
  }
...
inline vect_memory_access_type SLP_TREE_MEMORY_ACCESS_TYPE(slp_tree node) {
...
struct scalar_cond_masked_key {
  scalar_cond_masked_key(tree t, unsigned ncopies_) : ncopies(ncopies_) {
...
template <> struct default_hash_traits<scalar_cond_masked_key> {
  ...
  static inline bool is_empty(value_type v) { return v.ncopies == 0; }
...
template <>
template <>
inline bool is_a_helper<_loop_vec_info *>::test(vec_info *i) {
  return i->kind == vec_info::loop;
}

template <>
template <>
inline bool is_a_helper<_bb_vec_info *>::test(vec_info *i) {
...
struct vec_loop_masks {
  ...
  hash_set<pair_hash<nofree_ptr_hash<tree_node>, int_hash<unsigned, 0>>>
      mask_set;
...
#define LOOP_VINFO_NITERS_UNCOUNTED_P(L)                                       \
  (LOOP_VINFO_NITERS(L) == chrec_dont_know)
...
#define LOOP_VINFO_EARLY_BREAKS_LIVE_IVS(L) (L)->early_break_live_ivs
...
#define LOOP_VINFO_FULLY_MASKED_P(L)                                           \
  (LOOP_VINFO_USING_PARTIAL_VECTORS_P(L) && !LOOP_VINFO_MASKS(L).is_empty())

#define LOOP_VINFO_FULLY_WITH_LENGTH_P(L)                                      \
  (LOOP_VINFO_USING_PARTIAL_VECTORS_P(L) && !LOOP_VINFO_LENS(L).is_empty())
...
#define LOOP_REQUIRES_VERSIONING_FOR_ALIAS(L)                                  \
  ((L)->comp_alias_ddrs.length() > 0 ||                                        \
   (L)->check_unequal_addrs.length() > 0 || (L)->lower_bounds.length() > 0)
...
inline loop_vec_info loop_vec_info_for_loop(class loop *loop) {
...
inline vector_costs::vector_costs(vec_info *vinfo, bool costing_for_scalar)
    : m_vinfo(vinfo), m_costing_for_scalar(costing_for_scalar), m_costs(),
      m_suggested_unroll_factor(1), m_suggested_epilogue_mode(VOIDmode),
      m_masked_epilogue(-1), m_finished(false) {}

/* Return the cost of the prologue code (in abstract units).  */

inline unsigned int vector_costs::prologue_cost() const {
...
inline unsigned int vector_costs::body_cost() const {
  gcc_checking_assert(m_finished);
...
inline unsigned int vector_costs::epilogue_cost() const {
  gcc_checking_assert(m_finished);
...
inline unsigned int vector_costs::outside_cost() const {
...
inline unsigned int vector_costs::total_cost() const {
  return body_cost() + outside_cost();
}

/* Return the suggested unroll factor.  */

inline unsigned int vector_costs::suggested_unroll_factor() const {
  gcc_checking_assert(m_finished);
...
inline machine_mode vector_costs::suggested_epilogue_mode(int &masked_p) const {
  gcc_checking_assert(m_finished);
...
inline tree vect_phi_initial_value(gphi *phi) {
...
inline bool vect_use_mask_type_p(stmt_vec_info stmt_info) {
...
inline bool is_pattern_stmt_p(stmt_vec_info stmt_info) {
...
inline stmt_vec_info get_later_stmt(stmt_vec_info stmt1_info,
  ...
  else if (dominated_by_p(CDI_DOMINATORS, gimple_bb(stmt1), gimple_bb(stmt2)))
    return stmt1_info;
  else if (dominated_by_p(CDI_DOMINATORS, gimple_bb(stmt2), gimple_bb(stmt1)))
    return stmt2_info;
  gcc_unreachable();
}
...
inline bool is_loop_header_bb_p(basic_block bb) {
...
inline int builtin_vectorization_cost(enum vect_cost_for_stmt type_of_cost,
                                      tree vectype, int misalign) {
  return targetm.vectorize.builtin_vectorization_cost(type_of_cost, vectype,
                                                      misalign);
}

/* Get cost by calling cost target builtin.  */

inline int vect_get_stmt_cost(enum vect_cost_for_stmt type_of_cost) {
  return builtin_vectorization_cost(type_of_cost, NULL, 0);
}

/* Alias targetm.vectorize.init_cost.  */

inline vector_costs *init_cost(vec_info *vinfo, bool costing_for_scalar) {
  return targetm.vectorize.create_costs(vinfo, costing_for_scalar);
}

extern void dump_stmt_cost(FILE *, int, enum vect_cost_for_stmt, stmt_vec_info,
                           slp_tree, tree, int, unsigned,
                           enum vect_cost_model_location);

/* Dump and add costs.  */

inline unsigned add_stmt_cost(vector_costs *costs, int count,
                              enum vect_cost_for_stmt kind,
                              stmt_vec_info stmt_info, slp_tree node,
                              tree vectype, int misalign,
                              enum vect_cost_model_location where) {
...
inline unsigned add_stmt_cost(vector_costs *costs, stmt_info_for_cost *i) {
  return add_stmt_cost(costs, i->count, i->kind, i->stmt_info, i->node,
                       i->vectype, i->misalign, i->where);
}

inline void add_stmt_costs(vector_costs *costs,
                           stmt_vector_for_cost *cost_vec) {
  stmt_info_for_cost *cost;
  unsigned i;
  FOR_EACH_VEC_ELT(*cost_vec, i, cost)
  add_stmt_cost(costs, cost->count, cost->kind, cost->stmt_info, cost->node,
...
inline const poly_uint64 dr_target_alignment(dr_vec_info *dr_info) {
...
inline bool dr_safe_speculative_read_required(stmt_vec_info stmt_info) {
  dr_vec_info *dr_info;
...
inline void dr_set_safe_speculative_read_required(stmt_vec_info stmt_info,
...
inline void set_dr_target_alignment(dr_vec_info *dr_info, poly_uint64 val) {
...
inline bool aligned_access_p(dr_vec_info *dr_info, tree vectype) {
...
inline bool known_alignment_for_access_p(dr_vec_info *dr_info, tree vectype) {
...
inline unsigned int vect_known_alignment_in_bytes(dr_vec_info *dr_info,
                                                  tree vectype,
                                                  poly_int64 offset = 0) {
...
inline innermost_loop_behavior *vect_dr_behavior(vec_info *vinfo,
                                                 dr_vec_info *dr_info) {
...
inline tree get_dr_vinfo_offset(vec_info *vinfo, dr_vec_info *dr_info,
...
inline enum vect_cost_model loop_cost_model(loop_p loop) {
...
inline bool unlimited_cost_model(loop_p loop) {
...
inline bool vect_use_loop_mask_for_alignment_p(loop_vec_info loop_vinfo) {
...
inline unsigned int vect_get_num_vectors(poly_uint64 nunits, tree vectype) {
...
inline unsigned int vect_get_num_copies(vec_info *vinfo, slp_tree node) {
  poly_uint64 vf;
...
inline void vect_update_max_nunits(poly_uint64 *max_nunits,
                                   poly_uint64 nunits) {
...
inline void vect_update_max_nunits(poly_uint64 *max_nunits, tree vectype) {
...
inline unsigned int vect_vf_for_cost(loop_vec_info loop_vinfo) {
...
inline unsigned int vect_nunits_for_cost(tree vec_type) {
  return estimated_poly_value(TYPE_VECTOR_SUBPARTS(vec_type));
}

/* Return the maximum possible vectorization factor for LOOP_VINFO.  */

inline unsigned HOST_WIDE_INT vect_max_vf(loop_vec_info loop_vinfo) {
...
inline bool vect_apply_runtime_profitability_check_p(loop_vec_info loop_vinfo) {
...
extern void vect_set_loop_condition(class loop *, edge, loop_vec_info, tree,
                                    tree, tree, bool);
...
class loop *slpeel_tree_duplicate_loop_to_edge_cfg(class loop *, edge,
                                                   class loop *, edge, edge,
                                                   edge *, bool = true,
                                                   vec<basic_block> * = NULL,
                                                   bool = false, bool = false);
class loop *vect_loop_versioning(loop_vec_info, gimple *);
extern class loop *vect_do_peeling(loop_vec_info, tree, tree, tree *, tree *,
                                   tree *, int, bool, bool, tree *);
...
extern bool vect_is_simple_use(vec_info *, slp_tree, unsigned, tree *,
                               slp_tree *, enum vect_def_type *, tree *,
                               stmt_vec_info * = NULL);
extern bool vect_maybe_update_slp_op_vectype(slp_tree, tree);
extern tree perm_mask_for_reverse(tree);
extern bool supportable_widening_operation(code_helper, tree, tree, bool,
                                           code_helper *, code_helper *, int *,
                                           vec<tree> *);
extern bool supportable_narrowing_operation(code_helper, tree, tree,
                                            code_helper *, int *, vec<tree> *);
extern bool
supportable_indirect_convert_operation(code_helper, tree, tree,
                                       vec<std::pair<tree, tree_code>> &,
                                       tree = NULL_TREE, slp_tree = NULL);
extern int compare_step_with_zero(vec_info *, stmt_vec_info);

extern unsigned record_stmt_cost(stmt_vector_for_cost *, int,
                                 enum vect_cost_for_stmt, stmt_vec_info, tree,
                                 int, enum vect_cost_model_location);
extern unsigned record_stmt_cost(stmt_vector_for_cost *, int,
                                 enum vect_cost_for_stmt, slp_tree, tree, int,
                                 enum vect_cost_model_location);
extern unsigned record_stmt_cost(stmt_vector_for_cost *, int,
...
inline unsigned record_stmt_cost(stmt_vector_for_cost *body_cost_vec, int count,
                                 enum vect_cost_for_stmt kind,
...
void vect_get_vec_defs(vec_info *, slp_tree, tree, vec<tree> *, tree = NULL,
                       vec<tree> * = NULL, tree = NULL, vec<tree> * = NULL,
...
extern enum dr_alignment_support vect_supportable_dr_alignment(vec_info *,
                                                               dr_vec_info *,
                                                               tree, int,
                                                               bool = false);
extern tree vect_get_smallest_scalar_type(stmt_vec_info, tree);
extern opt_result vect_analyze_data_ref_dependences(loop_vec_info,
                                                    unsigned int *);
...
extern bool vect_gather_scatter_fn_p(vec_info *, bool, bool, tree, tree, tree,
                                     int, int *, internal_fn *, tree *, tree *,
                                     vec<int> * = nullptr);
...
extern tree vect_create_data_ref_ptr(vec_info *, stmt_vec_info, tree,
                                     class loop *, tree, tree *,
                                     gimple_stmt_iterator *, gimple **, bool,
                                     tree = NULL_TREE);
...
extern internal_fn vect_load_lanes_supported(tree, unsigned HOST_WIDE_INT, bool,
                                             vec<int> * = nullptr);
extern tree vect_setup_realignment(vec_info *, stmt_vec_info, tree,
                                   gimple_stmt_iterator *, tree *,
                                   enum dr_alignment_support, tree,
                                   class loop **);
extern tree vect_get_new_vect_var(tree, enum vect_var_kind, const char *);
extern tree vect_get_new_ssa_name(tree, enum vect_var_kind,
                                  const char * = NULL);
extern tree vect_create_addr_base_for_vector_ref(vec_info *, stmt_vec_info,
                                                 gimple_seq *, tree);
...
extern void vect_gen_vector_loop_niters(loop_vec_info, tree, tree *, tree *,
                                        bool);
extern tree vect_halve_mask_nunits(tree, machine_mode);
extern tree vect_double_mask_nunits(tree, machine_mode);
extern void vect_record_loop_mask(loop_vec_info, vec_loop_masks *, unsigned int,
                                  tree, tree);
extern tree vect_get_loop_mask(loop_vec_info, gimple_stmt_iterator *,
                               vec_loop_masks *, unsigned int, tree,
                               unsigned int);
...
extern bool vectorizable_live_operation(vec_info *, stmt_vec_info, slp_tree,
                                        slp_instance, int, bool,
                                        stmt_vector_for_cost *);
extern bool vectorizable_lane_reducing(loop_vec_info, stmt_vec_info, slp_tree,
                                       stmt_vector_for_cost *);
extern bool vectorizable_reduction(loop_vec_info, stmt_vec_info, slp_tree,
                                   slp_instance, stmt_vector_for_cost *);
extern bool vectorizable_induction(loop_vec_info, stmt_vec_info, slp_tree,
                                   stmt_vector_for_cost *);
extern bool vect_transform_reduction(loop_vec_info, stmt_vec_info,
                                     gimple_stmt_iterator *, slp_tree);
extern bool vect_transform_cycle_phi(loop_vec_info, stmt_vec_info, slp_tree,
                                     slp_instance);
...
extern bool vectorizable_phi(bb_vec_info, stmt_vec_info, slp_tree,
                             stmt_vector_for_cost *);
extern bool vectorizable_recurr(loop_vec_info, stmt_vec_info, slp_tree,
                                stmt_vector_for_cost *);
...
/* Nonlinear induction.  */
extern tree vect_peel_nonlinear_iv_init(gimple_seq *, tree, tree, tree,
                                        enum vect_induction_op_type, bool);

/* In tree-vect-slp.cc.  */
extern void vect_slp_init(void);
extern void vect_slp_fini(void);
extern void vect_free_slp_instance(slp_instance);
extern bool vect_transform_slp_perm_load(vec_info *, slp_tree,
                                         const vec<tree> &,
                                         gimple_stmt_iterator *, poly_uint64,
                                         bool, unsigned *, unsigned * = nullptr,
                                         bool = false);
...
extern void vect_mark_pattern_stmts(vec_info *, stmt_vec_info, gimple *, tree);
...
typedef hash_map<slp_tree, complex_perm_kinds_t> slp_tree_to_load_perm_map_t;

/* Cache from nodes pair to being compatible or not.  */
typedef pair_hash<nofree_ptr_hash<_slp_tree>, nofree_ptr_hash<_slp_tree>>
    slp_node_hash;
...
public:
  ...
  /* Default destructor.  */
  virtual ~vect_pattern() { this->m_ops.release(); }
};
...
inline bool vect_is_store_elt_extraction(vect_cost_for_stmt kind,
                                         stmt_vec_info stmt_info) {
  return (kind == vec_to_scalar && STMT_VINFO_DATA_REF(stmt_info) &&
          DR_IS_WRITE(STMT_VINFO_DATA_REF(stmt_info)));
}

/* Return true if STMT_INFO represents part of a reduction.  */
inline bool vect_is_reduction(stmt_vec_info stmt_info) {
  return STMT_VINFO_REDUC_IDX(stmt_info) != -1;
}

/* Return true if SLP_NODE represents part of a reduction.  */
inline bool vect_is_reduction(slp_tree slp_node) {
  return SLP_TREE_REDUC_IDX(slp_node) != -1;
}

/* If STMT_INFO describes a reduction, return the vect_reduction_type
   of the reduction it describes, otherwise return -1.  */
inline int vect_reduc_type(vec_info *vinfo, slp_tree node) {
  if (loop_vec_info loop_vinfo = dyn_cast<loop_vec_info>(vinfo)) {
...
inline tree vect_embedded_comparison_type(stmt_vec_info stmt_info) {
...
/* If STMT_INFO is a comparison or contains an embedded comparison, return the
   scalar type of the values being compared.  Return null otherwise.  */
inline tree vect_comparison_type(stmt_vec_info stmt_info) {
  if (auto *assign = dyn_cast<gassign *>(stmt_info->stmt))
...
inline bool vect_is_extending_load(class vec_info *vinfo,
                                   stmt_vec_info stmt_info) {
  ...
  return (def_stmt_info && STMT_VINFO_DATA_REF(def_stmt_info) &&
          DR_IS_READ(STMT_VINFO_DATA_REF(def_stmt_info)));
}

/* Return true if STMT_INFO is an integer truncation.  */
inline bool vect_is_integer_truncation(stmt_vec_info stmt_info) {
  gassign *assign = dyn_cast<gassign *>(stmt_info->stmt);
  if (!assign || !CONVERT_EXPR_CODE_P(gimple_assign_rhs_code(assign)))
    return false;
  ...
  return (INTEGRAL_TYPE_P(lhs_type) && INTEGRAL_TYPE_P(rhs_type) &&
          TYPE_PRECISION(lhs_type) < TYPE_PRECISION(rhs_type));
}
