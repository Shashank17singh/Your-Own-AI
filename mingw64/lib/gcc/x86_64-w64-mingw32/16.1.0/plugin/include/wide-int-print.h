...
inline bool print_dec_buf_size(const wide_int_ref &wi, signop sgn,
                               unsigned int *len) {
...
inline bool print_decs_buf_size(const wide_int_ref &wi, unsigned int *len) {
  return print_dec_buf_size(wi, SIGNED, len);
}

inline bool print_decu_buf_size(const wide_int_ref &wi, unsigned int *len) {
  return print_dec_buf_size(wi, UNSIGNED, len);
}

inline bool print_hex_buf_size(const wide_int_ref &wi, unsigned int *len) {
  unsigned int l;
  if (wi::neg_p(wi))
    l = WIDE_INT_MAX_HWIS(wi.get_precision());
  else
