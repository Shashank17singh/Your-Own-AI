...
public:
  ...
  iterator_range(const T &begin, const T &end) : m_begin(begin), m_end(end) {}
...
template <typename T, typename BaseIT> class derived_iterator {
public:
  using value_type = T;
  ...
  explicit derived_iterator(Ts... args) : m_base(std::forward<Ts>(args)...) {}
...
template <typename T, typename BaseCT>
inline T const_derived_container<T, BaseCT>::operator[](unsigned int i) const {
...
template <typename T, typename StoredT = T> class wrapper_iterator {
public:
  using value_type = T;
...
template <typename T, T *(T::*Next)() const>
inline list_iterator<T, Next> &list_iterator<T, Next>::operator++() {
  this->m_contents = (this->m_contents->*Next)();
  return *this;
}

template <typename T, T *(T::*Next)() const>
inline list_iterator<T, Next> list_iterator<T, Next>::operator++(int) {
...
template <typename T> class safe_iterator {
  T m_current;
  const T m_end;
  T m_next;
...
inline iterator_range<safe_iterator<typename Container::const_iterator>>
