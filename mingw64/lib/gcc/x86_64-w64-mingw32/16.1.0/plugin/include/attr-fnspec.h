...
public:
  attr_fnspec(const char *str, unsigned len) : str(str), len(len) {
    if (flag_checking)
      verify();
  }
  attr_fnspec(const char *str) : str(str), len(strlen(str)) {
    if (flag_checking)
      verify();
  }
  ...
  attr_fnspec() : str(NULL), len(0) {}

  /* Return true if fn spec is known.  */
  bool known_p() const { return len; }

  /* Return true if arg I is specified.  */
  bool arg_specified_p(unsigned int i) const { return len >= arg_idx(i + 1); }

  /* True if the argument is not dereferenced recursively, thus only
     directly reachable memory is read or written.  */
  bool arg_direct_p(unsigned int i) const {
  ...
  bool arg_used_p(unsigned int i) const {
    unsigned int idx = arg_idx(i);
    gcc_checking_assert(arg_specified_p(i));
  ...
  bool arg_readonly_p(unsigned int i) const {
    unsigned int idx = arg_idx(i);
    gcc_checking_assert(arg_specified_p(i));
  ...
  bool arg_maybe_read_p(unsigned int i) const {
    unsigned int idx = arg_idx(i);
    gcc_checking_assert(arg_specified_p(i));
    return str[idx] != 'o' && str[idx] != 'O' && str[idx] != 'x' &&
           str[idx] != 'X';
  }

  /* True if memory reached by the argument is written.
     (directly or indirectly)  */
  bool arg_maybe_written_p(unsigned int i) const {
    unsigned int idx = arg_idx(i);
    gcc_checking_assert(arg_specified_p(i));
    return str[idx] != 'r' && str[idx] != 'R' &&
           (str[idx] < '1' || str[idx] > '9') && str[idx] != 'x' &&
           str[idx] != 'X';
  }

  /* Return true if load of memory pointed to by argument I is bound
     by another argument.  In this case set ARG.  */
  bool arg_max_access_size_given_by_arg_p(unsigned int i,
  ...
  bool arg_access_size_given_by_type_p(unsigned int i) const {
    unsigned int idx = arg_idx(i);
    gcc_checking_assert(arg_specified_p(i));
    return str[idx + 1] == 't';
  }
  ...
  bool arg_copied_to_arg_p(unsigned int i, unsigned int *arg) const {
    unsigned int idx = arg_idx(i);
    gcc_checking_assert(arg_specified_p(i));
  ...
  bool arg_noescape_p(unsigned int i) const {
    unsigned int idx = arg_idx(i);
    gcc_checking_assert(arg_specified_p(i));
    return str[idx] == 'w' || str[idx] == 'W' || str[idx] == 'r' ||
           str[idx] == 'R' || str[idx] == 'o' || str[idx] == 'O';
  }
  ...
  bool returns_arg(unsigned int *arg_no) const {
    if (str[0] >= '1' && str[0] <= '4') {
      if (arg_no)
        *arg_no = str[0] - '1';
      return true;
  ...
  bool returns_noalias_p() const { return str[0] == 'm'; }

  /* Return true if all memory read by the function is specified by fnspec.  */
  bool global_memory_read_p() const { return str[1] != 'c' && str[1] != 'C'; }

  /* Return true if all memory written by the function
     is specified by fnspec.  */
  bool global_memory_written_p() const {
    return str[1] != 'c' && str[1] != 'C' && str[1] != 'p' && str[1] != 'P';
  }

  bool errno_maybe_written_p() const { return str[1] == 'C' || str[1] == 'P'; }

  /* Return EAF flags for arg I.  */
  int arg_eaf_flags(unsigned int i) const {
    int flags = 0;
