...
extern void *
ggc_internal_alloc_no_dtor(size_t, void (*)(void *), size_t,
...
extern void *ggc_internal_cleared_alloc(size_t, void (*)(void *), size_t,
                                        size_t CXX_MEM_STAT_INFO);
extern void *
ggc_internal_cleared_alloc_no_dtor(size_t, void (*)(void *), size_t,
                                   size_t CXX_MEM_STAT_INFO) ATTRIBUTE_MALLOC;
...
template <typename T> void finalize(void *p) { static_cast<T *>(p)->~T(); }
...
template <typename T> inline T *ggc_alloc(ALONE_CXX_MEM_STAT_INFO) {
  if (need_finalization_p<T>())
    ...
    return static_cast<T *>(
        ggc_internal_alloc_no_dtor(sizeof(T), NULL, 0, 1 PASS_MEM_STAT));
}
...
template <typename T> inline T *ggc_alloc_no_dtor(ALONE_CXX_MEM_STAT_INFO) {
  return static_cast<T *>(
      ggc_internal_alloc_no_dtor(sizeof(T), NULL, 0, 1 PASS_MEM_STAT));
}
...
template <typename T> inline T *ggc_vec_alloc(size_t c CXX_MEM_STAT_INFO) {
  if (need_finalization_p<T>())
    ...
    return static_cast<T *>(
        ggc_internal_alloc_no_dtor(c * sizeof(T), NULL, 0, 0 PASS_MEM_STAT));
}
...
inline T *ggc_cleared_vec_alloc(size_t c CXX_MEM_STAT_INFO) {
  if (need_finalization_p<T>())
    ...
    return static_cast<T *>(ggc_internal_cleared_alloc_no_dtor(
        c * sizeof(T), NULL, 0, 0 PASS_MEM_STAT));
}
...
template <typename T> inline void ggc_delete(T *ptr) {
  ptr->~T();
  ggc_free(ptr);
}
