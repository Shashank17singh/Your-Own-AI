...
/* This class is used to build vectors of integer type T using the same
   ...
   details.  */
template <typename T>
class int_vector_builder
    : public vector_builder<T, poly_uint64, int_vector_builder<T>> {
...
template <typename T>
inline int_vector_builder<T>::int_vector_builder(
...
template <typename T>
inline bool int_vector_builder<T>::equal_p(T elt1, T elt2) const {
...
template <typename T>
inline T int_vector_builder<T>::step(T elt1, T elt2) const {
  return elt2 - elt1;
}
...
template <typename T>
inline T int_vector_builder<T>::apply_step(T base, unsigned int factor,
                                           T step) const {
