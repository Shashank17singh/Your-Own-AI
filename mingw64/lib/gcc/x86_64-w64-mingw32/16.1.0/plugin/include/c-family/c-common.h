...
#define CASE_RID_FLOATN_NX                                                     \
  ...
  RID_BUILTIN_OPERATOR_NEW,
  RID_BUILTIN_OPERATOR_DELETE,
...
inline bool gnu_vector_type_p(const_tree type) {
...
extern void check_function_arguments_recurse(void (*)(void *, tree,
                                             ...
                                             void *, tree,
                                             unsigned HOST_WIDE_INT, opt_code);
...
/* Return true if division or modulo op0 / op1 or op0 % op1 may be shortened.
   ...
   is UB.  */
inline bool may_shorten_divmod(tree op0, tree op1) {
  ...
  if (TREE_CODE(op0) == NOP_EXPR &&
      ...
      (TYPE_PRECISION(TREE_TYPE(TREE_OPERAND(op0, 0))) < TYPE_PRECISION(type0)))
    return true;
...
extern tree c_build_shufflevector(location_t, tree, tree, const vec<tree> &,
                                  bool = true);
...
extern vec<tree, va_gc> *append_ctor_to_tree_vector(vec<tree, va_gc> *, tree);
...
typedef hash_map<tree, tree_pair,
                 simple_hashmap_traits<tree_decl_hash, tree_pair>>
    bc_hash_map_t;
...
extern tree c_genericize_control_stmt(tree *, int *, void *, walk_tree_fn,
                                      walk_tree_lh);
...
extern diagnostics::option_id
get_option_for_builtin_define(const char *macro_name);
...
public:
  c_omp_address_inspector(location_t loc, tree t)
      : loc(loc), root_term(NULL_TREE), indirections(false), map_supported(-1),
        orig(t) {}
  ...
  virtual tree convert_from_reference(tree) { gcc_unreachable(); }

  virtual tree build_array_ref(location_t loc, tree arr, tree idx) {
    tree eltype = TREE_TYPE(TREE_TYPE(arr));
    return build4_loc(loc, ARRAY_REF, eltype, arr, idx, NULL_TREE, NULL_TREE);
  }
...
c_omp_categorize_directive(const char *, const char *, const char *);
extern tree c_omp_expand_variant_construct(vec<struct omp_variant> &);

/* Return next tree in the chain for chain_next walking of tree nodes.  */
inline tree c_tree_chain_next(tree t) {
...
static inline tree c_hardbool_type_attr(tree type, tree *false_value = NULL,
                                        tree *true_value = NULL) {
  if (TREE_CODE(type) != ENUMERAL_TYPE || TYPE_LANG_SPECIFIC(type))
    return NULL_TREE;
...
/* A suffix-identifier value doublet that represents user-defined literals
   for C++-0x.  */
enum overflow_type { OT_UNDERFLOW = -1, OT_NONE, OT_OVERFLOW };
...
extern void warn_for_calloc(location_t *, tree, vec<tree, va_gc> *, tree *,
                            tree);
...
extern void maybe_emit_indirection_note(location_t loc, tree expr,
                                        tree expected_type);
