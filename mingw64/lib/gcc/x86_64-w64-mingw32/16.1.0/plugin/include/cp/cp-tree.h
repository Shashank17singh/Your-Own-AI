...
class cp_expr {
public:
  cp_expr() : m_value(NULL), m_loc(UNKNOWN_LOCATION), m_decimal(false) {}

  cp_expr(tree value)
      : m_value(value), m_loc(cp_expr_location(m_value)), m_decimal(false) {}
...
#define builtin_contract_violation_type                                        \
  cp_global_trees[CPTI_CONTRACT_VIOLATION_TYPE]
...
inline tree template_info_decl_check(const_tree t, const char *f, int l,
                                     const char *fn) {
  switch (TREE_CODE(t)) {
  case VAR_DECL:
  case FUNCTION_DECL:
  ...
  tree_check_failed(t, f, l, fn, VAR_DECL, FUNCTION_DECL, FIELD_DECL, TYPE_DECL,
...
#define DECL_MAIN_FREESTANDING_P(NODE)                                         \
  (DECL_MAIN_ANY_P(NODE) &&                                                    \
   (flag_hosted ||                                                             \
    TYPE_MAIN_VARIANT(TREE_TYPE(TREE_TYPE(NODE))) == integer_type_node))
...
#define OVL_NAME_INDEPENDENT_DECL_P(NODE) TREE_LANG_FLAG_6(OVERLOAD_CHECK(NODE))
...
public:
  explicit raw_data_iterator(tree t, unsigned int n) : t(t), n(n) {}

  operator bool() const { return n < (unsigned)RAW_DATA_LENGTH(t); }
...
public:
  ...
  raw_data_iterator end() { return raw_data_iterator(t, RAW_DATA_LENGTH(t)); }
};
...
inline bool vec_safe_reserve(releasing_vec &r, unsigned n,
                             bool e = false CXX_MEM_STAT_INFO) {
  return vec_safe_reserve(*&r, n, e PASS_MEM_STAT);
}
inline unsigned vec_safe_length(releasing_vec &r) { return r->length(); }
inline void vec_safe_splice(releasing_vec &r,
                            vec<tree, va_gc> *p CXX_MEM_STAT_INFO) {
...
ATTRIBUTE_PURE inline cp_identifier_kind get_identifier_kind(tree id) {
...
#define IDENTIFIER_KEYWORD_P(NODE) (get_identifier_kind(NODE) == cik_keyword)
...
#define IDENTIFIER_CTOR_P(NODE) (get_identifier_kind(NODE) == cik_ctor)

/* True if this identifier is the name of a destructor.  Value 3.  */
#define IDENTIFIER_DTOR_P(NODE) (get_identifier_kind(NODE) == cik_dtor)
...
/* True if this identifier is the name of a type-conversion
   operator.  Value 6.  */
#define IDENTIFIER_CONV_OP_P(NODE) (get_identifier_kind(NODE) == cik_conv_op)

/* True if this identifier is the name of a built-in trait.  */
#define IDENTIFIER_TRAIT_P(NODE) (get_identifier_kind(NODE) == cik_trait)
...
#define CONSTEVAL_BLOCK_P(NODE) TREE_LANG_FLAG_0(STATIC_ASSERT_CHECK(NODE))
...
#define DEFTRAIT(TCC, CODE, NAME, ARITY) CPTK_##CODE,
...
#define TRAIT_TYPE_KIND_RAW(NODE) TYPE_VALUES_RAW(TRAIT_TYPE_CHECK(NODE))
...
#define TRAIT_TYPE_TYPE1(NODE) TYPE_MIN_VALUE_RAW(TRAIT_TYPE_CHECK(NODE))

/* The rest of the arguments of the trait type NODE.  */
#define TRAIT_TYPE_TYPE2(NODE) TYPE_MAX_VALUE_RAW(TRAIT_TYPE_CHECK(NODE))
...
/* True if we should add "const" when figuring out the type of an entity
   ...
   present.  */
#define LAMBDA_EXPR_CONST_QUAL_P(NODE) TREE_LANG_FLAG_1(LAMBDA_EXPR_CHECK(NODE))
...
#define LAMBDA_EXPR_STATIC_P(NODE) TREE_LANG_FLAG_3(LAMBDA_EXPR_CHECK(NODE))
...
#define ATOMIC_CONSTR_MAP(NODE) TREE_OPERAND(TREE_CHECK(NODE, ATOMIC_CONSTR), 0)
...
#define SPLICE_EXPR_EXPRESSION_P(NODE) TREE_LANG_FLAG_0(SPLICE_EXPR_CHECK(NODE))
...
#define SET_SPLICE_EXPR_EXPRESSION_P(NODE)                                     \
  (SPLICE_EXPR_EXPRESSION_P(                                                   \
       TREE_CODE(NODE) == SPLICE_EXPR ? NODE : TREE_OPERAND(NODE, 0)) = true)
...
#define SET_SPLICE_EXPR_MEMBER_ACCESS_P(NODE, VAL)                             \
  (SPLICE_EXPR_MEMBER_ACCESS_P(                                                \
       TREE_CODE(NODE) == SPLICE_EXPR ? NODE : TREE_OPERAND(NODE, 0)) = (VAL))

/* True if we are taking the address of this SPLICE_EXPR.  */
#define SPLICE_EXPR_ADDRESS_P(NODE) TREE_LANG_FLAG_2(SPLICE_EXPR_CHECK(NODE))
...
#define SPLICE_EXPR_TEMPLATE_P(NODE) TREE_LANG_FLAG_3(SPLICE_EXPR_CHECK(NODE))
...
#define SPLICE_EXPR_TARGS_P(NODE) TREE_LANG_FLAG_4(SPLICE_EXPR_CHECK(NODE))
...
#define SPLICE_SCOPE_EXPR(NODE) (TYPE_VALUES_RAW(SPLICE_SCOPE_CHECK(NODE)))
...
union GTY(
  ...
  struct tree_tu_local_entity
      GTY((tag("TS_CP_TU_LOCAL_ENTITY"))) tu_local_entity;
};
...
public:
  ...
  ~processing_template_decl_sentinel() { processing_template_decl = saved; }
};
...
template <typename T> inline temp_override<T> make_temp_override(T &var) {
  return {var};
}
...
template <typename T>
inline temp_override<T> make_temp_override(T &var,
                                           type_identity_t<T> overrider) {
...
struct in_consteval_if_p_temp_override {
  ...
  ~in_consteval_if_p_temp_override() { reset(); }
};
...
#define WILDCARD_TYPE_P(T)                                                     \
  (TREE_CODE(T) == TEMPLATE_TYPE_PARM || TREE_CODE(T) == TYPENAME_TYPE ||      \
   ...
   TREE_CODE(T) == UNBOUND_CLASS_TEMPLATE || TREE_CODE(T) == DECLTYPE_TYPE ||  \
   TREE_CODE(T) == TRAIT_TYPE || TREE_CODE(T) == DEPENDENT_OPERATOR_TYPE ||    \
   TREE_CODE(T) == SPLICE_SCOPE || TREE_CODE(T) == PACK_INDEX_TYPE)
...
#define UNION_TYPE_P(T) (TREE_CODE(T) == UNION_TYPE && TYPE_LANG_FLAG_5(T))
...
#define CLASSTYPE_KEY_METHOD(NODE)                                             \
  (TYPE_POLYMORPHIC_P(NODE) ? LANG_TYPE_CLASS_CHECK(NODE)->key_method          \
                            : NULL_TREE)
...
#define IS_FAKE_BASE_TYPE(NODE)                                                \
  (RECORD_OR_UNION_TYPE_P(NODE) && TYPE_CONTEXT(NODE) &&                       \
...
enum lang_decl_selector { lds_min, lds_fn, lds_ns, lds_parm, lds_decomp };
...
#define CONTRACT_HELPER(NODE) (LANG_DECL_FN_CHECK(NODE)->contract_helper)
...
#define DECL_DISCRIMINATOR_P(NODE)                                             \
  (((VAR_P(NODE) && TREE_STATIC(NODE)) || DECL_IMPLICIT_TYPEDEF_P(NODE)) &&    \
...
#define DECL_FUNCTION_XOBJ_FLAG(NODE) (LANG_DECL_FN_CHECK(NODE)->xobj_func)
...
/* Nonzero for FUNCTION_DECL means that this decl is a contract
   wrapper function.  */
#define DECL_CONTRACT_WRAPPER(NODE) LANG_DECL_FN_CHECK(NODE)->contract_wrapper
...
#define DECL_NONSTATIC_MEMBER_P(NODE)                                          \
  (DECL_OBJECT_MEMBER_FUNCTION_P(NODE) || TREE_CODE(NODE) == FIELD_DECL)
...
#define DECL_DECLARED_CONSTINIT_P(NODE) (DECL_LANG_FLAG_7(VAR_DECL_CHECK(NODE)))
...
#define DECL_NTTP_OBJECT_P(NODE) TREE_LANG_FLAG_5(TREE_CHECK(NODE, VAR_DECL))
...
#define TYPE_DECL_WAS_UNNAMED(NODE)                                            \
  (TREE_CODE(NODE) == TYPE_DECL && TYPE_DECL_FOR_LINKAGE_PURPOSES_P(NODE) &&   \
...
#define TEMPLATE_ARGS_TYPE_CONSTRAINT_P(NODE) TREE_PRIVATE(TREE_VEC_CHECK(NODE))
...
#define PACK_INDEX_P(NODE)                                                     \
  (TREE_CODE(NODE) == PACK_INDEX_TYPE || TREE_CODE(NODE) == PACK_INDEX_EXPR)
...
#define PACK_EXPANSION_AUTO_P(NODE) TREE_LANG_FLAG_2(PACK_EXPANSION_CHECK(NODE))
...
/* In a RETURN_EXPR, whether the expression refers to the address
   of a local variable.  */
#define RETURN_EXPR_LOCAL_ADDR_P(NODE) TREE_LANG_FLAG_0(RETURN_EXPR_CHECK(NODE))
...
#define INIT_EXPR_NRV_P(NODE) TREE_LANG_FLAG_0(INIT_EXPR_CHECK(NODE))
...
inline tree templated_operator_saved_lookups(tree t) {
...
#define REF_PARENTHESIZED_P(NODE)                                              \
  TREE_LANG_FLAG_2(TREE_CHECK5((NODE), COMPONENT_REF, INDIRECT_REF, SCOPE_REF, \
                               VIEW_CONVERT_EXPR, PAREN_EXPR))

/* True on a dependent COMPONENT_REF with a splice expression.  */

#define COMPONENT_REF_SPLICE_P(NODE) TREE_LANG_FLAG_0(COMPONENT_REF_CHECK(NODE))
...
inline void
init_aggr_init_expr_arg_iterator(tree exp, aggr_init_expr_arg_iterator *iter) {
  iter->t = exp;
...
inline tree &get_nth_callarg(tree t, int n) {
  switch (TREE_CODE(t)) {
  case CALL_EXPR:
...
inline tree get_vec_init_expr(tree t) {
...
#define TYPENAME_IS_ENUM_P(NODE) (get_typename_tag(NODE) == enum_type)
...
#define TYPENAME_IS_UNION_P(NODE) (get_typename_tag(NODE) == union_type)
...
/* [class.virtual]
   ...
   polymorphic class.  */
#define TYPE_POLYMORPHIC_P(NODE) (TREE_LANG_FLAG_2(RECORD_OR_UNION_CHECK(NODE)))
...
#define ARITHMETIC_TYPE_P(TYPE)                                                \
  (CP_INTEGRAL_TYPE_P(TYPE) || SCALAR_FLOAT_TYPE_P(TYPE) ||                    \
...
#define SCALAR_TYPE_P(TYPE)                                                    \
  (TYPE_PTRDATAMEM_P(TYPE) || TREE_CODE(TYPE) == ENUMERAL_TYPE ||              \
   ...
   NULLPTR_TYPE_P(TYPE) || REFLECTION_TYPE_P(TYPE))
...
/* Nonzero when the ENUMERAL_TYPE is being defined (enumerators parsed
   or instantiated).  */
#define ENUM_BEING_DEFINED_P(NODE) (TYPE_LANG_FLAG_2(ENUMERAL_TYPE_CHECK(NODE)))
...
#define OLD_PARM_DECL_P(NODE) TREE_LANG_FLAG_0(PARM_DECL_CHECK(NODE))
...
#define MULTIPLE_NAMES_PARM_P(NODE) TREE_LANG_FLAG_2(PARM_DECL_CHECK(NODE))
...
#define DECL_CONTRACT_CAPTURE_P(NODE) DECL_LANG_FLAG_8(FIELD_DECL_CHECK(NODE))
...
#define TYPE_WAS_UNNAMED(NODE)                                                 \
  (TYPE_NAME(TYPE_MAIN_VARIANT(NODE)) !=                                       \
   ...
   TYPE_DECL_WAS_UNNAMED(TYPE_NAME(TYPE_MAIN_VARIANT(NODE))))
...
#define DECL_TEMPLATE_PARMS(NODE)                                              \
  ((struct tree_template_decl *)const_cast<tree>(TEMPLATE_DECL_CHECK(NODE)))   \
      ->arguments
...
inline tree decl_template_parm_check(const_tree t, const char *f, int l,
                                     const char *fn) {
...
#define DECL_RAMP_P(NODE) DECL_COROUTINE_P(NODE) && !DECL_RAMP_FN(NODE)
...
#define OMP_FOR_GIMPLIFYING_P(NODE) (TREE_LANG_FLAG_0(OMP_LOOPING_CHECK(NODE)))
...
/* TEMPLATE_FOR_STMT accessors.  These give access to the declarator,
   expression, body, and scope of the statement, respectively.  */
#define TEMPLATE_FOR_DECL(NODE) TREE_OPERAND(TEMPLATE_FOR_STMT_CHECK(NODE), 0)
#define TEMPLATE_FOR_EXPR(NODE) TREE_OPERAND(TEMPLATE_FOR_STMT_CHECK(NODE), 1)
#define TEMPLATE_FOR_BODY(NODE) TREE_OPERAND(TEMPLATE_FOR_STMT_CHECK(NODE), 2)
#define TEMPLATE_FOR_SCOPE(NODE) TREE_OPERAND(TEMPLATE_FOR_STMT_CHECK(NODE), 3)
...
#define TARGET_EXPR_ELIDING_P(NODE) TREE_LANG_FLAG_3(TARGET_EXPR_CHECK(NODE))
...
#define TARGET_EXPR_INTERNAL_P(NODE) TREE_LANG_FLAG_4(TARGET_EXPR_CHECK(NODE))
...
/* True if T is a TARGET_EXPR for which we'll need to replace_decl to use it as
   an initializer.  */
inline bool target_expr_needs_replace(tree t) {
  if (!t || TREE_CODE(t) != TARGET_EXPR)
    return false;
...
inline tag_types get_typename_tag(tree t) {
...
inline void set_typename_tag(tree t, tag_types tag) {
...
enum duration_kind { dk_static, dk_thread, dk_auto, dk_dynamic };
...
enum unification_kind_t { DEDUCE_CALL, DEDUCE_CONV, DEDUCE_EXACT };
...
/* True if this TREE_LIST in {static,tls}_aggregates is a for dynamic
   ...
   it.  */
#define STATIC_INIT_DECOMP_BASE_P(NODE) TREE_LANG_FLAG_1(TREE_LIST_CHECK(NODE))
...
#define TEMPLATE_PARM_DESCENDANTS(NODE)                                        \
  (TREE_CHAIN(TEMPLATE_PARM_INDEX_CHECK(NODE)))
...
private:
  ...
  bool not_list_p() const { return !split_list_p() && !tree_list_p(); }
...
extern tree build_new_method_call(tree, tree, vec<tree, va_gc> **, tree, int,
                                  tree *, tsubst_flags_t);
extern tree build_special_member_call(tree, tree, vec<tree, va_gc> **, tree,
                                      int, tsubst_flags_t);
...
inline tree build_new_op(const op_location_t &loc, enum tree_code code,
  ...
  return build_new_op(loc, code, flags, arg1, arg2, NULL_TREE, NULL_TREE, NULL,
                      complain);
}
extern tree keep_unused_object_arg(tree, tree, tree);
extern tree build_op_call(tree, vec<tree, va_gc> **, tsubst_flags_t);
extern tree build_op_subscript(const op_location_t &, tree, vec<tree, va_gc> **,
                               tree *, tsubst_flags_t);
...
extern tree build_op_delete_call(enum tree_code, tree, tree, bool, tree, tree,
                                 tsubst_flags_t);
extern tree build_coroutine_op_delete_call(enum tree_code, tree, tree, bool,
                                           tree, tree, tsubst_flags_t complain);
...
extern void
maybe_inform_about_fndecl_for_bogus_argument_init(tree, int,
                                                  const char * = nullptr);
...
extern tree build_cxx_call(tree, int, tree *, tsubst_flags_t, tree = NULL_TREE);
...
extern bool
    cp_handle_deprecated_or_unavailable(tree,
...
extern tree duplicate_decls(tree, tree, bool hiding = false,
...
extern void pop_abi_namespace(unsigned flags, tree node = abi_node);
...
extern tree start_decl(const cp_declarator *, cp_decl_specifier_seq *, int,
                       tree, tree, tree *);
...
extern tree xref_tag(tag_types, tree, TAG_how = TAG_how::CURRENT_ONLY,
...
extern bool start_function(cp_decl_specifier_seq *, const cp_declarator *,
                           tree);
...
extern tree grok_array_decl(location_t, tree, tree, vec<tree, va_gc> **,
                            tsubst_flags_t);
extern tree grok_omp_array_section(location_t, tree, tree, tree);
extern tree delete_sanity(location_t, tree, tree, bool, int, tsubst_flags_t);
...
extern void maybe_warn_cpp0x(cpp0x_warn_str str, location_t = input_location);
extern bool pedwarn_cxx98(location_t, diagnostics::option_id option_id,
...
extern tree build_throw(location_t, tree, tsubst_flags_t);
...
extern tree do_friend(tree, tree, tree, enum overload_flags, bool);
...
extern tree build_new(location_t, vec<tree, va_gc> **, tree, tree,
                      vec<tree, va_gc> **, int, tsubst_flags_t);
extern tree get_temp_regvar(tree, tree);
extern tree build_vec_init(tree, tree, tree, bool, int, tsubst_flags_t,
                           vec<tree, va_gc> ** = nullptr);
extern tree build_delete(location_t, tree, tree, special_function_kind, int,
                         int, tsubst_flags_t);
extern void push_base_cleanups(void);
extern tree build_vec_delete(location_t, tree, tree, special_function_kind, int,
                             tsubst_flags_t);
...
extern bool is_trivially_xible(enum tree_code, tree, tree, bool = false);
extern bool is_nothrow_xible(enum tree_code, tree, tree, bool = false);
extern bool is_xible(enum tree_code, tree, tree, bool = false);
...
extern void lazily_declare_fn(special_function_kind, tree);
...
extern tree build_invoke(tree, const_tree, tsubst_flags_t);
...
inline bool decl_specialization_friend_p(tree decl) {
  return (TREE_CODE(decl) == FUNCTION_DECL && DECL_UNIQUE_FRIEND_P(decl) &&
...
inline bool module_p() { return module_kind & (MK_NAMED | MK_HEADER); }
inline bool named_module_p() { return module_kind & MK_NAMED; }
inline bool header_module_p() { return module_kind & MK_HEADER; }
inline bool module_interface_p() { return module_kind & MK_INTERFACE; }
inline bool module_partition_p() { return module_kind & MK_PARTITION; }
inline bool module_has_cmi_p() {
  return module_kind & (MK_INTERFACE | MK_PARTITION | MK_HEADER);
}

inline bool module_purview_p() { return module_kind & MK_PURVIEW; }
inline bool module_attach_p() { return module_kind & MK_ATTACH; }
...
inline bool module_exporting_p() { return module_kind & MK_EXPORTING; }
...
extern module_state *preprocess_module(module_state *, location_t,
                                       bool in_purview, bool is_import,
                                       bool export_p, cpp_reader *reader);
extern void preprocessed_module(cpp_reader *reader);
extern void import_module(module_state *, location_t, bool export_p, tree attr,
                          cpp_reader *);
extern void declare_module(module_state *, location_t, bool export_p, tree attr,
                           cpp_reader *);
...
extern tree cp_convert_range_for(tree, tree, tree, cp_decomp *, bool, tree,
                                 bool);
extern tree build_range_temp(tree, tree = NULL_TREE);
extern tree cp_perform_range_for_lookup(tree, tree *, tree *,
                                        tsubst_flags_t = tf_warning_or_error);
extern void cp_convert_omp_range_for(tree &, tree &, tree &, tree &, tree &,
                                     tree &, tree &, tree &, bool);
...
extern tree lookup_template_class(tree, tree, tree, tree, tsubst_flags_t);
...
extern bool comp_template_args(tree, tree, tree * = NULL, tree * = NULL);
...
extern void print_candidates(location_t, tree, candidate_context * = nullptr);
...
extern void walk_specializations(bool, void (*)(bool, spec_entry *, void *),
                                 void *);
...
extern unsigned get_mergeable_specialization_flags(bool is_decl, tree tmpl,
                                                   tree spec);
extern void add_mergeable_specialization(bool is_decl, spec_entry *, tree outer,
                                         unsigned);
...
extern tree build_dynamic_cast(location_t, tree, tree, tsubst_flags_t);
...
public:
  ...
  ~deferring_access_check_sentinel() { pop_deferring_access_checks(); }
};
...
extern tree finish_asm_stmt(location_t, int, tree, tree, tree, tree, tree, bool,
                            bool);
...
extern tree pack_index_element(tree, tree, bool, tsubst_flags_t);
...
extern void finish_static_assert(tree, tree, location_t, bool, bool,
                                 bool = false);
extern tree finish_decltype_type(tree, bool, tsubst_flags_t);
extern tree fold_builtin_is_corresponding_member(location_t, int, tree *);
extern bool pointer_interconvertible_base_of_p(tree, tree, bool = false);
extern tree fold_builtin_is_pointer_inverconvertible_with_class(location_t, int,
                                                                tree *);
...
extern tree cp_build_bit_cast(location_t, tree, tree, tsubst_flags_t);
...
extern tree strip_typedefs(tree, bool * = NULL, unsigned int = 0);
...
extern tree build_min_non_dep_op_overload(tree, tree, tree, vec<tree, va_gc> *);
...
extern tree get_target_expr(tree, tsubst_flags_t = tf_warning_or_error);
...
extern tree ovl_insert(tree fn, tree maybe_ovl, int using_or_hidden = 0);
...
extern tree cp_walk_subtrees(tree *, int *, walk_tree_fn, void *,
                             hash_set<tree> *);
...
extern void cxx_print_error_function(diagnostics::text_sink &, const char *,
...
extern tree require_complete_type(tree, tsubst_flags_t = tf_warning_or_error);
...
extern tree cxx_sizeof_or_alignof_expr(location_t, tree, enum tree_code, bool,
                                       bool);
extern tree cxx_sizeof_or_alignof_type(location_t, tree, enum tree_code, bool,
                                       bool);
...
extern tree build_x_indirect_ref(location_t, tree, ref_operator, tree,
                                 tsubst_flags_t);
...
extern tree cp_build_function_call_vec(tree, vec<tree, va_gc> **,
                                       tsubst_flags_t, tree = NULL_TREE);
extern tree build_x_binary_op(const op_location_t &, enum tree_code, tree,
                              enum tree_code, tree, enum tree_code, tree,
                              tree *, tsubst_flags_t);
inline tree build_x_binary_op(const op_location_t &loc, enum tree_code code,
                              tree arg1, tree arg2, tsubst_flags_t complain) {
...
extern tree build_x_unary_op(location_t, enum tree_code, cp_expr, tree,
                             tsubst_flags_t);
...
extern tree build_x_compound_expr_from_vec(vec<tree, va_gc> *, const char *,
                                           tsubst_flags_t);
extern tree build_x_compound_expr(location_t, tree, tree, tree, tsubst_flags_t);
extern tree build_compound_expr(location_t, tree, tree);
extern tree cp_build_compound_expr(tree, tree, tsubst_flags_t);
extern tree build_static_cast(location_t, tree, tree, tsubst_flags_t);
extern tree build_reinterpret_cast(location_t, tree, tree, tsubst_flags_t);
extern tree build_const_cast(location_t, tree, tree, tsubst_flags_t);
extern tree build_c_cast(location_t, tree, tree);
extern cp_expr build_c_cast(location_t loc, tree type, cp_expr expr);
extern tree cp_build_c_cast(location_t, tree, tree, tsubst_flags_t);
extern bool maybe_warn_self_move(location_t, tree, tree);
extern cp_expr build_x_modify_expr(location_t, tree, enum tree_code, tree, tree,
                                   tsubst_flags_t);
...
extern tree composite_pointer_type(const op_location_t &, tree, tree, tree,
                                   tree, composite_pointer_operation,
                                   tsubst_flags_t);
...
extern tree cp_build_binary_op(const op_location_t &, enum tree_code, tree,
                               tree, tsubst_flags_t);
extern tree build_x_vec_perm_expr(location_t, tree, tree, tree, tsubst_flags_t);
extern tree build_x_shufflevector(location_t, vec<tree, va_gc> *,
                                  tsubst_flags_t);
#define cxx_sizeof(T)                                                          \
  cxx_sizeof_or_alignof_type(input_location, T, SIZEOF_EXPR, false, true)
...
extern bool cxx_incomplete_type_diagnostic(location_t, const_tree, const_tree,
                                           enum diagnostics::kind);
inline location_t loc_or_input_loc(location_t loc) {
...
inline location_t cp_expr_location(const_tree t_) {
...
inline location_t cp_expr_loc_or_input_loc(const_tree t) {
  return cp_expr_loc_or_loc(t, input_location);
}

inline bool cxx_incomplete_type_diagnostic(const_tree value, const_tree type,
                                           enum diagnostics::kind diag_kind) {
  return cxx_incomplete_type_diagnostic(cp_expr_loc_or_input_loc(value), value,
                                        type, diag_kind);
}
...
extern tree build_functional_cast(location_t, tree, tree, tsubst_flags_t);
...
extern tree cp_fold_immediate(tree *, mce_value, tree = current_function_decl);
...
enum cp_oracle_request { CP_ORACLE_IDENTIFIER };
...
enum value_cat { vc_prvalue = 0, vc_glvalue = 1, vc_discard = 2 };

extern tree cxx_eval_constant_expression(const constexpr_ctx *, tree, value_cat,
                                         bool *, bool *, tree *);
...
extern tree process_metafunction(const constexpr_ctx *, tree, tree, bool *,
                                 bool *, tree *);
...
extern bool valid_splice_for_member_access_p(const_tree,
                                             bool = true) ATTRIBUTE_PURE;
...
inline hashval_t named_decl_hash::hash(const value_type decl) {
  tree name = (TREE_CODE(decl) == BINDING_VECTOR ? BINDING_VECTOR_NAME(decl)
                                                 : OVL_NAME(decl));
...
inline bool named_decl_hash::equal(const value_type existing,
  ...
  tree name =
      (TREE_CODE(existing) == BINDING_VECTOR ? BINDING_VECTOR_NAME(existing)
                                             : OVL_NAME(existing));
...
inline tree concept_prototype_parameter(const_tree t) {
...
inline bool implicit_rvalue_p(const_tree t) {
  if (REFERENCE_REF_P(t))
    t = TREE_OPERAND(t, 0);
  return (
      (TREE_CODE(t) == NON_LVALUE_EXPR || TREE_CODE(t) == STATIC_CAST_EXPR) &&
      IMPLICIT_RVALUE_P(t));
}
inline tree set_implicit_rvalue_p(tree ot) {
  tree t = ot;
  if (REFERENCE_REF_P(t))
    t = TREE_OPERAND(t, 0);
...
inline bool is_constrained_auto(const_tree t) {
...
inline bool unevaluated_p(tree_code code) {
  return (code == DECLTYPE_TYPE || code == ALIGNOF_EXPR ||
          code == SIZEOF_EXPR || code == NOEXCEPT_EXPR ||
          code == REQUIRES_EXPR || code == REFLECT_EXPR);
}
...
struct push_access_scope_guard {
  tree decl;
  push_access_scope_guard(tree t) : decl(t) {
    if (VAR_OR_FUNCTION_DECL_P(decl) || TREE_CODE(decl) == TYPE_DECL)
...
public:
  ...
  bool extract(location_t location, const char *&msg, int &len,
               const constexpr_ctx * = NULL, bool * = NULL, bool * = NULL,
               tree * = NULL);
...
inline bool extended_float_type_p(tree type) {
...
inline bool name_independent_decl_p(tree decl) {
  return ((VAR_P(decl) || TREE_CODE(decl) == FIELD_DECL) && DECL_NAME(decl) &&
          id_equal(DECL_NAME(decl), "_") && !TREE_STATIC(decl) &&
          !DECL_EXTERNAL(decl));
}
