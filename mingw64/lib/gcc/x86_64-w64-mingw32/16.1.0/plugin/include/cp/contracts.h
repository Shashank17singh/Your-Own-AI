...
#define CONTRACT_ASSERTION_KIND(NODE) (TREE_OPERAND(CONTRACT_CHECK(NODE), 1))
...
#define PRECONDITION_P(NODE) (TREE_CODE(NODE) == PRECONDITION_STMT)

/* True if NODE is a postcondition.  */
#define POSTCONDITION_P(NODE) (TREE_CODE(NODE) == POSTCONDITION_STMT)
...
#define CONTRACT_SOURCE_LOCATION_WRAPPER(NODE) (TREE_PURPOSE(TREE_VALUE(NODE)))
...
#define CONTRACT_STATEMENT(NODE) (TREE_VALUE(TREE_VALUE(NODE)))

/* The parsed condition of the contract.  */
#define CONTRACT_CONDITION(NODE) (TREE_OPERAND(CONTRACT_CHECK(NODE), 2))
...
#define CONTRACT_COMMENT(NODE) (TREE_OPERAND(CONTRACT_CHECK(NODE), 3))

/* A std::source_location, if provided.  */
#define CONTRACT_STD_SOURCE_LOC(NODE) (TREE_OPERAND(CONTRACT_CHECK(NODE), 4))
...
#define DECL_PRE_FN(NODE) (get_precondition_function((NODE)))

/* For a FUNCTION_DECL of a guarded function, this holds the function decl
   where post contract checks are emitted.  */
#define DECL_POST_FN(NODE) (get_postcondition_function((NODE)))
...
enum contract_match_kind { cmk_all, cmk_pre, cmk_post };
...
inline bool contract_const_wrapper_p(const_tree exp) {
...
inline tree strip_contract_const_wrapper(tree exp) {
