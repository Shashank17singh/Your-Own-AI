...
struct GTY(()) binding_slot {
  ...
  bool is_lazy() const { return bool(uintptr_t(u.binding) & 1); }
...
extern tree lookup_qualified_name(tree scope, tree name,
                                  LOOK_want = LOOK_want::NORMAL, bool = true);
...
extern void walk_namespace_bindings(tree ns, void (*callback)(tree, void *data),
                                    void *data);
...
extern bool import_module_binding(tree ctx, tree name, unsigned mod,
                                  unsigned snum);
extern bool set_module_binding(tree ctx, tree name, unsigned mod, bool global_p,
                               bool partition_p, tree value, tree type,
                               tree visible, tree internal);
...
inline WMB_Flags &operator|=(WMB_Flags &x, WMB_Flags y) { return x = x | y; }
