...
inline bool return_false_with_message_1(const char *message,
...
inline bool return_with_result(bool result, const char *filename,
...
#define return_with_debug(result)                                              \
  return_with_result(result, __FILE__, __func__, __LINE__)
...
inline bool return_different_stmts_1(gimple *s1, gimple *s2, const char *code,
                                     const char *func, unsigned int line) {
  if (dump_file && (dump_flags & TDF_DETAILS)) {
...
public:
  sem_bb(basic_block bb_, unsigned nondbg_stmt_count_, unsigned edge_count_)
      : bb(bb_), nondbg_stmt_count(nondbg_stmt_count_),
...
public:
  ...
  func_checker(tree source_func_decl, tree target_func_decl,
               bool ignore_labels = false, bool tbaa = true,
  ...
  static operand_access_type
  get_operand_access_type(operand_access_type_map *map, tree);
...
public:
  ...
  bool operand_equal_p(const_tree, const_tree,
                       unsigned int flags) final override;
