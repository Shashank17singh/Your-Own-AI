...
public:
  ...
  virtual bool fold_range(irange &r, tree type, const irange &lh,
                          const irange &rh, relation_trio = TRIO_VARYING) const;
  virtual bool fold_range(frange &r, tree type, const frange &lh,
                          const frange &rh, relation_trio = TRIO_VARYING) const;
  virtual bool fold_range(irange &r, tree type, const frange &lh,
                          const irange &rh, relation_trio = TRIO_VARYING) const;
  virtual bool fold_range(irange &r, tree type, const frange &lh,
                          const frange &rh, relation_trio = TRIO_VARYING) const;
  virtual bool fold_range(frange &r, tree type, const irange &lh,
                          const irange &rh, relation_trio = TRIO_VARYING) const;
  virtual bool fold_range(frange &r, tree type, const irange &lh,
                          const frange &rh, relation_trio = TRIO_VARYING) const;
  virtual bool fold_range(prange &r, tree type, const prange &lh,
                          const prange &rh, relation_trio = TRIO_VARYING) const;
  virtual bool fold_range(prange &r, tree type, const prange &lh,
                          const irange &rh, relation_trio = TRIO_VARYING) const;
  virtual bool fold_range(irange &r, tree type, const prange &lh,
                          const prange &rh, relation_trio = TRIO_VARYING) const;
  virtual bool fold_range(prange &r, tree type, const irange &lh,
                          const prange &rh, relation_trio = TRIO_VARYING) const;
  virtual bool fold_range(irange &r, tree type, const prange &lh,
                          const irange &rh, relation_trio = TRIO_VARYING) const;
  ...
  virtual bool op1_range(irange &r, tree type, const irange &lhs,
                         const irange &op2, relation_trio = TRIO_VARYING) const;
  virtual bool op1_range(prange &r, tree type, const prange &lhs,
                         const prange &op2, relation_trio = TRIO_VARYING) const;
  virtual bool op1_range(prange &r, tree type, const irange &lhs,
                         const prange &op2, relation_trio = TRIO_VARYING) const;
  virtual bool op1_range(prange &r, tree type, const prange &lhs,
                         const irange &op2, relation_trio = TRIO_VARYING) const;
  virtual bool op1_range(irange &r, tree type, const prange &lhs,
                         const irange &op2, relation_trio = TRIO_VARYING) const;
  virtual bool op1_range(frange &r, tree type, const frange &lhs,
                         const frange &op2, relation_trio = TRIO_VARYING) const;
  virtual bool op1_range(frange &r, tree type, const irange &lhs,
                         const frange &op2, relation_trio = TRIO_VARYING) const;
  virtual bool op1_range(irange &r, tree type, const frange &lhs,
                         const irange &op2, relation_trio = TRIO_VARYING) const;

  virtual bool op2_range(irange &r, tree type, const irange &lhs,
                         const irange &op1, relation_trio = TRIO_VARYING) const;
  virtual bool op2_range(prange &r, tree type, const irange &lhs,
                         const prange &op1, relation_trio = TRIO_VARYING) const;
  virtual bool op2_range(irange &r, tree type, const prange &lhs,
                         const prange &op1, relation_trio = TRIO_VARYING) const;
  virtual bool op2_range(frange &r, tree type, const frange &lhs,
                         const frange &op1, relation_trio = TRIO_VARYING) const;
  virtual bool op2_range(frange &r, tree type, const irange &lhs,
                         const frange &op1, relation_trio = TRIO_VARYING) const;
  ...
  virtual relation_kind lhs_op1_relation(const prange &lhs, const prange &op1,
                                         const prange &op2,
                                         relation_kind = VREL_VARYING) const;
  virtual relation_kind lhs_op1_relation(const prange &lhs, const irange &op1,
                                         const irange &op2,
                                         relation_kind = VREL_VARYING) const;
  virtual relation_kind lhs_op1_relation(const irange &lhs, const prange &op1,
                                         const prange &op2,
                                         relation_kind = VREL_VARYING) const;
  virtual relation_kind lhs_op1_relation(const prange &lhs, const prange &op1,
                                         const irange &op2,
                                         relation_kind = VREL_VARYING) const;
  virtual relation_kind lhs_op1_relation(const frange &lhs, const frange &op1,
                                         const frange &op2,
                                         relation_kind = VREL_VARYING) const;
  virtual relation_kind lhs_op1_relation(const irange &lhs, const frange &op1,
                                         const frange &op2,
                                         relation_kind = VREL_VARYING) const;
  ...
  virtual relation_kind lhs_op2_relation(const irange &lhs, const irange &op1,
                                         const irange &op2,
                                         relation_kind = VREL_VARYING) const;
  virtual relation_kind lhs_op2_relation(const frange &lhs, const frange &op1,
                                         const frange &op2,
                                         relation_kind = VREL_VARYING) const;
  virtual relation_kind lhs_op2_relation(const irange &lhs, const frange &op1,
                                         const frange &op2,
                                         relation_kind = VREL_VARYING) const;

  virtual relation_kind op1_op2_relation(const irange &lhs, const irange &op1,
                                         const irange &op2) const;
  virtual relation_kind op1_op2_relation(const irange &lhs, const prange &op1,
                                         const prange &op2) const;
  virtual relation_kind op1_op2_relation(const irange &lhs, const frange &op1,
                                         const frange &op2) const;
  virtual relation_kind op1_op2_relation(const frange &lhs, const frange &op1,
                                         const frange &op2) const;
...
protected:
  ...
  virtual void wi_fold(irange &r, tree type, const wide_int &lh_lb,
  ...
  void wi_fold_in_parts(irange &r, tree type, const wide_int &lh_lb,
                        const wide_int &lh_ub, const wide_int &rh_lb,
                        const wide_int &rh_ub) const;

  // Called by fold range to split small subranges into parts when op1 == op2
  void wi_fold_in_parts_equiv(irange &r, tree type, const wide_int &lb,
                              const wide_int &ub, unsigned limit) const;
  ...
  virtual void rv_fold(frange &r, tree type, const REAL_VALUE_TYPE &lh_lb,
                       const REAL_VALUE_TYPE &lh_ub,
                       const REAL_VALUE_TYPE &rh_lb,
                       const REAL_VALUE_TYPE &rh_ub, relation_kind) const;
};
...
public:
  ...
  bool fold_range(vrange &r, tree type, const vrange &lh, const vrange &rh,
                  relation_trio = TRIO_VARYING) const;
  bool op1_range(vrange &r, tree type, const vrange &lhs, const vrange &op2,
                 relation_trio = TRIO_VARYING) const;
  bool op2_range(vrange &r, tree type, const vrange &lhs, const vrange &op1,
                 relation_trio = TRIO_VARYING) const;
  relation_kind lhs_op1_relation(const vrange &lhs, const vrange &op1,
                                 const vrange &op2,
                                 relation_kind = VREL_VARYING) const;
  relation_kind lhs_op2_relation(const vrange &lhs, const vrange &op1,
                                 const vrange &op2,
                                 relation_kind = VREL_VARYING) const;
...
inline bool range_cast(vrange &r, tree type) {
...
inline bool range_cast(value_range &r, tree type) {
  value_range tmp(r);
  value_range varying(type);
  varying.set_varying(type);
...
extern void wi_set_zero_nonzero_bits(tree type, const wide_int &,
                                     const wide_int &, wide_int &maybe_nonzero,
