...
public:
  impl_sm_context(exploded_graph &eg, int sm_idx, const state_machine &sm,
                  exploded_node *enode_for_diag, const program_state *old_state,
                  program_state *new_state, const sm_state_map *old_smap,
                  sm_state_map *new_smap, path_context *path_ctxt,
                  bool unknown_side_effects = false)
      : sm_context(sm_idx, sm), m_logger(eg.get_logger()), m_eg(eg),
        m_enode_for_diag(enode_for_diag), m_old_state(old_state),
        m_new_state(new_state), m_old_smap(old_smap), m_new_smap(new_smap),
        m_path_ctxt(path_ctxt), m_unknown_side_effects(unknown_side_effects) {}

  logger *get_logger() const { return m_logger.get_logger(); }

  tree get_fndecl_for_call(const gcall &call) final override {
    impl_region_model_context old_ctxt(m_eg, m_enode_for_diag, nullptr, nullptr,
                                       nullptr /*m_enode->get_state ()*/,
                                       nullptr, &call);
  ...
  void set_next_state(tree var, state_machine::state_t to,
    ...
    if (logger)
      logger->log("%s: state transition of %qE: %s -> %s", m_sm.get_name(), var,
                  current->get_name(), to->get_name());
    m_new_smap->set_state(m_new_state->m_region_model, var_new_sval, to,
                          origin_new_sval, m_eg.get_ext_state());
  }

  void set_next_state(const svalue *sval, state_machine::state_t to,
                      tree origin) final override {
    logger *const logger = get_logger();
    LOG_FUNC(logger);
    impl_region_model_context old_ctxt(m_eg, m_enode_for_diag, nullptr, nullptr,
    ...
    if (logger) {
      ...
      logger->log_partial("%s: state transition of ", m_sm.get_name());
      sval->dump_to_pp(logger->get_printer(), true);
      logger->log_partial(": %s -> %s", current->get_name(), to->get_name());
      logger->end_log_line();
    }
    m_new_smap->set_state(m_new_state->m_region_model, sval, to,
                          origin_new_sval, m_eg.get_ext_state());
  }
  ...
  void warn(tree var, std::unique_ptr<pending_diagnostic> d) final override {
    ...
    const svalue *var_old_sval =
        m_old_state->m_region_model->get_rvalue(var, nullptr);
    state_machine::state_t current =
        (var ? m_old_smap->get_state(var_old_sval, m_eg.get_ext_state())
             : m_old_smap->get_global_state());
    bool terminate_path = d->terminate_path_p();
    pending_location ploc(m_enode_for_diag);
    m_eg.get_diagnostic_manager().add_diagnostic(
        &m_sm, std::move(ploc), var, var_old_sval, current, std::move(d));
    if (m_path_ctxt && terminate_path && flag_analyzer_suppress_followups)
  ...
  void warn(const svalue *sval,
    ...
    gcc_assert(d);
    state_machine::state_t current =
        (sval ? m_old_smap->get_state(sval, m_eg.get_ext_state())
              : m_old_smap->get_global_state());
    bool terminate_path = d->terminate_path_p();
    pending_location ploc(m_enode_for_diag);
    m_eg.get_diagnostic_manager().add_diagnostic(
        &m_sm, std::move(ploc), NULL_TREE, sval, current, std::move(d));
    if (m_path_ctxt && terminate_path && flag_analyzer_suppress_followups)
      m_path_ctxt->terminate_path();
  }
  ...
  void on_custom_transition(custom_transition *transition) final override {
    transition->impl_transition(
        &m_eg, const_cast<exploded_node *>(m_enode_for_diag), m_sm_idx);
  }
  ...
  tree is_zero_assignment(const gimple *stmt) final override {
    ...
    if (!assign_stmt)
      return NULL_TREE;
    impl_region_model_context old_ctxt(m_eg, m_enode_for_diag, m_old_state,
                                       m_new_state, nullptr, nullptr, stmt);
    if (const svalue *sval = m_new_state->m_region_model->get_gassign_result(
            assign_stmt, &old_ctxt))
  ...
  path_context *get_path_context() const final override { return m_path_ctxt; }
