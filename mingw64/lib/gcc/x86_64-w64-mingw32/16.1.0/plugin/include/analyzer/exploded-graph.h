...
public:
  impl_region_model_context(exploded_graph &eg, exploded_node *enode_for_diag,

                            /* TODO: should we be getting the ECs from the
                               old state, rather than the new?  */
                            const program_state *old_state,
                            program_state *new_state,
                            uncertainty_t *uncertainty, path_context *path_ctxt,
  ...
  logger *get_logger() final override { return m_logger.get_logger(); }

  void on_state_leak(const state_machine &sm, const svalue *sval,
...
public:
  ...
  hashval_t hash() const { return m_hash; }
...
struct eg_traits {
  ...
  struct dump_args_t {
    ...
    virtual void dump_extra_info(const exploded_node *,
...
public:
  ...
  void on_longjmp(exploded_graph &eg, const gcall &call,
                  program_state *new_state, region_model_context *ctxt);
  void on_throw(exploded_graph &eg, const gcall &call,
                const program_point &after_throw_point,
                program_state *new_state, bool is_rethrow,
                region_model_context *ctxt);

  void detect_leaks(exploded_graph &eg);

  const program_point &get_point() const { return m_ps.get_point(); }
  const supernode *get_supernode() const { return get_point().get_supernode(); }
  location_t get_location() const { return get_point().get_location(); }
  function *get_function() const { return get_point().get_function(); }
  int get_stack_depth() const { return get_point().get_stack_depth(); }
  ...
  unsigned get_num_diagnostics() const { return m_saved_diagnostics.length(); }
...
public:
  exploded_edge(exploded_node *src, exploded_node *dest, const superedge *sedge,
                bool could_do_work,
...
public:
  interprocedural_call(const gcall &call_stmt, function &callee_fun)
      : m_call_stmt(call_stmt), m_callee_fun(callee_fun) {}
  ...
  bool update_state(program_state *state, const exploded_edge *eedge,
                    region_model_context *ctxt) const final override;

  bool update_model(region_model *model, const exploded_edge *eedge,
                    region_model_context *ctxt) const final override;
...
public:
  interprocedural_return(const gcall &call_stmt) : m_call_stmt(call_stmt) {}
...
public:
  rewind_info_t(const setjmp_record &setjmp_record, const gcall &longjmp_call)
      : m_setjmp_record(setjmp_record), m_longjmp_call(longjmp_call) {}
  ...
  const gcall &get_longjmp_call() const { return m_longjmp_call; }
...
private:
  struct per_node_data {
    per_node_data() : m_id(-1), m_lowlink(-1), m_on_stack(false) {}
...
private:
  ...
  public:
    ...
    bool operator<(const key_t &other) const { return cmp(*this, other) < 0; }
...
public:
  ...
  exploded_graph(const supergraph &sg, logger *logger,
                 const extrinsic_state &ext_state,
                 const state_purge_map *purge_map, const analysis_plan &plan,
                 int verbosity);
  ~exploded_graph();
  ...
  void save_diagnostic(const state_machine &sm, const exploded_node *enode,
                       const supernode *node, const gimple *stmt, tree var,
                       state_machine::state_t state, pending_diagnostic *d);

  diagnostic_manager &get_diagnostic_manager() { return m_diagnostic_manager; }
  ...
  void unwind_from_exception(exploded_node &enode, const gimple *stmt,
                             region_model_context *ctxt);
...
private:
  ...
  typedef hash_map<const point_and_state *, exploded_node *, eg_hash_map_traits>
      map_t;
...
public:
  feasibility_problem(unsigned eedge_idx, const exploded_edge &eedge,
                      std::unique_ptr<rejected_constraint> rc)
      : m_eedge_idx(eedge_idx), m_eedge(eedge), m_rc(std::move(rc)) {}
...
public:
  ...
  bool maybe_update_for_edge(logger *logger, const exploded_edge *eedge,
                             region_model_context *ctxt,
