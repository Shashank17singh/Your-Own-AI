...
private:
  ...
  make_state_node(enum state_node_properties::kind_t kind, std::string id);
  ...
  void create_state_nodes_for_binding_cluster(const binding_cluster &cluster,
                                              bool create_all);
  ...
  void populate_state_node_for_typed_region(
      diagnostics::digraphs::node &, const region &reg,
      const concrete_bindings_t &conc_bindings, bool create_all);

  void set_attr_for_dynamic_extents(const region &reg,
                                    diagnostics::digraphs::node &);

  bool show_child_state_node_for_child_region_p(
      const region &reg, const concrete_bindings_t &conc_bindings,
      bool create_all);
  ...
  std::map<const region *, diagnostics::digraphs::node *>
