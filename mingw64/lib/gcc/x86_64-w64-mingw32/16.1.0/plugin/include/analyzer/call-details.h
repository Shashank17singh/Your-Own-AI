...
public:
  ...
  const svalue *check_for_null_terminated_string_arg(
      unsigned arg_idx, bool include_terminator, const svalue **out_sval) const;

  void complain_about_overlap(unsigned arg_idx_a, unsigned arg_idx_b,
...
public:
  call_arg_details(const call_details &cd, unsigned arg_idx)
      : m_call(cd.get_call_stmt()), m_called_fndecl(cd.get_fndecl_for_call()),
        m_arg_idx(arg_idx), m_arg_expr(cd.get_arg_tree(arg_idx)) {}
