...
inline bool useful_location_p(location_t loc) {
...
class path_var {
public:
  path_var(tree t, int stack_depth) : m_tree(t), m_stack_depth(stack_depth) {
    // TODO: ignore stack depth for globals and constants
  }

  bool operator==(const path_var &other) const {
    return (m_tree == other.m_tree && m_stack_depth == other.m_stack_depth);
  }

  operator bool() const { return m_tree != NULL_TREE; }
...
public:
  ...
  bool operator==(const region_offset &other) const {
    return (m_base_region == other.m_base_region &&
            m_offset == other.m_offset && m_sym_offset == other.m_sym_offset);
  }
...
public:
  ...
  virtual void impl_call_pre(const call_details &) const { return; }
  virtual void impl_call_post(const call_details &) const { return; }
...
public:
  ...
  virtual void get_dot_attrs(const char *&out_style,
                             const char *&out_color) const;

  /* Hook for updating STATE when handling bifurcation.  */
  virtual bool update_state(program_state *state, const exploded_edge *eedge,
                            region_model_context *ctxt) const;

  /* Hook for updating MODEL within exploded_path::feasible_p
     and when handling bifurcation.  */
  virtual bool update_model(region_model *model, const exploded_edge *eedge,
...
extern std::unique_ptr<json::value> tree_to_json(tree node);
...
extern tristate compare_constants(tree lhs_const, enum tree_code op,
                                  tree rhs_const);

extern tree get_string_cst_size(const_tree string_cst);

extern tree get_ssa_default_def(const function &fun, tree var);

extern const svalue *strip_types(const svalue *sval, region_model_manager &mgr);

extern region_offset strip_types(const region_offset &offset,
...
extern label_text make_label_text_n(bool can_colorize, unsigned HOST_WIDE_INT n,
...
template <typename Type> struct pod_hash_traits : typed_noop_remove<Type> {
...
template <typename T> class consolidation_map {
public:
  ...
  void put(const key_t &k, T *instance) { m_inner_map.put(k, instance); }
