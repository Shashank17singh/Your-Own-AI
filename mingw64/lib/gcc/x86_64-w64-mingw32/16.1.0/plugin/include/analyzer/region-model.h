...
template <typename T> class one_way_id_map {
public:
...
template <typename T> inline void one_way_id_map<T>::put(T src, T dst) {
  m_src_to_dst[src.as_int()] = dst;
}

/* Get the new value for SRC within the map.  */

template <typename T> inline T one_way_id_map<T>::get_dst_for_src(T src) const {
  if (src.null_p())
    return src;
...
template <typename T>
inline void one_way_id_map<T>::dump_to_pp(pretty_printer *pp) const {
...
template <typename T>
DEBUG_FUNCTION inline void one_way_id_map<T>::dump() const {
  pretty_printer pp;
...
template <typename T> inline void one_way_id_map<T>::update(T *id) const {
...
public:
  ...
  void put(const region *reg, const svalue *sval) { m_hash_map.put(reg, sval); }
  void remove(const region *reg) { m_hash_map.remove(reg); }
...
struct purge_stats {
  purge_stats()
      : m_num_svalues(0), m_num_regions(0), m_num_equiv_classes(0),
        m_num_constraints(0), m_num_bounded_ranges_constraints(0),
...
struct exception_node {
  exception_node(const svalue *exception_sval, const svalue *typeinfo_sval,
                 const svalue *destructor_sval)
      : m_exception_sval(exception_sval), m_typeinfo_sval(typeinfo_sval),
...
public:
  ...
  bool operator!=(const region_model &other) const { return !(*this == other); }
  ...
  void on_stmt_pre(const gimple *stmt, bool *out_unknown_side_effects,
  ...
  void on_call_post(const gcall &stmt, bool unknown_side_effects,
                    region_model_context *ctxt);
  ...
  void update_for_int_cst_return(const call_details &cd, int retval,
                                 bool unmergeable);
  ...
  void get_reachable_svalues(svalue_set *out, const svalue *extra_sval,
  ...
  void on_setjmp(const gcall &stmt, const exploded_node &enode,
                 const superedge &sedge, region_model_context *ctxt);
  void on_longjmp(const gcall &longjmp_call, const gcall &setjmp_call,
                  int setjmp_stack_depth, region_model_context *ctxt);

  void update_for_gcall(const gcall &call_stmt, region_model_context *ctxt,
                        function *callee = nullptr);

  void update_for_return_gcall(const gcall &call_stmt,
                               region_model_context *ctxt);

  const region *push_frame(const function &fun, const gcall *call_stmt,
  ...
  void pop_frame(tree result_lvalue, const svalue **out_result,
                 region_model_context *ctxt, const gcall *call_stmt,
  ...
  const svalue *get_rvalue_for_bits(tree type, const region *reg,
  ...
  void write_bytes(const region *dest_reg, const svalue *num_bytes_sval,
                   const svalue *sval, region_model_context *ctxt);
  const svalue *read_bytes(const region *src_reg, tree src_ptr_expr,
                           const svalue *num_bytes_sval,
                           region_model_context *ctxt) const;
  void copy_bytes(const region *dest_reg, const region *src_reg,
                  tree src_ptr_expr, const svalue *num_bytes_sval,
                  region_model_context *ctxt);
  ...
  tristate eval_condition(tree lhs, enum tree_code op, tree rhs,
                          region_model_context *ctxt) const;
  ...
  tree get_representative_tree(const region *reg,
                               logger *logger = nullptr) const;
  path_var get_representative_path_var(const svalue *sval, svalue_set *visited,
                                       logger *logger) const;
  path_var get_representative_path_var(const region *reg, svalue_set *visited,
                                       logger *logger) const;

  /* For selftests.  */
  constraint_manager *get_constraints() { return m_constraints; }
  ...
  void set_dynamic_extents(const region *reg, const svalue *size_in_bytes,
                           region_model_context *ctxt);
  ...
  bool can_merge_with_p(const region_model &other_model,
                        const program_point &point, region_model *out_model,
  ...
  const svalue *get_store_value(const region *reg,
                                region_model_context *ctxt) const;
  const svalue *get_store_bytes(const region *base_reg, const byte_range &bytes,
                                region_model_context *ctxt) const;
  const svalue *scan_for_null_terminator(const region *reg, tree expr,
                                         const svalue **out_sval,
                                         region_model_context *ctxt) const;
  const svalue *scan_for_null_terminator_1(const region *reg, tree expr,
                                           const svalue **out_sval,
                                           region_model_context *ctxt) const;
  ...
  void on_realloc_with_move(const call_details &cd, const svalue *old_ptr_sval,
  ...
  const svalue *check_for_poison(const svalue *sval, tree expr,
                                 const region *src_region,
                                 region_model_context *ctxt) const;

  void check_region_for_write(const region *dest_reg, const svalue *sval_hint,
                              region_model_context *ctxt) const;

  const svalue *check_for_null_terminated_string_arg(const call_details &cd,
                                                     unsigned idx) const;
  const svalue *
  check_for_null_terminated_string_arg(const call_details &cd, unsigned idx,
                                       bool include_terminator,
                                       const svalue **out_sval) const;

  const builtin_known_function *
  get_builtin_kf(const gcall &call, region_model_context *ctxt = nullptr) const;
...
private:
  ...
  path_var get_representative_path_var_1(const svalue *sval,
                                         svalue_set *visited,
                                         logger *logger) const;
  path_var get_representative_path_var_1(const region *reg, svalue_set *visited,
                                         logger *logger) const;
  ...
  bool add_constraints_from_binop(const svalue *outer_lhs,
                                  ...
                                  const svalue *outer_rhs, bool *out,
                                  region_model_context *ctxt);
  ...
  void check_region_for_taint(const region *reg, enum access_direction dir,
                              region_model_context *ctxt) const;

  void check_for_writable_region(const region *dest_reg,
                                 region_model_context *ctxt) const;
  bool check_region_access(const region *reg, enum access_direction dir,
  ...
  bool check_symbolic_bounds(const region *base_reg,
                             ...
                             const svalue *num_bytes_sval,
                             const svalue *capacity, enum access_direction dir,
                             const svalue *sval_hint,
                             region_model_context *ctxt) const;
  ...
  void check_function_attr_access(const gcall &call, tree callee_fndecl,
                                  region_model_context *ctxt,
                                  rdwr_map &rdwr_idx) const;
  void check_function_attr_null_terminated_string_arg(
      const gcall &call, tree callee_fndecl, region_model_context *ctxt,
      rdwr_map &rdwr_idx);
  void check_one_function_attr_null_terminated_string_arg(
      const gcall &call, tree callee_fndecl, region_model_context *ctxt,
      rdwr_map &rdwr_idx, tree attr);
  void check_function_attrs(const gcall &call, tree callee_fndecl,
                            region_model_context *ctxt);

  void check_for_throw_inside_call(const gcall &call, tree fndecl,
                                   region_model_context *ctxt);
...
public:
  ...
  virtual bool warn_at(std::unique_ptr<pending_diagnostic> d,
  ...
  bool get_fd_map(sm_state_map **out_smap, const state_machine **out_sm,
                  unsigned *out_sm_idx,
  ...
  bool get_malloc_map(sm_state_map **out_smap, const state_machine **out_sm,
                      unsigned *out_sm_idx) {
    return get_state_map_by_name("malloc", out_smap, out_sm, out_sm_idx,
                                 nullptr);
  }
  bool get_taint_map(sm_state_map **out_smap, const state_machine **out_sm,
                     unsigned *out_sm_idx) {
...
public:
  ...
  bool warn_at(std::unique_ptr<pending_diagnostic>,
  ...
  bool get_state_map_by_name(const char *, sm_state_map **,
                             const state_machine **, unsigned *,
...
public:
  ...
  bool warn_at(std::unique_ptr<pending_diagnostic> d,
  ...
  void on_condition(const svalue *lhs, enum tree_code op,
...
struct model_merger {
  model_merger(const region_model *model_a, const region_model *model_b,
               const program_point &point, region_model *merged_model,
               const extrinsic_state *ext_state, const program_state *state_a,
               const program_state *state_b)
      : m_model_a(model_a), m_model_b(model_b), m_point(point),
        m_merged_model(merged_model), m_ext_state(ext_state),
  ...
  region_model_manager *get_manager() const { return m_model_a->get_manager(); }

  bool mergeable_svalue_p(const svalue *) const;

  const supernode *get_supernode() const { return m_point.get_supernode(); }
...
protected:
  rejected_constraint(const region_model &model) : m_model(model) {}
...
public:
  rejected_op_constraint(const region_model &model, const svalue *lhs,
                         enum tree_code op, const svalue *rhs)
      : rejected_constraint(model), m_lhs(lhs), m_op(op), m_rhs(rhs) {}
...
public:
  rejected_ranges_constraint(const region_model &model, tree expr,
                             const bounded_ranges *ranges)
      : rejected_constraint(model), m_expr(expr), m_ranges(ranges) {}
...
public:
  ...
  void on_unexpected_tree_code(tree t, const dump_location_t &) final override {
...
void assert_condition(const location &loc, region_model &model,
                      const svalue *lhs, tree_code op, const svalue *rhs,
                      tristate expected);

void assert_condition(const location &loc, region_model &model, tree lhs,
                      tree_code op, tree rhs, tristate expected);
