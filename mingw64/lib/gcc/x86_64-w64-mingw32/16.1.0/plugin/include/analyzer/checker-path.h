...
public:
  checker_path(const diagnostics::logical_locations::manager &logical_loc_mgr,
               const extrinsic_state &ext_state, logger *logger)
      : diagnostics::paths::path(logical_loc_mgr), m_ext_state(ext_state),
        m_thread("main"), m_logger(logger) {}
  ...
  checker_event *get_checker_event(int idx) { return m_events[idx]; }
  ...
  void add_region_creation_events(pending_diagnostic *pd, const region *reg,
                                  const region_model *model,
                                  const event_loc_info &loc_info, bool debug);
