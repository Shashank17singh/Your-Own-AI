...
public:
  extrinsic_state(std::vector<std::unique_ptr<state_machine>> &&checkers,
                  engine *eng, logger *logger = nullptr)
      : m_checkers(std::move(checkers)), m_logger(logger), m_engine(eng) {}

  // For use in selftests that use just one state machine
  extrinsic_state(std::unique_ptr<state_machine> sm, engine *eng,
                  logger *logger = nullptr)
      : m_logger(logger), m_engine(eng) {
    m_checkers.push_back(std::move(sm));
  }

  const state_machine &get_sm(int idx) const { return *m_checkers[idx]; }

  const char *get_name(int idx) const { return m_checkers[idx]->get_name(); }
...
public:
  ...
  void print(const region_model *model, bool simple, bool multiline,
  ...
  bool operator!=(const sm_state_map &other) const { return !(*this == other); }
  ...
  void set_state(region_model *model, const svalue *sval,
                 state_machine::state_t state, const svalue *origin,
                 const extrinsic_state &ext_state);
  bool set_state(const equiv_class &ec, state_machine::state_t state,
                 const svalue *origin, const extrinsic_state &ext_state);
  bool impl_set_state(const svalue *sval, state_machine::state_t state,
                      const svalue *origin, const extrinsic_state &ext_state);
  ...
  void on_unknown_change(const svalue *sval, bool is_mutable,
                         const extrinsic_state &ext_state);
  ...
  static const svalue *canonicalize_svalue(const svalue *sval,
                                           const extrinsic_state &ext_state);

  bool replay_call_summary(call_summary_replay &r, const sm_state_map &summary);

  bool can_merge_with_p(const sm_state_map &other, const state_machine &sm,
                        const extrinsic_state &ext_state,
...
public:
  ...
  void dump_to_pp(const extrinsic_state &ext_state, bool simple, bool multiline,
                  pretty_printer *pp) const;
  ...
  program_state prune_for_point(exploded_graph &eg, const program_point &point,
  ...
  bool can_merge_with_p(const program_state &other,
                        const extrinsic_state &ext_state,
                        const program_point &point, program_state *out) const;
