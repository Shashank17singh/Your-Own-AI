...
public:
  ...
  virtual const binop_svalue *dyn_cast_binop_svalue() const { return nullptr; }
  virtual const sub_svalue *dyn_cast_sub_svalue() const { return nullptr; }
  ...
  dyn_cast_const_fn_result_svalue() const {
    return nullptr;
  }
  ...
  const svalue *can_merge_p(const svalue *other, region_model_manager *mgr,
  ...
  const svalue *extract_bit_range(tree type, const bit_range &subrange,
                                  region_model_manager *mgr) const;

  virtual const svalue *maybe_fold_bits_within(tree type,
  ...
  bool maybe_print_for_user(pretty_printer *pp, const region_model &model,
...
private:
  ...
  virtual void add_dump_widget_children(text_art::tree_widget &,
...
public:
  ...
  struct key_t {
    key_t(tree type, const region *reg) : m_type(type), m_reg(reg) {}
...
template <>
template <>
inline bool is_a_helper<const region_svalue *>::test(const svalue *sval) {
...
public:
  /* A support class for uniquifying instances of region_svalue.  */
  struct key_t {
    key_t(tree type, tree cst) : m_type(type), m_cst(cst) {}
  ...
  constant_svalue(symbol::id_t id, tree type, tree cst_expr)
      : svalue(complexity(1, 1), id, type), m_cst_expr(cst_expr) {
    gcc_assert(cst_expr);
  ...
  static tristate eval_condition(const constant_svalue *lhs, enum tree_code op,
                                 const constant_svalue *rhs);

  const svalue *
  maybe_fold_bits_within(tree type, const bit_range &subrange,
...
template <>
template <>
inline bool is_a_helper<const constant_svalue *>::test(const svalue *sval) {
...
public:
  ...
  struct key_t {
    key_t(enum poison_kind kind, tree type) : m_kind(kind), m_type(type) {}
...
template <>
template <>
inline bool is_a_helper<const poisoned_svalue *>::test(const svalue *sval) {
...
struct setjmp_record {
  setjmp_record(const exploded_node *enode, const superedge *sedge,
  ...
  bool operator==(const setjmp_record &other) const {
    return (m_enode == other.m_enode && m_sedge == other.m_sedge &&
...
template <>
template <>
inline bool is_a_helper<const setjmp_svalue *>::test(const svalue *sval) {
...
template <>
template <>
inline bool is_a_helper<const initial_svalue *>::test(const svalue *sval) {
...
public:
  ...
  struct key_t {
    ...
    bool operator==(const key_t &other) const {
      return (m_type == other.m_type && m_op == other.m_op &&
              m_arg == other.m_arg);
    }
...
template <>
template <>
inline bool is_a_helper<const unaryop_svalue *>::test(const svalue *sval) {
...
public:
  ...
  struct key_t {
    ...
    bool operator==(const key_t &other) const {
      return (m_type == other.m_type && m_op == other.m_op &&
              m_arg0 == other.m_arg0 && m_arg1 == other.m_arg1);
    }
...
template <>
template <>
inline bool is_a_helper<const binop_svalue *>::test(const svalue *sval) {
...
public:
  ...
  const sub_svalue *dyn_cast_sub_svalue() const final override { return this; }
...
template <>
template <>
inline bool is_a_helper<const sub_svalue *>::test(const svalue *sval) {
...
public:
  ...
  struct key_t {
    ...
    bool operator==(const key_t &other) const {
      return (m_type == other.m_type && m_outer_size == other.m_outer_size &&
  ...
  repeated_svalue(symbol::id_t id, tree type, const svalue *outer_size,
...
template <>
template <>
inline bool is_a_helper<const repeated_svalue *>::test(const svalue *sval) {
...
public:
  ...
  struct key_t {
    ...
    bool operator==(const key_t &other) const {
      return (m_type == other.m_type && m_bits == other.m_bits &&
              m_inner_svalue == other.m_inner_svalue);
    }
  ...
  bits_within_svalue(symbol::id_t id, tree type, const bit_range &bits,
                     const svalue *inner_svalue);
...
template <>
template <>
inline bool is_a_helper<const bits_within_svalue *>::test(const svalue *sval) {
...
template <>
template <>
inline bool is_a_helper<const unmergeable_svalue *>::test(const svalue *sval) {
...
template <>
template <>
inline bool is_a_helper<const placeholder_svalue *>::test(const svalue *sval) {
...
public:
  ...
  struct key_t {
    key_t(tree type, const supernode *snode, const svalue *base_sval,
          const svalue *iter_sval)
        : m_type(type), m_snode(snode), m_base_sval(base_sval),
    ...
    bool operator==(const key_t &other) const {
      return (m_type == other.m_type && m_snode == other.m_snode &&
  ...
  widening_svalue(symbol::id_t id, tree type, const supernode *snode,
      ...
      : svalue(complexity::from_pair(base_sval->get_complexity(),
               ...
               id, type),
        m_snode(snode), m_base_sval(base_sval), m_iter_sval(iter_sval) {
...
template <>
template <>
inline bool is_a_helper<const widening_svalue *>::test(const svalue *sval) {
...
public:
  ...
  struct key_t {
    ...
    bool operator==(const key_t &other) const {
      return (m_type == other.m_type && *m_map_ptr == *other.m_map_ptr);
    }
  ...
  struct key_t make_key() const { return key_t(get_type(), &m_map); }
...
template <>
template <>
inline bool is_a_helper<const compound_svalue *>::test(const svalue *sval) {
...
public:
  ...
  struct key_t {
    ...
    bool operator==(const key_t &other) const {
      return (m_type == other.m_type && m_stmt == other.m_stmt &&
              m_id_reg == other.m_id_reg && m_idx == other.m_idx);
    }
  ...
  conjured_svalue(symbol::id_t id, tree type, const gimple *stmt,
      ...
      : svalue(complexity(id_reg), id, type), m_stmt(stmt), m_id_reg(id_reg),
        m_idx(idx) {
...
template <>
template <>
inline bool is_a_helper<const conjured_svalue *>::test(const svalue *sval) {
...
public:
  ...
  struct key_t {
    key_t(tree type, const char *asm_string, unsigned output_idx,
  ...
  asm_output_svalue(symbol::id_t id, tree type, const char *asm_string,
                    unsigned output_idx, unsigned num_outputs,
                    const vec<const svalue *> &inputs)
      : svalue(complexity::from_vec_svalue(inputs), id, type),
        m_asm_string(asm_string), m_output_idx(output_idx),
        m_num_outputs(num_outputs), m_num_inputs(inputs.length()) {
    gcc_assert(inputs.length() <= MAX_INPUTS);
    for (unsigned i = 0; i < m_num_inputs; i++)
      m_input_arr[i] = inputs[i];
  }
...
private:
  void dump_input(pretty_printer *pp, unsigned input_idx, const svalue *sval,
                  bool simple) const;
...
template <>
template <>
inline bool is_a_helper<const asm_output_svalue *>::test(const svalue *sval) {
...
public:
  ...
  struct key_t {
    key_t(tree type, tree fndecl, const vec<const svalue *> &inputs)
        : m_type(type), m_fndecl(fndecl), m_num_inputs(inputs.length()) {
      gcc_assert(inputs.length() <= MAX_INPUTS);
      for (unsigned i = 0; i < m_num_inputs; i++)
        m_input_arr[i] = inputs[i];
    }
    ...
    bool operator==(const key_t &other) const {
      if (!(m_type == other.m_type && m_fndecl == other.m_fndecl &&
            m_num_inputs == other.m_num_inputs))
        return false;
      for (unsigned i = 0; i < m_num_inputs; i++)
        if (m_input_arr[i] != other.m_input_arr[i])
    ...
    bool is_deleted() const { return m_fndecl == reinterpret_cast<tree>(1); }
  ...
  const_fn_result_svalue(symbol::id_t id, tree type, tree fndecl,
                         const vec<const svalue *> &inputs)
      : svalue(complexity::from_vec_svalue(inputs), id, type), m_fndecl(fndecl),
        m_num_inputs(inputs.length()) {
    gcc_assert(inputs.length() <= MAX_INPUTS);
    for (unsigned i = 0; i < m_num_inputs; i++)
      m_input_arr[i] = inputs[i];
