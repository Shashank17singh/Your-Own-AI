...
public:
  ...
  virtual state_t alt_get_inherited_state(const sm_state_map &, const svalue *,
                                          const extrinsic_state &) const {
    return nullptr;
  }

  virtual bool has_alt_get_inherited_state_p() const { return false; }
  ...
  on_leak(tree var ATTRIBUTE_UNUSED, const program_state *old_state,
  ...
  virtual void add_state_to_state_graph(analyzer_state_graph &out_state_graph,
                                        const svalue &sval,
...
public:
  ...
  virtual void impl_transition(exploded_graph *eg, exploded_node *src_enode,
                               int sm_idx) = 0;
};
...
public:
  ...
  /* Set the next state of VAR to be TO, recording the "origin" of the
     state as ORIGIN.  */
  virtual void set_next_state(tree var, state_machine::state_t to,
                              tree origin = NULL_TREE) = 0;
  virtual void set_next_state(const svalue *var, state_machine::state_t to,
                              tree origin = NULL_TREE) = 0;
  ...
  void on_transition(tree var, state_machine::state_t from,
                     state_machine::state_t to, tree origin = NULL_TREE) {
    state_machine::state_t current = get_state(var);
    if (current == from)
      set_next_state(var, to, origin);
  }

  void on_transition(const svalue *var, state_machine::state_t from,
                     state_machine::state_t to, tree origin = NULL_TREE) {
    state_machine::state_t current = get_state(var);
    if (current == from)
      set_next_state(var, to, origin);
  ...
  virtual tree get_diagnostic_tree(tree expr) { return expr; }
  ...
  virtual path_context *get_path_context() const { return nullptr; }
