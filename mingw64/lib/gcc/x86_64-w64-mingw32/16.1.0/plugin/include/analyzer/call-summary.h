...
public:
  call_summary(per_function_data *per_fn_data, const exploded_node *enode)
      : m_per_fn_data(per_fn_data), m_enode(enode) {}
  const program_state &get_state() const;
  tree get_fndecl() const;

  label_text get_desc() const;

  void dump_to_pp(const extrinsic_state &ext_state, pretty_printer *pp,
                  bool simple) const;
...
public:
  call_summary_replay(const call_details &cd, const function &called_fn,
                      call_summary &summary, const extrinsic_state &ext_state);
