...
struct operation_context {
  operation_context(exploded_graph &eg, exploded_node &src_enode,
                    const superedge &sedge)
      : m_eg(eg), m_src_enode(src_enode), m_sedge(sedge) {}
  ...
  void add_outcome(const program_point &dst_point, program_state dst_state,
                   bool could_do_work, uncertainty_t *uncertainty,
...
public:
  ...
  virtual void print_as_edge_label(pretty_printer *pp,
                                   bool user_facing) const = 0;

  virtual bool defines_ssa_name_p(const_tree ssa_name) const = 0;

  virtual void walk_load_store_addr_ops(void *, walk_stmt_load_store_addr_fn,
                                        walk_stmt_load_store_addr_fn,
                                        walk_stmt_load_store_addr_fn) const = 0;

  virtual const gimple *maybe_get_stmt() const { return nullptr; }

  virtual void execute(operation_context &op_ctxt) const = 0;

  virtual bool execute_for_feasibility(
      const exploded_edge &, feasibility_state &, region_model_context *,
  ...
  virtual void update_state_for_bulk_merger(const program_state &,
                                            program_state &) const {
    /* Must be implemented for any subclasses that return true
       for supports_bulk_merge_p.  */
    gcc_unreachable();
  }
  virtual void add_any_events_for_eedge(const exploded_edge &eedge,
...
protected:
  operation(enum kind kind_) : m_kind(kind_) {}

  static void handle_on_stmt_for_state_machines(operation_context &op_ctxt,
...
public:
  ...
  void
  walk_load_store_addr_ops(void *, walk_stmt_load_store_addr_fn,
                           walk_stmt_load_store_addr_fn,
                           walk_stmt_load_store_addr_fn) const final override;

  const gimple *maybe_get_stmt() const final override { return &m_stmt; }

  void execute(operation_context &op_ctxt) const override;

  void execute_on_state(operation_context &op_ctxt,
                        program_state dst_state) const;

  bool execute_for_feasibility(
      const exploded_edge &, feasibility_state &, region_model_context *,
      std::unique_ptr<rejected_constraint> *out_rc) const override;

  virtual bool supports_bulk_merge_p() const override;

  void add_any_events_for_eedge(const exploded_edge &eedge,
...
public:
  gasm_op(const gasm &asm_stmt) : gimple_stmt_op(kind::asm_stmt, asm_stmt) {}

  std::unique_ptr<operation> clone() const final override {
    return std::make_unique<gasm_op>(get_gasm());
  }

  const gasm &get_gasm() const { return *as_a<const gasm *>(&get_stmt()); }
};
...
public:
  ...
  bool execute_for_feasibility(
      const exploded_edge &, feasibility_state &, region_model_context *ctxt,
      std::unique_ptr<rejected_constraint> *out_rc) const override;

  bool supports_bulk_merge_p() const final override { return false; }
  ...
  tree get_retval() const { return gimple_return_retval(&get_greturn()); }
};
...
public:
  ...
  const gcall &get_gcall() const { return *as_a<const gcall *>(&get_stmt()); }
  ...
  tree map_expr_from_caller_to_callee(tree callee_fndecl, tree caller_expr,
                                      callsite_expr *out) const;
  tree map_expr_from_callee_to_caller(tree callee_fndecl, tree callee_expr,
                                      callsite_expr *out) const;
...
private:
  ...
  void replay_call_summaries(operation_context &op_ctxt, function &called_fn,
                             per_function_data &called_fn_data,
                             region_model_context *ctxt) const;

  void replay_call_summary(operation_context &op_ctxt, function &called_fn,
                           call_summary &summary,
                           region_model_context *ctxt) const;

  tree get_arg_for_parm(tree callee_fndecl, tree parm,
                        callsite_expr *out) const;
...
public:
  enum dump_kind { state, sarif, dot, state_2 };

  dump_op(const gcall &call_stmt, enum dump_kind dump_kind_)
      : call_and_return_op(call_stmt), m_dump_kind(dump_kind_) {}
...
public:
  setjmp_op(const gcall &call_stmt) : call_and_return_op(call_stmt) {}
...
public:
  longjmp_op(const gcall &call_stmt) : call_and_return_op(call_stmt) {}
...
public:
  cxa_throw_op(const gcall &call_stmt, bool is_rethrow)
      : call_and_return_op(call_stmt), m_is_rethrow(is_rethrow) {}
...
public:
  resx_op(const gresx &resx_stmt) : gimple_stmt_op(kind::resx, resx_stmt) {}

  std::unique_ptr<operation> clone() const final override {
    return std::make_unique<resx_op>(get_gresx());
  }

  const gresx &get_gresx() const { return *as_a<const gresx *>(&get_stmt()); }
...
public:
  ...
  bool defines_ssa_name_p(const_tree) const final override { return false; }
...
protected:
  control_flow_op(enum kind kind_, ::edge cfg_edge, const gimple &ctrlflow_stmt)
      : operation(kind_), m_cfg_edge(cfg_edge), m_ctrlflow_stmt(ctrlflow_stmt) {
  }

private:
  virtual bool
  apply_constraints(const superedge *sedge, region_model &model,
...
public:
  ...
  std::unique_ptr<operation> clone() const final override {
    return std::make_unique<gcond_edge_op>(get_cfg_edge(), get_gcond());
  }

  void print_as_edge_label(pretty_printer *pp,
...
private:
  static label_text maybe_describe_condition(bool can_colorize, tree lhs,
                                             enum tree_code op, tree rhs);
  static bool should_print_expr_p(tree expr);

  bool apply_constraints(
      const superedge *sedge, region_model &model, region_model_context *ctxt,
      std::unique_ptr<rejected_constraint> *out) const final override;
...
public:
  ...
  std::unique_ptr<operation> clone() const final override {
    return std::make_unique<ggoto_edge_op>(get_cfg_edge(), get_ggoto(),
                                           m_dst_label);
  }
...
public:
  switch_case_op(function &fun, ::edge cfg_edge, const gswitch &switch_stmt,
...
public:
  ...
  make(supernode *src, supernode *dest, ::edge cfg_edge,
...
protected:
  eh_dispatch_edge_op(supernode *src_snode, enum kind kind_, ::edge cfg_edge,
                      const geh_dispatch &geh_dispatch_stmt, eh_region eh_reg);
...
private:
  ...
  virtual bool
  apply_eh_constraints(const superedge *sedge, region_model &model,
                       region_model_context *ctxt, tree exception_type,
                       std::unique_ptr<rejected_constraint> *out) const = 0;
...
public:
  eh_dispatch_try_edge_op(supernode *src_snode, ::edge cfg_edge,
                          const geh_dispatch &geh_dispatch_stmt,
                          eh_region eh_reg, eh_catch ehc);
...
public:
  ...
  eh_dispatch_allowed_edge_op(supernode *src_snode, supernode *dst_snode,
                              ::edge cfg_edge,
                              const geh_dispatch &geh_dispatch_stmt,
                              eh_region eh_reg);
...
public:
  ...
  bool supports_bulk_merge_p() const final override { return true; }
  void
...
private:
  ...
  void update_state(const program_state &src_state, program_state &dst_state,
                    region_model_context *ctxt) const;
