...
public:
  ...
  public:
    ...
    virtual void fixup_for_epath(const exploded_path &epath,
  ...
  location_t get_location() const { return m_event_loc_info.m_loc; }
...
public:
  saved_diagnostic(const state_machine *sm, pending_location &&ploc, tree var,
                   const svalue *sval, state_machine::state_t state,
                   std::unique_ptr<pending_diagnostic> d, unsigned idx);
...
public:
  ...
  bool add_diagnostic(const state_machine *sm, pending_location &&ploc,
...
private:
  ...
  void build_emission_path(const path_builder &pb, const exploded_path &epath,
  ...
  void add_events_for_eedge(const path_builder &pb, const exploded_edge &eedge,
                            checker_path *emission_path,
                            interesting_t *interest) const;
  ...
  void prune_path(checker_path *path, const state_machine *sm,
                  const svalue *sval, state_machine::state_t state) const;

  void prune_for_sm_diagnostic(checker_path *path, const state_machine *sm,
