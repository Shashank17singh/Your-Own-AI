...
void bp_pack_string(struct output_block *, struct bitpack_d *, const char *,
                    bool);
...
inline void bp_pack_value(struct bitpack_d *bp, bitpack_word_t val,
                          unsigned nbits) {
...
inline void streamer_write_bitpack(struct bitpack_d *bp) {
...
inline void streamer_write_char_stream(struct lto_output_stream *obs, char c) {
  /* No space left.  */
...
inline unsigned char streamer_read_uchar(class lto_input_block *ib) {
  if (ib->p >= ib->len)
...
inline void streamer_write_hwi_in_range(struct lto_output_stream *obs,
...
inline HOST_WIDE_INT streamer_read_hwi_in_range(class lto_input_block *ib,
                                                const char *purpose,
                                                HOST_WIDE_INT min,
                                                HOST_WIDE_INT max) {
  HOST_WIDE_INT range = max - min;
...
inline void bp_pack_int_in_range(struct bitpack_d *bp, HOST_WIDE_INT min,
...
inline HOST_WIDE_INT bp_unpack_int_in_range(struct bitpack_d *bp,
                                            const char *purpose,
                                            HOST_WIDE_INT min,
                                            HOST_WIDE_INT max) {
  HOST_WIDE_INT range = max - min;
...
#define streamer_read_enum(ib, enum_name, enum_last)                           \
  (enum enum_name)                                                             \
      streamer_read_hwi_in_range((ib), #enum_name, 0, (int)(enum_last) - 1)
...
#define bp_unpack_enum(bp, enum_name, enum_last)                               \
  (enum enum_name)                                                             \
      bp_unpack_int_in_range((bp), #enum_name, 0, (int)(enum_last) - 1)

/* Output the start of a record with TAG to output block OB.  */

inline void streamer_write_record_start(struct output_block *ob,
                                        enum LTO_tags tag) {
...
inline enum LTO_tags streamer_read_record_start(class lto_input_block *ib) {
