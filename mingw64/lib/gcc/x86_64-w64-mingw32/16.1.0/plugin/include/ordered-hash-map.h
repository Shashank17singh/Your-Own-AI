...
public:
  ...
  ordered_hash_map(const ordered_hash_map &other)
      : m_map(other.m_map), m_keys(other.m_keys.length()),
  ...
  Value *get(const Key &k) { return m_map.get(k); }
  ...
  void remove(const Key &k) { m_map.remove(k); }
  ...
  public:
    ...
    reference_pair operator*() {
      ...
      Value *slot = const_cast<ordered_hash_map &>(m_ordered_hash_map).get(k);
      gcc_assert(slot);
