...
public:
  ...
  dr_with_seg_len_pair_t(const dr_with_seg_len &, const dr_with_seg_len &,
                         sequencing);
...
inline dr_with_seg_len_pair_t::dr_with_seg_len_pair_t(const dr_with_seg_len &d1,
...
opt_result dr_analyze_innermost(innermost_loop_behavior *, tree, class loop *,
                                const gimple *);
extern bool compute_data_dependences_for_loop(class loop *, bool, vec<loop_p> *,
...
initialize_data_dependence_relation(struct data_reference *,
                                    struct data_reference *, vec<loop_p>);
extern void compute_affine_dependence(struct data_dependence_relation *,
                                      loop_p);
extern void compute_self_dependence(struct data_dependence_relation *);
extern bool compute_all_dependences(const vec<data_reference_p> &, vec<ddr_p> *,
...
inline bool same_data_refs_base_objects(data_reference_p a,
                                        data_reference_p b) {
...
inline bool same_data_refs(data_reference_p a, data_reference_p b,
                           int offset = 0) {
  unsigned int i;
...
inline unsigned dependence_level(lambda_vector dist_vect, int length) {
  int i;
...
inline unsigned ddr_dependence_level(ddr_p ddr) {
  unsigned vector;
  unsigned level = 0;
  ...
  for (vector = 1; vector < DDR_NUM_DIST_VECTS(ddr); vector++)
    level = MIN(
        level, dependence_level(DDR_DIST_VECT(ddr, vector), DDR_NB_LOOPS(ddr)));
  return level;
}

/* Return the index of the variable VAR in the LOOP_NEST array.  */

inline int index_in_loop_nest(int var, const vec<loop_p> &loop_nest) {
  class loop *loopi;
  int var_index;
...
inline bool adjacent_dr_p(struct data_reference *dr) {
  ...
  return tree_int_cst_equal(
      fold_unary(ABS_EXPR, TREE_TYPE(DR_STEP(dr)), DR_STEP(dr)),
...
inline lambda_int lambda_vector_gcd(lambda_vector vector, int size) {
  int i;
  lambda_int gcd1 = 0;
...
inline lambda_vector lambda_vector_new(int size) {
...
inline void lambda_vector_clear(lambda_vector vec1, int size) {
...
inline bool lambda_vector_lexico_pos(lambda_vector v, unsigned n) {
  unsigned i;
  for (i = 0; i < n; i++) {
    if (v[i] == 0)
      continue;
...
inline bool lambda_vector_zerop(lambda_vector vec1, int size) {
  int i;
  for (i = 0; i < size; i++)
    if (vec1[i] != 0)
      return false;
...
inline lambda_matrix lambda_matrix_new(int m, int n,
