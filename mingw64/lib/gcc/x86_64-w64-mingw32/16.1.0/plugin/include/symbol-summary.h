...
public:
  ...
  function_summary_base(symbol_table *symtab, cgraph_node_hook symtab_insertion,
                        cgraph_node_hook symtab_removal,
                        cgraph_2node_hook symtab_duplication CXX_MEM_STAT_INFO)
      : m_symtab(symtab), m_symtab_insertion(symtab_insertion),
        m_symtab_removal(symtab_removal),
        m_symtab_duplication(symtab_duplication), m_symtab_insertion_hook(NULL),
...
public:
  ...
  bool exists(cgraph_node *node) { return m_map.get(node->get_uid()) != NULL; }
...
private:
  /* Indication if we use ggc summary.  */
  bool is_ggc() final override { return m_ggc; }
  ...
  template <typename U>
  friend void gt_pch_nx(function_summary<U *> *const &, gt_pointer_operator,
                        void *);
};

template <typename T>
function_summary<T *>::function_summary(symbol_table *symtab,
                                        bool ggc MEM_STAT_DECL)
    : function_summary_base<T>(
          symtab, function_summary::symtab_insertion,
          function_summary::symtab_removal,
          function_summary::symtab_duplication PASS_MEM_STAT),
      m_ggc(ggc), m_map(13, ggc, true, GATHER_STATISTICS PASS_MEM_STAT) {}

template <typename T> function_summary<T *>::~function_summary() {
...
template <typename T>
void function_summary<T *>::symtab_insertion(cgraph_node *node, void *data) {
...
template <typename T>
void function_summary<T *>::symtab_removal(cgraph_node *node, void *data) {
  gcc_checking_assert(node->get_uid());
  function_summary *summary = (function_summary<T *> *)(data);
  summary->remove(node);
}
...
template <typename T> void gt_pch_nx(function_summary<T *> *const &) {
  gcc_unreachable();
}
...
class GTY((user))
    fast_function_summary<T *, V> : public function_summary_base<T> {
public:
  /* Default construction takes SYMTAB as an argument.  */
...
fast_function_summary<T *, V>::fast_function_summary(
    symbol_table *symtab MEM_STAT_DECL)
    : function_summary_base<T>(
          symtab, fast_function_summary::symtab_insertion,
          fast_function_summary::symtab_removal,
          fast_function_summary::symtab_duplication PASS_MEM_STAT),
      m_vector(NULL) {
...
template <typename T, typename V>
inline bool fast_function_summary<T *, V>::is_ggc() {
...
template <typename T>
void gt_pch_nx(fast_function_summary<T *, va_gc> *const &, gt_pointer_operator,
               void *) {
  gcc_unreachable();
}
...
public:
  ...
  call_summary_base(symbol_table *symtab, cgraph_edge_hook symtab_removal,
      ...
      : m_symtab(symtab), m_symtab_removal(symtab_removal),
        ...
        m_symtab_duplication_hook(NULL), m_initialize_when_cloning(false),
  ...
  void enable_duplication_hook() {
    if (m_symtab_duplication_hook == NULL)
      m_symtab_duplication_hook =
          m_symtab->add_edge_duplication_hook(m_symtab_duplication, this);
  }
...
public:
  ...
  bool exists(cgraph_edge *edge) { return m_map.get(edge->get_uid()) != NULL; }
...
private:
  ...
  template <typename U>
  friend void gt_pch_nx(call_summary<U *> *const &, gt_pointer_operator,
                        void *);
};

template <typename T> call_summary<T *>::~call_summary() {
  this->unregister_hooks();
...
template <typename T>
void call_summary<T *>::symtab_removal(cgraph_edge *edge, void *data) {
...
template <typename T> void gt_pch_nx(call_summary<T *> *const &) {
  gcc_unreachable();
}
...
public:
  ...
  fast_call_summary(symbol_table *symtab CXX_MEM_STAT_INFO)
      : call_summary_base<T>(
            symtab, fast_call_summary::symtab_removal,
  ...
  bool exists(cgraph_edge *edge) {
    int id = edge->get_summary_id();
    return (id != -1 && (unsigned)id < m_vector->length() &&
            (*m_vector)[id] != NULL);
  }
...
template <typename T, typename V>
void fast_call_summary<T *, V>::symtab_duplication(cgraph_edge *edge1,
                                                   cgraph_edge *edge2,
                                                   void *data) {
  fast_call_summary *summary = (fast_call_summary<T *, V> *)(data);
  T *edge1_summary = NULL;
...
template <typename T, typename V>
inline bool fast_call_summary<T *, V>::is_ggc() {
  return is_same<V, va_gc>::value;
}

template <typename T>
void gt_ggc_mx(
    fast_call_summary<T *, va_heap> *const &summary ATTRIBUTE_UNUSED) {}
...
template <typename T>
void gt_pch_nx(fast_call_summary<T *, va_heap> *const &summary ATTRIBUTE_UNUSED,
...
template <typename T> void gt_pch_nx(fast_call_summary<T *, va_gc> *const &) {
  gcc_unreachable();
}

template <typename T>
void gt_pch_nx(fast_call_summary<T *, va_gc> *const &, gt_pointer_operator,
               void *) {
  gcc_unreachable();
}
