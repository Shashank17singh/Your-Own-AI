...
public:
  ...
  FILE *get_stream() const { return m_outfile.get_open_file(); }
...
public:
  auto_inc_depth(logger *log) : m_logger(log) {
    if (m_logger)
      m_logger->inc_depth();
  }
  ~auto_inc_depth() {
...
public:
  log_function_params(logger *logger_, const char *name)
      : m_logger(logger_), m_first_param(true) {
    if (m_logger) {
  ...
  log_function_params &log_param_location_t(const char *name,
                                            location_t value) {
    if (m_logger) {
      add_any_comma();
      fprintf(m_logger->get_stream(), "%s: " HOST_SIZE_T_PRINT_HEX, name,
              (fmt_size_t)value);
    }
    return *this;
  }

  log_function_params &log_param_rich_location(const char *name,
                                               const rich_location *richloc) {
    if (m_logger) {
      add_any_comma();
      fprintf(m_logger->get_stream(), "%s: %p", name,
              const_cast<void *>((const void *)richloc));
    }
    return *this;
  }

  log_function_params &log_param_option_id(const char *name,
                                           diagnostics::option_id value) {
    if (m_logger) {
      add_any_comma();
  ...
  log_function_params &log_param_kind(const char *name,
                                      enum diagnostics::kind value) {
    if (m_logger) {
      add_any_comma();
      fprintf(m_logger->get_stream(), "%s: %s", name,
              get_debug_string_for_kind(value));
    }
    return *this;
  }

  log_function_params &log_param_uhwi(const char *name,
                                      unsigned HOST_WIDE_INT value) {
    if (m_logger) {
      add_any_comma();
      fprintf(m_logger->get_stream(), "%s: " HOST_WIDE_INT_PRINT_DEC, name,
              value);
    }
    return *this;
  }

  log_function_params &log_params_n_gmsgids(unsigned HOST_WIDE_INT n,
