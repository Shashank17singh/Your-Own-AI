...
typedef void (*text_finalizer_fn)(text_sink &, const diagnostic_info *,
                                  enum kind);
...
public:
  ...
  label_text get_location_text(const expanded_location &s, bool show_column,
                               bool colorize) const;
...
public:
  ...
  void print_text_span_start(const context &dc, pretty_printer &pp,
                             const expanded_location &exploc);

  void print_html_span_start(const context &dc, xml::printer &xp,
                             const expanded_location &exploc);
...
public:
  ...
  source_print_policy(const context &, const source_printing_options &);

  void print(pretty_printer &pp, const rich_location &richloc,
             enum kind diagnostic_kind, source_effect_info *effect_info) const;

  void print_as_html(xml::printer &xp, const rich_location &richloc,
                     enum kind diagnostic_kind, source_effect_info *effect_info,
                     html_label_writer *label_writer) const;

  const source_printing_options &get_options() const { return m_options; }

  start_span_fn<to_text> get_text_start_span_fn() const {
    return m_text_start_span_cb;
  }

  start_span_fn<to_html> get_html_start_span_fn() const {
    return m_html_start_span_cb;
  }
  ...
  text_art::theme *get_diagram_theme() const { return m_diagram_theme; }
...
public:
  ...
  bool emitting_diagnostic_p() const { return m_lock > 0; }

  bool emit_diagnostic_with_group(enum kind kind, rich_location &richloc,
                                  const metadata *metadata, option_id opt_id,
                                  const char *gmsgid, ...)
      ATTRIBUTE_GCC_DIAG(6, 7);
  bool emit_diagnostic_with_group_va(enum kind kind, rich_location &richloc,
                                     const metadata *metadata, option_id opt_id,
  ...
  enum kind classify_diagnostic(option_id opt_id, enum kind new_kind,
                                location_t where) {
    logging::log_function_params(m_logger,
    ...
    return m_option_classifier.classify_diagnostic(this, opt_id, new_kind,
                                                   where);
  }
  ...
  void set_path_format(enum diagnostic_path_format val) { m_path_format = val; }
  ...
  inline char *make_option_name(option_id opt_id, enum kind orig_diag_kind,
                                enum kind diag_kind) const {
    if (!m_option_id_mgr)
      return nullptr;
    return m_option_id_mgr->make_option_name(opt_id, orig_diag_kind, diag_kind);
  }
  ...
  void set_option_id_manager(std::unique_ptr<option_id_manager> option_id_mgr,
                             unsigned lang_mask);

  unsigned get_lang_mask() const { return m_lang_mask; }

  bool diagnostic_impl(rich_location *, const metadata *, option_id,
                       const char *, va_list *, enum kind)
      ATTRIBUTE_GCC_DIAG(5, 0);
  bool diagnostic_n_impl(rich_location *, const metadata *, option_id,
                         unsigned HOST_WIDE_INT, const char *, const char *,
                         va_list *, enum kind) ATTRIBUTE_GCC_DIAG(7, 0);
  ...
  int pch_save(FILE *f) { return m_option_classifier.pch_save(f); }

  int pch_restore(FILE *f) { return m_option_classifier.pch_restore(f); }

  void set_diagnostic_buffer(buffer *);
  buffer *get_diagnostic_buffer() const { return m_diagnostic_buffer; }
  ...
  pretty_printer *get_reference_printer() const { return m_reference_printer; }
  ...
  void set_abort_on_error(bool val) { m_abort_on_error = val; }
  ...
  void set_fatal_errors(bool fatal_errors) { m_fatal_errors = fatal_errors; }

  void set_internal_error_callback(void (*cb)(context *, const char *,
                                              va_list *)) {
    m_internal_error = cb;
  }

  void set_adjust_diagnostic_info_callback(void (*cb)(const context &,
                                                      diagnostic_info *)) {
    m_adjust_diagnostic_info = cb;
  }

  void inhibit_notes() { m_inhibit_notes_p = true; }
...
inline text_starter_fn &text_starter(context *dc) {
...
/* Client supplied function called between disjoint spans of source code,
   ...
   something to indicate that a new span of source code has begun.  */
inline start_span_fn<to_text> &start_span(context *dc) {
  return dc->m_text_callbacks.m_text_start_span;
}

/* Client supplied function called after a diagnostic message is
   displayed (for text-based diagnostic output).  */
inline text_finalizer_fn &text_finalizer(context *dc) {
