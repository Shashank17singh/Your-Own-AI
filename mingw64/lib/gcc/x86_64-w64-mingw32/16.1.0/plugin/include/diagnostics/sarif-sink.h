...
extern output_file
open_sarif_output_file(context &dc, line_maps *line_maps,
                       const char *base_file_name,
                       enum sarif_serialization_kind serialization_kind);

extern sink &init_sarif_stderr(context &dc, const line_maps *line_maps,
                               bool formatted);
extern sink &init_sarif_file(context &dc, line_maps *line_maps, bool formatted,
                             const char *base_file_name);
extern sink &init_sarif_stream(context &dc, const line_maps *line_maps,
                               bool formatted, FILE *stream);
...
public:
  sarif_serialization_format_json(bool formatted) : m_formatted(formatted) {}
...
extern std::unique_ptr<sink> make_sarif_sink(
    context &dc, const line_maps &line_maps,
    std::unique_ptr<sarif_serialization_format> serialization_format,
    const sarif_generation_options &sarif_gen_opts, output_file output_file_);
...
public:
  void set_logical_location(const char *property_name, sarif_builder &,
                            logical_locations::key logical_loc);
  void set_graph(const char *property_name, sarif_builder &,
...
make_sarif_graph(const digraphs::digraph &g, sarif_builder *builder,
                 sarif_location_manager *sarif_location_mgr);

extern std::unique_ptr<sarif_node>
make_sarif_node(const digraphs::node &n, sarif_builder *builder,
                sarif_location_manager *sarif_location_mgr);

extern std::unique_ptr<sarif_edge> make_sarif_edge(const digraphs::edge &e,
                                                   sarif_builder *builder);

extern void maybe_open_sarif_sink_for_socket(context &ctxt);
