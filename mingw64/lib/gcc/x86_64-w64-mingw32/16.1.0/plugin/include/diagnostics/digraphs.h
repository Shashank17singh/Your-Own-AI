...
public:
  ...
  /* Integer properties.  */
  bool maybe_get_property(const json::integer_property &property,
                          long &out) const;
  ...
  template <typename EnumType>
  void set_property(const json::enum_property<EnumType> &property,
                    EnumType value) {
  ...
  json::object *get_property_bag() const { return m_property_bag.get(); }
...
public:
  ...
  node &get_node(size_t idx) const { return *m_nodes[idx].get(); }

  size_t get_num_edges() const { return m_edges.size(); }

  edge &get_edge(size_t idx) const { return *m_edges[idx].get(); }
  ...
  void add_edge(const char *id, node &src_node, node &dst_node,
...
public:
  ...
  node(digraph &g, std::string id)
      : m_id(id), m_physical_loc(UNKNOWN_LOCATION) {
  ...
  node &get_child(size_t idx) const { return *m_children[idx].get(); }
  ...
  location_t get_physical_loc() const { return m_physical_loc; }

  void set_physical_loc(location_t physical_loc) {
    m_physical_loc = physical_loc;
  }

  logical_locations::key get_logical_loc() const { return m_logical_loc; }
  ...
  std::unique_ptr<node> clone(digraph &new_graph,
...
public:
  ...
  edge(digraph &g, const char *id, node &src_node, node &dst_node)
      : m_id(g.make_edge_id(id)), m_src_node(src_node), m_dst_node(dst_node) {
  ...
  node &get_src_node() const { return m_src_node; }

  node &get_dst_node() const { return m_dst_node; }
  ...
  clone(digraph &new_graph,
        const std::map<diagnostics::digraphs::node *,
                       diagnostics::digraphs::node *> &node_mapping) const;
