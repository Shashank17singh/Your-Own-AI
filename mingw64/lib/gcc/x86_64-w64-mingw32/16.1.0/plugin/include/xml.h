...
struct node {
  virtual ~node() {}
  virtual void write_as_xml(pretty_printer *pp, int depth,
                            bool indent) const = 0;
  virtual text *dyn_cast_text() { return nullptr; }
  virtual element *dyn_cast_element() { return nullptr; }
  void dump(FILE *out) const;
  void DEBUG_FUNCTION dump() const { dump(stderr); }
};

struct text : public node {
  text(std::string str) : m_str(std::move(str)) {}

  void write_as_xml(pretty_printer *pp, int depth,
                    bool indent) const final override;

  text *dyn_cast_text() final override { return this; }
...
struct element : public node_with_children {
  element(std::string kind, bool preserve_whitespace)
      : m_kind(std::move(kind)), m_preserve_whitespace(preserve_whitespace) {}

  element *dyn_cast_element() final override { return this; }
...
struct comment : public node {
  comment(std::string text) : m_text(std::move(text)) {}
...
struct raw : public node {
  raw(std::string xml_src) : m_xml_src(xml_src) {}
