...
public:
  ...
  /* Construct a sreal.  */
  sreal(int64_t sig, int exp = 0) { normalize(sig, exp); }
...
inline sreal &operator+=(sreal &a, const sreal &b) { return a = a + b; }
...
inline bool operator!=(const sreal &a, const sreal &b) { return !(a == b); }
...
inline sreal operator<<(const sreal &a, int exp) { return a.shift(exp); }
