...
struct auto_end_imm_use_fast_traverse {
  ...
  auto_end_imm_use_fast_traverse(imm_use_iterator *imm) : imm(imm) {}
...
#define FOR_EACH_IMM_USE_FAST(DEST, ITER, SSAVAR)                              \
  for (struct auto_end_imm_use_fast_traverse auto_end_imm_use_fast_traverse(   \
           (((DEST) = first_readonly_imm_use(&(ITER), (SSAVAR))), &(ITER)));   \
       !end_readonly_imm_use_p(&(ITER));                                       \
       (void)((DEST) = next_readonly_imm_use(&(ITER))))
#endif
...
struct auto_end_imm_use_stmt_traverse {
  imm_use_iterator *imm;
  auto_end_imm_use_stmt_traverse(imm_use_iterator *imm) : imm(imm) {}
  ~auto_end_imm_use_stmt_traverse() { end_imm_use_stmt_traverse(imm); }
};
...
#define FOR_EACH_IMM_USE_STMT(STMT, ITER, SSAVAR)                              \
  for (struct auto_end_imm_use_stmt_traverse auto_end_imm_use_stmt_traverse(   \
           (((STMT) = first_imm_use_stmt(&(ITER), (SSAVAR))), &(ITER)));       \
...
inline void delink_imm_use(ssa_use_operand_t *linknode) {
  ...
  if (linknode->loc.stmt
    ...
    gcc_assert(var->ssa_name.fast_iteration_depth == 0 &&
               (var->ssa_name.active_iterated_stmt == NULL ||
                (var->ssa_name.active_iterated_stmt == linknode->loc.stmt)));
  }
#endif
...
inline void link_imm_use(ssa_use_operand_t *linknode, tree def) {
...
inline void set_ssa_use_from_ptr(use_operand_p use, tree val) {
  delink_imm_use(use);
  *(use->use) = val;
...
/* Link ssa_imm_use node LINKNODE into the chain for DEF, with use occurring
   in STMT.  */
inline void link_imm_use_stmt(ssa_use_operand_t *linknode, tree def,
                              gimple *stmt) {
  if (stmt)
...
inline void relink_imm_use(ssa_use_operand_t *node, ssa_use_operand_t *old) {
...
/* Relink ssa_imm_use node LINKNODE into the chain for OLD, with use occurring
   in STMT.  */
inline void relink_imm_use_stmt(ssa_use_operand_t *linknode,
...
inline bool end_readonly_imm_use_p(const imm_use_iterator *imm) {
  return (imm->imm_use == imm->end_p);
}

/* Initialize iterator IMM to process the list for VAR.  */
inline use_operand_p first_readonly_imm_use(imm_use_iterator *imm, tree var) {
#if defined ENABLE_GIMPLE_CHECKING
...
inline use_operand_p next_readonly_imm_use(imm_use_iterator *imm) {
...
inline bool has_zero_uses(const_tree var) {
...
inline bool has_single_use(const_tree var) {
  const ssa_use_operand_t *const head = &(SSA_NAME_IMM_USE_NODE(var));
  const ssa_use_operand_t *ptr;
  bool single = false;
...
inline bool single_imm_use(const_tree var, use_operand_p *use_p,
                           gimple **stmt) {
...
inline unsigned int num_imm_uses(const_tree var) {
...
inline bool op_iter_done(const ssa_op_iter *ptr) { return ptr->done; }

/* Get the next iterator use value for PTR.  */
inline use_operand_p op_iter_next_use(ssa_op_iter *ptr) {
  use_operand_p use_p;
...
inline def_operand_p op_iter_next_def(ssa_op_iter *ptr) {
...
inline tree op_iter_next_tree(ssa_op_iter *ptr) {
  tree val;
...
inline void clear_and_done_ssa_iter(ssa_op_iter *ptr) {
  ptr->i = 0;
  ptr->numops = 0;
  ptr->uses = NULL;
...
inline void op_iter_init(ssa_op_iter *ptr, gimple *stmt, int flags) {
...
/* Initialize iterator PTR to the use operands in STMT based on FLAGS. Return
   the first use.  */
inline use_operand_p op_iter_init_use(ssa_op_iter *ptr, gimple *stmt,
                                      int flags) {
...
/* Initialize iterator PTR to the def operands in STMT based on FLAGS. Return
   the first def.  */
inline def_operand_p op_iter_init_def(ssa_op_iter *ptr, gimple *stmt,
                                      int flags) {
...
inline tree op_iter_init_tree(ssa_op_iter *ptr, gimple *stmt, int flags) {
  op_iter_init(ptr, stmt, flags);
...
/* If there is a single operand in STMT matching FLAGS, return it.  Otherwise
   return NULL.  */
inline tree single_ssa_tree_operand(gimple *stmt, int flags) {
  tree var;
  ssa_op_iter iter;
...
/* If there is a single operand in STMT matching FLAGS, return it.  Otherwise
   return NULL.  */
inline use_operand_p single_ssa_use_operand(gimple *stmt, int flags) {
  use_operand_p var;
  ssa_op_iter iter;
...
/* Return the single virtual use operand in STMT if present.  Otherwise
   return NULL.  */
inline use_operand_p ssa_vuse_operand(gimple *stmt) {
  if (!gimple_vuse(stmt))
    return NULL_USE_OPERAND_P;
...
/* If there is a single operand in STMT matching FLAGS, return it.  Otherwise
   return NULL.  */
inline def_operand_p single_ssa_def_operand(gimple *stmt, int flags) {
  def_operand_p var;
  ssa_op_iter iter;
...
/* Return true if there are zero operands in STMT matching the type
   given in FLAGS.  */
inline bool zero_ssa_operands(gimple *stmt, int flags) {
  ssa_op_iter iter;
...
inline int num_ssa_operands(gimple *stmt, int flags) {
  ssa_op_iter iter;
  tree t;
  int num = 0;
...
inline tree single_phi_def(gphi *stmt, int flags) {
...
inline use_operand_p op_iter_init_phiuse(ssa_op_iter *ptr, gphi *phi,
                                         int flags) {
...
inline def_operand_p op_iter_init_phidef(ssa_op_iter *ptr, gphi *phi,
                                         int flags) {
  tree phi_def = gimple_phi_result(phi);
  int comp;
...
inline bool end_imm_use_stmt_p(const imm_use_iterator *imm) {
  return (imm->imm_use == imm->end_p);
}
...
inline void end_imm_use_stmt_traverse(imm_use_iterator *ARG_UNUSED(imm)) {
#if defined ENABLE_GIMPLE_CHECKING
...
inline use_operand_p move_use_after_head(use_operand_p use_p,
                                         use_operand_p head,
...
inline use_operand_p link_use_stmts_after(use_operand_p head,
                                          imm_use_iterator *) {
  use_operand_p use_p;
...
inline gimple *first_imm_use_stmt(imm_use_iterator *imm, tree var) {
#if defined ENABLE_GIMPLE_CHECKING
...
inline gimple *next_imm_use_stmt(imm_use_iterator *imm) {
...
inline use_operand_p first_imm_use_on_stmt(imm_use_iterator *imm) {
...
inline bool end_imm_use_on_stmt_p(const imm_use_iterator *imm) {
  return (imm->imm_use == imm->next_stmt_use);
}

/* Bump to the next use on the stmt IMM refers to, return NULL if done.  */

inline use_operand_p next_imm_use_on_stmt(imm_use_iterator *imm) {
...
inline void delink_stmt_imm_use(gimple *stmt) {
  ssa_op_iter iter;
  use_operand_p use_p;
