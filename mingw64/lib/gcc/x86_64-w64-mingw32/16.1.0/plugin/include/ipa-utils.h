...
bool stmt_may_terminate_function_p(function *fun, gimple *stmt,
...
void dump_possible_polymorphic_call_targets(
    FILE *, tree, HOST_WIDE_INT, const ipa_polymorphic_call_context &,
    bool verbose = true);
...
bool vtable_pointer_value_to_vtable(const_tree, tree *,
...
possible_polymorphic_call_targets(struct cgraph_edge *e, bool *completep = NULL,
  ...
  return possible_polymorphic_call_targets(pii->otr_type, pii->otr_token,
                                           context, completep, cache_token,
                                           speculative);
}
...
inline void dump_possible_polymorphic_call_targets(FILE *f,
  ...
  if (!pii->usable_p())
    return;
  dump_possible_polymorphic_call_targets(f, pii->otr_type, pii->otr_token,
                                         context, verbose);
}
...
inline bool possible_polymorphic_call_target_p(struct cgraph_edge *e,
  ...
  if (!pii->usable_p())
    return true;
  return possible_polymorphic_call_target_p(pii->otr_type, pii->otr_token,
                                            context, n);
}
...
inline const char *get_odr_name_for_type(tree type) {
  tree type_name = TYPE_NAME(type);
  if (type_name == NULL_TREE || TREE_CODE(type_name) != TYPE_DECL ||
...
inline bool lto_streaming_expected_p() {
