...
#define _tile_dpbf8ps_internal(dst, src1, src2)                                \
  __asm__ volatile(                                                            \
    ...
    |tdpbf8ps\ttmm%c[_dst], tmm%c[_src1], tmm%c[_src2]}" ::[_dst] "i"(dst),    \
      [_src1] "i"(src1), [_src2] "i"(src2))

#define _tile_dpbhf8ps_internal(dst, src1, src2)                               \
  __asm__ volatile(                                                            \
      "{tdpbhf8ps\t%%tmm%c[_src2], %%tmm%c[_src1], %%tmm%c[_dst]		\
    |tdpbhf8ps\ttmm%c[_dst], tmm%c[_src1], tmm%c[_src2]}" ::[_dst] "i"(dst),   \
      [_src1] "i"(src1), [_src2] "i"(src2))

#define _tile_dphbf8ps_internal(dst, src1, src2)                               \
  __asm__ volatile(                                                            \
      "{tdphbf8ps\t%%tmm%c[_src2], %%tmm%c[_src1], %%tmm%c[_dst]		\
    |tdphbf8ps\ttmm%c[_dst], tmm%c[_src1], tmm%c[_src2]}" ::[_dst] "i"(dst),   \
      [_src1] "i"(src1), [_src2] "i"(src2))

#define _tile_dphf8ps_internal(dst, src1, src2)                                \
  __asm__ volatile(                                                            \
      "{tdphf8ps\t%%tmm%c[_src2], %%tmm%c[_src1], %%tmm%c[_dst]		\
    |tdphf8ps\ttmm%c[_dst], tmm%c[_src1], tmm%c[_src2]}" ::[_dst] "i"(dst),    \
      [_src1] "i"(src1), [_src2] "i"(src2))

#define _tile_dpbf8ps(dst, src1, src2) _tile_dpbf8ps_internal(dst, src1, src2)

#define _tile_dpbhf8ps(dst, src1, src2) _tile_dpbhf8ps_internal(dst, src1, src2)

#define _tile_dphbf8ps(dst, src1, src2) _tile_dphbf8ps_internal(dst, src1, src2)

#define _tile_dphf8ps(dst, src1, src2) _tile_dphf8ps_internal(dst, src1, src2)
